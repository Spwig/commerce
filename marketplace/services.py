...
from component_updates.models import UpdateServerConfig, ComponentRegistry
...
class MarketplaceService:
    ...
    def _api_get(self, path, params=None, timeout=15):
        ...
        try:
            ...
            if response.status_code == 401:
                logger.warning(
                    f"Marketplace API auth rejected (GET {path}). "
                    f"Ensure the license is activated."
                )
                return None
    ...
    def _api_post(self, path, data=None, timeout=15):
        ...
        try:
            ...
            if response.status_code == 401:
                logger.warning(
                    f"Marketplace API auth rejected (POST {path}). "
                    f"Ensure the license is activated."
                )
                return None
            response.raise_for_status()
    ...
    def browse(self, filters=None):
        ...
        if not self._get_jwt_token():
            return {
                'results': [], 'count': 0, 'page': 1, 'pages': 0,
                'auth_required': True,
            }
        ...
        if data:
            ...
            installed_slugs = set(
                ComponentRegistry.objects.values_list('slug', flat=True)
            )
    ...
    def get_reviews(self, slug, page=1):
        ...
        return self._api_get(
            ...
            params={'page': page}
        ) or {'results': [], 'count': 0, 'page': 1, 'pages': 0}
    ...
    def install_free_component(self, slug, version=None):
        ...
        try:
            ...
            cache.delete_pattern(f"{CACHE_PREFIX}:browse:*") if hasattr(cache, 'delete_pattern') else None
