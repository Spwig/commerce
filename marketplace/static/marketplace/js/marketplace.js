...
(function() {
    ...
    function fetchComponents() {
        ...
        .catch(err => {
            ...
            grid.innerHTML = '<div class="marketplace-loading"><i class="fas fa-exclamation-triangle"></i><p>' + escapeHtml(STR.failedToLoad || 'Failed to load components') + '</p></div>';
        });
    }
    ...
    function createCard(comp) {
        ...
        const compName = typeof ManifestI18n !== 'undefined'
            ? ManifestI18n.translate(comp.translations, 'meta.name', comp.name)
            : comp.name;
        const compDesc = typeof ManifestI18n !== 'undefined'
        ...
        // Author
        const verifiedBadge = comp.author && comp.author.is_verified ? ' <i class="fas fa-check-circle marketplace-verified"></i>' : '';
        ...
        if (comp.has_country_scope) {
            ...
            if (countries.length === 0) {
                countryHtml = '<div class="marketplace-card-countries"><i class="fas fa-globe"></i> ' + escapeHtml(STR.global || 'Global') + '</div>';
            } else {
    ...
    function openModal(slug) {
        ...
        content.innerHTML = '<div class="marketplace-loading"><i class="fas fa-spinner fa-spin"></i></div>';
        ...
        .catch(() => {
            content.innerHTML = '<p class="marketplace-modal-error">' + escapeHtml(STR.failedToLoadDetails || 'Failed to load details') + '</p>';
        });
    }
        ...
        .then(data => {
            if (data.success) {
                ...
                if (btnInstall) {
                    ...
                    btnInstall.innerHTML = '<i class="fas fa-download"></i> ' + escapeHtml(STR.install || 'Install');
                }
            }
        })
        .catch(() => {
    ...
    function initDetailPage() {
        ...
        document.querySelectorAll('.marketplace-tab-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.querySelectorAll('.marketplace-tab-content').forEach(c => c.classList.remove('active'));
        ...
        document.querySelectorAll('.marketplace-carousel-thumb').forEach(thumb => {
            thumb.addEventListener('click', () => {
                document.querySelectorAll('.marketplace-carousel-thumb').forEach(t => t.classList.remove('active'));
        ...
        if (CONFIG.reviewsSummary && CONFIG.ratingCount) {
            for (let star = 1; star <= 5; star++) {
                ...
                const countEl = document.querySelector(`.marketplace-rating-bar-count[data-star="${star}"]`);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (toggle && list) {
            toggle.addEventListener('click', function() {
                var expanded = !list.classList.contains('marketplace-hidden');
