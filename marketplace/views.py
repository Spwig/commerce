...
class DetailView(View):
    ...
    def get(self, request, slug):
        ...
        component["reviews_summary_json"] = json.dumps(component.get("reviews_summary") or {})
...
def install_ajax(request):
    ...
    if result and result.get("success"):
        ...
        error = (
            result.get("error", _("Installation failed")) if result else _("Installation failed")
        )
...
def review_ajax(request):
    ...
    if result and result.get("success"):
        ...
        error = (
            result.get("error", _("Failed to submit review"))
            if result
            else _("Failed to submit review")
        )
        return JsonResponse({"success": False, "error": error}, status=400)
