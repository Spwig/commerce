...
from core.api.throttling import VoucherValidationThrottle, AuthenticatedUserThrottle
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_("Get all voucher codes. Customers see only active vouchers. Staff see all vouchers with filtering by status and discount type. Supports search by code or name.")
    ),
    create=extend_schema(
        tags=['Vouchers'],
        summary=_("Create new voucher"),
        description=_("Create a new voucher code (admin only). Specify discount type (percentage, fixed, gift_card), value, validity dates, usage limits, and product/category restrictions.")
    ),
    retrieve=extend_schema(
        tags=['Vouchers'],
        summary=_("Get voucher details"),
        description=_("Get detailed information about a specific voucher including discount rules, usage limits, restrictions, and eligible products/categories.")
    ),
    update=extend_schema(
        tags=['Vouchers'],
        summary=_("Update voucher"),
        description=_("Update voucher configuration (admin only). Can modify discount value, dates, usage limits, and restrictions. Active vouchers should be updated carefully.")
    ),
    partial_update=extend_schema(
        tags=['Vouchers'],
        summary=_("Partially update voucher"),
        description=_("Update specific voucher fields (admin only). Useful for extending expiry dates or adjusting usage limits without resending full voucher data.")
    ),
    destroy=extend_schema(
        tags=['Vouchers'],
        summary=_("Delete voucher"),
        description=_("Delete a voucher code (admin only). Warning: This permanently removes the voucher and all usage history. Consider deactivating instead.")
    ),
    validate_code=extend_schema(
        tags=['Vouchers'],
        summary=_("Validate voucher code"),
        description=_("Validate a voucher code and check if it can be used by the authenticated user. Requires authentication to prevent enumeration attacks. Rate limited to 10 requests/minute. Returns discount amount if cart total provided.")
    ),
    check_eligibility=extend_schema(
        tags=['Vouchers'],
        summary=_("Check customer eligibility"),
        description=_("Check if the authenticated user is eligible to use a specific voucher. Validates usage limits, customer restrictions, and account requirements. Rate limited to 10 requests/minute.")
    ),
    calculate_discount=extend_schema(
        tags=['Vouchers'],
        summary=_("Calculate discount amount"),
        description=_("Calculate the discount amount for a given cart total. Applies percentage or fixed discount rules, minimum purchase requirements, and maximum discount caps.")
    ),
    usage_stats=extend_schema(
        tags=['Vouchers'],
        summary=_("Get voucher usage statistics"),
        description=_("Get detailed usage statistics for a voucher including total uses, unique customers, total discount given, average discount per use, and recent usage history.")
    ),
    bulk_generate=extend_schema(
        tags=['Vouchers'],
        summary=_("Bulk generate voucher codes"),
        description=_("Bulk generate multiple voucher codes (admin only). Maximum 1000 codes per request. All generated codes share the same discount rules and restrictions but have unique codes.")
    ),
)
class VoucherCodeViewSet(HeadlessAPIMixin, viewsets.ModelViewSet):
    ...
    def validate_code(self, request):
        ...
        try:
            ...
            if not voucher.is_valid:
                return Response({
                    'valid': False,
                    'message': 'This voucher is not currently valid'
    ...
    def usage_stats(self, request, pk=None):
        ...
        stats = {
            ...
            'total_discount_given': usage.aggregate(
                total=Sum('discount_amount')
            )['total'] or Money(0, get_default_currency()),
            'average_discount': usage.aggregate(
                avg=Sum('discount_amount')
            )['avg'] or Money(0, get_default_currency()),
            'uses_remaining': voucher.uses_remaining,
            'recent_uses': VoucherUsageSerializer(
                usage.order_by('-used_at')[:10],
                many=True
            ).data
        }
    ...
    def bulk_generate(self, request):
        ...
        try:
            ...
            return Response(e.args[0] if e.args else {'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_("Get voucher usage history for the authenticated user. Staff can view all usage. Filter by voucher ID or user ID. Shows when voucher was used, discount applied, and associated order.")
    ),
    retrieve=extend_schema(
        tags=['Vouchers'],
        summary=_("Get usage details"),
        description=_("Get detailed information about a specific voucher usage including timestamp, user, order, discount amount applied, and voucher code used.")
    ),
)
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_("Get gift cards for the authenticated user. Shows gift cards purchased by user or received by email. Staff can view all gift cards. Filter by status (active, partially_used, fully_used, expired).")
    ),
    create=extend_schema(
        tags=['Vouchers'],
        summary=_("Create gift card"),
        description=_("Create a new gift card (admin only). Specify original value, recipient email, optional message, and expiry date. Automatically generates unique gift card code.")
    ),
    retrieve=extend_schema(
        tags=['Vouchers'],
        summary=_("Get gift card details"),
        description=_("Get detailed information about a specific gift card including original value, current balance, recipient, status, and usage history.")
    ),
    update=extend_schema(
        tags=['Vouchers'],
        summary=_("Update gift card"),
        description=_("Update gift card information (admin only). Can modify balance, status, or expiry date. Use carefully as this affects customer balances.")
    ),
    partial_update=extend_schema(
        tags=['Vouchers'],
        summary=_("Partially update gift card"),
        description=_("Update specific gift card fields (admin only). Useful for extending expiry date or adjusting status without sending full gift card data.")
    ),
    destroy=extend_schema(
        tags=['Vouchers'],
        summary=_("Delete gift card"),
        description=_("Delete a gift card (admin only). Warning: This permanently removes the gift card and remaining balance. Ensure customer has been refunded if applicable.")
    ),
    check_balance=extend_schema(
        tags=['Vouchers'],
        summary=_("Check gift card balance"),
        description=_("Check the current balance of a gift card by code. Returns original value, remaining balance, currency, and status. Does not require owning the gift card.")
    ),
    redeem=extend_schema(
        tags=['Vouchers'],
        summary=_("Redeem gift card amount"),
        description=_("Redeem a specific amount from a gift card. Validates sufficient balance and updates remaining balance. Returns new balance after redemption.")
    ),
)
class GiftCardViewSet(HeadlessAPIMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_staff:
            queryset = queryset.filter(
                Q(purchased_by=self.request.user) |
    ...
    def check_balance(self, request):
        ...
        try:
            ...
            return Response({
                ...
                'currency': gift_card.balance.currency.code if gift_card.balance else get_default_currency(),
                'original_value': str(gift_card.original_value.amount) if gift_card.original_value else "0.00",
            ...
            return Response({
                'error': 'This voucher is not a gift card'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if gift_card.redeem_amount(amount_decimal):
                return Response({
                    ...
                    'remaining_balance': str(gift_card.balance.amount) if gift_card.balance else "0.00"
                })
            else:
                return Response({
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_("Get all voucher restrictions (admin only). Filter by voucher ID to see restrictions for a specific voucher. Includes minimum purchase, customer group, and usage limit rules.")
    ),
    create=extend_schema(
        tags=['Vouchers'],
        summary=_("Add voucher restriction"),
        description=_("Add a new restriction to a voucher (admin only). Can restrict by minimum purchase amount, customer group, product category, or usage limits per customer.")
    ),
    retrieve=extend_schema(
        tags=['Vouchers'],
        summary=_("Get restriction details"),
        description=_("Get detailed information about a specific voucher restriction including restriction type, conditions, and affected voucher.")
    ),
    update=extend_schema(
        tags=['Vouchers'],
        summary=_("Update restriction"),
        description=_("Update a voucher restriction (admin only). Can modify restriction conditions, thresholds, and applicability rules.")
    ),
    partial_update=extend_schema(
        tags=['Vouchers'],
        summary=_("Partially update restriction"),
        description=_("Update specific restriction fields (admin only). Useful for adjusting thresholds or conditions without resending full restriction data.")
    ),
    destroy=extend_schema(
        tags=['Vouchers'],
        summary=_("Remove restriction"),
        description=_("Remove a restriction from a voucher (admin only). This makes the voucher less restrictive and may increase its applicability.")
    ),
)
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_("Get all vouchers currently applied to the authenticated user's cart. Shows voucher codes, discount amounts, and application status.")
    ),
    create=extend_schema(
        tags=['Vouchers'],
        summary=_("Apply voucher to cart"),
        description=_("Apply a voucher to the user's cart. Validates voucher eligibility, calculates discount, and adds to cart. Returns error if voucher is invalid or already applied.")
    ),
    retrieve=extend_schema(
        tags=['Vouchers'],
        summary=_("Get applied voucher details"),
        description=_("Get detailed information about a specific voucher applied to cart including discount amount, voucher code, and application timestamp.")
    ),
    update=extend_schema(
        tags=['Vouchers'],
        summary=_("Update applied voucher"),
        description=_("Update an applied voucher in the cart. Can refresh discount calculation if cart contents changed.")
    ),
    partial_update=extend_schema(
        tags=['Vouchers'],
        summary=_("Partially update applied voucher"),
        description=_("Update specific fields of an applied voucher. Useful for recalculating discounts without full voucher reapplication.")
    ),
    destroy=extend_schema(
        tags=['Vouchers'],
        summary=_("Remove voucher from cart"),
        description=_("Remove a voucher from the cart. Discount is recalculated without this voucher. User can reapply the same voucher later if still eligible.")
    ),
    apply_to_cart=extend_schema(
        tags=['Vouchers'],
        summary=_("Apply voucher code to cart"),
        description=_("Apply a voucher to cart by entering voucher code. Validates code, checks eligibility, prevents duplicate application, and calculates discount. Returns applied voucher details or error message.")
    ),
)
