...
class VoucherCode(models.Model):
    ...
    max_uses_per_customer = models.PositiveIntegerField(
        null=True, 
        blank=True,
        help_text=_("Max uses per customer (0 = unlimited)")
    )
    ...
    exclude_sale_items = models.BooleanField(
        default=False,
        help_text=_("Cannot be used on items already on sale")
    )
    
    cannot_combine_with_other_vouchers = models.BooleanField(
        default=False,
        help_text=_("Cannot be used with other vouchers")
    )
    
    cannot_combine_with_sale_items = models.BooleanField(
        default=False,
        help_text=_("Cannot be used if cart contains sale items")
    )
    
    # Customer restrictions
    first_time_customers_only = models.BooleanField(
        default=False,
        help_text=_("Only for customers with no previous orders")
    )
    ...
    def can_be_used_by_customer(self, user, order_total=None, context=None):
        ...
        if self.first_time_customers_only:
            if user.is_authenticated and hasattr(user, 'orders') and user.orders.filter(status='delivered').exists():
...
class VoucherRestriction(models.Model):
    ...
    is_inclusive = models.BooleanField(
        default=True,
        help_text=_("True = must match, False = must not match")
    )
    ...
    def evaluate(self, context):
        ...
        """
        try:
            values = json.loads(self.restriction_value) if self.restriction_value.startswith('[') else [
                v.strip() for v in self.restriction_value.split(',')
            ]
...
class AppliedVoucher(models.Model):
    ...
    cart = models.ForeignKey(
        'cart.Cart', on_delete=models.CASCADE,
    ...
    def __str__(self):
        target = f"cart" if self.cart_id else f"order" if self.order_id else "unknown"
