...
La vendita di carte regalo è attiva. Quando un cliente ne acquista una, la carta viene creata e inviata via email automaticamente una volta che il pagamento è stato processato — mai prima, quindi nessuno riceve un codice per un pagamento che in seguito fallisce.

Qualche cosa da sapere prima di abilitare un prodotto per carte regalo:

- **Una carta regalo è denaro, non uno sconto.** Viene detratta dall'importo finale dopo le tasse e le spese di spedizione, e non riduce l'importo delle tasse che devi pagare. Questo è l'opposto di un buono, che riduce il prezzo dei prodotti.
...
- **Le carte regalo non possono essere scontate.** Un buono non si applicherà a una riga di carta regalo, perché vendere 100 sterline di credito per 80 sterline ti fa perdere 20 sterline ogni volta.
...
Ogni carta regalo — che verrà venduta in seguito o emessa manualmente oggi — ha bisogno di un prodotto di tipo Carta Regalo dietro.
...
Il prodotto è anche ciò che selezioni quando emetti una carta manualmente — quindi è utile crearne uno anche se hai intenzione di darle via solo.
...
1. Vai a **Prodotti > Carte Regalo** e clicca su **+ Aggiungi Carta Regalo**
...
4. Imposta opzionalmente una data **Scade il** e lascia **Attiva** selezionata in modo che la carta possa essere riscattata
5. Compila la sezione **Destinatario**, più in basso sulla stessa pagina:
   ...
   - **Invia a Data Pianificata** — opzionale; lascia vuoto e invia quando sei pronto, o imposta una data/ora futura (es. un compleanno)
6. Clicca su **Salva**
...
**Salvare la carta non la invia via email.** Per consegnarla, torna nell'elenco delle carte regalo, seleziona la casella della carta, scegli **Invia email delle carte regalo** dal menu a discesa Azioni e fai clic su **Vai**.
...
- **Totale saldo** — Saldo rimanente complessivo di tutte le carte
...
- **Stato** — Attiva, Inattiva, Scaduta, Riscattata completamente o Parzialmente utilizzata
...
- Seleziona una o più carte e usa il menu a discesa **Azioni** per **Invia email delle carte regalo** (consegna o riesegue l'invio dell'email) o **Marca le carte selezionate come inattive** (deattiva — il saldo viene conservato ma la carta non può più essere riscattata)
...
2. Il codice viene validato — attivo, non scaduto, con saldo e nella stessa valuta della vendita
3. Il saldo viene applicato all'importo totale dovuto, incluso l'IVA e la consegna
4. Se il saldo non copre l'intera vendita, il cliente paga il resto in un altro modo
...
Quando si rimborsano ordini o vendite che hanno utilizzato una carta regalo:
...
- **Una carta regalo utilizzata per pagare l'ordine che viene rimborsato** — il rimborso va prima sulla carta, prima di qualsiasi pagamento con carta o banca. Rimborsare denaro a una banca da cui il commerciante non ha mai effettivamente ricevuto è un errore peggiore, e restituire il valore dove è arrivato chiude anche una via nota di frode. Se la carta originale è scaduta o è stata disattivata, viene emessa una carta sostitutiva allo stesso destinatario senza data di scadenza.
...
- Utilizza il tipo di denominazione "Both" per offrire comodità (importi preimpostati) e flessibilità (un importo personalizzato).
- Monitora regolarmente il metrica Totale Saldo — rappresenta un'obbligazione pendente nei tuoi libri contabili.
- Una carta spende allo stesso modo online e in persona — al checkout web nella fase di pagamento, o al registratore di cassa.

L'e-mail di consegna include un link per controllare il saldo che i destinatari possono utilizzare in qualsiasi momento.
- Se vendi a clienti in diversi paesi, puoi emettere carte regalo in valute specifiche — consulta l'argomento di aiuto **Multi-Currency Gift Cards** per i dettagli.