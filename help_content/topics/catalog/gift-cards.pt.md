...
Cartões-presente são créditos de loja que os clientes podem comprar para alguém else — ou para si mesmos — entregues por e-mail como um código de resgate único. Você também pode emitir um cartão-presente diretamente do admin sem uma compra do cliente.

A venda de cartões-presente está ativa. Quando um cliente compra um, o cartão é criado e enviado por e-mail automaticamente uma vez que o pagamento for confirmado — nunca antes, para que ninguém receba um código para um pagamento que depois falhar.
...
- **Os cartões são em uma única moeda.** Um cartão comprado em euros só pode ser gasto em uma ordem em euros. Se você vender em várias moedas, crie um produto de cartão-presente separado para cada uma. Isso protege você de movimentos cambiais em um saldo que pode não ser gasto por um ano.
- **Cartões-presente não podem ser descontados.** Um cupom não se aplica a uma linha de cartão-presente, porque vender £100 de crédito por £80 perde £20 toda vez.
...
Publique o produto quando estiver pronto para vendê-lo. Os clientes podem comprá-lo diretamente no seu site de loja imediatamente, e o cartão é enviado por e-mail automaticamente uma vez que o pagamento for confirmado.

O produto também é o que você seleciona quando emite um cartão manualmente — então é útil criar um mesmo que você planeje dar cartões-presente apenas.

## Criando um Cartão-presente Manualmente

Este é o único jeito de criar um cartão-presente financiado no momento, e ele funciona totalmente hoje.
...
4. Defina opcionalmente uma data de **Validade** e deixe **Ativo** marcado para que o cartão possa ser resgatado
...
Note que o caixa recebe o código na etapa de **pagamento**, não ao montar o carrinho. Um cartão-presente é dinheiro que o cliente já entregou, então ele liquida a conta em vez de descontar os produtos.
...
- **Um cartão-presente usado para pagar o pedido sendo estornado** — o estorno volta primeiro para o cartão, antes de qualquer pagamento de cartão ou banco. Estornar dinheiro para um banco que o vendedor nunca realmente coletou é o pior erro, e devolver o valor onde ele veio também encerra uma rota conhecida de fraude. Se o cartão original expirou ou foi desativado, um novo cartão é emitido para o mesmo destinatário, sem data de expiração.
...
- Use a emissão manual para créditos de boa vontade, resoluções de atendimento ao cliente ou qualquer caso em que deseje conceder um crédito ao cliente sem uma compra no site.
