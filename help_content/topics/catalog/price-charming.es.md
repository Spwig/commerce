...
Spwig aplica las reglas de precios atractivos automáticamente en toda tu tienda, según la moneda, por lo que solo necesitas configurar cada regla una vez.

## Cómo funciona el precio atractivo

Cuando se calcula el precio de un producto (incluido después de promociones o descuentos), Spwig verifica si existe una regla activa de precios atractivos para esa moneda. Si es así, el precio se ajusta antes de mostrarse a los clientes. El ajuste se aplica a precios por encima del umbral mínimo que hayas elegido.
...
|---------------|---------|------------|
| **Atracción con terminación .99** | $20.50 → $19.99 | La mayoría de los productos de retail — el precio psicológico clásico |
...
| **Redondear al múltiplo más cercano de 5** | $23.00 → $25.00 | Retail y mercados con alto tráfico |
...
Utilice el campo **Umbral de precio mínimo** para omitir el ajuste de precios para artículos con precios muy bajos donde el ajuste parecería extraño. Por ejemplo, al establecer un umbral de `5.00`, los productos con un precio inferior a $5 se mostrarán con su precio calculado real sin aplicar el ajuste.

Deje el valor en `0` para aplicar el ajuste a todos los precios.

## Precios de venta

Por defecto, el ajuste se aplica tanto a precios regulares como a precios de venta. Si desea que los precios de venta muestren sus valores calculados exactos (útil para precios promocionales limitados en el tiempo donde las cifras exactas importan), desmarque **Aplicar a precios de venta**.

## Desactivar una regla

Para detener temporalmente el ajuste sin eliminar la regla, desmarque **Activo** y guarde. La regla se conserva y puede reactivarse en cualquier momento.

## Consejos

Conserva todo el formato markdown, las rutas de imágenes, los bloques de código y los términos técnicos.
...
- Verifica tus precios después de habilitar una nueva regla viendo un producto en el almacén — los precios encantados se muestran en tiempo real.
