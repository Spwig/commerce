...
Ouvrez **Paramètres → Achat par IA** et activez **Commerce agentique activé**. À partir de ce moment, les assistants qui prennent en charge le Protocole de Commerce Universel peuvent découvrir votre magasin et lire votre catalogue. Rien ne change concernant votre magasin normal.
...
- **« Les assistants IA peuvent acheter dans votre magasin »** — tout ce qui est nécessaire pour un achat est en place.
...
La liste de vérification reflète votre configuration **en temps réel** : connectez un fournisseur de paiement ou ajoutez une méthode d'expédition et le verdict se met à jour la prochaine fois que vous ouvrirez la page.
...
- **Lecture** (découverte et navigation) est à risque faible. Un assistant peut trouver votre magasin et lire les détails du produit.
...
Chaque produit a un paramètre **Visible pour les agents d'achat par IA** (activé par défaut). Désactivez-le pour garder un produit particulier caché aux assistants tout en le laissant visible sur votre magasin — pratique pour des articles que vous préférez vendre uniquement via votre propre site.

## Gérer les assistants individuels

Lorsqu'un assistant effectue son premier achat — ou tente de le faire — Spwig le note sous **Achat par IA → Identités des agents**. Chaque entrée affiche le lieu vérifié de l'assistant (le répertoire avec lequel il signe) et le nombre de requêtes qu'il a effectuées. Le nom et le logo présentés par l'assistant sont affichés uniquement comme des détails *revendiqués* — traitez-les comme une étiquette, pas comme une preuve d'identité ; le lieu vérifié est la partie sur laquelle vous pouvez vous fier.

Les nouveaux assistants commencent **limités** : ils peuvent effectuer des transactions, mais dans des limites. Pour en bloquer un, sélectionnez-le et choisissez **Bloquer les assistants sélectionnés** — les paiements en cours s'arrêtent et l'assistant ne peut plus acheter, tandis que tout paiement déjà effectué reste inchangé. **Débloquer les assistants sélectionnés** le ramène à l'état limité (jamais directement à l'état illimité — lever les limites est toujours une étape séparée et délibérée).

## Enregistrement des activités

**Achat par IA → Événements des agents** est un enregistrement à preuve de tampering de ce que les assistants ont fait — chaque demande vérifiée, chaque tentative bloquée, chaque modification que vous avez effectuée. Il est en lecture seule et ne peut être modifié ou supprimé, donc il sert de trace de preuve si un achat effectué par un assistant est jamais contesté.
