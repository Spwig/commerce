...
keywords:
  ...
  - advanced shipping
url_patterns:
  - /admin/shipping/shippingrule/
  - /admin/shipping/shippingrule/add/
  - /admin/shipping/shippingrule/\d+/change/
related:
  - shipping-methods
  - shipping-zones
  - sales-promotions
...
Use shipping rules when you need dynamic shipping costs that change based on order context, not just static rates from shipping methods.
...
Rules evaluate **ALL conditions must pass** for rule to apply:
...
4. **Set Stop Flag** (Optional)
   - Check "Stop Further Rules" if this rule should prevent lower-priority rules from executing
...
## Common Shipping Rule Scenarios
...
```
Rule A (Priority 100): 10% off for VIP → stop_further_rules=No
Rule B (Priority 50): 15% off orders >$100 → stop_further_rules=No

VIP customer with $120 order:
Base: $15
After Rule A: $13.50 (10% off)
After Rule B: $11.48 (15% off $13.50)
```
...
```
Rule A (Priority 100): Free shipping >$50 → stop_further_rules=Yes
Rule B (Priority 50): 20% off all shipping → stop_further_rules=Yes

Cart > $50:
Rule A applies → Free shipping → STOP
Rule B never executes
```
...
**Solution**: Use stop_further_rules=Yes on free shipping rule, verify conditions, check exclusions.
...
- **Set stop_further_rules for absolute rules** - Free shipping should stop further processing
- **Test rule combinations** - Multiple rules can interact unexpectedly
- **Use descriptive names** - "VIP 20% Discount (Priority 75)" better than "Rule 3"
...
- **Avoid excessive rules** - Too many rules slow checkout, use 5-10 max
