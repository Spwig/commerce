...
打开**设置 → AI购物**，并将**启用代理式商业**切换为开启状态。从那时起，支持通用商业协议的助手可以发现您的商店并读取您的商品目录。您的正常商店前端不会有任何变化。
...
- **"紧急停止已启用"** 或 **"代理式商业已关闭"** —— 没有任何内容被提供给助手。

在判断结果下方，您会看到一个简短的检查清单——已连接支付提供商、可以报价运输、产品对助手可见——在任何仍需关注的项目旁边会有一个提示。计数器显示助手可以销售的产品数量、您隐藏的产品数量、已访问的助手数量以及您已阻止的助手数量。

检查清单反映的是您的**实时**设置：连接一个支付提供商或添加一个运输方式，当您下次打开页面时，判断结果会更新。

## 紧急停止

**紧急停止**是一个与主开关分开的开关。使用它立即停止所有助手活动——例如，当您发现某些异常时——而无需更改您的配置。清除它以恢复。可以将主开关视为"此功能是否已配置开启"，而紧急停止则视为"立即停止所有操作"。
