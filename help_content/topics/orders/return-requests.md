...
title_i18n_key: Return Requests & Processing
...
url_patterns:
  ...
  - /admin/orders/returnrequest/\d+/change/
related:
  - manage-orders
published: true
---
...
Customers select a reason when submitting the return:

| Reason | When to Expect It |
|--------|------------------|
| **Defective Product** | Manufacturing defect or functional failure |
| **Wrong Item Received** | Fulfilment error — wrong product shipped |
| **Not as Described** | Product differs from listing description or images |
| **Changed Mind** | Customer no longer wants the item |
| **Wrong Size/Fit** | Item doesn't fit as expected |
| **Damaged in Shipping** | Item was damaged during transit |
| **Other** | Any reason not covered above |

**Use return reasons to:**
- Identify recurring issues with specific products
- Improve product descriptions to reduce "not as described" returns
- Track fulfilment accuracy to reduce "wrong item" returns

---

## Restocking Fees

Apply a fixed fee to offset return processing costs. The **Restocking Fee** field in the Inspection Results section accepts a money amount in your store's currency.
