...
Lors de la création d'un jeton, vous choisissez un type qui décrit son objectif. Ce type est à votre disposition et vous aide à garder une trace de ce que chaque jeton fait.
...
|-------|-------|:---:|-------------------|
| Analyse | **Analyse des ventes** | Seulement lecture | Tableaux de bord des ventes, KPI, analyse des produits/clients/catégories, comparaisons et exports |
| Analyse | **Analyse web** | Seulement lecture | Analyse des visiteurs et du trafic : aperçu, tendances, pages populaires, géographie et sources de trafic |
...
| Utilisateurs & Accès | **Personnel & Rôles** | Oui | Comptes de personnel, invitations, rôles et catalogue des permissions |

Les deux portées **Analyse** sont toujours en lecture seule — les données de reporting n'ont aucun concept de « écriture », donc le menu ne propose que **Aucun accès** ou **Lecture** pour elles.
...
Lorsqu'un jeton expire, les requêtes qui l'utilisent sont refusées. Vous pouvez prolonger l'accès en mettant à jour la date **Expire à** ou en créant un jeton de remplacement.
...
- **Dernière utilisation** — moment où le jeton a été utilisé pour faire une requête
...
Le jeton reste dans votre liste à titre de référence mais est refusé lors de toute requête ultérieure. Cela est utile lorsque vous devez temporairement suspendre une intégration tout en enquêtant sur un problème.
...
- **Accordez uniquement les portées nécessaires à l'intégration** — un outil de reporting n'a besoin que d'un accès en lecture aux Analyses des Ventes ou aux Analyses Web, et non en lecture et écriture sur les Produits ou les Employés & Rôles
...
- Souvenez-vous que l'accès en écriture dépend également du rôle du membre du personnel qui l'a créé — si une portée affiche Lecture & Écriture mais que les écritures échouent tout de même, vérifiez également les autorisations du rôle de cet utilisateur
...
- Revoyez votre liste de jetons toutes les quelques mois et désactivez tout jeton dont la date **Dernière Utilisation** est inattendument ancienne, car ceux-ci pourraient appartenir à des intégrations qui ne sont plus en cours d'exécution
