...
title_i18n_key: Email Configuration
...
Email configuration controls how your store sends transactional emails — order confirmations, shipping notifications, password resets, and more. Spwig includes a built-in SMTP server and supports external email providers for higher deliverability.
...
| Mode | Description |
|------|-------------|
| **Live** | Emails are delivered normally to real recipients |
| **Paused** | Emails are held in the queue and not sent until you switch back to Live |
| **Log Only** | Emails are recorded in the outbox but never delivered |

### Test Redirect Email

Set a **Test Redirect Email** address to intercept all outgoing emails and redirect them to a single address. When set, every email — regardless of the real recipient — goes to that address instead. This is useful for testing email templates without accidentally sending to real customers. Leave blank to send emails to actual recipients.

### Sandbox Email Whitelist

In sandbox or development mode, you can restrict email delivery to a whitelist of approved addresses. Only emails to addresses on the whitelist will be delivered. All other emails are logged but never sent. The admin email is always included automatically. You can add up to 10 addresses.
