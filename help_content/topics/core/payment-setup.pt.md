...
Fornecedores de pagamento conectam sua loja a gateways de pagamento, permitindo que você aceite cartões de crédito, carteiras digitais e outros métodos de pagamento no checkout. O Spwig suporta múltiplos fornecedores simultaneamente, oferecendo a seus clientes opções flexíveis de pagamento.
...
- Quaisquer pré-requisitos (ex.: verificação de negócio)
...
| Modo | Descrição |
|------|-------------|
| **Hospedado** | Os clientes são redirecionados para a página de pagamento do fornecedor (ex.: Stripe Checkout). Configuração mais simples, conformidade com o PCI é tratada pelo fornecedor. |
...
- **Ordem de Exibição** — Controla a ordem em que os fornecedores aparecem no checkout (números mais baixos aparecem primeiro)
...
Isso é útil quando um método de pagamento é popular em uma região, mas não em outra (ex.: iDEAL na Holanda, Bancontact na Bélgica).

## Webhooks

Webhooks mantêm sua loja sincronizada com o provedor de pagamento em tempo real.

Eles lidam com eventos como:
- Pagamento concluído ou falhado
- Estornos processados
- Disputas e cobranças abertas
- Renovações de assinaturas

### Configuração Automática

Quando você conecta um provedor, o Spwig registra automaticamente um endpoint de webhook com o provedor. O URL do webhook é exibido na página de configuração do provedor para referência.

### Monitoramento de Webhook

Cada webhook entrante é registrado com:
- **Tipo de evento** (ex., payment_intent.succeeded)
- **Carimbo de data/hora** e status de processamento
- **Payload** para depuração

Se um webhook falhar no processamento, ele será registrado como um erro para que você possa investigar.
...
- Monitore o Painel de Pagamentos semanalmente para identificar transações falhadas e problemas de conexão cedo.
