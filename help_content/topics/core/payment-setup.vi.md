...
Các nhà cung cấp thanh toán kết nối cửa hàng của bạn với cổng thanh toán để bạn có thể chấp nhận thẻ tín dụng, ví số, và các phương thức thanh toán khác tại bước thanh toán. Spwig hỗ trợ nhiều nhà cung cấp cùng lúc, mang lại cho khách hàng của bạn các lựa chọn thanh toán linh hoạt.
...
| Nhà cung cấp | Mô tả |
|----------|-------------|
| **Stripe** | Thẻ tín dụng, Apple Pay, Google Pay, và hơn 135 loại tiền tệ |
...
Chọn từ các nhà cung cấp thanh toán có sẵn. Mỗi thẻ hiển thị các tính năng và khu vực được hỗ trợ bởi nhà cung cấp.
...
| Chế độ | Mô tả |
|------|-------------|
| **Hosted** | Khách hàng được chuyển hướng đến trang thanh toán của nhà cung cấp (ví dụ: Stripe Checkout). Thiết lập đơn giản nhất, tuân thủ PCI được xử lý bởi nhà cung cấp. |
...
- **Tên hiển thị** — Tên được hiển thị cho khách hàng trong bước thanh toán
- **Thứ tự hiển thị** — Điều khiển thứ tự các nhà cung cấp hiển thị tại bước thanh toán (số nhỏ hơn sẽ hiển thị trước)
...
Các thẻ cảnh báo ở đầu trên sẽ làm nổi bật các vấn đề cần được chú ý:
...
Điều này hữu ích khi một phương thức thanh toán phổ biến ở một khu vực nhưng không phổ biến ở khu vực khác (ví dụ: iDEAL ở Hà Lan, Bancontact ở Bỉ).
...
- Luôn giữ **mã API** của bạn an toàn — chúng được lưu trữ dưới dạng mã hóa trong cơ sở dữ liệu nhưng không bao giờ nên được chia sẻ.