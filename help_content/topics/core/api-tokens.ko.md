...
API 토큰은 외부 서비스 및 통합이 가게와 통신할 수 있도록 해주는 보안 키입니다. 제3자 서비스나 도구가 가게의 데이터에 접근하거나 액션을 트리거해야 할 때, 요청에 함께 API 토큰을 보내어 요청이 승인되었는지 확인합니다. API 토큰 섹션에서 모든 토큰을 생성하고 관리하며, 각 토큰이 가게의 어떤 부분에 접근할 수 있는지 정확히 설정할 수 있습니다.

## API 토큰이 필요한 경우

가게에 API 토큰을 생성해야 하는 일반적인 경우는 다음과 같습니다:

- 외부 서비스나 자동화 도구를 연결할 때, 이 도구가 가게에 데이터를 읽거나 쓰는 것이 필요할 때
- 수신 웹훅이 들어오는 요청을 인증해야 할 때
- 설치된 Spwig 도움 시스템을 설정할 때
- Spwig의 API를 사용하여 커스텀 통합을 구축할 때
- Spwig 가게와 다른 시스템 간 데이터를 동기화할 때

각 통합에는 자체 토큰이 있어야 하며, 하나의 서비스에 대한 접근을 취소하더라도 다른 서비스에 영향을 주지 않도록 할 수 있습니다.

## 토큰 유형

토큰을 생성할 때, 그 목적을 설명하는 유형을 선택합니다. 이 유형은 참고용이며, 각 토큰이 무엇을 하는지 추적하는 데 도움이 됩니다.

| 유형 | 목적 |
|------|---------|
| **도움 시스템** | Spwig 도움 문서 시스템에서 사용됨 |
| **외부 통합** | 제3자 서비스, 자동화 도구(예: Zapier), 또는 데이터 동기화 도구 |
| **웹훅** | 웹훅 수신기 또는 엔드포인트의 인증 |
| **커스텀** | 위의 범주에 해당하지 않는 기타 목적 |
| **인스턴스 동기화** | Spwig 설치 간 또는 외부 Spwig 서비스 간의 동기화 |
...
**선택된 범위가 없는 토큰은 API에 접근할 수 없습니다.** 토큰이 활성화되고 유효하다고 해도 해당 범위가 없으면 접근이 불가능합니다.
...
- 범위에 **Read & Write**이 적용되려면 생성한 직원의 역할이 해당 영역에 대한 쓰기 액세스를 허용해야 합니다. 예를 들어, Products에 대해 보기 전용 역할을 가진 직원이 "Products: Read & Write"로 토큰을 생성하더라도 토큰은 여전히 읽기만 가능합니다. 역할은 범위의 위에 두 번째 게이트 역할을 합니다.
- 토큰을 생성한 직원이 삭제되거나 계정이 비활성화되면, 토큰은 즉시 API 액세스를 잃게 됩니다. 범위와 무관하게, 더 이상 토큰이 대신 행동할 수 있는 허용된 사용자가 없습니다.

이러한 의미에서, 토큰의 범위를 엄격하게 설정하는 가장 안전한 방법은, 토큰이 가질 권한과 동일한 역할을 가진 직원으로 로그인한 상태에서 토큰을 생성하는 것입니다.
...
- 제3자에게 제공하는 토큰으로, 자동으로 액세스가 제거되기를 원하는 경우
- 고권한 통합에 추가 보안 층을 제공하는 경우

토큰이 만료되면 해당 토큰으로 요청이 거부됩니다. 액세스를 연장하려면 **Expires At** 날짜를 업데이트하거나 대체 토큰을 생성하세요.

## Restricting to specific IP addresses

**Allowed IPs** 필드는 IP 주소 목록을 입력할 수 있습니다. 목록이 비어 있지 않은 경우, 토큰은 해당 주소 중 하나에서 요청이 올 때만 작동합니다.

예를 들어, 분석 도구가 `203.0.113.42` 서버에서 실행된다면, 해당 IP를 추가하면 토큰이 다른 위치에서 유출되어도 악용될 수 없습니다.

**Allowed IPs**를 비워두면 모든 IP 주소에서 요청을 허용합니다.
...
2. 작업 메뉴에서 **선택한 API 토큰 삭제**를 선택합니다.
3. 삭제를 확인합니다.
...
- 토큰을 제3자에게 전달하기 전에 변경 양식에서 **"이 토큰은 다음을 액세스할 수 있습니다:"** 요약을 확인하세요 — 이는 의도하지 않은 범위가 부여되지 않았는지 확인하는 가장 빠른 방법입니다
...
- 몇 달마다 토큰 목록을 검토하고 **Last Used** 날짜가 예상보다 오래된 토큰을 비활성화하세요 — 이는 더 이상 실행되지 않는 통합에 속할 수 있습니다
