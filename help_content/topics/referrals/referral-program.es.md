...
2. El amigo hace clic en el enlace y se le rastrea mediante una cookie durante un máximo de 30 días (configurable).
3. El amigo se registra y realiza su primera compra calificada.
...
Tu tienda tiene una sola configuración del programa de referidos. Navega a **Marketing > Programa de Referidos** para configurarlo.
...
- **Borrador** — El programa se está configurando pero aún no está activo. Los enlaces de referidos están inactivos.
- **Activo** — El programa está activo. Los clientes pueden compartir enlaces y ganar recompensas.
...
Define las recompensas que se otorgan cuando una referencia se convierte. El programa admite **recompensas de dos lados** — lo que significa que puedes recompensar tanto al referidor (el cliente que compartió el enlace) como al referido (el nuevo cliente que lo usó).

Configura las recompensas para cada destinatario en el campo **Configuración de Recompensas**. Los tipos de recompensas disponibles son:

| Tipo de Recompensa | Descripción |
|-------------------|-------------|
| **Crédito de Tienda** | Agrega crédito al monedero del cliente, utilizable en futuras compras |
| **Código de Cupón** | Genera un código de descuento único |
| **Descuento en Porcentaje** | Emite un descuento en porcentaje para usar en el checkout |
...
Las recompensas de Código de Cupón y Descuento en Porcentaje están bloqueadas para el cliente que las ganó — el código de cupón solo funciona cuando ese cliente está conectado. Si un referidor comparte su código de recompensa con alguien más en lugar de su enlace de referido, el amigo no podrá usarlo; solo el enlace de referido en sí está destinado a ser compartido.

**Configuración de ejemplo** — $10 de crédito de tienda para el referidor y $10 de descuento para el nuevo cliente:
...
|---------|--------------|
| `issue_on` | Cuándo otorgar la recompensa: `signup` (inmediatamente al registrarse), `first_purchase` (inmediatamente después del pedido) o `post_refund` (después de que expire el período de reembolso) |
| `refund_window_days` | Cuántos días esperar antes de otorgar recompensas al usar `post_refund` (por defecto: 14 días) |


Usar `post_refund` es el enfoque más cauteloso — espera hasta que haya pasado el período de devolución antes de otorgar recompensas, reduciendo el riesgo de recompensar pedidos que posteriormente se devuelvan.

### Límites y caps

Evite que un solo referidor gane recompensas ilimitadas estableciendo límites en el campo **Caps & Limits**:
...
Configure cómo se rastrean los enlaces de referidos en el campo **Tracking Configuration**:
...
El sistema de detección de fraude califica automáticamente cada atribución de referido por riesgo antes de aprobarla. Configure la política en el campo **Fraud Policy**:
...
Ingrese cualquier término y condición legal para el programa en el campo **Terms & Conditions**. Este texto se muestra a los clientes cuando ven el programa de referidos. Se admite el formato Markdown.

## Ver atribuciones de referidos

Navegue hasta **Marketing > Referral Attributions** para ver todos los casos de referidos — la conexión entre un referidor y un cliente referido.
...
|--------|---------------|
| **Pendiente** | A la espera de revisión — el puntaje de riesgo está en el rango de revisión manual |
| **Aprobado** | La referida es válida — las recompensas se han otorgado o se otorgarán |
| **Rechazado** | La referida no calificó o fue marcada como fraudulenta |
| **Caducado** | La referida no se convirtió dentro del período de seguimiento |
...
- Bajo riesgo (puntuación 0–30) — Aprobado automáticamente
- Medio riesgo (puntuación 31–70) — Revisión manual
- Alto riesgo (puntuación 71–89) — Revisión manual, trátelo con precaución
- Muy alto riesgo (puntuación 90+) — Rechazado automáticamente
...
- La política de fraude `balanced` es una buena opción predeterminada para la mayoría de tiendas. Cambie a `strict` si nota un pico inusual en las referencias provenientes de un número pequeño de cuentas.
...
- Mantenga sus Términos y Condiciones cortos y en lenguaje sencillo. Los clientes son más propensos a participar cuando entienden claramente las reglas.