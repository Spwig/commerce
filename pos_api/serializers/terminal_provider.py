...
class CloudPaymentInitiatedSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        ...
        help_text=_(
            'Initial status -- typically "pending" for async providers, "succeeded" for sync (Adyen)'
        ),
    )
...
class CloudPaymentStatusSerializer(serializers.Serializer):
    ...
    payment_id = serializers.CharField(
        required=False, help_text=_("Provider payment ID (when succeeded)")
    )
    cancel_reason = serializers.CharField(
        required=False, help_text=_("Reason for cancellation (when canceled)")
    )
