...
class POSTerminalSerializer(serializers.ModelSerializer):
    ...
    pos_display_name = serializers.CharField(
        source='warehouse.pos_display_name', read_only=True
    )
    ...
    class Meta:
        model = POSTerminal
        fields = [
            'uuid', 'name', 'warehouse', 'warehouse_name',
            'pos_display_name', 'hardware_config', 'is_active',
            'last_heartbeat', 'currency', 'effective_currency',
            'remote_unlock_at', 'order_sync_days', 'order_sync_limit',
        ]
