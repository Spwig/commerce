...
class POSStockAdjustmentSerializer(serializers.Serializer):
    ...
    product_id = serializers.IntegerField(
        help_text=_('ID of the product to adjust stock for.')
    )
    ...
    reason = serializers.CharField(
        min_length=1,
        help_text=_('Required explanation for this adjustment (audit trail).')
    )
    idempotency_key = serializers.CharField(
        required=False, max_length=64,
        help_text=_('Optional UUID for offline deduplication.')
    )
