...
from pos_api.serializers.customer import (
    POSCustomerSerializer, POSCustomerSearchSerializer,
    POSCustomerCreateSerializer,
)
from core.api.api_descriptions import (
    AUTH_REQUIRED, POS_LICENSE_REQUIRED, CUSTOMER_NOT_FOUND,
    INVALID_REQUEST,
)
    ...
    if hasattr(user, 'orders'):
        for order in user.orders.order_by('-created_at')[:5]:
            recent_orders.append({
                ...
                'total': str(order.total_amount.amount) if hasattr(order.total_amount, 'amount') else str(order.total_amount),
                'status': order.status,
