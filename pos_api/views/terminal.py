...
from pos_api.serializers.terminal import POSTerminalRegisterSerializer, POSTerminalSerializer
...
def _log_lock_event(
    ...
    from pos_app.models import POSShift, TerminalLockEvent

    # Get current shift if any
    shift = POSShift.objects.filter(terminal=terminal, ended_at__isnull=True).first()
...
@extend_schema(
    ...
    responses={
        200: OpenApiResponse(
            description=_("Terminal registered successfully, returns terminal config")
        ),
...
def terminal_heartbeat(request):
    ...
    POSTerminal.objects.filter(uuid=terminal_uuid).update(last_heartbeat=timezone.now())
...
def receipt_template(request):
    ...
    try:
        terminal = POSTerminal.objects.select_related("warehouse", "warehouse__store_group").get(
            uuid=terminal_uuid, is_active=True
        )
    except POSTerminal.DoesNotExist:
        return Response(
...
def promo_slides(request):
    ...
    slides = (
        PromoSlide.objects.filter(slide_filter)
...
def generate_display_pairing_code(request):
    ...
    from pos_app.models import DisplayPairingCode, POSTerminal
...
def list_managers(request):
    ...
    # POSStaffDiscount.save() hashes manager_pin into manager_pin_hash and
    # clears the plaintext column, so filtering on manager_pin excluded every
    # active manager. Filter on the hash instead (the CharField is
    # non-nullable — check for non-empty).
    managers = (
    ...
    request={
        "application/json": {
            ...
            "properties": {
                ...
                "locked_by_user_id": {
                    "type": "integer",
                    "description": "User ID who locked the terminal",
                },
                "failed_attempts": {
                    "type": "integer",
                ...
                "manager_override": {
                    "type": "boolean",
                    "description": "If true, verify against specific manager",
                },
                "manager_user_id": {
                    "type": "integer",
                    "description": "Manager user ID for targeted override",
                },
            },
            "required": ["pin"],
        }
...
def verify_unlock_pin(request):
    ...
    from pos_app.models import POSStaffDiscount, POSTerminal
        ...
        return Response(
            {
                ...
                "error": {
                    "code": "INVALID_EVENT",
                    "message": "Invalid event type. Use lock_manual or lock_auto.",
                },
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def verify_unlock_card(request):
    ...
    if not card_data:
        return Response(
            {
                "success": False,
                "error": {"code": "MISSING_CARD", "message": "Card data is required."},
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    # Try manager card
    manager_staff = (
        POSStaffDiscount.objects.filter(is_manager=True, card_identifier=card_hash)
        .select_related("user")
        .first()
    )
...
def register_staff_card(request):
    ...
    existing = (
        POSStaffDiscount.objects.filter(card_identifier=card_hash)
        ...
        return Response(
            {
                ...
                "error": {
                    "code": "CARD_IN_USE",
                    "message": "This card is registered to another user.",
                },
            },
...
def set_staff_pin(request):
    ...
    if not pin or not pin_confirm:
        return Response(
            {
                "success": False,
                "error": {"code": "MISSING_PIN", "message": "PIN and confirmation are required."},
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
