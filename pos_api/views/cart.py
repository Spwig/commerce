...
from core.api.api_descriptions import (
    AUTH_REQUIRED, POS_LICENSE_REQUIRED, PRODUCT_NOT_FOUND,
)
from pos_api.serializers.cart import (
    POSCartSerializer, POSAddToCartSerializer,
    POSUpdateCartItemSerializer, POSApplyDiscountSerializer,
    POSApplyGiftCardSerializer,
)
from pos_api.views.utils import serialize_cart, get_terminal_currency, get_terminal
...
def get_cart(request):
    ...
    return Response({'success': True, 'cart': serialize_cart(cart, currency=get_terminal_currency(request), request=request)})
...
def add_to_cart(request):
    ...
    if barcode and not product_id:
        try:
            product = Product.objects.get(
                barcode=barcode, status='published',
    ...
    success, message, cart_item = CartService.add_item(
        cart, product_id, quantity, variant_id=variant_id,
        channel='pos', warehouse=pos_warehouse,
        configuration=configuration, preset_id=preset_id,
    ...
    return Response({'success': True, 'message': str(message), 'cart': serialize_cart(cart, currency=get_terminal_currency(request), request=request)})
...
def update_cart_item(request, item_id):
    ...
    if cart_item.parent_bundle is not None:
        return Response(
            {'success': False, 'error': {'code': 'COMPONENT_ITEM', 'message': 'Cannot modify component items directly.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def remove_cart_item(request, item_id):
    ...
    if cart_item.parent_bundle is not None:
        return Response(
            {'success': False, 'error': {'code': 'COMPONENT_ITEM', 'message': 'Cannot remove component items directly.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def clear_cart(request):
    ...
    cart.save(update_fields=[
        'pos_manual_discount_type', 'pos_manual_discount_value',
        'pos_manual_discount_applied_by', 'pos_manual_discount_approved_by',
        'pos_manual_discount_reason',
    ])

    cart.refresh_from_db()
    return Response({'success': True, 'message': 'Cart cleared.', 'cart': serialize_cart(cart, currency=get_terminal_currency(request), request=request)})
...
def apply_item_discount(request, item_id):
    ...
    if limits is None or not limits.can_apply_item_discounts:
        return Response(
            {'success': False, 'error': {'code': 'NOT_AUTHORIZED', 'message': 'You are not authorized to apply item discounts.'}},
    ...
    if discount_type not in ('percentage', 'fixed'):
        return Response(
            {'success': False, 'error': {'code': 'INVALID_TYPE', 'message': 'Invalid discount type. Use "percentage" or "fixed".'}},
            status=status.HTTP_400_BAD_REQUEST,
        )

    if discount_value <= 0:
        return Response(
            {'success': False, 'error': {'code': 'INVALID_VALUE', 'message': 'Discount value must be greater than 0.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if limits.requires_reason and not reason.strip():
        return Response(
            {'success': False, 'error': {'code': 'REASON_REQUIRED', 'message': 'A reason is required for this discount.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if requires_approval:
        return Response({
            ...
            'max_allowed_type': 'percentage' if discount_type == 'percentage' or limits.max_discount_amount is None else 'fixed',
            'item_id': item_id,
    ...
    cart_item.save(update_fields=[
        ...
        'manual_discount_applied_by', 'manual_discount_reason',
    ])
...
def remove_item_discount(request, item_id):
    ...
    cart_item.save(update_fields=[
        'manual_discount_type', 'manual_discount_value',
        'manual_discount_applied_by', 'manual_discount_approved_by',
...
def apply_cart_discount(request):
    ...
    if limits is None or not limits.can_apply_cart_discounts:
        return Response(
            {'success': False, 'error': {'code': 'NOT_AUTHORIZED', 'message': 'You are not authorized to apply cart discounts.'}},
            status=status.HTTP_403_FORBIDDEN,
        )
    ...
    cart_subtotal = cart.total_amount.amount if hasattr(cart.total_amount, 'amount') else Decimal('0')
    ...
    cart.save(update_fields=[
        'pos_manual_discount_type', 'pos_manual_discount_value',
        'pos_manual_discount_applied_by', 'pos_manual_discount_reason',
    ])
...
@extend_schema(
    ...
    request={
        ...
        "properties": {
            ...
            "discount_value": {"type": "number"},
            "reason": {"type": "string"},
            "item_id": {"type": "integer", "description": "Cart item ID (for item-level discounts)"},
        },
...
def approve_discount(request):
    ...
    from cart.services.cart_service import CartService
    from cart.models import CartItem
    from pos_app.models import POSStaffDiscount, POSShift
    from decimal import Decimal
    ...
    if discount_type not in ('percentage', 'fixed'):
        return Response(
            {'success': False, 'error': {'code': 'INVALID_TYPE', 'message': 'Invalid discount type.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if discount_type == 'percentage':
        if item_id:
            cart_item = CartItem.objects.get(id=item_id, cart=cart)
            discount_amount = (cart_item.unit_price.amount * cart_item.quantity) * discount_value / 100
        else:
            cart_subtotal = cart.total_amount.amount if hasattr(cart.total_amount, 'amount') else Decimal('0')
...
def park_cart(request):
    ...
    if cart.items.count() == 0:
        return Response(
            {'success': False, 'error': {'code': 'EMPTY_CART', 'message': 'Cannot park an empty cart.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if not terminal:
        return Response(
            {'success': False, 'error': {'code': 'NO_TERMINAL', 'message': 'Terminal not configured.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    customer_name = cart_snapshot.get('customer', {}).get('name', '') if cart_snapshot.get('customer') else ''
...
def restore_parked_cart(request, parked_id):
    ...
    if manual_discount and manual_discount.get('type') and manual_discount['type'] != 'none':
        ...
        cart.save(update_fields=[
            'pos_manual_discount_type', 'pos_manual_discount_value',
            'pos_manual_discount_reason', 'pos_manual_discount_applied_by',
        ])
