...
def serialize_cart(cart, currency=None, request=None):
    ...
    for item in (
        ...
        if hasattr(item, "manual_discount_type") and item.manual_discount_type != "none":
            ...
            item_manual_discount = {
                ...
                "applied_by": (
                    item.manual_discount_applied_by.get_full_name()
                    or item.manual_discount_applied_by.username
                )
                if item.manual_discount_applied_by
                else None,
                "approved_by": (
                    item.manual_discount_approved_by.get_full_name()
                    or item.manual_discount_approved_by.username
                )
                if item.manual_discount_approved_by
                else None,
            }
        ...
        child_items = cart.items.filter(parent_bundle=item).select_related(
        ...
        if child_items.exists():
            ...
            for child in child_items:
                ...
                if (
                    not child_image
                    and hasattr(child.product, "images")
                    and child.product.images.exists()
                ):
                    child_primary = (
                        child.product.images.select_related("media_asset")
    ...
    (str(cart.grand_total.amount) if hasattr(cart.grand_total, "amount") else str(cart.grand_total))
    ...
    if hasattr(cart, "applied_vouchers"):
        ...
        if first_voucher:
            voucher_code = (
                first_voucher.voucher.code
                if hasattr(first_voucher, "voucher")
                else str(first_voucher)
            )
    ...
    if hasattr(cart, "pos_manual_discount_type") and cart.pos_manual_discount_type != "none":
        ...
        cart_manual_discount = {
            ...
            "applied_by": (
                cart.pos_manual_discount_applied_by.get_full_name()
                or cart.pos_manual_discount_applied_by.username
            )
            if cart.pos_manual_discount_applied_by
            else None,
            "approved_by": (
                cart.pos_manual_discount_approved_by.get_full_name()
                or cart.pos_manual_discount_approved_by.username
            )
            if cart.pos_manual_discount_approved_by
            else None,
        }
                ...
                if warehouse and warehouse.country:
                    ...
                    if tax_items:
                        ...
                        if subtotal_decimal > 0:
                            tax_rate_display = (tax_amount_decimal / subtotal_decimal).quantize(
                                Decimal("0.0001")
                            )
        except Exception:
            import logging

            logging.getLogger(__name__).warning("POS cart tax calculation failed", exc_info=True)
