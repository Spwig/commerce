...
from pos_api.serializers.product import (
    POSProductSerializer, POSProductListSerializer, POSCategorySerializer
)
...
def _serialize_product(product, warehouse_id=None, currency=None):
    ...
    if prefetched is not None:
        primary = next((i for i in prefetched if i.is_primary), None) or (prefetched[0] if prefetched else None)
    ...
    if warehouse_id and product.track_inventory:
        ...
        if product.product_type == 'variable':
            ...
            is_low_stock = stock_available > 0 and stock_available <= (product.low_stock_threshold or 5)
            ...
            for slot in slots:
                ...
                for opt in slot.options.all():
                    ...
                    if opt.option_variant:
                        ...
                        try:
                            si = StockItem.objects.get(
                                product=opt.option_product, variant__isnull=True,
    ...
    if product.product_type == 'variable':
        ...
        prefetched_variants = getattr(product, '_prefetched_objects_cache', {}).get('variants', None)
    ...
    if product.product_type == 'bundle':
        from catalog.models import BundleItem, StockItem as BundleStockItem
        bundle_items_qs = BundleItem.objects.filter(
            bundle=product
        ).select_related(
        ...
        for bi in bundle_items_qs:
            ...
            if bi.allow_variant_selection:
                for v in bi.component_product.variants.filter(is_active=True):
                    ...
                    if warehouse_id and bi.component_product.track_inventory:
                        try:
                            vsi = BundleStockItem.objects.get(
                                variant=v, warehouse_id=warehouse_id
                            )
                    ...
                    avail_variants.append({
                        ...
                        'name': v.name,
                        'sku': v.sku or '',
                        'price': str(v.get_effective_price().amount) if v.get_effective_price() else '0.00',
                        'stock_available': v_stock,
                        'image': v.image_asset.get_thumbnail('medium') if v.image_asset else None,
                    })
            ...
            items_data.append({
                ...
                'component_variant_name': bi.component_variant.name if bi.component_variant else None,
        ...
        data['bundle'] = {
            ...
            'discount_percentage': str(product.bundle_discount_percentage) if product.bundle_discount_percentage else '0',
            'items': items_data,
        }
        data['is_bundle'] = True
    ...
    if product.product_type == 'configurable':
        from catalog.models import (
            ConfigurationSlot, CompatibilityRule, ConfigurationPreset, StockItem
        )

        slots_data = []
        slots = ConfigurationSlot.objects.filter(
            product=product
        ).order_by('sort_order').prefetch_related(
        ...
        for slot in slots:
            ...
            for opt in slot.options.order_by('sort_order'):
                ...
                opt_price = str(opt.option_product.price.amount) if opt.option_product.price else '0.00'
                if currency and opt.option_product.price and str(opt.option_product.price.currency) != currency:
                    try:
                ...
                options_data.append({
                    ...
                    'price': opt_price,
                    'price_adjustment': str(opt.price_adjustment.amount) if opt.price_adjustment else '0.00',
                    'image': opt_image,
        ...
        data['configurator'] = {
            ...
            'base_price': str(product.configurator_base_price.amount) if product.configurator_base_price else None,
            'slots': slots_data,
            'rules': rules_data,
        ...
        try:
            ...
            scene = SceneConfig.objects.select_related(
                ...
                'mappings', 'geometry_assets__media_asset'
...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter("warehouse_id", OpenApiTypes.INT, description=_("Warehouse for stock levels")),
        OpenApiParameter("page", OpenApiTypes.INT, description=_("Page number (default: 1)")),
        OpenApiParameter("page_size", OpenApiTypes.INT, description=_("Items per page (default: 50, max: 200)")),
    ],
    responses={
...
def barcode_lookup(request, barcode):
    ...
    return Response(
        {'success': False, 'error': {'code': 'NOT_FOUND', 'message': f'No product found with barcode "{barcode}".'}},
        status=status.HTTP_404_NOT_FOUND
    )
...
def category_list(request):
    ...
    categories = Category.objects.filter(
        is_active=True
    ).select_related('image_asset').order_by('sort_order', 'name')
