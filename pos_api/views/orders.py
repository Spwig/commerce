...
from core.api.api_descriptions import (
    AUTH_REQUIRED, POS_LICENSE_REQUIRED, ORDER_NOT_FOUND, NO_OPEN_SHIFT,
)
from pos_api.serializers.order import (
    POSOrderSerializer, POSOrderListSerializer, POSReceiptSerializer,
)
...
def _get_order_thumbnails(order, request, limit=4):
    ...
    items = list(prefetched_items)[:limit] if prefetched_items is not None else order.items.all()[:limit]

    for item in items:
        if not item.product:
            continue
        # Check for prefetched images on the product
        prefetched_images = getattr(item.product, '_prefetched_objects_cache', {}).get('images', None)
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            ...
            description=_("Start date for range filter (ISO format, default: today). Ignored when search is provided."),
        ),
        OpenApiParameter(
            "date_to", OpenApiTypes.DATE,
            description=_("End date for range filter (ISO format, default: today). Ignored when search is provided."),
        ),
        OpenApiParameter(
            "origin", OpenApiTypes.STR,
            enum=['this_terminal', 'all_pos', 'all'],
            description=_("Order origin filter: this_terminal (default), all_pos, all (includes web). Ignored when search is provided."),
        ),
        OpenApiParameter(
            "customer_id", OpenApiTypes.INT,
            description=_("Filter by customer user ID. Ignored when search is provided."),
        ),
        OpenApiParameter(
            "cashier_id", OpenApiTypes.INT,
            description=_("Filter by cashier staff ID. Ignored when search is provided."),
        ),
        OpenApiParameter(
            "search", OpenApiTypes.STR,
            description=_("Search across ALL orders by order number, customer name, email, or phone. Bypasses date/origin filters."),
        ),
        OpenApiParameter("status", OpenApiTypes.STR, description=_("Filter by order status (applies in both modes)")),
        OpenApiParameter("page", OpenApiTypes.INT, description=_("Page number (default: 1)")),
        OpenApiParameter("page_size", OpenApiTypes.INT, description=_("Items per page (default: 20, max: 100)")),
    ],
    responses={
...
def order_list(request):
    ...
    if search:
        ...
        orders = orders.annotate(
            latest_refund_at=Max('refunds__processed_at')
        ).order_by('-latest_refund_at').distinct()
...
def order_detail(request, order_id):
    ...
    try:
        order = Order.objects.prefetch_related(
            'items', 'pos_payments', 'refunds', 'refunds__processed_by',
        ).select_related(
...
def order_receipt(request, order_id):
    ...
    for p in order.pos_payments.all():
        ...
        if p.amount_tendered:
            tendered_val = p.amount_tendered.amount if hasattr(p.amount_tendered, 'amount') else p.amount_tendered
...
def _process_provider_refund_pos(order, refund_amount, reason, currency):
    ...
    pos_payment = order.pos_payments.filter(
        method='terminal_card',
    ).exclude(provider_payment_id='').first()
...
def _process_provider_refund_web(order, refund_method, refund_amount, reason, currency):
    ...
    try:
        original_txn = PaymentTransaction.objects.select_related(
        ...
        'application/json': {
            ...
            'properties': {
                ...
                'reason': {
                    'type': 'string',
                    'enum': [
                        'customer_request', 'damaged', 'wrong_item',
                        'defective', 'not_as_described', 'other',
                    ],
...
def refund_order(request, order_id):
    ...
    if order.status == 'refunded':
        return Response(
            {'success': False, 'error': {'code': 'ALREADY_REFUNDED', 'message': 'Order is already fully refunded.'}},
    ...
    if refund_method == 'terminal_card':
        provider_refund_error = _process_provider_refund_pos(
            order, refund_amount, reason, currency
        )
    ...
    with transaction.atomic():
        ...
        if refund_type == 'full':
            ...
            order.status = 'refunded' if refund_amount >= order.total_amount.amount else 'processing'
            order.payment_status = 'refunded' if refund_amount >= order.total_amount.amount else 'partially_refunded'
            prev_refunded = order.amount_refunded.amount if hasattr(order.amount_refunded, 'amount') else Decimal('0')
        ...
        if return_to_stock and terminal.warehouse_id:
            _return_refund_stock(
                order, refund_type, refund_items, items_json,
                terminal.warehouse_id, request.user,
            )

    # Re-fetch order with prefetches for serialization
    order = Order.objects.prefetch_related(
        'items', 'pos_payments', 'refunds', 'refunds__processed_by',
    ).select_related('user', 'cashier', 'pos_terminal').get(id=order_id)
...
def void_order(request, order_id):
    ...
    if order.status == 'cancelled':
        return Response(
            {'success': False, 'error': {'code': 'ALREADY_VOIDED', 'message': 'Order is already voided.'}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    with transaction.atomic():
        ...
        void_refund.save(update_fields=['total_amount_base', 'shipping_refund_amount_base', 'tax_refund_amount_base'])
        ...
        for ij in items_json:
            ...
            if order_item.product.track_inventory:
                items_to_return.append(
                    (order_item.product, order_item.variant, ij['quantity'])
                )
