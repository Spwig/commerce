...
from django.db.models import F, Q, Case, When, Value, IntegerField, Prefetch, Sum
...
from pos_api.serializers.inventory import (
    POSStockItemSerializer, POSCrossLocationStockSerializer,
    POSStockAdjustmentSerializer, POSStockMovementSerializer,
)
...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter(
            "low_stock_only", OpenApiTypes.BOOL,
            description=_("Only return items at or below low stock threshold"),
        ),
        OpenApiParameter(
            "no_stock_only", OpenApiTypes.BOOL,
...
def stock_levels(request):
    ...
    """
    from catalog.models import Product, ProductVariant, ProductImage, StockItem
    ...
    for p in simple_qs:
        ...
        threshold = (si.low_stock_threshold if si and si.low_stock_threshold else 0) or p.low_stock_threshold
    ...
    for v in variant_list:
        ...
        allocated = si.allocated if si else 0
        available = max(0, on_hand - allocated)
        threshold = (si.low_stock_threshold if si and si.low_stock_threshold else 0) or v.product.low_stock_threshold
        is_low = available <= threshold
        results.append({
    ...
    results.sort(key=lambda r: (not r['is_low_stock'], r['product_name'].lower(), r['variant_name'] or ''))
...
def stock_detail(request, product_id):
    ...
    try:
        product = Product.objects.get(
            id=product_id, status='published',
            sales_channel__in=['all', 'pos_only'],
        )
...
def cross_location_stock(request, product_id):
    ...
    for wid, data in wh_stock.items():
        ...
        # Calculate distance
        distance_km = None
        if (current_wh and current_wh.latitude and current_wh.longitude
                and wh.latitude and wh.longitude):
            distance_km = round(_haversine_km(
                current_wh.latitude, current_wh.longitude,
...
def adjust_stock(request):
    ...
    # Idempotency check
    if idempotency_key:
        if StockMovement.objects.filter(reference_key=idempotency_key).exists():
            return Response({
                'success': True,
...
def stock_movements(request):
    ...
    movements = StockMovement.objects.filter(
        ...
        'stock_item__product', 'stock_item__variant', 'user',
    ).order_by('-created_at')
