...
def send_return_request_confirmation(return_request):
    ...
    try:
        ...
        EmailSendingService.send_template_email(
            ...
            context={
                'customer_name': return_request.user.get_full_name() or return_request.user.username,
        ...
        logger.info(f"Return request confirmation queued for {return_request.user.email} (return {return_request.id})")
        return True

    except Exception as e:
        logger.error(f"Failed to send return request confirmation for return {return_request.id}: {e}", exc_info=True)
        return False
...
def send_return_approved_notification(return_request):
    ...
    try:
        ...
        logger.info(f"Return approved notification queued for {return_request.user.email} (return {return_request.id})")

    except Exception as e:
        logger.error(f"Failed to send return approved notification for return {return_request.id}: {e}", exc_info=True)
...
def send_return_rejected_notification(return_request):
    ...
    try:
        ...
        logger.info(f"Return rejected notification queued for {return_request.user.email} (return {return_request.id})")

    except Exception as e:
        logger.error(f"Failed to send return rejected notification for return {return_request.id}: {e}", exc_info=True)
...
def send_return_received_notification(return_request):
    ...
    try:
        ...
        logger.info(f"Return received notification queued for {return_request.user.email} (return {return_request.id})")

    except Exception as e:
        logger.error(f"Failed to send return received notification for return {return_request.id}: {e}", exc_info=True)
...
def send_inspection_reminder_to_staff(return_request):
    ...
    try:
        ...
        if not staff_emails:
            logger.warning(f"No staff emails found for inspection reminder (return {return_request.id})")
            return
        ...
        for staff_email in staff_emails:
            try:
                ...
                logger.error(f"Failed to send inspection reminder to {staff_email}: {e}", exc_info=True)

        logger.info(f"Inspection reminder queued for {len(staff_emails)} staff member(s) (return {return_request.id})")

    except Exception as e:
        logger.error(f"Failed to send inspection reminders for return {return_request.id}: {e}", exc_info=True)
...
def send_refund_processed_notification(return_request):
    ...
    try:
        ...
        if not return_request.refund:
            logger.warning(f"Cannot send refund notification for return {return_request.id} - no refund linked")
            return
        ...
        logger.info(f"Refund processed notification queued for {return_request.user.email} (return {return_request.id})")

    except Exception as e:
        logger.error(f"Failed to send refund processed notification for return {return_request.id}: {e}", exc_info=True)
...
def send_order_status_update(order, old_status):
    ...
    try:
        ...
        logger.info(f"Order status update notification queued for {order.email} (order {order.order_number})")

    except Exception as e:
        logger.error(f"Failed to send order status update email for order {order.order_number}: {e}", exc_info=True)
