...
class OrderAddressSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Address
        fields = [
            'id', 'user', 'address_type', 'name', 'company',
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderItem
        fields = [
            'id', 'order', 'product', 'variant', 'product_name',
            'variant_name', 'sku', 'quantity', 'unit_price',
            'total_price', 'customizations', 'created_at'
        ]
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'order_number', 'user', 'status', 'tracking_number',
            'email', 'phone',
            'shipping_name', 'shipping_address1', 'shipping_address2',
            'shipping_city', 'shipping_state', 'shipping_postal_code',
            'shipping_country',
            'billing_same_as_shipping', 'billing_name', 'billing_address1',
            'billing_address2', 'billing_city', 'billing_state',
            'billing_postal_code', 'billing_country',
            'subtotal', 'tax_amount', 'shipping_cost', 'discount_amount',
            'total_amount', 'order_page_layout', 'show_order_progress',
            'show_shipping_updates', 'show_item_images', 'notes',
...
class OrderDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'total_amount', 'order_page_layout', 'show_order_progress',
            'show_shipping_updates', 'show_item_images', 'notes',
            'special_instructions', 'items', 'can_cancel',
            'created_at', 'updated_at'
        ]
...
class CancelOrderSerializer(serializers.Serializer):
    ...
    restore_stock = serializers.BooleanField(
        default=True,
        help_text=_("Restore product stock quantities")
    )
...
class ReturnItemSerializer(serializers.Serializer):
    ...
    order_item_id = serializers.IntegerField(
        help_text=_("ID of the order item being returned")
    )
    quantity = serializers.IntegerField(
        min_value=1,
        help_text=_("Quantity of this item to return")
    )
...
class CreateReturnRequestSerializer(serializers.Serializer):
    ...
    reason = serializers.ChoiceField(
        choices=ReturnRequest.RETURN_REASON_CHOICES,
        help_text=_("Main reason for return")
    )
    items = ReturnItemSerializer(
        many=True,
        help_text=_("List of items to return with quantities and reasons")
    )
...
class ReturnRequestSerializer(serializers.ModelSerializer):
    ...
    items_condition_display = serializers.CharField(
        source='get_items_condition_display',
        read_only=True,
        allow_null=True
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            'items_json', 'customer_notes', 'merchant_notes',
            'return_label_generated', 'return_tracking_number', 'return_label_url',
            'items_condition', 'items_condition_display',
            'restocking_fee', 'refund_status', 'suggested_refund',
            'requested_at', 'approved_at', 'received_at', 'inspected_at', 'completed_at'
        ]
        read_only_fields = [
            'id', 'user', 'status', 'return_label_generated',
            'return_tracking_number', 'return_label_url',
            'merchant_notes', 'items_condition', 'restocking_fee',
            'requested_at', 'approved_at', 'received_at', 'inspected_at', 'completed_at'
        ]
    ...
    def get_suggested_refund(self, obj) -> dict:
        ...
        return {
            'amount': str(amount),
            'currency': obj.order.total_amount.currency.code
        }
...
class ReturnRequestSummarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ReturnRequest
        fields = [
            'id', 'order_number', 'reason', 'reason_display',
            'status', 'status_display', 'items_count',
            'return_tracking_number', 'requested_at'
        ]
...
class OrderNoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = OrderNote
        fields = [
            'id', 'note', 'author_name', 'is_merchant',
            'created_at', 'updated_at'
        ]
...
class CreateOrderNoteSerializer(serializers.Serializer):
    ...
    note = serializers.CharField(
        max_length=2000,
        help_text=_("Note content (max 2000 characters)")
    )
