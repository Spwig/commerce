...
(function () {
  ...
  function initStatusSelect() {
    ...
    statusSelect.addEventListener('change', async function () {
      ...
      const sendNotification =
        document.getElementById('status-send-notification')?.checked || false;
      ...
      document.addEventListener('click', async function (e) {
        ...
        if (trackInventory && availableStock === 0) {
          ...
          const confirmed = await AdminModal.confirm(
            `Warning: "${productName}" is currently out of stock (0 available). Do you still want to add this item to the order?`
          );
          if (!confirmed) {
          ...
          const productName = resultItem.querySelector('.product-name').textContent;
          console.info(
            `Low stock warning: "${productName}" has only ${availableStock} units available`
          );
        }
  ...
  async function handleCustomerAddresses(
    addresses,
    hasExistingAddresses,
    orderId,
    previousOrderAddresses = null
  ) {
    /**
    ...
    let message =
      'This customer has saved addresses. Would you like to populate the order with these addresses?';

    if (hasExistingAddresses) {
      message =
        "This order already has addresses. Would you like to replace them with the customer's saved addresses?";
    }

    const addressPreview = [];
    if (addresses.shipping) {
      addressPreview.push(
        `Shipping: ${addresses.shipping.name}, ${addresses.shipping.city}, ${addresses.shipping.state}`
      );
    }
    if (addresses.billing) {
      addressPreview.push(
        `Billing: ${addresses.billing.name}, ${addresses.billing.city}, ${addresses.billing.state}`
      );
    }
        ...
        if (result.addresses_html) {
          ...
          if (addressesSection) {
            ...
            const fallbackSection = document.querySelector(
              '.addresses-content, .shipping-billing-wrapper'
            );
            if (fallbackSection) {
