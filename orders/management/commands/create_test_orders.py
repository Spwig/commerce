...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--count", type=int, default=30, help="Number of orders to create (default: 30)"
        )
        parser.add_argument(
            "--days", type=int, default=30, help="Spread orders over this many days (default: 30)"
        )
    ...
    def handle(self, *args, **options):
        ...
        for i in range(count):
            ...
            shipping_cost = Decimal(str(random.choice([0.0, 5.99, 9.99, 14.99]))).quantize(
                Decimal("0.01")
            )
            ...
            if random.random() < 0.2:  # 20% of orders have a discount
                discount_amount = (subtotal * Decimal(str(random.uniform(0.05, 0.25)))).quantize(
                    Decimal("0.01")
                )
        ...
        # Calculate totals
        total_revenue = sum(
            [Decimal(str(random.uniform(25.0, 500.0))) * Decimal("1.10") for _ in range(count)]
        )
        ...
        self.stdout.write("  Channel distribution:")
