...
class OrderEditForm(forms.ModelForm):
    ...
    class Meta:
        model = Order
        fields = [
            'status', 'email', 'phone', 'notes', 'special_instructions',
            'tracking_number', 'carrier', 'payment_status'
        ]
        widgets = {
...
class OrderItemInlineForm(forms.ModelForm):
    ...
    class Meta:
        model = OrderItem
        fields = ['product', 'variant', 'quantity', 'unit_price', 'base_price', 'discount_type',
                  'discount_value', 'discount_reason', 'exclude_from_vouchers', 'customizations']
        widgets = {
            ...
            'exclude_from_vouchers': forms.CheckboxInput(attrs={'class': 'exclude-vouchers-checkbox'}),
    ...
    def clean(self):
        ...
        if base_price and unit_price:
            if unit_price.amount > base_price.amount:
...
class OrderAddressInlineForm(forms.Form):
    ...
    phone = forms.CharField(
        max_length=20,
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'type': 'tel', 'placeholder': '+1 (555) 123-4567'}),
    ...
    def clean_voucher_code(self):
        ...
        if self.order and self.order.user and voucher.max_uses_per_customer:
            ...
            if customer_uses >= voucher.max_uses_per_customer:
                raise ValidationError(_('You have already used this voucher the maximum number of times'))
...
class OrderCustomerChangeForm(forms.Form):
    ...
    guest_email = forms.EmailField(
        ...
        widget=forms.EmailInput(attrs={
            'class': 'form-control',
            'placeholder': _('guest@example.com')
        }),
        label=_('Guest Email')
    )
...
class OrderManualDiscountForm(forms.Form):
    ...
    reason = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': _('Reason for discount (optional)')
        }),
        label=_('Reason')
    )
