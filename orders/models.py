...
class Order(CustomFieldsMixin, DesignMixin):
    ...
    gift_card_discount = MoneyField(max_digits=10, decimal_places=2, default_currency='USD', default=0)
    ...
    exchange_rate_provider = models.CharField(
        max_length=255,
        blank=True,
        help_text="Name of exchange rate provider used for conversion"
    )
    base_currency = models.CharField(
        max_length=3,
        blank=True,
        default='USD',
        help_text="Store's base currency at time of order"
    )
    ...
    metadata = models.JSONField(
        ...
        help_text="Extra metadata (e.g., marketplace purchase context)"
    )
    ...
    receipt_email_sent_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When digital receipt was sent via email'
    )
    receipt_sms_sent_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When digital receipt was sent via SMS/WhatsApp'
    )
    ...
    def compute_base_amounts(self):
        ...
        if not self.customer_currency or self.customer_currency == self.base_currency:
            ...
            self.subtotal_base = self.subtotal.amount if hasattr(self.subtotal, 'amount') else Decimal(str(self.subtotal or 0))
            self.tax_amount_base = self.tax_amount.amount if hasattr(self.tax_amount, 'amount') else Decimal(str(self.tax_amount or 0))
            self.shipping_cost_base = self.shipping_cost.amount if hasattr(self.shipping_cost, 'amount') else Decimal(str(self.shipping_cost or 0))
            self.discount_amount_base = self.discount_amount.amount if hasattr(self.discount_amount, 'amount') else Decimal(str(self.discount_amount or 0))
            self.gift_card_discount_base = self.gift_card_discount.amount if hasattr(self.gift_card_discount, 'amount') else Decimal(str(self.gift_card_discount or 0))
            self.total_amount_base = self.total_amount.amount if hasattr(self.total_amount, 'amount') else Decimal(str(self.total_amount or 0))
            self.amount_paid_base = self.amount_paid.amount if hasattr(self.amount_paid, 'amount') else Decimal(str(self.amount_paid or 0))
            self.amount_refunded_base = self.amount_refunded.amount if hasattr(self.amount_refunded, 'amount') else Decimal(str(self.amount_refunded or 0))
        else:
            ...
            def _to_base(money_val):
                amt = money_val.amount if hasattr(money_val, 'amount') else Decimal(str(money_val or 0))
    ...
    def total_item_quantity(self):
        ...
        return self.items.filter(parent_bundle__isnull=True).aggregate(total=Sum('quantity'))['total'] or 0
...
class OrderItem(models.Model):
    ...
    discount_reason = models.CharField(
        max_length=200,
        blank=True,
        help_text='Optional reason for the discount'
    )
    exclude_from_vouchers = models.BooleanField(
        default=False,
        help_text='Exclude this item from order-level voucher discounts'
    )
    ...
    stock_allocated = models.BooleanField(
        default=False,
        help_text='Whether stock has been allocated for this item'
    )
    stock_fulfilled = models.BooleanField(
        default=False,
        help_text='Whether stock has been fulfilled (shipped) for this item'
    )
    ...
    def has_discount(self):
        ...
        if self.base_price and self.unit_price.amount < self.base_price.amount:
            return True
    ...
    def get_discount_percentage(self):
        ...
        if self.discount_type == 'percentage':
            ...
            from decimal import Decimal
            discount_pct = (Decimal(str(self.discount_value)) / Decimal(str(self.base_price.amount))) * 100
            ...
            if discount_amount > 0:
                discount_pct = (Decimal(str(discount_amount)) / Decimal(str(self.base_price.amount))) * 100
                return round(float(discount_pct), 2)
...
class Address(models.Model):
    ...
    is_active = models.BooleanField(
        default=True,
        db_index=True,
        help_text='Whether this address version is currently active'
    )
    edited_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When this address was superseded by a new version'
    )
    version = models.PositiveIntegerField(
        default=1,
        help_text='Version number of this address'
    )
    ...
    def get_version_history(self):
        ...
        if self.original_address:
            ...
            return Address.objects.filter(
                Q(pk=root.pk) | Q(original_address=root)
            ).order_by('version')
        else:
            # This is the root, get all versions
            return Address.objects.filter(
                Q(pk=self.pk) | Q(original_address=self)
            ).order_by('version')
...
class OrderNote(models.Model):
    ...
    is_customer_note = models.BooleanField(
        default=False,
        help_text='Whether this note is visible to the customer'
    )
    is_read = models.BooleanField(
        default=False,
        help_text='Whether merchant has read this note (for customer messages)'
    )
...
class Refund(models.Model):
    ...
    refund_method_display = models.CharField(
        max_length=100,
        blank=True,
        help_text='Human-readable name of the refund method'
    )
    ...
    customer_notes = models.TextField(
        blank=True,
        help_text='Customer-facing notes about the refund'
    )
    ...
    approved_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When the refund was approved'
    )
    processed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When the refund processing started'
    )
    completed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When the refund was completed'
    )
    ...
    def compute_base_amounts(self):
        ...
        if not order.customer_currency or order.customer_currency == order.base_currency:
            ...
            self.shipping_refund_amount_base = shipping.amount if hasattr(shipping, 'amount') else Decimal(str(shipping or 0))
            tax = self.tax_refund_amount
            self.tax_refund_amount_base = tax.amount if hasattr(tax, 'amount') else Decimal(str(tax or 0))
        else:
...
class ReturnRequest(models.Model):
    ...
    items_json = models.JSONField(
        default=list,
        help_text='Items being returned with quantities and item-specific reasons'
    )

    # Return label tracking
    return_label_generated = models.BooleanField(
        default=False,
        help_text='Whether return shipping label has been generated'
    )
    return_tracking_number = models.CharField(
        max_length=100,
        blank=True,
        help_text='Tracking number for return shipment'
    )
    return_label_url = models.URLField(
        blank=True,
        help_text='URL to download return shipping label PDF'
    )

    # Notes and communication
    customer_notes = models.TextField(
        blank=True,
        help_text='Customer explanation for the return'
    )
    merchant_notes = models.TextField(
        blank=True,
        help_text='Internal merchant notes about the return'
    )
    rejection_reason = models.TextField(
        blank=True,
        help_text='Reason for rejecting the return request'
    )
    inspection_notes = models.TextField(
        blank=True,
        help_text='Notes from inspection after receiving items'
    )
    ...
    requested_at = models.DateTimeField(
        auto_now_add=True,
        help_text='When customer requested the return'
    )
    approved_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When return was approved by merchant'
    )
    rejected_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When return was rejected by merchant'
    )
    label_sent_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When return label was sent to customer'
    )
    received_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When items were received at warehouse'
    )
    inspected_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When items were inspected'
    )
    completed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When return process was completed'
    )
    updated_at = models.DateTimeField(
        auto_now=True,
        help_text='Last update to this return request'
    )
    ...
    def mark_received(self, user=None):
        ...
        from django.db import transaction
        from orders.emails import send_return_received_notification, send_inspection_reminder_to_staff
    ...
    def process_refund(self, refund_data):
        ...
        refund = Refund.objects.create(
            ...
            reason=self.reason if self.reason in dict(Refund.REFUND_REASON_CHOICES).keys() else 'customer_request',
            ...
            staff_notes=refund_data.get('staff_notes', f'Return request inspection: {self.items_condition}'),
            approved_at=timezone.now(),
        )
        refund.compute_base_amounts()
        refund.save(update_fields=['total_amount_base', 'shipping_refund_amount_base', 'tax_refund_amount_base'])
