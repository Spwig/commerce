...
from .models import Order, OrderItem, Address, OrderNote, Refund, ReturnRequest
...
class OrderItemInline(admin.TabularInline):
    ...
    model = OrderItem
    extra = 0
    fields = ['product_name', 'product_status', 'variant_name', 'sku', 'quantity', 'unit_price',
              'total_price', 'customization_summary', 'warehouse_info', 'fulfillment_status']
    readonly_fields = ['product', 'variant', 'product_name', 'product_status', 'variant_name',
                       'sku', 'quantity', 'unit_price', 'total_price', 'created_at',
                       'customization_summary', 'warehouse_info', 'fulfillment_status']
    can_delete = False
    ...
    def customization_summary(self, obj):
        ...
        for option_id, customization_data in obj.customizations.items():
            ...
            price_display = f' <span class="admin-text-success">+${escape(str(price))}</span>' if float(price) > 0 else ""
...
class OrderAdmin(CustomFieldsAdminMixin, admin.ModelAdmin):
    ...
    list_display = ['order_number', 'user', 'test_order_badge', 'status_badge', 'payment_status_badge', 'source_badge', 'total_amount', 'created_at']
    list_filter = ['status', 'payment_status', 'is_test_order', 'source', 'created_at', 'updated_at']
    search_fields = ['order_number', 'user__username', 'user__email', 'user__first_name', 'user__last_name', 'phone', 'email', 'items__product__name']
    readonly_fields = [
    ...
    actions = [
        ...
        'mark_as_paid', 'mark_as_unpaid', 'mark_payment_pending',
    ...
    fieldsets = (
        (_('Order Information'), {
            'fields': ('order_number', 'user', 'status', 'source', 'tracking_number', 'item_count')
        }),
        (_('Customer Information'), {
            'fields': ('email', 'phone')
        }),
        (_('Shipping Address'), {
            'fields': ('shipping_name', 'shipping_address1', 'shipping_address2',
                      'shipping_city', 'shipping_state', 'shipping_postal_code',
                      'shipping_country')
        }),
        (_('Billing Address'), {
            'fields': ('billing_same_as_shipping', 'billing_name', 'billing_address1',
        ...
        (_('Payment Information'), {
            'fields': ('payment_status', 'payment_provider', 'payment_method_type',
                      'payment_method_last4', 'paid_at', 'amount_paid', 'amount_refunded'),
            'description': _('Payment status and details for this order.')
        }),
        (_('Currency & Exchange Rate'), {
            'fields': ('customer_currency', 'base_currency', 'exchange_rate_used',
                      'exchange_rate_provider', 'fx_policy'),
            'classes': ('collapse',),
            'description': _('Multi-currency details. Shows the customer\'s payment currency and the exchange rate used to compute base-currency equivalents for reporting.')
        }),
        (_('Documents (Phase 6)'), {
            'fields': ('packing_slip_preview', 'commercial_invoice_preview', 'customs_form_preview'),
            'classes': ('collapse',),
            'description': _('Shipping documents are generated from the order\'s shipment. If no shipment exists, documents cannot be generated.')
        }),
        (_('Display Preferences'), {
            'fields': ('order_page_layout', 'show_order_progress', 'show_shipping_updates', 'show_item_images'),
            'classes': ('collapse',)
        }),
        (_('Notes'), {
    ...
    def packing_slip_preview(self, obj):
        ...
        if not shipment.packing_slip_url:
            return format_html('<em class="admin-text-muted">{}</em>', _('No packing slip available'))
    ...
    def commercial_invoice_preview(self, obj):
        ...
        if not shipment.commercial_invoice_url:
            return format_html('<em class="admin-text-muted">{}</em>', _('No commercial invoice available'))
    ...
    def customs_form_preview(self, obj):
        ...
        if not shipment.customs_form_url:
            return format_html('<em class="admin-text-muted">{}</em>', _('No customs form available'))
    ...
    def generate_licenses_now(self, request, queryset):
        ...
        for order in queryset:
            ...
            items_needing_license = order.items.filter(
    ...
    def generate_packing_slips(self, request, queryset):
        ...
        if skipped_count > 0:
            self.message_user(
                request,
                _('Skipped %(count)d order(s) without shipments.') % {
                    'count': skipped_count
                },
                level='warning'
            )
    ...
    def add_tracking_view(self, request, object_id):
        ...
        try:
            ...
            messages.success(
                request,
                _(f'Tracking number {tracking_number} added via {carrier.name}')
            )
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        try:
            ...
            User = get_user_model()
            customers = User.objects.filter(is_active=True).order_by('email')[:100]  # Limit to 100 for performance
            extra_context['customers'] = customers

            # Get order notes for timeline (including system notes for payment changes, etc.)
            order_notes = OrderNote.objects.filter(order=order).select_related('author').order_by('-created_at')
            ...
            try:
                ...
                if action == 'mark_as_processing':
                    ...
                    if new_status and new_status in valid_statuses and new_status != order.status:
                        ...
                        if new_status == 'refunded':
                            try:
                                ...
                                if gc_count > 0:
                                    messages.info(request, _('%(count)d gift card(s) processed for refund') % {'count': gc_count})
                            except Exception as e:
                                logger.error(f"Gift card refund processing failed for order {order.order_number}: {e}")
                        ...
                        messages.success(request, _(f'Order status changed to {order.get_status_display()}'))
                    ...
                    if payment_reference:
                        order.payment_method_last4 = payment_reference[-4:] if len(payment_reference) > 4 else payment_reference
                        ...
                        try:
                            ...
                            if not program:
                                ...
                                if program.commission_type == 'percentage':
                                    commission_amount = (order.total_amount.amount * Decimal(program.commission_value)) / Decimal(100)
                                else:
                                ...
                                messages.success(request, _(f'Order attributed to {affiliate.user.get_full_name()}'))
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        stats = Order.objects.aggregate(
            ...
            total_revenue=Sum('total_amount', filter=Q(status__in=['processing', 'shipped', 'delivered']))
        )
...
class AddressAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'name', 'city', 'state', 'address_type_badge',
        'is_default', 'version_info', 'usage_count', 'is_active'
    ]
    list_filter = ['address_type', 'is_default', 'is_active', 'country', 'version']
    search_fields = ['user__username', 'user__email', 'name', 'city', 'address1']
    readonly_fields = [
        'created_at', 'updated_at', 'edited_at', 'version',
        'original_address', 'order_usage'
    ]

    fieldsets = (
        (_('Address Information'), {
            'fields': (
                'user', 'address_type', 'is_default', 'is_active',
    ...
    def version_info(self, obj):
        ...
        if obj.version > 1:
            return format_html(
                '<span class="admin-text-warning admin-text-bold">v{}</span>',
                obj.version
            )
    ...
    def order_usage(self, obj):
        ...
        if count > 0:
            return format_html(
                ...
                reverse('admin:orders_order_changelist'), obj.pk, _('View as Shipping'),
                reverse('admin:orders_order_changelist'), obj.pk, _('View as Billing')
            )
...
class OrderNoteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('order', 'author', 'note', 'is_customer_note')
        }),
        (_('Timestamps'), {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class RefundAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Item Details'), {
            'fields': ('items_json',),
            'description': 'Item-level refund details in JSON format'
        }),
        (_('Notes'), {
...
class ReturnRequestAdmin(admin.ModelAdmin):
            ...
            try:
                ...
                if obj:
                    ...
                    try:
                        ...
                        currency = obj.order.total_amount.currency.code if hasattr(obj.order.total_amount, 'currency') else get_default_currency()
    ...
    def approve_returns(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d return request(s) approved. Return labels should be generated and sent to customers.') % {
                'count': updated
            },
            level='success'
        )
    ...
    def reject_returns(self, request, queryset):
        ...
        if pending_count > 0:
            self.message_user(
                request,
                _('Cannot bulk reject returns. Please reject individually to provide rejection reasons.'),
                level='warning'
            )
        else:
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['in_transit_count'] = ReturnRequest.objects.filter(status='in_transit').count()
