...
class ShippingMethodWizardStep1Form(forms.Form):
    ...
    carrier_service_code = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': 'e.g., PRIORITY, GROUND'
        })
    )
    ...
    def _get_configured_carrier_choices(self):
        ...
        try:
            ...
            active_accounts = ProviderAccount.objects.filter(
                is_active=True
            ).select_related('component').order_by('component__name')
            ...
            for account in active_accounts:
                try:
                    ...
                    if capabilities.get('rates', False):
                        ...
                        choices.append((
                            account.component.slug,
                            f"{display_name} ({account.component.name})"
                        ))
