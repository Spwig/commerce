...
class MethodWizardStep4View(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if method_data.get("method_type") == "local_pickup":
            context["all_pickup_locations"] = Location.objects.filter(accepts_pickup=True).order_by(
                "name"
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if wizard_data.get("method_type") == "local_pickup":
            ...
            wizard_data["pickup_locations"] = (
                [int(loc_id) for loc_id in pickup_locations] if pickup_locations else []
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if wizard_data.get("flat_rate_cost"):
                try:
                    flat_rate_cost = Money(
                        Decimal(str(wizard_data["flat_rate_cost"])), get_default_currency()
                    )
            ...
            if wizard_data.get("method_type") == "table_rate":
                ...
                messages.info(request, _("Please configure rate tables for this method."))
            ...
            messages.error(
                request, _("Failed to create shipping method: %(error)s") % {"error": str(e)}
            )
            return redirect("cart:method_wizard_step5")
