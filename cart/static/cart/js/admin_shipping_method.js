...
(function() {
    ...
    function handleToggleMethod(button) {
        ...
        setTimeout(() => {
            ...
            hideLoading();
            showNotification(
                `Method ${newStatus ? 'enabled' : 'disabled'} successfully`,
                'success'
            );
        }, 300);
    }
    ...
    async function handleDeleteMethod(button) {
        ...
        if (!await AdminModal.confirm({ message: `Are you sure you want to delete "${methodName}"?`, danger: true, confirmText: 'Delete' })) {
            return;
        }
    ...
    function applyFilters() {
        state.filteredMethods = state.allMethods.filter(method => {
            ...
            // Search filter
            if (state.currentFilters.search && !method.name.toLowerCase().includes(state.currentFilters.search)) {
                return false;
            }
