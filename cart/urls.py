...
from .method_wizard import (
    MethodWizardStep1View,
    MethodWizardStep2View,
    MethodWizardStep3View,
    MethodWizardStep4View,
    MethodWizardStep5View,
)
from .mini_cart_api import (
    cart_empty_recommendations,
    mini_cart_get,
    mini_cart_remove,
    mini_cart_update,
)
...
urlpatterns = [
    ...
    path(
        "cart/empty-recommendations/", cart_empty_recommendations, name="cart-empty-recommendations"
    ),
    # Cart endpoints
    ...
    path(
        "cart/apply-voucher/",
        CartViewSet.as_view({"post": "apply_voucher"}),
        name="cart-apply-voucher",
    ),
    path(
        "cart/remove-voucher/<str:code>/",
        CartViewSet.as_view({"delete": "remove_voucher"}),
    ...
    path(
        ...
        CheckoutViewSet.as_view({"post": "set_billing_address"}),
        name="checkout-billing-address",
    ),
    path(
        "checkout/shipping-methods/",
        CheckoutViewSet.as_view({"get": "get_shipping_methods"}),
        name="checkout-shipping-methods",
    ),
    path(
        "checkout/shipping-method/",
        CheckoutViewSet.as_view({"post": "set_shipping_method"}),
        name="checkout-shipping-method",
    ),
    path(
        "checkout/payment-providers/",
        CheckoutViewSet.as_view({"get": "get_payment_providers"}),
        name="checkout-payment-providers",
    ),
    path(
        "checkout/payment-method/",
        CheckoutViewSet.as_view({"post": "set_payment_method"}),
    ...
    path(
        "checkout/complete/",
        CheckoutViewSet.as_view({"post": "complete"}),
