...
from .models import (
    Cart, CartItem, Wishlist, WishlistItem, RecentlyViewed,
    ShippingMethod, TaxRate, CheckoutSession
)
...
class CartAdmin(admin.ModelAdmin):
    ...
    list_display = ['__str__', 'user', 'total_items', 'total_amount', 'shipping_cost', 'grand_total', 'updated_at']
    list_filter = ['cart_layout', 'checkout_flow', 'shipping_method', 'created_at']
    search_fields = ['user__username', 'session_key']
    readonly_fields = ['total_items', 'total_amount', 'total_savings', 'grand_total', 'requires_shipping', 'total_weight']
    ...
    fieldsets = (
        ...
        ('Layout & Display', {
            'fields': ('cart_layout', 'show_product_images', 'show_product_variants',
                      'show_remove_button', 'show_quantity_controls')
        }),
        ('Pricing Display', {
            'fields': ('show_item_totals', 'show_cart_summary', 'show_savings')
        }),
        ('Checkout', {
            'fields': ('checkout_flow',)
        }),
        ...
        ('Summary', {
            'fields': ('total_items', 'total_amount', 'total_savings', 'requires_shipping',
...
class ShippingMethodAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('cart/css/admin_shipping_method.css',)
        }
        ...
        if db_field.name == 'icon':
            return forms.CharField(
                widget=IconPickerWidget(
                    priority_icons=[
                        'fa-truck', 'fa-truck-fast', 'fa-box',
                        'fa-warehouse', 'fa-dolly', 'fa-cube',
                        'fa-ship', 'fa-plane', 'fa-bicycle',
                    ],
                    style_prefix=False,
                ),
                required=False,
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if object_id:
            ...
            if obj:
                ...
                targeting_all = ShippingPromotion.objects.filter(
                    is_active=True
                ).annotate(
    ...
    def bulk_action_view(self, request):
        ...
        try:
            ...
            if not action or not method_ids:
                return JsonResponse({
                    'success': False,
                    'message': _('Action and method IDs required')
                }, status=400)
...
class TaxRateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Geographic Scope'), {
            'fields': ('country', 'state', 'city', 'postal_codes'),
            'classes': ('tab-geography',)
        }),
        (_('Application Rules'), {
            'fields': ('applies_to_shipping', 'compound'),
            'classes': ('tab-rules',)
        }),
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['countries'] = (
            TaxRate.objects.values('country')
        ...
        tax_rates = (
            qs.prefetch_related('exempt_categories')
            .order_by('-priority', 'country', 'state', 'city')
        )
...
class CheckoutSessionAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        'cart', 'subtotal', 'tax_amount', 'discount_amount', 'total_amount',
        'tax_breakdown', 'available_shipping_methods', 'created_at', 'updated_at'
    ]

    fieldsets = (
        ('Cart & Session', {
            'fields': ('cart', 'step_completed', 'expires_at')
        }),
        ('Addresses', {
            'fields': ('shipping_address', 'billing_address', 'billing_same_as_shipping')
        }),
        ('Shipping', {
            'fields': ('selected_shipping_method', 'shipping_cost', 'estimated_delivery_date',
