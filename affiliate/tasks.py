...
@shared_task(name='affiliate.send_affiliate_monthly_reports', base=BackgroundDBTask, ignore_result=True)
def send_affiliate_monthly_reports():
    ...
    """
    from affiliate.models import AffiliateReportSettings, Affiliate, Commission
    ...
    if today.day != settings.monthly_report_day:
        logger.debug(f"Not send day (today: {today.day}, configured: {settings.monthly_report_day})")
    ...
    if today.hour != settings.monthly_report_hour:
        logger.debug(f"Not send hour (now: {today.hour}, configured: {settings.monthly_report_hour})")
    ...
    for affiliate in affiliates:
        try:
            ...
            avg_commission = total_earned / commission_count if commission_count > 0 else Decimal('0.00')

            # Get top N orders by commission amount
            top_orders_data = []
            top_orders = commissions.select_related('order').order_by('-amount')[:settings.include_top_orders_count]
            ...
            latest_payout = Payout.objects.filter(affiliate=affiliate).order_by('-created_at').first()
            ...
            if outbox.status == 'skipped':
                logger.info(f"Skipped monthly report to {affiliate.user.email} - preference disabled")
                skipped_count += 1
            elif outbox.status in ['pending', 'queued']:
                logger.info(f"Queued monthly report to {affiliate.user.email}")
                sent_count += 1
            else:
                logger.warning(f"Failed to queue monthly report to {affiliate.user.email}: {outbox.status}")
                skipped_count += 1

        except Exception as e:
            logger.error(f"Error sending monthly report to {affiliate.user.email}: {e}", exc_info=True)
            skipped_count += 1
            continue
