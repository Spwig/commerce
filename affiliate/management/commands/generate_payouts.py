...
from affiliate.models import Commission, Payout, Affiliate
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=float,
            default=50.00,
            help='Minimum payout amount (default: 50.00)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
            help='Preview payouts without creating them'
        )
        parser.add_argument(
            '--affiliate-id',
            type=int,
            help='Generate payout for specific affiliate ID only'
        )
    ...
    def handle(self, *args, **options):
        ...
        affiliate_totals = approved_commissions.values(
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Found {len(affiliate_totals)} affiliate(s) eligible for payout:\n"
        ))
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS(
            f"Total payout amount: ${total_payout_amount:.2f}"
        ))
        self.stdout.write(self.style.SUCCESS(
            f"Total commissions: {total_commissions}"
        ))
        self.stdout.write("")

        if dry_run:
            self.stdout.write(self.style.WARNING(
                "DRY RUN - No payouts created. Remove --dry-run to execute."
            ))
            return
        ...
        for item in affiliate_totals:
            try:
                with transaction.atomic():
                    ...
                    if commissions.aggregate(total=Coalesce(Sum('amount_base'), Sum('amount')))['total'] < min_amount:
                        self.stdout.write(self.style.WARNING(
                            f"  ✗ Skipping {affiliate.user.username}: "
