...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=365,
            help='Delete clicks older than N days (default: 365)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if respect_cookie_lifetime:
            ...
            expired_clicks = Click.objects.filter(
        ...
        if total_expired == 0:
            self.stdout.write(self.style.SUCCESS(
                f"✓ No clicks found older than {retention_days} days"
            ))
            return

        # Get breakdown by program
        program_stats = expired_clicks.values(
            'link__program__name'
        ).annotate(
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING(
                "DRY RUN - No clicks deleted. Remove --dry-run to execute."
            ))
            # Show sample of clicks that would be deleted
            sample_clicks = expired_clicks.select_related(
                'link', 'link__program'
            )[:5]
        ...
        self.stdout.write(self.style.WARNING(
            f"Deleting {total_expired:,} clicks in batches of {batch_size}..."
        ))
        self.stdout.write("")
        ...
        while True:
            ...
            batch_ids = list(
                expired_clicks.values_list('id', flat=True)[:batch_size]
            )
        ...
        self.stdout.write(self.style.SUCCESS('=' * 70))
        self.stdout.write(self.style.SUCCESS(
            f"✓ Deleted {deleted_total:,} expired clicks"
        ))
        self.stdout.write(self.style.SUCCESS(
