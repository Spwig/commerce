...
class ProgramAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("name", "slug", "merchant", "description", "status")}),
        (
        ...
        (_("Settings"), {"fields": ("auto_approve_affiliates",), "classes": ("collapse",)}),
        (_("Statistics"), {"fields": ("program_stats",), "classes": ("collapse",)}),
    ...
    def status_badge(self, obj):
        ...
        colors = {"active": "success", "paused": "warning", "archived": "secondary"}
        icons = {"active": "fa-check-circle", "paused": "fa-pause-circle", "archived": "fa-archive"}
        return format_html(
    ...
    def affiliates_count(self, obj):
        ...
        count = obj.affiliates.filter(affiliateprogrammembership__status="approved").count()
        return format_html(
            '<a href="{}?programs__id__exact={}"><i class="fas fa-users"></i> {}</a>',
    ...
    def revenue_this_month(self, obj):
        ...
        total = (
            obj.commissions.filter(
                created_at__gte=start_of_month, status__in=["approved", "paid"]
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        try:
            ...
            conversions = program.commissions.filter(status__in=["approved", "paid"]).count()
...
class AffiliateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            ...
            {"fields": ("company_name", "website"), "classes": ("collapse",)},
        ),
        (
        ...
        (_("Statistics"), {"fields": ("affiliate_stats",), "classes": ("collapse",)}),
        (_("Timestamps"), {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def customer_link(self, obj):
        ...
        try:
            ...
            return format_html(
                '<a href="{}" title="View customer profile"><i class="fas fa-user"></i> {}</a>',
                url,
                display_name,
            )
        except Exception:
            # Fallback if no profile exists
            return format_html('<i class="fas fa-user-slash text-muted"></i> {}', obj.user.username)
    ...
    def programs_count(self, obj):
        ...
        count = obj.programs.filter(affiliateprogrammembership__status="approved").count()
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        try:
            ...
            conversions = affiliate.commissions.filter(status__in=["approved", "paid"]).count()

            # Get memberships for the right column
            memberships = (
                AffiliateProgramMembership.objects.filter(affiliate=affiliate)
                .select_related("program")
                .order_by("-applied_at")
            )

            # Get recent commissions
            recent_commissions = affiliate.commissions.select_related("program").order_by(
                "-created_at"
            )[:5]
...
class AffiliateProgramMembershipAdmin(admin.ModelAdmin):
    ...
    list_display = ("affiliate", "program", "status_badge", "applied_at", "approved_at")
    ...
    fieldsets = (
        (_("Membership"), {"fields": ("affiliate", "program", "status")}),
        (
            _("Notes"),
            {
                "fields": ("notes",),
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        memberships = (
            AffiliateProgramMembership.objects.select_related("affiliate__user", "program")
            .annotate(
                ...
                links_count=Count(
                    "affiliate__links", filter=Q(affiliate__links__program=F("program"))
                ),
            )
            .order_by("-applied_at")
        )
...
class LinkAdmin(admin.ModelAdmin):
    ...
    search_fields = ("link_code", "affiliate__affiliate_code", "label", "destination_url")
    ...
    fieldsets = (
        ...
        (_("Tracking"), {"fields": ("tracking_url", "link_stats"), "classes": ("collapse",)}),
        (_("Timestamps"), {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def clicks_count(self, obj):
        ...
        return format_html(
            '<a href="{}?link__id__exact={}"><i class="fas fa-mouse-pointer"></i> {}</a>',
...
class ClickAdmin(admin.ModelAdmin):
    ...
    list_display = ("link", "ip_address", "clicked_at", "has_conversion")
    list_filter = ("clicked_at",)
    search_fields = (
        "ip_address",
        "link__link_code",
        "link__affiliate__affiliate_code",
        "session_id",
        "cookie_value",
    )
    ...
    fieldsets = (
        (_("Click Information"), {"fields": ("link", "ip_address", "clicked_at")}),
        (
            _("Technical Details"),
            {
        ...
        (_("Additional Information"), {"fields": ("click_details",), "classes": ("collapse",)}),
    )
    ...
    def has_conversion(self, obj):
        ...
        return format_html(
            '<span class="badge badge-secondary"><i class="fas fa-times"></i> {}</span>', _("No")
        )
    ...
    def click_details(self, obj):
        ...
        if commissions:
            ...
            commission_html = (
                f"<p><em><i class='fas fa-info-circle'></i> {_('No conversions yet')}</em></p>"
            )
...
class CommissionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Timestamps"),
            {"fields": ("created_at", "approved_at", "paid_at"), "classes": ("collapse",)},
        ),
    )
    ...
    def order_link(self, obj):
        """Link to order admin"""
        return format_html(
            '<a href="{}" target="_blank"><i class="fas fa-external-link-alt"></i> Order #{}</a>',
    ...
    def status_badge(self, obj):
        """Display status with colored badge"""
        colors = {"pending": "warning", "approved": "info", "rejected": "danger", "paid": "success"}
        icons = {
            "pending": "fa-clock",
            "approved": "fa-check",
            "rejected": "fa-times",
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["affiliates"] = (
            Affiliate.objects.select_related("user")
            .filter(commissions__isnull=False)
            .distinct()
            .order_by("user__username")
        )
        extra_context["programs"] = (
            Program.objects.filter(commissions__isnull=False).distinct().order_by("name")
        )

        # Get recent commissions for card display (limit 20)
        extra_context["recent_commissions"] = (
            Commission.objects.select_related("affiliate__user", "program", "order")
...
class PayoutAdmin(admin.ModelAdmin):
    ...
    search_fields = ("affiliate__affiliate_code", "affiliate__user__username", "reference", "notes")
    readonly_fields = (
        "created_at",
        "processed_at",
        "completed_at",
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["pending_amount"] = (
            all_payouts.filter(status="pending").aggregate(total=Sum("amount"))["total"] or 0
        )
        ...
        extra_context["completed_amount"] = (
            all_payouts.filter(status="completed").aggregate(total=Sum("amount"))["total"] or 0
        )
        ...
        extra_context["method_choices"] = all_payouts.values_list("method", flat=True).distinct()

        # Payouts for display
        extra_context["payouts"] = (
            self.get_queryset(request)
            .select_related("affiliate__user", "provider_account")
    ...
    def mark_as_processing(self, request, queryset):
        ...
        self.message_user(
            request,
            _("Successfully marked {} payout(s) as processing and notified affiliates.").format(
                count
            ),
    ...
    def mark_as_completed(self, request, queryset):
        ...
        if count > 0:
            self.message_user(
                request,
                _(
                    "Successfully completed {} payout(s) covering {} commission(s). Commissions marked as paid and affiliates notified."
                ).format(count, commission_count),
                level="success",
            )
        else:
            self.message_user(
    ...
    def cancel_payouts(self, request, queryset):
        ...
        if count > 0:
            self.message_user(
                request,
                _(
                    "Cancelled {} payout(s). {} commission(s) reverted to approved status. Affiliates notified."
                ).format(count, commission_count),
                level="warning",
            )
        else:
            self.message_user(request, _("No payouts to cancel."), level="info")
    ...
    def process_with_provider(self, request, queryset):
        ...
        """
        from payout_providers.models import PayoutProviderAccount
        from payout_providers.tasks import process_batch_payouts, process_payout

        # Get available provider accounts
        paypal_account = PayoutProviderAccount.objects.filter(
            provider_type="paypal", is_active=True, is_default=True
        ).first()

        airwallex_account = PayoutProviderAccount.objects.filter(
            provider_type="airwallex", is_active=True, is_default=True
        ).first()
        ...
        if paypal_payout_ids and paypal_account:
            try:
                ...
                self.message_user(
                    request, _("Failed to queue PayPal payouts: {}").format(str(e)), level="error"
                )
        ...
        if queued_count:
            self.message_user(
                request,
                _(
                    "Queued {} payout(s) for async processing. Status will update via webhooks."
                ).format(queued_count),
                level="success",
            )
        ...
        if skipped_count:
            self.message_user(
                request,
                _("Skipped {} payout(s) - no provider available for their payment method.").format(
                    skipped_count
                ),
                level="warning",
            )
    ...
    fieldsets = (
        ...
        (
            _("Features Section"),
            {
                ...
                "description": _(
                    "List of features to display. Each item should have: icon (FontAwesome class), title, and description"
                ),
            },
        ),
        (
            _("How It Works Section"),
            {
                ...
                "description": _(
                    "Steps showing how the affiliate program works. Each item should have: title and description"
                ),
            },
        ),
        (
        ...
        (
            _("Registration Form"),
            {
                ...
                "description": _(
                    "Configure the affiliate registration form. Leave form blank to use default fields."
                ),
            },
        ),
        (
    ...
    fieldsets = (
        (
            _("Monthly Report Settings"),
            {
                ...
                "description": _(
                    "Configure when and how monthly performance reports are sent to affiliates"
                ),
            },
        ),
