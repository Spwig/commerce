...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function validateStep(step) {
      ...
      if (step === 2) {
        ...
        if (!commissionValue || parseFloat(commissionValue) <= 0) {
          showError(
            'commission_value',
            t.validCommissionValue || 'Please enter a valid commission value.'
          );
          isValid = false;
    ...
    autoApproveCheckbox.addEventListener('change', function () {
      if (this.checked) {
        approvalMessage.textContent =
          t.autoApproveOn || 'New affiliates will be automatically approved.';
      } else {
        approvalMessage.textContent =
          t.autoApproveOff || 'You will manually review and approve each affiliate application.';
      }
    });
    ...
    function updateReview() {
      ...
      document.getElementById('review-cookie').textContent =
        document.getElementById('cookie_lifetime_days').value + ' ' + (t.days || 'days');

      const autoApprove = document.getElementById('auto_approve_affiliates').checked;
      document.getElementById('review-auto-approve').textContent = autoApprove
        ? t.yes || 'Yes'
        : t.no || 'No';

      document.getElementById('review-min-payout').textContent =
        '$' + document.getElementById('minimum_payout').value;
    }
    ...
    form.addEventListener('submit', function (e) {
      ...
      btnCreate.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> ' + (t.creating || 'Creating...');
      ...
      fetch(form.action || window.location.href, {
        ...
        .then(data => {
          if (data.success) {
            ...
            btnCreate.innerHTML =
              '<i class="fas fa-check"></i> ' + (t.createProgram || 'Create Program');
