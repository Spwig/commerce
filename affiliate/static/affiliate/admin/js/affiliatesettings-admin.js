...
(function () {
    ...
    var T = {};
    var translationsEl = document.getElementById('affiliatesettings-translations');
    if (translationsEl) {
        try { T = JSON.parse(translationsEl.textContent); } catch (e) {}
    }
    ...
    var heroTitleField, heroSubtitleField, featuresTitleField, featuresField;
    var howItWorksTitleField, stepsField, ctaTitleField, ctaDescriptionField;
    var previewHeroTitle, previewHeroSubtitle, previewFeaturesTitle, previewFeaturesGrid;
    var previewStepsTitle, previewStepsContainer, previewCtaTitle, previewCtaDescription;
    ...
    function createFeatureItem(feature, index) {
        ...
        item.innerHTML =
            '<div class="json-item-header">' +
                '<span class="item-number">' + T.feature + ' #' + (index + 1) + '</span>' +
                '<div class="json-item-actions">' +
                    '<button type="button" class="move-up-btn" title="' + T.moveUp + '"><i class="fas fa-arrow-up"></i></button>' +
                    '<button type="button" class="move-down-btn" title="' + T.moveDown + '"><i class="fas fa-arrow-down"></i></button>' +
            ...
            '<div class="json-item-field">' +
                ...
                '<div class="icon-selector-wrapper">' +
                    '<div class="icon-preview"><i class="' + (feature.icon || 'fas fa-star') + '"></i></div>' +
            ...
            '<div class="json-item-field">' +
                ...
                '<div class="field-with-translate">' +
                    '<input type="text" class="feature-title" value="' + escapeHtml(feature.title || '') + '">' +
                    '<button type="button" class="translate-btn" data-field="features.' + index + '.title" title="' + T.translate + '">' +
            ...
            '<div class="json-item-field">' +
                ...
                '<div class="field-with-translate">' +
                    '<textarea class="feature-description">' + escapeHtml(feature.description || '') + '</textarea>' +
                    '<button type="button" class="translate-btn" data-field="features.' + index + '.description" title="' + T.translate + '">' +
                        '<i class="fas fa-language"></i>' +
                    '</button>' +
                '</div>' +
            '</div>';

        var iconContainer = item.querySelector('.icon-select-container');
        var iconSelect = createIconSelect(feature.icon || 'fas fa-star');
        ...
        item.querySelectorAll('.translate-btn').forEach(function (btn) {
            btn.addEventListener('click', function () { openTranslationEditor(btn.dataset.field, 'text'); });
        });
    ...
    function createStepItem(step, index) {
        ...
        item.innerHTML =
            '<div class="json-item-header">' +
                '<span class="item-number">' + T.step + ' #' + (index + 1) + '</span>' +
                '<div class="json-item-actions">' +
                    '<button type="button" class="move-up-btn" title="' + T.moveUp + '"><i class="fas fa-arrow-up"></i></button>' +
                    '<button type="button" class="move-down-btn" title="' + T.moveDown + '"><i class="fas fa-arrow-down"></i></button>' +
                    '<button type="button" class="delete-btn" title="' + T.delete + '"><i class="fas fa-trash"></i></button>' +
            ...
            '<div class="json-item-field">' +
                ...
                '<div class="field-with-translate">' +
                    ...
                    '<button type="button" class="translate-btn" data-field="steps.' + index + '.title" title="' + T.translate + '">' +
                        '<i class="fas fa-language"></i>' +
                    '</button>' +
                '</div>' +
            '</div>' +
            '<div class="json-item-field">' +
                '<label>' + T.description + '</label>' +
                '<div class="field-with-translate">' +
                    '<textarea class="step-description">' + escapeHtml(step.description || '') + '</textarea>' +
                    '<button type="button" class="translate-btn" data-field="steps.' + index + '.description" title="' + T.translate + '">' +
                        '<i class="fas fa-language"></i>' +
                    '</button>' +
                '</div>' +
            '</div>';
    ...
    function renumberItems(type) {
        var container = document.getElementById(type + '-editor');
        if (!container) return;

        var label = type === 'features' ? T.feature : T.step;
        container.querySelectorAll('.json-item').forEach(function (item, idx) {
            item.dataset.index = idx;
            item.querySelector('.item-number').textContent = label + ' #' + (idx + 1);

            item.querySelectorAll('.translate-btn').forEach(function (btn) {
                var currentField = btn.dataset.field;
                if (currentField) {
                    var parts = currentField.split('.');
                    if (parts.length === 3) {
    ...
    async function openTranslationEditor(fieldPath) {
        ...
        var objectId = objectIdMatch[1];
        var modelType = 'affiliate.affiliatesettings';
        ...
        if (parts.length === 3) {
            ...
            var container = document.getElementById(type + '-editor');
            if (container) {
                var items = container.querySelectorAll('.json-item');
                if (items[index]) {
                    var inputClass = type === 'features' ? 'feature-' + field : 'step-' + field;
                    var input = items[index].querySelector('.' + inputClass);
        ...
        try {
            var statusResponse = await fetch(endpoints.status);
        ...
        var fieldLabel = fieldPath.replace(/\./g, ' › ').replace(/_/g, ' ').replace(/\b\w/g, function (l) { return l.toUpperCase(); });
        ...
        modal.innerHTML =
            '<div class="json-translation-content" style="background: var(--body-bg, #fff); color: var(--body-fg, #333); border-radius: 8px; max-width: 700px; width: 90%; max-height: 85vh; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.2); display: flex; flex-direction: column;">' +
                '<div class="json-translation-header" style="padding: 16px 20px; border-bottom: 1px solid var(--hairline-color, #ddd); display: flex; justify-content: space-between; align-items: center;">' +
                    '<h3 style="margin: 0; font-size: 16px;"><i class="fas fa-globe"></i> ' + T.translate + ': ' + fieldLabel + '</h3>' +
                ...
                '<div class="json-translation-body" style="padding: 20px; overflow-y: auto; flex: 1;">' +
                    '<div style="margin-bottom: 16px;">' +
                        '<label style="font-weight: 600; display: block; margin-bottom: 6px;">' + T.sourceText + ':</label>' +
                        '<div style="padding: 10px; background: var(--darkened-bg, #f5f5f5); border-radius: 4px; font-size: 13px;">' + escapeHtml(currentValue) + '</div>' +
                    '</div>' +
                    '<div style="margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center;">' +
                        '<label style="font-weight: 600;">' + T.targetLanguages + ':</label>' +
                        '<span style="font-size: 12px; color: var(--body-quiet-color, #666);">' + T.coverage + ': ' + (coverage.percentage || 0) + '%</span>' +
                    '</div>' +
                ...
                '<div class="json-translation-footer" style="padding: 16px 20px; border-top: 1px solid var(--hairline-color, #ddd); display: flex; justify-content: flex-end; gap: 10px;">' +
                    '<button type="button" class="cancel-btn" style="padding: 8px 16px; border: 1px solid var(--hairline-color, #ddd); background: var(--body-bg, #fff); color: var(--body-fg, #333); cursor: pointer; border-radius: 4px;">' + T.cancel + '</button>' +
                    '<button type="button" class="save-btn" style="padding: 8px 16px; border: 1px solid var(--hairline-color, #ddd); background: var(--body-bg, #fff); color: var(--body-fg, #333); cursor: pointer; border-radius: 4px;" disabled><i class="fas fa-save"></i> ' + T.saveEdits + '</button>' +
                    '<button type="button" class="translate-action-btn" style="padding: 8px 16px; border: none; background: var(--primary, #417690); color: white; cursor: pointer; border-radius: 4px;"><i class="fas fa-language"></i> ' + T.translate + '</button>' +
                '</div>' +
            '</div>';
        ...
        var messageEl = modal.querySelector('.translation-message');
        var saveBtn = modal.querySelector('.save-btn');
        var translateBtn = modal.querySelector('.translate-action-btn');
        ...
        availableLanguages.forEach(function (lang) {
            ...
            langItem.style.cssText = 'padding: 10px; border: 1px solid var(--hairline-color, #ddd); border-radius: 4px; background: var(--darkened-bg, #f8f8f8);' + (isTranslated ? ' border-left: 3px solid var(--primary, #417690);' : '');
            langItem.innerHTML =
                '<div style="display: flex; align-items: center; gap: 10px; margin-bottom: ' + (isTranslated ? '8px' : '0') + ';">' +
                    '<input type="checkbox" id="lang_' + lang.code + '" value="' + lang.code + '" ' + (isTranslated ? '' : 'checked') + ' style="width: 16px; height: 16px;">' +
                    '<label for="lang_' + lang.code + '" style="flex: 1; margin: 0; font-weight: 500;">' + lang.name +
                        (isTranslated ? ' <span style="color: var(--primary, #417690); margin-left: 6px; font-size: 11px;"><i class="fas fa-check-circle"></i> ' + T.translated + '</span>' : '') +
                    '</label>' +
                '</div>' +
                (isTranslated ? '<textarea class="translation-edit" data-lang="' + lang.code + '" data-original="' + escapeHtml(existingTranslation) + '" style="width: 100%; padding: 8px; border: 1px solid var(--hairline-color, #ddd); border-radius: 4px; font-size: 13px; min-height: 50px; resize: vertical; box-sizing: border-box;">' + escapeHtml(existingTranslation) + '</textarea>' : '');
        ...
        saveBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                var saveResponse = await fetch(endpoints.save, {
                    method: 'POST',
                ...
                if (result.success) {
                    messageEl.innerHTML = '<div style="padding: 10px; background: #d4edda; border-radius: 4px; color: #155724;"><i class="fas fa-check-circle"></i> ' + T.savedSuccessfully + '</div>';
                    result.saved_languages.forEach(function (lang) {
                        var textarea = languagesList.querySelector('.translation-edit[data-lang="' + lang + '"]');
                        if (textarea) textarea.dataset.original = textarea.value;
                    });
                    updateSaveButton();
                }
            } catch (error) {
                messageEl.innerHTML = '<div style="padding: 10px; background: #f8d7da; border-radius: 4px; color: #721c24;"><i class="fas fa-exclamation-triangle"></i> ' + error.message + '</div>';
            } finally {
        ...
        translateBtn.addEventListener('click', async function () {
            ...
            messageEl.innerHTML = '<div style="padding: 10px; background: #e7f3ff; border-radius: 4px; color: #004085;"><i class="fas fa-spinner fa-spin"></i> ' + T.processingTranslation + '</div>';

            try {
                var csrfToken = document.querySelector('[name=csrfmiddlewaretoken]')?.value;
                var transResponse = await fetch(endpoints.translate, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrfToken },
                    body: JSON.stringify({ text: currentValue, languages: selectedLanguages, force_immediate: true })
                });

                var transResult = await transResponse.json();
                if (!transResponse.ok) throw new Error(transResult.error || T.translationFailed);

                if (transResult.success) {
                    messageEl.innerHTML = '<div style="padding: 10px; background: #d4edda; border-radius: 4px; color: #155724;"><i class="fas fa-check-circle"></i> ' + T.translatedTo + ' ' + transResult.successful_languages.length + ' ' + T.languages + '</div>';

                    transResult.successful_languages.forEach(function (lang) {
                        var existingTextarea = languagesList.querySelector('.translation-edit[data-lang="' + lang + '"]');
                        if (existingTextarea) {
                            existingTextarea.value = transResult.translations[lang];
                            existingTextarea.dataset.original = transResult.translations[lang];
                        } else {
                            var checkboxEl = languagesList.querySelector('#lang_' + lang);
                            var langItem = checkboxEl ? checkboxEl.closest('div[style]') : null;
                            if (langItem) {
                                ...
                                textarea.style.cssText = 'width: 100%; padding: 8px; border: 1px solid var(--hairline-color, #ddd); border-radius: 4px; font-size: 13px; min-height: 50px; resize: vertical; margin-top: 8px; box-sizing: border-box;';
                                textarea.addEventListener('input', updateSaveButton);
                                langItem.appendChild(textarea);
                                langItem.style.borderLeft = '3px solid var(--primary, #417690)';

                                var labelEl = langItem.querySelector('label');
                                if (labelEl && !labelEl.querySelector('.fa-check-circle')) {
                                    labelEl.innerHTML += ' <span style="color: var(--primary, #417690); margin-left: 6px; font-size: 11px;"><i class="fas fa-check-circle"></i> ' + T.translated + '</span>';
                                }
                            }
                        }
                    });
    ...
    function updateFeatures() {
        var container = document.getElementById('features-editor');
        if (!container) return;
        var features = [];
    ...
    function updateSteps() {
        var container = document.getElementById('steps-editor');
        if (!container) return;
        var steps = [];
        container.querySelectorAll('.json-item').forEach(function (item) {
            steps.push({
    ...
    function updatePreviewFeatures(features) {
        ...
        previewFeaturesGrid.innerHTML = html || '<p style="text-align:center;color:#999;font-size:11px;">' + T.noFeaturesAdded + '</p>';
    }

    function updatePreviewSteps(steps) {
        var html = '';
        var validSteps = steps.filter(function (s) { return s.title; });
        validSteps.forEach(function (s, idx) {
            if (idx > 0) html += '<div class="preview-step-arrow"><i class="fas fa-chevron-right"></i></div>';
            html +=
        ...
        previewStepsContainer.innerHTML = html || '<p style="text-align:center;color:#999;font-size:11px;width:100%;">' + T.noStepsAdded + '</p>';
    }

    function initFeaturesEditor() {
        if (!featuresField) return;
        var fieldRow = featuresField.closest('.form-row');
        if (!fieldRow) return;
        ...
        var features = parseJSON(featuresField.value);
        features.forEach(function (f, idx) { editor.appendChild(createFeatureItem(f, idx)); });
        ...
        addBtn.addEventListener('click', function () {
            var items = editor.querySelectorAll('.json-item');
            editor.insertBefore(createFeatureItem({ icon: 'fas fa-star', title: '', description: '' }, items.length), addBtn);
            updateFeatures();
        });
    ...
    function initStepsEditor() {
        ...
        var fieldRow = stepsField.closest('.form-row');
        if (!fieldRow) return;
        ...
        var steps = parseJSON(stepsField.value);
        steps.forEach(function (s, idx) { editor.appendChild(createStepItem(s, idx)); });
