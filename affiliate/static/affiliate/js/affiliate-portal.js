...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const i18nEl = document.getElementById('affiliate-portal-i18n');
    if (i18nEl) {
      try {
  ...
  function hideRegistrationModal() {
    ...
    const sucEl = document.getElementById('registrationSuccess');
  ...
  function submitRegistration(form) {
    ...
    const isGuestMode = form.dataset.guestMode === 'true';
    const dashboardUrl = form.dataset.dashboardUrl || '/';

    const submittingLabel = i18n.submitting || 'Submitting...';
    const submitLabel = i18n.submitApplication || 'Submit Application';
    ...
    if (isGuestMode && currentAuthMode === 'register') {
      ...
      if (!data.email || !data.password) {
        showError(
          errorDiv,
          i18n.fillEmailPassword || 'Please fill in your email and password to create an account.'
        );
    ...
    if (!data.terms_accepted) {
      showError(
        errorDiv,
        i18n.acceptTerms || 'You must accept the terms and conditions to continue.'
      );
      resetBtn(submitBtn, submitLabel);
      return;
    }
    ...
    fetch('/api/affiliate/affiliates/register/', {
      ...
      .then(function (result) {
        const message =
          result.message ||
          i18n.applicationSubmitted ||
          'Your affiliate application has been submitted successfully!';
        const redirectingMsg = i18n.redirecting || 'Redirecting to your dashboard...';
        if (successDiv) {
          successDiv.innerHTML =
            '<i class="fas fa-check-circle"></i> ' + message + ' ' + redirectingMsg;
      ...
      .catch(function (error) {
        let errorMessage = i18n.generalError || 'An error occurred. Please try again.';
  ...
  function initAffiliatePrograms() {
    document.addEventListener('click', async function (e) {
      const btn = e.target.closest('[data-action="apply-program"]');
      if (!btn) return;

      const programId = btn.dataset.programId;
      const programName = btn.dataset.programName || '';
      const confirmMsg =
        (i18n.confirmApply || 'Would you like to apply to join') + ' "' + programName + '"?';
      ...
      fetch('/api/affiliate/programs/' + programId + '/apply/', {
        ...
        .catch(function (err) {
          ...
          AdminModal.alert({
            message:
              i18n.applicationError || 'Unable to submit application. Please try again later.',
            type: 'error',
          });
        });
    });
  ...
  function hideCreateLinkModal() {
    ...
    if (form) form.reset();
    const errorDiv = document.getElementById('createLinkError');
    if (errorDiv) errorDiv.classList.add('hidden');
  }
  ...
  function initCreateLinkForm() {
    ...
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      const errorDiv = document.getElementById('createLinkError');
      const programSelect = document.getElementById('linkProgram');
      const labelInput = document.getElementById('linkLabel');
      const destInput = document.getElementById('linkDestination');
          ...
          if (errorDiv) {
            ...
            if (err && typeof err === 'object') {
              Object.keys(err).forEach(function (key) {
                const val = Array.isArray(err[key]) ? err[key].join(', ') : err[key];
                msg += val + ' ';
              });
            }
            errorDiv.textContent =
              msg || i18n.createLinkError || 'Failed to create link. Please try again.';
            errorDiv.classList.remove('hidden');
          }
        });
    });
  ...
  function showLinkStats(linkId) {
    if (!linkId) return;
    const modal = document.getElementById('linkStatsModal');
    const body = document.getElementById('linkStatsBody');
    ...
    fetch('/api/affiliate/links/' + linkId + '/', {
      ...
      .then(function (data) {
        ...
        const label = data.label || data.link_code;
        body.innerHTML =
          '<h3 class="stats-link-name">' +
          escapeHtml(label) +
          '</h3>' +
          '<div class="stats-grid">' +
          '  <div class="stats-grid-item"><div class="stats-grid-label">' +
          (i18n.totalClicks || 'Total Clicks') +
          '</div><div class="stats-grid-value">' +
          formatNumber(stats.clicks.total) +
          '</div></div>' +
          '  <div class="stats-grid-item"><div class="stats-grid-label">' +
          (i18n.last7Days || 'Last 7 Days') +
          '</div><div class="stats-grid-value">' +
          formatNumber(stats.clicks.last_7_days) +
          '</div></div>' +
          '  <div class="stats-grid-item"><div class="stats-grid-label">' +
          (i18n.totalConversions || 'Conversions') +
          '</div><div class="stats-grid-value">' +
          formatNumber(stats.conversions.total) +
          '</div></div>' +
          '  <div class="stats-grid-item"><div class="stats-grid-label">' +
          (i18n.conversionRate || 'Conversion Rate') +
          '</div><div class="stats-grid-value">' +
          stats.conversions.rate +
          '%</div></div>' +
          '  <div class="stats-grid-item highlight"><div class="stats-grid-label">' +
          (i18n.totalRevenue || 'Total Revenue') +
          '</div><div class="stats-grid-value">' +
      ...
      .catch(function () {
        body.innerHTML =
          '<div class="affiliate-alert affiliate-alert-danger"><i class="fas fa-exclamation-circle"></i> ' +
          (i18n.statsError || 'Failed to load statistics. Please try again.') +
          '</div>';
      });
  }
  ...
  function showEditLink(linkId) {
    ...
    const successDiv = document.getElementById('editLinkSuccess');
    if (errorDiv) errorDiv.classList.add('hidden');
    if (successDiv) successDiv.classList.add('hidden');
      ...
      fetch('/api/affiliate/links/' + linkId + '/', {
        ...
        .then(function () {
          if (successDiv) {
            successDiv.innerHTML =
              ...
              (i18n.linkUpdated || 'Link updated successfully!');
            successDiv.classList.remove('hidden');
          }
          setTimeout(function () {
            hideEditLinkModal();
        ...
        .catch(function (err) {
          if (errorDiv) {
            ...
            errorDiv.textContent =
              msg || i18n.editLinkError || 'Failed to update link. Please try again.';
            errorDiv.classList.remove('hidden');
          }
        });
    });
  ...
  function showPayoutModal() {
    ...
    if (!modal) return;
    const errorDiv = document.getElementById('payoutRequestError');
    const successDiv = document.getElementById('payoutRequestSuccess');
    if (errorDiv) errorDiv.classList.add('hidden');
    if (successDiv) successDiv.classList.add('hidden');
    modal.classList.remove('hidden');
    document.body.classList.add('modal-open');
  ...
  function initPayoutRequestForm() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      const affiliateId = document.getElementById('payoutAffiliateId');
      ...
      if (submitBtn) {
        submitBtn.disabled = true;
        submitBtn.innerHTML =
          '<i class="fas fa-spinner fa-spin"></i> ' + (i18n.submitting || 'Submitting...');
      }
      ...
      fetch('/api/affiliate/payouts/', {
        ...
        .then(function () {
          if (successDiv) {
            successDiv.innerHTML =
              '<i class="fas fa-check-circle"></i> ' +
              (i18n.payoutSuccess || 'Payout request submitted successfully!');
            successDiv.classList.remove('hidden');
          }
          setTimeout(function () {
            hidePayoutModal();
        ...
        .catch(function (err) {
          ...
          if (errorDiv) {
            errorDiv.textContent =
              msg || i18n.payoutError || 'Failed to submit payout request. Please try again.';
            errorDiv.classList.remove('hidden');
          }
          if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.innerHTML =
              '<i class="fas fa-paper-plane"></i> ' + (i18n.submitRequest || 'Submit Request');
          }
        });
    });
  }
  ...
  function initCharts() {
    ...
    const dataEl = document.getElementById('affiliate-chart-data');
    if (!dataEl) return;
    ...
    const canvas = document.getElementById('affiliatePerformanceChart');
    if (!canvas) return;

    const labels = config.labels.map(function (d) {
