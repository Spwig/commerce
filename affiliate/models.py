...
class Program(models.Model):
    ...
    description = models.TextField(
        ...
        help_text=_('Program description for affiliates')
    )
...
class Affiliate(models.Model):
    ...
    payment_email = models.EmailField(
        ...
        help_text=_('Email for PayPal or other payment methods')
    )
    ...
    def get_balance_summary(self):
        ...
        total_earned = self.commissions.filter(
            status__in=['approved', 'paid']
        ).aggregate(Sum('amount'))['amount__sum'] or 0

        # Pending approval
        pending_approval = self.commissions.filter(
            status='pending'
        ).aggregate(Sum('amount'))['amount__sum'] or 0

        # Approved but not paid (outstanding balance)
        outstanding = self.commissions.filter(
            status='approved'
        ).aggregate(Sum('amount'))['amount__sum'] or 0

        # Already paid
        paid = self.commissions.filter(
            status='paid'
        ).aggregate(Sum('amount'))['amount__sum'] or 0

        # Total from all completed payouts
        total_payouts = self.payouts.filter(
            status='completed'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        ...
        return {
            ...
            'pending_payouts': self.payouts.filter(
...
class Link(models.Model):
    ...
    link_code = models.CharField(
        ...
        help_text=_('Unique tracking code')
    )
    destination_url = models.URLField(
        verbose_name=_('destination URL'),
        help_text=_('Where this link redirects to')
    )
...
class Click(models.Model):
    ...
    # Timestamps
    clicked_at = models.DateTimeField(default=timezone.now, db_index=True, verbose_name=_('clicked at'))
...
class Payout(models.Model):
    ...
    def mark_as_processing(self, reference='', provider_account=None, provider_reference='', provider_response=None):
        """
    ...
    def mark_as_completed(self, reference='', provider_reference='', provider_response=None):
        ...
        self.commissions.filter(status='approved').update(
            status='paid',
    ...
    def cancel(self, notes=''):
        ...
        self.commissions.filter(status='paid').update(
            status='approved',
            paid_at=None
        )
...
class AffiliateSettings(models.Model):
    ...
    require_approval = models.BooleanField(
        ...
        help_text=_('If enabled, new affiliates must be approved before they can access the dashboard')
    )
