...
class ProgramListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Program
        fields = [
            'id', 'name', 'slug', 'merchant', 'merchant_name',
            'description', 'commission_type', 'commission_value',
            'commission_display', 'cookie_lifetime_days', 'status',
            'auto_approve_affiliates', 'minimum_payout',
            'affiliates_count', 'created_at'
        ]
...
class ProgramDetailSerializer(serializers.ModelSerializer):
    ...
    def get_statistics(self, obj) -> dict:
        ...
        total_commissions = obj.commissions.aggregate(
            Sum('amount')
        )['amount__sum'] or 0
        pending_commissions = obj.commissions.filter(
            status='pending'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        paid_commissions = obj.commissions.filter(
            status='paid'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
...
class AffiliateListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Affiliate
        fields = [
            'id', 'user', 'user_email', 'user_name', 'affiliate_code',
            'company_name', 'website', 'payment_email', 'payment_method',
            'status', 'programs_count', 'total_earned', 'created_at'
        ]
...
class AffiliateDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'user_email', 'user_name', 'affiliate_code',
            'company_name', 'website', 'payment_email', 'payment_method',
            'status', 'statistics', 'created_at', 'updated_at'
        ]
    ...
    def get_statistics(self, obj) -> dict:
        ...
        pending = obj.commissions.filter(
            status='pending'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        approved = obj.commissions.filter(
            status='approved'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        paid = obj.commissions.filter(
            status='paid'
        ).aggregate(Sum('amount'))['amount__sum'] or 0

        total_payouts = obj.payouts.filter(
            status='completed'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
...
class AffiliateProgramMembershipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'affiliate', 'affiliate_code', 'program', 'program_name',
            'status', 'notes', 'applied_at', 'approved_at'
        ]
...
class ProgramApplicationSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if AffiliateProgramMembership.objects.filter(
            affiliate=affiliate, program=program
        ).exists():
...
class LinkListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'affiliate', 'affiliate_code', 'program', 'program_name',
            'link_code', 'destination_url', 'label', 'is_active',
            'clicks_count', 'tracking_url', 'created_at'
        ]
...
class LinkDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'affiliate', 'affiliate_code', 'program', 'program_name',
            'link_code', 'destination_url', 'label', 'is_active',
            'tracking_url', 'statistics', 'created_at', 'updated_at'
        ]
...
class LinkCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Link
        fields = [
            'affiliate', 'program', 'destination_url', 'label', 'is_active'
        ]
...
class ClickSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Click
        fields = [
            'id', 'link', 'link_code', 'affiliate_code', 'ip_address',
            'user_agent', 'referrer', 'session_id', 'cookie_value',
            'clicked_at', 'has_conversion'
        ]
...
class CommissionListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'affiliate', 'affiliate_code', 'program', 'program_name',
            'order', 'order_id', 'click', 'amount', 'status',
...
class CommissionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Commission
        fields = [
            'id', 'affiliate', 'affiliate_code', 'affiliate_name',
            'program', 'program_name', 'order', 'order_details',
            'click', 'amount', 'status', 'notes',
            'created_at', 'approved_at', 'paid_at'
        ]
        read_only_fields = ['created_at', 'approved_at', 'paid_at']
...
class PayoutListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payout
        fields = [
            'id', 'affiliate', 'affiliate_code', 'affiliate_email',
...
class PayoutDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payout
        fields = [
            'id', 'affiliate', 'affiliate_code', 'affiliate_name', 'affiliate_email',
            'amount', 'method', 'status', 'reference', 'notes',
            'created_at', 'processed_at', 'completed_at'
        ]
        read_only_fields = ['created_at', 'processed_at', 'completed_at']
...
class PayoutRequestSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        approved_balance = affiliate.commissions.filter(
            status='approved'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
...
class AffiliateDashboardSerializer(serializers.Serializer):
    ...
    def get_overview(self, obj) -> dict:
        ...
        total_earned = self.affiliate.commissions.filter(
            status__in=['approved', 'paid']
        ).aggregate(Sum('amount'))['amount__sum'] or 0
        pending_balance = self.affiliate.commissions.filter(
            status='approved'
        ).aggregate(Sum('amount'))['amount__sum'] or 0
    ...
    def get_recent_clicks(self, obj) -> int:
        ...
        return Click.objects.filter(
            link__affiliate=self.affiliate,
    ...
    def get_top_links(self, obj) -> list:
        ...
        links = self.affiliate.links.annotate(
...
class MerchantDashboardSerializer(serializers.Serializer):
    ...
    def get_overview(self, obj) -> dict:
        ...
        total_affiliates = Affiliate.objects.filter(
