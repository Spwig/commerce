...
class AnnouncementAdmin(TranslatableAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Image"),
            {
                ...
                "classes": ("collapse",),
                "description": _(
                    "Optional image for modal display. Choose between banner (above content) or background (behind text with overlay)."
                ),
            },
        ),
        (
        ...
        (
            _("Visibility Rules"),
            {
                ...
                "classes": ("collapse",),
                "description": _(
                    "Optional: restrict when this announcement is shown based on advanced conditions."
                ),
            },
        ),
    )
    ...
    def expiry_status(self, obj):
        ...
        if obj.expires_at is None:
            return format_html('<span class="badge">{}</span>', _("No expiry"))
        elif obj.is_expired():
            return format_html('<span class="badge error">{}</span>', _("Expired"))
        else:
            return format_html('<span class="badge success">{}</span>', _("Active"))
