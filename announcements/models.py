...
class Announcement(models.Model):
    ...
    body = models.TextField(
        blank=True,
        help_text=_('Optional detailed content for modal display')
    )
    ...
    image_overlay_opacity = models.DecimalField(
        ...
        help_text=_('Overlay opacity when image is used as background (0 = transparent, 1 = opaque)')
    )
    ...
    custom_url = models.CharField(
        max_length=500,
        blank=True,
        help_text=_('Custom URL for the link')
    )
    ...
    is_enabled = models.BooleanField(
        default=True,
        help_text=_('Enable or disable this announcement')
    )
    expires_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_('Announcement will automatically stop displaying after this date/time. Leave blank for no expiry.')
    )
    priority = models.IntegerField(
        default=0,
        help_text=_('Lower number = higher priority (displayed first)')
    )
    ...
    translations = models.JSONField(
        default=dict,
        blank=True,
        help_text=_('Translations for announcement content')
    )
