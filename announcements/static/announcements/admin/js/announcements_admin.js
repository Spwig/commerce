...
(function() {
    ...
    function initLinkSelector() {
        ...
        var initialData = {};
        var dataEl = document.getElementById('link-selector-initial');
        if (dataEl) {
            try { initialData = JSON.parse(dataEl.textContent); } catch(e) {}
        }
    ...
    function initImageDisplayToggle() {
        var displayModeField = document.getElementById('id_image_display_mode');
        if (!displayModeField) return;

        function updateOverlay() {
            var opacityField = document.getElementById('ann-overlay-opacity');
    ...
    function initCollapsibleCards() {
        var toggles = document.querySelectorAll('.ann-card-toggle');
        for (var i = 0; i < toggles.length; i++) {
            toggles[i].addEventListener('click', function() {
                var card = this.closest('.ann-card-collapsible');
                if (card) card.classList.toggle('open');
            });
        }

        // Auto-expand visibility card if any rules are selected
        var visBody = document.getElementById('ann-visibility-body');
        if (visBody) {
            var sel = visBody.querySelector('select');
            var hasSelection = sel && sel.querySelector('option:checked');
            if (hasSelection) {
                var visCard = visBody.closest('.ann-card-collapsible');
