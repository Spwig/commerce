...
def get_form_data(context, form_slug):
    ...
    for field in form.fields.all().select_related('step').order_by('step__order', 'order'):
        field_data = {
            ...
            'placeholder': field.get_translated_field('placeholder', language_code) or field.placeholder,
    ...
    if form.is_multi_step:
        for step in form.steps.all().order_by('order'):
            steps_data.append({
                ...
                'description': step.get_translated_field('description', language_code) or step.description,
                'order': step.order,
                'is_skippable': step.is_skippable,
                'next_button_text': step.get_translated_field('next_button_text', language_code) or step.next_button_text,
                'back_button_text': step.get_translated_field('back_button_text', language_code) or step.back_button_text,
            })

    # Build conditional rules data
    rules_data = []
    for rule in form.rules.filter(is_active=True).select_related('source_field', 'target_field', 'target_step'):
        rule_data = {
            'id': rule.pk,
    ...
    return {
        ...
        'submit_button_text': form.get_translated_field('submit_button_text', language_code) or form.submit_button_text,
        'success_message': form.get_translated_field('success_message', language_code) or form.success_message,
        'error_message': form.get_translated_field('error_message', language_code) or form.error_message,
