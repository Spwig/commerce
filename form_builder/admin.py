...
from .models import Form, FormField, FormResponse, FormStep
...
class FormFieldInline(admin.TabularInline):
    ...
    fields = ["field_name", "field_type", "label", "is_required", "width", "order", "step"]
    readonly_fields = []
...
class FormAdmin(TranslatableAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("name", "slug", "title", "description")}),
        (
        ...
        (
            _("Settings"),
            {"fields": ("is_active", "is_multi_step", "require_login", "save_partial_responses")},
        ),
        (
            _("Spam Protection"),
            {
        ...
        (_("Translations"), {"fields": ("translations",), "classes": ("collapse",)}),
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["total_responses"] = FormResponse.objects.filter(status="completed").count()
        extra_context["deleted_count"] = Form.all_objects.filter(is_deleted=True).count()
...
class FormStepAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Step Information"), {"fields": ("form", "title", "description", "order")}),
        (_("Navigation"), {"fields": ("is_skippable", "next_button_text", "back_button_text")}),
        (_("Translations"), {"fields": ("translations",), "classes": ("collapse",)}),
    )
...
class FormFieldAdmin(admin.ModelAdmin):
    ...
    list_display = ["label", "form", "field_type", "is_required", "width", "order"]
    list_filter = ["form", "field_type", "is_required", "width"]
    search_fields = ["label", "field_name", "form__name"]
    ordering = ["form", "step__order", "order"]

    fieldsets = (
        (_("Field Identity"), {"fields": ("form", "step", "field_name", "field_type")}),
        (_("Display"), {"fields": ("label", "placeholder", "help_text", "default_value")}),
        (
            _("Validation"),
            {
                "fields": (
        ...
        (_("Layout"), {"fields": ("order", "width", "css_class")}),
        (_("Translations"), {"fields": ("translations",), "classes": ("collapse",)}),
    )
...
class FormResponseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Response Information"), {"fields": ("form", "user", "session_key", "status")}),
        (_("Submitted Data"), {"fields": ("data",)}),
        (_("Metadata"), {"fields": ("ip_address", "user_agent", "referrer", "language")}),
        (_("Progress"), {"fields": ("current_step", "completed_steps"), "classes": ("collapse",)}),
        (_("Action Results"), {"fields": ("action_results",), "classes": ("collapse",)}),
        (
            _("Timestamps"),
            {"fields": ("started_at", "submitted_at", "completed_at", "time_to_complete")},
        ),
    )
    ...
    def user_display(self, obj):
        ...
        return format_html(
            '<span class="quiet"><i class="fas fa-user-secret"></i> {}</span>', _("Anonymous")
        )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["completed_responses"] = queryset.filter(status="completed").count()
        extra_context["draft_responses"] = queryset.filter(status="draft").count()

        return super().changelist_view(request, extra_context=extra_context)

    class Media:
        css = {"all": ["form_builder/css/admin_form_builder.css"]}
