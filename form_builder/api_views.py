...
from .models import Form, FormField, FormResponse, FormConditionalRule
        ...
        if form.is_multi_step:
            for step in form.steps.all().order_by('order'):
                steps_data.append({
                    ...
                    'next_button_text': step.get_translated_field('next_button_text') or step.next_button_text,
                    'back_button_text': step.get_translated_field('back_button_text') or step.back_button_text,
                })
...
class SavePartialView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        ...
        **Behavior:**
        ...
        responses={
            200: OpenApiResponse(description=_('Partial response saved with response ID and current step')),
...
class FileUploadView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        ...
        **Response:**
        ...
        responses={
            200: OpenApiResponse(description=_('File uploaded successfully with path and metadata')),
            400: OpenApiResponse(description=_('Validation error - no file, invalid field, size/type exceeded')),
            404: OpenApiResponse(description=_('Form not found or inactive')),
        }
    )
    def post(self, request, slug):
