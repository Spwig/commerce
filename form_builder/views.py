...
def duplicate_form(request, pk):
    ...
    messages.success(
        request, _('Form "%(name)s" duplicated successfully.') % {"name": original_form.name}
    )
...
def reorder_fields(request, pk):
    ...
    try:
        ...
        for index, field_id in enumerate(field_order):
            FormField.objects.filter(pk=field_id, form=form).update(order=index)
...
def export_responses(request, form_pk):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{form.slug}_responses_{datetime.now().strftime("%Y%m%d")}.csv"'
    )
...
def visual_builder(request, pk):
    ...
    logger.info(
        f"[VISUAL_BUILDER] Form has {form.fields.count()} fields, {form.steps.count()} steps"
    )
    ...
    for field in all_fields:
        logger.info(
            f"[VISUAL_BUILDER] Field: id={field.pk}, name={field.field_name}, type={field.field_type}"
        )
        fields_data.append(
            {
                "id": field.pk,
    ...
    rules_data = []
    for rule in form.rules.select_related("source_field", "target_field", "target_step").order_by(
        "-priority", "id"
    ):
        rules_data.append(
            {
    ...
    try:
        ...
        languages = [
            {"code": code, "name": str(name), "is_primary": code == primary_language}
...
def save_form_builder(request, pk):
    ...
    try:
        ...
        logger.info(
            f"[SAVE_FORM_BUILDER] 'steps' in data: {'steps' in data}, 'fields' in data: {'fields' in data}"
        )
        ...
        form.save_partial_responses = form_data.get(
            "save_partial_responses", form.save_partial_responses
        )
        ...
        return JsonResponse(
            {"success": True, "message": str(_("Form saved successfully.")), "form_id": form.pk}
        )
...
def add_field(request, pk):
    ...
    if request.headers.get("X-Requested-With") != "XMLHttpRequest":
        logger.warning("[ADD_FIELD] Invalid request - missing XMLHttpRequest header")
        return JsonResponse({"error": "Invalid request"}, status=400)
    ...
    try:
        ...
        logger.info(
            f"[ADD_FIELD] Field created successfully: id={field.pk}, field_name={field.field_name}"
        )
...
def add_step(request, pk):
    ...
    if request.headers.get("X-Requested-With") != "XMLHttpRequest":
        logger.warning("[ADD_STEP] Invalid request - missing XMLHttpRequest header")
        return JsonResponse({"error": "Invalid request"}, status=400)
    ...
    try:
        ...
        logger.info(
            f"[ADD_STEP] Step created successfully: id={step.pk}, title={step.title}, order={step.order}"
        )
        ...
        return JsonResponse(
            {"error": "Could not create step. Please try again.", "details": str(e)}, status=409
        )  # 409 Conflict
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
