...
class EmailNotificationAction(BaseAction):
    ...
    def execute(self):
        ...
        if not to_emails:
            logger.warning(
                "EmailNotificationAction: No recipients configured for action %s", self.action.pk
            )
...
class AutoReplyAction(BaseAction):
    ...
    def execute(self):
        ...
        try:
            ...
            logger.info(
                "AutoReplyAction: Queued for %s for response %s", to_email, self.form_response.pk
            )
            return {"status": "sent", "recipient": to_email}
        except Exception as e:
            logger.error("AutoReplyAction: Failed for response %s: %s", self.form_response.pk, e)
