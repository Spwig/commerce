...
class WebhookAction(BaseAction):
    ...
    def execute(self):
        ...
        if not url:
            logger.warning(
                "WebhookAction: No URL configured for action %s",
                self.action.pk
            )
        ...
        try:
            ...
            if not response.ok:
                ...
                logger.warning(
                    ...
                    response.status_code, url, self.form_response.pk
                )
            else:
                logger.info(
            ...
            logger.error(
                "WebhookAction: Timeout for %s (response %s)",
                url, self.form_response.pk
            )
            return {'status': 'error', 'error': f'Timeout after {self.TIMEOUT}s', 'url': url}

        except requests.RequestException as e:
            logger.error(
                "WebhookAction: Failed for %s (response %s): %s",
                url, self.form_response.pk, e
            )
