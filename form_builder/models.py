...
class Form(SoftDeleteModel):
    ...
    slug = models.SlugField(
        _('Slug'),
        unique=True,
        help_text=_('URL-friendly identifier for the form')
    )
    ...
    title = models.CharField(
        _('Form Title'),
        max_length=255,
        help_text=_('Title displayed to users')
    )
    description = models.TextField(
        _('Description'),
        blank=True,
        help_text=_('Optional description shown above the form')
    )
    ...
    is_active = models.BooleanField(
        _('Active'),
        default=True,
        help_text=_('Only active forms can receive submissions')
    )
    is_multi_step = models.BooleanField(
        _('Multi-step Form'),
        default=False,
        help_text=_('Enable multi-step form with sections')
    )
    require_login = models.BooleanField(
        _('Require Login'),
        default=False,
        help_text=_('Users must be logged in to submit')
    )
    ...
    spam_protection = models.CharField(
        _('Spam Protection'),
        max_length=20,
        choices=SPAM_PROTECTION_CHOICES,
        default='honeypot'
    )
...
class FormStep(models.Model):
    ...
    is_skippable = models.BooleanField(
        _('Skippable'),
        default=False,
        help_text=_('Allow users to skip this step')
    )
    ...
    translations = models.JSONField(
        default=dict,
        blank=True,
        help_text=_('Translations for step content')
    )
...
class FormField(models.Model):
    ...
    translations = models.JSONField(
        default=dict,
        blank=True,
        help_text=_('Translations for field labels and help text')
    )
...
class FormResponse(models.Model):
    ...
    session_key = models.CharField(
        ...
        max_length=40,
        blank=True,
        help_text=_('For anonymous submissions')
    )
    ...
    data = models.JSONField(
        _('Response Data'),
        default=dict,
        help_text=_('Field values submitted by the user')
    )
...
class FormConditionalRule(models.Model):
    ...
    value = models.JSONField(
        ...
        default=dict,
        blank=True,
        help_text=_('Value to compare against. Can be single value or list for "in_list" operators.')
    )
    ...
    priority = models.PositiveIntegerField(
        _('Priority'),
        default=0,
        help_text=_('Rules with higher priority are evaluated first')
    )
    ...
    def clean(self):
        ...
        field_actions = ['show_field', 'hide_field', 'require_field', 'unrequire_field', 'set_value']
        step_actions = ['skip_to_step', 'show_step', 'hide_step']

        if self.action in field_actions and not self.target_field:
            raise ValidationError({
                'target_field': _('Target field is required for this action.')
            })

        if self.action in step_actions and not self.target_step:
            raise ValidationError({
                'target_step': _('Target step is required for this action.')
            })
    ...
    def evaluate(self, field_value):
        ...
        try:
            ...
            if self.operator in ['greater_than', 'less_than', 'greater_than_or_equal', 'less_than_or_equal']:
                return False
...
class FormAction(models.Model):
    ...
    name = models.CharField(
        _('Action Name'),
        max_length=255,
        help_text=_('Descriptive name for this action')
    )
    is_active = models.BooleanField(
        _('Active'),
        default=True
    ...
    config = models.JSONField(
        _('Configuration'),
        default=dict,
        help_text=_('Action-specific configuration')
    )
