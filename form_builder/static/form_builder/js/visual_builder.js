...
class FormBuilder {
    ...
    onFieldReorder(evt) {
        ...
        const fieldIds = Array.from(
            document.querySelectorAll('.field-wrapper[data-field-id]')
    ...
    async addField(fieldType, position = null) {
        ...
        let baseName = fieldType.replace(/_/g, '');
    ...
    async updateField(fieldId, data) {
        ...
        try {
            ...
            if (error.message && error.message.includes('404')) {
                ...
                this.showNotification(FB_TRANSLATIONS.fieldNotFound || 'Field not found. It may have been deleted.', 'warning');
            } else {
                this.showNotification(FB_TRANSLATIONS.saveFailed, 'error');
            }
        }
    ...
    loadFieldProperties(field) {
        ...
        if (typeInfo.hasFileConfig) {
            ...
            document.getElementById('prop-allowed-types').value = (field.file_config?.allowed_types || []).join(', ');
        } else {
    ...
    initPropertyPanel() {
        ...
        // Content properties
        document.getElementById('prop-label').addEventListener('input', (e) => saveProperty('label', e.target.value));
        document.getElementById('prop-placeholder').addEventListener('input', (e) => saveProperty('placeholder', e.target.value));
        document.getElementById('prop-help-text').addEventListener('input', (e) => saveProperty('help_text', e.target.value));
        document.getElementById('prop-default-value').addEventListener('input', (e) => saveProperty('default_value', e.target.value));

        // Validation properties
        document.getElementById('prop-required').addEventListener('change', (e) => saveProperty('is_required', e.target.checked));
        document.getElementById('prop-min-length').addEventListener('input', (e) => saveProperty('min_length', e.target.value ? parseInt(e.target.value) : null));
        document.getElementById('prop-max-length').addEventListener('input', (e) => saveProperty('max_length', e.target.value ? parseInt(e.target.value) : null));
        document.getElementById('prop-min-value').addEventListener('input', (e) => saveProperty('min_value', e.target.value ? parseFloat(e.target.value) : null));
        document.getElementById('prop-max-value').addEventListener('input', (e) => saveProperty('max_value', e.target.value ? parseFloat(e.target.value) : null));
        document.getElementById('prop-regex').addEventListener('input', (e) => saveProperty('validation_regex', e.target.value));
        document.getElementById('prop-validation-message').addEventListener('input', (e) => saveProperty('validation_message', e.target.value));

        // Style properties
        document.getElementById('prop-css-class').addEventListener('input', (e) => saveProperty('css_class', e.target.value));

        // Advanced properties
        document.getElementById('prop-field-name').addEventListener('input', (e) => saveProperty('field_name', e.target.value));
        document.getElementById('prop-step').addEventListener('change', (e) => saveProperty('step_id', e.target.value ? parseInt(e.target.value) : null));
    ...
    updateOption(index, label) {
        ...
        if (options[index]) {
            ...
            options[index].value = label.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
            this.updateField(this.selectedField.id, { options });
        }
    }
        ...
        switch (field.field_type) {
            ...
            case 'rating_stars':
                ...
                preview += `<div class="stars-preview">
                    ${Array(maxStars).fill(0).map(() => '<i class="fas fa-star star-preview"></i>').join('')}
                </div>`;
                break;
    ...
    async saveStep() {
        ...
        try {
            ...
            if (error.message && error.message.includes('404')) {
                ...
                this.refreshStepUI();
                this.closeStepModal();
                this.showNotification(FB_TRANSLATIONS.stepNotFound || 'Step not found. It may have been deleted.', 'warning');
            } else {
                this.showNotification(FB_TRANSLATIONS.saveFailed, 'error');
            }
        }
    ...
    async performDeleteStep(stepId) {
        try {
            ...
            if (error.message && error.message.includes('404')) {
                ...
                this.refreshStepUI();
                this.showNotification(FB_TRANSLATIONS.stepNotFound || 'Step not found. It may have already been deleted.', 'warning');
            } else {
                this.showNotification(FB_TRANSLATIONS.saveFailed, 'error');
            }
        }
    ...
    refreshStepUI() {
        ...
        tabsContainer.innerHTML = this.steps.map((step, index) => `
        ...
        listContainer.innerHTML = this.steps.map((step, index) => `
    ...
    openFormSettings() {
        ...
        document.getElementById('setting-spam-protection').value = this.form.spam_protection || 'honeypot';
        document.getElementById('setting-recaptcha-site').value = this.form.recaptcha_site_key || '';
        document.getElementById('setting-recaptcha-secret').value = this.form.recaptcha_secret_key || '';
        ...
        spamSelect.addEventListener('change', () => {
            document.getElementById('recaptcha-settings').classList.toggle('fb-hidden',
                spamSelect.value !== 'recaptcha');
        });
    ...
    async saveFormSettings() {
        ...
        try {
            ...
            if (response.success) {
                ...
                document.getElementById('step-management').classList.toggle('fb-hidden', !formData.is_multi_step);
                document.getElementById('step-tabs').classList.toggle('fb-hidden', !formData.is_multi_step);
                document.getElementById('step-assignment').classList.toggle('fb-hidden', !formData.is_multi_step);
    ...
    initModalTabs() {
        document.querySelectorAll('.modal-tab').forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                tab.classList.add('active');
                modal.querySelector(`.modal-tab-content[data-modal-tab-content="${tab.dataset.modalTab}"]`).classList.add('active');
            });
        });
    }
    ...
    handleTranslateClick(buttonEl) {
        ...
        if (modelType === 'form_builder.formfield') {
            ...
            const inputMap = { label: '#prop-label', placeholder: '#prop-placeholder', help_text: '#prop-help-text' };
            inputEl = document.querySelector(inputMap[fieldKey]);
        } else if (modelType === 'form_builder.form') {
            objectId = this.formId;
            const inputMap = {
                title: '#setting-title', description: '#setting-description',
                submit_button_text: '#setting-submit-text',
                success_message: '#setting-success', error_message: '#setting-error'
            };
            inputEl = document.querySelector(inputMap[fieldKey]);
            ...
            const inputMap = {
                ...
                next_button_text: '#step-next-text', back_button_text: '#step-back-text'
            };
            inputEl = document.querySelector(inputMap[fieldKey]);
        }
    ...
    async openTranslationEditor(modelType, objectId, fieldKey, fieldInput, triggerBtn) {
        const currentValue = fieldInput.value ? fieldInput.value.trim() : (fieldInput.textContent || '').trim();
        if (!currentValue) {
        ...
        try {
            ...
            this._buildTranslationModal(modelType, objectId, fieldKey, currentValue, languages, existingTranslations,
                coverage, lockedFields, endpoints, csrfToken, lockEndpoint, fieldInput);
        } catch (error) {
            console.error('Error opening translation editor:', error);
            AdminModal.alert({message: 'Failed to open translation editor: ' + error.message, type: 'error'});
        } finally {
            if (triggerBtn) {
    ...
    _buildTranslationModal(modelType, objectId, fieldKey, currentValue, languages, existingTranslations,
                           coverage, lockedFields, endpoints, csrfToken, lockEndpoint, fieldInput) {
        const self = this;
        const fieldLabel = fieldKey.replace(/_/g, ' ').replace(/\b\w/g, function(l) { return l.toUpperCase(); });
        ...
        modal.style.cssText = 'position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 10100;';
        ...
        header.innerHTML =
            '<h3 class="utility-title" style="margin: 0; color: var(--body-fg);"><i class="fas fa-globe"></i> Manage Translations - ' + this.escapeHtml(fieldLabel) + '</h3>' +
        ...
        languages.forEach(function(lang) {
            ...
            const lockBtnStyle = 'background: ' + (isLocked ? '#fce4ec' : 'none') + '; border: 1px solid ' + (isLocked ? '#c62828' : 'var(--hairline-color)') + '; border-radius: 4px; cursor: pointer; color: ' + (isLocked ? '#c62828' : 'var(--body-quiet-color)') + '; padding: 4px 8px; font-size: 0.85em; transition: all 0.15s;';
            ...
            const savedBadge = (isTranslated && !isLocked)
                ? '<span class="translation-status-badge" data-lang="' + lang.code + '" style="padding: 2px 8px; border-radius: 3px; font-size: 0.75em; font-weight: bold; background: var(--success-bg, #d4edda); color: var(--success-color, #28a745); border: 1px solid var(--success-color, #28a745);">Saved</span>'
                : '';
            let editField = '';
            if (isTranslated && !isLocked) {
                editField = '<textarea class="translation-edit-field" data-lang="' + lang.code + '" data-original-value="' + escapedTranslation + '" placeholder="Edit ' + self.escapeHtml(lang.name) + ' translation..." style="width: 100%; padding: 8px; border: 1px solid var(--hairline-color); border-radius: 4px; background: var(--body-bg); color: var(--body-fg); font-family: inherit; font-size: 0.9em; min-height: 60px; resize: vertical;">' + escapedTranslation + '</textarea>';
            ...
            langItem.innerHTML =
                ...
                '<input type="checkbox" id="te_lang_' + lang.code + '" value="' + lang.code + '" ' + checkboxAttrs + ' style="width: 18px; height: 18px; flex-shrink: 0;">' +
                '<label for="te_lang_' + lang.code + '" style="flex: 1; margin: 0; color: var(--body-fg);"><strong>' + self.escapeHtml(lang.name) + '</strong>' + translationStatus + '</label>' +
                (lockEndpoint ? '<button type="button" class="translation-lock-btn" data-lang="' + lang.code + '" data-locked="' + isLocked + '" title="' + (isLocked ? 'Unlock translation' : 'Lock translation') + '" style="' + lockBtnStyle + '"><i class="fas ' + (isLocked ? 'fa-lock' : 'fa-lock-open') + '"></i></button>' : '') +
                ...
                btn.addEventListener('click', async function(e) {
                    ...
                    try {
                        const resp = await fetch(lockEndpoint, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrfToken },
                            body: JSON.stringify({ content_type: modelType, object_id: objectId, field_name: fieldKey, language: langCode })
                        });
        ...
        function setupChangeTracking() {
            languagesList.querySelectorAll('.translation-edit-field').forEach(function(textarea) {
                textarea.addEventListener('input', function() {
                    ...
                    const badge = languagesList.querySelector('.translation-status-badge[data-lang="' + lang + '"]');
                    if (badge) {
                        if (this.value !== originalValue) {
                            badge.style.background = 'var(--warning-bg, #fff3cd)'; badge.style.color = 'var(--warning-color, #856404)'; badge.style.borderColor = 'var(--warning-color, #856404)'; badge.innerHTML = 'Unsaved';
                        } else {
                            badge.style.background = 'var(--success-bg, #d4edda)'; badge.style.color = 'var(--success-color, #28a745)'; badge.style.borderColor = 'var(--success-color, #28a745)'; badge.innerHTML = 'Saved';
                        }
                    }
        ...
        function updateSaveButtonState() {
            const hasUnsaved = Array.from(languagesList.querySelectorAll('.translation-edit-field')).some(function(t) {
        ...
        selectAllBtn.addEventListener('click', function() { languagesList.querySelectorAll('input[type="checkbox"]:not(:disabled)').forEach(function(cb) { cb.checked = true; }); });
        selectNoneBtn.addEventListener('click', function() { languagesList.querySelectorAll('input[type="checkbox"]:not(:disabled)').forEach(function(cb) { cb.checked = false; }); });
            ...
            try {
                ...
                if (result.success) {
                    ...
                    result.saved_languages.forEach(function(lang) {
                        const badge = languagesList.querySelector('.translation-status-badge[data-lang="' + lang + '"]');
                        const textarea = languagesList.querySelector('.translation-edit-field[data-lang="' + lang + '"]');
                        if (badge) { badge.style.background = 'var(--success-bg, #d4edda)'; badge.style.color = 'var(--success-color, #28a745)'; badge.style.borderColor = 'var(--success-color, #28a745)'; badge.innerHTML = 'Saved'; }
                        if (textarea) { textarea.setAttribute('data-original-value', self.escapeHtml(textarea.value)); existingTranslations[lang] = textarea.value; }
                    });
        ...
        applyBtn.addEventListener('click', async function() {
            ...
            try {
                const response = await fetch(endpoints.translate, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrfToken },
                    body: JSON.stringify({ text: currentValue, languages: selectedLanguages, force_immediate: forceImmediate })
                });
                const result = await response.json();

                if (response.status === 202 && result.recommend_schedule) {
                    recommendationEl.innerHTML = '<div style="padding: 12px; background: var(--blue-subtle); border-left: 3px solid var(--blue-accent); color: var(--body-fg); border-radius: 4px;"><strong><i class="fas fa-clock"></i> Heavy Workload Detected</strong><br>' + result.reason + '<br><em>Estimated time: ' + result.estimated_time + '</em><br><button data-action="te-translate-anyway" style="margin-top: 8px; padding: 6px 12px; background: var(--button-bg); color: var(--button-fg); border: none; border-radius: 4px; cursor: pointer;"><i class="fas fa-bolt"></i> Translate Anyway</button></div>';
                    recommendationEl.querySelector('[data-action="te-translate-anyway"]').addEventListener('click', function() { applyBtn.dataset.forceImmediate = 'true'; applyBtn.click(); });
                    applyBtn.disabled = false; applyBtn.innerHTML = '<i class="fas fa-language"></i> Translate';
                    return;
                }
                ...
                if (result.success) {
                    ...
                    languages.forEach(function(lang) {
                        ...
                        const savedBadge2 = isTranslated ? '<span class="translation-status-badge" data-lang="' + lang.code + '" style="padding: 2px 8px; border-radius: 3px; font-size: 0.75em; font-weight: bold; background: var(--success-bg, #d4edda); color: var(--success-color, #28a745); border: 1px solid var(--success-color, #28a745);">Saved</span>' : '';
                        const editField2 = isTranslated
                            ? '<textarea class="translation-edit-field" data-lang="' + lang.code + '" data-original-value="' + escapedTranslation + '" placeholder="Edit ' + self.escapeHtml(lang.name) + ' translation..." style="width: 100%; padding: 8px; border: 1px solid var(--hairline-color); border-radius: 4px; background: var(--body-bg); color: var(--body-fg); font-family: inherit; font-size: 0.9em; min-height: 60px; resize: vertical;">' + escapedTranslation + '</textarea>'
                            : '';
                        langItem.innerHTML = '<div style="display: flex; align-items: center; gap: 12px;"><input type="checkbox" id="te_lang_' + lang.code + '" value="' + lang.code + '" ' + (isTranslated ? '' : 'checked') + ' style="width: 18px; height: 18px; flex-shrink: 0;"><label for="te_lang_' + lang.code + '" style="flex: 1; margin: 0; color: var(--body-fg);"><strong>' + self.escapeHtml(lang.name) + '</strong>' + (isTranslated ? '<span style="color: var(--info-color); margin-left: 8px;"><i class="fas fa-check-circle"></i> Translated' + (isNew ? ' (New)' : '') + '</span>' : '<span style="color: var(--body-quiet-color); margin-left: 8px;">Not translated</span>') + '</label>' + savedBadge2 + '</div>' + editField2;
                        languagesList.appendChild(langItem);
                    });
                    ...
                    if (analysisItems.length >= 3) {
                        ...
                        if (coverageEl) coverageEl.textContent = (translatedCount / totalCount * 100).toFixed(1) + '%';
                    }
    ...
    renderRulesList() {
        ...
        container.innerHTML = this.rules.map(rule => {
            ...
            const sourceLabel = sourceField ? sourceField.label : (rule.source_field_label || 'Unknown Field');
            ...
            if (rule.action.includes('field')) {
                ...
                targetDescription = targetField ? targetField.label : (rule.target_field_label || 'Unknown Field');
            } else if (rule.action.includes('step')) {
                const targetStep = this.steps.find(s => s.id === rule.target_step_id);
                targetDescription = targetStep ? targetStep.title : (rule.target_step_title || 'Unknown Step');
            }
    ...
    populateFieldSelects() {
        ...
        const triggerableFields = this.fields.filter(f => !['heading', 'paragraph', 'divider', 'hidden'].includes(f.field_type));
    ...
    updateOperatorOptions() {
        ...
        const textOperators = ['equals', 'not_equals', 'contains', 'not_contains', 'starts_with', 'ends_with', 'is_empty', 'is_not_empty'];
        const numberOperators = ['equals', 'not_equals', 'greater_than', 'less_than', 'greater_than_or_equal', 'less_than_or_equal', 'is_empty', 'is_not_empty'];
        const selectOperators = ['equals', 'not_equals', 'in_list', 'not_in_list', 'is_empty', 'is_not_empty'];
        ...
        operatorSelect.innerHTML = operators.map(op =>
            `<option value="${op}">${window.FB_OPERATORS[op] || op}</option>`
        ).join('');
        ...
        if (valueContainer) {
            valueContainer.style.display = noValueOperators.includes(operatorSelect.value) ? 'none' : 'block';
        }
    ...
    updateActionTargets() {
        ...
        if (['show_field', 'hide_field', 'require_field', 'unrequire_field', 'set_value'].includes(action)) {
    ...
    async saveRule() {
        ...
        // Validation
        if (!sourceFieldId) {
            this.showNotification(FB_TRANSLATIONS.selectSourceField || 'Please select a source field', 'error');
            return;
        }
        ...
        if (!noValueOperators.includes(operator) && !value) {
            this.showNotification(FB_TRANSLATIONS.enterValue || 'Please enter a comparison value', 'error');
            return;
        }

        // Check target based on action
        const fieldActions = ['show_field', 'hide_field', 'require_field', 'unrequire_field', 'set_value'];
        const stepActions = ['skip_to_step', 'show_step', 'hide_step'];

        if (fieldActions.includes(action) && !targetFieldId) {
            this.showNotification(FB_TRANSLATIONS.selectTargetField || 'Please select a target field', 'error');
            return;
        }

        if (stepActions.includes(action) && !targetStepId) {
            this.showNotification(FB_TRANSLATIONS.selectTargetStep || 'Please select a target step', 'error');
            return;
        }
        ...
        try {
            ...
            if (isNew) {
                response = await this.apiRequest(
                    'POST',
                    `${this.baseUrl}/builder/rules/add/`,
                    ruleData
                );
            } else {
                response = await this.apiRequest(
            ...
            if (response.success) {
                ...
                this.showNotification(
                    isNew ? (FB_TRANSLATIONS.ruleAdded || 'Rule added') : (FB_TRANSLATIONS.ruleUpdated || 'Rule updated'),
                    'success'
                );
            }
        } catch (error) {
    ...
    renderConditions(field) {
        ...
        conditionsList.innerHTML = targetingRules.map(rule => {
            const sourceField = this.fields.find(f => f.id === rule.source_field_id);
...
// =========================================================================
(function() {
    var el = document.getElementById('fb-csrf-token');
    if (el) {
        try { window.csrfToken = JSON.parse(el.textContent); } catch (e) { window.csrfToken = ''; }
    } else {
        window.csrfToken = '';
    }
})();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var formData = readIsland('fb-form-data', null);
    if (!formData) return;

    var fieldsData = readIsland('fb-fields-data', []);
    var stepsData = readIsland('fb-steps-data', []);
    var rulesData = readIsland('fb-rules-data', []);
    ...
    var ruleAction = document.getElementById('rule-action');
    if (ruleAction) {
    ...
    document.addEventListener('click', function(e) {
        ...
        switch (action) {
            case 'fb-undo': fb.undo(); break;
            case 'fb-redo': fb.redo(); break;
            case 'fb-preview': fb.previewForm(); break;
            case 'fb-save': fb.save(); break;
            case 'fb-toggle-save-dropdown':
                e.stopPropagation();
                var menu = document.getElementById('saveDropdownMenu');
                if (menu) menu.classList.toggle('show');
                break;
            case 'fb-save-and-close': fb.saveAndClose(); break;
            case 'fb-duplicate-form': fb.duplicateForm(); break;
            case 'fb-open-rules-modal': fb.openRulesModal(); break;
            case 'fb-add-step': fb.addStep(); break;
            case 'fb-edit-step': fb.editStep(parseInt(el.dataset.stepId, 10)); break;
            case 'fb-delete-step': fb.deleteStep(parseInt(el.dataset.stepId, 10)); break;
            case 'fb-duplicate-field': fb.duplicateField(parseInt(el.dataset.fieldId, 10)); break;
            case 'fb-delete-field': fb.deleteField(parseInt(el.dataset.fieldId, 10)); break;
            case 'fb-select-field': fb.selectField(parseInt(el.dataset.fieldId, 10)); break;
            case 'fb-deselect-field': fb.deselectField(); break;
            case 'fb-open-translation-editor': fb.handleTranslateClick(el); break;
            case 'fb-add-option': fb.addOption(); break;
            case 'fb-open-condition-editor': fb.openConditionEditor(); break;
            case 'fb-open-form-settings': fb.openFormSettings(); break;
            case 'fb-close-form-settings': fb.closeFormSettings(); break;
            case 'fb-save-form-settings': fb.saveFormSettings(); break;
            case 'fb-close-step-modal': fb.closeStepModal(); break;
            case 'fb-save-step': fb.saveStep(); break;
            case 'fb-close-rules-modal': fb.closeRulesModal(); break;
            case 'fb-open-rule-editor': fb.openRuleEditor(); break;
            case 'fb-close-rule-editor': fb.closeRuleEditor(); break;
            case 'fb-save-rule': fb.saveRule(); break;
            case 'fb-close-delete-modal': fb.closeDeleteModal(); break;
            case 'fb-confirm-delete': fb.confirmDelete(); break;
            case 'fb-remove-option': fb.removeOption(parseInt(el.dataset.optionIndex, 10)); break;
        }
    });
