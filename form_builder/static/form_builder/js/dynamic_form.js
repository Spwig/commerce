...
class DynamicForm {
  ...
  nextStep() {
    ...
    const skipRule = this.formData.rules?.find(
      r =>
  ...
  goToStep(stepIndex) {
    ...
    const currentStepEl = this.container.querySelector(
      `.fb-form__step[data-step="${this.currentStep}"]`
    );
  ...
  updateStepIndicator() {
    ...
    const currentText = this.container.querySelector(
      '.fb-progress-bar__current, .fb-steps-text__current'
    );
    if (currentText) {
  ...
  validateCurrentStep() {
    ...
    if (!isValid && this.config.scrollToError) {
      const firstError = stepEl.querySelector(
        '.form-input--error, .form-error:not([style*="display: none"])'
      );
      if (firstError) {
