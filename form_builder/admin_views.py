...
def filter_forms(request):
    ...
    html = render_to_string(
        "admin/form_builder/partials/form_cards.html", {"forms": forms}, request=request
    )
...
def form_recycle_bin(request):
    ...
    if request.method == "POST":
        ...
        if action == "restore":
            ...
            if count:
                messages.success(request, _("Permanently deleted %d form(s).") % count)
            if errors:
                messages.error(
                    request, _("Some forms could not be deleted: %s") % ", ".join(errors)
                )
            ...
            if count:
                messages.success(
                    request, _("Recycle bin emptied (%d forms permanently deleted).") % count
                )
            if errors:
                messages.warning(
                    request,
    ...
    deleted_forms = (
        Form.all_objects.filter(is_deleted=True)
