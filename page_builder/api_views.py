...
class ElementAPIView(PageBuilderAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not page_id and not parent_element_id:
                return JsonResponse(
                    {"error": _("Either page_id or parent_element_id is required")}, status=400
                )
            ...
            if element_config:
                config_defaults = (
                    element_config.config_data.get("defaults", {})
                    if hasattr(element_config, "config_data")
                    else {}
                )
                if config_defaults:
    ...
    def get(self, request, element_id):
        ...
        try:
            ...
            return JsonResponse(
                {"error": f"Element with id {element_id} does not exist"}, status=404
            )
            ...
            return JsonResponse(
                {
                    ...
                    "parent_element_id": element.parent_element.id
    ...
    def render_element_html(self, element, request=None):
        ...
        try:
            ...
            from .context_providers import ELEMENT_CONTEXT_PROVIDERS, get_element_context
            from .element_registry import get_registry
            ...
            dynamic_context = {}
            if (
                content.get("data_source") == "dynamic"
                or element.element_type in ELEMENT_CONTEXT_PROVIDERS
            ):
...
def reorder_elements(request):
    ...
    try:
        ...
        print(
            f"Reorder request - Element: {element_id}, New parent: {new_parent_type} {new_parent_id}"
        )
...
def _inject_dynamic_options(config_data):
    ...
    # Process tabs
    if "tabs" in config:
        for _tab_key, tab in config["tabs"].items():
...
def get_page_settings_config(request, page_id):
    ...
    """
    from design.header_footer_models import FooterTemplate, HeaderTemplate
    from design.models import DesignToken
    from design.theme_models import Theme

    try:
        page = get_object_or_404(
            Page.objects.select_related("theme", "header_template", "footer_template"), id=page_id
        )
        ...
        if active_theme:
            theme_tokens = DesignToken.objects.filter(
                theme=active_theme, token_type="spacing", name__icontains="container"
            ).values("name", "value")
        ...
        return JsonResponse({"success": True, "config": config, "page_data": page_data})
...
def update_page_settings(request, page_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "page_id": page.id, "message": _("Page settings saved successfully")}
        )
...
def save_page_draft(request, page_id):
    ...
    try:
        ...
        version = page.create_draft_version(user=request.user, description=description)
...
def publish_page(request, page_id):
    ...
    try:
        ...
        # Publish the draft
        published_version = page.publish_current_draft(user=request.user, notes=notes)
...
def revert_to_version(request, page_id, version_id):
    ...
    try:
        ...
        new_draft = page.revert_to_version(version_id=version_id, user=request.user)
...
def get_publish_history(request, page_id):
    ...
    try:
            ...
            history.append(
                {
                    ...
                    "version_number": record.published_version.version_number
        ...
        return JsonResponse({"page_id": page.id, "page_title": page.title, "history": history})
...
@extend_schema(
    ...
    description=_(
        "Get published page content by slug. Public endpoint for headless frontends. Only returns published pages. Includes SEO metadata and page elements."
    ),
    parameters=[
        OpenApiParameter(
            name="slug",
...
def get_public_page(request, slug):
    ...
    try:
        page = (
            Page.objects.filter(slug=slug, status="published")
        ...
        if not page:
            return JsonResponse({"success": False, "error": _("Page not found")}, status=404)

        # Check if page requires authentication
        if page.requires_auth and not request.user.is_authenticated:
            return JsonResponse(
                {"success": False, "error": _("Authentication required")}, status=401
            )
        ...
        return JsonResponse(
            {
                ...
                "data": {
                    ...
                    "seo": {
                        ...
                        "meta_description": page.meta_description,
                        "meta_keywords": page.meta_keywords,
                        "og_image": request.build_absolute_uri(page.og_image.url)
                        if page.og_image
                        else None,
                    },
                    "elements": elements_data,
...
@extend_schema(
    ...
    description=_(
        "Get all legal and policy pages for footer navigation. Returns published pages like Terms of Service, Privacy Policy, About Us, Contact, FAQ, Refund Policy, and Shipping Policy. Useful for building site footers in headless frontends."
    ),
    responses={
...
def get_legal_pages(request):
    ...
    try:
        ...
        pages = Page.objects.filter(status="published", page_type__in=legal_page_types).values(
            "id", "title", "slug", "page_type", "meta_description"
        )
        ...
        slug_pages = Page.objects.filter(status="published", slug__in=legal_slugs).values(
            "id", "title", "slug", "page_type", "meta_description"
        )
        ...
        return JsonResponse(
            {"success": True, "data": {"pages": unique_pages, "count": len(unique_pages)}}
        )
...
@extend_schema(
    ...
    description=_(
        "Get the default page for a specific page type. Useful for headless frontends to get the configured home page, category page template, cart page, checkout page, etc. Returns the page marked as default for the type, or the first published page of that type."
    ),
    parameters=[
        OpenApiParameter(
            name="page_type",
            ...
            location=OpenApiParameter.PATH,
            description=_(
                "Page type: home, category, product, cart, checkout, about, contact, or custom"
            ),
        ),
    ],
    responses={
...
def get_page_by_type(request, page_type):
    ...
    try:
        ...
        if page_type not in valid_types:
            return JsonResponse({"success": False, "error": _("Invalid page type")}, status=400)

        # Get default page for this type, or first published page
        page = (
            Page.objects.filter(page_type=page_type, status="published", is_default_for_type=True)
            .select_related("theme")
            .first()
        )
        ...
        if not page:
            ...
            page = (
                Page.objects.filter(page_type=page_type, status="published")
                .select_related("theme")
                .first()
            )
        ...
        if not page:
            return JsonResponse(
                {"success": False, "error": _("No page found for this type")}, status=404
            )
...
def get_visibility_rule_groups(request):
    ...
    try:
        rule_groups = (
            RuleGroup.objects.filter(is_active=True).prefetch_related("rules").order_by("name")
        )
        ...
        for group in rule_groups:
            ...
            rules_summary = ", ".join(
                [
                    f"{r.rule_type} {r.operator} {r.value[:20]}..."
                    if len(str(r.value)) > 20
                    else f"{r.rule_type} {r.operator} {r.value}"
                    for r in rules[:3]
                ]
            )
            if rules.count() > 3:
        ...
        return JsonResponse({"success": True, "rule_groups": data})
...
class RuleGroupAPIView(PageBuilderAPIView):
        ...
        try:
            if group_id:
                ...
                group = get_object_or_404(
                    RuleGroup.objects.prefetch_related("rules", "child_groups"), id=group_id
                )
                return JsonResponse(
                    {"success": True, "group": self._build_group_data(group, include_rules=True)}
                )
            else:
                # List all groups
                groups = (
                    RuleGroup.objects.filter(parent_group__isnull=True)
                    .prefetch_related("rules", "child_groups")
                    .order_by("name")
                )

                return JsonResponse(
                    {"success": True, "groups": [self._build_group_data(g) for g in groups]}
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse({"success": True, "group": self._build_group_data(group)})
    ...
    def delete(self, request, group_id):
        ...
        try:
            ...
            return JsonResponse(
                {"success": True, "message": f'Rule group "{group_name}" deleted successfully'}
            )
...
def save_rule_group_structure(request, group_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": f'Rule group "{group.name}" saved successfully'}
        )
...
class VisibilityRuleAPIView(PageBuilderAPIView):
    ...
    def get(self, request, rule_id=None):
        ...
        try:
            if rule_id:
                ...
                return JsonResponse({"success": True, "rule": self._build_rule_data(rule)})
            else:
                ...
                return JsonResponse(
                    {"success": True, "rules": [self._build_rule_data(r) for r in rules]}
                )
    ...
    def delete(self, request, rule_id):
        ...
        try:
            ...
            return JsonResponse(
                {"success": True, "message": f'Rule "{rule_name}" deleted successfully'}
            )
...
def get_rule_types_config(request):
    ...
    try:
        ...
        rule_types_by_category = {
            ...
            "user": {
                ...
                "types": [
                    {"id": "user_logged_in", "label": _("User Logged In"), "value_type": "boolean"},
                    {
                        "id": "user_group",
                        "label": _("User Group"),
                        "value_type": "user_group_select",
                    },
                    {
                        "id": "user_segment",
                        "label": _("Customer Segment"),
                        "value_type": "segment_select",
                    },
                    {
            ...
            "device": {
                ...
                "types": [
                    ...
                    {"id": "browser", "label": _("Browser"), "value_type": "browser_select"},
                    {
                        ...
                        "value_type": "os_select",
                    },
                    {
                        "id": "screen_size",
                        "label": _("Screen Size"),
                        "value_type": "screen_size_select",
                    },
                    {
                        "id": "connection_speed",
                        "label": _("Connection Speed"),
                        "value_type": "connection_select",
                    },
                ],
            },
            "time": {
            ...
            "language": {
                ...
                "types": [
                    {
                        "id": "browser_language",
                        "label": _("Browser Language"),
                        "value_type": "language_select",
                    },
                    {
        ...
        operators = {
            ...
            "not_contains": {
                "label": _("Not Contains"),
                "symbol": "⊅",
                "compatible": ["text", "list"],
            },
            "greater_than": {
                "label": _("Greater Than"),
                "symbol": ">",
                "compatible": ["number", "currency", "duration"],
            },
            "less_than": {
                "label": _("Less Than"),
                "symbol": "<",
            ...
            "in_list": {
                "label": _("In List"),
                "symbol": "∈",
                "compatible": ["select", "multi_select"],
            },
            "not_in_list": {
                "label": _("Not in List"),
                "symbol": "∉",
            ...
            "between": {
                "label": _("Between"),
                "symbol": "↔",
                "compatible": ["number", "date_range", "time_range"],
            },
...
def get_link_sources(request):
    ...
    if source_type in ("all", "blog"):
        try:
            ...
            if search:
                posts_qs = posts_qs.filter(Q(title__icontains=search) | Q(slug__icontains=search))
