...
class Page(DesignMixin):
    ...
    seo_auto_generated = models.BooleanField(
        default=False,
        help_text=_('Automatically regenerate SEO content when page is saved')
    )
    ...
    preview_thumbnail_updated_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_('When the preview thumbnail was last captured')
    )
    ...
    hide_header = models.BooleanField(
        default=False,
        help_text=_('Hide the header on this page')
    )
    hide_footer = models.BooleanField(
        default=False,
        help_text=_('Hide the footer on this page')
    )
    ...
    def publish_current_draft(self, user=None, notes=""):
        ...
        PagePublishHistory.objects.create(
            page=self,
            published_version=current_draft,
            published_by=user,
            publish_notes=notes
        )
    ...
    def revert_to_version(self, version_id, user=None):
        ...
        new_draft = self.create_draft_version(
            user,
...
class Element(DesignMixin):
    ...
    def __str__(self):
        parent = self.page.title if self.page else (self.parent_element.name if self.parent_element else "No parent")
...
class PageVersion(models.Model):
    ...
    content_snapshot = models.JSONField(
        default=dict,
        help_text=_('Complete page configuration including all elements')
    )
    ...
    def __str__(self):
        status = "Published" if self.is_published else ("Draft" if self.is_current_draft else "Version")
    ...
    def restore_snapshot(self):
        ...
        # Update page fields
        for field in ['title', 'slug', 'page_type', 'meta_title', 'meta_description',
                     'meta_keywords', 'template_variant', 'css_classes', 'layout_config',
                     'style_overrides', 'responsive_config', 'requires_auth', 'cache_timeout',
                     'header_template_id', 'footer_template_id', 'hide_header', 'hide_footer',
                     'page_design_config']:
        ...
        def create_element(element_data, parent=None):
            ...
            element = Element.objects.create(
                page=self.page if not parent else None,
...
class VisibilityRule(models.Model):
    ...
    operator = models.CharField(
        ...
        choices=OPERATORS,
        default='equals',
        help_text=_('How to compare the value')
    )

    # Value to compare against (stored as JSON for flexibility)
    value = models.JSONField(
        default=dict,
        help_text=_('Value or values to compare against')
    )
    ...
    cache_duration = models.IntegerField(
        default=300,
        help_text=_('How long to cache rule evaluation results (seconds)')
    )
...
class PagePublishHistory(models.Model):
    ...
    rolled_back_by = models.ForeignKey(User, related_name='rollbacks', on_delete=models.SET_NULL, null=True, blank=True)
