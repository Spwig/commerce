...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildLightbox() {
      ...
      thumbnails.forEach(thumb => {
        ...
        images.push({
          ...
          thumbSrc: thumb.querySelector('img')
      ...
      if (images.length > 1) {
        thumbsHtml =
          '<div class="product-lightbox__thumbs">' +
          images
            .map(
              (img, i) =>
                '<button class="product-lightbox__thumb' +
                (i === 0 ? ' product-lightbox__thumb--active' : '') +
                '" data-index="' +
                i +
                '">' +
                '<img src="' +
      ...
      lightbox.innerHTML =
        ...
        (images.length > 1
          ? '<button class="product-lightbox__nav product-lightbox__nav--prev" aria-label="Previous"><i class="fas fa-chevron-left"></i></button>'
          : '') +
        '<div class="product-lightbox__image-wrap">' +
        '<img class="product-lightbox__image" src="' +
        images[0].src +
        '" alt="' +
        images[0].alt +
        '">' +
        '</div>' +
        (images.length > 1
          ? '<button class="product-lightbox__nav product-lightbox__nav--next" aria-label="Next"><i class="fas fa-chevron-right"></i></button>'
          : '') +
        (images.length > 1
          ? '<div class="product-lightbox__counter"><span class="product-lightbox__current">1</span> / ' +
            images.length +
            '</div>'
          : '') +
        thumbsHtml;
      ...
      function updateImages(newImages) {
        ...
        if (counterEl) {
          ...
          counterEl.innerHTML =
            '<span class="product-lightbox__current">1</span> / ' + images.length;
        }
      }
    ...
    function updateAttributeVariant() {
      ...
      const matchingVariant = variantsData.find(variant => {
        return Object.keys(selectedAttributes).every(attr => {
          return (
            variant.attributes[attr] && variant.attributes[attr].slug === selectedAttributes[attr]
          );
        });
      });
    ...
    const bundleItemsData = bundleItemsDataEl
      ? JSON.parse(bundleItemsDataEl.textContent || '[]')
      : [];
    ...
    if (addToCartBtn && !isBookingProduct) {
      addToCartBtn.addEventListener('click', function () {
        ...
        if (
          window.DesignEditor &&
          document.getElementById('design-editor') &&
