...
(function () {
  ...
  function init() {
    const el = document.getElementById('visibility-rule-wizard-i18n');
    if (el) {
      try {
  ...
  function initRuleTypeSelection() {
    const ruleTypeItems = document.querySelectorAll('.rule-type-item');
    ruleTypeItems.forEach(function (item) {
      item.addEventListener('click', function () {
        ruleTypeItems.forEach(function (i) {
  ...
  function setupStep2() {
    if (!selectedRuleType || !ruleTypeConfig[selectedRuleType]) {
      return;
    }
    const config = ruleTypeConfig[selectedRuleType];
    ...
    const geoipWarning = document.getElementById('geoip-warning');
    const geoipDetail = document.getElementById('geoip-warning-detail');
    const geoRuleTypes = ['geo_country', 'geo_region', 'geo_city', 'geo_timezone'];
    ...
    config.operators.forEach(function (op, index) {
      const opInfo = operatorLabels[op] || { label: op, symbol: op };
      ...
      div.addEventListener(
        ...
        (function (opValue) {
          return function () {
            document.querySelectorAll('.operator-item').forEach(function (i) {
              i.classList.remove('selected');
            });
            this.classList.add('selected');
            this.querySelector('input[type="radio"]').checked = true;
  ...
  function updateValueInput() {
    ...
    if (
      ...
      const select = document.getElementById('rule-value-select');
      select.innerHTML = '';
  ...
  function validateStep(step) {
    ...
    if (step === 1) {
      if (!selectedRuleType) {
        errors.push(ui.selectRuleType || 'Please select a rule type');
      }
    }
    if (step === 2) {
      const value = getValue();
      if (!selectedOperator) {
        errors.push(ui.selectOperator || 'Please select an operator');
      }
    ...
    if (step === 3) {
      ...
      if (!name) {
        errors.push(ui.enterRuleName || 'Please enter a rule name');
      }
    }
    if (errors.length > 0) {
  ...
  function getValue() {
    ...
    if (
      ...
      const selected = document.querySelector('.boolean-option.selected');
    ...
    if (valueType === 'select') {
      const sel = document.getElementById('rule-value-select');
      const selOpts = Array.from(sel.selectedOptions).map(function (o) {
        return o.value;
      });
  ...
  function populateReview() {
    const config = ruleTypeConfig[selectedRuleType];
    const opInfo = operatorLabels[selectedOperator] || { label: selectedOperator };
    const value = getValue();
    ...
    document.getElementById('review-status').innerHTML = isActive
      ? '<span class="status-badge active"><i class="fas fa-check-circle"></i> ' +
        ...
        '</span>'
      : '<span class="status-badge inactive"><i class="fas fa-times-circle"></i> ' +
        (ui.inactiveStatus || 'Inactive') +
        '</span>';
    const description = document.getElementById('rule-description').value;
    const descGroup = document.getElementById('review-description-group');
    if (description) {
