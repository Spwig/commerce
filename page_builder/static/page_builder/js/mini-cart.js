...
(function() {
    ...
    const MiniCart = {
        ...
        updateBadges(count) {
            ...
            document.querySelectorAll('.header__cart-count, .cart-count, .widget-cart-count').forEach(badge => {
                badge.textContent = count;
                badge.style.display = count > 0 ? 'flex' : 'none';
            });
        },
        ...
        loadRecommendations() {
            ...
            // Show loading state
            container.innerHTML = '<div class="mini-cart__recommendations-loading"><div class="mini-cart__spinner"></div></div>';
        ...
        renderRecommendationItem(product) {
            const saleTag = product.on_sale
                ? '<span class="recommendation-item__badge">Sale</span>'
                : '';

            const priceDisplay = product.on_sale && product.sale_price_formatted
        ...
        getCsrfToken() {
            ...
            let cookieValue = tokenElement ? tokenElement.value : null;
