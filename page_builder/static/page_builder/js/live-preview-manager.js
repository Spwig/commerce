...
class LivePreviewManager {
  ...
  initializeSavedStyles() {
    ...
    setTimeout(() => {
      ...
      if (hasPreloadedData) {
        ...
        console.warn(
          '[LivePreviewManager] allElementsData not available, falling back to API calls'
        );
  ...
  _applyElementStyles(element, elementId, content, elementType) {
    const hasTypography =
      content.typography && content.typography !== 'inherit' && content.typography.trim() !== '';
    ...
    const elementStyleMappings = {
      hero: {
        ...
        description: {
          ...
          typography: 'description_typography',
          color: 'description_color',
          shadow: 'description_shadow',
        },
      },
      cta_banner: {
        title: {
          selector: '.cta-banner__title',
          typography: 'title_typography',
          color: 'title_color',
          shadow: 'title_shadow',
        },
        subtitle: {
          selector: '.cta-banner__subtitle',
          typography: 'subtitle_typography',
          color: 'subtitle_color',
          shadow: 'subtitle_shadow',
        },
  ...
  applyTextContentFromProperties(elementWrapper, elementType, properties) {
    ...
    const textMappings = {
      ...
      image: {
        caption: {
          selector: '.pb-image__caption',
          allowHtml: false,
          toggleVisibility: 'show_caption',
        },
  ...
  applyFormElementClasses(elementWrapper, properties) {
    ...
    const formButtons = elementWrapper.querySelectorAll(
      '.fb-form__btn--submit, .fb-form__btn--next'
    );
    ...
    const classMappings = {
      ...
      label_position: {
        target: formElement,
        prefix: 'fb-form--labels-',
        values: ['above', 'floating', 'inline'],
      },
      field_size: { target: formElement, prefix: 'fb-form--size-', values: ['sm', 'md', 'lg'] },
      field_spacing: {
        target: formElement,
        prefix: 'fb-form--spacing-',
    ...
    if (properties.step_indicator_style !== undefined) {
      ...
      if (indicatorContainer) {
        ...
        indicatorContainer.classList.add(
          `fb-form__step-indicator--${properties.step_indicator_style}`
        );
        ...
        if (progressBar)
          progressBar.style.display =
            properties.step_indicator_style === 'progress_bar' ? '' : 'none';
        if (progressBarText)
          progressBarText.style.display =
            properties.step_indicator_style === 'progress_bar' ? '' : 'none';
        if (circles)
          circles.style.display =
            properties.step_indicator_style === 'numbered_circles' ? '' : 'none';
        if (breadcrumb)
          breadcrumb.style.display = properties.step_indicator_style === 'breadcrumb' ? '' : 'none';
        if (stepsText)
          stepsText.style.display = properties.step_indicator_style === 'steps_text' ? '' : 'none';
      }
    }
  ...
  applyImageFilters(elementWrapper, properties) {
    ...
    const propertyForm = document.querySelector(
      `form.element-properties-form[data-element-id="${elementId}"]`
    );
    const formValues = {};
    if (propertyForm) {
  ...
  applyStyles(elementId, properties) {
    ...
    console.log(
      '[LivePreviewManager] Element wrapper found:',
      !!elementWrapper,
    ...
    if (
      properties.animation_type ||
      properties.animation_duration ||
      properties.animation_delay ||
      properties.animation_timing ||
    ...
    if (
      properties.hover_animation_type ||
      properties.hover_animation_duration ||
      properties.hover_animation_timing ||
      properties.hover_animation_intensity
    ) {
    ...
    if (elementType === 'spacer') {
      if (properties.height !== undefined || properties.height_unit !== undefined) {
        ...
        const height =
          properties.height !== undefined ? properties.height : heightMatch ? heightMatch[1] : '40';
        const unit =
          properties.height_unit !== undefined
            ? properties.height_unit
            : unitMatch
              ? unitMatch[0]
              : 'px';
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (
        this.isNonStyleProperty(key) ||
        key.startsWith('animation_') ||
        key.startsWith('hover_animation_')
      ) {
        continue;
      }
      ...
      if (key === 'background') {
        if (isEmpty) {
          ...
          if (
            elementWrapper.hasAttribute('data-video-url') &&
            elementWrapper.getAttribute('data-video-url')
          ) {
  ...
  applyBorderFromCSS(element, borderCSS) {
    ...
    if (!hasAnimation) {
      const animMatch = borderCSS.match(
        /\/\*\s*animation:\s*border-(\w+)\s+([\d.]+s)\s+(\w+(?:-\w+)?)\s+(\w+|\d+)\s*\*\//
      );
      if (animMatch) {
  ...
  findTargetElement(wrapper, elementType) {
    ...
    console.log(
      '[LivePreviewManager] findTargetElement using selector:',
      selector,
      'for type:',
      elementType
    );
    ...
    console.log(
      '[LivePreviewManager] findTargetElement querySelector result:',
      target?.tagName,
      target?.className
    );
    ...
    if (!target) {
      ...
      console.log(
        '[LivePreviewManager] findTargetElement: Using fallback - firstElementChild:',
        target?.tagName,
        target?.className
      );
    }
  ...
  async sendUpdateToServer(elementId, properties) {
    console.log('[LivePreviewManager] sendUpdateToServer called:', {
      ...
      hoverBackgroundValue: properties.hover_background
        ? properties.hover_background.substring(0, 50) + '...'
        : null,
    });
    ...
    console.log('[LivePreviewManager] Sending to server:', {
      ...
      hoverBackgroundValue: allProperties.hover_background
        ? allProperties.hover_background.substring(0, 80) + '...'
        : null,
  ...
  applyHoverBackground(elementWrapper, hoverCSS) {
    ...
    if (elementWrapper.tagName === 'INPUT' || elementWrapper.tagName === 'TEXTAREA') {
      console.warn(
        '[LivePreviewManager] applyHoverBackground: Received input element instead of wrapper, skipping'
      );
      return;
    }
    ...
    if (!targetElement) {
      ...
      if (elementContent) {
        ...
        targetElement = elementContent.querySelector(
          'section, .hero, .cta-banner, .testimonials, .gallery, .newsletter, .product-carousel, .product-grid'
        );
        if (!targetElement) {
        ...
        console.log(
          '[LivePreviewManager] Fallback found:',
    ...
    if (!targetElement) {
      console.warn(
        '[LivePreviewManager] Could not find target element for hover background after fallbacks'
      );
      return;
    }
    ...
    if (hoverCSS && hoverCSS.trim()) {
      ...
      console.log(
        '[LivePreviewManager] Applied hover background for',
        selector,
        '- Rule:',
      ...
      console.log(
        '[LivePreviewManager] Removed hover background for',
        selector,
        '(hoverCSS was empty/null)'
      );
    }
  }
