...
(function () {
    ...
    var deviceLabel = document.querySelector('.device-label');
    var loadingIndicator = document.querySelector('.loading-indicator');
    var orientationToggle = document.querySelector('.orientation-toggle');
    var deviceFrame = document.querySelector('.device-frame');
    ...
    function applyDevice() {
        var presetKey = currentDevice === 'desktop'
            ? 'desktop'
            : currentDevice + '-' + currentOrientation;

        var preset = PRESETS[presetKey];
        var w = preset.w, h = preset.h, label = preset.label;
        ...
        var orientationIcon = orientationToggle.querySelector('i');
        if (orientationIcon) {
            orientationIcon.style.transform = currentOrientation === 'landscape' ? 'rotate(90deg)' : 'none';
        }

        deviceLabel.textContent = label;

        setTimeout(function () { fitToWrapper(); }, 10);
    ...
    function fitToWrapper() {
        ...
        var iframeWidth = parseInt(iframe.style.width) || iframe.offsetWidth;
        var iframeHeight = parseInt(iframe.style.height) || iframe.offsetHeight;

        var extraWidth = 0, extraHeight = 0;
        if (currentDevice === 'desktop') {
            extraWidth = 32; extraHeight = 100;
        } else if (currentDevice === 'tablet') {
            extraWidth = 54; extraHeight = 48;
        } else if (currentDevice === 'mobile') {
            extraWidth = 30; extraHeight = 40;
        }

        var scaleX = (bounds.width - padding) / (iframeWidth + extraWidth);
        var scaleY = (bounds.height - padding) / (iframeHeight + extraHeight);
        var autoScale = Math.min(scaleX, scaleY, 1);
        var scale = autoScale * (manualZoom / 100);
        ...
        var effectiveZoom = Math.round(scale * 100);
    ...
    var closeBtn = document.querySelector('[data-action="close-preview"]');
    if (closeBtn) {
    ...
    window.addEventListener('resize', function () {
        ...
        resizeTimeout = setTimeout(function () { fitToWrapper(); }, 100);
    });
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (!e.ctrlKey && !e.altKey) {
            if (e.key === '1') { currentDevice = 'desktop'; applyDevice(); }
            else if (e.key === '2') { currentDevice = 'tablet'; currentOrientation = 'portrait'; applyDevice(); }
            else if (e.key === '3') { currentDevice = 'mobile'; currentOrientation = 'portrait'; applyDevice(); }
