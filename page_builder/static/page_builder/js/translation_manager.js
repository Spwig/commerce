...
class TranslationManager {
    ...
    async checkHealth() {
        try {
            ...
            document.dispatchEvent(new CustomEvent('translation:health-update', {
                detail: health
            }));
    ...
    async handleTranslateRequest(detail) {
        ...
        const shouldSchedule = await this.shouldScheduleTranslation(
            charCount,
            languages.length
        );
        ...
        try {
            ...
            if (result.success) {
                ...
                this.showNotification(
                    `Partially translated: ${result.message}`,
                    'warning'
                );
            } else {
                // Failed
    ...
    async handlePageTranslation(detail) {
        ...
        try {
            ...
            if (result.success) {
                ...
                this.showNotification(
                    result.error || 'Failed to schedule page translation',
                    'error'
                );
            }
