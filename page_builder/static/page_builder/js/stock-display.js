    ...
    fetch(`/api/catalog/products/${productSlug}/`)
      ...
      .then(data => {
        ...
        if (regionalStock.available) {
          statusClass =
            regionalStock.quantity && regionalStock.quantity <= 5 ? 'low-stock' : 'in-stock';
          iconHtml =
      ...
      .catch(error => {
        ...
        if (container) {
          ...
          container.innerHTML =
            '<span class="stock-status__text stock-status__text--muted">Stock information unavailable</span>';
        }
      });
  }
