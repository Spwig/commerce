...
(function() {
    ...
    function init() {
        document.querySelectorAll('.product-grid-section[data-element-type="product_grid"]').forEach(initProductGrid);
    }
    ...
    function initProductCards(section) {
        ...
        section.addEventListener('click', function(e) {
            ...
            if (addBtn) {
                ...
                if (!productId) return;
                addBtn.dispatchEvent(new CustomEvent('product:addToCart', {
                    bubbles: true,
            ...
            // Quick view buttons
            const qvBtn = e.target.closest('.product-card__quickview-btn, .product-card__action-btn--quickview');
            if (qvBtn) {
                e.preventDefault();
                const productSlug = qvBtn.dataset.productSlug;
                if (productSlug) {
                    qvBtn.dispatchEvent(new CustomEvent('product:quickView', {
                        bubbles: true,
                        detail: { productSlug }
                    }));
                }
            ...
            if (wishBtn) {
                ...
                if (productId) {
                    wishBtn.dispatchEvent(new CustomEvent('product:toggleWishlist', {
                        bubbles: true,
    ...
    function initCarouselLayout(section) {
        ...
        section.querySelectorAll('.cat-carousel').forEach(function(carousel) {
            ...
            [prevBtn, headerPrev].forEach(function(btn) {
                if (btn) btn.addEventListener('click', function() { scrollToIndex(currentIndex - getVisibleCount()); resetAutoplay(); });
            });
            [nextBtn, headerNext].forEach(function(btn) {
                if (btn) btn.addEventListener('click', function() { scrollToIndex(currentIndex + getVisibleCount()); resetAutoplay(); });
            });
            ...
            function startAutoplay() {
                ...
                stopAutoplay();
                autoplayTimer = setInterval(function() { scrollToIndex(currentIndex + getVisibleCount()); }, config.autoplaySpeed);
            }
            ...
            carousel.addEventListener('keydown', function(e) {
                if (e.key === 'ArrowLeft') { e.preventDefault(); scrollToIndex(currentIndex - 1); resetAutoplay(); }
                else if (e.key === 'ArrowRight') { e.preventDefault(); scrollToIndex(currentIndex + 1); resetAutoplay(); }
            });
            ...
            window.addEventListener('resize', function() {
                ...
                resizeTimer = setTimeout(function() { buildProgress(); updateArrows(); }, 200);
            });
