...
class VisualPageBuilder {
      ...
      if (state.parentChildMap) {
        ...
        for (const [parentId, childIds] of Object.entries(state.parentChildMap)) {
          ...
          const containerContent =
            parentElement.querySelector('.container-content') ||
          ...
          wrappedElements.forEach((element, index) => {
            console.log(
              `Adding element at position ${index}:`,
  ...
  updatePreviewAfterMove(change) {
    ...
    const isInContainer =
      targetContainer.classList.contains('pb-container-content') ||
      targetContainer.classList.contains('pb-modal-builder__inner');
    const draggableSelector = isInContainer
      ? ':scope > .container-child-wrapper'
      : '.element-wrapper';

    console.log(
      `Target container type: ${isInContainer ? 'container' : 'root'}, draggable unit: ${draggableSelector}`
    );
    ...
    console.log(
      `Current index: ${currentIndex}, requested index: ${requestedIndex}, same parent: ${isSameParent}`
    );
    ...
    if (targetIndex >= updatedSiblings.length) {
      ...
      console.log(
        `Inserted before sibling at index ${targetIndex} (requested ${requestedIndex}, current ${currentIndex})`
      );
    }
  ...
  getDraggableElements(container) {
    ...
    const isContainer =
      container.classList.contains('pb-container-content') ||
      ...
      this.pageLevelSortable = new Sortable(pageElements, {
        ...
        onEnd: evt => {
          ...
          if (oldIndex !== newIndex && elementId) {
            ...
            this.reorderPageElements(elementId, newIndex, evt);
          }
        },
      });
    }
  ...
  handleDragEnd(e) {
    ...
    document
      .querySelectorAll('.drop-target-valid, .drop-target-invalid, .drag-source')
      .forEach(el => {
  ...
  handleDragOver(e) {
    ...
    if (
      e.currentTarget.classList.contains('container-drop-zone') ||
  ...
  handleDragEnter(e) {
    ...
    if (
      ...
      const containerWrapper = e.currentTarget.closest(
        '.element-wrapper[data-element-type="container"], .element-wrapper[data-element-type="modal_popup"]'
      );
      if (containerWrapper) {
  ...
  async addElementToPageAtPosition(elementType, position) {
    try {
      ...
      if (command.createdElementId) {
        ...
        await this.reorderElementsAfterInsert(
          'root',
          position,
          command.createdElementId,
          existingElements
        );
  ...
  async addElementToContainerAtPosition(containerId, elementType, position) {
    try {
      ...
      const container = document.querySelector(
        `[data-element-id="${containerId}"] .pb-container-content`
      );
  ...
  moveElementToPosition(containerId, elementId, position) {
    ...
    if (elementWrapper.contains(parentContainer)) {
      console.warn(
        `[moveElementToPosition] Cannot move element ${elementId}: element contains the parent container`
      );
      return;
    }
    ...
    if (!parentContainer.contains(elementWrapper)) {
      console.warn(
        `[moveElementToPosition] Element ${elementId} is not in the expected parent container`
      );
      return;
    }
    ...
    const currentSiblings = Array.from(parentContainer.children).filter(
      child =>
        child.classList.contains('element-wrapper') ||
        child.classList.contains('container-child-wrapper')
    );
  ...
  async reorderElementsAfterInsert(
    containerId,
    insertPosition,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (elementData.element && elementData.element.html) {
          ...
          console.log(
            'Using client rendering for:',
            elementData.element?.element_type,
            'Data:',
            elementData
          );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (container) {
          ...
          const contentArea =
            container.querySelector('.pb-container-content') ||
          ...
          const newElement = contentArea.querySelector(
          ...
          if (newElement) {
            const wrapper = newElement.classList.contains('element-wrapper')
              ? newElement
              : newElement.querySelector('.element-wrapper');
            if (wrapper) {
          ...
          this.showNotification(
            `${config.direction === 'row' ? 'Row' : 'Column'} added to container!`,
            'success'
          );
  ...
  createSectionHTML(section) {
    ...
    if (section.elements && section.elements.length > 0) {
      elementsHTML = section.elements.map(element => this.createElementHTML(element)).join('');
    }
  ...
  getNextElementOrder(sectionId = null) {
    if (sectionId) {
      const elements = document.querySelectorAll(
        `[data-section-id="${sectionId}"] .element-wrapper`
      );
      return elements.length + 1;
    } else {
      // Get order for page-level elements
      const pageElements = document.querySelectorAll(
        '#page-elements > .element-wrapper, #page-elements > .container-wrapper'
      );
  ...
  // Clicking on an element wrapper (either via the wrapper-scoped listener at
  // setupWrapperInteractions or the document-level delegate) selects it,
  // populates the properties panel, and syncs the structure tree so the
  // sidebar highlights the same node.
  //
  // NOTE: this used to have a duplicate definition earlier in the class that
  // JavaScript silently overrode. That older version called
  // `syncStructureSelection` — behavior we restored here — but was otherwise
  // superseded by the newer selection flow.
  handleElementClick(e) {
    ...
    // Sync structure/tree view selection so the sidebar highlights this
    // element. Previously done by the shadowed earlier definition; regressed
    // when that definition became dead code.
  ...
  async reorderPageElements(elementId, newIndex, evt) {
    try {
  ...
  async updateContainerElementOrder(evt) {
    ...
    if (movedWrapper.classList.contains('container-child-wrapper')) {
      ...
      console.error(
        'Unexpected element structure in updateContainerElementOrder - all elements should be wrapped'
      );
      return;
    }
    ...
    const newContainer = evt.to.closest(
      '.element-wrapper[data-element-type="container"], .element-wrapper[data-element-type="modal_popup"]'
    );
    ...
    const oldContainer = evt.from.closest(
      '.element-wrapper[data-element-type="container"], .element-wrapper[data-element-type="modal_popup"]'
    );
    ...
    try {
      ...
      console.log(
        `🎯 Container move: old=${oldIndex}, new=${newIndex}, same container=${evt.from === evt.to}`
      );
      ...
      if (actualNewIndex !== -1) {
        if (actualNewIndex !== newIndex) {
          console.log(
            `⚠️ Sortable misreported! Correcting from ${newIndex} to actual ${actualNewIndex}`
          );
    ...
    try {
      ...
      sections.forEach((section, index) => {
        ...
        if (index >= insertPosition) {
          updatePromises.push(this.updateSectionOrderOnServer(sectionId, index + 2));
        }
      });
  ...
  buildPageStructure() {
    ...
    sections.forEach(sectionEl => {
      ...
      const sectionContainer = sectionEl.querySelector(
        '.element-wrapper[data-element-type="container"]'
      );
      if (sectionContainer) {
  ...
  extractElementData(elementEl) {
    ...
    if (elementType === 'container') {
      ...
      if (containerContent) {
        ...
        const directChildren = containerContent.querySelectorAll(
          ':scope > .container-child-wrapper, :scope > .element-wrapper'
        );
  ...
  getElementDisplayName(elementEl, elementType) {
    ...
    try {
      switch (elementType) {
        case 'text':
        case 'heading':
          const textContent = elementEl
            .querySelector('.text-element, h1, h2, h3, h4, h5, h6, p')
            ?.textContent?.trim();
        ...
        case 'image':
          ...
          break;
        case 'container':
          const childCount = elementEl.querySelectorAll(
            '.container-child-wrapper .element-wrapper'
          ).length;
  ...
  initializeStructureSortables() {
    ...
    const sectionContainers = structureTree.querySelectorAll(
      '.structure-node[data-node-type="section"] > .structure-children'
    );
    ...
    allChildrenContainers.forEach((container, index) => {
      ...
      try {
        ...
        this.structureSortables.push(sortable);
        console.log(
          `Container ${index} sortable created for level ${parentLevel + 1} with ${container.children.length} elements`
        );
      } catch (error) {
        console.error(`Failed to create sortable for container ${index}:`, error);
      }
  ...
  syncPreviewFromStructure(elementId, parentId, newIndex, actualParentId = null) {
    ...
    if (targetParentId && targetParentId !== 'page') {
      ...
      const parentElement = document.querySelector(
        `.element-wrapper[data-element-id="${targetParentId}"]`
      );
      if (parentElement) {
    ...
    if (!targetContainer) {
      console.error(
        'Target container not found. Available IDs:',
    ...
    try {
      if (type === 'section') {
        ...
        if (response.ok) {
          ...
          const sectionsContainer = document.getElementById('page-elements');
          const sectionToMove = document.querySelector(
            `.section-wrapper[data-section-id="${elementId}"]`
          );
        ...
        const actualPosition = allNodes.findIndex(node => node.dataset.elementId === elementId);
        ...
        if (actualPosition !== -1) {
          if (actualPosition !== newIndex) {
            console.log(
              `⚠️ Sortable misreported! Correcting from ${newIndex} to actual ${actualPosition}`
            );
        ...
        console.log(
          `📤 Calling StateManager.moveElement(${elementId}, ${newParentId}, ${correctedNewIndex})`
        );
        ...
        if (moved) {
          ...
          console.log(
            `🔍 VERIFICATION: Element ${elementId} is at index ${actualIndex} in parent ${newParentId || 'root'}`
          );
        ...
        if (moved) {
          ...
          if (newParentId) {
            const parentNode = document.querySelector(
              `.structure-node[data-element-id="${newParentId}"]`
            );
            if (parentNode && parentNode.dataset.nodeType === 'section') {
              // Find the section's default container
              const sectionEl = document.querySelector(
                `.section-wrapper[data-section-id="${newParentId}"]`
              );
              if (sectionEl) {
                const container = sectionEl.querySelector(
                  '.element-wrapper[data-element-type="container"]'
                );
                if (container) {
          ...
          const moveCommand = new MoveElementCommand(
            ...
            actualParentId !== null && actualParentId !== undefined
              ? parseInt(actualParentId)
              : null,
            oldIndex,
...
async function saveDraft() {
  try {
    ...
    const response = await fetch(
      `${builderInstance.getApiBaseUrl()}/page/${builderInstance.pageData.id}/save-draft/`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
async function publishPage() {
  if (
    !(await AdminModal.confirm(
      'Are you sure you want to publish this page? This will make your changes live.'
    ))
  ) {
    return;
  }
  ...
  try {
    ...
    const response = await fetch(
      `${builderInstance.getApiBaseUrl()}/page/${builderInstance.pageData.id}/publish/`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
async function checkPageStatus() {
  try {
    const response = await fetch(
      `${builderInstance.getApiBaseUrl()}/page/${builderInstance.pageData.id}/versions/`
    );
    if (response.ok) {
      const data = await response.json();
...
async function loadVersionHistory() {
  ...
  listEl.innerHTML =
    '<div style="padding: 20px; text-align: center; color: var(--pb-text-secondary);"><i class="fas fa-spinner fa-spin"></i> Loading...</div>';
  ...
  try {
    ...
    listEl.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--pb-text-secondary);">Failed to load version history</div>';
  }
}
...
function renderVersionHistory(versions) {
  ...
  if (!versions.length) {
    listEl.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--pb-text-secondary);">No versions yet</div>';
    return;
  }
  ...
  listEl.innerHTML = versions
    .map(function (v) {
      ...
      const badges = [];
      if (v.is_published)
        badges.push('<span class="version-badge version-badge--published">Published</span>');
      if (v.is_current_draft)
        badges.push('<span class="version-badge version-badge--draft">Current Draft</span>');
...
async function revertToVersion(versionId) {
  if (!(await AdminModal.confirm('Revert to this version? Your current draft will be replaced.')))
    return;

  try {
    const pageId = builderInstance.pageData.id;
    const response = await fetch(
      `${builderInstance.getApiBaseUrl()}/page/${pageId}/revert/${versionId}/`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
document.addEventListener('click', function (e) {
  const isUndoDropdown =
    e.target.closest('#undo-dropdown-btn') || e.target.closest('#undo-history-dropdown');
  const isRedoDropdown =
    e.target.closest('#redo-dropdown-btn') || e.target.closest('#redo-history-dropdown');
...
const containerLayoutConfigs = {
  'full-width': [{ flex: '1 1 100%', minWidth: '100%', name: 'Full Width Column' }],
  '2-equal': [
...
async function applyContainerLayout(containerId, layoutType) {
  ...
  try {
    ...
    const updateResponse = await fetch(
      `${builderInstance.getApiBaseUrl()}/elements/${containerId}/`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
...
async function refreshContainerAfterLayout(containerId) {
  try {
    ...
    const response = await fetch(
      `${window.adminPageBuilderPrefix}ajax/element/${containerId}/?visual_builder=true`
    );
    if (!response.ok) {
    ...
    const elementWrapper = document.querySelector(
      `.element-wrapper[data-element-id="${containerId}"]`
    );
    if (!elementWrapper) {
...
async function deleteSection(sectionId) {
  if (
    await AdminModal.confirm({
      message:
        'Are you sure you want to delete this section? This will also delete all elements within it.',
      danger: true,
      confirmText: 'Delete',
    })
  ) {
...
function editElement(elementId, event) {
  ...
  if (allWrappers.length === 1) {
    ...
    for (const wrapper of allWrappers) {
      const parentWrapper = wrapper.parentElement?.closest(
...
async function showElementProperties(elementId, elementType) {
  ...
  propertiesPanel.innerHTML =
    '<div class="properties-empty"><p>Unable to load element properties. Please refresh and try again.</p></div>';
}
...
function previewPage() {
  ...
  window.open(
    `${window.adminPageBuilderPrefix}builder-preview/${builderInstance.pageData.slug}/`,
    '_blank'
  );
}
...
function applyLivePreview(elementId, content) {
  ...
  if (!container) {
    ...
    const elementWrapper = document.querySelector(
      `[data-element-id="${elementId}"][data-element-type="container"]`
    );
    if (elementWrapper) {
  ...
  if (!container) {
    console.warn(
      `Container element ${elementId} not found for live preview. Available elements:`,
...
function toggleBackgroundControls(backgroundType) {
  ...
  if (gradientControls)
    gradientControls.style.display = backgroundType === 'gradient' ? 'block' : 'none';
...
function toggleBorderControls(borderType) {
  ...
  if (borderControls)
    borderControls.style.display = borderType && borderType !== 'none' ? 'block' : 'none';
}
...
function updateElementPreview(elementId, properties) {
  ...
  if (elementType === 'heading') {
    ...
    targetElement =
      elementContent.querySelector('.text-element') ||
      elementContent.querySelector('p') ||
      elementContent;
  } else if (elementType === 'button') {
...
class PageSettingsManager {
  ...
  renderSettingsPanel(containerId) {
    ...
    if (sidebarHeader) {
      sidebarHeader.innerHTML =
        '<button class="page-settings-back-btn" title="Back to Properties"><i class="fas fa-arrow-left"></i></button> <i class="fas fa-file-alt"></i> Page Settings';
  ...
  async saveSettings() {
    try {
      ...
      if (
        this.currentValues.hide_header !== undefined ||
  ...
  enterPageSettingsMode() {
    ...
    if (typeof deselectAllElements === 'function') {
      ...
      window.builderInstance &&
      typeof window.builderInstance.deselectElement === 'function'
    ) {
...
function triggerThumbnailCapture(pageId, slug) {
  ...
  captureFrame.src =
    window.adminPageBuilderPrefix + 'preview/' + encodeURIComponent(slug) + '/?capture=1';
