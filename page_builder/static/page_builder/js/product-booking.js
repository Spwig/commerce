...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderCalendar() {
            ...
            fetchAvailability(state.currentYear, state.currentMonth).then(data => {
                ...
                if (data.config) {
                    ...
                    if (data.config.standard_occupancy) config.standardOccupancy = data.config.standard_occupancy;
                    if (data.config.max_occupancy) config.maxOccupancy = data.config.max_occupancy;
                }
                state.availableDates = (data.available_dates || []).filter(d => d.available).map(d => d.date);
                ...
                for (let i = 0; i < startDow; i++) {
                    html += '<button type="button" class="booking-calendar__day booking-calendar__day--empty" disabled></button>';
                }
                // Day cells
                ...
                for (let d = 1; d <= totalDays; d++) {
                    ...
                    if (isAccom && state.selectedDate) {
                        ...
                        } else if (state.selectedEndDate && dateStr > state.selectedDate && dateStr < state.selectedEndDate) {
                ...
                calDays.innerHTML = '<p style="grid-column: span 7; text-align: center; color: #999; padding: 20px;">Unable to load availability</p>';
                calDays.classList.remove('booking-calendar__days--loading');
            });
        }
        ...
        function initRangeHover(calDays) {
            calDays.addEventListener('mouseover', function(e) {
                const btn = e.target.closest('.booking-calendar__day:not([disabled]):not(.booking-calendar__day--empty)');
                ...
                calDays.querySelectorAll('.booking-calendar__day').forEach(dayBtn => {
                    ...
                    if (d && d > state.selectedDate && d <= hoverDate && !dayBtn.classList.contains('booking-calendar__day--range-start')) {
        ...
        function loadTimeSlots(dateStr) {
            ...
            fetchSlots(dateStr).then(data => {
                state.availableSlots = data.slots || [];

                if (state.availableSlots.length === 0) {
                    slotsContainer.innerHTML = '<p class="booking-slots__placeholder">No available time slots for this date</p>';
                    return;
                }
                ...
                slotsContainer.innerHTML = '<p class="booking-slots__placeholder">Unable to load time slots</p>';
            });
        }
        ...
        function updateResourceAvailability() {
            ...
            fetchResourceAvailability(state.selectedDate, state.selectedEndDate).then(data => {
                const availMap = {};
                (data.resources || []).forEach(r => { availMap[String(r.id)] = r.available; });
            ...
            btn.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (resp.ok) {
                        ...
                        this.disabled = false;
                        AdminModal.alert({message: err.error || 'Failed to add booking to cart', type: 'error'});
                    }
                } catch (e) {
                    this.innerHTML = originalHTML;
                    this.disabled = false;
        ...
        function openResourceModal(resourceId) {
            ...
            fetchResourceDetail(resourceId).then(data => {
                if (loading) loading.style.display = 'none';
                if (body) body.style.display = '';
                renderResourceModal(data);
            }).catch(() => {
                if (loading) loading.innerHTML = '<p style="color:var(--theme-color-error,#ef4444)">Failed to load details</p>';
            });
        }
        ...
        function renderResourceModal(data) {
            ...
            if (priceEl) {
                ...
                if (adj !== 0) {
                    priceEl.textContent = (adj > 0 ? '+' : '') + config.currencySymbol + Math.abs(adj).toFixed(2) + ' per night';
            ...
            if (mainImageEl) {
                ...
                if (images.length) {
                    ...
                    mainImageEl.innerHTML = '<div class="booking-resource-modal__no-image"><i class="fas fa-image"></i></div>';
                }
            }
            ...
            if (thumbsEl) {
                ...
                if (images.length > 1) {
                    images.forEach((img, i) => {
                        ...
                        btn.type = 'button';
                        btn.className = 'booking-resource-modal__thumb' + (i === 0 || img.is_primary ? ' booking-resource-modal__thumb--active' : '');
                        if (img.is_video) {
                        ...
                        btn.addEventListener('click', function() {
                            thumbsEl.querySelectorAll('.booking-resource-modal__thumb--active').forEach(t => t.classList.remove('booking-resource-modal__thumb--active'));
