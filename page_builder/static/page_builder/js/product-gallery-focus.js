...
(function() {
    ...
    var gridItems = document.querySelectorAll('.gallery-focus__item');

    gridItems.forEach(function(item) {
        item.addEventListener('click', function() {
            if (!mainImage) return;
            var img = this.querySelector('.gallery-focus__image');
            if (!img) return;
            ...
            var hero = document.querySelector('.product-gallery__zoom-container');
            if (hero) {
                hero.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }

            // Update active states
            gridItems.forEach(function(gi) { gi.classList.remove('gallery-focus__item--active'); });
