...
(function () {
  ...
  const skipBtn = document.getElementById('skip-account');
  ...
  function showSuccessMessage(message) {
    const banner = document.querySelector('.confirmation__banner');
  ...
  function showErrorMessage(message) {
    const existingError = form.querySelector('.confirmation__error-message');
    ...
    fetch('/api/accounts/convert-guest/', {
      ...
      .then(function (result) {
        if (result.ok && result.data.success) {
          ...
          const card = document.querySelector('.confirmation__card--highlight');
          if (card) {
            setTimeout(function () {
    ...
    skipBtn.addEventListener('click', async function () {
      ...
      if (
        ...
        (await AdminModal.confirm(
          'Are you sure you want to skip creating an account? You can always create one later from your email.'
        ))
      ) {
        card.style.opacity = '0';
        card.style.transition = 'opacity 0.3s ease';
