...
(function() {
    ...
    var currencyEl = document.getElementById('shop-currency-config');
    if (currencyEl) {
        try {
    ...
    var countryEl = document.getElementById('shop-country-config');
    if (countryEl) {
        try {
    ...
    window.addToCart = function(productId, quantity = 1, variantId = null) {
        ...
        .then(data => {
            if (data.success) {
                ...
                window.showNotification(window._mt('product_added_to_cart', 'Product added to cart!'), 'success');
            } else {
                window.showNotification(data.message || window._mt('error_adding_to_cart', 'Error adding to cart'), 'error');
            }
        })
        .catch(error => {
            window.showNotification(window._mt('error_adding_to_cart', 'Error adding to cart'), 'error');
        });
    };
    ...
    window.expressCheckout = function(providerSlug, methodSlug) {
        ...
        // Show loading state
        window.showNotification(window._mt('initializing_express_checkout', 'Initializing express checkout...'), 'info');
        ...
        .then(response => {
            ...
            if (response.status === 404) {
                ...
                window.showNotification(window._mt('redirecting_to_checkout', 'Redirecting to checkout...'), 'info');
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                window.showNotification(data.message || window._mt('express_checkout_failed', 'Express checkout failed'), 'error');
            }
        })
        .catch(error => {
