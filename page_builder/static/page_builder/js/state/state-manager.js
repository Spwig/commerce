...
class PageStateManager {
  ...
  extractElementFromDOM(element) {
    ...
    if (elementData.type === 'container' || elementData.type === 'modal_popup') {
      ...
      const containerContent =
        elementData.type === 'modal_popup'
          ? element.querySelector('.pb-modal-builder__inner')
          : element.querySelector('.pb-container-content');
      if (containerContent) {
        const children = containerContent.querySelectorAll(
          ':scope > .container-child-wrapper > .element-wrapper'
        );
  ...
  moveElement(elementId, newParentId, newIndex, skipAPI = false) {
    ...
    console.log(
      `Same parent move: ${isSameParent}, oldParent: ${oldParentId}, newParent: ${newParentId}`
    );
    ...
    console.log(
      `📍 Target position: ${newIndex} (oldIndex: ${oldIndex}, isSameParent: ${isSameParent})`
    );
    ...
    if (newParentId) {
      ...
      if (newParent) {
        ...
        console.log(
          `✅ Final position in parent: ${finalPosition} (expected: ${adjustedNewIndex})`
        );
      }
    } else {
  ...
  getStructure() {
    const buildStructure = (parentId = null) => {
      ...
      return children.map(child => ({
        ...child,
        children:
          child.type === 'container' || child.type === 'modal_popup'
            ? buildStructure(child.id)
            : [],
      }));
    };
