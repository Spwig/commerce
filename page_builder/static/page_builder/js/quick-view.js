...
(function () {
  ...
  let modal, backdrop, loading, body, closeBtn;
  let imgEl, thumbsEl, titleEl, priceCurrent, priceOriginal;
  let descEl, variantsEl, stockEl, addBtn, addBtnText, qtyInput, fullLink;
  ...
  function render(data) {
    ...
    const hasAttributeMappings = variants.some(function (v) {
  ...
  function renderDirectVariants(variants) {
    ...
    variants.forEach(function (variant) {
      ...
      const parentName = currentProduct ? currentProduct.name : '';
      if (parentName && displayName.indexOf(parentName) === 0) {
        const suffix = displayName.substring(parentName.length).replace(/^\s*[-–—]\s*/, '');
  ...
  function selectDirectVariant(variant, displayName, optionsContainer) {
    ...
    optionsContainer
      .querySelectorAll('.quick-view__swatch, .quick-view__option')
      .forEach(function (btn) {
        const isSelected = String(btn.dataset.variantId) === String(variant.id);
        btn.classList.toggle(
          'quick-view__swatch--selected',
          isSelected && btn.classList.contains('quick-view__swatch')
        );
        btn.classList.toggle(
          'quick-view__option--selected',
          isSelected && btn.classList.contains('quick-view__option')
        );
      });

    // Update label
    const group = optionsContainer.closest('.quick-view__variant-group');
    const selectedSpan = group.querySelector('.qv-selected-value');
  ...
  function selectAttribute(attrSlug, valueSlug, valueLabel, variants, optionsContainer) {
    ...
    optionsContainer
      .querySelectorAll('.quick-view__swatch, .quick-view__option')
      .forEach(function (btn) {
        const isSelected = btn.dataset.valueSlug === valueSlug;
        btn.classList.toggle(
          'quick-view__swatch--selected',
          isSelected && btn.classList.contains('quick-view__swatch')
        );
  ...
  function matchVariant(variants) {
    ...
    for (let i = 0; i < variants.length; i++) {
      ...
      const attrs = variant.attributes_structured || [];
      let match = true;
