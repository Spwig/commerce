...
(function() {
    ...
    function initMultiStep() {
        ...
        if (summaryToggle && checkoutSummary) {
            ...
            if (window.Checkout && window.Checkout.cartData) {
                const updateToggleTotal = () => {
                    const total = window.Checkout.cartData.grand_total || window.Checkout.cartData.final_amount || '0.00';
                    if (summaryToggleTotal) {
                        summaryToggleTotal.textContent = window.Checkout.formatCurrency
                            ? window.Checkout.formatCurrency(total)
                            : new Intl.NumberFormat(undefined, { style: 'currency', currency: window.__shopCurrency || 'USD', minimumFractionDigits: 2 }).format(parseFloat(total) || 0);
                    }
                };
        ...
        function updateProgressBar(activeStep) {
            ...
            document.querySelectorAll('.progress-step, .multistep__progress-item').forEach((el, i) => {
                el.classList.remove('progress-step--active', 'progress-step--completed',
                                   'multistep__progress-item--active', 'multistep__progress-item--completed');
            ...
            document.querySelectorAll('.progress-dot, .multistep__dot').forEach((dot, i) => {
                dot.classList.remove('progress-dot--active', 'progress-dot--completed',
                                   'multistep__dot--active', 'multistep__dot--completed');
                if (i < activeIdx || C.completedSteps.has(steps[i])) {
        ...
        function showStep(stepName, direction) {
            ...
            steps.forEach(s => {
                s.classList.remove('multistep__step--active', 'multistep__step--slide-left', 'multistep__step--fade');
            });
