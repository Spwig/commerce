...
(function () {
  ...
  document.addEventListener('click', function (e) {
    let btn = e.target.closest('[data-action="toggle-group-status"]');
    if (btn) {
      ...
      toggleStatus(
        '/' + langCode + '/admin/page_builder/rulegroup/' + pk + '/toggle-status/',
        activate
      );
      return;
    }
    ...
    if (btn) {
      ...
      var langCode = pathParts[1];
      toggleStatus(
        '/' + langCode + '/admin/page_builder/visibilityrule/' + pk + '/toggle-status/',
        activate
      );
    }
  });
