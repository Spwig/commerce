        ...
        if (targetContainer) {
            ...
            if (window.pageStateManager && newElement) {
                ...
                if (elementData) {
                    ...
                    if (parent) {
                        const siblings = parent.querySelectorAll(':scope > .element-wrapper, :scope > .container-child-wrapper > .element-wrapper');
                        index = Array.from(siblings).indexOf(newElement);

                        // If element is wrapped in container-child-wrapper, get the actual element
                        if (index === -1 && newElement.parentElement?.classList.contains('container-child-wrapper')) {
                    ...
                    // Add to StateManager
                    window.pageStateManager.addElement(elementData, this.parentId || null, index >= 0 ? index : null);
...
class DeleteElementCommand extends BaseCommand {
    ...
    captureElementData() {
        ...
        if (element) {
            ...
            const parentWrapper = element.closest('.pb-container-content')?.closest('[data-element-id]') ||
    ...
    removeElementFromDOM() {
        ...
        if (element) {
            ...
            if (containerContent && parentContainer) {
                const remainingElements = containerContent.querySelectorAll(':scope > .element-wrapper, :scope > .container-child-wrapper');
                if (remainingElements.length === 0) {
                    ...
                    if (window.builderInstance) {
                        ...
                        if (newZone) {
                            window.builderInstance.setupDropZone(newZone, window.builderInstance.handleDropElement.bind(window.builderInstance));
    ...
    restoreElementInDOM() {
        ...
        if (this.elementPosition.parentId !== null && this.elementPosition.parentId !== undefined) {
            const parentElement = document.querySelector(`[data-element-id="${this.elementPosition.parentId}"]`);
            // Check for different container content areas (regular container, modal popup, etc.)
            parent = parentElement?.querySelector('.pb-container-content') ||
                     parentElement?.querySelector('.pb-modal-builder__inner') ||
                     parentElement;
        ...
        if (parent) {
            ...
            if (this.elementPosition.nextSiblingId) {
                const nextSibling = parent.querySelector(`[data-element-id="${this.elementPosition.nextSiblingId}"]`);
                if (nextSibling) {
