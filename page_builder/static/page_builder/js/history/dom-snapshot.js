...
class DOMSnapshot {
  ...
  getElementContent(wrapper) {
    ...
    const textElements = wrapper.querySelectorAll(
      '[contenteditable="true"], .element-text, .element-content'
    );
  ...
  captureSelections() {
    ...
    const activeContainer = document.querySelector(
      '.pb-container-content.active, .container-drop-zone.active'
    );
    if (activeContainer) {
  ...
  restoreSelections(selections) {
    ...
    if (selections.activeContainer) {
      const container = document.querySelector(
        `[data-element-id="${selections.activeContainer}"] .pb-container-content`
      );
      if (container) {
  ...
  triggerRestoreEvents() {
    ...
    window.dispatchEvent(
      new CustomEvent('domSnapshotRestored', {
  ...
  compare(snapshot1, snapshot2) {
    ...
    return elements1.length === elements2.length && elements1.every(id => elements2.includes(id));
  }
