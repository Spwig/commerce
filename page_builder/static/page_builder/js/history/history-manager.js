...
class HistoryManager {
        ...
        try {
            ...
            for (let i = 0; i < commandsToProcess.length; i++) {
                ...
                if (isOldestCommand && command.beforeSnapshot) {
                    ...
                    console.log('  📸 Captured target snapshot from oldest command (state before all operations)');
                }
     ...
     */
    getUndoHistory() {
        const history = this.undoStack.slice().reverse().map((cmd, index) => ({
            description: cmd.getDescription ? cmd.getDescription() : `Action ${this.undoStack.length - index}`,
            index: index + 1,
     ...
     */
    getRedoHistory() {
        const history = this.redoStack.slice().reverse().map((cmd, index) => ({
    ...
    getFullHistory() {
        ...
        this.redoStack.slice().reverse().forEach((cmd, index) => {
            history.push({
                type: 'future',
                description: cmd.getDescription ? cmd.getDescription() : `Action ${index + 1}`,
