    ...
    const CartPage = {
        ...
        renderItems(items) {
            this.els.list.innerHTML = items.map(item => {
                ...
                const unitPrice = item.product ? (product.effective_price_formatted || product.price_formatted || item.total_price) : item.total_price;
                        ...
                        componentsHtml = `
                            <div class="cart-item__components">
                                ${allComps.map(comp => {
                                    ...
                                    const compImage = (comp.product && comp.product.images && comp.product.images.length > 0)
                                        ? (comp.product.images[0].thumbnail_url || comp.product.images[0].image_url || comp.product.images[0].url || '')
                                        : '';
        ...
        renderDisplayOnlyNotice(data) {
            ...
            const totalNum = parseFloat(data.grand_total || data.final_amount || data.total_amount || '0');
            ...
            notice.innerHTML =
                ...
                '<span>Approximate total in ' + this.escapeHtml(displayCurrency) + ': <strong>' + formattedDisplay + '</strong>. ' +
                'Your payment will be processed in ' + this.escapeHtml(chargeCurrency) + '.</span>';
        ...
        renderVouchers(vouchers) {
            ...
            this.els.appliedVouchers.innerHTML = vouchers.map(v => {
        ...
        async applyVoucher() {
            ...
            try {
                ...
                if (response.ok && data.success !== false) {
                    ...
                    this.els.voucherMessage.innerHTML =
                        `<div class="cart-voucher__error">${this.escapeHtml(data.message || data.detail || 'Invalid voucher code')}</div>`;
                }
            } catch (error) {
        ...
        async removeVoucher(code) {
            try {
                const response = await fetch(`${this.endpoints.removeVoucher}${encodeURIComponent(code)}/`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRFToken': this.getCsrfToken(),
                    }
