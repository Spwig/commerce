...
class RuleBuilder {
    constructor() {
        ...
        // Popup mode flag
        this.isPopupMode = document.querySelector('.rule-builder-container')?.classList.contains('popup-mode');
    ...
    bindEvents() {
        ...
        document.querySelectorAll('.logic-toggle').forEach(btn => {
            btn.addEventListener('click', (e) => this.setLogicOperator(e.target.closest('.logic-toggle').dataset.logic));
        });
    ...
    bindModalEvents() {
        ...
        if (closeAddRuleModal) closeAddRuleModal.addEventListener('click', () => this.hideAddRuleModal());
        ...
        if (closeNewGroupModal) closeNewGroupModal.addEventListener('click', () => this.hideNewGroupModal());
        if (cancelNewGroupBtn) cancelNewGroupBtn.addEventListener('click', () => this.hideNewGroupModal());
        if (confirmNewGroupBtn) confirmNewGroupBtn.addEventListener('click', () => this.createNewGroup());
    ...
    renderOperatorOptions(rule) {
        const operators = [
            ...
            'is_true', 'is_false', 'between', 'regex'
        ];
    ...
    renderValueInput(rule) {
        ...
        // Boolean types
        if (ruleType === 'user_logged_in' || ruleType === 'first_visit' ||
            ruleType === 'has_purchased' || ruleType === 'abandoned_cart' ||
            ruleType === 'wishlist_items' || ruleType === 'business_hours') {
            return `
        ...
        // Numeric types
        if (['cart_value', 'cart_items', 'visit_count', 'page_views',
             'time_on_site', 'user_lifetime_value', 'user_order_count',
             'screen_size'].includes(ruleType)) {
    ...
    applyRuleChanges(rule) {
        ...
        if (rule.rule_type === 'day_of_week') {
            value = { values: Array.from(document.querySelectorAll('input[name="prop-rule-value-day"]:checked')).map(el => parseInt(el.value)) };
        } else {
            const valueInput = document.getElementById('prop-rule-value');
            if (valueInput) {
                if (valueInput.tagName === 'SELECT') {
                    value = valueInput.value === 'true' ? true : (valueInput.value === 'false' ? false : valueInput.value);
                    ...
                    if (strValue.includes(',')) {
                        value = { values: strValue.split(',').map(s => s.trim()).filter(s => s) };
                    } else {
                        value = strValue;
                    }
                }
    ...
    async switchGroup(groupId) {
        ...
        if (groupId) {
            window.location.href = window.location.pathname.replace(/\/builder\/?\d*\/?$/, `/builder/${groupId}/`);
        } else {
    ...
    handleKeyboard(e) {
        ...
        if (e.key === 'Escape') {
            this.selectedRule = null;
            document.querySelectorAll('.rule-card.selected').forEach(el => el.classList.remove('selected'));
            this.clearPropertiesPanel();
    ...
    getDefaultOperator(ruleType) {
        ...
        if (['user_logged_in', 'first_visit', 'has_purchased',
             'abandoned_cart', 'wishlist_items', 'business_hours'].includes(ruleType)) {
            return 'is_true';
        }

        // Numeric types default to greater_than
        if (['cart_value', 'cart_items', 'visit_count', 'page_views',
             'time_on_site', 'user_lifetime_value', 'user_order_count'].includes(ruleType)) {
            return 'greater_than';
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (backBtn && backBtn.dataset.url) {
        backBtn.addEventListener('click', () => { window.location.href = backBtn.dataset.url; });
    }
});
