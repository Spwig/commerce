...
(function() {
    ...
    function initAccordion(accordion) {
        var panels = accordion.querySelectorAll('.cat-accordion__panel');
        ...
        accordion.addEventListener('keydown', function(e) {
            var activePanel = accordion.querySelector('.cat-accordion__panel--active');
            var index = activePanel ? Array.from(panels).indexOf(activePanel) : -1;

            if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
                e.preventDefault();
                var nextIndex = (index + 1) % panels.length;
                panels.forEach(function(p) { p.classList.remove('cat-accordion__panel--active'); });
                panels[nextIndex].classList.add('cat-accordion__panel--active');
                panels[nextIndex].focus();
            } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
                e.preventDefault();
                var prevIndex = index <= 0 ? panels.length - 1 : index - 1;
                panels.forEach(function(p) { p.classList.remove('cat-accordion__panel--active'); });
                ...
                if (activePanel) {
                    var link = activePanel.getAttribute('href');
                    if (link) {
