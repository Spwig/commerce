...
(function () {
  ...
  const SCOPE_PREFIX = '.pb-content-preview';
  ...
  function _scopeBlock(css) {
    ...
    while (pos < len) {
      ...
      const selector = css.substring(pos, openIdx);

      // Find matching } via depth counting
      let depth = 1;
      let scanPos = openIdx + 1;
      while (scanPos < len && depth > 0) {
        const c = css.charCodeAt(scanPos);
        if (c === 123)
          depth++; // {
        else if (c === 125) depth--; // }
        scanPos++;
      ...
      const body = css.substring(openIdx + 1, scanPos - 1);
  ...
  async function fetchAndScopeCSS(url, layerName) {
    ...
    const cacheKey = 'scopedCSS:' + layerName + ':' + url;
    try {
    ...
    if (imports.length > 0) {
      const importResults = await Promise.all(
        imports.map(async function (imp) {
          const abs = resolveUrl(imp.importUrl, url);
          try {
            const r = await fetch(abs);
            return {
              imp: imp,
              content: r.ok ? await r.text() : '/* Failed to load: ' + imp.importUrl + ' */',
            };
          } catch (e) {
      ...
      importResults.forEach(function (r) {
        css = css.replace(r.imp.fullMatch, r.content);
      });
    }

    let scopedCSS = applyScopeToCSS(css);
    ...
    try {
      sessionStorage.setItem(cacheKey, scopedCSS);
    } catch (e) {
  ...
  async function loadAndScopeCSS(url, id) {
    try {
      ...
      const scopedCSS = applyScopeToCSS(css);
      let el = document.getElementById('scoped-' + id);
      if (!el) {
  ...
  async function loadThemeSystemCSS(cssConfig) {
    ...
    cssToLoad.forEach(function (item) {
      let el = document.getElementById('scoped-' + item.id);
      if (!el) {
        el = document.createElement('style');
    ...
    const results = await Promise.all(
      cssToLoad.map(async function (item) {
        try {
          const css = await fetchAndScopeCSS(item.url, item.layer);
    ...
    results.forEach(function (r) {
      if (styleElements[r.id]) {
        styleElements[r.id].textContent = r.css;
      }
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (config.forceLightMode) {
      const preview = document.querySelector('.pb-content-preview');
      if (preview) {
        preview.setAttribute('data-theme', 'light');
      }
    }
    ...
    const canvasFrame = document.getElementById('page-canvas');
    if (canvasFrame) {
      canvasFrame.addEventListener('click', function (e) {
        const target = e.target;
        if (target.id === 'page-canvas' || target.id === 'page-elements') {
          if (
            !target.closest('.element-wrapper, .page-drop-zone, .empty-drop-zone, .empty-message')
          ) {
            openPageSettings();
          }
        }
