...
(function () {
  ...
  document.addEventListener('click', function (e) {
    const btn =
      e.target.closest('.product-card__quickview-btn') ||
  ...
  document.addEventListener('click', function (e) {
    const btn = e.target.closest('.product-card__action-btn--wishlist');
    ...
    if (isCurrentlyActive) {
      ...
      const itemId = btn.dataset.wishlistItemId;
      if (!itemId) {
        btn.classList.remove('is-loading');
        return;
      }
      ...
      fetch('/api/wishlists/items/' + itemId + '/', {
        ...
        .then(function (response) {
          ...
          if (response.ok) {
            ...
            document
              .querySelectorAll(
                '.product-card__action-btn--wishlist[data-product-id="' + productId + '"]'
              )
              .forEach(function (b) {
      ...
      fetch('/api/wishlists/add/', {
        ...
        .then(function (response) {
          if (response.status === 401 || response.status === 403) {
            ...
            window.location.href =
              '/' + lang + '/account/login/?next=' + encodeURIComponent(window.location.pathname);
            return null;
          }
          return response.json();
        })
        .then(function (data) {
          ...
          if (data.success) {
            ...
            const items = data.wishlist && data.wishlist.items;
            if (items) {
