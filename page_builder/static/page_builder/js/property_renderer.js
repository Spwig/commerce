...
class PropertyRenderer {
    ...
    renderProperties(containerId, elementConfig, elementData, activeTab = null) {
        ...
        const allProperties = elementConfig.tabs ? this.flattenTabProperties(elementConfig.tabs) : elementConfig.properties;
        if (allProperties) {
    ...
    groupPropertiesBySection(properties) {
        ...
        for (const [key, prop] of Object.entries(properties)) {
            ...
            } else if (['size', 'weight', 'font_family', 'align', 'line_height', 'letter_spacing', 'uppercase', 'lowercase', 'capitalize', 'italic', 'underline', 'line_through'].includes(key)) {
    ...
    renderTabs(form, tabs, values, activeTab = null) {
        ...
        for (const [tabKey, tabConfig] of Object.entries(tabs)) {
            ...
            for (const [propKey, propConfig] of Object.entries(tabConfig.properties)) {
                ...
                const isFullWidth = propConfig.type === 'textarea' ||
    ...
    renderSection(sectionName, properties, values) {
        ...
        for (const [key, config] of Object.entries(properties)) {
            const isFullWidth = config.type === 'textarea' ||
    ...
    renderPropertyGroup(key, config, values) {
        ...
        propertiesContainer.className = columns === 1 ? 'property-group-properties single-column' : 'property-group-properties';
        ...
        if (config.properties) {
            for (const [subKey, subConfig] of Object.entries(config.properties)) {
                ...
                const isFullWidth = subConfig.full_width === true || subConfig.type === 'textarea' || subConfig.type === 'aspect_ratio_selector';
    ...
    renderLinkSelectorInput(key, config, value) {
        ...
        typeSelect.addEventListener('change', () => {
            ...
            this.updateLinkSelectorInputArea(wrapper, { type: newType, url: '', id: null, title: '' }, hiddenInput);
        });
        ...
        searchInput.addEventListener('input', () => {
            ...
            searchTimeout = setTimeout(() => {
                this.searchLinkSources(state.type, query, resultsDropdown, (selected) => {
                    ...
                    // Show preview
                    this.showLinkSelectorPreview(wrapper, {
                        type: state.type,
                        id: selected.id,
                        url: selected.url,
                        title: selected.title || selected.name,
                        thumbnail: selected.thumbnail
                    }, hiddenInput, searchWrapper);
    ...
    async searchLinkSources(type, query, resultsContainer, onSelect) {
        resultsContainer.innerHTML = '<div class="link-selector-loading"><i class="fas fa-spinner fa-spin"></i> Searching...</div>';
        resultsContainer.style.display = 'block';

        try {
            const apiBaseUrl = window.builderInstance?.getApiBaseUrl() || '/api/page-builder';
            const response = await fetch(`${apiBaseUrl}/link-sources/?type=${type}&search=${encodeURIComponent(query)}&limit=10`);
            ...
            resultsContainer.innerHTML = '<div class="link-selector-error">Search failed. Please try again.</div>';
        }
    }
    ...
    renderProductPicker(key, config, value) {
        ...
        emptyState.innerHTML = '<i class="fas fa-search"></i><span>Search and add products to display</span>';
    ...
    async _searchProducts(query, resultsContainer, currentIds, searchInput, onSelect) {
        resultsContainer.innerHTML = '<div class="product-picker__loading"><i class="fas fa-spinner fa-spin"></i> Searching...</div>';
        resultsContainer.style.display = 'block';
        this._positionFixedDropdown(resultsContainer, searchInput);

        try {
            const apiBaseUrl = window.builderInstance?.getApiBaseUrl() || '/api/page-builder';
            const response = await fetch(`${apiBaseUrl}/product-search/?search=${encodeURIComponent(query)}&limit=10`);
            const data = await response.json();

            const products = (data.products || []).filter(p => !currentIds.includes(p.id));

            if (products.length === 0) {
                resultsContainer.innerHTML = '<div class="product-picker__no-results">No products found</div>';
                return;
            }
    ...
    async _searchEntities(entityType, query, resultsContainer, searchInput, onSelect) {
        resultsContainer.innerHTML = '<div class="entity-picker__loading"><i class="fas fa-spinner fa-spin"></i> Searching...</div>';
        resultsContainer.style.display = 'block';
        this._positionFixedDropdown(resultsContainer, searchInput);
        ...
        try {
            ...
            if (results.length === 0) {
                resultsContainer.innerHTML = '<div class="entity-picker__no-results">No results found</div>';
                return;
            }
    ...
    async _loadEntityPreview(entityType, entityId, preview, searchInput, syncValue) {
        try {
            const apiBaseUrl = window.builderInstance?.getApiBaseUrl() || '/api/page-builder';
            const response = await fetch(`${apiBaseUrl}/link-sources/?type=${entityType}&search=&limit=50`);
            const data = await response.json();
    ...
    renderCategoryPicker(key, config, value) {
        ...
        emptyState.innerHTML = '<i class="fas fa-search"></i><span>Search and add categories to display</span>';
        wrapper.appendChild(emptyState);
        ...
        searchInput.addEventListener('input', () => {
            ...
            searchTimeout = setTimeout(async () => {
                await this._searchCategories(query, resultsContainer, categoryIds, searchInput, (category) => {
                    // Add category ID to the list
                    categoryIds.push(category.id);
                    syncValue();

                    // Add to selected list UI
                    this._addCategoryPickerCard(selectedList, category, categoryIds, key, emptyState, syncValue);
                    emptyState.style.display = 'none';
    ...
    async _searchCategories(query, resultsContainer, currentIds, searchInput, onSelect) {
        resultsContainer.innerHTML = '<div class="category-picker__loading"><i class="fas fa-spinner fa-spin"></i> Searching...</div>';
        resultsContainer.style.display = 'block';
        this._positionFixedDropdown(resultsContainer, searchInput);

        try {
            const apiBaseUrl = window.builderInstance?.getApiBaseUrl() || '/api/page-builder';
            const response = await fetch(`${apiBaseUrl}/link-sources/?type=category&search=${encodeURIComponent(query)}&limit=20`);
            const data = await response.json();

            const categories = (data.categories || []).filter(c => !currentIds.includes(c.id));

            if (categories.length === 0) {
                resultsContainer.innerHTML = '<div class="category-picker__no-results">No categories found</div>';
                return;
            }
            ...
            if (this.utilityMap[config.type]) {
                ...
                if (UtilityConstructor) {
                    ...
                    if (config.type === 'focal_point') {
                        ...
                        const imageUrl = srcInput ? (srcInput.value || srcInput.dataset.originalUrl || '') : (values['src'] || '');
                        console.log('[PropertyRenderer] Focal point image URL:', imageUrl, 'srcInput:', srcInput, 'srcInput.value:', srcInput?.value);
                    ...
                    if (config.type === 'background') {
                        ...
                        if (values[dataKey]) {
                            ...
                            dataInput.value = typeof values[dataKey] === 'string'
                                ? values[dataKey]
    ...
    setupChangeHandlers(form, elementId) {
        ...
        form.addEventListener('input', (e) => {
            ...
            const target = e.target;
            if (target.name && (target.type === 'text' || target.type === 'number' || target.tagName === 'TEXTAREA' || target.type === 'range')) {
        ...
        form.addEventListener('change', (e) => {
            ...
            if (target.name === 'tag' && target.tagName === 'SELECT') {
                ...
                const hasTypography = typographyInput && typographyInput.value && typographyInput.value.trim() !== '';
                const hasTextColor = textColorInput && textColorInput.value && textColorInput.value.trim() !== '';

                // If typography or text_color is set, show confirmation dialog
                if ((hasTypography || hasTextColor) && this.previousTagValue && target.value !== this.previousTagValue) {
    ...
    async updateElementProperties(elementId, properties) {
        try {
            ...
            if (typeof elementId === 'string' && elementId.startsWith('page-')) {
                console.log('[PropertyRenderer] Skipping element update for page settings (handled by PageSettingsManager)');
                return;
            }
            ...
            console.log('[PropertyRenderer] updateElementProperties called:', {
                ...
                hasSendUpdateToServer: !!(window.livePreview && typeof window.livePreview.sendUpdateToServer === 'function')
            });
    ...
    renderArrayInput(key, config, value) {
        ...
        addButton.addEventListener('click', () => {
            ...
            const newItemElement = this.renderArrayItem(key, currentItems.length - 1, newItem, itemProperties, config);
    ...
    renderArrayItem(arrayKey, index, item, itemProperties, arrayConfig) {
        ...
        const displayTitle = item.title || item.name || item.label || item.question || `Item ${index + 1}`;
        ...
        header.querySelector('.array-item__btn--delete').addEventListener('click', async (e) => {
            e.stopPropagation();
            if (await AdminModal.confirm({ message: 'Are you sure you want to delete this item?', danger: true, confirmText: 'Delete' })) {
    ...
    renderArrayItemIconPicker(arrayKey, index, propKey, config, value) {
        ...
        if (typeof window.IconPickerUtility === 'function') {
            var self = this;
            var utility = new window.IconPickerUtility({
                propertyKey: propKey,
    ...
    renderArrayItemLinkSelector(arrayKey, index, propKey, config, value) {
        ...
        typeSelect.addEventListener('change', () => {
            const newType = typeSelect.value;
            this.updateArrayItemLinkInput(wrapper, { type: newType, url: '', id: null, title: '' }, arrayKey, index, propKey);
        });
    ...
    renderArrayItemImageInput(arrayKey, index, propKey, config, value) {
        ...
        selectBtn.addEventListener('click', async () => {
            ...
            if (window.selectImageFromLibrary) {
                ...
                console.warn('Media library not available. Falling back to URL input.');
                const url = await AdminModal.prompt({message: 'Enter image URL:', defaultValue: hiddenInput.value});
                if (url) {
                    hiddenInput.value = url;
                    previewContainer.innerHTML = `
                        <img src="${url}" alt="Preview" class="media-library-preview-img">
    ...
    refreshArrayInput(arrayKey) {
        const container = document.querySelector(`.array-input-container[data-array-key="${arrayKey}"]`);
        if (!container) return;
        ...
        if (itemsList) {
            ...
            if (Array.isArray(items)) {
                items.forEach((item, index) => {
                    const itemElement = this.renderArrayItem(arrayKey, index, item, itemProperties, arrayConfig);
                    itemsList.appendChild(itemElement);
                });
            }
        }
