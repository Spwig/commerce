...
def translation_indicator(element, show_always=False):
    ...
    total_target_languages = len([l for l in available_langs if l[0] != primary_lang])
...
def should_lazy_load_translation(context, element):
    ...
    # Check if content is large (worth lazy loading)
    text_content = content.get('text', '')
    if len(text_content) > 500:  # Threshold for lazy loading
        return True
