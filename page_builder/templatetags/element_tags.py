...
def render_element(context, element):
    ...
    try:
        ...
        logger.debug(
            f"Successfully rendered element {element.element_type} using {'modular' if use_modular else 'legacy'} template"
        )
        ...
        return {
            ...
            "rendered_content": mark_safe(
                f'<div class="element-error bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">Element "{element.element_type}" failed to render: {str(e)}</div>'
            ),
            "use_modular": False,
            "template_path": template_path,
            "error": str(e),
...
def get_page_elements(page):
    ...
    return Element.objects.filter(page=page, parent_element__isnull=True).order_by("order")
...
def focal_point_responsive_css(element):
    ...
    if isinstance(default_fp, dict) and "x" in default_fp:
        ...
        styles.append("  object-position: var(--fp-x) var(--fp-y);")
        styles.append("}")
    ...
    if has_tablet:
        ...
        if isinstance(tablet_fp, dict) and "x" in tablet_fp:
            ...
            styles.append("@media (max-width: 1024px) {")
            styles.append(f'  .pb-image__img[data-element-id="{element_id}"] {{')
    ...
    if has_mobile:
        ...
        if isinstance(mobile_fp, dict) and "x" in mobile_fp:
            ...
            styles.append("@media (max-width: 640px) {")
            styles.append(f'  .pb-image__img[data-element-id="{element_id}"] {{')
