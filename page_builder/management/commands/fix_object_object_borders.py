...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not affected_elements:
            self.stdout.write(
                self.style.SUCCESS("No elements found with [object Object] in border field")
            )
            return

        self.stdout.write(
            f"Found {len(affected_elements)} element(s) with [object Object] in border field"
        )
        ...
        for element in affected_elements:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ Cleared border field for element {element.id}")
            )
