...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if locale:
            ...
            locales_to_compile = [locale]
        else:
            # Compile all available locales
            locales_to_compile = ['de', 'en', 'es', 'fr', 'ja', 'pt', 'zh-hans']
        ...
        if errors:
            self.stdout.write(
                self.style.ERROR(f'Failed to compile {len(errors)} file(s)')
            )
            for error in errors:
        ...
        self.stdout.write('')
        self.stdout.write(self.style.WARNING('Remember to add the utilities locale path to your Django settings:'))
        self.stdout.write(
            f"LOCALE_PATHS = [\n"
            f"    ...,\n"
            f"    BASE_DIR / 'page_builder' / 'utilities' / 'locale',\n"
            f"]"
        )