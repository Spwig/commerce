...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "action", choices=["list", "reload", "info", "validate"], help="Action to perform"
        )

        parser.add_argument("--element", type=str, help="Element type for info/validate actions")

        parser.add_argument("--category", type=str, help="Filter by category for list action")
    ...
    def list_elements(self, registry, options):
        ...
        if output_format == "json":
            data = []
            for _element_type, element in elements_dict.items():
    ...
    def show_element_info(self, registry, options):
        ...
        if output_format == "json":
            ...
            info_data = [
                ...
                (
                    "Translation Domain",
                    element.translation_domain if element.supports_translation else "N/A",
                ),
            ...
            if element.properties:
                ...
                for prop_name, prop_config in element.properties.items():
                    ...
                    translatable = (
                        " (translatable)" if prop_config.get("translatable", False) else ""
                    )
                    self.stdout.write(
                        f"  {prop_name:<20}: {prop_label} [{prop_type}]{required}{translatable}"
                    )
    ...
    def validate_elements(self, registry, options):
        ...
        for element_type, element in elements_to_validate.items():
            if element is None:
                self.stdout.write(self.style.ERROR(f"✗ {element_type}: Element not found"))
                invalid_count += 1
                continue
            ...
            if is_valid:
                self.stdout.write(self.style.SUCCESS(f"✓ {element_type}: Valid"))
                valid_count += 1
            else:
                issues = []
                ...
                self.stdout.write(self.style.ERROR(f"✗ {element_type}: {', '.join(issues)}"))
                invalid_count += 1
