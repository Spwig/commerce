...
class PageView(TemplateView):
    ...
    def get_page_object(self):
        ...
        if slug:
            # Get page by slug
            try:
                page = Page.objects.select_related('theme').prefetch_related(
                    'elements'
    ...
    def get_home_context(self):
        ...
        return {
            'featured_products': Product.objects.filter(is_featured=True, status='published', hide_from_storefront=False).exclude(sales_channel='pos_only')[:8],
    ...
    def get_product_context(self):
        ...
        if product_slug:
            product = get_object_or_404(Product, slug=product_slug, status='published', sales_channel__in=['all', 'online_only'])
            related_products = Product.objects.filter(
                category=product.category, status='published'
            ).exclude(id=product.id).exclude(sales_channel='pos_only')[:4]
            return {
                'product': product,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if page.cache_timeout > 0:
            ...
            return cache_page(timeout, key_prefix=cache_key)(
                super().dispatch
            )(request, *args, **kwargs)
...
def home_view(request):
    ...
    featured_products = Product.objects.filter(
        status='published',
        is_featured=True
    ).exclude(sales_channel='pos_only').select_related('category').prefetch_related('images')[:8]

    if not featured_products.exists():
        featured_products = Product.objects.filter(
            status='published'
        ).exclude(sales_channel='pos_only').select_related('category').prefetch_related('images').order_by('-created_at')[:8]

    return render(request, 'page_builder/simple_home.html', {
...
def category_view(request, category_slug=None):
    ...
    if template_key == 'accordion' and not category:
        ...
        cats = context['categories'] if isinstance(context['categories'], list) else list(context['categories'])
...
def checkout_view(request):
    ...
    from design.template_registry import (
        get_checkout_options, get_checkout_template_path,
    )
        ...
        try:
            ...
            if intent.status == 'succeeded' and intent.order:
                return redirect(
                    ...
                    order_number=intent.order.order_number
                )
            ...
            return render(request, 'page_builder/checkout_return.html', {
...
def order_confirmation_view(request, order_number):
    ...
    is_guest_user = (
        ...
        request.user.username.startswith('guest_')
    )
...
def product_view(request, product_slug):
    ...
    if region:
        region_unavailable = not Product.objects.filter(
            pk=product.pk, status='published'
        ).available_in_region(region).exists()
    ...
    variants = product.variants.filter(is_active=True).select_related(
        'image_asset'
    ).prefetch_related(
        'images__media_asset',
    ...
    for variant in variants:
        ...
        variant_info = {
            ...
            'in_stock': (not product.track_inventory) or variant.stock_status != 'out_of_stock' or product.allow_backorders,
            'color_swatch': None,
            'attributes': {}
        }
    ...
    attribute_assignments = product.attribute_assignments.all().prefetch_related(
    ...
    related_products = Product.objects.filter(
        ...
        status='published'
    ).exclude(id=product.id).exclude(sales_channel='pos_only').prefetch_related('images__media_asset')[:4]
    ...
    if product.show_reviews:
        ...
        if stats['total_count'] > 0:
            review_stats = {
                ...
                'breakdown': {
                    5: {'count': stats['five_star'], 'percent': round(stats['five_star'] / stats['total_count'] * 100)},
                    4: {'count': stats['four_star'], 'percent': round(stats['four_star'] / stats['total_count'] * 100)},
                    3: {'count': stats['three_star'], 'percent': round(stats['three_star'] / stats['total_count'] * 100)},
                    2: {'count': stats['two_star'], 'percent': round(stats['two_star'] / stats['total_count'] * 100)},
                    1: {'count': stats['one_star'], 'percent': round(stats['one_star'] / stats['total_count'] * 100)},
                }
            }
    ...
    from design.models import PageTemplateConfig
    from design.template_registry import (
        get_product_options, get_product_template_path,
    )
    ...
    if product.product_type == 'bundle':
        bundle_items = list(
            product.bundle_items.select_related(
                'component_product', 'component_variant'
            ).prefetch_related(
    ...
    if product.product_type == 'booking':
        from catalog.models import BookingConfig, BookingResource, BookingPersonType
        try:
...
def _render_configurator(request, product):
    ...
    from catalog.models import ConfigurationSlot, CompatibilityRule, ConfigurationPreset
    ...
    try:
        ...
        scene = SceneConfig.objects.select_related(
            'base_model', 'environment_image'
        ).prefetch_related(
    ...
    if product.category:
        related_products = Product.objects.filter(
            category=product.category,
            status='published'
        ).exclude(id=product.id).exclude(sales_channel='pos_only').prefetch_related(
            'images__media_asset'
        )[:4]
...
def visual_builder(request, page_id):
    ...
    return render(request, 'page_builder/visual_builder.html', {
        'page': page,
...
def render_element_ajax(request, element_id):
    ...
    element = get_object_or_404(
        Element.objects.select_related('page'),
        id=element_id, is_active=True
    )
