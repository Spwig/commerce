...
from .models import (
    Page, Element,
    PageTemplate, ElementTemplate,
    VisibilityRule, RuleGroup, RuleGroupMember
)
...
class ElementInline(admin.TabularInline):
    ...
    extra = 0
    fields = [
        'name', 'element_type', 'order', 'column_span', 
...
# PageSectionInline removed - sections no longer used
# Elements are added directly to pages or nested in containers
    extra = 0
    fields = [
        ('name', 'order'),
        ('width', 'background_type'),
        ('is_active', 'show_on_mobile', 'show_on_tablet', 'show_on_desktop'),
        ('animation_type', 'animation_delay'),
    ]
    ordering = ['order']
...
class PageAdmin(TranslatableAdminMixin, SEOGeneratorAdminMixin, admin.ModelAdmin):
    ...
    list_display = [
        'title', 'page_type', 'status_display', 'theme_display',
        'elements_count', 'is_default_for_type', 'system_page_display', 'preview_link', 'updated_at'
    ]
    list_filter = [
        'page_type', 'status', 'is_default_for_type', 'is_system_page',
        'requires_auth', 'theme', 'created_at'
    ]
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('title', 'slug', 'page_type', 'status')
        }),
        (_('Content & Design'), {
            'fields': ('theme', 'template_variant', 'is_default_for_type', 'is_system_page')
        }),
        (_('SEO & Metadata'), {
            'fields': ('meta_title', 'meta_description', 'meta_keywords', 'og_image', 'seo_auto_generated'),
            'classes': ('collapse',)
        }),
        (_('Advanced Settings'), {
            'fields': ('requires_auth', 'cache_timeout'),
            'classes': ('collapse',)
        }),
        ...
        (_('Publishing Info'), {
            'fields': ('created_by', 'created_at', 'updated_at', 'published_at', 'preview_link'),
            'classes': ('collapse',)
        })
    )
    ...
    actions = ['publish_pages', 'unpublish_pages', 'duplicate_pages', 'create_template_from_pages', 'regenerate_thumbnails']
    ...
    def has_delete_permission(self, request, obj=None):
        ...
        if obj.is_system_page:
            ...
            try:
                ...
                page_fields = [
                    'home_page', 'privacy_page', 'terms_page',
                    'cookie_page', 'shipping_page', 'returns_page',
                    'error_404_page', 'error_500_page'
                ]
            ...
            try:
                ...
                for field, label in field_labels.items():
                    if getattr(settings, field, None) == obj:
                        messages.error(
                            request,
                            _("Cannot delete this page: It is currently assigned as the {} in Site Settings. "
                              "Please assign a different page first.").format(label)
                        )
                        return
            except:
                pass
    ...
    def delete_queryset(self, request, queryset):
        ...
        try:
            ...
            if protected.exists():
                ...
                messages.warning(
                    request,
                    _("The following pages could not be deleted because they are assigned in Site Settings: {}").format(
                        ', '.join(protected_titles)
                    )
                )
    ...
    def create_template_from_pages(self, request, queryset):
        ...
        return redirect(f'/admin/page_builder/page/create-templates/?pages={",".join(map(str, selected))}')
    ...
    def regenerate_thumbnails(self, request, queryset):
        ...
        if pages_to_capture:
            ...
            self.message_user(
                request,
                _('Queued %(count)d page(s) for thumbnail regeneration. Thumbnails will be captured in the background.') % {'count': len(pages_to_capture)}
            )
        else:
...
class ElementAdmin(TranslationAdmin):
    ...
    list_display = [
        'name', 'element_type', 'page', 'order', 'column_span',
        'visibility_info', 'is_active', 'responsive_display', 'has_link'
    ]
    list_filter = [
        'page', 'element_type', 'is_active', 'show_on_mobile',
        'show_on_tablet', 'show_on_desktop', 'text_align'
    ]
    ...
    fieldsets = (
        ...
        (_('Content'), {
            ...
            'description': _('Content is stored as JSON. Examples:<br>'
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('page').prefetch_related('visibility_rules')
    ...
    def responsive_display(self, obj):
        ...
        return format_html(' '.join(devices)) if devices else format_html('<span style="color: #999;">—</span>')
    responsive_display.short_description = _('Devices')
...
class PageTemplateAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'page_type', 'category', 'version', 'usage_count',
        'is_public', 'is_premium', 'created_by', 'created_at'
    ]
    list_filter = [
        'page_type', 'category', 'is_public', 'is_premium', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        (_('Settings'), {
            'fields': ('is_public', 'is_premium', 'preview_image')
        }),
        (_('Usage Statistics'), {
            'fields': ('usage_count', 'template_preview'),
            'classes': ('collapse',)
        }),
        (_('Author Info'), {
...
class ElementTemplateAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'element_type', 'category', 'usage_count',
        'is_public', 'created_by', 'created_at'
    ]
...
class VisibilityRuleAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'rule_type_display', 'operator', 'is_active',
        'priority', 'cache_duration', 'updated_at', 'wizard_link'
    ]
    list_filter = [
        'rule_type', 'operator', 'is_active', 'priority'
    ]
...
class RuleGroupAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'logic_operator_display', 'rules_count',
        'parent_group', 'is_active', 'updated_at', 'builder_link'
    ]
