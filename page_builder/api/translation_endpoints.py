...
def translate_element(request):
    ...
    try:
        ...
        logger.info(
            f"Translation request: element_id={element_id}, languages={target_languages}, fields={fields}, force_immediate={force_immediate}"
        )
        ...
        if not health_status.get("available", False):
            ...
            response = fallback.handle_service_unavailable(
                element.content, target_languages, element_id
            )
            return JsonResponse(response, status=503)

        # Check if we should recommend scheduling
        text_content = " ".join([str(element.content.get(field, "")) for field in fields])
        char_count = len(text_content)

        should_schedule, reason = should_schedule_translation(char_count, len(target_languages))
            ...
            return JsonResponse(
                {
                    ...
                    "reason": reason,
                    "estimated_time": estimate_translation_time(char_count, len(target_languages)),
        ...
        if warnings:
            fallback = TranslationFallbackHandler()
            degradation_response = fallback.handle_service_degraded(text_content, target_languages)

            # Add to response but continue
            response_data = {"warnings": warnings, "degradation_info": degradation_response}
        else:
            response_data = {}
        ...
        if schedule:
            ...
            job = _create_translation_job(element, target_languages, fields, char_count)
        ...
        logger.info(
            f"Starting immediate translation for element {element_id} to {len(target_languages)} languages: {target_languages}"
        )
        try:
            ...
            for lang in target_languages:
                ...
                try:
                    ...
                    for field in fields:
                        ...
                        field_content = content_to_translate or element.content.get(field, "")

                        if field_content:
                            logger.info(
                                f"Translating field '{field}' to {lang}: '{field_content[:50]}...'"
                            )
                            print(
                                f"[DEBUG] Calling translate: text='{field_content}', source={get_primary_language()}, target={lang}"
                            )
                            try:
                                ...
                                if translated:
                                    ...
                                    logger.info(
                                        f"Translation result for {lang}: '{translated[:50]}...'"
                                    )
                                else:
                                    logger.warning(
                                        f"No translation returned for field '{field}' to {lang}"
                                    )
            ...
            if failed_languages:
                ...
                partial_response = fallback.handle_partial_failure(
                    translations, failed_languages, "Translation service error"
                )
                ...
                response_data.update(
                    {
                        ...
                        if message_parts
                        else "No translations completed",
...
def clear_element_translations(request, element_id):
    ...
    return JsonResponse({"success": True, "message": "Translations cleared successfully"})
...
def schedule_page_translation(request):
    ...
    try:
        ...
        elements = Element.objects.filter(page_id=page_id, element_type="text")

        if not elements:
            return JsonResponse({"message": "No text elements found on page"}, status=404)
        ...
        should_schedule, reason = health.should_schedule_translation(
            total_chars, len(target_languages)
        )

        estimated_time = health.estimate_translation_time(total_chars, len(target_languages))

        # Create translation jobs for each element
        jobs_created = []
        for element in elements:
            job = _create_translation_job(
                element, target_languages, ["text"], len(element.content.get("text", ""))
            )
            if job:
    ...
    try:
        ...
        job = TranslationJob.objects.create(
            ...
            content_snapshot={
                ...
                "fields": fields,
                "char_count": char_count,
                "content": {field: element.content.get(field, "") for field in fields},
            },
        )
