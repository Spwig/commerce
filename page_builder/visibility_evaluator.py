...
class ContextCollector:
    ...
    def _get_user_context(self, request) -> dict[str, Any]:
        ...
        if user.is_authenticated:
            ...
            try:
                ...
                if metrics:
                    context["lifetime_value"] = (
                        float(metrics.lifetime_value.amount) if metrics.lifetime_value else 0
                    )
    ...
    def _get_geo_context(self, request) -> dict[str, Any]:
        ...
        context["timezone"] = request.session.get(
            "django_timezone", timezone.get_current_timezone_name()
        )
        ...
        if request.user.is_authenticated:
            try:
                ...
                context["has_abandoned_cart"] = AbandonedCart.objects.filter(
                    user=request.user, recovered=False
                ).exists()
    ...
    def _is_mobile(self, user_agent: str) -> bool:
        ...
        return (
            any(pattern in user_agent for pattern in mobile_patterns) and "ipad" not in user_agent
        )
    ...
    def _evaluate_rule_type(self, rule, context: dict[str, Any]) -> bool:
        ...
        # E-commerce rules
            ...
            return self._compare_numeric(
                context["ecommerce"]["wishlist_items"], operator, rule_value
            )
    ...
    def _compare_numeric(self, actual_value, operator: str, expected_value) -> bool:
        ...
        try:
            ...
            if operator == "equals":
                ...
                return actual < expected
            elif operator == "between" and isinstance(expected_value, dict):
