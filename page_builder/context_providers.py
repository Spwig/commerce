...
def reviews_display_context(content: dict, request=None) -> dict:
    ...
    queryset = ProductReview.objects.filter(is_approved=True).select_related("product", "user")
    ...
    stats = None
    if queryset.exists():
        agg = queryset.aggregate(average_rating=Avg("rating"), total_reviews=Count("id"))
        # Build rating breakdown (5 stars down to 1) - single query
        # Clear ordering to avoid it being included in GROUP BY
        rating_breakdown = dict.fromkeys(range(5, 0, -1), 0)
...
def category_showcase_context(content: dict, request=None) -> dict:
    ...
    def _base_queryset():
        return (
            ...
            .annotate(product_count=Count("products", filter=Q(products__status="published")))
        )
    ...
    if source_type == "top_level":
        ...
        queryset = queryset.filter(parent_id=parent_id) if parent_id else queryset.none()
    elif source_type == "specific":
        category_ids = _parse_category_ids(content.get("category_ids", []))
        queryset = queryset.filter(id__in=category_ids) if category_ids else queryset.none()
        ...
        if promotion_id:
            try:
                promotion = Promotion.objects.get(
                    id=promotion_id, is_active=True, start_date__lte=now
                )
...
def promotion_banner_context(content: dict, request=None) -> dict:
    ...
    if promotion_id:
        try:
            promotion = Promotion.objects.get(id=promotion_id, is_active=True, start_date__lte=now)
            # Check end date
            if promotion.end_date and promotion.end_date < now:
...
def product_grid_context(content: dict, request=None) -> dict:
    ...
    if layout == "featured" and products:
        ...
        int(content.get("slides_per_view", 4))
...
def loyalty_banner_context(content: dict, request=None) -> dict:
    ...
    from loyalty.models import LoyaltyMember, LoyaltyProgram, LoyaltyTier
    ...
    tiers = list(
        LoyaltyTier.objects.filter(program=program, is_active=True).order_by("points_required")
    )

    # Get member info if user is authenticated
    member = None
    if request and request.user.is_authenticated:
        member = LoyaltyMember.objects.filter(program=program, user=request.user).first()

    return {"program": program, "tiers": tiers, "member": member}
...
def voucher_code_display_context(content: dict, request=None) -> dict:
    ...
    vouchers = (
        VoucherCode.objects.filter(is_active=True, is_public=True, valid_from__lte=now)
...
def navigation_menu_context(content: dict, request=None) -> dict:
    ...
    try:
        menu = Menu.objects.prefetch_related(
            "items__page_reference", "items__category_reference", "items__children"
...
def _filter_menu_items_by_visibility(items: list, request) -> list:
    ...
    for item in items:
        ...
        for rule in visibility_rules:
            ...
            if rule_type == "user_status" and (
                rule_value == "logged_in"
                and not user_authenticated
                or rule_value == "logged_out"
