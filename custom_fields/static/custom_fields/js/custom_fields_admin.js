...
(function () {
  ...
  document.querySelectorAll('.cf-modal-close, .cf-modal-cancel').forEach(function (btn) {
    btn.addEventListener('click', function () {
      const modal = this.closest('.cf-modal-overlay');
  ...
  document.querySelectorAll('.cf-btn-edit-group').forEach(function (btn) {
    btn.addEventListener('click', function () {
      const card = this.closest('.field-group-card');
      const groupId = card.dataset.groupId;
      const name = card.querySelector('.field-group-header-left h3').textContent.trim();
      const showStorefront = card.dataset.showStorefront === 'true';
  ...
  document.getElementById('groupModalSave').addEventListener('click', function () {
    ...
    const showStorefront = document.getElementById('groupShowStorefront').checked;
    const ctId = document.getElementById('groupModalContentTypeId').value;
  ...
  document.querySelectorAll('.cf-btn-delete-group').forEach(function (btn) {
    btn.addEventListener('click', async function () {
      const confirmMsg = i18n.confirmDeleteGroup || 'Are you sure you want to delete this group?';
      if (
      ...
      const groupId = this.closest('.field-group-card').dataset.groupId;
  ...
  function updateValidationVisibility(fieldType) {
    ...
    const transOpt = document.getElementById('translatableOption');
    if (fieldType === 'text' || fieldType === 'textarea') {
      ...
      fetch(BASE_URL + '/fields/' + fieldId + '/', {
        ...
        .then(function (resp) {
          ...
          if (f.field_type === 'text' || f.field_type === 'textarea') {
            ...
            const choices = vc.choices || [];
            choices.forEach(function (c) {
              addChoiceRow(c.value, c.label);
            });
          }
  ...
  document.getElementById('fieldModalSave').addEventListener('click', function () {
    const fieldId = document.getElementById('fieldModalId').value;
    const groupId = document.getElementById('fieldModalGroupId').value;
    ...
    if (fieldType === 'text' || fieldType === 'textarea') {
      const minLen = document.getElementById('fieldMinLength').value;
      const maxLen = document.getElementById('fieldMaxLength').value;
      ...
      const nMin = document.getElementById('fieldMin').value;
      const nMax = document.getElementById('fieldMax').value;
      if (nMin !== '') validationConfig.min = parseFloat(nMin);
      if (nMax !== '') validationConfig.max = parseFloat(nMax);
    } else if (fieldType === 'decimal') {
      const dMin = document.getElementById('fieldDecMin').value;
      const dMax = document.getElementById('fieldDecMax').value;
      const places = document.getElementById('fieldDecPlaces').value;
      ...
      document.querySelectorAll('#choicesList .cf-choice-row').forEach(function (row) {
        const val = row.querySelector('.choice-value').value.trim();
        const label = row.querySelector('.choice-label').value.trim();
        if (val && label) {
  ...
  document.querySelectorAll('.cf-btn-delete-field').forEach(function (btn) {
    btn.addEventListener('click', async function () {
      const confirmMsg = i18n.confirmDeleteField || 'Are you sure you want to delete this field?';
      if (
        !(await AdminModal.confirm({
          message: confirmMsg,
          danger: true,
  ...
  function addChoiceRow(value, label) {
    ...
    row.innerHTML =
      '<input type="text" class="choice-value" placeholder="Value" value="' +
      (value || '') +
      '">' +
      '<input type="text" class="choice-label" placeholder="Label" value="' +
      (label || '') +
      '">' +
  ...
  const addChoiceBtn = document.querySelector('.cf-btn-add-choice');
  if (addChoiceBtn) {
  ...
  document.addEventListener('click', function (e) {
    const hex = e.target.closest('.color-hex-display[data-action="trigger-color-picker"]');
    if (hex) {
      const prev = hex.previousElementSibling;
      if (prev) {
        prev.click();
      }
    }
