...
class CustomFieldGroup(SoftDeleteModel):
    ...
    name = models.CharField(
        _("group name"), max_length=100, help_text=_("Display name for this group of fields")
    )
    slug = models.SlugField(
        _("slug"), max_length=100, help_text=_("URL-safe identifier (auto-generated from name)")
    )
    ...
    sort_order = models.PositiveIntegerField(
        _("sort order"), default=0, help_text=_("Display order (lower numbers appear first)")
    )
    ...
    translations = models.JSONField(
        default=dict, blank=True, help_text=_("Translated group names by language code")
    )
    ...
    def get_cached_for_content_type(cls, content_type):
        ...
        if groups is None:
            groups = list(
                cls.objects.filter(content_type=content_type, is_active=True).order_by(
                    "sort_order", "name"
                )
            )
...
class CustomFieldDefinition(SoftDeleteModel):
    ...
    name = models.CharField(
        _("field name"), max_length=100, help_text=_("Display label shown in forms")
    )
    slug = models.SlugField(
        _("slug"),
        max_length=100,
    ...
    is_required = models.BooleanField(
        _("required"), default=False, help_text=_("Whether this field must be filled in")
    )
    is_active = models.BooleanField(
        _("active"),
        default=True,
    ...
    sort_order = models.PositiveIntegerField(
        _("sort order"), default=0, help_text=_("Display order within the group")
    )
    translations = models.JSONField(
        default=dict,
        blank=True,
    ...
    def get_cached_for_content_type(cls, content_type):
        ...
        if definitions is None:
            definitions = list(
                cls.objects.filter(content_type=content_type, is_active=True)
