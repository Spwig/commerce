        ...
        try:
            ...
            if model_class:
                groups = CustomFieldGroup.objects.filter(
                    content_type=ct,
                    is_active=True
                ).prefetch_related('fields').order_by('sort_order')
...
def recycle_bin_view(request):
    ...
    """
    deleted_groups = CustomFieldGroup.all_objects.filter(
        is_deleted=True
    ).select_related('content_type').order_by('-deleted_at')

    deleted_fields = CustomFieldDefinition.all_objects.filter(
        is_deleted=True
    ).select_related('group', 'content_type').order_by('-deleted_at')
...
def restore_group(request, group_id):
    ...
    try:
        ...
        for field_def in CustomFieldDefinition.all_objects.filter(
