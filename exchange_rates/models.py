...
class ExchangeRateProviderAccount(models.Model):
    ...
    name = models.CharField(
        ...
        help_text=_(
            "Friendly name for this provider account (e.g., 'Main Exchange Rate Provider')"
        ),
    )
...
class ExchangeRateHistory(models.Model):
    ...
    target_currency = models.CharField(max_length=3, verbose_name=_("Target Currency"))

    rate = models.DecimalField(max_digits=18, decimal_places=6, verbose_name=_("Exchange Rate"))
...
class ManualExchangeRate(models.Model):
    ...
    exclude_from_auto_sync = models.BooleanField(
        ...
        help_text=_(
            "When enabled, this rate will not be overwritten by provider sync operations. "
    ...
    def clean(self):
        ...
        if self.rate is not None and self.rate <= 0:
            raise ValidationError({"rate": _("Exchange rate must be greater than zero.")})
