    ...
    class ProviderInstaller {
        async install(providerSlug, providerName) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/exchange-rates/providers/install/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
        ...
        updateCardToInstalled(providerSlug) {
            ...
            if (installBtn) {
                ...
                configureBtn.href = `/${langCode}/admin/exchange-rates/wizard/step1/`;
    ...
    class ProviderUpdater {
        async update(providerSlug, providerVersion) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/exchange-rates/providers/update/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
                        'X-CSRFToken': AdminUtils.getCsrfToken(),
                        'Content-Type': 'application/json',
        ...
        detailsButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                try {
                    ...
                    providerData.provider_type = 'exchange_rate';
