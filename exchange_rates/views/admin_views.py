...
def filter_exchange_rate_providers(request):
    ...
    # Build query
    providers = ExchangeRateProviderAccount.objects.select_related(
        'component', 'site'
    ).order_by('-is_primary', '-is_active', 'priority', 'name')
    ...
    provider_cards_html = ''.join([
        render_to_string('admin/exchange_rates/exchangerateprovideraccount/provider_card.html', {
            'provider': provider,
        }, request=request)
...
def toggle_provider_active(request, provider_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'message': _('An error occurred while updating the provider.')
        }, status=400)
...
def sync_provider_rates(request, provider_id):
    ...
    try:
        ...
        try:
            ...
            logger.error("Failed to decrypt credentials for provider %s: %s", provider_id, decrypt_error, exc_info=True)
            return JsonResponse({
                'success': False,
                'message': _('Failed to decrypt provider credentials. Please reconfigure the provider.')
            }, status=500)

        # Create provider instance
        provider_instance = provider_class(
            credentials=credentials,
            config=provider.settings or {}
        )

        # Fetch exchange rates
        try:
            # Get base currency and enabled currencies from site settings
            from core.models import SiteSettings, SupportedCurrency
            ...
            if not rates:
                ...
                return JsonResponse({
                    'success': False,
                    'message': _('No rates returned from provider')
                })
            ...
            logger.error("Error syncing rates for provider %s: %s", provider_id, sync_error, exc_info=True)
            provider.sync_status = 'error'
            ...
            return JsonResponse({
                'success': False,
                'message': _('Failed to sync exchange rates. Check the provider configuration.')
            }, status=500)

    except Exception as e:
        logger.error("Unexpected error in sync_provider_rates for %s: %s", provider_id, e, exc_info=True)
        return JsonResponse({
            'success': False,
...
def delete_provider(request, provider_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'message': _('An error occurred while deleting the provider.')
        }, status=400)
...
def provider_bulk_action(request):
    ...
    try:
        ...
        if not action or not provider_ids:
            return JsonResponse({
                'success': False,
                'message': _('Invalid action or no providers selected')
            }, status=400)
        ...
        if action == 'enable':
            ...
            return JsonResponse({
                'success': False,
                'message': _('Unknown action: {}').format(action)
            }, status=400)
        ...
        return JsonResponse({
            'success': False,
            'message': _('An error occurred while performing the bulk action.')
        }, status=400)
...
def toggle_manual_rate_active(request, rate_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'message': _('An error occurred while updating the rate.')
        }, status=400)
...
def sync_from_provider(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            'message': _('Sync complete: %(created)d created, %(updated)d updated, %(skipped)d locked (skipped)') % {
        ...
        return JsonResponse({
            'success': False,
            'message': _('An error occurred during sync.')
        }, status=500)
