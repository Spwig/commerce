...
from exchange_rates.models import ExchangeRateProviderAccount, ExchangeRate, ExchangeRateHistory, ManualExchangeRate
...
class ExchangeRateProviderAccountAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('exchange_rates/css/admin_provider_list.css',)
        }
    ...
    def _attach_logo_urls(providers):
        ...
        for provider in providers:
            ...
            provider_dir = INTEGRATIONS_DIR / 'exchange_rate_provider' / provider.component.slug / version
            ...
            if manifest_path.exists():
                ...
                if logo_filename:
                    ...
                    if logo_path.exists():
                        logo_url = static(f'exchange_rate_provider/{provider.component.slug}/current/{logo_filename}')
...
class ManualExchangeRateAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('exchange_rates/css/admin_manual_rates.css',)
        }
        js = ('exchange_rates/js/admin_manual_rates.js',)

    fieldsets = (
        (_('Currency Pair'), {
            'fields': ('site', 'base_currency', 'target_currency')
        }),
        (_('Rate'), {
            'fields': ('rate',)
        }),
        (_('Settings'), {
            'fields': ('is_active', 'exclude_from_auto_sync', 'notes')
        }),
        (_('Timestamps'), {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
            ...
            for rate in all_rates:
                ...
                if db_rate:
                    provider_rates[f'{rate.base_currency}/{rate.target_currency}'] = {
                        ...
                        'provider': db_rate.provider_account.name or db_rate.provider_account.component.name,
                        'fetched_at': db_rate.fetched_at.isoformat() if db_rate.fetched_at else None,
                    }
        except Exception:
            pass
