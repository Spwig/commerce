...
class MigrationStep(models.Model):
    ...
    step_type = models.CharField(
        max_length=30, choices=STEP_TYPES, help_text=_("Type of data being imported in this step")
    )
    ...
    # Statistics
    items_total = models.IntegerField(default=0, help_text=_("Total items to import in this step"))
    items_imported = models.IntegerField(default=0, help_text=_("Items successfully imported"))
    items_skipped = models.IntegerField(
        default=0, help_text=_("Items skipped (duplicates, validation failures)")
    )
    items_failed = models.IntegerField(default=0, help_text=_("Items that failed to import"))

    # Timing
    started_at = models.DateTimeField(null=True, blank=True, help_text=_("When this step started"))
    completed_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When this step completed")
    )
    duration_seconds = models.IntegerField(
        null=True, blank=True, help_text=_("Duration of this step in seconds")
    )

    # Error Tracking
    error_message = models.TextField(blank=True, help_text=_("Error message if step failed"))
    error_count = models.IntegerField(
        default=0, help_text=_("Number of errors encountered in this step")
    )
    warning_count = models.IntegerField(
        default=0, help_text=_("Number of warnings encountered in this step")
    )

    # Progress Details
    current_item = models.CharField(
        max_length=255, blank=True, help_text=_("Currently processing item (for progress display)")
    )
