...
class MigrationLog(models.Model):
    ...
    timestamp = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text=_("When this log entry was created")
    )
    ...
    level = models.CharField(
        max_length=10, choices=LEVEL_CHOICES, db_index=True, help_text=_("Log severity level")
    )

    message = models.TextField(help_text=_("Log message"))

    # Reference to source item (WooCommerce, Shopify, etc.)
    source_type = models.CharField(
        max_length=50, blank=True, help_text=_("Source data type (e.g., 'woocommerce_product')")
    )
    source_id = models.CharField(
        max_length=100, blank=True, db_index=True, help_text=_("ID from source system")
    )
    source_name = models.CharField(
        max_length=255, blank=True, help_text=_("Name/title of source item")
    )
    ...
    object_id = models.PositiveIntegerField(
        null=True, blank=True, help_text=_("ID of created object")
    )
    content_object = GenericForeignKey("content_type", "object_id")

    # Additional context (JSON for structured data)
    context = models.JSONField(default=dict, blank=True, help_text=_("Additional context data"))
    ...
    action = models.CharField(
        max_length=20, choices=ACTION_CHOICES, blank=True, help_text=_("Action performed")
    )
    ...
    @classmethod
    def log_error(
        cls, job, message, step=None, source_type=None, source_id=None, error=None, **context
    ):
