...
class MigrationMapping(models.Model):
    ...
    source_type = models.CharField(
        ...
        help_text=_("Type of source data")
    )

    source_field = models.CharField(
        max_length=100,
        help_text=_("Source field name (e.g., '_custom_brand', 'metafields.brand')")
    )

    source_field_label = models.CharField(
        max_length=200,
        blank=True,
        help_text=_("Human-readable label for source field")
    )
    ...
    dest_model = models.CharField(
        max_length=50,
        choices=DEST_MODELS,
        help_text=_("Destination model name")
    )

    dest_field = models.CharField(
        max_length=100,
        help_text=_("Destination field name (e.g., 'brand', 'short_description')")
    )

    dest_field_label = models.CharField(
        max_length=200,
        blank=True,
        help_text=_("Human-readable label for destination field")
    )
    ...
    transform_function = models.CharField(
        max_length=100,
        blank=True,
        help_text=_("Python function name for custom transformation")
    )

    # Default value if source field is empty
    default_value = models.TextField(
        blank=True,
        help_text=_("Default value if source field is missing or empty")
    )

    # Mapping rules
    is_required = models.BooleanField(
        default=False,
        help_text=_("Whether this field is required (skip item if missing)")
    )

    skip_if_empty = models.BooleanField(
        default=False,
        help_text=_("Skip entire item if this field is empty")
    )

    # Usage statistics
    times_used = models.IntegerField(
        default=0,
        help_text=_("Number of times this mapping was applied")
    )

    times_failed = models.IntegerField(
        default=0,
        help_text=_("Number of times transformation failed")
    )

    # Auto-detected or manually created
    is_auto_detected = models.BooleanField(
        default=False,
        help_text=_("Whether this mapping was auto-detected from data")
    )
