...
PAGE_FIELDS = [
    ...
    'meta_title', 'meta_description', 'meta_keywords', 'seo_auto_generated',
    'translations', 'is_default_for_type', 'is_system_page',
    'requires_auth', 'cache_timeout',
    'hide_header', 'hide_footer', 'page_design_config',
    'template_variant', 'css_classes', 'layout_config',
    'style_overrides', 'responsive_config', 'inherit_parent_theme',
]

ELEMENT_FIELDS = [
    'element_type', 'name', 'content', 'translations',
    'order', 'column_span', 'column_offset',
...
PAGE_TEMPLATE_FIELDS = [
    'name', 'description', 'page_type', 'template_data',
    'version', 'category', 'is_premium', 'is_public',
]

ELEMENT_TEMPLATE_FIELDS = [
    'name', 'description', 'element_type', 'template_data',
...
class PageContentSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from page_builder.models import Page, PageTemplate, ElementTemplate
        return (Page.objects.count() +
                PageTemplate.objects.count() +
                ElementTemplate.objects.count())

    def export(self, credential_mode='redact'):
        from page_builder.models import Page, Element, PageTemplate, ElementTemplate

        items = []
        files = {}

        # Pages with elements
        for page in Page.objects.select_related('theme', 'header_template', 'footer_template').all():
            ...
            data['_elements'] = []
            root_elements = Element.objects.filter(
                page=page, parent_element__isnull=True
            ).order_by('order')
    ...
    def import_data(self, data, dry_run=False, sync_mode='additive'):
        ...
        ordered = sorted(items, key=lambda x: {
            'PageTemplate': 0, 'ElementTemplate': 1, 'Page': 2,
        }.get(x.get('_model', ''), 99))
