        ...
        if existing:
            ...
            if "_component_slug" in item:
                try:
                    ...
                    provider.component = ComponentRegistry.objects.get(slug=item["_component_slug"])
                except Exception:
                    pass
            provider.save()
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if model_type == "SupportedCurrency":
                ...
                existing = ExchangeRateProviderAccount.objects.filter(name=item.get("name")).first()
                ...
                if existing:
                    ...
                    if field_changes:
                        changes.append(
                            {
                                "type": "modify",
                                "model": "ExchangeRateProviderAccount",
                                "name": name,
                                "changes": field_changes,
                            }
                        )
