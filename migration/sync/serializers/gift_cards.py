...
class GiftCardsSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        ...
        from vouchers.models import AppliedVoucher, VoucherUsage

        return (
            GiftCard.objects.count()
            + GiftCardTransaction.objects.count()
            + VoucherUsage.objects.count()
            + AppliedVoucher.objects.count()
        )
    ...
    def export(self, credential_mode="redact"):
        ...
        for gc in (
            GiftCard.objects.select_related("product", "order_item")
            .prefetch_related(
                "transactions",
            )
            .all()
