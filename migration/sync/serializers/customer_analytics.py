...
CUSTOMER_NOTE_FIELDS = [
    ...
    'requires_follow_up', 'completed', 'is_internal',
]

ABANDONED_CART_FIELDS = [
    'estimated_reason', 'total_items',
    'recovery_emails_sent', 'recovered',
]

COHORT_FIELDS = [
    'cohort_date', 'acquisition_channel', 'first_product_category',
    'customer_count',
]

COHORT_METRICS_FIELDS = [
    'months_since_acquisition', 'active_customers',
    'cumulative_orders', 'retention_rate',
]
COHORT_METRICS_MONEY_FIELDS = ['cumulative_revenue', 'average_ltv']

PREFERENCE_LOG_FIELDS = [
    'action', 'old_value', 'new_value',
    'ip_address', 'user_agent', 'source', 'notes',
]
...
class CustomerAnalyticsSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from customers.models import CustomerNote, AbandonedCart, CustomerCohort, CohortMetrics
    ...
    def export(self, credential_mode='redact'):
        from customers.models import CustomerNote, AbandonedCart, CustomerCohort
        from accounts.models import PreferenceChangeLog
        ...
        for cohort in CustomerCohort.objects.prefetch_related('metrics').all():
            ...
            if hasattr(data.get('cohort_date', None), 'isoformat'):
    ...
    def _import_cohort(self, item):
        from customers.models import CustomerCohort, CohortMetrics
        from django.utils.dateparse import parse_date

        cohort_date = parse_date(item['cohort_date']) if isinstance(item['cohort_date'], str) else item['cohort_date']
