...
ACCEPTANCE_FIELDS = [
    'license_version', 'software_version', 'accepted_by_email',
    'accepted_via', 'license_checksum', 'is_current', 'notes',
]
...
class PlatformLicenseSerializer(BaseSyncSerializer):
    ...
    def import_data(self, data, dry_run=False, sync_mode='additive'):
        ...
        if not items:
            return {
                ...
                'transfer_token': None, 'deferred': False,
            }
    ...
    def _import_acceptance_record(self, a_data):
        ...
        if license_version and license_checksum:
            if LicenseAcceptanceRecord.objects.filter(
        ...
        if accepted_at:
            LicenseAcceptanceRecord.objects.filter(pk=record.pk).update(
    ...
    def generate_diff(self, remote_data):
        ...
        return {
            ...
            'warnings': warnings,
            'summary': (
                f'License transfer for {license_key}, '
                f'{acceptance_count} acceptance record(s)'
            ),
        }
