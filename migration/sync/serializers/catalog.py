...
PRODUCT_FIELDS = [
    ...
    "gift_card_currency",
    "allow_customization",
    "customization_preview_template",
...
def _serialize_money_field(data, instance, field_name):
    ...
    if money_obj is not None and hasattr(money_obj, "amount"):
        ...
        data[f"_{field_name}_currency"] = str(getattr(instance, f"{field_name}_currency", ""))
    else:
...
class CatalogSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode="redact"):
        ...
        # 3. Category
        for cat in Category.objects.select_related("parent", "image_asset", "banner_asset").all():
            ...
            if cat.image_asset:
                data["_image_asset_meta"] = _export_media_asset_meta(cat.image_asset)
            ...
            if cat.banner_asset:
                data["_banner_asset_meta"] = _export_media_asset_meta(cat.banner_asset)
        ...
        for product in Product.objects.select_related("category", "brand").all():
            data = {f: getattr(product, f) for f in PRODUCT_FIELDS}
            data["_source_pk"] = product.pk
            data["_model"] = "Product"
            data["_category_slug"] = product.category.slug
            data["_brand_slug"] = product.brand.slug if product.brand else None
        ...
        for assign in (
            ProductAttributeAssignment.objects.select_related("product", "attribute")
            .prefetch_related("allowed_values")
            .all()
        ):
            data = {f: getattr(assign, f) for f in PRODUCT_ATTRIBUTE_ASSIGNMENT_FIELDS}
        ...
        # 7. ProductVariant
        for variant in (
            ProductVariant.objects.select_related("product", "image_asset")
            .prefetch_related("selected_attributes__attribute")
            .all()
        ):
            ...
            data["_selected_attributes"] = [
                ...
                for av in variant.selected_attributes.select_related("attribute").all()
            ]
            if variant.image_asset:
                data["_image_asset_meta"] = _export_media_asset_meta(variant.image_asset)
                fd = export_file_field(variant.image_asset, "original_file")
                if fd:
                    files[f"ProductVariant:{variant.sku}:image_asset"] = fd
            items.append(data)

        # 8. ProductImage
        for img in ProductImage.objects.select_related("product", "media_asset").all():
            ...
            if img.media_asset:
                data["_media_asset_meta"] = _export_media_asset_meta(img.media_asset)
                fd = export_file_field(img.media_asset, "original_file")
                if fd:
                    files[f"ProductImage:{img.product.sku}:{img.position}:media_asset"] = fd
            items.append(data)
    ...
    def import_data(self, data, dry_run=False, sync_mode="additive"):
        ...
        try:
            with transaction.atomic():
                ...
                attributes = [i for i in items if i.get("_model") == "ProductAttribute"]
                attr_values = [i for i in items if i.get("_model") == "AttributeValue"]
                categories = [i for i in items if i.get("_model") == "Category"]
                brands = [i for i in items if i.get("_model") == "Brand"]
                products = [i for i in items if i.get("_model") == "Product"]
                assignments = [i for i in items if i.get("_model") == "ProductAttributeAssignment"]
                variants = [i for i in items if i.get("_model") == "ProductVariant"]
                images = [i for i in items if i.get("_model") == "ProductImage"]
                bundle_items_list = [i for i in items if i.get("_model") == "BundleItem"]
                ...
                for item in attributes:
                    try:
                        ...
                        failed += 1
                        errors.append(f"ProductAttribute '{item.get('slug', '?')}': {e}")
                ...
                for item in attr_values:
                    try:
                        ...
                        failed += 1
                        errors.append(f"AttributeValue '{item.get('slug', '?')}': {e}")
                ...
                for item in categories:
                    try:
                        ...
                        failed += 1
                        errors.append(f"Category '{item.get('slug', '?')}': {e}")
                ...
                for item in brands:
                    try:
                        ...
                        failed += 1
                        errors.append(f"Brand '{item.get('slug', '?')}': {e}")
                ...
                for item in products:
                    try:
                        ...
                        failed += 1
                        errors.append(f"Product '{item.get('sku', '?')}': {e}")
                ...
                for item in variants:
                    try:
                        ...
                        failed += 1
                        errors.append(f"ProductVariant '{item.get('sku', '?')}': {e}")
    ...
    def _import_product_attribute(self, item):
        ...
        existing = ProductAttribute.objects.filter(slug=item["slug"]).first()
        if existing:
    ...
    def _import_attribute_value(self, item):
        ...
        attr = ProductAttribute.objects.filter(slug=item["_attribute_slug"]).first()
        if not attr:
            raise ValueError(f"ProductAttribute '{item['_attribute_slug']}' not found")

        existing = AttributeValue.objects.filter(attribute=attr, slug=item["slug"]).first()
        if existing:
    ...
    def _import_category_assets(self, cat, item, files, slug):
        ...
        if meta:
            asset = _import_media_asset_inline(meta, files, f"Category:{slug}:image_asset")
            if asset:
                cat.image_asset = asset
        meta = item.get("_banner_asset_meta")
        if meta:
            asset = _import_media_asset_inline(meta, files, f"Category:{slug}:banner_asset")
            if asset:
    ...
    def _set_category_parent(self, item):
        ...
        if cat and parent:
            ...
            logger.warning("Parent category '%s' not found for '%s'", parent_slug, slug)
    ...
    def _import_product(self, item):
        ...
        if not category:
            raise ValueError(f"Category '{cat_slug}' not found for product '{sku}'")
        ...
        if brand_slug:
            ...
            if not brand:
                logger.warning(
                    "Brand '%s' not found for product '%s', setting to None", brand_slug, sku
                )
    ...
    def _set_product_subscription_plans(self, product, item):
        ...
        try:
            ...
            for slug in missing:
                logger.warning(
                    "SubscriptionPlan '%s' not found for product '%s'", slug, product.sku
                )
        except ImportError:
            pass
    ...
    def _import_attribute_assignment(self, item):
        ...
        if not product:
            raise ValueError(f"Product '{item['_product_sku']}' not found for assignment")
        ...
        vals = AttributeValue.objects.filter(attribute=attr, slug__in=value_slugs)
    ...
    def _import_product_variant(self, item, files):
        ...
        product = Product.objects.filter(sku=item["_product_sku"]).first()
        if not product:
            raise ValueError(f"Product '{item['_product_sku']}' not found for variant")
    ...
    def _import_product_image(self, item, files):
        ...
        product = Product.objects.filter(sku=item["_product_sku"]).first()
        if not product:
            raise ValueError(f"Product '{item['_product_sku']}' not found for image")

        position = item.get("position", 0)
        existing = ProductImage.objects.filter(product=product, position=position).first()

        # Import inline MediaAsset
        media_asset = None
        meta = item.get("_media_asset_meta")
        if meta:
            file_key = f"ProductImage:{item['_product_sku']}:{position}:media_asset"
    ...
    def _import_bundle_item(self, item):
        from catalog.models import BundleItem, Product, ProductVariant

        bundle = Product.objects.filter(sku=item["_bundle_sku"]).first()
        if not bundle:
            raise ValueError(f"Bundle product '{item['_bundle_sku']}' not found")

        component = Product.objects.filter(sku=item["_component_product_sku"]).first()
        if not component:
            raise ValueError(f"Component product '{item['_component_product_sku']}' not found")

        comp_variant = None
        variant_sku = item.get("_component_variant_sku")
        if variant_sku:
            comp_variant = ProductVariant.objects.filter(sku=variant_sku).first()
            if not comp_variant:
                logger.warning("Component variant '%s' not found for bundle item", variant_sku)
    ...
    def _delete_absent(self, items):
        ...
        remote_attr_slugs = {i["slug"] for i in items if i.get("_model") == "ProductAttribute"}
        remote_val_keys = {
            (i["_attribute_slug"], i["slug"]) for i in items if i.get("_model") == "AttributeValue"
        }
        remote_cat_slugs = {i["slug"] for i in items if i.get("_model") == "Category"}
        remote_brand_slugs = {i["slug"] for i in items if i.get("_model") == "Brand"}
        remote_product_skus = {i["sku"] for i in items if i.get("_model") == "Product"}
        remote_assign_keys = {
            (i["_product_sku"], i["_attribute_slug"])
            for i in items
            if i.get("_model") == "ProductAttributeAssignment"
        }
        remote_variant_skus = {i["sku"] for i in items if i.get("_model") == "ProductVariant"}
        remote_image_keys = {
            (i["_product_sku"], i["position"]) for i in items if i.get("_model") == "ProductImage"
        }
        remote_bundle_keys = {
            (
                i["_bundle_sku"],
        ...
        for cat in Category.objects.all().order_by("-sort_order"):
            if cat.slug not in remote_cat_slugs:
                try:
                    ...
                    logger.warning("Cannot delete category '%s': %s", cat.slug, e)
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if model_type == "ProductAttribute":
                existing = ProductAttribute.objects.filter(slug=item.get("slug")).first()
                compare_fields = PRODUCT_ATTRIBUTE_FIELDS
                display_name = f"Attribute: {item.get('name', '?')}"

            elif model_type == "AttributeValue":
                attr = ProductAttribute.objects.filter(slug=item.get("_attribute_slug")).first()
                existing = (
                    AttributeValue.objects.filter(attribute=attr, slug=item.get("slug")).first()
                    if attr
                    else None
                )
                compare_fields = ATTRIBUTE_VALUE_FIELDS
                display_name = f"Value: {item.get('_attribute_slug', '?')}/{item.get('value', '?')}"

            elif model_type == "Category":
                existing = Category.objects.filter(slug=item.get("slug")).first()
                compare_fields = CATEGORY_FIELDS
                display_name = f"Category: {item.get('name', '?')}"

            elif model_type == "Brand":
                existing = Brand.objects.filter(slug=item.get("slug")).first()
                compare_fields = BRAND_FIELDS
                display_name = f"Brand: {item.get('name', '?')}"

            elif model_type == "Product":
                existing = Product.objects.filter(sku=item.get("sku")).first()
                compare_fields = PRODUCT_FIELDS
                display_name = f"Product: {item.get('name', '?')} ({item.get('sku', '?')})"
                ...
                existing = ProductVariant.objects.filter(sku=item.get("sku")).first()
                compare_fields = PRODUCT_VARIANT_FIELDS
                display_name = f"Variant: {item.get('name', '?')} ({item.get('sku', '?')})"
                ...
                display_name = (
                    f"Image: {item.get('_product_sku', '?')} pos {item.get('position', '?')}"
                )

            elif model_type == "BundleItem":
                bundle = Product.objects.filter(sku=item.get("_bundle_sku")).first()
                comp = Product.objects.filter(sku=item.get("_component_product_sku")).first()
                v_sku = item.get("_component_variant_sku")
                comp_v = ProductVariant.objects.filter(sku=v_sku).first() if v_sku else None
                existing = (
