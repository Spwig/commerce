...
class CommerceRulesSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        ...
        from subscriptions.models import PlanAddon, SubscriptionPlan
        from vouchers.models import VoucherCode, VoucherRestriction

        return (
            VoucherCode.objects.count()
    ...
    def export(self, credential_mode="redact"):
        ...
        for v in VoucherCode.objects.prefetch_related(
            ...
            data["_eligible_category_slugs"] = list(
                v.eligible_categories.values_list("slug", flat=True)
            )

            # Nested restrictions
            data["_restrictions"] = []
            for r in v.restrictions.all():
                data["_restrictions"].append({f: getattr(r, f) for f in VOUCHER_RESTRICTION_FIELDS})
        ...
        for reward in LoyaltyReward.objects.select_related("product", "required_tier").all():
            ...
            data["_required_tier_slug"] = (
                reward.required_tier.slug if reward.required_tier else None
            )
        ...
        # Loyalty campaigns
        for camp in (
            LoyaltyCampaign.objects.prefetch_related("target_tiers")
    ...
    def _import_loyalty_campaign(self, item):
        from loyalty.models import LoyaltyCampaign, LoyaltySegment, LoyaltyTier
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if model_type == "VoucherCode":
                ...
                existing = (
                    PlanAddon.objects.filter(plan=plan, name=item.get("name")).first()
                    if plan
                    else None
                )
