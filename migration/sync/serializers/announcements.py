...
ANNOUNCEMENT_FIELDS = [
    'title', 'body',
    'image_display_mode', 'image_overlay_opacity',
    'link_type', 'custom_url', 'link_text',
    'show_modal', 'is_enabled', 'priority',
    'translations',
]
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if existing:
                ...
                if field_changes:
                    changes.append({
                        'type': 'modify', 'model': 'Announcement',
                        'name': item.get('title', '?')[:50], 'changes': field_changes,
                    })
            else:
                changes.append({
                    'type': 'add', 'model': 'Announcement',
