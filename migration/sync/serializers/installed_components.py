...
class InstalledComponentsSerializer(CollectionSyncSerializer):
    ...
    def _import_package(self, comp, package):
        ...
        if source == "directory":
            ...
            if path_type == "theme":
                ...
                target = components_root / "integrations" / int_ctype / slug / f"v{version}"
                _extract_zip_to(raw, str(target))
                self._create_symlink(target.parent / "current", target)
                ...
                if static_data:
                    ...
                    target = components_root / "static" / "utilities" / slug / f"v{version}"
                    _extract_zip_to(raw, str(target))
                    self._create_symlink(target.parent / "current", target)
                ...
                if tmpl_data:
                    ...
                    target = components_root / "templates" / "utilities" / slug / f"v{version}"
                    _extract_zip_to(raw, str(target))
                    self._create_symlink(target.parent / "current", target)
    ...
    def _install_theme_package(self, comp, raw_zip, components_root, version):
        ...
        if comp.theme:
            ...
            target = components_root / "static" / "design" / "themes" / comp.slug / version
    ...
    def _delete_absent(self, remote_items):
        ...
        remote_keys = {(item["component_type"], item["slug"]) for item in remote_items}
        deleted = 0
    ...
    def _remove_component_files(self, comp):
        ...
        dirs_to_remove = []
        if ctype == "theme":
            dirs_to_remove.append(components_root / "static" / "design" / "themes" / slug)
            ...
            dirs_to_remove.append(components_root / "integrations" / ctype / slug)
