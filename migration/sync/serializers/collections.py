...
class CollectionsSerializer(BaseCollectionSerializer):
    ...
    def export(self, credential_mode='redact'):
        ...
        for coll in Collection.objects.prefetch_related('products').all():
            ...
            data['_product_skus'] = list(
                coll.products.values_list('sku', flat=True)
            )
    ...
    def _delete_absent(self, remote_items):
        ...
        remote_slugs = {
            item['slug'] for item in remote_items if item.get('_model') == 'Collection'
        }
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if existing:
                ...
                if field_changes:
                    changes.append({
                        'type': 'modify', 'model': 'Collection',
                        'name': slug, 'changes': field_changes,
                    })
            else:
                changes.append({
                    'type': 'add', 'model': 'Collection',
                    'name': slug,
