...
SHIFT_FIELDS = [
    ...
    'total_manual_discounts', 'manual_discount_count', 'notes',
]
...
PAYMENT_FIELDS = [
    'method', 'amount', 'amount_tendered', 'change_given',
    'card_last_four', 'card_reference', 'provider_payment_id',
    'card_brand', 'gift_card_code',
]

PARKED_CART_FIELDS = [
    'cart_data', 'item_count', 'total_amount', 'customer_name',
]
...
class POSTransactionsSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from pos_app.models import POSShift, CashMovement, POSPayment, ParkedCart
        return (
    ...
    def export(self, credential_mode='redact'):
        from pos_app.models import POSShift, POSPayment, ParkedCart

        items = []

        # Shifts with nested cash movements
        for shift in POSShift.objects.select_related(
            'terminal', 'cashier',
        ).prefetch_related('cash_movements', 'cash_movements__performed_by').all():
        ...
        for payment in POSPayment.objects.select_related('order', 'shift').all():
            ...
            data['_shift_started_at'] = payment.shift.started_at.isoformat() if payment.shift and payment.shift.started_at else None
            # Convert Decimals
    ...
    def _import_shift(self, item):
        from pos_app.models import POSShift, POSTerminal, CashMovement
        ...
        for f in SHIFT_FIELDS:
            val = item.get(f)
            if val is not None:
                if f in ('opening_cash', 'closing_cash', 'expected_cash',
                         'cash_difference', 'total_sales', 'total_refunds',
    ...
    def _import_payment(self, item):
        ...
        if created_at and POSPayment.objects.filter(
            order=order, method=item.get('method'), created_at=created_at,
        ).exists():
            return
    ...
    def _import_parked_cart(self, item):
        ...
        # Deduplicate
        if parked_at and ParkedCart.objects.filter(
            terminal=terminal, parked_at=parked_at,
        ).exists():
            return
