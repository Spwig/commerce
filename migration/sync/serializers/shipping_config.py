...
class ShippingConfigSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode="redact"):
        ...
        for method in ShippingMethod.objects.prefetch_related("zones", "pickup_locations").all():
            ...
            data["_pickup_location_codes"] = list(
                method.pickup_locations.values_list("code", flat=True)
            )
        ...
        for rule in ShippingPromotion.objects.prefetch_related(
            ...
            data["_requires_product_skus"] = list(
                rule.requires_products.values_list("sku", flat=True)
            )
            data["_requires_category_slugs"] = list(
                rule.requires_categories.values_list("slug", flat=True)
            )
            data["_excludes_product_skus"] = list(
                rule.excludes_products.values_list("sku", flat=True)
            )
            data["_excludes_category_slugs"] = list(
                rule.excludes_categories.values_list("slug", flat=True)
            )
            data["_customer_group_names"] = list(
                rule.customer_groups.values_list("name", flat=True)
            )
        ...
        for table in (
            ShippingRateTable.objects.select_related("shipping_method")
            .prefetch_related("zones", "tiers")
            .all()
        ):
    ...
    def _import_package(self, item):
        ...
        for f in SHIPPING_PACKAGE_FIELDS:
            if f in item:
                ...
                if (
                    f
                    in ("length", "width", "height", "wall_thickness", "max_weight", "tare_weight")
                    and val is not None
                ):
                    val = Decimal(str(val))
                setattr(pkg, f, val)
    ...
    def _delete_absent(self, remote_items):
        ...
        remote = {
            ...
            "ShippingPackage": set(),
            "ShippingMethod": set(),
            "ShippingPromotion": set(),
            "ShippingRateTable": set(),
            "Location": set(),
        }
        ...
        for item in remote_items:
            ...
            if m == "ProviderAccount":
                ...
                m == "ShippingZone"
                or m == "ShippingPackage"
                or m == "ShippingMethod"
                or m == "ShippingPromotion"
                or m == "ShippingRateTable"
            ):
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if model_type == "ProviderAccount":
                existing = ProviderAccount.objects.filter(
