...
AFFILIATE_FIELDS = [
    'affiliate_code', 'company_name', 'website',
    'payment_email', 'payment_method',
    'bank_account_holder', 'bank_account_number',
    'bank_routing_code', 'bank_swift_code',
...
class AffiliateDataSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from affiliate.models import (
            Affiliate, AffiliateProgramMembership, Link,
            Commission, Payout,
        )
        return (
    ...
    def export(self, credential_mode='redact'):
        ...
        items = []
        for aff in Affiliate.objects.select_related('user').prefetch_related(
            'affiliateprogrammembership_set',
            'affiliateprogrammembership_set__program',
            'links', 'links__program',
            'commissions', 'commissions__program', 'commissions__order',
            'payouts',
        ).all():
            ...
            for payout in aff.payouts.all().order_by('created_at'):
                ...
                p_data['_commission_order_numbers'] = [
                    c.order.order_number for c in payout.commissions.all()
                    if c.order
                ]
    ...
    def _import_affiliate(self, item):
        ...
        for p_data in item.get('_payouts', []):
            ...
            if created_at and Payout.objects.filter(
                affiliate=aff, created_at=created_at,
            ).exists():
                continue

            payout = Payout(affiliate=aff)
            for f in ['method', 'status', 'reference', 'notes',
                       'provider_reference', 'provider_response',
            ...
            for dt_field, key in [('processed_at', '_processed_at'), ('completed_at', '_completed_at')]:
                val = p_data.get(key)
                if val:
            ...
            for order_num in p_data.get('_commission_order_numbers', []):
                comm = Commission.objects.filter(
                    affiliate=aff, order__order_number=order_num,
                ).first()
                if comm:
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if user:
                existing = Affiliate.objects.filter(user=user).first()
                if existing:
                    changes.append({'type': 'modify', 'model': 'Affiliate', 'name': name, 'changes': []})
                else:
