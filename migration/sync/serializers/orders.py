...
class OrdersSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode="redact"):
        ...
        for order in (
            ...
            data["_pickup_location_code"] = (
                order.pickup_location.code if order.pickup_location else None
            )
    ...
    def _import_order(self, item):
        ...
        for oi_data in items_data:
            ...
            for f in ORDER_ITEM_FIELDS:
                if f in oi_data:
                    val = oi_data[f]
                    if (
                        f in ("discount_value", "unit_price_base", "total_price_base")
                        and val is not None
                    ):
                        val = Decimal(str(val))
                    setattr(oi, f, val)
        ...
        for _i, oi_data in enumerate(items_data):
    ...
    def _delete_absent(self, remote_items):
        ...
        remote_numbers = {
            item.get("order_number") for item in remote_items if item.get("_model") == "Order"
        }
