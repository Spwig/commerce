...
DIGITAL_ASSET_FIELDS = [
    'filename', 'file_size', 'file_type', 'file_hash', 'version', 'changelog',
    'is_active', 'download_limit', 'expiration_days', 'requires_license',
]

LICENSE_TEMPLATE_FIELDS = [
    'name', 'description', 'is_active', 'pattern', 'prefix', 'suffix',
    'separator', 'character_set', 'min_length', 'max_length',
]

LICENSE_POOL_FIELDS = [
    'name', 'description', 'total_keys', 'keys_generated', 'keys_distributed',
    'status', 'key_type', 'max_activations', 'expires_after_days',
]

LICENSE_PROVIDER_FIELDS = [
    'name', 'provider_type', 'is_active', 'api_endpoint',
    'sync_on_order', 'sync_on_activation', 'sync_on_deactivation',
    'sync_bidirectional', 'webhook_events', 'provider_config', 'product_mapping',
    'connection_status',
]
...
class DigitalProductsSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode='redact'):
        ...
        for pool in LicensePool.objects.select_related(
            'product', 'license_template', 'sync_to_provider',
        ).all():
    ...
    def import_data(self, data, dry_run=False, sync_mode='additive'):
        ...
        order_map = {
            'LicenseKeyTemplate': 0, 'LicenseProvider': 1,
            'DigitalAsset': 2, 'LicensePool': 3,
        }
    ...
    def _import_asset(self, item):
        from catalog.models import Product, DigitalAsset
        ...
        existing = DigitalAsset.objects.filter(
            product=product, filename=item['filename'],
        ).first()
    ...
    def _import_pool(self, item):
        from catalog.models import Product, LicensePool, LicenseKeyTemplate, LicenseProvider
    ...
    def generate_diff(self, remote_data):
        from catalog.models import (
            DigitalAsset, LicenseKeyTemplate, LicensePool, LicenseProvider, Product,
        )
        ...
        for item in items:
            ...
            if model_type == 'LicenseKeyTemplate':
                ...
                if product:
                    existing = DigitalAsset.objects.filter(
                        product=product, filename=item.get('filename', ''),
                    ).first()
                ...
                if product:
                    existing = LicensePool.objects.filter(
                        product=product, name=item.get('name', ''),
                    ).first()
