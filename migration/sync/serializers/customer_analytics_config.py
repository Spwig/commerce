...
CUSTOMER_SEGMENT_FIELDS = [
    ...
    'min_days_since_last_purchase', 'max_days_since_last_purchase',
    'color', 'priority', 'is_active',
]
...
LTV_SETTINGS_FIELDS = [
    'calculation_method', 'default_discount_rate',
    'min_data_quality_threshold',
]

LTV_MULTIPLIER_FIELDS = [
    'category_name', 'repeat_purchase_multiplier',
    'notes', 'is_active',
]
...
class CustomerAnalyticsConfigSerializer(CollectionSyncSerializer):
    ...
    def import_data(self, data, dry_run=False, sync_mode='additive'):
        ...
        for item in items:
            ...
            try:
                ...
                failed += 1
                errors.append(f"{model_type} '{item.get('name', item.get('category_name', '?'))}': {e}")
    ...
    def _delete_absent(self, remote_items):
        ...
        remote_mult_names = {
            item['category_name'] for item in remote_items
