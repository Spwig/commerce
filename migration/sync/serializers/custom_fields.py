...
class CustomFieldsSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode="redact"):
        ...
        # Export definitions
        for defn in CustomFieldDefinition.objects.select_related("group", "content_type").all():
            ...
            with transaction.atomic():
                ...
                for item in groups:
                    try:
                        ...
                        logger.error(
                            "Failed to import custom field group '%s': %s", item.get("slug"), e
                        )
                ...
                for item in definitions:
                    try:
                        ...
                        logger.error(
                            "Failed to import custom field definition '%s': %s", item.get("slug"), e
                        )
    ...
    def _import_group(self, item):
        ...
        existing = CustomFieldGroup.all_objects.filter(slug=item["slug"], content_type=ct).first()
    ...
    def _import_definition(self, item):
        ...
        group = CustomFieldGroup.objects.filter(slug=item["_group_slug"], content_type=ct).first()
        if not group:
            raise ValueError(f"Group '{item['_group_slug']}' not found for {item['_content_type']}")
