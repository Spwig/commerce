...
STORE_GROUP_FIELDS = [
    'name', 'code', 'currency', 'language', 'timezone',
    'settings', 'sort_order', 'is_active',
]

TERMINAL_PROVIDER_FIELDS = [
    'provider_key', 'display_name',
    'provider_settings', 'is_active',
]

TERMINAL_FIELDS = [
    'name', 'hardware_config', 'currency',
    'order_sync_days', 'order_sync_limit', 'is_active',
]

RECEIPT_TEMPLATE_FIELDS = [
    'name', 'paper_width',
    'header_text', 'show_store_address', 'custom_address',
    'show_store_phone', 'custom_phone',
    'show_store_email', 'custom_email',
    'tax_id_label', 'tax_id_number', 'business_registration',
    'show_sku', 'show_cashier', 'show_terminal_name',
    'footer_text', 'return_policy',
    'qr_enabled', 'qr_url', 'qr_label',
    'show_powered_by',
]
...
STAFF_DISCOUNT_FIELDS = [
    'max_discount_percentage', 'max_discount_amount',
...
class POSConfigSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from pos_app.models import (
            StoreGroup, POSTerminalProvider, POSTerminal,
            ReceiptTemplate, PromoSlide, POSStaffDiscount,
        )
        return (
        ...
        for tp in POSTerminalProvider.objects.all():
            ...
            if credential_mode == 'decrypt':
                creds = decrypt_credentials_for_export(
                    'pos_config', 'POSTerminalProvider', tp,
                )
                if creds:
    ...
    def import_data(self, data, dry_run=False, sync_mode='additive'):
        ...
        order_map = {
            'StoreGroup': 0, 'POSTerminalProvider': 1, 'POSTerminal': 2,
            'ReceiptTemplate': 3, 'PromoSlide': 4, 'POSStaffDiscount': 5,
        }
    ...
    def _import_terminal_provider(self, item):
        ...
        if '_credentials' in item and item['_credentials']:
            encrypted = encrypt_credentials_for_import(
                'pos_config', 'POSTerminalProvider', item['_credentials'],
            )
            if encrypted:
    ...
    def _delete_absent(self, remote_items):
        ...
        remote_sd_emails = {
            item['_user_email'] for item in remote_items
            if item.get('_model') == 'POSStaffDiscount'
        }
        for sd in POSStaffDiscount.objects.select_related('user').all():
        ...
        remote_codes = {
            item['code'] for item in remote_items if item.get('_model') == 'StoreGroup'
        }
        remote_provider_keys = {
            item['provider_key'] for item in remote_items
    ...
    def generate_diff(self, remote_data):
        from pos_app.models import StoreGroup, POSTerminalProvider, POSTerminal
