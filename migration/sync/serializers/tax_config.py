...
TAX_PRESET_GROUP_FIELDS = [
    ...
    'tax_type', 'region', 'is_active', 'version',
]

TAX_PRESET_RATE_FIELDS = [
    'country', 'country_name', 'state', 'state_name',
    'rate', 'tax_type', 'notes', 'is_active',
]

TAX_RATE_FIELDS = [
    'name', 'country', 'state', 'city', 'postal_codes',
    'rate', 'tax_type',
    'applies_to_shipping', 'compound',
...
class TaxConfigSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from cart.models import TaxRate, TaxPresetGroup, TaxPresetRate
        return TaxPresetGroup.objects.count() + TaxPresetRate.objects.count() + TaxRate.objects.count()
    ...
    def _delete_absent(self, remote_items):
        ...
        for item in remote_items:
            if item.get('_model') == 'TaxRate':
                remote_tax_keys.add((item.get('country', ''), item.get('state', ''), item.get('city', ''), item.get('name', '')))
        ...
        remote_group_keys = {item['key'] for item in remote_items if item.get('_model') == 'TaxPresetGroup'}
