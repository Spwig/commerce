...
LICENSE_KEY_FIELDS = [
    'key', 'key_type', 'max_activations', 'current_activations',
    'status', 'is_lifetime', 'notes',
]

ACTIVATION_FIELDS = [
    'device_identifier', 'device_name', 'device_info',
    'is_active', 'ip_address', 'location',
]
...
class LicenseKeysSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode='redact'):
        ...
        items = []
        for lk in LicenseKey.objects.select_related(
            'license_pool', 'license_pool__license_template',
            'digital_asset', 'digital_asset__product',
            'order_item', 'order_item__order',
            'user',
        ).prefetch_related('activations').all():
            ...
            data['_order_number'] = lk.order_item.order.order_number if lk.order_item and lk.order_item.order else None
            data['_order_item_sku'] = lk.order_item.sku if lk.order_item else None
            data['_digital_asset_product_sku'] = lk.digital_asset.product.sku if lk.digital_asset and lk.digital_asset.product else None
            data['_digital_asset_filename'] = lk.digital_asset.filename if lk.digital_asset else None
            data['_license_pool_template_name'] = lk.license_pool.license_template.name if lk.license_pool and lk.license_pool.license_template else None
    ...
    def _import_license_key(self, item):
        from catalog.models import (
            LicenseKey, LicenseActivation, LicensePool,
            LicenseKeyTemplate, DigitalAsset, Product,
        )
        ...
        if order_number and order_item_sku:
            ...
            if order:
                lk.order_item = OrderItem.objects.filter(
                    order=order, sku=order_item_sku,
                ).first()
        ...
        if da_product_sku and da_filename:
            ...
            if product:
                lk.digital_asset = DigitalAsset.objects.filter(
                    product=product, filename=da_filename,
                ).first()
        ...
        if pool_template_name and lk.digital_asset:
            ...
            if template:
                lk.license_pool = LicensePool.objects.filter(
                    product=lk.digital_asset.product, template=template,
                ).first()
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if existing:
                ...
                if field_changes:
                    changes.append({
                        'type': 'modify', 'model': 'LicenseKey',
                        'name': item.get('key', '?'), 'changes': field_changes,
                    })
            else:
                changes.append({
                    'type': 'add', 'model': 'LicenseKey',
                    'name': item.get('key', '?'),
