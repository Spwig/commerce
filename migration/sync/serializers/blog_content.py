...
BLOG_CATEGORY_FIELDS = [
    ...
    'is_active', 'sort_order', 'external_id',
    'template_variant', 'css_classes', 'layout_config',
    'style_overrides', 'responsive_config', 'inherit_parent_theme',
]

BLOG_POST_FIELDS = [
    'title', 'slug', 'status', 'page_template',
    'excerpt', 'simple_content', 'use_page_builder',
    'translations', 'meta_title', 'meta_description', 'seo_auto_generated',
    'published_at', 'scheduled_at',
    'is_featured', 'is_pinned', 'reading_time_minutes',
    'notify_subscribers', 'notification_sent',
    'auto_share_facebook', 'auto_share_instagram', 'auto_share_linkedin',
    'social_share_message', 'external_id',
    'template_variant', 'css_classes', 'layout_config',
    'style_overrides', 'responsive_config', 'inherit_parent_theme',
]
    ...
    def export(self, credential_mode='redact'):
        ...
        for cat in BlogCategory.objects.select_related('parent', 'image_asset').all():
            ...
            if data.get('created_at'):
                data['created_at'] = cat.created_at.isoformat() if hasattr(cat.created_at, 'isoformat') else str(cat.created_at)

            items.append(data)

        # Posts
        for post in BlogPost.objects.select_related(
            ...
            data['_tag_names'] = {
                t.slug: t.name for t in post.tags.all()
            }
