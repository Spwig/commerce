...
HEADER_FIELDS = [
    'name', 'slug', 'description', 'layout_type',
    'is_sticky', 'sticky_offset', 'has_top_bar', 'top_bar_content',
    'enable_notification_zone', 'notification_zone_config',
    'zone_overrides', 'zone_layouts', 'zones',
    'mobile_layout', 'mobile_menu_position',
    'custom_css', 'css_classes',
    'is_active', 'is_default', 'is_preset', 'preset_category',
    'source', 'draft_data', 'published_data', 'has_unpublished_changes',
]

FOOTER_FIELDS = [
    'name', 'slug', 'description', 'layout_type', 'column_count',
    'zones', 'has_bottom_bar', 'bottom_bar_content',
    'custom_css', 'css_classes', 'background_color', 'text_color',
    'is_active', 'is_default', 'is_preset', 'preset_category',
    'source', 'draft_data', 'published_data', 'has_unpublished_changes',
]

WIDGET_FIELDS = [
    'name', 'widget_type', 'config', 'content', 'translations',
    'show_on_mobile', 'show_on_tablet', 'show_on_desktop',
    'visibility_rules', 'custom_css', 'css_classes',
    'cache_duration', 'is_active',
]

WIDGET_PLACEMENT_FIELDS = [
    'zone', 'order', 'override_config', 'is_active',
]

MENU_FIELDS = [
    'name', 'slug', 'description', 'location', 'display_type',
    'custom_css', 'css_classes', 'global_style', 'mobile_config',
    'translations', 'is_active',
]

MENU_ITEM_FIELDS = [
    'item_type', 'title', 'url', 'target',
    'icon', 'badge_text', 'badge_color',
    'style_config', 'widget_config', 'tree_config', 'mega_menu_content',
    'visibility_rules', 'translations', 'order', 'is_active', 'css_classes',
]
...
class DesignLayoutSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from design.models import (
            HeaderTemplate, FooterTemplate, Widget, WidgetPlacement, Menu,
        )
        return (HeaderTemplate.objects.count() + FooterTemplate.objects.count() +
                Widget.objects.count() + WidgetPlacement.objects.count() +
                Menu.objects.count())
    ...
    def import_data(self, data, dry_run=False, sync_mode='additive'):
        ...
        ordered = sorted(items, key=lambda x: {
            'Widget': 0, 'HeaderTemplate': 1, 'FooterTemplate': 2, 'Menu': 3,
        }.get(x.get('_model', ''), 99))
    ...
    def _import_widget(self, item):
        ...
        existing = Widget.objects.filter(
            name=item['name'], widget_type=item['widget_type']
        ).first()
    ...
    def _delete_absent(self, remote_items):
        from design.models import (
            HeaderTemplate, FooterTemplate, Widget, Menu,
        )
    ...
    def generate_diff(self, remote_data):
        from design.models import HeaderTemplate, FooterTemplate, Widget, Menu
