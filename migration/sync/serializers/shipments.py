...
SHIPMENT_FIELDS = [
    'origin_country', 'dest_country', 'packages',
    'service_level', 'pricing_mode_used',
    'tracking_id', 'label_url', 'packing_slip_url',
    'commercial_invoice_url', 'customs_form_url',
    'status', 'provider_reference', 'provider_meta', 'audit_log',
]

SHIPMENT_MONEY_FIELDS = ['shipping_cost', 'carrier_cost']

TRACKING_EVENT_FIELDS = [
    'status', 'description', 'location', 'raw',
]
...
class ShipmentsSerializer(CollectionSyncSerializer):
    ...
    def export(self, credential_mode='redact'):
        ...
        items = []
        for ship in Shipment.objects.select_related(
            'order', 'carrier_preset', 'provider_account',
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if existing:
                ...
                if field_changes:
                    changes.append({'type': 'modify', 'model': 'Shipment', 'name': name, 'changes': field_changes})
            else:
