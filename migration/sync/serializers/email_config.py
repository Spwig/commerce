...
EMAIL_ACCOUNT_FIELDS = [
    'provider_key', 'name', 'from_email', 'from_name', 'reply_to',
    'is_active', 'is_default', 'settings',
    'dns_validated', 'dns_domain',
]

EMAIL_TEMPLATE_FIELDS = [
    'template_type', 'language_code', 'subject',
    'html_content', 'text_content',
    'is_active', 'is_system', 'version',
]

EMAIL_TEMPLATE_TRANSLATION_FIELDS = [
    'language_code', 'subject', 'html_content', 'text_content',
    'is_verified', 'quality_score', 'base_template_version',
]
        ...
        for item in items:
            ...
            if model_type == 'EmailAccount':
                ...
                if existing:
                    ...
                    if field_changes:
                        changes.append({
                            'type': 'modify', 'model': 'EmailAccount',
                            'name': item.get('name', 'Unknown'), 'changes': field_changes,
                        })
                else:
                    changes.append({
                        'type': 'add', 'model': 'EmailAccount',
                ...
                if existing:
                    ...
                    if field_changes:
                        changes.append({
                            'type': 'modify', 'model': 'EmailTemplate',
                            'name': f"{item.get('template_type')} ({item.get('language_code', 'en')})",
                            'changes': field_changes,
                        })
                else:
                    changes.append({
                        'type': 'add', 'model': 'EmailTemplate',
                        'name': f"{item.get('template_type')} ({item.get('language_code', 'en')})",
                        'fields': {k: v for k, v in item.items() if not k.startswith('_')},
                    })
