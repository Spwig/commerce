...
TRANSACTION_FIELDS = [
    'transaction_type', 'points', 'status',
    'description', 'reason',
...
REDEMPTION_FIELDS = [
    ...
    'cancellation_reason', 'admin_note',
]
...
class LoyaltyMembersSerializer(CollectionSyncSerializer):
    ...
    def get_count(self):
        from loyalty.models import (
            LoyaltyMember, LoyaltyBalance, LoyaltyTransaction,
            LoyaltyRedemption, LoyaltyMemberBadge,
        )
        return (
    ...
    def export(self, credential_mode='redact'):
        ...
        items = []
        for member in LoyaltyMember.objects.select_related(
            'customer', 'current_tier', 'balance',
        ).prefetch_related(
            'transactions', 'redemptions', 'redemptions__reward',
            'badges_earned', 'badges_earned__badge',
        ).all():
    ...
    def _import_member(self, item):
        from loyalty.models import (
            LoyaltyMember, LoyaltyBalance, LoyaltyTransaction,
            LoyaltyRedemption, LoyaltyMemberBadge,
            LoyaltyTier, LoyaltyBadge, LoyaltyReward,
        )
    ...
    def generate_diff(self, remote_data):
        ...
        for item in items:
            ...
            if user:
                existing = LoyaltyMember.objects.filter(customer=user).first()
                if existing:
                    changes.append({'type': 'modify', 'model': 'LoyaltyMember', 'name': name, 'changes': []})
                else:
                    changes.append({'type': 'add', 'model': 'LoyaltyMember', 'name': name, 'fields': {}})
            else:
                changes.append({'type': 'add', 'model': 'LoyaltyMember', 'name': name, 'fields': {}})
