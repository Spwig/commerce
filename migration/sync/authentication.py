...
def authenticate_sync_request(request):
    ...
    if not auth_header.startswith("SyncToken "):
        raise SyncTokenAuthError(
            _("Missing or invalid Authorization header. Expected: SyncToken <token>")
        )
    ...
    api_token = validate_api_token(
        token_string, token_type=APIToken.TOKEN_TYPE_SYNC, record_usage=True, ip_address=ip_address
    )
