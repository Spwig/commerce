...
(function () {
    ...
    function init() {
        var el = document.getElementById('step2-data');
        if (el) {
            try {
        ...
        if (cfg.platform === 'woocommerce' || cfg.platform === 'shopify') {
            form.addEventListener('submit', function (e) {
                var testConnection = document.querySelector('input[name="test_connection"]');
    ...
    function testApiConnection() {
        ...
        if (submitButton) {
            ...
            submitButton.innerHTML = '<div class="loading-spinner"></div> ' + (t.processing || 'Processing...');
        }
        ...
        .then(function (data) {
            if (data.success) {
                var firstStepLabel = cfg.platform === 'shopify'
                ...
                var delays = [500, 500, 400, 400, 400, 400, 400];
                var steps = getSuccessSteps();
                var totalDelay = 500;
                ...
                setTimeout(function () {
                    if (submitButton) {
                        submitButton.innerHTML = '<div class="loading-spinner"></div> ' + (t.loadingPreview || 'Loading preview...');
                    }
                ...
                showConnectionError(
                    ...
                    progressBox, submitButton, originalHTML
                );
            }
        })
        .catch(function (err) {
    ...
    function showConnectionError(title, detail, progressBox, submitButton, originalHTML) {
        ...
        errorBox.innerHTML = '<i class="fas fa-exclamation-circle"></i> <div><strong>' +
            title + '</strong><br>' + (detail || '') + '</div>';
        var wizContent = document.querySelector('.wizard-content');
        if (wizContent) { wizContent.insertBefore(errorBox, progressBox); }

        var retryBtn = document.createElement('button');
        retryBtn.type = 'button';
        retryBtn.className = 'button';
        retryBtn.style.cssText = 'margin-top: 12px; display: inline-flex; align-items: center; gap: 6px;';
        retryBtn.innerHTML = '<i class="fas fa-redo"></i> ' + (t.retryConnection || 'Retry Connection');
        retryBtn.addEventListener('click', function () {
            document.querySelectorAll('.alert-error').forEach(function (el) { el.remove(); });
            if (progressBox && progressBox.parentNode) { progressBox.remove(); }
            if (submitButton) {
    ...
    function updateProgressStep(index, status, text) {
        var items = document.querySelectorAll('#progress-feedback .progress-item');
        ...
        if (text) { item.querySelector('.progress-text').textContent = text; }
    }

    /* ── Shopify Setup Guide Modal ── */

    function initSetupGuide() {
        var overlay = document.getElementById('shopify-setup-guide');
        if (!overlay) return;

        var openBtn = document.getElementById('open-setup-guide');
        var closeBtn = document.getElementById('close-setup-guide');
        ...
        var counterEl = document.getElementById('guide-current-step');
        var steps = overlay.querySelectorAll('.guide-step');
        ...
        function showStep(n) {
            ...
            var target = overlay.querySelector('[data-guide-step="' + n + '"]');
