...
class WooCommerceExtensionImportService:
    ...
    def import_subscription_data(
        ...
        trial_days = self._calculate_trial_days(
            meta.get('_subscription_trial_period', ''),
            meta.get('_subscription_trial_length', '0')
        )
    ...
    def _create_customization_from_addon(self, product, addon_data: Dict, sort_order: int):
        ...
        option = CustomizationOption.objects.create(
            ...
            max_length=self._safe_int(addon_data.get('max', None), None) if option_type in ('text', 'textarea') else None,
            min_value=self._safe_decimal(addon_data.get('min', None)) if option_type == 'number' else None,
            max_value=self._safe_decimal(addon_data.get('max', None)) if option_type == 'number' else None,
        )

        return option

    def _map_addon_type(self, wc_type: str) -> Optional[str]:
    ...
    def import_bundle_data(
        ...
        """
        from catalog.models import BundleItem, Product as CatalogProduct
    ...
    def import_gift_card_data(
        self,
        product,
        product_data: Dict,
        gift_card_details: Dict
    ) -> None:
        """
    ...
    def import_composite_data(
        self,
        product,
        product_data: Dict,
        composite_details: Dict
    ) -> list:
        """
        ...
        from catalog.models import (
            ConfigurationSlot, ConfigurationSlotOption,
    ...
    def import_booking_data(
        ...
        duration_unit = wc_unit_map.get(
            meta.get('_wc_booking_duration_unit', 'hour'), 'hour'
        )
    ...
    def resolve_deferred_extensions(self):
        ...
        """
        from catalog.models import BundleItem, ConfigurationSlotOption, ConfigurationSlot
