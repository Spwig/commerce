        ...
        try:
            mapped = {
                ...
                "slug": self.generate_slug(
                    source_data.get("slug", "") or source_data.get("name", "")
                ),
                ...
                # SEO
                "meta_title": "",
                "meta_description": source_data.get("description", "")[:255]
    ...
    def map(self, source_data: dict) -> dict:
        ...
        try:
            mapped = {
                ...
                "short_description": self.parse_html(source_data.get("short_description", ""))[
                    :500
                ],
                # Pricing
                ...
                "primary_category_id": source_data.get("categories", [{}])[0].get("id")
                if source_data.get("categories")
                else None,
                # Inventory
                "track_inventory": source_data.get("manage_stock", False),
                "stock_quantity": int(source_data.get("stock_quantity", 0))
                if source_data.get("stock_quantity")
                else 0,
                "allow_backorders": source_data.get("backorders", "no") != "no",
                "low_stock_threshold": int(source_data.get("low_stock_amount", 5))
                if source_data.get("low_stock_amount")
                else 5,
                # Physical attributes
                ...
                "is_digital": source_data.get("virtual", False)
                or source_data.get("downloadable", False),
                # SEO
                "meta_title": source_data.get("name", ""),
                "meta_description": source_data.get("short_description", "")[:255]
                if source_data.get("short_description")
                else "",
                # Images
    ...
    def map(self, source_data: dict) -> dict:
        ...
        try:
            mapped = {
                ...
                or source_data.get("email", "").split("@")[0],
    ...
    def map(self, source_data: dict) -> dict:
        ...
        try:
            ...
            self.log_error(
                ...
                {"id": source_data.get("id"), "number": source_data.get("number")},
            )
            raise
