...
class MagentoCategoryMapper(BaseMapper):
        ...
        try:
            ...
            mapped = {
                ...
                "slug": self.generate_slug(
                    resolve_custom_attribute(custom_attrs, "url_key") or source_data.get("name", "")
                ),
    ...
    def flatten_category_tree(
        tree_node: dict, result: list[dict] = None, skip_root: bool = True
    ) -> list[dict]:
...
class MagentoProductMapper(BaseMapper):
    ...
    def __init__(
        self, migration_job=None, attribute_options_cache: dict = None, store_url: str = ""
    ):
    ...
    def map(self, source_data: dict) -> dict:
        ...
        try:
            ...
            mapped = {
                ...
                # Categories
                "category_ids": [
                    int(link.get("category_id", 0)) for link in ext_attrs.get("category_links", [])
                ],
                "primary_category_id": (
                ...
                # SEO
                "meta_title": resolve_custom_attribute(custom_attrs, "meta_title")
            ...
            mapped_images.append(
                {
                    ...
                    "src": f"{self.store_url}/media/catalog/product{file_path}"
                    if file_path
                    else "",
                    "name": "",
...
class MagentoCustomerMapper(BaseMapper):
    ...
    def _find_address(self, addresses: list[dict], address_id, flag_field: str) -> dict:
...
class MagentoOrderMapper(BaseMapper):
    ...
    def map(self, source_data: dict) -> dict:
        ...
        try:
            ...
            shipping_info = (
                shipping_assignments[0].get("shipping", {}) if shipping_assignments else {}
            )
            ...
            mapped = {
                ...
                "order_number": str(
                    source_data.get("increment_id") or source_data.get("entity_id")
                ),
                # Customer
                "customer_id": source_data.get("customer_id", 0),
                "customer_email": source_data.get("customer_email", ""),
                # Status
                "status": transform_magento_order_status(source_data.get("status", "pending")),
                ...
                "shipping_total": self._parse_price(source_data.get("shipping_amount", 0)),
                ...
                "shipping_address": self._map_address(shipping_info.get("address", {})),
                # Line items
    ...
    def map(self, source_data: dict) -> dict:
        ...
        try:
            ...
            mapped = {
                "title": source_data.get("title", ""),
                "slug": self.generate_slug(
                    source_data.get("identifier", "") or source_data.get("title", "")
                ),
                ...
                "published_at": source_data.get("creation_time")
                if source_data.get("active")
                else None,
                # SEO
