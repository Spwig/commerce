...
class WooCommerceCategoryImporter(BaseImporter):
    ...
    def import_data(self, mapped_data: Dict) -> Optional[Category]:
...
class WooCommerceProductImporter(BaseImporter):
    ...
    def import_data(self, mapped_data: Dict) -> Optional[Product]:
        ...
        try:
            ...
            product, created = self.get_or_create(
                ...
                defaults={
                    ...
                    'sale_type': 'fixed_price' if (
                        mapped_data.get('sale_price') and mapped_data.get('price') and
                        mapped_data['sale_price'] < mapped_data['price']
                    ) else 'none',
                    'sale_value': mapped_data['sale_price'] if (
                        mapped_data.get('sale_price') and mapped_data.get('price') and
                        mapped_data['sale_price'] < mapped_data['price']
                    ) else None,
    ...
    def _get_or_create_category(self, product_data: Dict) -> Optional[Category]:
    ...
    def _validate_category_hierarchy(self):
        ...
        while activated_any:
            ...
            for parent in inactive_parents:
                ...
                reactivated += 1
                activated_any = True
                logger.info(f"Reactivated parent category '{parent.name}' (ID: {parent.id}) for hierarchy integrity")
