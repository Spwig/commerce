...
class ImportExecutor:
    ...
    def __init__(self, migration_job: MigrationJob):
        ...
        self.extension_service = WooCommerceExtensionImportService(
            migration_job=self.job, currency=self._get_currency()
        )
        ...
        try:
            ...
            if config.get("import_products", False):
                ...
                if resolved:
                    self._log(
                        "info",
                        f"Resolved {resolved} deferred extension references (bundles/composites)",
                    )
    ...
    def _import_categories(self):
        ...
        try:
            ...
            for _idx, cat_data in enumerate(categories):
                try:
            ...
            for cat_data in categories:
                ...
                if parent_id and parent_id > 0:
                    try:
                        ...
                        logger.warning(
                            f"Failed to set parent for category {cat_data.get('id')}: {e}"
                        )
            ...
            logger.info(
                f"Category import complete: {step.items_imported} imported, {step.items_failed} failed"
            )
        ...
        while activated_any:
            ...
            for ancestor in inactive_ancestors:
                ...
                self._log(
                    ...
                    source_type="category",
                    source_id=str(ancestor.external_id or ancestor.id),
                )
    ...
    def _import_products(self):
        ...
        try:
            ...
            self._log(
                "info",
                f"Product import complete: {step.items_imported} imported, {step.items_failed} failed",
            )
    ...
    def _import_single_product(self, product_data: dict, step: MigrationStep) -> Product | None:
        ...
        price = transform_money(
            product_data.get("regular_price", product_data.get("price")), self._get_currency()
        )
        ...
        if product_data.get("type") in ("variable", "variable-subscription") and product_data.get(
            "variations"
        ):
        ...
        if has_addons:
            try:
                options = self.extension_service.import_product_addons(product, product_data)
                if options:
                    self._log(
                        "info",
        ...
        if is_gift_card:
            try:
                self.extension_service.import_gift_card_data(product, product_data, gift_card_data)
                self._log(
                    "info",
        ...
        if is_booking:
            try:
                self.extension_service.import_booking_data(product, product_data, booking_data)
                self._log(
                    "info",
    ...
    def _import_product_images(self, product: Product, images_data: list[dict]):
        ...
        for idx, img_data in enumerate(images_data[:5]):  # Limit to 5 images
            try:
                ...
                for preset in image_presets:
                    try:
                        ...
                        if original_content:
                            ...
                            thumbnail.save()
                            logger.debug(
                                f"Generated {preset.slug} thumbnail for product image {idx + 1}"
                            )
                    except Exception as e:
                ...
                logger.debug(
                    f"Imported image {idx + 1} for product {product.name} with WebP and thumbnails"
                )
    ...
    def _import_product_variants(self, product: Product, variant_ids: list[int]):
        ...
        try:
            ...
            self._log(
                "error",
                f"Failed to fetch variations for product {external_product_id}: {e}",
                source_type="product",
        ...
        for variation_data in tqdm(
            variations, desc=f"Variants for {product.name[:30]}", leave=False
        ):
            try:
    ...
    def _import_single_variant(
        ...
        variant_name = (
            attribute_service.build_variant_name(attribute_pairs) or f"Variant {external_id}"
        )
    ...
    def _import_customers(self):
        ...
        try:
            ...
            self._log(
                "info",
                f"Customer import complete: {step.items_imported} imported, {step.items_failed} failed",
            )
    ...
    def _import_single_customer(self, customer_data: dict, step: MigrationStep) -> User | None:
    ...
    def _import_orders(self):
        ...
        try:
            ...
            self._log(
                "info",
                f"Order import complete: {step.items_imported} imported, {step.items_failed} failed",
            )
    ...
    def _import_single_order(self, order_data: dict, step: MigrationStep) -> Order | None:
    ...
    def _import_order_item(self, order: Order, item_data: dict, currency: str):
        ...
        if not product:
            ...
            logger.warning(
                f"Product {product_id} not found for order {order.order_number}, skipping line item"
            )
            return
    ...
    def _import_reviews(self):
        ...
        try:
            ...
            self._log(
                "info",
                f"Review import complete: {step.items_imported} imported, {step.items_failed} failed",
            )
    ...
    def _import_single_review(self, review_data: dict, step: MigrationStep) -> ProductReview | None:
        ...
        if not user:
            ...
            user = User.objects.create(
                ...
                last_name=" ".join(reviewer_name.split()[1:])
                if len(reviewer_name.split()) > 1
                else "",
        ...
        if existing_review:
            logger.debug(
                f"Review already exists for product {product.name} by {user.email}, skipping"
            )
            step.items_skipped += 1
            step.save()
        ...
        review = ProductReview.objects.create(
            ...
            created_at=timezone.datetime.fromisoformat(
                review_data.get("date_created", "").replace("Z", "+00:00")
            )
            if review_data.get("date_created")
            else timezone.now(),
        )
    ...
    def _import_coupons(self):
        ...
        try:
            ...
            with tqdm(total=len(coupons), desc="Importing coupons") as pbar:
                for coupon_data in coupons:
                    try:
                        ...
                        self._log(
                            "error",
                            f"Failed to import coupon {coupon_data.get('id')}: {e}",
                            source_type="coupon",
                            source_id=str(coupon_data.get("id")),
                        )
            ...
            self._log(
                "info",
                f"Coupon import complete: {step.items_imported} imported, {step.items_failed} failed",
            )
    ...
    def _import_single_coupon(self, coupon_data: dict, step: MigrationStep) -> VoucherCode | None:
    ...
    def _import_blog(self):
        ...
        try:
            ...
            step.items_total = (
                cat_stats.get("created", 0)
                + cat_stats.get("updated", 0)
                + cat_stats.get("skipped", 0)
                + cat_stats.get("errors", 0)
                + tag_stats.get("created", 0)
                + tag_stats.get("updated", 0)
                + tag_stats.get("skipped", 0)
                + tag_stats.get("errors", 0)
                + post_stats.get("created", 0)
                + post_stats.get("updated", 0)
            ...
            self.job.blog_categories_total = (
                cat_stats.get("created", 0)
                + cat_stats.get("updated", 0)
                + cat_stats.get("skipped", 0)
                + cat_stats.get("errors", 0)
            )
            self.job.blog_categories_imported = cat_stats.get("created", 0) + cat_stats.get(
                "updated", 0
            )
            self.job.blog_categories_skipped = cat_stats.get("skipped", 0)
            self.job.blog_categories_failed = cat_stats.get("errors", 0)

            self.job.blog_tags_total = (
                tag_stats.get("created", 0)
                + tag_stats.get("updated", 0)
                + tag_stats.get("skipped", 0)
                + tag_stats.get("errors", 0)
            ...
            self.job.blog_posts_total = (
                post_stats.get("created", 0)
                + post_stats.get("updated", 0)
                + post_stats.get("skipped", 0)
                + post_stats.get("errors", 0)
            )
            self.job.blog_posts_imported = post_stats.get("created", 0) + post_stats.get(
                "updated", 0
            )
    ...
    def _import_affiliates(self):
        ...
        try:
            ...
            affiliates_step.items_total = (
                aff_stats.get("created", 0)
                + aff_stats.get("skipped", 0)
            ...
            commissions_step.items_total = (
                comm_stats.get("created", 0)
                + comm_stats.get("skipped", 0)
            ...
            payouts_step.items_total = (
                pay_stats.get("created", 0)
                + pay_stats.get("skipped", 0)
