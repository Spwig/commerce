...
class ShopifyImportExecutor:
    ...
    def _import_single_collection(self, col_data: Dict, step: MigrationStep,
                                  mapper: ShopifyCollectionMapper) -> Optional[Category]:
        ...
        category = Category.objects.create(
            ...
            slug=self._get_unique_slug(
                mapped.get('slug', col_data.get('handle')), Category
            ),
    ...
    def _import_single_product(self, product_data: Dict, step: MigrationStep,
                               mapper: ShopifyProductMapper) -> Optional[Product]:
        ...
        product = Product.objects.create(
            ...
            slug=self._get_unique_slug(
                mapped.get('slug', product_data.get('handle')), Product
            ),
    ...
    def _import_product_images(self, product: Product, images_data: List[Dict]):
        ...
        PRODUCT_PRESET_SLUGS = {
            ...
            'product_listing', 'product_detail', 'product_thumbnail',
        }
        image_presets = list(
            ImageSizePreset.objects.filter(
                is_active=True, slug__in=PRODUCT_PRESET_SLUGS
            )
        )
        ...
        for idx, img_data in enumerate(images_data[:5]):
            try:
                ...
                clean_url = re.sub(r'_(?:small|medium|large|grande|compact|\d+x\d*|\d*x\d+)', '', img_url)
                    ...
                    try:
                        ...
                        if original_content:
                            ...
                            thumbnail.file.save(
                                ...
                                original_content, save=False
                            )
                            if webp_content:
                                thumbnail.webp_file.save(
                                    f"{media_asset.id}_{preset.slug}.webp",
                                    webp_content, save=False
                                )
                            thumbnail.save()
                    except Exception as e:
                        logger.warning(
                            f"Failed to generate {preset.slug} thumbnail: {e}"
                        )
                ...
                logger.warning(
                    f"Failed to import image for product {product.name}: {e}"
                )
                continue

    def _import_shopify_variants(self, product: Product, variants: List[Dict],
                                 options: List[Dict], currency: str):
        """
        ...
        for variant_data in tqdm(
            variants, desc=f"  Variants for {product.name[:30]}", leave=False
        ):
            try:
        ...
        self._log(
            'info',
            f"Imported {imported_count} variants for {product.name} "
            f"({failed_count} failed)"
        )

    def _import_single_shopify_variant(
        self, product: Product, variant_data: Dict,
        option_names: Dict, attribute_service: AttributeService,
        product_attribute_values: Dict, currency: str
        ...
        for pos, name in option_names.items():
            ...
            if value_str and value_str != 'Default Title':
                attr, val = attribute_service.get_or_create_attribute_value(
                    name, value_str
                )
        ...
        # Build variant name
        variant_name = (
            attribute_service.build_variant_name(attribute_pairs)
            or variant_data.get('title', f"Variant {external_id}")
        )
        ...
        regular_price = transform_money(
            variant_data.get('compare_at_price') or variant_data.get('price'),
            currency
        )
        ...
        if self.default_warehouse and variant_data.get('inventory_management') == 'shopify':
            variant_stock = transform_integer_nullable(
                variant_data.get('inventory_quantity')
            ) or 0
            StockItem.objects.create(
                product=product,
                warehouse=self.default_warehouse,
    ...
    def _import_single_customer(self, customer_data: Dict, step: MigrationStep,
                                mapper: ShopifyCustomerMapper) -> Optional[User]:
    ...
    def _import_single_order(self, order_data: Dict, step: MigrationStep,
                             mapper: ShopifyOrderMapper) -> Optional[Order]:
        ...
        order = Order.objects.create(
            ...
            shipping_state=shipping_addr.get('province') or shipping_addr.get('province_code') or '',
            shipping_postal_code=shipping_addr.get('zip') or '',
            shipping_country=shipping_addr.get('country_code') or shipping_addr.get('country') or '',
            # Billing address
        ...
        for item_data in order_data.get('line_items', []):
            try:
                ...
                logger.warning(
                    f"Failed to import order item for order {order.order_number}: {e}"
                )
    ...
    def _import_order_item(self, order: Order, item_data: Dict, currency: str):
        ...
        if not product:
            logger.warning(
                f"Product {product_id} not found for order {order.order_number}"
            )
            return
    ...
    def _import_discounts(self):
        ...
        try:
            ...
            step.items_total = self.job.connection_config.get(
                'total_coupons', len(all_price_rules)
            )
            step.save()
    ...
    def _import_single_discount(self, combined_data: Dict, step: MigrationStep,
                                mapper: ShopifyDiscountMapper) -> Optional[VoucherCode]:
    ...
    def _import_blog(self):
        ...
        try:
            ...
            for blog_data in blogs:
                ...
                self._log(
                    'info',
                    f"Fetched {len(articles)} articles from blog "
                    f"'{blog_data.get('title')}'"
                )
                    ...
                    for article_data in articles:
                        try:
                            self._import_single_article(
                                article_data, step, mapper,
                                blog_category, BlogPost, BlogTag
                            )
                            articles_imported += 1
                        except Exception as e:
                            self._log(
            ...
            self._log(
                'info',
                f"Blog import complete: {articles_imported} articles, "
                f"{len(blogs)} categories"
            )
    ...
    def _import_single_article(self, article_data: Dict, step: MigrationStep,
                               mapper: ShopifyArticleMapper,
                               blog_category, BlogPost, BlogTag):
        """Import a single Shopify blog article"""
        mapped = mapper.map(article_data)
        external_id = mapped.get('source_id', str(article_data.get('id')))
        ...
        for tag_name in mapped.get('tags', []):
            if tag_name:
                tag, _ = BlogTag.objects.get_or_create(
                    slug=self._get_unique_slug(
                        tag_name.lower().replace(' ', '-'), BlogTag
                    ),
        ...
        if mapped.get('featured_image_url'):
            self._download_blog_featured_image(
                post, mapped['featured_image_url'],
                mapped.get('featured_image_alt', '')
            )
