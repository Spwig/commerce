...
from migration.utils.transformers import transform_money, safe_money
...
class MagentoImportExecutor:
    ...
    def execute(self):
        ...
        try:
            ...
            if self.job.import_products:
                ...
                self._log('info', f"Cached {len(self._attribute_options_cache)} attribute option sets")
    ...
    def _import_categories(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='categories', status='running',
    ...
    def _import_single_category(self, cat_data: Dict, step: MigrationStep,
                                mapper: MagentoCategoryMapper) -> Optional[Category]:
    ...
    def _import_products(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='products', status='running',
            started_at=timezone.now(),
        )
        self.current_step = step
    ...
    def _import_single_product(self, product_data: Dict, step: MigrationStep,
                               mapper: MagentoProductMapper) -> Optional[Product]:
    ...
    def _import_configurable_variants(self, parent_product: Product,
                                      parent_data: Dict, step: MigrationStep):
        ...
        option_attribute_ids = {
            opt.get('attribute_id'): opt.get('label', '')
            for opt in options
        }
            ...
            try:
                ...
                for attr in child_custom_attrs:
                    ...
                    if code in self._attribute_options_cache:
                        label = resolve_attribute_label(
                            value, code, self._attribute_options_cache
                        )
                ...
                variant = ProductVariant.objects.create(
                    ...
                    sku=child_sku,
                    name=' / '.join(option_values.values()) if option_values else child.get('name', ''),
    ...
    def _import_customers(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='customers', status='running',
            started_at=timezone.now(),
        )
        self.current_step = step
    ...
    def _import_orders(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='orders', status='running',
            started_at=timezone.now(),
        )
        self.current_step = step
    ...
    def _import_single_order(self, order_data: Dict, step: MigrationStep,
        ...
        for item in mapped.get('line_items', []):
            ...
            if not product:
                self._log('warning', f"Order {external_id}: skipping line item '{item.get('name')}' - product not found (ext_id={product_ext_id})")
                continue
    ...
    def _import_reviews(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='reviews', status='running',
            started_at=timezone.now(),
        )
        self.current_step = step
            ...
            with tqdm(total=len(all_reviews), desc="⭐ Reviews", unit="rev") as pbar:
                for review_data in all_reviews:
                    try:
                        ...
                        review_user = User.objects.filter(
    ...
    def _import_coupons(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='coupons', status='running',
            started_at=timezone.now(),
        )
        self.current_step = step
    ...
    def _import_cms_pages(self):
        ...
        step = MigrationStep.objects.create(
            job=self.job, step_type='blog', status='running',
            started_at=timezone.now(),
        )
        self.current_step = step
            ...
            with tqdm(total=len(all_pages), desc="📝 CMS Pages", unit="page") as pbar:
                for page_data in all_pages:
                    try:
                        ...
                        if identifier in ('home', 'no-route', 'enable-cookies', 'privacy-policy-cookie-restriction-mode'):
                            self._step_increment(step, 'items_skipped')
                            pbar.update(1)
                            continue
