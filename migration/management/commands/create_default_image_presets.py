    ...
    def handle(self, *args, **options):
        ...
        for preset_data in presets:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Created preset: {preset.name} ({preset.width}x{preset.height})')
                )
            else:
                updated_count += 1
                self.stdout.write(
                    self.style.WARNING(f'↻ Updated preset: {preset.name} ({preset.width}x{preset.height})')
                )
