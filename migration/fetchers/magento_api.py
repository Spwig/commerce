...
class MagentoAPIClient:
    ...
    def __init__(
        self, store_url: str, access_token: str, timeout: int = 30, verify_ssl: bool = True
    ):
        """
        Args:
    ...
    def fetch_configurable_children(self, parent_sku: str) -> list[dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to fetch children for {parent_sku}: {response.status_code}")
                return []
        except Exception as e:
    ...
    def fetch_configurable_options(self, parent_sku: str) -> list[dict]:
        ...
        try:
            response = self._request("GET", f"/configurable-products/{parent_sku}/options/all")
            if response.status_code == 200:
                return response.json()
            return []
        except Exception as e:
    ...
    def fetch_all_sales_rules(self, progress_callback: Callable | None = None) -> list[dict]:
    ...
    def fetch_all_cms_pages(self, progress_callback: Callable | None = None) -> list[dict]:
        """Fetch all CMS pages."""
    ...
    def _build_search_criteria(
        ...
        if filters:
            for i, (field, value, condition) in enumerate(filters):
                ...
                params[f"searchCriteria[filter_groups][{i}][filters][0][condition_type]"] = (
                    condition
                )
