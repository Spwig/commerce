...
class ShopifyAPIClient:
    ...
    def test_connection(self) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    "success": False,
                    "error": "Invalid access token (401 Unauthorized). Check your Client ID and Secret.",
                }
            elif response.status_code == 403:
                return {
                    "success": False,
                    "error": "Access denied (403 Forbidden). The app may not be installed on this store.",
                }
            elif response.status_code == 404:
                return {
                    "success": False,
                    "error": "Shopify Admin API not found (404). Check your store domain.",
                }
            else:
                return {
                    "success": False,
            ...
            return {
                "success": False,
                "error": "Cannot connect to store. Check the domain is correct and the store is online.",
            }
    ...
    def get_total_counts(self) -> dict[str, int]:
        ...
        counts["collections"] = counts.pop("custom_collections", 0) + counts.pop(
            "smart_collections", 0
        )
    ...
    def fetch_all_custom_collections(self, progress_callback: Callable | None = None) -> list[dict]:
        """Fetch all custom (manually curated) collections."""
        return self._fetch_all_paginated(
            "/custom_collections.json", "custom_collections", progress_callback
        )

    def fetch_all_smart_collections(self, progress_callback: Callable | None = None) -> list[dict]:
        """Fetch all smart (rule-based) collections."""
        return self._fetch_all_paginated(
            "/smart_collections.json", "smart_collections", progress_callback
        )
    ...
    def fetch_all_orders(
        self, progress_callback: Callable | None = None, status: str = "any"
    ) -> list[dict]:
        """Fetch all orders with cursor pagination."""
        return self._fetch_all_paginated(
            "/orders.json", "orders", progress_callback, extra_params={"status": status}
        )

    def fetch_all_price_rules(self, progress_callback: Callable | None = None) -> list[dict]:
    ...
    def fetch_discount_codes(self, price_rule_id: int) -> list[dict]:
        ...
        return self._fetch_all_paginated(
            f"/price_rules/{price_rule_id}/discount_codes.json", "discount_codes"
        )
    ...
    def fetch_all_articles(
        self, blog_id: int, progress_callback: Callable | None = None
    ) -> list[dict]:
    ...
    def fetch_collects(self, product_id: int = None, collection_id: int = None) -> list[dict]:
        ...
        return self._fetch_all_paginated("/collects.json", "collects", extra_params=params)
    ...
    def _fetch_page(
        self, endpoint: str, resource_key: str, limit: int = 50, extra_params: dict | None = None
    ) -> list[dict]:
    ...
    def _rate_limit_wait(self):
        ...
        if self._bucket_max > 0 and self._bucket_used >= (self._bucket_max - 2):
            logger.debug(
                f"Rate limit near capacity ({self._bucket_used}/{self._bucket_max}), backing off"
            )
            time.sleep(1.0)
    ...
    def _update_rate_limit_info(self, response: requests.Response):
        ...
        if self._bucket_used > 0 and self._bucket_used >= (self._bucket_max - 5):
            logger.warning(
                f"Rate limit warning: {self._bucket_used}/{self._bucket_max} requests used"
            )
