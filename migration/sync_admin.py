...
from .models import SyncConnection, SyncJob, SyncStep
from .sync.category_registry import (
    get_categories_grouped, get_categories_for_feature, SYNC_CATEGORIES
)
...
class SyncConnectionAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'role_badge', 'remote_url', 'verified_status', 'last_sync_display', 'actions_column']
    list_filter = ['role', 'is_verified']
    readonly_fields = ['id', 'remote_version', 'remote_site_name', 'is_verified',
...
class SyncJobAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'job_id_display', 'job_type_badge', 'direction_display',
        'status_badge', 'progress_bar', 'items_summary',
        'created_display', 'actions_column',
    ]
    list_filter = ['job_type', 'status', 'direction', 'created_at']
    readonly_fields = [
        'id', 'connection', 'created_by', 'created_at',
        'started_at', 'completed_at', 'duration_seconds',
        'diff_preview', 'rollback_snapshot',
    ]
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path('sync/wizard/step1/', self.admin_site.admin_view(self.sync_wizard_step1),
                 name='sync_wizard_step1'),
            path('sync/wizard/step2/<uuid:job_id>/', self.admin_site.admin_view(self.sync_wizard_step2),
                 name='sync_wizard_step2'),
            path('sync/wizard/step3/<uuid:job_id>/', self.admin_site.admin_view(self.sync_wizard_step3),
                 name='sync_wizard_step3'),
            path('sync/wizard/step4/<uuid:job_id>/', self.admin_site.admin_view(self.sync_wizard_step4),
                 name='sync_wizard_step4'),

            # Full Migration Wizard
            path('fullmig/wizard/step1/', self.admin_site.admin_view(self.fullmig_wizard_step1),
                 name='fullmig_wizard_step1'),
            path('fullmig/wizard/step2/<uuid:job_id>/', self.admin_site.admin_view(self.fullmig_wizard_step2),
                 name='fullmig_wizard_step2'),
            path('fullmig/wizard/step3/<uuid:job_id>/', self.admin_site.admin_view(self.fullmig_wizard_step3),
                 name='fullmig_wizard_step3'),
            path('fullmig/wizard/step4/<uuid:job_id>/', self.admin_site.admin_view(self.fullmig_wizard_step4),
                 name='fullmig_wizard_step4'),
            path('fullmig/wizard/step5/<uuid:job_id>/', self.admin_site.admin_view(self.fullmig_wizard_step5),
                 name='fullmig_wizard_step5'),

            # Shared API endpoints
            path('api/test-connection/', self.admin_site.admin_view(self.api_test_connection),
                 name='sync_api_test_connection'),
            path('api/sync-progress/<uuid:job_id>/', self.admin_site.admin_view(self.api_sync_progress),
                 name='sync_api_progress'),
            path('api/start-sync/<uuid:job_id>/', self.admin_site.admin_view(self.api_start_sync),
                 name='sync_api_start'),
            path('api/rollback/<uuid:job_id>/', self.admin_site.admin_view(self.api_rollback),
                 name='sync_api_rollback'),

            # Token management
            path('tokens/', self.admin_site.admin_view(self.token_management),
    ...
    def job_id_display(self, obj):
        ...
        return format_html(
            ...
            'border-radius: 4px; font-size: 12px;">{}</code>', short_id
        )
    ...
    def job_type_badge(self, obj):
        ...
        return format_html(
            ...
            '</div>', icon, color, obj.get_job_type_display()
        )
    ...
    def direction_display(self, obj):
        ...
        return format_html(
            ...
            icons.get(obj.direction, ''), obj.get_direction_display()
        )
    direction_display.short_description = _('Direction')

    def status_badge(self, obj):
        colors = {
            'pending': '#ffa500', 'previewing': '#9c27b0',
            'awaiting_confirmation': '#ff9800', 'running': '#2196f3',
            'completed': '#4caf50', 'failed': '#f44336',
            'rolling_back': '#ff5722', 'rolled_back': '#607d8b',
            'cancelled': '#999',
        }
    ...
    def progress_bar(self, obj):
        ...
        return format_html(
            ...
            '{}%</span></div>', color, percent, percent
        )
    ...
    def items_summary(self, obj):
        ...
        return format_html(
            ...
            obj.items_synced, obj.items_failed, obj.items_total
        )
    ...
    def sync_wizard_step1(self, request):
        ...
        return render(request, 'admin/migration/sync/step1_connection.html', {
    ...
    def sync_wizard_step2(self, request, job_id):
        ...
        return render(request, 'admin/migration/sync/step2_categories.html', {
            **self.admin_site.each_context(request),
    ...
    def sync_wizard_step3(self, request, job_id):
        ...
        if not job.diff_preview:
            try:
                ...
                messages.error(request, _("Preview generation failed. Please check the connection and try again."))

        return render(request, 'admin/migration/sync/step3_preview.html', {
            **self.admin_site.each_context(request),
    ...
    def sync_wizard_step4(self, request, job_id):
        ...
        return render(request, 'admin/migration/sync/step4_progress.html', {
            **self.admin_site.each_context(request),
    ...
    def fullmig_wizard_step1(self, request):
        ...
        return render(request, 'admin/migration/fullmig/step1_connection.html', {
            **self.admin_site.each_context(request),
    ...
    def fullmig_wizard_step2(self, request, job_id):
        ...
        return render(request, 'admin/migration/fullmig/step2_scope.html', {
            **self.admin_site.each_context(request),
    ...
    def fullmig_wizard_step3(self, request, job_id):
        ...
        try:
            ...
            messages.error(request, _("Compatibility check failed. Please verify the connection and try again."))

        if request.method == 'POST':
            return redirect(reverse('admin:fullmig_wizard_step4', args=[job.pk]))

        return render(request, 'admin/migration/fullmig/step3_preflight.html', {
            **self.admin_site.each_context(request),
    ...
    def fullmig_wizard_step4(self, request, job_id):
        ...
        return render(request, 'admin/migration/fullmig/step4_progress.html', {
            **self.admin_site.each_context(request),
    ...
    def fullmig_wizard_step5(self, request, job_id):
        ...
        return render(request, 'admin/migration/fullmig/step5_results.html', {
            **self.admin_site.each_context(request),
    ...
    def api_start_sync(self, request, job_id):
        ...
        from .sync.tasks import execute_settings_sync, execute_full_migration
    ...
    def token_management(self, request):
        ...
        return render(request, 'admin/migration/sync/token_management.html', {
            **self.admin_site.each_context(request),
