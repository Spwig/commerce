...
class TranslationProviderAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'priority', 'total_translations', 'last_used_at'
    ]
    list_filter = ['provider_type', 'is_active', 'is_default']
    search_fields = ['name', 'api_endpoint']
    readonly_fields = [
        'total_translations', 'total_characters',
        'total_errors', 'last_used_at', 'created_at', 'updated_at'
    ]
    ...
    fieldsets = (
        ...
        (_('Limits & Performance'), {
            'fields': (
                'max_chars_per_request', 'rate_limit', 'timeout_seconds'
            )
        }),
        (_('Statistics'), {
            'fields': (
                'total_translations', 'total_characters',
                'total_errors', 'last_used_at'
            ),
            'classes': ('collapse',),
        }),
        (_('Timestamps'), {
    ...
    def mark_as_default(self, request, queryset):
        if queryset.count() > 1:
            self.message_user(
                request,
                _('Please select only one provider to set as default'),
                level='error'
            )
            return
    ...
    def activate_providers(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d provider(s) activated') % {'count': updated}
        )
    ...
    def deactivate_providers(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d provider(s) deactivated') % {'count': updated}
        )
...
class TranslationProviderAccountAdmin(admin.ModelAdmin):
    list_display = [
        'display_name', 'component', 'is_active', 'is_default',
        'connection_status', 'total_translations', 'last_used_at'
    ]
    list_filter = ['is_active', 'is_default', 'connection_status']
    search_fields = ['display_name', 'component__name']
    readonly_fields = [
        'id', 'connection_status', 'connection_error', 'last_tested_at',
        'total_translations', 'total_characters', 'total_errors',
        'last_used_at', 'created_at', 'updated_at',
    ]

    fieldsets = (
        (_('Account'), {
            'fields': ('id', 'component', 'user', 'display_name')
        }),
        (_('Status'), {
        ...
        (_('Connection'), {
            'fields': (
                'connection_status', 'connection_error', 'last_tested_at'
            ),
            'classes': ('collapse',),
        }),
        (_('Metrics'), {
...
class TranslationMetaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Translation Information'), {
            'fields': (
                'source_checksum', 'translation_provider',
        ...
        (_('Quality Control'), {
            'fields': (
                'confidence_score', 'is_reviewed', 'reviewed_by', 'reviewed_at'
            ),
            'classes': ('collapse',),
        }),
        (_('Timestamps'), {
    ...
    def lock_translations(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d translation(s) locked') % {'count': count}
        )
    ...
    def unlock_translations(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d translation(s) unlocked') % {'count': count}
        )
    ...
    def mark_as_reviewed(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d translation(s) marked as reviewed') % {'count': count}
        )
...
class InstalledModelAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'version', 'model_type', 'size_mb',
        'is_active', 'is_downloaded', 'download_progress_bar'
    ]
    list_filter = ['model_type', 'is_active', 'is_downloaded', 'device']
    search_fields = ['name', 'version']
    readonly_fields = [
        'total_translations', 'avg_latency_ms',
    ...
    fieldsets = (
        ...
        (_('Status'), {
            'fields': (
                'is_active', 'is_downloaded', 'download_progress'
            )
        }),
        (_('Metrics'), {
            'fields': (
    ...
    class Media:
        css = {
            'all': ('translations/admin/css/translations.css',)
        }
    ...
    def activate_models(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d model(s) activated') % {'count': count}
        )
    ...
    def deactivate_models(self, request, queryset):
        ...
        self.message_user(
            request,
            _('%(count)d model(s) deactivated') % {'count': count}
        )
    ...
    def download_models(self, request, queryset):
        ...
        self.message_user(
            request,
            _('Download initiated for %(count)d model(s)') % {'count': queryset.count()}
        )
