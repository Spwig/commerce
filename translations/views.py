...
def shared_fleet_blocked(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if _is_shared_fleet():
            return JsonResponse({
                'success': False,
                'error': _('This feature is not available on your hosting plan. '
...
from .models import (
    TranslationProvider, TranslationProviderAccount, TranslationJob,
    TranslationMeta, InstalledModel, SiteLanguage
)
...
def dashboard_view(request):
    ...
    stats = {
        ...
        'total_characters': TranslationJob.objects.aggregate(
    ...
    if is_shared_fleet:
        ...
        if cpu_count < 4:
            ...
            system_check['warnings'].append(_('CPU: %(cores)d cores (minimum: 4)') % {'cores': cpu_count})

        if system_info.total / (1024**3) < 8:
            system_check['meets_requirements'] = False
            system_check['warnings'].append(_('RAM: %(ram).1fGB (minimum: 8GB)') % {'ram': system_info.total / (1024**3)})

        if disk.free / (1024**3) < 3:
            system_check['meets_requirements'] = False
            system_check['warnings'].append(_('Disk: %(disk).1fGB free (minimum: 3GB)') % {'disk': disk.free / (1024**3)})
    ...
    try:
        ...
        coverage = {'overall_percentage': 0, 'has_languages': False, 'languages': [], 'content_types': []}
            ...
            for svc_model in service_models:
                if isinstance(svc_model, dict):
                    ...
                    if model_name in expected_models:
                        ...
                        if not created:
                            installed_model.is_downloaded = svc_model.get('status') != 'not_installed'
                            installed_model.size_mb = svc_model.get('size_mb', installed_model.size_mb)
                            installed_model.save()

                        # Set as default if it's the loaded model
                        if loaded_model_name and (model_name == loaded_model_name or model_name.replace('_', '-') == loaded_model_name):
...
def local_service_view(request):
    ...
    if _is_shared_fleet():
        messages.info(request, _('The translation service is managed by Spwig on your hosting plan.'))
    ...
    if client.is_available():
        ...
        if full_status:
            ...
            for model_id, model_info in all_models.items():
                ...
                loaded_quant = full_status.get('system', {}).get('quantization') if is_loaded else None
                ...
                config = {
                    ...
                    'size_gb': round(db_model.size_mb / 1024.0, 2) if db_model and db_model.size_mb > 0 else model_info['size_gb'],
                    ...
                    'hardware_supported_quantizations': full_status.get('hardware', {}).get('supported_quantizations', []),
                    'is_loaded': is_loaded,
            ...
            response_data['disk_usage'] = {
                ...
                'system_used_gb': disk_info.get('total_gb', 0) - disk_info.get('free_gb', 0) - disk_info.get('models_size_gb', 0)
            }
...
def download_status_view(request):
    ...
    if status:
        ...
        return JsonResponse({
            'status': 'unknown',
            'error': _('Could not get download status')
        })
...
def start_download_view(request):
    ...
    return JsonResponse({
        'success': False,
        'error': _('Failed to start download')
    })
...
def toggle_translation_service_view(request):
    ...
    return JsonResponse({
        'success': True,
        'enabled': enabled,
        'message': _('Translation service enabled') if enabled else _('Translation service disabled')
    })
...
def quantization_list_view(request):
    ...
    return JsonResponse({
        'success': False,
        'error': _('Failed to get quantization list'),
        'quantizations': []
    })
...
def quantization_set_view(request):
    ...
    if not quantization:
        return JsonResponse({
            'success': False,
            'error': _('No quantization level specified')
        })
    ...
    if quantization not in valid_quants:
        return JsonResponse({
            'success': False,
            'error': _('Invalid quantization level')
        })
    ...
    return JsonResponse({
        'success': False,
        'error': _('Failed to set quantization level')
    })
...
def languages_list_api(request):
    ...
    return JsonResponse({
        ...
        'active_count': sum(1 for l in language_data if l['is_active']),
    })
...
def languages_reorder_api(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'message': _('Language order updated successfully')
        })
    except Exception as e:
...
def languages_bulk_update_api(request):
    ...
    try:
        ...
        if deactivate_ids:
            SiteLanguage.objects.filter(
                id__in=deactivate_ids
            ).exclude(is_default=True).update(is_active=False)
...
def browse_providers_view(request):
    ...
    for provider in available_from_server:
        ...
        provider_data = {
            ...
            'documentation_url': _safe_external_url(provider.get('documentation_url') or manifest.get('documentation_url', '')),
    ...
    try:
        ...
        with transaction.atomic():
            ...
            try:
                ...
                logger.warning("Could not reload providers after installing %s: %s", provider_slug, e)
        ...
        return JsonResponse({
            'success': False,
            'error': _('An unexpected error occurred during installation.')
        }, status=500)
...
def update_translation_provider_ajax(request, provider_slug):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'error': _('An unexpected error occurred during update.')
        }, status=500)
...
def test_provider_api(request):
    ...
    try:
        ...
        if not provider_type:
            return JsonResponse({
                'success': False,
                'error': _('Provider type is required')
            }, status=400)
        ...
        for field_key, field_def in credential_schema.items():
            ...
            if field_def.get('required') and not value:
                return JsonResponse({
                    'success': False,
                    'error': _('%(field)s is required') % {'field': field_def.get('label', field_key)}
                }, status=400)
...
def test_translation_api(request):
    ...
    try:
        ...
        if not text:
            return JsonResponse({
                'success': False,
                'error': _('Please enter text to translate')
            }, status=400)

        if not target_lang:
            return JsonResponse({
                'success': False,
                'error': _('Please select a target language')
            }, status=400)
...
def _test_custom_translation(request, text, source_lang, target_lang):
    ...
    if not api_endpoint:
        return JsonResponse({
            'success': False,
            'error': _('API endpoint URL is required for custom providers')
        }, status=400)
...
def toggle_provider_api(request, account_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'error': _('Failed to update provider status. Please try again.')
        }, status=500)
...
def translation_jobs_view(request):
    ...
    # Get recent jobs
    recent_jobs = TranslationJob.objects.select_related('provider', 'created_by').order_by('-created_at')[:10]
        ...
        if client and client.is_available():
            ...
            try:
                ...
                if system_info and system_info.get('model_loaded'):
                    ...
                    loaded_model_name = system_info.get('model_name') or system_info.get('current_model')
                    if loaded_model_name:
                        # Mark the loaded model as active
                        InstalledModel.objects.filter(name=loaded_model_name.replace('-', '_')).update(is_active=True)
                else:
...
def retry_job_api(request, job_id):
    ...
    try:
        ...
        if job.status != 'failed':
            return JsonResponse({
                'success': False,
                'error': _('Can only retry failed jobs')
            }, status=400)
...
def start_job_api(request, job_id):
    ...
    try:
        ...
        if job.status != 'pending':
            return JsonResponse({
                'success': False,
                'error': _('Can only start pending jobs')
            }, status=400)
...
def queue_status_api(request):
    ...
    priority_breakdown = TranslationJob.objects.filter(
...
def ui_translations_editor_view(request):
    ...
    from translations.ui_string_registry import (
        UI_STRING_REGISTRY, UI_STRING_SECTIONS,
        get_strings_by_section, get_total_string_count
    )
    ...
    active_languages = SiteLanguage.objects.filter(
...
def ui_translations_export_api(request, language_code):
    ...
    response['Content-Disposition'] = f'attachment; filename="ui-translations-{site_lang.code}.json"'
    return response
...
def _validate_import_file(request, language_code):
    ...
    try:
        ...
        return None, None, JsonResponse({'success': False, 'error': 'Language not found'}, status=404)
    ...
    if upload.size > 1_048_576:
        return None, None, JsonResponse({'success': False, 'error': 'File too large (max 1MB)'}, status=400)

    try:
        raw = upload.read().decode('utf-8')
        pack = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError):
        return None, None, JsonResponse({'success': False, 'error': 'Invalid JSON file'}, status=400)

    if not isinstance(pack, dict) or 'strings' not in pack:
        return None, None, JsonResponse({'success': False, 'error': 'Invalid pack format: missing "strings" field'}, status=400)
    ...
    if not isinstance(strings, dict):
        return None, None, JsonResponse({'success': False, 'error': 'Invalid pack format: "strings" must be a dictionary'}, status=400)
...
def ui_translations_import_apply_api(request, language_code):
    ...
    override_obj.total_strings = get_total_string_count()
    override_obj.translated_count = sum(1 for v in existing.values() if v)
    override_obj.verified_count = sum(1 for m in meta_info.values() if isinstance(m, dict) and m.get('verified'))
    override_obj.save()
...
def ui_translations_translate_string_api(request, language_code):
    ...
    if not client.enabled:
        return JsonResponse({'success': False, 'error': 'Translation service is not available'}, status=503)
...
def coverage_detail_view(request):
    ...
    groups = {}
    priority_labels = {1: _('Core Storefront'), 2: _('Content & Communications'), 3: _('Supporting Content'), 4: _('Optional')}
...
def translate_all_estimate_api(request):
    ...
    lang_codes = [l['code'] for l in target_languages]
    ...
    return JsonResponse({
        ...
        'languages': [{'code': l['code'], 'name': l['name']} for l in target_languages],
...
def translate_all_api(request):
    ...
    from .content_registry import get_all_content_types, get_model_class, get_content_type, get_content_type_keys
    from .coverage_service import TranslationCoverageService
    ...
    if filter_lang:
        target_languages = [l for l in target_languages if l['code'] == filter_lang]

    job_ids = []
    registry_keys = get_content_type_keys()
...
def translate_all_status_api(request):
    ...
    if not job_ids_str:
        return JsonResponse({'success': True, 'total_jobs': 0, 'completed': 0, 'processing': 0, 'pending': 0, 'failed': 0, 'overall_progress': 0})
...
def bulk_job_action_api(request):
    ...
    try:
        ...
        if not action or not job_ids:
            return JsonResponse({
                'success': False,
                'error': _('Action and job IDs required')
            }, status=400)
