...
class TranslationCoverageService:
    ...
    def _calculate_jsonfield_coverage(self, ct_entry, target_languages):
        ...
        if not rows:
            return {
                ...
                "percentage": 0,
                "by_language": {
                    lang["code"]: {"total": 0, "translated": 0, "pct": 0}
                    for lang in target_languages
                },
            }
        ...
        for row in rows:
            ...
            for _field_idx, field_name in enumerate(fields):
                # Get source value — check if this field has content
                src_field_idx = (
                    source_field_names.index(field_name)
    ...
    def _calculate_email_coverage(self, target_languages):
        ...
        existing_translations = set(
            EmailTemplateTranslation.objects.filter(template_id__in=base_templates).values_list(
        ...
        for lang in target_languages:
            ...
            translated = sum(1 for tid in base_templates if (tid, code) in existing_translations)
            by_language[code] = {
                "total": total,
    ...
    def _calculate_ui_string_coverage(self, target_languages):
        ...
        from .ui_string_registry import UI_STRING_REGISTRY, get_total_string_count
        ...
        for lang in target_languages:
            ...
            if code in BUILTIN_LANGUAGES and code != "en":
                translated = self._count_effective_ui_coverage(code, override, UI_STRING_REGISTRY)
            else:
