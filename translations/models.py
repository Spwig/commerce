...
class TranslationProvider(models.Model):
    ...
    provider_type = models.CharField(
        max_length=20, choices=PROVIDER_TYPES, default="local", verbose_name=_("Provider Type")
    )
    ...
    rate_limit = models.PositiveIntegerField(
        default=100, verbose_name=_("Rate Limit"), help_text=_("Maximum requests per minute")
    )
    timeout_seconds = models.PositiveIntegerField(
        default=30, verbose_name=_("Timeout (seconds)"), help_text=_("Request timeout in seconds")
    )

    # Status
    is_active = models.BooleanField(
        default=True, verbose_name=_("Active"), help_text=_("Enable this translation provider")
    )
    ...
    total_translations = models.PositiveIntegerField(
        default=0, verbose_name=_("Total Translations")
    )
    total_characters = models.BigIntegerField(
        default=0, verbose_name=_("Total Characters Translated")
    )
    total_errors = models.PositiveIntegerField(default=0, verbose_name=_("Total Errors"))
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            TranslationProvider.objects.filter(is_default=True).exclude(pk=self.pk).update(
                is_default=False
            )
...
class TranslationJob(models.Model):
    ...
    job_type = models.CharField(
        max_length=20, choices=JOB_TYPES, default="custom", verbose_name=_("Job Type")
    )
    ...
    # Content
    source_language = models.CharField(max_length=10, verbose_name=_("Source Language"))
    target_languages = models.JSONField(default=list, verbose_name=_("Target Languages"))
    ...
    fields_to_translate = models.JSONField(default=list, verbose_name=_("Fields to Translate"))
    ...
    translated_characters = models.PositiveIntegerField(
        default=0, verbose_name=_("Translated Characters")
    )
...
class TranslationMeta(models.Model):
    ...
    translated_at = models.DateTimeField(null=True, blank=True, verbose_name=_("Translated At"))
    ...
    is_reviewed = models.BooleanField(
        default=False, verbose_name=_("Reviewed"), help_text=_("Has been reviewed by a human")
    )
...
class SiteLanguage(models.Model):
    ...
    name = models.CharField(
        max_length=100, verbose_name=_("English Name"), help_text=_("Language name in English")
    )
    ...
    order = models.IntegerField(
        default=0, verbose_name=_("Display Order"), help_text=_("Order in language selector")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            SiteLanguage.objects.filter(is_default=True).exclude(pk=self.pk).update(
                is_default=False
            )
        super().save(*args, **kwargs)
    ...
    def get_support_indicator(self):
        ...
        indicators = {"full": "🟢", "limited": "🟡", "external": "🔵", "none": "🔴"}
...
class InstalledModel(models.Model):
    ...
    name = models.CharField(max_length=200, unique=True, verbose_name=_("Model Name"))
    version = models.CharField(max_length=50, verbose_name=_("Version"))
    model_type = models.CharField(max_length=50, default="m2m100", verbose_name=_("Model Type"))
    ...
    source_languages = models.JSONField(default=list, verbose_name=_("Source Languages"))
    target_languages = models.JSONField(default=list, verbose_name=_("Target Languages"))

    # Storage
    file_path = models.CharField(max_length=500, verbose_name=_("File Path"))
    size_mb = models.FloatField(verbose_name=_("Size (MB)"))
    ...
    device = models.CharField(
        max_length=20, default="cpu", verbose_name=_("Device"), help_text=_("cpu or cuda")
    )

    # Status
    is_active = models.BooleanField(default=True, verbose_name=_("Active"))
    is_downloaded = models.BooleanField(default=False, verbose_name=_("Downloaded"))
    is_default = models.BooleanField(
        default=False,
    ...
    avg_latency_ms = models.FloatField(
        null=True, blank=True, verbose_name=_("Average Latency (ms)")
    )
...
class UITranslationOverride(models.Model):
    ...
    def __str__(self):
        return (
            f"UI Translations ({self.language.code}) - {self.translated_count}/{self.total_strings}"
        )
...
class TranslationProviderAccount(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            TranslationProviderAccount.objects.filter(is_default=True).exclude(pk=self.pk).update(
                is_default=False
            )
        super().save(*args, **kwargs)
