...
(function () {
  ...
  const CoverageDetail = {
      ...
      for (let i = 0; i < cts.length; i++) {
        ...
        for (const code in bl) {
          ...
          const fill = document.querySelector(
            '[data-ct="' + ct.key + '"][data-lang="' + code + '"]'
          );
          if (fill) fill.style.width = stats.pct + '%';
          const stat = document.querySelector(
            '[data-ct="' + ct.key + '"][data-lang="' + code + '-stats"]'
          );
          if (stat)
            stat.textContent = stats.translated + '/' + stats.total + ' (' + stats.pct + '%)';
        }
      }
    },
    ...
    _showProgress: function (data) {
      ...
      html +=
        '<p class="translate-all-progress__tip"><i class="fas fa-info-circle"></i> You can close this dialog. Jobs will continue in the background. <a href="' +
