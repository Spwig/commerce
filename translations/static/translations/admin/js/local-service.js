...
(function () {
  ...
  function initializeService() {
    ...
    const serviceDataUrl = (document.getElementById('translations-config') || document.body).dataset
      .serviceDataUrl;
    if (!serviceDataUrl) {
      ...
      if (data.quality_settings) {
        Object.keys(data.quality_settings).forEach(modelId => {
          ...
          if (settingsDiv) {
            ...
            if (qualityGrid) {
              try {
                qualityGrid.innerHTML = data.quality_settings[modelId]
                  .map(quality => generateQualityCard(modelId, quality))
                  .join('');
              } catch (e) {
  ...
  function generateQualityMetrics(quant, modelId) {
    ...
    const memSize =
      quant === 'float32' ? 1.6 : quant === 'float16' || quant === 'int16' ? 0.8 : 0.4;
  ...
  function generateQualityCard(modelId, quality) {
    ...
    return `
            <div class="quality-card ${quality.is_active ? 'active' : quality.is_available ? 'available' : ''} ${!quality.is_supported ? 'hardware-unsupported' : ''}"
                ...
                ${
                  quality.is_active
                    ? '<span class="available-badge active"><i class="fas fa-play-circle"></i> Active</span>'
                    : quality.is_available
                      ? '<span class="available-badge"><i class="fas fa-check"></i> Available</span>'
                      : ''
                }
  ...
  function generateDiskUsage(disk) {
    const modelsPercent =
      disk.disk_total_gb > 0 ? Math.round((disk.disk_usage_gb / disk.disk_total_gb) * 100) : 0;
    const otherPercent =
      disk.disk_total_gb > 0 ? Math.round((disk.system_used_gb / disk.disk_total_gb) * 100) : 0;
  ...
  function activateQuality(modelId, quality) {
    ...
    if (card && card.classList.contains('hardware-unsupported')) {
      AdminModal.alert({
        message: 'This quality setting is not supported on your current hardware.',
        type: 'warning',
      });
      return;
    }
    ...
    const quantizationSetUrl = (document.getElementById('translations-config') || document.body)
      .dataset.quantizationSetUrl;
  ...
  async function installAndActivate(modelId, quality) {
    ...
    const startDownloadUrl = (document.getElementById('translations-config') || document.body)
      .dataset.startDownloadUrl;
    if (!startDownloadUrl) {
  ...
  function pollDownloadStatus(modelId, quality, btn, originalContent, progressDiv, card) {
    const downloadStatusUrl = (document.getElementById('translations-config') || document.body)
      .dataset.downloadStatusUrl;
    const quantizationSetUrl = (document.getElementById('translations-config') || document.body)
      .dataset.quantizationSetUrl;
      ...
      fetch(downloadStatusUrl + '?model=' + modelId, {
        ...
        .then(data => {
          ...
          if (data.status === 'completed' || data.status === 'complete') {
            ...
            progressDiv.querySelector('.progress-message').textContent =
              'Download complete! Activating...';
