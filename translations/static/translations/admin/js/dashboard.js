...
(function() {
    ...
    function startModelDownload(callback) {
        const statusUrl = (document.getElementById('translations-config') || document.body).dataset.downloadStatusUrl;
        const startUrl = (document.getElementById('translations-config') || document.body).dataset.downloadStartUrl;
        ...
        .then(statusData => {
            ...
            .then(data => {
                if (data.success || data.status === 'downloading') {
                    ...
                    AdminModal.alert({message: 'Failed to start download: ' + (data.error || data.message), type: 'error'});
                    closeDownloadModal();
                }
            });
        });
    ...
    const CoverageManager = {
        ...
        confirmTranslateAll: function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    AdminModal.alert({message: data.error || 'Failed to start translation', type: 'error'});
                }
            })
            .catch(err => {
                btn.disabled = false;
        ...
        _showProgress: function(data) {
            const jobsUrl = (document.getElementById('translations-config') || document.body).dataset.jobsUrl || '/admin/translations/translation-jobs/';
            document.getElementById('translate-all-footer').setAttribute('hidden', '');
            ...
            html += '<p class="translate-all-progress__tip"><i class="fas fa-info-circle"></i> You can close this dialog. Jobs will continue in the background. Monitor progress in <a href="' + jobsUrl + '">Translation Jobs</a>.</p>';
            html += '</div>';
            document.getElementById('translate-all-body').innerHTML = html;
    ...
    function handleQuickTestForm(e) {
        ...
        const quickTranslateUrl = (document.getElementById('translations-config') || document.body).dataset.quickTranslateUrl;
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // Show error
                resultDiv.classList.remove('hidden');
                resultText.innerHTML = '<div class="translation-error">❌ ' + (data.error || 'Translation failed') + '</div>';
            }
        })
        .catch(error => {
            ...
            // Show error
            resultDiv.classList.remove('hidden');
            resultText.innerHTML = '<div class="translation-error">❌ Connection error: ' + error.message + '</div>';
        });
    }
    ...
    function handleServiceToggle(e) {
        const toggleUrl = (document.getElementById('translations-config') || document.body).dataset.toggleServiceUrl;
    ...
    async function handleBenchmark(e) {
        ...
        const benchmarkUrl = (document.getElementById('translations-config') || document.body).dataset.benchmarkUrl;
