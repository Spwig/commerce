...
var UITransEditor = {
    ...
    loadStrings: function(langCode) {
        var container = document.getElementById('strings-container');
        container.innerHTML = '<div class="ui-trans-loading"><i class="fas fa-spinner fa-spin"></i><p>Loading translations...</p></div>';
        ...
        fetch('/api/translations/service/ui-translations/' + langCode + '/')
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    container.innerHTML = '<div class="ui-trans-loading"><i class="fas fa-exclamation-triangle"></i><p>' + (data.error || 'Failed to load') + '</p></div>';
                }
            })
            .catch(function(err) {
                container.innerHTML = '<div class="ui-trans-loading"><i class="fas fa-exclamation-triangle"></i><p>Failed to load translations</p></div>';
                console.error(err);
            });
    },
    ...
    renderStrings: function(data) {
        ...
        for (var sectionKey in sections) {
            ...
            html += ' <span class="ui-trans-section__count">' + translated + '/' + count + '</span></div>';
            ...
            for (var stringKey in strings) {
                var entry = strings[stringKey];
                var meta = entry.meta || {};
                var isEmpty = !entry.translation;
                var isLocked = !!meta.locked;

                var badge = '';
                if (isLocked) {
                    badge = '<span class="ui-trans-badge ui-trans-badge--locked"><i class="fas fa-lock"></i> Locked</span>';
                } else if (isEmpty) {
                    badge = '<span class="ui-trans-badge ui-trans-badge--missing">Missing</span>';
                } else if (entry.source === 'shipped') {
                    badge = '<span class="ui-trans-badge ui-trans-badge--shipped"><i class="fas fa-box"></i> Shipped</span>';
                ...
                html += '<div class="ui-trans-row' + (isLocked ? ' ui-trans-row--locked' : '') + '" data-key="' + stringKey + '"';
                ...
                html += '<textarea class="ui-trans-row__input' + (isEmpty ? ' ui-trans-row__input--empty' : '') + '"';
                ...
                html += '<button type="button" class="ui-trans-row__lock' + (isLocked ? ' ui-trans-row__lock--active' : '') + '" data-key="' + stringKey + '"';
                html += ' onclick="UITransEditor.toggleLock(this)" title="' + (isLocked ? 'Unlock translation' : 'Lock translation') + '">';
                html += '<i class="fas ' + (isLocked ? 'fa-lock' : 'fa-lock-open') + '"></i></button>';
                html += '<button type="button" class="ui-trans-row__translate" data-key="' + stringKey + '"';
    ...
    toggleSection: function(headerEl) {
        var section = headerEl.closest('.ui-trans-section');
    ...
    saveAll: function() {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                UITransEditor.updateProgress(data.translated_count, data.total_strings);
                // Update tab badge
                var tab = document.querySelector('.ui-trans-tab[data-lang="' + UITransEditor.currentLang + '"]');
                if (tab) {
                    var pct = data.total_strings > 0 ? Math.round((data.translated_count / data.total_strings) * 100) : 0;
                    var badge = tab.querySelector('.ui-trans-tab__badge');
                    if (badge) {
    ...
    exportPack: function() {
        ...
        window.location.href = '/api/translations/service/ui-translations/' + this.currentLang + '/export/';
    },
    ...
    previewImport: function(file) {
        ...
        var url = '/api/translations/service/ui-translations/' + this.currentLang + '/import/preview/';

        document.getElementById('import-dropzone').hidden = true;
        var previewEl = document.getElementById('import-preview');
        previewEl.hidden = false;
        previewEl.innerHTML = '<div class="ui-trans-loading"><i class="fas fa-spinner fa-spin"></i><p>Validating...</p></div>';
        ...
        .then(function(data) {
            if (data.success) {
                ...
                previewEl.innerHTML = '<div class="ui-trans-import-error"><i class="fas fa-exclamation-triangle"></i><p>' + self.esc(data.error) + '</p></div>';
                document.getElementById('import-footer').hidden = true;
            }
        })
        .catch(function(err) {
            previewEl.innerHTML = '<div class="ui-trans-import-error"><i class="fas fa-exclamation-triangle"></i><p>Validation failed</p></div>';
        });
    },

    _renderPreview: function(preview, filename) {
        var html = '<div class="ui-trans-import-summary">';
        html += '<h4><i class="fas fa-file-alt"></i> ' + this.esc(filename) + '</h4>';
        html += '<div class="ui-trans-import-stats">';
        html += '<div class="ui-trans-import-stat ui-trans-import-stat--new"><span class="ui-trans-import-stat__value">' + preview.new + '</span><span class="ui-trans-import-stat__label">New</span></div>';
        html += '<div class="ui-trans-import-stat ui-trans-import-stat--updated"><span class="ui-trans-import-stat__value">' + preview.updated + '</span><span class="ui-trans-import-stat__label">Updated</span></div>';
        html += '<div class="ui-trans-import-stat ui-trans-import-stat--unchanged"><span class="ui-trans-import-stat__value">' + preview.unchanged + '</span><span class="ui-trans-import-stat__label">Unchanged</span></div>';
        html += '<div class="ui-trans-import-stat ui-trans-import-stat--skipped"><span class="ui-trans-import-stat__value">' + preview.skipped + '</span><span class="ui-trans-import-stat__label">Skipped</span></div>';
        html += '</div>';

        if (preview.skipped > 0 && preview.skipped_keys && preview.skipped_keys.length > 0) {
            html += '<details class="ui-trans-import-skipped"><summary>Skipped keys (not in registry)</summary><ul>';
            for (var i = 0; i < preview.skipped_keys.length; i++) {
        ...
        if (preview.pack_registry_version && preview.pack_registry_version !== preview.current_registry_version) {
            html += '<p class="ui-trans-import-notice"><i class="fas fa-info-circle"></i> ';
            html += 'Pack was exported with ' + preview.pack_registry_version + ' strings; current registry has ' + preview.current_registry_version + '.</p>';
        }
    ...
    applyImport: function() {
        ...
        var url = '/api/translations/service/ui-translations/' + this.currentLang + '/import/apply/';
        ...
        .then(function(data) {
            if (data.success) {
                ...
                // Update tab badge
                var tab = document.querySelector('.ui-trans-tab[data-lang="' + self.currentLang + '"]');
                if (tab) {
                    var pct = data.total_strings > 0 ? Math.round((data.translated_count / data.total_strings) * 100) : 0;
                    var badge = tab.querySelector('.ui-trans-tab__badge');
                    if (badge) {
    ...
    toggleLock: function(btnEl) {
        ...
        var url = '/api/translations/service/ui-translations/' + this.currentLang + '/lock/';
        ...
        .then(function(data) {
            if (data.success) {
                var row = btnEl.closest('.ui-trans-row');
                ...
                var textarea = row.querySelector('.ui-trans-row__input');
                if (textarea) textarea.readOnly = isLocked;

                // Toggle AI translate button
                var translateBtn = row.querySelector('.ui-trans-row__translate');
                if (translateBtn) translateBtn.disabled = isLocked;

                // Update badge
                var statusDiv = row.querySelector('.ui-trans-row__status');
                if (statusDiv) {
                    if (isLocked) {
                        statusDiv.innerHTML = '<span class="ui-trans-badge ui-trans-badge--locked"><i class="fas fa-lock"></i> Locked</span>';
                    } else {
                        // Restore original badge based on data attributes
                        var hasTranslation = row.dataset.hasTranslation === 'true';
                        var isAuto = row.dataset.isAuto === 'true';
                        var isVerified = row.dataset.isVerified === 'true';
                        var source = row.dataset.source || '';
                        if (!hasTranslation) {
                            statusDiv.innerHTML = '<span class="ui-trans-badge ui-trans-badge--missing">Missing</span>';
                        } else if (source === 'shipped') {
                            statusDiv.innerHTML = '<span class="ui-trans-badge ui-trans-badge--shipped"><i class="fas fa-box"></i> Shipped</span>';
                        } else if (isVerified) {
                            statusDiv.innerHTML = '<span class="ui-trans-badge ui-trans-badge--verified">Verified</span>';
                        } else if (isAuto) {
                            statusDiv.innerHTML = '<span class="ui-trans-badge ui-trans-badge--auto">Auto</span>';
                        } else {
                            statusDiv.innerHTML = '';
                        }
                    }
    ...
    translateString: function(btnEl) {
        ...
        var url = '/api/translations/service/ui-translations/' + this.currentLang + '/translate-string/';
        ...
        .then(function(data) {
            if (data.success && data.translated_text) {
                var textarea = document.querySelector('textarea[data-key="' + key + '"]');
                if (textarea) {
