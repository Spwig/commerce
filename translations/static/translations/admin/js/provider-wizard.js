...
(function () {
  ...
  function updateSummary() {
    ...
    if (summaryCredentials) {
      ...
      credFields.forEach(field => {
        if (field.name === 'name' || field.name === 'provider_type' || field.type === 'hidden')
          return;
        if (field.value) {
  ...
  function testConnection() {
    ...
    resultDiv.innerHTML =
      '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Testing connection...</div>';

    const formData = new FormData(document.getElementById('provider-form'));
    const testUrl = (document.getElementById('translations-config') || document.body).dataset
      .testProviderUrl;
  ...
  function testTranslation() {
    ...
    if (!text) {
      resultDiv.innerHTML =
        '<div class="alert alert-error"><i class="fas fa-exclamation-circle"></i> Please enter text to translate.</div>';
      return;
    }

    resultDiv.innerHTML =
      '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Translating...</div>';
  ...
  function saveConfiguration() {
    ...
    const saveUrl = (document.getElementById('translations-config') || document.body).dataset
      .saveProviderUrl;
    const providersUrl = (document.getElementById('translations-config') || document.body).dataset
      .providersUrl;
