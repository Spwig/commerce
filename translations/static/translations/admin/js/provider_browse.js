...
(function() {
    ...
    function loadI18n() {
        var el = document.getElementById('translation-browse-i18n');
        if (!el) return {};
        try {
    ...
    class ProviderInstaller {
        async install(providerSlug, providerName) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/translations/providers/install/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
        ...
        updateCardToInstalled(providerSlug) {
            ...
            if (installBtn) {
                ...
                configureBtn.href = AdminUtils.buildAdminUrl(`/admin/translations/provider-wizard/${providerSlug}/`);
    ...
    class ProviderUpdater {
        async update(providerSlug, providerVersion) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/translations/providers/update/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
                        'X-CSRFToken': AdminUtils.getCsrfToken(),
                        'Content-Type': 'application/json',
