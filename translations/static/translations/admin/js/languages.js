...
class LanguageManager {
    ...
    this.resetButton.addEventListener('click', async () => {
      if (
        await AdminModal.confirm({
          message: 'Reset to the original configuration? This will discard all changes.',
          danger: true,
          confirmText: 'Reset',
        })
      ) {
  ...
  updateCounts() {
    const availableCount = this.availableList.querySelectorAll(
      '.language-item:not(.filtered-out)'
    ).length;
  ...
  showMessage(message, type = 'info') {
    ...
    if (!messageEl) {
      ...
      document
        ...
        .insertBefore(messageEl, document.querySelector('#content-main').firstChild);
    }
