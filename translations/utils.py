...
class SystemValidator:
    ...
    def check_requirements(self) -> dict:
        ...
        if cpu_count < self.MIN_CPU_CORES:
            ...
            result["warnings"].append(
                ...
                % {"cores": cpu_count, "min": self.MIN_CPU_CORES}
            )
        ...
        if ram_gb < self.MIN_RAM_GB:
            ...
            result["warnings"].append(
                ...
                % {"ram": ram_gb, "min": self.MIN_RAM_GB}
            )
        ...
        if disk_free_gb < self.MIN_DISK_GB:
            ...
            result["warnings"].append(
                ...
                % {"disk": disk_free_gb, "min": self.MIN_DISK_GB}
            )
    ...
    def _check_translator_service(self) -> dict:
        ...
        if client.is_available():
            ...
            return {"available": True, "status": "running", "info": info}
        else:
            return {"available": False, "status": "offline", "info": None}
    ...
    def test_deepl(api_key: str, endpoint: str = None) -> tuple[bool, str]:
        ...
        try:
            ...
            response = requests.get(
                url, headers={"Authorization": f"DeepL-Auth-Key {api_key}"}, timeout=5
            )
