...
def subscription_detail(request, subscription_id):
    ...
    # Get billing history
    billing_logs = BillingCycleLog.objects.filter(subscription=subscription).order_by(
        "-billing_date"
    )[:12]  # Last 12 billing cycles
...
def subscription_pause(request, subscription_id):
    ...
    """
    subscription = get_object_or_404(
        CustomerSubscription, subscription_id=subscription_id, user=request.user
    )
...
def payment_token_list(request):
    ...
    """
    tokens = PaymentToken.objects.filter(user=request.user).order_by("-is_default", "-created_at")
...
def payment_token_delete(request, token_id):
    ...
    """
    token = get_object_or_404(PaymentToken, token_id=token_id, user=request.user)

    if request.method == "POST":
        # Check if token is being used by active subscriptions
        active_subs = CustomerSubscription.objects.filter(
            payment_token=token, status__in=["active", "trial", "past_due"]
        ).exists()
    ...
    if request.method == "POST":
        try:
            ...
            if not all(update_data.get(field) for field in required_fields):
                ...
                return render(
                    request,
                    "accounts/addresses/form.html",
                    {"address": address, "used_in_orders": used_in_orders},
                )

            # Use AddressService to handle versioning
            success, message, updated_address = AddressService.update_address(
                address=address, user=request.user, **update_data
            )
            ...
            if success:
                if used_in_orders and updated_address != address:
                    ...
                    messages.info(
                        request,
                        _(
                            "Address updated. A new version was created because this address was used in previous orders."
                        ),
                    )
                else:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            Address.objects.filter(
                user=request.user, address_type=address_type, is_default=True
...
def dashboard(request):
    ...
    try:
        ...
        referral_rewards_total = referral_rewards.filter(
    ...
    try:
        ...
        unread_reply_count = CustomerMessage.objects.filter(
            Q(user=user) | Q(email=user.email, user__isnull=True), status="replied"
        ).count()
    except Exception:
        pass
        ...
        # Process activation if POST request
            ...
            try:
                ...
                if success:
                    ...
                    messages.success(
                        request,
                        _("Your account has been activated! Welcome to {}").format(
                            request.get_host()
                        ),
                    )
                    return redirect("accounts:dashboard")
                ...
                messages.error(
                    request, _("An error occurred while activating your account. Please try again.")
                )
                context = {
                    "user": user,
                    "uidb64": uidb64,
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if action == "unsubscribe_type":
                ...
                if result["success"]:
                    messages.success(
                        request, _("You have been unsubscribed from all marketing communications.")
                    )
                else:
        ...
        messages.error(request, _("Invalid unsubscribe link. Please use the link from your email."))
        return redirect("page_builder:home")
...
def communication_preferences(request):
    ...
    if request.method == "POST":
        ...
        if "blog" not in prefs.app_preferences:
            prefs.app_preferences["blog"] = CommunicationPreference.get_default_app_preferences()[
                "blog"
            ]
        ...
        if "loyalty" not in prefs.app_preferences:
            prefs.app_preferences["loyalty"] = (
                CommunicationPreference.get_default_app_preferences()["loyalty"]
            )

        prefs.app_preferences["loyalty"]["enabled"] = loyalty_enabled
        prefs.app_preferences["loyalty"]["points_earned"] = (
            request.POST.get("loyalty_points_earned") == "on"
        )
        prefs.app_preferences["loyalty"]["tier_changes"] = (
            request.POST.get("loyalty_tier_changes") == "on"
        )
        prefs.app_preferences["loyalty"]["rewards_available"] = (
            request.POST.get("loyalty_rewards_available") == "on"
        )
        prefs.app_preferences["loyalty"]["points_expiring"] = (
            request.POST.get("loyalty_points_expiring") == "on"
        )
        ...
        if "referrals" not in prefs.app_preferences:
            prefs.app_preferences["referrals"] = (
                CommunicationPreference.get_default_app_preferences()["referrals"]
            )

        prefs.app_preferences["referrals"]["enabled"] = referrals_enabled
        prefs.app_preferences["referrals"]["reward_issued"] = (
            request.POST.get("referrals_reward_issued") == "on"
        )
        prefs.app_preferences["referrals"]["referral_converted"] = (
            request.POST.get("referrals_referral_converted") == "on"
        )
        ...
        if "affiliate" not in prefs.app_preferences:
            prefs.app_preferences["affiliate"] = (
                CommunicationPreference.get_default_app_preferences()["affiliate"]
            )

        prefs.app_preferences["affiliate"]["enabled"] = affiliate_enabled
        prefs.app_preferences["affiliate"]["commission_earned"] = (
            request.POST.get("affiliate_commission_earned") == "on"
        )
        prefs.app_preferences["affiliate"]["payout_processed"] = (
            request.POST.get("affiliate_payout_processed") == "on"
        )
        prefs.app_preferences["affiliate"]["monthly_report"] = (
            request.POST.get("affiliate_monthly_report") == "on"
        )
    ...
    blog_prefs = prefs.app_preferences.get(
        "blog", CommunicationPreference.get_default_app_preferences()["blog"]
    )
    loyalty_prefs = prefs.app_preferences.get(
        "loyalty", CommunicationPreference.get_default_app_preferences()["loyalty"]
    )
    referrals_prefs = prefs.app_preferences.get(
        "referrals", CommunicationPreference.get_default_app_preferences()["referrals"]
    )
    affiliate_prefs = prefs.app_preferences.get(
        "affiliate", CommunicationPreference.get_default_app_preferences()["affiliate"]
    )
...
def preference_history(request):
    ...
    logs = PreferenceChangeLog.objects.filter(user=request.user).order_by("-timestamp")
...
def message_list(request):
    ...
    user = request.user
    user_messages = (
        CustomerMessage.objects.filter(Q(user=user) | Q(email=user.email, user__isnull=True))
...
def message_new(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, _("Your message has been submitted. We will respond shortly.")
            )
...
def guest_order_lookup(request):
    ...
    if request.method == "POST":
        ...
        guest_user = (
            User.objects.filter(email__iexact=email, username__startswith="guest_")
        ...
        if guest_user and guest_user.orders.exists():
            try:
                ...
                magic_link = (
                    f"{site_url}/{request.LANGUAGE_CODE}/accounts/guest-orders/{uid}/{token}/"
                )
...
def booking_list(request):
    ...
    if profile:
        base_qs = Booking.objects.filter(customer=profile).select_related(
            "product", "resource", "order"
        )
    else:
    ...
    past_bookings = base_qs.exclude(pk__in=upcoming_bookings.values_list("pk", flat=True)).order_by(
        "-start_datetime"
    )
...
def booking_reschedule_action(request, booking_id):
    ...
    try:
        ...
        new_start = tz.make_aware(
            datetime.strptime(f"{date_str} {time_start_str}", "%Y-%m-%d %H:%M")
        )
