...
class OAuthProviderSettingsAdmin(admin.ModelAdmin):
    ...
    def provider_display(self, obj):
        ...
        return format_html(
            '<span style="font-size: 16px;">{} {}</span>', icon, obj.get_provider_display()
        )
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields["button_order"].help_text = _("Display order (lower numbers appear first)")
...
class CustomSocialAppAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not obj.sites.exists():
            self.message_user(
                request, _("Remember to add your site to the Sites list!"), level="WARNING"
            )
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    def staff_profile_view(self, request):
        ...
        # ---- Trusted devices ----
        trusted_devices = TrustedDevice.objects.filter(user=user, is_revoked=False).order_by(
            "-created_at"
        )
        ...
        try:
            ...
            active_devices = (
                MobileAuthToken.objects.filter(user=user, token_type="refresh", is_revoked=False)
            ...
            device_regs = {dr.device_id: dr for dr in DeviceRegistration.objects.filter(user=user)}
            ...
            for device in active_devices:
                ...
                revoke_token = (
                    MobileAuthToken.objects.filter(user=user, device_id=did, is_revoked=False)
                    .order_by("-expires_at")
                    .first()
                )
        ...
        if has_pos:
            try:
                ...
                from pos_app.models import POSShift, POSStaffDiscount, POSTerminal
                ...
                pos_terminals = list(
                    POSTerminal.objects.filter(assigned_users=user, is_active=True).select_related(
                        "warehouse"
                    )[:10]
                )
        ...
        try:
            ...
            recent_blog_posts = list(
                BlogPost.objects.filter(created_by=user).order_by("-created_at")[:3]
            )
    ...
    def revoke_mobile_session_view(self, request, token_id):
        ...
        try:
            ...
            token = MobileAuthToken.objects.get(pk=token_id, user=request.user, is_revoked=False)
            ...
            DeviceRegistration.objects.filter(user=request.user, device_id=device_id).delete()
        ...
        if request.method == "POST":
            try:
                device = TrustedDevice.objects.get(
                    pk=device_id, user=request.user, is_revoked=False
                )
                device.is_revoked = True
                device.save(update_fields=["is_revoked"])
                messages.success(
                    request, _('Device "{}" has been revoked.').format(device.device_name)
                )
    ...
    def revoke_all_trusted_devices_view(self, request):
        ...
        if request.method == "POST":
            count = TrustedDevice.revoke_all_for_user(
                request.user, reason="User revoked all devices"
            )
            if count:
                messages.success(
                    request, _("All {} trusted device(s) have been revoked.").format(count)
                )
            else:
                ...
                if totp_wrapper.validate_code(code):
                    ...
                    if trust_device and show_trust_device:
                        try:
                            ...
                            messages.success(
                                request, _("Verification successful. This device is now trusted.")
                            )
                            return response
                        except Exception:
...
class AccessTypeFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        admin_groups = StaffRole.objects.filter(can_access_admin=True).values_list(
            "group_id", flat=True
        )
        pos_groups = StaffRole.objects.filter(can_access_pos=True).values_list(
            "group_id", flat=True
        )

        if self.value() == "admin_only":
            return (
                queryset.filter(groups__in=admin_groups).exclude(groups__in=pos_groups).distinct()
            )
        elif self.value() == "pos_only":
            return (
                queryset.filter(groups__in=pos_groups).exclude(groups__in=admin_groups).distinct()
            )
...
class MFAStatusFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        try:
            ...
            users_with_mfa = Authenticator.objects.filter(type=Authenticator.Type.TOTP).values_list(
                "user_id", flat=True
            )
...
class StaffMemberAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        from staff_roles.services import can_access_admin, can_access_pos, get_user_roles
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        try:
            ...
            trusted_devices = list(
                TrustedDevice.objects.filter(user=staff_user, is_revoked=False).order_by(
                    "-created_at"
                )[:10]
            )
        except Exception:
        ...
        try:
            ...
            recent_blog_posts = list(
                BlogPost.objects.filter(created_by=staff_user).order_by("-created_at")[:3]
            )
    ...
    def revoke_session_view(self, request, user_id, token_id):
        ...
        try:
            ...
            token = MobileAuthToken.objects.get(pk=token_id, user_id=user_id, is_revoked=False)
...
class CommunicationPreferenceAdmin(admin.ModelAdmin):
    ...
    def email_status(self, obj):
        ...
        if obj.email_enabled:
            return format_html(
                '<span class="badge-yes" title="{}">&#10003;</span>', _("Email enabled")
            )
        return format_html('<span class="badge-no" title="{}">&#9675;</span>', _("Email disabled"))
    ...
    def sms_status(self, obj):
        ...
        if obj.sms_enabled:
            return format_html(
                '<span class="badge-yes" title="{}">&#10003;</span>', _("SMS enabled")
            )
        return format_html('<span class="badge-no" title="{}">&#9675;</span>', _("SMS disabled"))
    ...
    def marketing_status(self, obj):
        ...
        if obj.email_marketing or obj.sms_marketing:
            return format_html('<span class="badge-yes">&#10003; {}</span>', _("Opted In"))
        return format_html('<span class="badge-no">&#9675; {}</span>', _("Opted Out"))
    ...
    def verification_status(self, obj):
        ...
        return format_html(
            ...
            _("Email verified"),
            _("Yes") if obj.email_verified else _("No"),
            email_badge,
            _("SMS verified"),
            _("Yes") if obj.sms_verified else _("No"),
            sms_badge,
        )
    ...
    def bulk_verify_email(self, request, queryset):
        ...
        self.message_user(
            request, _(f"Successfully verified email for {count} user(s)."), level="SUCCESS"
        )
        ...
        for pref in queryset.select_related("user"):
            writer.writerow(
                [
                    ...
                    pref.consent_source,
                    pref.consent_timestamp.strftime("%Y-%m-%d %H:%M:%S")
                    if pref.consent_timestamp
                    else "",
                    pref.language_code,
                ]
            )
    ...
    def export_preferences_json(self, request, queryset):
        ...
        if queryset.count() == 1:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="preferences_{pref.user.username}.json"'
            )
        ...
        # Multiple users export
            ...
            response = HttpResponse(json.dumps(exports, indent=2), content_type="application/json")
