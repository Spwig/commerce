...
class CustomerProfile(CustomFieldsMixin, DesignMixin):
    ...
    def get_or_create_for_user(cls, user):
        ...
        profile, created = cls.objects.get_or_create(user=user, defaults={"user": user})
        return profile
    ...
    def average_order_value(self):
        ...
        result = self.user.orders.filter(status="delivered").aggregate(avg=Avg("total_amount"))[
            "avg"
        ]
        if not result:
            return Decimal("0")
    ...
    def is_at_risk(self):
        ...
        return bool(days_since_last > 90 and self.completed_orders_count > 0)
    ...
    def favorite_categories(self):
        ...
        return [
            cat["product__category__name"] for cat in categories if cat["product__category__name"]
        ]
...
class OAuthProviderSettings(models.Model):
    ...
    provider = models.CharField(
        max_length=50, choices=PROVIDER_CHOICES, unique=True, help_text="OAuth provider type"
    )
    enabled = models.BooleanField(
        default=False, help_text="Enable this provider for customer authentication"
    )
    display_name = models.CharField(max_length=100, help_text="Display name shown on login button")
    ...
    custom_scopes = models.JSONField(
        default=list, blank=True, help_text="Additional OAuth scopes (JSON array)"
    )

    # Configuration status (auto-calculated)
    is_configured = models.BooleanField(
        default=False, editable=False, help_text="Whether OAuth credentials are configured"
    )
    configuration_notes = models.TextField(
        blank=True, help_text="Setup notes or troubleshooting information"
    )
...
class PreferenceChangeLog(models.Model):
    ...
    old_value = models.JSONField(
        default=dict, verbose_name=_("Old Value"), help_text=_("Previous state before the change")
    )

    new_value = models.JSONField(
        default=dict, verbose_name=_("New Value"), help_text=_("New state after the change")
    )
    ...
    def change_summary(self):
        ...
        if not self.old_value:
            parts = [f"{k}: {v}" for k, v in self.new_value.items() if not k.startswith("_")]
            return ", ".join(parts[:5])
        ...
        for key in sorted(all_keys):
            ...
            if old_v != new_v:
                old_display = (
                    ...
                    else (str(old_v) if old_v is not None else "none")
                )
                new_display = (
                    str(new_v).lower()
                    if isinstance(new_v, bool)
                    else (str(new_v) if new_v is not None else "none")
                )
