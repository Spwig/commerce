...
(function() {
    ...
    var TOGGLE_SECTIONS = [
        ...
        { main: 'loyalty_enabled', subs: ['loyalty_points_earned', 'loyalty_tier_changes', 'loyalty_rewards_available', 'loyalty_points_expiring'] },
        { main: 'referrals_enabled', subs: ['referrals_reward_issued', 'referrals_referral_converted'] },
        { main: 'affiliate_enabled', subs: ['affiliate_commission_earned', 'affiliate_payout_processed', 'affiliate_monthly_report'] }
    ];
    ...
    function sendVerificationEmail(btn) {
        var successMsg = (btn && btn.dataset.successMsg) || 'Verification email sent! Please check your inbox.';
        var errorMsg = (btn && btn.dataset.errorMsg) || 'Error sending verification email. Please try again.';
    ...
    function setupToggleSections() {
        TOGGLE_SECTIONS.forEach(function(section) {
            var mainCheckbox = document.getElementById(section.main);
            if (!mainCheckbox) return;

            mainCheckbox.addEventListener('change', function() {
                if (section.frequency) {
                    var freqSelect = document.querySelector('select[name="' + section.frequency + '"]');
                    if (freqSelect) freqSelect.disabled = !mainCheckbox.checked;
                }
                if (section.subs) {
                    section.subs.forEach(function(subName) {
                        var subCheckbox = document.querySelector('input[name="' + subName + '"]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', async function(e) {
            ...
            if (action === 'subscribe-all') {
                ...
                var confirmMsg = btn.dataset.confirmMsg || 'Are you sure you want to unsubscribe from all marketing communications?';
