...
(function () {
  ...
  function initRoleToggles() {
    document
      .querySelectorAll('.role-toggle-switch input[type="checkbox"]')
      .forEach(function (toggle) {
        toggle.addEventListener('change', function () {
          const card = this.closest('.role-toggle-card');
          const url = this.dataset.url;
          const roleId = this.dataset.roleId;
          const roleName = this.dataset.roleName;
          const isAssigning = this.checked;
            ...
            .then(function (data) {
              if (data.success) {
                ...
                if (data.permissions_html) {
                  const permGrid = document.getElementById('effective-permissions');
  ...
  function initPosSettings() {
    const saveBtn = document.getElementById('pos-settings-save');
    if (!saveBtn) return;
  ...
  function initPosSecurityActions() {
    ...
    const removeCardBtn = document.getElementById('pos-remove-card');
    if (removeCardBtn) {
      removeCardBtn.addEventListener('click', async function () {
        if (
          !(await AdminModal.confirm({
            message: "Remove this staff member's unlock card?",
            danger: true,
            confirmText: 'Remove',
          }))
        )
        ...
        postJSON(url, { remove_card: true })
          .then(function (data) {
            if (data.success) {
              const row = removeCardBtn.closest('.pos-setting-row');
    ...
    const removeBioBtn = document.getElementById('pos-remove-biometric');
    if (removeBioBtn) {
      removeBioBtn.addEventListener('click', async function () {
        if (
          !(await AdminModal.confirm({
            message: 'Remove all biometric credentials for this staff member?',
            danger: true,
            confirmText: 'Remove',
          }))
        )
        ...
        postJSON(url, { remove_biometric: true })
          .then(function (data) {
            if (data.success) {
              const row = removeBioBtn.closest('.pos-setting-row');
              const container = row.querySelector('div');
              container.innerHTML =
                '<span style="color: var(--body-quiet-color);">Not registered</span>' +
                '<span style="font-size: 11px; color: var(--body-quiet-color);">Register at POS terminal</span>';
  ...
  function initSessionRevoke() {
    document.querySelectorAll('.revoke-session-btn').forEach(function (btn) {
      btn.addEventListener('click', async function (e) {
        ...
        if (
          !(await AdminModal.confirm({
            message: 'Revoke this mobile session? The device will be signed out.',
            danger: true,
            confirmText: 'Revoke',
          }))
        )
  ...
  function initFilters() {
    ...
    const filterUrl = '/' + languageCode + '/admin/accounts/staffmember/filter/';
    let debounceTimer;
    ...
    function applyFilters() {
      ...
      const mfa = document.getElementById('filter-mfa').value;
    ...
    function updateActiveFilters() {
      ...
      const label = '<span class="active-filters-label">Active:</span>';
      let tags = '';
      const search = document.getElementById('filter-search').value.trim();
      ...
      if (role.value)
        tags +=
          '<span class="active-filter-tag">Role: ' +
          role.options[role.selectedIndex].text +
          '</span>';
      const access = document.getElementById('filter-access');
      if (access.value)
        tags +=
          '<span class="active-filter-tag">Access: ' +
          access.options[access.selectedIndex].text +
          '</span>';
      ...
      if (statusEl.value)
        tags +=
          ...
          '</span>';
      const mfa = document.getElementById('filter-mfa');
      if (mfa.value)
        tags +=
          '<span class="active-filter-tag">2FA: ' + mfa.options[mfa.selectedIndex].text + '</span>';
      container.innerHTML =
        label +
        (tags || '<span style="color: var(--body-quiet-color); font-size: 12px;">None</span>');
    }
