...
urlpatterns = [
    ...
    path(
        "oauth/wizard/<str:provider_type>/", oauth_views.oauth_provider_wizard, name="oauth_wizard"
    ),
    ...
    path(
        "subscriptions/<uuid:subscription_id>/update-payment/",
        views.subscription_update_payment,
        name="subscription_update_payment",
    ),
    # Payment Token Management
    path("payment-methods/", views.payment_token_list, name="payment_token_list"),
    path(
        "payment-methods/<uuid:token_id>/delete/",
        views.payment_token_delete,
        name="payment_token_delete",
    ),
    path(
        "payment-methods/<uuid:token_id>/set-default/",
        views.payment_token_set_default,
        name="payment_token_set_default",
    ),
    # Address Management
    ...
    path(
        "addresses/<int:address_id>/delete/", views.customer_address_delete, name="address_delete"
    ),
    path(
        "addresses/<int:address_id>/set-default/<str:address_type>/",
        views.customer_address_set_default,
    ...
    path(
        "activate-guest/<str:uidb64>/<str:token>/",
        views.activate_guest_account,
        name="activate_guest_account",
    ),
    # Guest Order Lookup (magic link)
    path("guest-orders/", views.guest_order_lookup, name="guest_order_lookup"),
    path(
        "guest-orders/<str:uidb64>/<str:token>/", views.guest_orders_view, name="guest_orders_view"
    ),
    ...
    path(
        ...
        views.booking_cancel_action,
        name="booking_cancel_action",
    ),
    path(
        "bookings/<int:booking_id>/reschedule/",
        views.booking_reschedule_action,
        name="booking_reschedule_action",
    ),
]
...
if getattr(_settings, "SPWIG_IS_HQ", False):
    ...
    urlpatterns += [
        ...
        path(
            "hosting/update-payment/",
            views_hosting.hosting_update_payment,
            name="hosting_update_payment",
        ),
        ...
        path(
            "hosting/change-interval/",
            views_hosting.hosting_change_interval,
            name="hosting_change_interval",
        ),
    ]
