    ...
    def handle(self, *args, **options):
        ...
        for provider_data in providers:
            provider_obj, created = OAuthProviderSettings.objects.get_or_create(
                provider=provider_data["provider"], defaults=provider_data
            )
        ...
        site, site_created = Site.objects.get_or_create(
            pk=1, defaults={"domain": "example.com", "name": "My Shop"}
        )

        if site_created:
            self.stdout.write(self.style.SUCCESS(f"✓ Created default site: {site.domain}"))
            self.stdout.write(
                self.style.WARNING(
        ...
        self.stdout.write("\n")
        self.stdout.write(self.style.SUCCESS("✓ OAuth provider initialization complete!"))
        self.stdout.write(self.style.SUCCESS(f"  - Created: {created_count} providers"))
        self.stdout.write(self.style.SUCCESS(f"  - Existing: {updated_count} providers"))
        self.stdout.write("\n")
        self.stdout.write(self.style.WARNING("Next steps:"))
        self.stdout.write("  1. Configure OAuth credentials in Django admin → Social Applications")
        self.stdout.write("  2. Enable providers in Django admin → OAuth Provider Settings")
        self.stdout.write("  3. Update site domain in Django admin → Sites")
