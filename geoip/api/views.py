...
from ..models import CountryMapping, BusinessRule, VisitorLocation
...
@extend_schema(
    ...
    description=_("Automatically detect user's geographic location from their IP address. Returns country, region, city, coordinates, currency, language, and timezone. Cached for performance."),
    responses={
...
def resolve_location(request):
    ...
    if country_code:
        ...
        try:
            mapping = CountryMapping.objects.get(
...
@extend_schema(
    ...
    description=_("Override automatic location detection with user's preferred currency, language, and country. Stored in session and persists across visits."),
    request=inline_serializer(
        name='LocationPreferenceRequest',
        fields={
            'currency': serializers.CharField(required=False, help_text="Preferred currency code (e.g., EUR)"),
            'language': serializers.CharField(required=False, help_text="Preferred language code (e.g., fr)"),
            'country': serializers.CharField(required=False, help_text="Preferred country code (e.g., FR)"),
        }
    ),
    responses={
...
@extend_schema(
    ...
    description=_("Get recommended currency for a specific country or the user's detected location. Returns default currency, accepted alternatives, and currency symbol."),
    parameters=[
        OpenApiParameter(
            name="country",
            type=str,
            location=OpenApiParameter.QUERY,
            description=_("Two-letter country code (e.g., 'US', 'GB'). Uses detected location if not provided."),
            required=False
        ),
    ],
    responses={
        200: inline_serializer(
            ...
            fields={
                ...
                'accepted': serializers.ListField(
                    ...
                    help_text="List of accepted currency codes"
                ),
...
@extend_schema(
    ...
    description=_("Get recommended language for a specific country or the user's detected location. Returns default language and list of supported languages for that region."),
    parameters=[
        OpenApiParameter(
            name="country",
            type=str,
            location=OpenApiParameter.QUERY,
            description=_("Two-letter country code (e.g., 'US', 'CA'). Uses detected location if not provided."),
            required=False
        ),
    ],
    responses={
        200: inline_serializer(
            ...
            fields={
                ...
                'supported': serializers.ListField(
                    child=serializers.CharField(),
                    help_text="List of supported language codes"
                ),
            }
        )
    }
...
@extend_schema(
    ...
    description=_("Get list of all supported countries with their default currency, language, flag emoji, and EU membership status. Used for country selector components."),
    responses={
        200: inline_serializer(
...
@extend_schema(
    ...
    description=_("Report incorrect location detection to help improve accuracy. Users can submit their actual location when automatic detection is wrong. Logged for analysis and model improvement."),
    request=inline_serializer(
        name='LocationCorrectionRequest',
        fields={
            'actual_country': serializers.CharField(required=False, help_text="2-letter country code"),
...
def report_correction(request):
    ...
    try:
        ...
        logger.info(f"Location correction reported: {visitor.resolved_country} -> {visitor.actual_country}")
