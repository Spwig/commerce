        ...
        if dry_run:
            ...
            if expired_count > 0:
                ...
                for entry in expired_qs[:5]:
                    self.stdout.write(f"  - {entry.ip_address} ({entry.country_code}) expired {entry.expires_at}")

            if old_count > 0 and days > 0:
                self.stdout.write(f"\nSample of old entries to be deleted (>{days} days):")
                for entry in old_qs[:5]:
                    self.stdout.write(f"  - {entry.ip_address} ({entry.country_code}) from {entry.resolved_at}")
        else:
