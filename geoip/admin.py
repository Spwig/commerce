...
class GeoLocationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("IP Information"), {"fields": ("ip_address", "ip_prefix")}),
        (
            _("Location"),
            {
                "fields": (
        ...
        (_("Network"), {"fields": ("asn", "isp")}),
        (
            _("Metadata"),
            {"fields": ("source", "confidence", "is_proxy", "is_vpn", "is_tor", "is_mobile")},
        ),
        (_("Timestamps"), {"fields": ("resolved_at", "updated_at", "expires_at")}),
    )
    ...
    def confidence_display(self, obj):
        ...
        return format_html('<span class="{}">{}%</span>', css_class, int(confidence))
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Top countries
        top_countries = (
            GeoLocation.objects.values("country_code", "country_name")
        ...
        # Source distribution
        source_stats = (
            GeoLocation.objects.values("source")
...
class CountryMappingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Country Information"), {"fields": ("country_code", "country_name", "is_active")}),
        (
            _("Currency & Language"),
            {
                "fields": (
        ...
        (_("Regional Settings"), {"fields": ("timezone", "date_format", "uses_metric")}),
        (
            _("Tax & Shipping"),
            {"fields": ("tax_rate", "is_eu_member", "requires_vat", "shipping_zone")},
        ),
        (_("Payment Options"), {"fields": ("supports_cod", "blocked_payment_methods")}),
        (_("Custom Rules"), {"fields": ("custom_rules",), "classes": ("collapse",)}),
    )
...
class GeoIPProviderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Provider Information"), {"fields": ("name", "provider_type", "is_active", "priority")}),
        (
            _("Configuration"),
            {"fields": ("config", "database_path", "database_url", "license_key")},
        ),
        (_("Database Updates"), {"fields": ("last_update", "next_update", "database_version")}),
        (
            _("Statistics"),
            {
                "fields": (
    ...
    def accuracy_display(self, obj):
        ...
        if rate >= 95:
            ...
            css_class = "geoip-confidence-low"
        return format_html('<span class="{}">{}%</span>', css_class, f"{rate:.1f}")
    ...
    def update_database(self, request, queryset):
        ...
        for provider in queryset:
            if not provider.database_url:
                errors.append(
                    _("%(name)s has no database URL configured") % {"name": provider.name}
                )
                continue

            try:
                response = http_requests.head(
                    provider.database_url, timeout=10, allow_redirects=True
                )
                if response.status_code == 200:
                    ...
                    errors.append(
                        _("%(name)s: HTTP %(status)s from database URL")
                        % {
                            "name": provider.name,
        ...
        if updated:
            self.message_user(
                request, _("Database update checked for %(count)d provider(s)") % {"count": updated}
            )
        for error in errors:
        ...
        for provider in queryset:
            ...
            try:
                ...
                if location:
                    ...
                    results.append(
                        _("%(name)s: No data returned for %(ip)s")
                        % {
                            "name": provider.name,
                            "ip": test_ip,
                        }
                ...
                results.append(
                    _(
                        "%(name)s: Provider configured (service module not available for direct test)"
                    )
                    % {
                        "name": provider.name,
                    }
                ...
                results.append(
                    _("%(name)s: Error — %(error)s") % {"name": provider.name, "error": str(e)}
                )
...
class VisitorLocationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("User Corrections"), {"fields": ("actual_country", "actual_region", "actual_city")}),
        (_("User Preferences"), {"fields": ("selected_currency", "selected_language")}),
        (
        ...
        (_("Timestamps"), {"fields": ("first_seen", "last_seen")}),
    )

    def country_display(self, obj):
        if obj.actual_country:
            return format_html(
                "{} → <strong>{}</strong>", obj.resolved_country or "?", obj.actual_country
            )
    ...
    def city_display(self, obj):
        if obj.actual_city:
            return format_html(
                "{} → <strong>{}</strong>", obj.resolved_city or "?", obj.actual_city
            )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        corrected_count = (
            VisitorLocation.objects.filter(actual_country__isnull=False)
...
class BusinessRuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Rule Information"), {"fields": ("name", "description", "is_active", "priority")}),
        (
            _("Conditions"),
            {
                "fields": ("conditions",),
        ...
        (
            _("Statistics"),
            {"fields": ("times_triggered", "last_triggered"), "classes": ("collapse",)},
        ),
    ...
    def times_triggered_display(self, obj):
        if obj.times_triggered > 1000:
            return format_html("<strong>{}</strong>", f"{obj.times_triggered:,}")
    ...
    def test_rule(self, request, queryset):
        ...
        for rule in queryset:
            ...
            if matched:
                actions_summary = ", ".join(f"{k}={v}" for k, v in rule.actions.items()) or _(
                    "no actions"
                )
                self.message_user(
                    request,
