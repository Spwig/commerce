...
(function initCountryMappingFilters() {
  if (
    ...
    document.getElementById('geolocation-results') ||
    document.getElementById('visitor-results')
  )
    return;
  ...
  function updateActiveFilters() {
    ...
    if (search)
      filters.push({ field: 'filter-search', label: msg.label_search || 'Search', value: search });
    if (active.value)
      filters.push({
        field: 'filter-active',
        label: msg.label_active_f || 'Active',
        value: active.options[active.selectedIndex].text,
      });
    if (eu.value)
      filters.push({
        field: 'filter-eu',
        label: msg.label_eu || 'EU',
        value: eu.options[eu.selectedIndex].text,
      });
    if (currency)
      filters.push({
        field: 'filter-currency',
        label: msg.label_currency || 'Currency',
    ...
    filters.forEach(function (f) {
      html +=
        ...
        ': ' +
        f.value +
        ' <i class="fas fa-times" data-clear-field="' +
        f.field +
        '"></i></span>';
    });
...
(function initGeoIPActions() {
  ...
  function getProviderUrls() {
    const el = document.getElementById('geoip-provider-config');
    if (!el) return null;
    try {
  ...
  function showCardFeedback(btn, success, message) {
    const card = btn.closest('.list-row-card');
    if (!card) return;
    // Remove any existing feedback
    const existing = card.querySelector('.geoip-card-feedback');
    if (existing) existing.remove();

    const div = document.createElement('div');
    div.className = 'geoip-card-feedback';
    div.style.cssText =
      'padding:8px 16px;margin-top:8px;border-radius:6px;font-size:13px;' +
      (success
  ...
  function handleTestProvider(btn) {
    ...
    const url = urls.testUrl.replace('PLACEHOLDER', providerType);
  ...
  function handleUpdateDatabase(btn) {
    ...
    const url = urls.updateUrl.replace('0', providerId);
    ...
    fetch(url, {
      ...
      .then(function (data) {
        ...
        if (data.success && data.last_update) {
          const card = btn.closest('.list-row-card');
          if (card) {
            const syncIcon = card.querySelector('.fa-sync');
  ...
  document.addEventListener('click', function (e) {
    ...
    if (action === 'clear-expired-cache') {
      const msg = document.getElementById('geoip-translations')
        ? JSON.parse(document.getElementById('geoip-translations').textContent)
        : {};
      const statusEl = btn.closest('.quick-actions');
      if (statusEl) {
...
(function initProviderWizard() {
  ...
  function updateSummary() {
    ...
    for (const [key, value] of formData.entries()) {
      if (key !== 'csrfmiddlewaretoken' && key !== 'step') {
        const existing = hiddenFields.querySelector('input[name="' + key + '"]');
        if (existing) {
  ...
  function runFinalTest() {
    ...
    resultDiv.innerHTML =
      '<div class="loading"><i class="fas fa-spinner fa-spin"></i> ' +
      (msg.running_final_test || 'Running final test...') +
      '</div>';
    ...
    setTimeout(function () {
      resultDiv.innerHTML =
        ...
        '<i class="fas fa-check-circle"></i>' +
        '<strong>' +
        (msg.all_tests_passed || 'All tests passed!') +
        '</strong>' +
        '<p>' +
        (msg.provider_configured || 'Your provider is configured correctly and ready to use.') +
        '</p>' +
        '</div>';
    }, 1500);
  }
