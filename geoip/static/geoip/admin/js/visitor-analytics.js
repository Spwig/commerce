...
(function() {
    ...
    var configEl = document.getElementById('analytics-config');
    ...
    var primary = getCSSVar('--primary', '#417690');
    var success = getCSSVar('--success-fg', '#28a745');
    var warning = getCSSVar('--warning-fg', '#ffc107');
    var danger = getCSSVar('--error-fg', '#dc3545');
    var muted = getCSSVar('--body-quiet-color', '#999');
    ...
    function initCharts() {
        ...
        var nvr = config.new_vs_returning || {};
    ...
    function loadData(period) {
        var url = config.data_url + '?period=' + encodeURIComponent(period);
        ...
        .then(function(data) {
            ...
            // Update tables
            if (data.top_pages) updateTable('top-pages-table', data.top_pages, ['url_path', 'views', 'unique_visitors', 'entries']);
            if (data.campaigns) updateTable('campaigns-table', data.campaigns, ['utm_source', 'utm_medium', 'utm_campaign', 'visitors']);
            if (data.geographic) updateTable('geo-table', data.geographic, ['resolved_country', 'visitors', 'page_views']);
            if (data.referrers) updateTable('referrers-table', data.referrers, ['referrer', 'count']);
            if (data.landing_pages) updateTable('landing-pages-table', data.landing_pages, ['url_path', 'entries', 'unique_visitors']);
        })
        .catch(function(err) {
    ...
    function updateTable(tableId, rows, columns) {
        ...
        if (!rows.length) {
            tbody.innerHTML = '<tr><td colspan="' + columns.length + '" class="va-empty">No data for this period</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = rows.map(function(row) {
            return '<tr>' + columns.map(function(col) {
                ...
                const cls = (col === 'url_path' || col === 'referrer') ? ' class="font-monospace va-url-cell"' : '';
    ...
    function bindPeriodSelector() {
        document.querySelectorAll('.va-period-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                document.querySelectorAll('.va-period-btn').forEach(function(b) { b.classList.remove('active'); });
