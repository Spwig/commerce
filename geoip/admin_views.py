...
def filter_geolocations(request):
    ...
    if expired == "true":
        ...
        queryset = queryset.filter(Q(expires_at__isnull=True) | Q(expires_at__gte=timezone.now()))
    ...
    html = render_to_string(
        "admin/geoip/partials/geolocation_cards.html", {"locations": locations}, request=request
    )
...
def filter_visitor_locations(request):
    ...
    if corrected == "true":
        queryset = (
            queryset.filter(actual_country__isnull=False)
    ...
    html = render_to_string(
        "admin/geoip/partials/visitorlocation_cards.html", {"visitors": visitors}, request=request
    )
