...
def provider_wizard(request, provider_type):
    ...
    if request.method == "POST":
        ...
        if step == "4":  # Final step - save configuration
            try:
                provider = existing_provider or GeoIPProvider()
                ...
                messages.success(
                    request,
                    _("%(name)s has been configured successfully!")
                        ...
                        try:
                            ...
                            if _inst.initialize():
                                ...
                                if _result:
                                    messages.success(
                                        request,
                                        _("Test passed — resolved 8.8.8.8 in %(ms).0f ms")
                                        % {"ms": _elapsed},
                                    )
                                else:
                                    messages.warning(
                                        request, _("Test returned no data for 8.8.8.8")
                                    )
                                _inst.close()
                            else:
                                messages.warning(
                                    request, _("Provider failed to initialize during test")
                                )
                        except Exception as _e:
                            messages.warning(
                                request, _("Test failed: %(error)s") % {"error": str(_e)}
                            )
                ...
                messages.error(
                    request, _("Error saving configuration: %(error)s") % {"error": str(e)}
                )
    ...
    try:
        ...
        if provider_path:
            ...
            result = {
                "success": True,
                "message": _(
                    "%(name)s is configured. This provider type will be tested on first real request."
                )
                % {"name": provider.name},
                "data": {
...
def toggle_provider(request, provider_id):
    ...
    try:
        ...
        messages.success(
            request, _("%(name)s has been %(status)s") % {"name": provider.name, "status": status}
        )
    ...
    try:
        ...
        if provider.provider_type not in DATABASE_PROVIDER_TYPES:
            return JsonResponse(
                {
                    "success": False,
                    "message": _("%(name)s does not use a local database")
                    % {"name": provider.name},
                }
            )
        ...
        if not provider.database_url:
            return JsonResponse(
                {
                    "success": False,
                    "message": _("No database URL configured for %(name)s")
                    % {"name": provider.name},
                }
            )
