...
from .models import BusinessRule, GeoLocation
...
class GeoIPMiddleware:
    ...
    def _initialize_providers(self):
        ...
        provider_classes = geoip_config.get(
            "PROVIDERS",
            [
    ...
    def __call__(self, request):
        ...
        request.geo_location = SimpleLazyObject(lambda: self._resolve_location(request))
            ...
            try:
                ...
                if provider.is_available():
                    ...
                    if location_data:
                        ...
                        if (
                            location_data.get("source") == "private_network"
                            or location_data.get("data_source") == "private_network"
                        ):
                            is_private_ip = True
                            logger.info(f"Private/internal IP detected: {ip}")

                        logger.debug(
                            f"Provider {provider.name} resolved IP {ip} in {lookup_time:.2f}ms"
                        )
                        break
            except Exception as e:
    ...
    def _get_user_override(self, request) -> dict[str, Any] | None:
        """
        ...
        if hasattr(request, "COOKIES"):
            ...
            if country:
                return {"country_code": country, "source": "user_preference", "confidence": 1.0}

        return None

    def _get_db_cache(self, ip: str) -> dict[str, Any] | None:
        """
    ...
    def _get_default_location(self, is_internal_ip: bool = False) -> dict[str, Any]:
        ...
        return {"country_code": default_country, "source": "default", "confidence": 0.0}
    ...
    def _is_ajax_request(self, request):
        ...
        return bool("application/json" in accept and "text/html" not in accept)
