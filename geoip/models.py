...
class GeoLocation(models.Model):
    ...
    ip_prefix = models.CharField(
        max_length=50,
        db_index=True,
        help_text="IP prefix for cache grouping (/24 for IPv4, /48 for IPv6)",
    )
    ...
    source = models.CharField(
        max_length=50, default="unknown", help_text="Data source (e.g., maxmind, dbip, ip2location)"
    )
...
class CountryMapping(models.Model):
    ...
    default_language = models.CharField(
        max_length=10, default="en", help_text="Default language code (e.g., en, es, fr, zh-hans)"
    )
    ...
    tax_rate = models.DecimalField(
        max_digits=5, decimal_places=2, default=0.00, help_text="Default tax rate %"
    )
    ...
    shipping_zone = models.CharField(
        max_length=50, blank=True, help_text="Shipping zone identifier"
    )

    # Feature flags
    supports_cod = models.BooleanField(default=False, help_text="Supports Cash on Delivery")
    blocked_payment_methods = ArrayField(models.CharField(max_length=50), default=list, blank=True)

    # Custom rules (JSON)
    custom_rules = models.JSONField(
        default=dict, blank=True, help_text="Custom business rules for this country"
    )
...
class VisitorLocation(models.Model):
    ...
    referrer_url = models.URLField(
        max_length=2048, blank=True, help_text="Referrer URL from HTTP header"
    )
    utm_source = models.CharField(
        max_length=255,
        blank=True,
        db_index=True,
        help_text="Campaign source (e.g., google, newsletter)",
    )
    utm_medium = models.CharField(
        max_length=255,
        blank=True,
        db_index=True,
        help_text="Campaign medium (e.g., cpc, email, social)",
    )
    utm_campaign = models.CharField(
        max_length=255, blank=True, db_index=True, help_text="Campaign name (e.g., summer_sale)"
    )
    utm_term = models.CharField(max_length=255, blank=True, help_text="Paid keywords")
    utm_content = models.CharField(
        max_length=255, blank=True, help_text="Content variation (e.g., A/B test)"
    )
    ...
    timezone_offset = models.IntegerField(
        null=True, blank=True, help_text="Minutes offset from UTC"
    )

    # Bot / automation detection
    is_bot = models.BooleanField(
        default=False, db_index=True, help_text="Detected as bot, crawler, or headless browser"
    )
    is_admin_traffic = models.BooleanField(
        default=False, db_index=True, help_text="Traffic originated from admin/staff pages"
    )
...
class BusinessRule(models.Model):
    ...
    def evaluate(self, location_data):
        ...
        for key, value in self.conditions.items():
            if (
                key == "country_in"
                and location_data.get("country") not in value
                or key == "country_not_in"
                and location_data.get("country") in value
                or key == "region_in"
                and location_data.get("region") not in value
                or key == "region_not_in"
                and location_data.get("region") in value
                or key == "is_mobile"
                and location_data.get("is_mobile") != value
                or key == "is_vpn"
                and location_data.get("is_vpn") != value
            ):
                return False
