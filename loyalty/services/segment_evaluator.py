...
class SegmentEvaluator:
    ...
    def evaluate_member_for_segment(self, member: LoyaltyMember, segment: LoyaltySegment) -> bool:
        ...
        try:
            ...
            if "min_orders" in rules or "max_orders" in rules:
                order_count = Order.objects.filter(user=member.customer, status="completed").count()

                if "min_orders" in rules and order_count < rules["min_orders"]:
                    return False

                if "max_orders" in rules and order_count > rules["max_orders"]:
                    return False

            # Total spend rules
            if "min_spend" in rules or "max_spend" in rules:
                total_spend = Order.objects.filter(
                    user=member.customer, status="completed"
            ...
            if "last_order_days" in rules:
                last_order = (
                    Order.objects.filter(user=member.customer, status="completed")
                    .order_by("-created_at")
                    .first()
                )
            ...
            if "has_redeemed" in rules:
                has_redemptions = LoyaltyRedemption.objects.filter(member=member).exists()

                if (
                    rules["has_redeemed"]
                    and not has_redemptions
                    or not rules["has_redeemed"]
                    and has_redemptions
                ):
                    return False

            if "min_redemptions" in rules or "max_redemptions" in rules:
                redemption_count = LoyaltyRedemption.objects.filter(member=member).count()
            ...
            if "email_verified" in rules:
                ...
                email_verified = (
                    member.customer.emailaddress_set.filter(verified=True).exists()
                    if hasattr(member.customer, "emailaddress_set")
                    else True
                )
    ...
    def get_segment_members(self, segment: LoyaltySegment) -> list[LoyaltyMember]:
        ...
        if segment.criteria_type == "manual":
            ...
            return list(
                LoyaltyMember.objects.filter(
                    segment_memberships__segment=segment, is_active=True
                ).distinct()
            )
        ...
        self.logger.debug(
            f"Segment {segment.id} ({segment.name}): {len(qualifying_members)} qualifying members"
        )
    ...
    def refresh_segment_memberships(self, segment: LoyaltySegment) -> dict[str, int]:
        ...
        for member_id in to_add:
            LoyaltySegmentMembership.objects.create(segment=segment, member_id=member_id)
            added_count += 1
        ...
        segment.member_count = LoyaltySegmentMembership.objects.filter(segment=segment).count()
        ...
        return {"added": added_count, "removed": removed_count, "total": segment.member_count}
    ...
    def refresh_all_segments(self) -> dict[str, Any]:
        ...
        """
        active_segments = LoyaltySegment.objects.filter(is_active=True).exclude(
        ...
        self.logger.info(
            f"Refreshed {segments_processed} segments: +{total_added}, -{total_removed}"
        )
    ...
    def add_member_to_segment(
        self, member: LoyaltyMember, segment: LoyaltySegment, automatically: bool = False
    ) -> LoyaltySegmentMembership | None:
        """
        ...
        if created:
            ...
            self.logger.info(
                f"Added member {member.id} to segment {segment.id} (auto={automatically})"
            )

        return membership if created else None

    def remove_member_from_segment(self, member: LoyaltyMember, segment: LoyaltySegment) -> bool:
        """
