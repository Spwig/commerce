...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_(
            "Get the authenticated customer's loyalty program status including points balance and tier information."
        ),
        responses={
...
class LoyaltyStatusViewSet(HeadlessAPIMixin, viewsets.ViewSet):
    ...
    def progress(self, request):
        ...
        order_stats = Order.objects.filter(
            user=request.user, status__in=["completed", "delivered"]
        ).aggregate(total_spend=Sum("total"), order_count=Count("id"))
        ...
        if next_tier:
            ...
            if next_tier.min_spend > 0:
                progress_data["spend_progress_percent"] = min(
                    100, int((current_spend / next_tier.min_spend) * 100)
                )
                progress_data["spend_remaining"] = max(
                    Decimal("0.00"), next_tier.min_spend - current_spend
                )

            if next_tier.min_orders > 0:
                progress_data["orders_progress_percent"] = min(
                    100, int((current_orders / next_tier.min_orders) * 100)
                )
                progress_data["orders_remaining"] = max(0, next_tier.min_orders - current_orders)

            if next_tier.min_points_earned > 0:
                progress_data["points_progress_percent"] = min(
                    100, int((current_points_earned / next_tier.min_points_earned) * 100)
                )
                progress_data["points_remaining"] = max(
                    0, next_tier.min_points_earned - current_points_earned
                )
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                name="offset", description=_("Offset for pagination"), required=False, type=int
            ),
        ],
        responses={
...
class LoyaltyHistoryViewSet(HeadlessAPIMixin, viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        # Get transactions
        transactions = LoyaltyTransaction.objects.filter(member=member).order_by("-created_at")
...
class LoyaltyRewardViewSet(HeadlessAPIMixin, viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        ...
        description=_(
            "Redeem a reward using loyalty points. Returns the redemption details including any voucher codes generated."
        ),
        responses={
    ...
    def redeem(self, request, uuid=None):
        ...
        try:
            ...
            return Response(
                {"success": False, "message": _("Reward not found"), "redemption": None},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if not can_redeem:
            return Response(
                {"success": False, "message": message, "redemption": None},
    ...
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="status",
                description=_(
                    "Filter by status (pending, confirmed, fulfilled, cancelled, expired)"
                ),
                required=False,
                type=str,
            ),
...
class LoyaltyRedemptionViewSet(HeadlessAPIMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        try:
            # Trigger LoyaltyMember.DoesNotExist if the user isn't enrolled.
            _ = request.user.loyalty_member
        except LoyaltyMember.DoesNotExist:
            return Response(
                {
                    "success": False,
...
class LoyaltyEarningRulesViewSet(HeadlessAPIMixin, viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        # Get active rules
        active_rules = (
            LoyaltyRule.objects.filter(is_active=True)
