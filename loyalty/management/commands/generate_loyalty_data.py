...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--campaigns", type=int, default=5, help="Number of campaigns to create (default: 5)"
        )
        parser.add_argument(
            "--clear", action="store_true", help="Clear existing loyalty data before generating"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"✓ Generated {transaction_count} transactions from orders")
        )
    ...
    def create_tiers(self):
        ...
        for tier_data in tiers_data:
            tier, _ = LoyaltyTier.objects.get_or_create(slug=tier_data["slug"], defaults=tier_data)
            tiers.append(tier)
    ...
    def create_segments(self):
        ...
        for segment_data in segments_data:
            segment, _ = LoyaltySegment.objects.get_or_create(
                slug=segment_data["slug"], defaults=segment_data
            )
    ...
    def create_rules(self):
        ...
        for rule_data in rules_data:
            rule, _ = LoyaltyRule.objects.get_or_create(name=rule_data["name"], defaults=rule_data)
            rules.append(rule)
    ...
    def create_rewards(self):
        ...
        for reward_data in rewards_data:
            reward, _ = LoyaltyReward.objects.get_or_create(
                slug=reward_data["slug"], defaults=reward_data
            )
    ...
    def generate_transactions_from_orders(self, members, rules):
        ...
        purchase_rule = next(
            (r for r in rules if r.rule_type == LoyaltyRule.TYPE_SPEND_BASED), None
        )
    ...
    def assign_segments(self, members, segments):
        ...
        for member in members:
            ...
            if (
                ...
                and balance.last_earned_at < sixty_days_ago
                and balance.lifetime_earned > 0
            ):
    ...
    def create_campaigns(self, count, segments):
        ...
        campaign_templates = [
            {
                ...
                "journey_steps": [
                    ...
                    },
                    {
                        ...
                        "name": "Signup Bonus",
                        "actions": [
                            {"type": "award_points", "points": 100, "reason": "Welcome bonus"}
                        ],
                        "delay_days": 0,
                    },
                    {
            ...
            },
            {
                ...
                "actions": [{"type": "send_email", "template": "double_points_promo"}],
                "status": LoyaltyCampaign.STATUS_ACTIVE,
                "is_active": True,
            },
            {
        ...
        for _i, template in enumerate(campaign_templates[:count]):
        ...
        for campaign in campaigns:
            ...
            for member in selected_members:
                ...
                execution_data = {
                    ...
                    "emails_sent": ["loyalty@example.com"]
                    if status == LoyaltyCampaignExecution.STATUS_COMPLETED
                    else [],
                    "rewards_issued": [],
                    "steps_completed": [1]
                    if campaign.is_journey and status == LoyaltyCampaignExecution.STATUS_COMPLETED
                    else [],
                }
    ...
    def generate_redemptions(self, members, rewards):
        ...
        for member in selected_members:
            ...
            for _ in range(num_redemptions):
                ...
                affordable_rewards = [
                    r for r in rewards if r.points_cost <= member.balance.available_points
                ]
    ...
    def print_summary(self):
        ...
        total_available = (
            LoyaltyBalance.objects.aggregate(total=Sum("available_points"))["total"] or 0
        )
        total_earned = LoyaltyBalance.objects.aggregate(total=Sum("lifetime_earned"))["total"] or 0
        total_redeemed = (
            LoyaltyBalance.objects.aggregate(total=Sum("lifetime_redeemed"))["total"] or 0
        )
