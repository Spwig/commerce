...
class LoyaltyMemberAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            _("Member Information"),
            {
                ...
                "description": _(
                    "Basic member information. Points balance displayed in dashboard cards above."
                ),
            },
        ),
        (_("Statistics"), {"fields": ("member_stats",), "classes": ("collapse",)}),
        (_("Timestamps"), {"fields": ("enrolled_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def member_id_display(self, obj):
        ...
        return format_html('<code style="font-size: 11px;">{}</code>', obj.id)
    ...
    def customer_name(self, obj):
        ...
        return format_html('<a href="{}"><i class="fas fa-user"></i> {}</a>', url, name)
            ...
            if points != 0 and reason:
                try:
                    ...
                    ledger.manual_adjustment(
                        member=member, points=points, reason=reason, admin_user=request.user
                    )

                    messages.success(
                        request,
                        _("Successfully adjusted points by {} for {}").format(
                            points, member.customer.get_full_name()
                        ),
                    )
                    return HttpResponseRedirect(
                        reverse("admin:loyalty_loyaltymember_change", args=[member_id])
                    )
    ...
    def activate_members(self, request, queryset):
        ...
        self.message_user(request, _("{} members activated").format(updated), messages.SUCCESS)
    ...
    def deactivate_members(self, request, queryset):
        ...
        self.message_user(request, _("{} members deactivated").format(updated), messages.SUCCESS)
        ...
        for member in queryset.select_related("customer", "current_tier", "balance").order_by("id"):
            ...
            writer.writerow(
                [
                    ...
                    balance.last_earned_at.strftime("%Y-%m-%d %H:%M:%S")
                    if balance and balance.last_earned_at
                    else "",
                    balance.last_redeemed_at.strftime("%Y-%m-%d %H:%M:%S")
                    if balance and balance.last_redeemed_at
                    else "",
                ]
            )

        self.message_user(
            request, _("{} members exported to CSV").format(queryset.count()), messages.SUCCESS
        )
        return response
...
class LoyaltyTransactionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            ...
            {"fields": ("expires_at", "created_at"), "classes": ("collapse",)},
        ),
    )
    ...
    def points_display(self, obj):
        ...
        if obj.points > 0:
            return format_html(
                '<strong style="color: var(--success-fg, #10b981);">+{}</strong>', obj.points
            )
        else:
            return format_html(
                '<strong style="color: var(--error-fg, #ef4444);">{}</strong>', obj.points
            )
...
class LoyaltyRuleAdmin(admin.ModelAdmin):
    ...
    def activate_rules(self, request, queryset):
        ...
        updated = queryset.update(is_active=True)
        self.message_user(request, _("{} rules activated").format(updated), messages.SUCCESS)
    ...
    def deactivate_rules(self, request, queryset):
        ...
        updated = queryset.update(is_active=False)
        self.message_user(request, _("{} rules deactivated").format(updated), messages.SUCCESS)
...
class LoyaltyTierAdmin(admin.ModelAdmin):
    ...
    def members_count(self, obj):
        ...
        return format_html(
            '<a href="{}?current_tier__id__exact={}"><i class="fas fa-users"></i> {}</a>',
...
class LoyaltyBadgeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Appearance"),
            {
                "fields": ("image", "icon"),
                "description": _(
                    "Select an image from the media library or choose a Font Awesome icon. Image takes priority if both are provided."
                ),
            },
        ),
        (
    ...
    def badge_icon_display(self, obj):
        ...
        if obj.image and obj.image.file:
            ...
            return format_html(
                '<i class="fas {} fa-2x" style="color: var(--primary);"></i>', obj.icon
            )
        return "—"
...
class LoyaltyBalanceAdmin(admin.ModelAdmin):
    ...
    def available_display(self, obj):
        """Display available points"""
        return format_html(
            '<strong style="color: var(--primary);">{}</strong>', obj.available_points
        )
    ...
    def pending_display(self, obj):
        ...
        return format_html(
            '<span style="color: var(--warning-fg, #f59e0b);">{}</span>', obj.pending_points
        )
    ...
    def lifetime_earned_display(self, obj):
        ...
        return format_html(
            '<span style="color: var(--success-fg, #10b981);">{}</span>', obj.lifetime_earned
        )
    ...
    def lifetime_redeemed_display(self, obj):
        ...
        return format_html(
            '<span style="color: var(--error-fg, #ef4444);">{}</span>', obj.lifetime_redeemed
        )
...
class LoyaltyRewardAdmin(admin.ModelAdmin):
    ...
    formfield_overrides = {
        models.ImageField: {
            "widget": MediaLibrarySelectWidget(attrs={"folder": "loyalty/rewards"})
        },
    }
    ...
    fieldsets = (
        ...
        (_("Expiration"), {"fields": ("redemption_expires_days",), "classes": ("collapse",)}),
        (_("Terms & Conditions"), {"fields": ("terms",), "classes": ("collapse",)}),
        (_("Preview"), {"fields": ("reward_preview",), "classes": ("collapse",)}),
        (
            _("Timestamps"),
    ...
    def availability_status(self, obj):
        ...
        if not obj.is_active:
            return format_html(
                '<span style="color: var(--body-quiet-color);">{}</span>', _("Inactive")
            )

        if obj.is_available():
            return format_html(
                '<span style="color: var(--success-fg, #10b981);">✓ {}</span>', _("Available")
            )
        else:
            return format_html(
                '<span style="color: var(--error-fg, #ef4444);">✗ {}</span>', _("Unavailable")
            )
    ...
    def quantity_display(self, obj):
        ...
        if obj.quantity_remaining == 0:
            ...
            return format_html("{} / {}", obj.quantity_remaining, obj.quantity_total)
    ...
    def reward_preview(self, obj):
        ...
        if obj.image:
            ...
            image_html = format_html(
                '<i class="fas {} fa-5x" style="color: var(--primary);"></i>', obj.icon
            )
    ...
    def activate_rewards(self, request, queryset):
        ...
        updated = queryset.update(is_active=True)
        self.message_user(request, _("{} rewards activated").format(updated), messages.SUCCESS)
    ...
    def deactivate_rewards(self, request, queryset):
        ...
        updated = queryset.update(is_active=False)
        self.message_user(request, _("{} rewards deactivated").format(updated), messages.SUCCESS)
    ...
    def feature_rewards(self, request, queryset):
        ...
        self.message_user(
            request, _("{} rewards marked as featured").format(updated), messages.SUCCESS
        )
    ...
    def unfeature_rewards(self, request, queryset):
        ...
        self.message_user(
            request, _("{} rewards unmarked as featured").format(updated), messages.SUCCESS
        )
...
class LoyaltyRedemptionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Linked Records"),
            {"fields": ("transaction", "order", "voucher_code"), "classes": ("collapse",)},
        ),
        (
            _("Expiration"),
            {
    ...
    def expires_display(self, obj):
        ...
        if obj.is_expired():
            return format_html(
                '<span style="color: var(--error-fg, #ef4444);">✗ {}</span>', _("Expired")
            )
        else:
            return format_html(
    ...
    def mark_fulfilled(self, request, queryset):
        ...
        self.message_user(
            request, _("{} redemptions marked as fulfilled").format(updated), messages.SUCCESS
        )
    ...
    def cancel_redemptions(self, request, queryset):
        ...
        self.message_user(request, _("{} redemptions cancelled").format(updated), messages.SUCCESS)
...
class LoyaltyCampaignAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {"all": ("loyalty/css/campaign-builder.css",)}
    ...
    fieldsets = [
        (
            _("Basic Information"),
            {"fields": ["uuid", "name", "slug", "description", "status", "is_active"]},
        ),
        (
            _("Campaign Configuration"),
            {"fields": ["campaign_type", "trigger_event", "trigger_conditions", "actions"]},
        ),
        (
            _("Journey Configuration"),
            {"fields": ["is_journey", "journey_steps"], "classes": ["collapse"]},
        ),
        (
            _("Scheduling"),
            {
                "fields": ["schedule_type", "schedule_config", "start_date", "end_date"],
                "classes": ["collapse"],
            },
        ),
        (_("Targeting"), {"fields": ["target_all_members", "target_segment", "target_tiers"]}),
        (_("Limits & Controls"), {"fields": ["max_triggers_per_member", "cooldown_days"]}),
        (
            _("A/B Testing"),
            {
                "fields": ["is_ab_test", "ab_variant", "ab_split_percentage"],
                "classes": ["collapse"],
            },
        ),
        (
            _("Statistics"),
            {"fields": ["total_triggered", "total_completed", "total_failed", "last_triggered_at"]},
        ),
    ...
    def activate_campaigns(self, request, queryset):
        ...
        self.message_user(
            request, _("{} campaigns activated").format(queryset.count()), messages.SUCCESS
        )
    ...
    def pause_campaigns(self, request, queryset):
        ...
        self.message_user(
            request, _("{} campaigns paused").format(queryset.count()), messages.SUCCESS
        )
...
class LoyaltyCampaignExecutionAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "campaign", "triggered_at", "completed_at"]
    ...
    fieldsets = [
        (
            _("Execution Details"),
            {"fields": ["uuid", "campaign", "member", "status", "triggered_at", "completed_at"]},
        ),
        (_("Journey Progress"), {"fields": ["current_step", "next_step_at", "steps_completed"]}),
        (
            _("Results"),
            {"fields": ["actions_executed", "points_awarded", "rewards_issued", "emails_sent"]},
        ),
        (
            _("Error Information"),
            {"fields": ["error_message", "retry_count"], "classes": ["collapse"]},
        ),
        (_("A/B Testing"), {"fields": ["ab_variant_assigned"], "classes": ["collapse"]}),
        (_("Context"), {"fields": ["trigger_context"], "classes": ["collapse"]}),
    ]
    ...
    def points_awarded_display(self, obj):
        ...
        if obj.points_awarded == 0:
            return "-"
        return format_html(
            '<strong style="color: var(--success-fg);">+{}</strong>', obj.points_awarded
        )
...
class LoyaltySegmentAdmin(admin.ModelAdmin):
    ...
    list_filter = ["criteria_type", "is_active", "created_at", "last_calculated_at"]
    ...
    fieldsets = [
        ...
        (
            ...
            {
                "fields": ["criteria_type", "criteria_config"],
                "classes": ("tab-config",),
            },
        ),
        (
            _("Statistics"),
            {
                "fields": ["member_count", "last_calculated_at", "segment_preview"],
                "classes": ("tab-stats",),
            },
        ),
        (
    ...
    def member_count_display(self, obj):
        ...
        return format_html("<strong>{}</strong> members", obj.member_count)
...
class LoyaltySegmentMembershipAdmin(admin.ModelAdmin):
    ...
    list_display = ["segment", "member_display", "assigned_at", "assigned_by"]

    list_filter = ["segment", "assigned_at"]
    ...
    fieldsets = [
        (_("Membership Details"), {"fields": ["segment", "member", "assigned_at", "assigned_by"]})
    ]
