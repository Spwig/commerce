...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const island = document.getElementById('loyalty-rule-i18n');
    if (island) {
      try {
  ...
  async function toggleRuleStatus(ruleId, activate) {
    ...
    const message = activate ? msgs.confirmActivate : msgs.confirmDeactivate;
    ...
    const url = '/' + languageCode + '/admin/loyalty/rules/' + ruleId + '/' + action + '/';
