...
(function () {
    ...
    var enrollmentData = [];
    var pointsTrendData = [];
    var tierDistributionData = [];
    var translations = {};

    function init() {
        var dataEl = document.getElementById('loyalty-dashboard-data');
        if (dataEl) {
            try {
    ...
    function initEnrollmentChart(textColor, gridColor) {
        ...
        if (!enrollmentCtx || !enrollmentData || enrollmentData.length === 0) { return; }
    ...
    function initPointsTrendChart(textColor, gridColor) {
        var pointsTrendCtx = document.getElementById('pointsTrendChart');
        if (!pointsTrendCtx || !pointsTrendData || pointsTrendData.length === 0) { return; }
                    ...
                    datasets: [
                        {
                            ...
                            data: pointsTrendData.map(function (d) { return d.earned; }),
                            borderColor: '#10b981',
                        ...
                        },
                        {
                            ...
                            data: pointsTrendData.map(function (d) { return d.redeemed; }),
                            borderColor: '#ef4444',
    ...
    function initTierDistributionChart(gridColor) {
        var tierDistCtx = document.getElementById('tierDistributionChart');
        if (!tierDistCtx || !tierDistributionData || tierDistributionData.length === 0) { return; }

        try {
            new Chart(tierDistCtx, {
                type: 'bar',
                data: {
                    labels: tierDistributionData.map(function (d) { return d.name; }),
                    datasets: [{
                        label: translations.members || 'Members',
                        data: tierDistributionData.map(function (d) { return d.count; }),
                        backgroundColor: tierDistributionData.map(function (d) { return d.color; }),
                        borderColor: tierDistributionData.map(function (d) { return d.color; }),
                        borderWidth: 2,
                        borderRadius: 6,
                        borderSkipped: false
                    }]
