...
class CampaignBuilder {
  ...
  renderJourneySteps() {
    ...
    container.innerHTML = this.journeySteps
      .map((step, index) => this.renderJourneyStep(step, index))
      .join('');
  ...
  getJourneyStepFormData(modal) {
    ...
    const data = {
      ...
      next_step_delay_days:
        parseInt(form.querySelector('[name="next_step_delay_days"]').value) || 0,
    };
  ...
  _renderStepActions(modal) {
    ...
    if (this._tempStepActions.length === 0) {
      container.innerHTML =
        '<p style="color: var(--body-quiet-color); font-size: 13px;">No actions configured for this step</p>';
      return;
    }

    container.innerHTML = this._tempStepActions
      .map((action, index) => {
        const icons = {
          award_points: '\u2B50',
          send_email: '\uD83D\uDCE7',
  ...
  renderActions() {
    ...
    container.innerHTML = this.actions
      .map((action, index) => this.renderAction(action, index))
      .join('');
  ...
  renderTriggerConditions() {
    ...
    container.innerHTML = Object.entries(this.triggerConditions)
      .map(([key, value]) => {
