...
def loyalty_dashboard(request):
    ...
    active_members = (
        LoyaltyTransaction.objects.filter(created_at__gte=start_date, created_at__lte=end_date)
        .values("member")
        .distinct()
        .count()
    )
    ...
    points_outstanding = (points_stats["total_available"] or 0) + (
        points_stats["total_pending"] or 0
    )
    ...
    redemption_rate = (
        round((lifetime_redeemed / lifetime_earned) * 100, 1) if lifetime_earned > 0 else 0
    )
    ...
    avg_points_per_member = round(
        LoyaltyBalance.objects.filter(member__is_active=True).aggregate(
    ...
    for i in range(30):
        ...
        count = LoyaltyMember.objects.filter(
            enrolled_at__gte=date, enrolled_at__lt=next_date
        ).count()
        enrollment_data.append({"date": date.strftime("%Y-%m-%d"), "count": count})
    ...
    for i in range(30):
        ...
        points_trend.append(
            {"date": date.strftime("%Y-%m-%d"), "earned": earned, "redeemed": redeemed}
        )
    ...
    for tier in tiers:
        count = LoyaltyMember.objects.filter(current_tier=tier, is_active=True).count()
        tier_distribution.append(
            {"name": tier.name, "count": count, "color": tier.color or "#667eea"}
        )

    # Members with no tier
    no_tier_count = LoyaltyMember.objects.filter(current_tier__isnull=True, is_active=True).count()
    if no_tier_count > 0:
        tier_distribution.append({"name": "No Tier", "count": no_tier_count, "color": "#999999"})
    ...
    active_campaigns = LoyaltyCampaign.objects.filter(
        is_active=True, status=LoyaltyCampaign.STATUS_ACTIVE
    ).count()

    # Campaign executions in period
    campaign_executions = LoyaltyCampaignExecution.objects.filter(
        triggered_at__gte=start_date, triggered_at__lte=end_date
    )
    ...
    top_campaigns = (
        LoyaltyCampaign.objects.filter(is_active=True)
        .annotate(
            executions_in_period=Count(
                "executions",
                filter=Q(
                    executions__triggered_at__gte=start_date, executions__triggered_at__lte=end_date
                ),
            )
        )
...
def customer_dashboard(request):
    ...
    try:
        member = (
            LoyaltyMember.objects.select_related("customer", "current_tier", "balance")
            .prefetch_related("badges_earned")
            .get(customer=request.user, is_active=True)
        )
    except LoyaltyMember.DoesNotExist:
        # Auto-enroll the customer
        member = LoyaltyMember.objects.create(customer=request.user, is_active=True)
    ...
    if member.current_tier:
        ...
        next_tier = (
            LoyaltyTier.objects.filter(is_active=True, rank__gt=member.current_tier.rank)
            .order_by("rank")
            .first()
        )
    ...
    recent_transactions = LoyaltyTransaction.objects.filter(member=member).order_by("-created_at")[
        :10
    ]

    # Available rewards (featured or popular)
    available_rewards = LoyaltyReward.objects.filter(is_active=True, featured=True).order_by(
        "display_order", "points_cost"
    )[:6]
...
def customer_tier_info(request):
    ...
    for tier in all_tiers:
        if tier.min_points_earned > 0:
            progress = min(
                100, int((member.balance.lifetime_earned / tier.min_points_earned) * 100)
            )
...
def customer_badges(request):
    ...
    all_badges = LoyaltyBadge.objects.filter(is_active=True, is_visible=True).order_by(
        "display_order", "name"
    )
...
def filter_members(request):
    ...
    members = LoyaltyMember.objects.select_related("customer", "current_tier", "balance").all()
    ...
    html = render_to_string(
        "admin/loyalty/partials/member_cards.html",
        {
            "members": members,
        },
    )
...
def filter_transactions(request):
    ...
    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/transaction_cards.html",
        {
...
def filter_rewards(request):
    ...
    # Build query with proper prefetching for performance
    rewards = LoyaltyReward.objects.select_related("product", "required_tier").all()
    ...
    if availability == "available":
        ...
        rewards = rewards.filter(is_active=True)
        rewards = rewards.filter(Q(start_date__isnull=True) | Q(start_date__lte=today))
        rewards = rewards.filter(Q(end_date__isnull=True) | Q(end_date__gte=today))
        # Either unlimited or has quantity remaining
        rewards = rewards.filter(Q(quantity_total__isnull=True) | Q(quantity_remaining__gt=0))
    ...
    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/reward_cards.html",
        {
            "rewards": rewards,
        },
    )
...
def filter_redemptions(request):
    ...
    redemptions = LoyaltyRedemption.objects.select_related(
        "member", "member__customer", "reward"
    ).all()
    ...
    if expiry == "active":
        ...
        redemptions = redemptions.filter(Q(expires_at__lt=today) | Q(status="expired"))
    elif expiry == "expiring-soon":
        # Expiring within 7 days
        seven_days = today + timedelta(days=7)
        redemptions = redemptions.filter(
            expires_at__gte=today, expires_at__lte=seven_days, status="pending"
        )
    ...
    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/redemption_cards.html",
        {
            "redemptions": redemptions,
        },
    )

    return JsonResponse({"html": html, "count": redemptions.count()})
...
def filter_tiers(request):
    ...
    # Apply search filter
    if search:
        tiers = tiers.filter(Q(name__icontains=search) | Q(description__icontains=search))
    ...
    # Apply ordering
    tiers = tiers.order_by(order) if order else tiers.order_by("rank")

    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/tier_cards.html",
        {
            "tiers": tiers,
        },
    )

    return JsonResponse({"html": html, "count": tiers.count()})
...
def filter_badges(request):
    ...
    # Apply ordering
    badges = badges.order_by(order) if order else badges.order_by("display_order", "name")

    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/badge_cards.html",
        {
            "badges": badges,
        },
    )

    return JsonResponse({"html": html, "count": badges.count()})
...
def filter_rules(request):
    ...
    # Apply sorting
    rules = rules.order_by(sort) if sort else rules.order_by("-created_at")

    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/rule_cards.html",
        {
            "rules": rules,
        },
    )
...
def toggle_rule_status(request, rule_id):
    ...
    try:
        ...
        return JsonResponse({"success": True, "is_active": rule.is_active, "message": action_msg})
    except LoyaltyRule.DoesNotExist:
        return JsonResponse({"success": False, "error": _("Rule not found.")}, status=404)
    except Exception as e:
...
def filter_segments(request):
    ...
    # Apply ordering
    segments = segments.order_by(order) if order else segments.order_by("-created_at")

    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/segment_cards.html",
        {
            "segments": segments,
        },
    )

    return JsonResponse({"html": html, "count": segments.count()})
...
def refresh_segment(request, segment_id):
    ...
    try:
        ...
        return JsonResponse({"success": True, "message": _("Segment refresh has been queued.")})
    except LoyaltySegment.DoesNotExist:
        return JsonResponse({"success": False, "error": _("Segment not found.")}, status=404)
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
...
def filter_campaigns(request):
    ...
    # Apply ordering
    campaigns = campaigns.order_by(order) if order else campaigns.order_by("-created_at")

    # Render results as HTML
    html = render_to_string(
        "admin/loyalty/partials/campaign_cards.html",
        {
            "campaigns": campaigns,
        },
    )

    return JsonResponse({"html": html, "count": campaigns.count()})
...
def toggle_campaign_status(request, campaign_id):
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": _("Campaign not found.")}, status=404)
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
...
def campaign_wizard(request):
        ...
        "purchase": [
            {
                ...
                "name": _("Order Placed"),
                "icon": "shopping-cart",
                "description": _("When a customer completes a purchase"),
            },
            {
                "value": "first_purchase",
                "name": _("First Purchase"),
                "icon": "star",
                "description": _("Customer makes their first purchase"),
            },
            {
                "value": "nth_purchase",
                "name": _("Nth Purchase"),
                "icon": "layer-group",
                "description": _("Customer reaches a purchase milestone"),
            },
        ],
        "account": [
            {
                "value": "customer_signup",
                "name": _("Customer Signup"),
                "icon": "user-plus",
                "description": _("New customer creates an account"),
            },
            {
                "value": "birthday",
                "name": _("Birthday"),
                "icon": "birthday-cake",
                "description": _("Customer's birthday"),
            },
            {
                "value": "anniversary",
                "name": _("Anniversary"),
                "icon": "gift",
                "description": _("Account creation anniversary"),
            },
        ],
        "tier": [
            {
                "value": "tier_promoted",
                "name": _("Tier Promoted"),
                "icon": "arrow-up",
                "description": _("Customer moves up a tier"),
            },
            {
                "value": "tier_demoted",
                "name": _("Tier Demoted"),
                "icon": "arrow-down",
                "description": _("Customer moves down a tier"),
            },
        ],
        "engagement": [
            {
                "value": "cart_abandoned",
                "name": _("Cart Abandoned"),
                "icon": "shopping-basket",
                "description": _("Customer abandons their cart"),
            },
            {
                "value": "no_purchase_90d",
                "name": _("Inactive 90 Days"),
                "icon": "clock",
                "description": _("No purchase in 90 days"),
            },
            {
                "value": "points_expiring",
                "name": _("Points Expiring"),
                "icon": "hourglass-end",
                "description": _("Points about to expire"),
            },
            {
                "value": "review_submitted",
                "name": _("Review Submitted"),
                "icon": "comment",
                "description": _("Customer submits a review"),
            },
            {
                "value": "referral_converted",
                "name": _("Referral Converted"),
                "icon": "users",
                "description": _("A referred customer makes a purchase"),
            },
        ],
    }
    ...
    action_types = [
        {
            ...
            "name": _("Award Points"),
            "icon": "coins",
            "description": _("Give a fixed number of points"),
        },
        {
            "value": "bonus_points",
            "name": _("Bonus Points"),
            "icon": "percentage",
            "description": _("Multiply points earned"),
        },
        {
            "value": "send_email",
            "name": _("Send Email"),
            "icon": "envelope",
            "description": _("Send a notification email"),
        },
        {
            "value": "upgrade_tier",
            "name": _("Upgrade Tier"),
            "icon": "arrow-up",
            "description": _("Move customer to higher tier"),
        },
        {
            "value": "grant_badge",
            "name": _("Grant Badge"),
            "icon": "medal",
            "description": _("Award an achievement badge"),
        },
    ]
    ...
    if request.method == "POST":
        try:
            ...
            if not name:
                return JsonResponse(
                    {"success": False, "error": _("Campaign name is required.")}, status=400
                )

            campaign_type = data.get("campaign_type", "").strip()
            if not campaign_type:
                return JsonResponse(
                    {"success": False, "error": _("Campaign type is required.")}, status=400
                )
            ...
            messages.success(
                request, _('Campaign "%(name)s" created successfully.') % {"name": name}
            )
