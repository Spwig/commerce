...
def campaign_analytics(request, campaign_id):
    ...
    daily_trend = (
        executions.annotate(date=TruncDate("triggered_at"))
        .values("date")
...
def export_campaign_report(request, campaign_id):
    ...
    campaign = get_object_or_404(LoyaltyCampaign, pk=campaign_id)
    executions = LoyaltyCampaignExecution.objects.filter(campaign=campaign).select_related(
        "member__customer"
    )
...
def member_analytics(request):
    ...
    # Member growth
    member_growth = (
        LoyaltyMember.objects.filter(joined_at__gte=start_date)
    ...
    # Active members
    active_members = LoyaltyMember.objects.filter(balance__last_earned_at__gte=start_date).count()

    # Top members by points
    top_members = (
        LoyaltyMember.objects.select_related("customer", "current_tier")
        .annotate(total_points=F("balance__lifetime_earned"))
        .order_by("-total_points")[:20]
    )

    # Member distribution by tier
    tier_distribution = (
        LoyaltyMember.objects.values("current_tier__name")
        .annotate(count=Count("id"))
        .order_by("-count")
    )
...
def get_program_overview():
    ...
    recent_transactions = LoyaltyTransaction.objects.filter(created_at__gte=week_ago).count()
    ...
    campaigns = (
        ...
        .annotate(
            executions_count=Count(
                "executions", filter=Q(executions__triggered_at__gte=start_date)
            ),
...
def get_member_engagement(start_date):
    ...
    active_earners = LoyaltyMember.objects.filter(balance__last_earned_at__gte=start_date).count()

    # Members who redeemed
    active_redeemers = (
        LoyaltyRedemption.objects.filter(created_at__gte=start_date)
        .values("member")
        .distinct()
        .count()
    )
    ...
    avg_balance = LoyaltyBalance.objects.aggregate(avg=Avg("available_points"))["avg"] or 0
...
def get_points_economy(start_date):
    ...
    transactions = LoyaltyTransaction.objects.filter(created_at__gte=start_date)

    earned = (
        transactions.filter(transaction_type=LoyaltyTransaction.TYPE_EARN).aggregate(
            total=Sum("points")
        )["total"]
        or 0
    )
    ...
    redeemed = (
        transactions.filter(transaction_type=LoyaltyTransaction.TYPE_REDEEM).aggregate(
            total=Sum("points")
        )["total"]
        or 0
    )
    ...
    expired = (
        transactions.filter(transaction_type=LoyaltyTransaction.TYPE_EXPIRE).aggregate(
            total=Sum("points")
        )["total"]
        or 0
    )
    ...
    bonus = (
        transactions.filter(transaction_type=LoyaltyTransaction.TYPE_BONUS).aggregate(
            total=Sum("points")
        )["total"]
        or 0
    )
...
def get_tier_distribution():
    ...
    tiers = LoyaltyTier.objects.annotate(member_count=Count("members")).order_by("rank")
...
def analyze_journey_performance(campaign):
    ...
    if campaign.journey_steps:
        for step in campaign.journey_steps:
            ...
            step_stats[step_num] = {
                ...
                "completion_rate": (completed_count / executions.count() * 100)
    ...
    completed_executions = executions.filter(
        status=LoyaltyCampaignExecution.STATUS_COMPLETED, completed_at__isnull=False
    )
    ...
    if completed_executions.exists():
        ...
        for execution in completed_executions:
            if execution.triggered_at and execution.completed_at:
                duration = (
                    execution.completed_at - execution.triggered_at
