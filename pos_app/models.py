...
class StoreGroup(models.Model):
    ...
    name = models.CharField(
        _('group name'),
        max_length=200,
        help_text=_('e.g. "NZ Stores", "Singapore Region"')
    )
...
class POSTerminal(models.Model):
    ...
    order_sync_days = models.PositiveIntegerField(
        _('order history (days)'), default=14,
        help_text=_(
    ...
    order_sync_limit = models.PositiveIntegerField(
        _('order sync limit'), default=500,
        help_text=_(
    ...
    remote_unlock_at = models.DateTimeField(
        ...
        null=True,
        blank=True,
        help_text=_('Set by admin to remotely unlock the terminal. Frontend polls and unlocks if this is newer than lock time.')
    )
...
class DisplayPairingCode(models.Model):
    ...
    code = models.CharField(
        _('pairing code'),
        max_length=6,
        default=generate_display_pairing_code
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class POSShift(models.Model):
    ...
    def close_shift(self, closing_cash_amount):
        ...
        with transaction.atomic():
            ...
            cash_in = shift.cash_movements.filter(movement_type='in').aggregate(
                total=models.Sum('amount')
            )['total'] or 0
            cash_out = shift.cash_movements.filter(movement_type='out').aggregate(
                total=models.Sum('amount')
            )['total'] or 0

            # Cash from sales
            cash_payments = shift.pos_payments.filter(method='cash').aggregate(
                total=models.Sum('amount')
            )['total'] or 0
            cash_refund_payments = shift.pos_payments.filter(
                method='cash', amount__lt=0
            ...
            shift.save(update_fields=[
                'ended_at', 'closing_cash', 'expected_cash', 'cash_difference'
            ])
...
class CashMovement(models.Model):
    ...
    amount = models.DecimalField(
        ...
        max_digits=10,
        decimal_places=2,
        help_text=_('Amount in base currency')
    )
...
class POSTerminalProvider(models.Model):
    ...
    credentials_encrypted = models.JSONField(
        ...
        default=dict,
        blank=True,
        help_text=_('Encrypted API credentials')
    )
    ...
    def get_provider_instance(self):
        ...
        credentials = decrypt_credentials(self.credentials_encrypted) if self.credentials_encrypted else {}
...
class POSTerminalReader(models.Model):
    ...
    splash_override_image = models.ForeignKey(
        ...
        help_text=_('Upload a custom splash screen image. Leave blank to auto-generate from site logo.')
    )
...
class POSPayment(models.Model):
    ...
    amount = models.DecimalField(
        ...
        max_digits=10,
        decimal_places=2,
        help_text=_('Amount paid via this method')
    )
...
class ReceiptTemplate(models.Model):
    ...
    store_group = models.OneToOneField(
        ...
        verbose_name=_('store group'),
        help_text=_('Template for all stores in this group. Leave blank for default or specific store.')
    )
    ...
    custom_email = models.EmailField(
        _('custom email'),
        blank=True,
        help_text=_('Leave blank to use the store contact email.')
    )
    ...
    show_sku = models.BooleanField(
        _('show SKU'),
        default=False,
        help_text=_('Display product SKU on receipt items')
    )
    show_cashier = models.BooleanField(
        _('show cashier'),
        default=True,
        help_text=_('Display cashier name on receipt')
    )
    show_terminal_name = models.BooleanField(
        _('show terminal name'),
        default=False,
        help_text=_('Display terminal name on receipt')
    )
    ...
    qr_label = models.CharField(
        ...
        max_length=200,
        blank=True,
        help_text=_('Text printed below the QR code, e.g. "Scan for 10% off your next online order"')
    )
    ...
    def clean(self):
        ...
        if self.store_group and self.warehouse:
            raise ValidationError(
                _('A template cannot be assigned to both a store group and a specific store. Choose one.')
            )
...
class PromoSlide(models.Model):
    ...
    title = models.CharField(
        ...
        max_length=200,
        blank=True,
        help_text=_('Large text overlay on the slide.')
    )
    subtitle = models.CharField(
        _('subtitle'),
        max_length=500,
        blank=True,
        help_text=_('Secondary text below the title.')
    )
    sort_order = models.PositiveIntegerField(
        _('sort order'),
        default=0,
    ...
    def clean(self):
        ...
        if self.store_group and self.warehouse:
            raise ValidationError(
                _('A slide cannot be assigned to both a store group and a specific store. Choose one.')
            )
...
class POSStaffDiscount(models.Model):
    ...
    manager_pin = models.CharField(
        ...
        max_length=6,
        blank=True,
        help_text=_('4-6 digit PIN for approving discounts (managers only). '
    ...
    cashier_pin = models.CharField(
        ...
        max_length=6,
        blank=True,
        help_text=_('4-6 digit PIN for terminal lock/unlock. '
                     'Migrated to hashed storage on first use.')
    )
...
class TerminalLockEvent(models.Model):
    ...
    failed_attempt_count = models.PositiveIntegerField(
        _('failed attempts'),
        default=0,
        help_text=_('Cumulative failed attempts before this event')
    )

    # Cart state at time of event (for audit trail)
    cart_item_count = models.PositiveIntegerField(
        _('cart items'),
        default=0
    )
...
class WebAuthnCredential(models.Model):
    ...
    public_key = models.BinaryField(
        _('public key'),
        help_text=_('CBOR-encoded attested credential data')
    )
...
class ParkedCart(models.Model):
    ...
    cart_data = models.JSONField(
        _('cart data'),
        help_text=_('Full cart snapshot: items, customer, discounts, etc.')
    )
