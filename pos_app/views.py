...
POS_DIST_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "frontend_build")
...
class POSAssetView(View):
    ...
    def get(self, request, path="", prefix=""):
        ...
        response = FileResponse(open(file_path, "rb"))  # noqa: SIM115  # FileResponse takes ownership + closes on stream end
...
class PublicReceiptView(View):
    ...
    def get(self, request, token):
        ...
        order = get_object_or_404(
            Order.objects.prefetch_related("items", "pos_payments").select_related(
