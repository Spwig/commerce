...
from .models import (
    POSTerminal, POSShift, CashMovement, POSPayment, ReceiptTemplate,
    POSTerminalProvider, POSTerminalReader, PromoSlide, StoreGroup,
    POSStaffDiscount, TerminalLockEvent,
)
...
class POSTerminalAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'warehouse', 'is_active', 'last_heartbeat_display',
        'assigned_users_count', 'pairing_code'
    ]
    ...
    def get_queryset(self, request):
        ...
        qs = qs.select_related('warehouse').prefetch_related('assigned_users').annotate(
        ...
        if object_id:
            ...
            if obj:
                ...
                extra_context['is_online'] = (
                    obj.last_heartbeat is not None
                    and obj.last_heartbeat >= online_cutoff
                )
    ...
    def regenerate_pairing_codes(self, request, queryset):
        ...
        self.message_user(request, _('Pairing codes regenerated for %d terminals.') % queryset.count())
    ...
    def remote_unlock_terminals(self, request, queryset):
        ...
        self.message_user(
            request,
            _('Remote unlock signal sent to %d terminal(s). They will unlock within a few seconds.') % count,
        )
...
class StoreGroupAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Regional Settings'), {
            ...
            'description': _('Settings applied to all stores in this group. Blank = inherit from site default.'),
        }),
        (_('Advanced'), {
            'fields': ('settings',),
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context = extra_context or {}
        extra_context['currencies'] = StoreGroup.objects.exclude(
            currency=''
...
class POSTerminalReaderInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ['provider_reader_id', 'reader_type', 'serial_number', 'status', 'last_seen_at']
    fields = ['reader_label', 'provider_reader_id', 'reader_type', 'terminal', 'status', 'last_seen_at']
...
class POSTerminalProviderAdmin(admin.ModelAdmin):
    ...
    list_display = ['display_name', 'provider_key', 'is_active', 'connection_status', 'last_tested_at']
    list_filter = ['is_active', 'connection_status']
    readonly_fields = ['id', 'connection_status', 'connection_error', 'last_tested_at', 'created_at', 'updated_at']
    ...
    fieldsets = (
        ...
        (_('Component'), {
            'fields': ('component',),
            'description': _('Component package from upgrade server. Null for built-in manual provider.'),
        }),
        (_('Credentials'), {
            'fields': ('credentials_encrypted',),
            'description': _('Encrypted API credentials. Use set_credentials() to update safely.'),
        }),
        (_('Provider Settings'), {
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        qs = POSTerminalProvider.objects.annotate(
...
class POSTerminalReaderAdmin(admin.ModelAdmin):
    ...
    list_display = ['reader_label', 'provider', 'terminal', 'reader_type', 'status', 'last_seen_at', 'splash_status']
    list_filter = ['status', 'provider']
    search_fields = ['reader_label', 'provider_reader_id', 'serial_number']
    readonly_fields = [
        'id', 'created_at', 'updated_at', 'last_seen_at',
        'stripe_splash_file_id', 'stripe_splash_config_id', 'splash_generated_at'
    ]
    ...
    def regenerate_splash_screen(self, request, queryset):
        ...
        if count > 0:
            ...
            messages.warning(
                request,
                _('No Stripe Terminal readers selected.')
            )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['providers'] = POSTerminalProvider.objects.filter(is_active=True).order_by('display_name')
...
class POSPaymentInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ['created_at']
    fields = ['method', 'amount', 'amount_tendered', 'change_given',
              'card_last_four', 'card_reference', 'card_brand',
              'provider_payment_id', 'gift_card_code', 'created_at']
...
class POSShiftAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'cashier', 'terminal', 'started_at', 'ended_at',
        'total_sales', 'total_transactions', 'shift_status'
    ]
    list_filter = ['terminal', 'started_at', 'ended_at']
    search_fields = ['cashier__username', 'cashier__first_name', 'cashier__last_name']
    readonly_fields = [
        'started_at', 'ended_at', 'expected_cash', 'cash_difference',
        'total_sales', 'total_refunds', 'total_transactions'
    ]
    inlines = [CashMovementInline, POSPaymentInline]

    fieldsets = (
        (None, {
            'fields': ('terminal', 'cashier', 'started_at', 'ended_at')
        }),
    ...
    def changelist_view(self, request, extra_context=None):
        extra_context = extra_context or {}
        extra_context['terminals'] = (
            POSTerminal.objects.filter(is_active=True).order_by('name')
        )
...
class ReceiptTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Store Information'), {
            'fields': (
                'show_store_address', 'custom_address',
                'show_store_phone', 'custom_phone',
                'show_store_email', 'custom_email',
            ),
        }),
        (_('Business Details'), {
    ...
    def changelist_view(self, request, extra_context=None):
        extra_context = extra_context or {}
        extra_context['store_groups'] = StoreGroup.objects.filter(
            is_active=True
        ).order_by('sort_order', 'name')
        extra_context['warehouses'] = Warehouse.objects.filter(
...
class POSStaffDiscountAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'user', 'max_discount_percentage', 'max_discount_amount_display',
        'can_apply_item_discounts', 'can_apply_cart_discounts',
        'requires_reason', 'is_manager'
    ]
    list_filter = ['is_manager', 'requires_reason', 'can_apply_item_discounts', 'can_apply_cart_discounts']
    ...
    fieldsets = (
        ...
        (_('Permissions'), {
            'fields': ('can_apply_item_discounts', 'can_apply_cart_discounts', 'requires_reason'),
        }),
        (_('Manager Approval'), {
        ...
        (_('Terminal Lock'), {
            'fields': ('cashier_pin',),
            'description': _(
                'Personal PIN for unlocking the terminal after locking. '
...
class TerminalLockEventAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'created_at', 'terminal', 'event_type', 'performed_by',
        'locked_by', 'manager_override', 'cart_summary'
    ]
    list_filter = ['event_type', 'manager_override', 'terminal', 'created_at']
    search_fields = [
        'terminal__name', 'performed_by__username', 'performed_by__email',
    ...
    readonly_fields = [
        'terminal', 'shift', 'event_type', 'performed_by', 'locked_by',
        'manager_override', 'failed_attempt_count', 'cart_item_count',
        'cart_total', 'ip_address', 'created_at'
    ]
