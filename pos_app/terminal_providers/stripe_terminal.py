...
class StripeTerminalProvider(TerminalProviderBase):
    ...
    def list_readers(self, location_id=None):
        ...
        try:
            params = {"limit": 100}
            loc = (
                location_id or self.config.get("location_id") or self.credentials.get("location_id")
            )
            if loc:
                params["location"] = loc
        ...
        try:
            ...
            if intent.status == "succeeded":
                ...
                if latest_charge_id:
                    ...
                    pm_details = (charge.get("payment_method_details") or {}).get(
                        "card_present", {}
                    )
    ...
    def assign_configuration_to_reader(self, reader_id: str, config_id: str):
        ...
        try:
            ...
            s.terminal.Location.modify(location_id, configuration_overrides=config_id)
            logger.info(
                f"Assigned configuration {config_id} to location {location_id} (reader {reader_id})"
            )
        except Exception as e:
