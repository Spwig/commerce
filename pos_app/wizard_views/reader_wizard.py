...
from pos_app.models import POSTerminalProvider, POSTerminalReader, POSTerminal
...
class ReaderWizardStep1View(ReaderWizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        providers = POSTerminalProvider.objects.filter(
            is_active=True
        ).select_related('component').order_by('display_name')

        if not providers.exists():
            messages.warning(
                request,
                _('No active terminal providers found. Please configure a provider first.')
            )
        ...
        if method == 'discover':
            try:
                ...
                if result.get('success'):
                    ...
                    existing_ids = set(
                        POSTerminalReader.objects.filter(
                            provider=provider
                        ).values_list('provider_reader_id', flat=True)
                    )
                    available_readers = [
                        r for r in result.get('readers', [])
        ...
        if method == 'register':
            ...
            try:
                    ...
                    if reader_data['id'] in selected_ids:
                        reader, created = POSTerminalReader.objects.update_or_create(
                            ...
                            defaults={
                                ...
                                'status': 'online' if reader_data.get('status') == 'online' else 'offline',
                                'last_seen_at': timezone.now(),
                            }
                        )
                ...
                if len(readers_created) == 1:
                    ...
                    messages.success(
                        request,
                        _('Successfully imported %(count)d readers.') % {'count': len(readers_created)}
                    )
...
class ReaderWizardStep3View(ReaderWizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        terminals = POSTerminal.objects.filter(
            is_active=True
        ).select_related('card_reader', 'warehouse').order_by('name')
    ...
    def post(self, request):
        ...
        messages.success(
            request,
            _('Reader "%(name)s" has been added successfully.') % {
                'name': reader.reader_label
            }
        )
