...
from .license import activate_pos_license, get_pos_license_status
from .models import (
    POSPayment,
    POSShift,
    POSTerminal,
...
def pos_dashboard(request):
    ...
    if not license_status["valid"]:
        return render(
            request,
            "admin/pos_app/dashboard_upsell.html",
            {
                "title": "Point of Sale",
    ...
    recent_shifts = (
        POSShift.objects.filter(ended_at__isnull=False)
...
def filter_terminals(request):
    ...
    queryset = (
        POSTerminal.objects.select_related("warehouse")
        .prefetch_related("assigned_users")
        .annotate(
    ...
    if search:
        queryset = queryset.filter(Q(name__icontains=search) | Q(pairing_code__icontains=search))
    if warehouse:
...
def filter_shifts(request):
    ...
    base_qs = POSShift.objects.select_related("terminal", "terminal__warehouse", "cashier")
    if search:
...
def filter_promo_slides(request):
    ...
    queryset = PromoSlide.objects.select_related("warehouse", "store_group", "image").order_by(
...
def filter_receipt_templates(request):
    ...
    # Build queryset
    queryset = ReceiptTemplate.objects.select_related("warehouse", "store_group", "logo").all()
...
def filter_terminal_providers(request):
    ...
    queryset = POSTerminalProvider.objects.annotate(
...
def filter_terminal_readers(request):
    ...
    queryset = POSTerminalReader.objects.select_related(
        "provider", "terminal", "terminal__warehouse"
...
def quick_assign_reader(request):
    ...
    if terminal_id:
        try:
            ...
            POSTerminalReader.objects.filter(terminal=terminal).exclude(id=reader_id).update(
                terminal=None
            )
                    ...
                    return JsonResponse(
                        {
                            ...
                            "last_seen": reader.last_seen_at.isoformat()
