...
(function () {
  ...
  function showMessage(text, type) {
    ...
    if (!messageList) {
      ...
      const header = document.querySelector('.change-list-header');
      if (!header) return;
  ...
  function syncReaders() {
    const btn = document.getElementById('sync-readers-btn');
    if (!btn) return;
    btn.disabled = true;
    ...
    fetch('/' + lang + '/admin/pos/readers/sync/', {
      ...
      .then(function (data) {
        ...
        if (data.success) {
          showMessage(
            (T.synced || 'Synced') + ' ' + data.synced + ' ' + (T.readers || 'readers'),
            'success'
          );
        } else if (data.errors && data.errors.length > 0) {
          showMessage(
            (T.syncWithErrors || 'Sync completed with errors:') + ' ' + data.errors.join(', '),
            'warning'
          );
        }
        if (window.AdminListFilters) {
          window.AdminListFilters.applyFilters();
        }
      })
  ...
  function quickAssignTerminal(select) {
    const readerId = select.dataset.readerId;
    ...
    fetch('/' + lang + '/admin/pos/readers/' + readerId + '/refresh-status/', {
      ...
      .then(function (data) {
        ...
        if (data.success) {
          ...
          const statusBadge = card.querySelector('.reader-status-badge');
          ...
          if (data.status === 'online') {
            ...
            statusBadge.innerHTML =
              '<span class="terminal-status-dot online"></span> ' + (T.online || 'Online');
          } else if (data.status === 'busy') {
            statusBadge.classList.add('list-row-card-badge-warning');
            statusBadge.innerHTML =
              '<span class="terminal-status-dot busy"></span> ' + (T.busy || 'Busy');
          } else {
            statusBadge.innerHTML =
              '<span class="terminal-status-dot offline"></span> ' + (T.offline || 'Offline');
          }
          ...
          const lastSeenEl = card.querySelector('.reader-last-seen');
          if (lastSeenEl) {
            lastSeenEl.textContent = T.justNow || 'Just now';
          }
        } else {
  ...
  document.addEventListener('click', function (e) {
    if (e.target.closest('[data-action="sync-readers"]')) {
      ...
      const refreshBtn = e.target.closest('[data-action="refresh-reader-status"]');
      if (refreshBtn) {
        e.preventDefault();
  ...
  document.addEventListener('change', function (e) {
    const select = e.target.closest('[data-action="quick-assign-terminal"]');
    if (select) {
      quickAssignTerminal(select);
    }
  });

  document.addEventListener('DOMContentLoaded', function () {
    const el = document.getElementById('reader-list-translations');
    if (el) {
      try {
