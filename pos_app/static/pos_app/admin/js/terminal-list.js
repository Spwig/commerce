...
(function () {
  ...
  function initEnableStore() {
    const enableBtn = document.getElementById('enable-store-btn');
    if (!enableBtn) return;

    enableBtn.addEventListener('click', function () {
      const warehouseId = this.dataset.warehouseId;
      const displayName = document.getElementById('store-display-name').value.trim();
      const btn = this;
      btn.disabled = true;
      ...
      fetch('/' + lang + '/admin/pos/store-location/enable/', {
        ...
        .then(function (data) {
          if (data.success) {
            ...
            btn.innerHTML =
              ...
              (T.enableStoreLocation || 'Enable as Store Location');
            AdminModal.alert({
              message:
                data.error || T.somethingWentWrong || 'Something went wrong. Please try again.',
              type: 'error',
            });
          }
        })
        .catch(function () {
          btn.disabled = false;
          btn.innerHTML =
            '<i class="fas fa-check"></i> ' + (T.enableStoreLocation || 'Enable as Store Location');
          AdminModal.alert({
            message: T.somethingWentWrong || 'Something went wrong. Please try again.',
            type: 'error',
          });
        });
    });
  ...
  async function handleUnlock(btn) {
    ...
    if (
      !(await AdminModal.confirm(T.confirmUnlock || 'Send remote unlock signal to this terminal?'))
    )
      return;
    ...
    fetch('/' + lang + '/admin/pos/terminals/unlock/', {
      ...
      .then(function (data) {
        ...
        if (data.success) {
          ...
          AdminModal.alert({
            message: data.error || T.failedUnlock || 'Failed to unlock terminal.',
            type: 'error',
          });
        }
      })
      .catch(function () {
        ...
        icon.className = 'fas fa-unlock';
        AdminModal.alert({
          message: T.failedUnlock || 'Failed to unlock terminal.',
          type: 'error',
        });
      });
  }
  ...
  document.addEventListener('click', function (e) {
    const unlockBtn = e.target.closest('.terminal-unlock-btn');
    if (unlockBtn) {
      e.preventDefault();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const el = document.getElementById('terminal-list-translations');
    if (el) {
      try {
