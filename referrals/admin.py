...
class ReferralProgramAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('name', 'status', 'terms_and_conditions')
        }),
        (_('Reward Configuration'), {
            'fields': ('reward_config',),
            'classes': ('collapse',),
            'description': _('Configure referrer and referee rewards (amounts, types, double-sided)')
        }),
        (_('Eligibility Rules'), {
            'fields': ('eligibility_rules',),
            'classes': ('collapse',),
            'description': _('Define minimum order values, product restrictions, customer segments')
        }),
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        from referrals.models import ReferralIdentity, ReferralReward, ReferralAttribution
        ...
        stats = {
            ...
            'total_clicks': ReferralIdentity.objects.aggregate(total=Sum('total_clicks'))['total'] or 0,
            'total_signups': ReferralIdentity.objects.aggregate(total=Sum('total_signups'))['total'] or 0,
            'total_conversions': ReferralIdentity.objects.aggregate(total=Sum('total_conversions'))['total'] or 0,
...
class ReferralIdentityAdmin(admin.ModelAdmin):
    ...
    list_display = ['get_customer_name', 'get_token_short', 'total_clicks', 'total_signups', 'total_conversions', 'get_rewards_earned', 'created_at']
    list_filter = ['created_at']
    search_fields = ['customer__email', 'customer__first_name', 'customer__last_name', 'token']
    readonly_fields = ['token', 'total_clicks', 'total_signups', 'total_conversions', 'total_rewards_earned', 'created_at', 'updated_at', 'get_referral_link', 'get_qr_code']
    ...
    fieldsets = (
        ...
        (_('Statistics'), {
            'fields': ('total_clicks', 'total_signups', 'total_conversions', 'total_rewards_earned'),
    ...
    def get_referral_link(self, obj):
        ...
        return format_html(
            '<input type="text" value="{}" readonly class="referral-link-input">',
            link
        )
...
class ReferralEventAdmin(admin.ModelAdmin):
    ...
    list_display = ['event_type', 'get_referrer_name', 'get_customer_name', 'get_ip_address', 'created_at']
    list_filter = ['event_type', 'created_at']
    search_fields = ['referrer_identity__customer__email', 'customer__email', 'ip_address']
    readonly_fields = ['program', 'referrer_identity', 'customer', 'order', 'event_type', 'ip_address', 'user_agent', 'device_fingerprint', 'referrer_url', 'landing_url', 'metadata', 'created_at']
    date_hierarchy = 'created_at'

    fieldsets = (
        (_('Event Details'), {
            'fields': ('event_type', 'program', 'created_at')
        }),
        (_('Related Objects'), {
            'fields': ('referrer_identity', 'customer', 'order')
        }),
        (_('Tracking Data'), {
            'fields': ('ip_address', 'user_agent', 'device_fingerprint', 'referrer_url', 'landing_url', 'metadata'),
            'classes': ('collapse',)
        }),
    )
...
class ReferralAttributionAdmin(admin.ModelAdmin):
    ...
    list_display = ['get_referrer_name', 'get_referee_name', 'get_status_badge', 'get_order_value', 'get_risk_badge', 'created_at']
    list_filter = ['status', 'rejection_reason', RiskScoreFilter, 'created_at']
    search_fields = ['referrer_identity__customer__email', 'referee_customer__email', 'first_order__order_number']
    readonly_fields = ['program', 'created_at', 'approved_at', 'reviewed_at', 'validation_data', 'get_order_link']
    date_hierarchy = 'created_at'
    ...
    fieldsets = (
        (_('Attribution'), {
            'fields': ('program', 'referrer_identity', 'referee_customer', 'first_order', 'get_order_link')
        }),
        (_('Status'), {
            'fields': ('status', 'risk_score', 'rejection_reason', 'rejection_notes')
        }),
        (_('Review'), {
    ...
    def get_referrer_name(self, obj):
        ...
        if obj.referrer_identity and obj.referrer_identity.customer:
            ...
            name = obj.referrer_identity.customer.get_full_name() or obj.referrer_identity.customer.email
            return format_html('<a href="{}">{}</a>', url, name)
        return '-'
    get_referrer_name.short_description = _('Referrer')
    ...
    def get_order_link(self, obj):
        ...
        if obj.first_order:
            ...
            return format_html('<a href="{}" target="_blank">Order #{}</a>', url, obj.first_order.order_number)
        return '-'
    ...
    def approve_attributions(self, request, queryset):
        ...
        for attribution in pending:
            ...
            try:
                ...
                messages.error(request, _('Error creating rewards for attribution %(id)s: %(error)s') % {'id': attribution.id, 'error': e})
    ...
    def reject_attributions(self, request, queryset):
        ...
        count = pending.count()
        messages.success(request, ngettext(
            'Rejected %(count)d attribution.',
            'Rejected %(count)d attributions.',
            count
        ) % {'count': count})
...
class ReferralRewardAdmin(admin.ModelAdmin):
    ...
    list_display = ['get_customer_name', 'get_recipient_badge', 'get_kind_badge', 'amount', 'get_status_badge', 'created_at', 'get_expires_at']
    list_filter = ['status', 'kind', 'recipient_type', RewardExpiringFilter, 'created_at']
    search_fields = ['customer__email', 'customer__first_name', 'customer__last_name', 'description']
    readonly_fields = ['program', 'attribution', 'created_at', 'issued_at', 'redeemed_at', 'revoked_at']
    date_hierarchy = 'created_at'
    actions = ['issue_rewards', 'revoke_rewards']

    fieldsets = (
        (_('Reward Details'), {
            'fields': ('program', 'attribution', 'referrer_identity', 'customer', 'recipient_type')
        }),
        (_('Reward Configuration'), {
            'fields': ('kind', 'amount', 'percentage', 'description')
        }),
        (_('Status'), {
            'fields': ('status', 'revocation_reason')
        }),
        (_('Integration'), {
            'fields': ('wallet_transaction', 'voucher_code_id'),
            'classes': ('collapse',),
            'description': _('Links to wallet transactions or voucher codes (populated when issued)')
        }),
        (_('Lifecycle'), {
    ...
    def get_expires_at(self, obj):
        ...
        if obj.expires_at < timezone.now():
            ...
            return format_html('<span class="referral-text-warning">{}</span>', obj.expires_at.strftime('%Y-%m-%d'))
        else:
        ...
        for reward in pending:
            try:
                ...
                if success:
                    ...
                    messages.warning(request, _('Failed to issue reward %(id)s') % {'id': reward.id})
            except Exception as e:
                messages.error(request, _('Error issuing reward %(id)s: %(error)s') % {'id': reward.id, 'error': e})

        messages.success(request, ngettext(
            'Issued %(count)d reward.',
            'Issued %(count)d rewards.',
            issued_count
        ) % {'count': issued_count})
    ...
    def revoke_rewards(self, request, queryset):
        ...
        for reward in issued:
            try:
                ...
                messages.error(request, _('Error revoking reward %(id)s: %(error)s') % {'id': reward.id, 'error': e})

        messages.success(request, ngettext(
            'Revoked %(count)d reward.',
            'Revoked %(count)d rewards.',
            revoked_count
        ) % {'count': revoked_count})
