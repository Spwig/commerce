...
def referral_dashboard(request):
    ...
    # Pending approvals
    pending_approvals = ReferralAttribution.objects.filter(
        status='pending'
    ).count()

    # Success rate (approved / total)
    approved_count = ReferralAttribution.objects.filter(
        status='approved'
    ).count()
    ...
    # Top referrer
    top_referrer = None
    top_referrer_data = ReferralAttribution.objects.filter(
        status='approved',
        referrer_identity__isnull=False
    ).values(
    ...
    # Top 10 referrers
    top_referrers = []
    top_referrer_list = ReferralAttribution.objects.filter(
        status='approved',
        referrer_identity__isnull=False
    ).values(
        'referrer_identity__id',
    ...
    fraud_flags = ReferralAttribution.objects.filter(
        status='pending',
        risk_score__gte=50
    ).select_related(
        'referrer_identity__customer',
...
def approve_attribution(request, pk):
    ...
    if attribution.status != 'pending':
        return JsonResponse({
            'success': False,
            'message': _('Attribution is not pending')
        }, status=400)
...
def issue_reward_view(request, pk):
    ...
    if reward.status != 'pending':
        return JsonResponse({
            'success': False,
            'message': _('Reward is not pending')
        }, status=400)
    ...
    try:
        ...
        if success:
            ...
            return JsonResponse({
                'success': False,
                'message': _('Failed to issue reward')
            }, status=500)
...
def revoke_reward_view(request, pk):
    ...
    if reward.status not in ['pending', 'issued']:
        return JsonResponse({
            'success': False,
            'message': _('Reward cannot be revoked')
        }, status=400)
    ...
    try:
        ...
        if success:
            ...
            return JsonResponse({
                'success': False,
                'message': _('Failed to revoke reward')
            }, status=500)
...
def filter_referrers(request):
    ...
    valid_sorts = ['created_at', '-created_at', 'total_conversions', '-total_conversions',
                   'total_rewards_earned', '-total_rewards_earned', 'total_clicks', '-total_clicks']
    ...
    html = render_to_string(
        'admin/referrals/partials/referrer_cards.html',
        {'referrers': referrers},
        request=request
    )
...
def filter_rewards(request):
    ...
    # Render partial template
    html = render_to_string(
        'admin/referrals/partials/reward_cards.html',
        {'rewards': rewards},
        request=request
    )
...
def filter_events(request):
    ...
    # Render partial template
    html = render_to_string(
        'admin/referrals/partials/event_cards.html',
        {'events': events},
        request=request
    )
