...
(function() {
    ...
    function initReferralsDashboard() {
        var dataEl = document.getElementById('referrals-dashboard-data');
        if (!dataEl) return;

        var cfg = JSON.parse(dataEl.textContent);
        var referralsTrendData = cfg.referralsTrendData;
        var funnelData = cfg.funnelData;
        var rewardDistributionData = cfg.rewardDistributionData;
        var i18n = cfg.i18n;
        ...
        var referralsTrendCtx = document.getElementById('referralsTrendChart');
            ...
            new Chart(referralsTrendCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: referralsTrendData.map(function(d) { return d.count; }),
                        borderColor: '#667eea',
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    var word = context.parsed.y !== 1 ? (i18n.referral_plural || 'referrals') : (i18n.referral_singular || 'referral');
        ...
        var funnelCtx = document.getElementById('conversionFunnelChart');
        ...
        var rewardDistCtx = document.getElementById('rewardDistributionChart');
        if (rewardDistCtx && rewardDistributionData && rewardDistributionData.length > 0) {
            new Chart(rewardDistCtx, {
                type: 'doughnut',
                data: {
                    labels: rewardDistributionData.map(function(d) { return d.name; }),
                    datasets: [{
                        data: rewardDistributionData.map(function(d) { return d.count; }),
                        backgroundColor: rewardDistributionData.map(function(d) { return d.color; }),
