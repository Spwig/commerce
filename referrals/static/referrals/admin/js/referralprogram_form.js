...
(function() {
    ...
    function getCfg() {
        ...
        var el = document.getElementById('referralprogram-form-config');
        if (el) {
            try { _cfg = JSON.parse(el.textContent || '{}'); } catch (e) { _cfg = {}; }
        }
        _cfg.loaded = true;
        _cfg.i18n = _cfg.i18n || {};
    ...
    function initConfigButtons() {
        ...
        document.getElementById('save-eligibility-config')?.addEventListener('click', saveEligibilityConfig);
    ...
    function updateRewardPreview() {
        ...
        if (!config.referrer && !config.referee) {
            preview.innerHTML = '<p class="empty-state">' + (cfg.i18n.configure_rewards_empty || "Click 'Configure Rewards' to set up your reward structure.") + '</p>';
            return;
        }

        var cs = cfg.currency_symbol;
        let html = '<div class="config-summary">';

        if (config.referrer?.enabled) {
            const amount = config.referrer.is_percentage ? `${config.referrer.amount}%` : `${cs}${config.referrer.amount}`;
        ...
        if (config.referee?.enabled) {
            const amount = config.referee.is_percentage ? `${config.referee.amount}%` : `${cs}${config.referee.amount}`;
    ...
    function saveEligibilityConfig() {
        const allowedCats = getFieldValue('elig-allowed-categories').split(',').filter(x => x.trim()).map(x => parseInt(x.trim()));
        const excludedProds = getFieldValue('elig-excluded-products').split(',').filter(x => x.trim()).map(x => parseInt(x.trim()));
    ...
    function updateEligibilityPreview() {
        ...
        if (Object.keys(config).length === 0) {
            preview.innerHTML = '<p class="empty-state">' + (cfg.i18n.configure_eligibility_empty || "Click 'Configure Eligibility' to set up participation rules.") + '</p>';
            return;
        }
        ...
        if (config.new_customers_only) {
            html += '<div class="summary-item">✓ ' + (cfg.i18n.new_customers_only || 'New customers only') + '</div>';
        }
    ...
    function updateTimingPreview() {
        ...
        if (Object.keys(config).length === 0) {
            preview.innerHTML = '<p class="empty-state">' + (cfg.i18n.configure_timing_empty || "Click 'Configure Timing' to set up reward timing rules.") + '</p>';
            return;
        }
    ...
    function updateCapsPreview() {
        ...
        if (Object.keys(config).length === 0) {
            preview.innerHTML = '<p class="empty-state">' + (cfg.i18n.configure_limits_empty || "Click 'Configure Limits' to set up reward caps.") + '</p>';
            return;
        }
    ...
    function updateTrackingPreview() {
        ...
        if (Object.keys(config).length === 0) {
            preview.innerHTML = '<p class="empty-state">' + (cfg.i18n.configure_tracking_empty || "Click 'Configure Tracking' to set up tracking settings.") + '</p>';
            return;
        }
    ...
    function updateFraudPreview() {
        ...
        if (Object.keys(config).length === 0) {
            preview.innerHTML = '<p class="empty-state">' + (cfg.i18n.configure_fraud_empty || "Click 'Configure Fraud Detection' to set up fraud prevention.") + '</p>';
            return;
        }
