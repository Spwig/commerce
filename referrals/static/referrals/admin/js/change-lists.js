...
(function() {
    ...
    function getI18n() {
        var el = document.getElementById('referrals-changelist-config');
        if (el) {
            try {
    ...
    function performAction(url) {
        ...
        .then(function(data) {
            if (data.success || data.status === 'ok') {
                ...
                AdminModal.alert({message: data.error || i18n.action_failed || 'Action failed. Please try again.', type: 'error'});
            }
        })
        .catch(function() {
            AdminModal.alert({message: i18n.error_performing_action || 'Error performing action. Please try again.', type: 'error'});
        });
    }
    ...
    function initActionButtons() {
        ...
        document.addEventListener('click', async function(e) {
            ...
            if (action === 'approve-attribution') {
                ...
                if (!await AdminModal.confirm({ message: i18n.confirm_reject_attribution || 'Reject this referral attribution?', danger: true, confirmText: 'Reject' })) return;
                ...
                if (!await AdminModal.confirm({ message: i18n.confirm_revoke_reward || 'Revoke this reward?', danger: true, confirmText: 'Revoke' })) return;
