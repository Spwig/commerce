...
def calculate_risk_score(attribution):
    ...
    if attribution.validation_data and 'fraud_signals' in attribution.validation_data:
        ...
        risk_score = 0
        for signal_name, signal_data in fraud_signals.items():
...
def check_ip_similarity(identity, order):
    ...
    referrer_events = ReferralEvent.objects.filter(
        referrer_identity=identity,
        created_at__gte=timezone.now() - timedelta(days=30)
    ).values_list('ip_address', flat=True).distinct()

    # Get referee's recent IP addresses
    referee_events = ReferralEvent.objects.filter(
        customer=order.user,
        created_at__gte=timezone.now() - timedelta(days=30)
    ).values_list('ip_address', flat=True).distinct()
    ...
    return 0, {
        'matched': False,
        'risk_points': 0,
        'message': 'No IP similarity detected'
    }
...
def check_device_fingerprint(identity, order):
    ...
    referrer_fingerprints = ReferralEvent.objects.filter(
        referrer_identity=identity,
        created_at__gte=timezone.now() - timedelta(days=30)
    ).values_list('device_fingerprint', flat=True).distinct()

    # Get referee's device fingerprints
    referee_fingerprints = ReferralEvent.objects.filter(
        customer=order.user,
        created_at__gte=timezone.now() - timedelta(days=30)
    ).values_list('device_fingerprint', flat=True).distinct()
