...
def get_referral_dashboard_stats(start_date=None, end_date=None):
    ...
    attributions_qs = ReferralAttribution.objects.filter(
    ...
    # Overall metrics
    total_referrers = ReferralIdentity.objects.filter(
        referrals_made__created_at__range=(start_date, end_date)
    ).distinct().count()

    total_conversions = attributions_qs.filter(
        status='approved'
    ).count()

    total_rewards_value = rewards_qs.filter(
    ...
    avg_risk_score = ReferralAttribution.objects.filter(
        status='pending'
    ).aggregate(
        avg=Avg('risk_score')
    )['avg'] or 0
    ...
    top_referrers = list(
        ReferralIdentity.objects.filter(
            ...
            conversions=Count('referrals_made', filter=Q(
                referrals_made__status='approved',
                referrals_made__created_at__range=(start_date, end_date)
            )),
            total_rewards=Sum('referrals_made__rewards__amount', filter=Q(
                referrals_made__rewards__status__in=['issued', 'redeemed'],
                referrals_made__created_at__range=(start_date, end_date)
            ))
        ).filter(
            conversions__gt=0
        ).order_by('-conversions')[:5].values(
            'id',
