...
(function () {
  ...
  const configEl = document.getElementById('promotion-wizard-step3-config');
  let i18n = {};
  if (configEl) {
    try {
  ...
  function addSelectButtons(container, fieldName) {
    ...
    buttonsDiv.innerHTML =
      '<button type="button" class="btn-select-all" data-field="' +
      fieldName +
      '">' +
      '<i class="fas fa-check-double"></i> ' +
      (i18n.selectAll || 'Select All') +
      '</button>' +
      '<button type="button" class="btn-select-none" data-field="' +
      fieldName +
      '">' +
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const zoneCheckboxGroup = document.querySelector(
      '.form-section:nth-of-type(2) .checkbox-group'
    );
    ...
    const methodCheckboxGroup = document.querySelector(
      '.form-section:nth-of-type(3) .checkbox-group'
    );
