...
(function () {
      ...
      if (wrapper) {
        ...
        if (valueText) {
          valueText.textContent =
            countryFilter.getAttribute('data-placeholder') || 'Search countries...';
        }
        // Clear the icon
  ...
  async function executeBulkAction(action) {
    ...
    if (action === 'delete') {
      if (
        !(await AdminModal.confirm({
          message: `Are you sure you want to delete ${selectedIds.length} carrier(s)?`,
          danger: true,
          confirmText: 'Delete',
        }))
      ) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl('/admin/shipping/carrierpreset/bulk-action/'),
        {
          method: 'POST',
          headers: {
  ...
  async function toggleCarrierActive(carrierId, button) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/shipping/carrierpreset/${carrierId}/toggle-active/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async function setCarrierDefault(carrierId, button) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/shipping/carrierpreset/${carrierId}/set-default/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async function deleteCarrier(carrierId, button) {
    ...
    if (
      !(await AdminModal.confirm({
        message: `Are you sure you want to delete "${carrierName}"?`,
        danger: true,
        confirmText: 'Delete',
      }))
    ) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/shipping/carrierpreset/${carrierId}/delete/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
