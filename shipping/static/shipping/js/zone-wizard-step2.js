...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const zoneConfigEl = document.getElementById('zone-wizard-config');
    let getStatesUrl = '';
    if (zoneConfigEl) {
      try {
        const zoneConfig = JSON.parse(zoneConfigEl.textContent);
    ...
    function updateCountryCounts() {
      const availableCount = availableCountriesEl.querySelectorAll(
        '.listbox-item:not(.hidden)'
      ).length;
    ...
    function updateStateCounts() {
      ...
      const availableCount = availableStatesEl.querySelectorAll(
        '.listbox-item:not(.hidden)'
      ).length;
    ...
    function updateStatesList(statesData) {
      ...
      const selectedStateCodes = Array.from(selectedStatesEl.querySelectorAll('.listbox-item')).map(
