...
window.ZoneEditForm = (function () {
  ...
  function updateCountriesField() {
    ...
    const selectedCodes = Array.from(selectedList.querySelectorAll('.listbox-item')).map(
  ...
  function loadStatesForSelectedCountries() {
    ...
    const selectedCountries = Array.from(
      selectedCountriesList.querySelectorAll('.listbox-item')
    ).map(item => item.dataset.code);

    // If no countries selected, clear states
    if (selectedCountries.length === 0) {
      availableStatesEl.innerHTML =
        '<div class="info-box"><p>Select countries above to load states.</p></div>';
    ...
    // Show loading
    availableStatesEl.innerHTML =
      '<div class="info-box"><i class="fas fa-spinner fa-spin"></i> Loading states...</div>';
    ...
    fetch(url, {
      ...
      .catch(error => {
        ...
        availableStatesEl.innerHTML =
          '<div class="info-box" style="border-color: #dc3545;"><i class="fas fa-exclamation-triangle"></i> Failed to load states.</div>';
      });
  }
...
function initSaveButtons() {
  const form = document.getElementById('shippingzone-form');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const configEl = document.getElementById('zone-edit-config');
  if (!configEl) return;
  let config;
  try {
