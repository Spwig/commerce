...
(function () {
  ...
  function applyFilters() {
    state.filteredPromotions = state.allPromotions.filter(rule => {
      ...
      if (state.currentFilters.priority) {
        ...
        if (state.currentFilters.priority === 'medium' && (priority < 0 || priority >= 50))
          return false;
      ...
      if (
        ...
        !rule.name.toLowerCase().includes(state.currentFilters.search)
      ) {
        return false;
      }
  ...
  async function handleTogglePromotion(e) {
    ...
    if (await AdminModal.confirm(`Are you sure you want to ${action} this promotion?`)) {
      showLoading();
