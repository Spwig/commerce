...
(function() {
    ...
    var configEl = document.getElementById('promotion-wizard-step1-config');
    var i18n = {};
    if (configEl) {
        try {
    ...
    function updateRuleValueField() {
        var ruleTypeEl = document.getElementById('id_rule_type');
        if (\!ruleTypeEl) return;

        var ruleType = ruleTypeEl.value;
        var valueGroup = document.getElementById('rule-value-group');
        var valueInput = document.getElementById('id_rule_value');
        var valueLabel = document.getElementById('rule-value-label');
        var valueHelpText = document.getElementById('rule-value-help-text');
        ...
        if (ruleType === 'free_shipping') {
            ...
            if (ruleType === 'discount_percentage' || ruleType === 'surcharge_percentage') {
                ...
                if (valueHelpText) valueHelpText.textContent = i18n.enterPercentage || 'Enter percentage (0-100)';
                valueInput.setAttribute('max', '100');
            } else if (ruleType === 'discount_fixed' || ruleType === 'surcharge_fixed') {
                if (valueLabel) valueLabel.textContent = i18n.amount || 'Amount';
                if (valueHelpText) valueHelpText.textContent = i18n.enterAmount || 'Enter fixed amount in your default currency';
                valueInput.removeAttribute('max');
            } else if (ruleType === 'set_cost') {
                if (valueLabel) valueLabel.textContent = i18n.shippingCost || 'Shipping Cost';
                if (valueHelpText) valueHelpText.textContent = i18n.enterShippingCost || 'Enter the fixed shipping cost';
                valueInput.removeAttribute('max');
            }
        }
    }
