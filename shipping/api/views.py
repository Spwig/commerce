...
from shipping.models import CarrierPreset, ProviderAccount, Shipment, TrackingEvent
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_(
            "Get all active shipping carrier presets available for creating shipments. Returns carrier name, slug, logo, and supported service types."
        ),
    ),
    retrieve=extend_schema(
        ...
        description=_(
            "Get detailed information about a specific carrier preset including configuration, service types, and integration status."
        ),
    ),
)
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_(
            "Get all shipments for the authenticated user. Staff members can view all shipments. Supports filtering by status, order, carrier, and provider."
        ),
    ),
    retrieve=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Get detailed information about a specific shipment including tracking number, status, carrier details, and tracking events."
        ),
    ),
    create=extend_schema(
        ...
        description=_(
            "Create a new shipment for an order. Requires order ID, carrier preset, shipping address, and package details. Optionally integrates with shipping provider APIs to generate labels."
        ),
    ),
    update=extend_schema(
        ...
        description=_(
            "Update shipment information (staff only). Can modify tracking number, status, carrier details, and package information."
        ),
    ),
    partial_update=extend_schema(
        ...
        description=_(
            "Partially update shipment fields (staff only). Useful for updating specific fields like status or tracking number without sending full shipment data."
        ),
    ),
    destroy=extend_schema(
        ...
        description=_(
            "Delete a shipment record (staff only). Caution: This is permanent and cannot be undone."
        ),
    ),
    tracking=extend_schema(
        ...
        description=_(
            "Get all tracking events for a shipment in chronological order. Includes status updates, location changes, and delivery confirmations."
        ),
    ),
    by_order=extend_schema(
        ...
        description=_(
            "Get all shipments associated with a specific order. Useful for orders with multiple packages or split shipments."
        ),
    ),
    packing_slip=extend_schema(
        ...
        description=_(
            "Generate and retrieve packing slip PDF for this shipment. Returns base64-encoded data URI. Cached after first generation for performance."
        ),
    ),
    commercial_invoice=extend_schema(
        ...
        description=_(
            "Generate commercial invoice PDF for international shipments. Includes customs data, HS codes, country of origin, and item values. Required for customs clearance."
        ),
    ),
    customs_form=extend_schema(
        ...
        description=_(
            "Generate customs declaration form (CN22 for small parcels or CN23 for larger shipments). Required for international shipping with postal services."
        ),
    ),
)
class ShipmentViewSet(HeadlessAPIMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            Shipment.objects.filter(order__user=user)
            .select_related(
                "order", "user", "carrier_preset", "provider_account", "provider_account__component"
            )
            .prefetch_related("tracking_events")
            .all()
        )
    ...
    def by_order(self, request):
        ...
        if not order_id:
            return Response(
                {"error": "order_id parameter is required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_(
            "Get all tracking events for the authenticated user's shipments. Staff can view all events. Filter by shipment ID or status. Ordered by occurrence date (newest first)."
        ),
    ),
    retrieve=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Get detailed information about a specific tracking event including timestamp, location, status code, and description."
        ),
    ),
)
class TrackingEventViewSet(HeadlessAPIMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            TrackingEvent.objects.filter(shipment__order__user=user)
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=_(
            "Get all shipping provider accounts for the authenticated user. Staff can view all accounts. Returns account names, providers, and connection status. Credentials are never exposed."
        ),
    ),
    retrieve=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Get detailed information about a specific provider account including display name, provider type, connection status, and configuration. API credentials are hidden for security."
        ),
    ),
    create=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Create a new shipping provider account (staff only). Requires provider component slug, display name, and API credentials. Credentials are encrypted before storage."
        ),
    ),
    update=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Update provider account settings including display name and credentials. Credentials are encrypted. Previous credentials are replaced, not merged."
        ),
    ),
    partial_update=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Update specific fields of a provider account. Useful for changing display name or connection settings without resending credentials."
        ),
    ),
    destroy=extend_schema(
        tags=["Shipping"],
        ...
        description=_(
            "Delete a shipping provider account. Warning: This will disable all shipments using this account. Ensure you have migrated to another account first."
        ),
    ),
    test_connection=extend_schema(
        ...
        description=_(
            "Test connection to the shipping provider API using stored credentials. Verifies authentication and API accessibility. Updates account connection status based on result."
        ),
    ),
)
