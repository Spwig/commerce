...
class ShippingCountryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Country Information"), {"fields": ("site", "country_code", "is_active")}),
        (
    ...
    def country_flag_and_name(self, obj):
        ...
        return format_html(
            '<div class="shipping-country-flag-name"><img src="{}" /><span>{}</span></div>',
            country.flag,
            country.name,
        )
    ...
    def source_warehouse_display(self, obj):
        ...
        return format_html(
            '<span class="shipping-text-muted"><em>{}</em></span>', _("Any warehouse")
        )
...
class CarrierPresetAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {"all": ("shipping/css/admin_carrier_preset.css",)}
    ...
    fieldsets = (
        ...
        (
            _("Branding"),
            {
                ...
                "description": mark_safe(
                    ...
                    "<strong>"
                    + str(_("Logo Attribution:"))
                    + "</strong> "
                    + str(
                        _(
                            "Carrier logos are trademarks of their respective owners and are used solely for identification and reference purposes. No affiliation or endorsement is implied."
                        )
                    )
                    + "</div>"
                ),
        ...
        (
            _("Tracking Configuration"),
            {
                ...
                "description": mark_safe(
                    '<div class="info-box">'
                    "<strong>"
                    + str(_("URL Override System:"))
                    + "</strong><br/>"
                    + str(
                        _(
                            "System carriers come with default tracking URLs. You can override these URLs if needed."
                        )
                    )
                    + "<br/>"
                    + str(
                        _(
                            "Use {tracking_number} as the placeholder for the tracking number in your URL."
                        )
                    )
                    + "<br/>"
                    "<strong>"
                    + str(_("Example:"))
                    + "</strong> <code>https://example.com/track?id={tracking_number}</code><br/>"
                    + str(
                        _(
                            "Your override will be reported to our update server to help improve default URLs for all users."
                        )
                    )
                    + "</div>"
                ),
        ...
        (_("Status"), {"fields": ("is_active", "is_default", "is_system")}),
        (
            _("Metadata"),
    ...
    def logo_preview_full(self, obj):
        ...
        if logo_info["type"] == "image":
            return format_html(
                '<div class="logo-preview-full"><img src="{}" /></div>', logo_info["url"]
            )
        else:
            return format_html(
                '<div class="logo-preview-full"><i class="{}"></i><p>{}</p></div>',
                logo_info["class"],
                _("No logo uploaded"),
            )
    ...
    def country_badge(self, obj):
        ...
        if obj.country_of_operation:
            return format_html(
                '<div class="country-badge" title="{}"><img src="{}" /><span>{}</span></div>',
    ...
    def tracking_url_display(self, obj):
        ...
        if obj.tracking_url_template:
            return format_html(
                '<div class="url-display-box"><strong>{}</strong><br/><code>{}</code></div>',
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Country counts
        country_counts = (
            all_carriers.exclude(country_of_operation="")
            .exclude(country_of_operation=None)
...
class ShippingPackageAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        "created_at",
        "updated_at",
        "volume_display_detailed",
        "external_dimensions_display",
    ]
    ...
    fieldsets = (
        ...
        (
            _("Dimensions"),
            {
                ...
                "description": mark_safe(
                    ...
                    + str(
                        _(
                            "• <strong>Internal dimensions</strong> are used for packing algorithm calculations (determining what fits inside)"
                        )
                    )
                    + "<br/>"
                    + str(
                        _(
                            "• <strong>Wall thickness</strong> defines the package material thickness (default 0.5cm for standard cardboard)"
                        )
                    )
                    + "<br/>"
                    + str(
                        _(
                            "• <strong>External dimensions</strong> are automatically calculated and sent to shipping carriers for rate calculations"
                        )
                    )
                    + "</p>"
                    "</div>"
        ...
        (
            _("Configuration"),
            {
                ...
                "description": _(
                    "Package cost and selection priority (higher = preferred for auto-packing)"
                ),
            },
        ),
        (_("Metadata"), {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    ...
    def volume_display_detailed(self, obj):
        ...
        if volume_cm3 is None or volume_liters is None:
            return format_html(
                '<em class="shipping-em-quiet">{}</em>', _("Enter dimensions to calculate volume")
            )

        return format_html("<strong>{:.2f} cm³</strong> ({:.2f} liters)", volume_cm3, volume_liters)
    ...
    def external_dimensions_display(self, obj):
        ...
        if external_dims is None or external_volume is None:
            return format_html(
                '<div class="shipping-dims-empty"><em>{}</em></div>',
...
class ProviderAccountAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {"all": ("shipping/css/admin_provider_account.css", "shipping/css/admin_badges.css")}
            ...
            return (
                (
                    _("Provider Information"),
                    {
                        ...
                        "description": _(
                            "Component and User cannot be changed after creation for security and audit purposes."
                        ),
                    },
                ),
                ...
                (
                    _("Connection Health"),
                    {
                        ...
                        "classes": ("collapse",),
                        "description": _(
                            "Connection health is automatically updated when you test the connection from the wizard."
                        ),
                    },
                ),
                (_("Metadata"), {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
                ...
                (
                    _("Create Provider"),
                    {
                        ...
                        "description": mark_safe(
                            ...
                            '<div class="warning">'
                            "<strong>"
                            + str(_("Please use the Provider Connection Wizard"))
                            + "</strong><br/>"
                            + str(
                                _(
                                    'For the best experience, please use the <a href="/admin/shipping/wizard/step1/">Provider Connection Wizard</a> to set up new providers.'
                                )
                            )
                            + "</div>"
                            "</div>"
    ...
    def connection_status_badge(self, obj):
        css_map = {
            "connected": "shipping-badge-status-connected",
            "error": "shipping-badge-status-error",
            "unknown": "shipping-badge-status-unknown",
        }
        css_class = css_map.get(obj.connection_status, "shipping-badge-status-unknown")
        return format_html(
            '<span class="shipping-badge-status {}">{}</span>',
            css_class,
...
class ShipmentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Shipping Details"),
            {"fields": ("origin_country", "dest_country", "service_level", "packages")},
        ),
        (
            _("Costs"),
            {
                "fields": ("shipping_cost", "carrier_cost", "pricing_mode_used"),
                "classes": ("collapse",),
            },
        ),
    ...
    def provider_display(self, obj):
        if obj.carrier_preset:
            return format_html(
                '<i class="fas fa-warehouse shipping-icon-warehouse"></i> {}',
                obj.carrier_preset.name,
            )
        elif obj.provider_account:
            return format_html(
                '<i class="fas fa-plug shipping-icon-provider"></i> {}',
                obj.provider_account.display_name or obj.provider_account.component.name,
            )
        return "-"
    ...
    def tracking_id_link(self, obj):
        ...
        if tracking_url:
            return format_html(
                '<a href="{}" target="_blank" rel="noopener">{} <i class="fas fa-external-link-alt shipping-icon-external"></i></a>',
                tracking_url,
                obj.tracking_id,
            )
    ...
    def tracking_url_display(self, obj):
        ...
        if tracking_url:
            return format_html(
                '<a href="{}" target="_blank" rel="noopener">{}</a>', tracking_url, tracking_url
            )
        return "-"
    ...
    def packing_slip_preview(self, obj):
        ...
        if not obj.packing_slip_url:
            return format_html(
                '<em class="shipping-em-muted">{}</em>', _("No packing slip available")
            )
    ...
    def commercial_invoice_preview(self, obj):
        ...
        if not obj.commercial_invoice_url:
            return format_html(
                '<em class="shipping-em-muted">{}</em>', _("No commercial invoice available")
            )
    ...
    def customs_form_preview(self, obj):
        ...
        if not obj.customs_form_url:
            return format_html(
                '<em class="shipping-em-muted">{}</em>', _("No customs form available")
            )
    ...
    def purchase_labels(self, request, queryset):
        ...
        valid_shipments = queryset.filter(
            status="created", provider_account__isnull=False, provider_account__is_active=True
        )
        ...
        if not valid_shipments.exists():
            self.message_user(
                request,
                _(
                    'No valid shipments selected. Shipments must be in "created" status and have an active provider account.'
                ),
                level="warning",
            )
            return
        ...
        for shipment in valid_shipments:
            try:
                ...
                self.message_user(
                    ...
                    % {"id": str(shipment.id)[:8], "error": str(e)},
                    level="error",
                )
        ...
        if queued_count > 0:
            self.message_user(
                request,
                _(
                    "Successfully queued %(count)d label purchase task(s). Labels will be generated in the background."
                )
                % {"count": queued_count},
                level="success",
            )
    ...
    generate_commercial_invoices.short_description = _(
        "Generate commercial invoices for selected shipments"
    )
...
class TrackingEventAdmin(admin.ModelAdmin):
    ...
    def shipment_link(self, obj):
        if obj.shipment:
            ...
            return format_html(
                '<a href="{}">{}</a>', url, obj.shipment.tracking_id or str(obj.shipment.id)[:8]
            )
        return "-"
    ...
    def changelist_view(self, request, extra_context=None):
        """Add context for the custom change list template"""
        extra_context = extra_context or {}
        extra_context["info_received_count"] = TrackingEvent.objects.filter(
            status="info_received"
        ).count()
        extra_context["in_transit_count"] = TrackingEvent.objects.filter(
            status="in_transit"
        ).count()
        extra_context["out_for_delivery_count"] = TrackingEvent.objects.filter(
...
class WebhookLogAdmin(admin.ModelAdmin):
    ...
    def processing_status_badge(self, obj):
        css_map = {
            "pending": "shipping-badge-status-pending",
            "processed": "shipping-badge-status-processed",
            "failed": "shipping-badge-status-failed",
        }
        css_class = css_map.get(obj.processing_status, "shipping-badge-status-pending")
        return format_html(
            '<span class="shipping-badge-status {}">{}</span>',
            css_class,
            obj.get_processing_status_display().upper(),
        )
    ...
    def changelist_view(self, request, extra_context=None):
        """Add context for the custom change list template"""
        extra_context = extra_context or {}
        extra_context["pending_count"] = WebhookLog.objects.filter(
            processing_status="pending"
        ).count()
        extra_context["processed_count"] = WebhookLog.objects.filter(
            processing_status="processed"
        ).count()
        extra_context["failed_count"] = WebhookLog.objects.filter(
            processing_status="failed"
        ).count()
        extra_context["providers"] = WebhookLog.objects.values_list(
...
class ShippingZoneAdmin(admin.ModelAdmin):
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        all_countries = [{"code": code, "name": str(name)} for code, name in django_countries_list]
    ...
    class Media:
        css = {"all": ("shipping/css/admin_shipping_zone.css",)}
        js = ("shipping/js/admin_shipping_zone.js",)

    fieldsets = (
        (_("Zone Information"), {"fields": ("name", "description", "priority", "is_active")}),
        (
            _("Geographic Coverage"),
            {
                ...
                "description": mark_safe(
                    '<div class="info-box">'
                    "<strong>"
                    + str(_("Zone Configuration:"))
                    + "</strong><br/>"
                    + str(_("Define the geographic areas covered by this shipping zone."))
                    + "<br/>"
                    + str(
                        _(
                            '- <strong>Countries:</strong> List of ISO 3166-1 alpha-2 country codes (e.g., ["US", "CA", "MX"]). Leave empty for all countries.'
                        )
                    )
                    + "<br/>"
                    + str(
                        _(
                            '- <strong>States:</strong> Dict mapping country codes to state codes (e.g., {"US": ["CA", "NY"], "CA": ["ON", "BC"]}).'
                        )
                    )
                    + "<br/>"
                    + str(
                        _(
                            '- <strong>Postal Patterns:</strong> List of regex patterns for postal codes (e.g., ["^90[0-9]{3}$"] for Los Angeles area).'
                        )
                    )
                    + "</div>"
                ),
        ...
        (
            _("Coverage Summary"),
            {"fields": ("coverage_details_display",), "classes": ("collapse",)},
        ),
        (
            _("Metadata"),
            {"fields": ("created_by", "created_at", "updated_at"), "classes": ("collapse",)},
    ...
    def coverage_details_display(self, obj):
        ...
        if obj.countries:
            html += (
                '<div class="coverage-section"><strong>' + str(_("Countries:")) + "</strong><br/>"
            )
        ...
        if obj.states:
            html += (
                '<div class="coverage-section"><strong>'
                + str(_("States/Provinces:"))
                + "</strong><br/>"
            )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["medium_priority_count"] = all_zones.filter(
            priority__gt=5, priority__lte=10
        ).count()
        extra_context["low_priority_count"] = all_zones.filter(priority__gt=10).count()

        # Zones with/without methods
        extra_context["zones_with_methods"] = (
            all_zones.annotate(methods_count=Count("shipping_methods"))
            .filter(methods_count__gt=0)
            .count()
        )
        extra_context["zones_without_methods"] = (
            all_zones.annotate(methods_count=Count("shipping_methods"))
    ...
    def get_queryset(self, request):
        ...
        qs = super().get_queryset(request).annotate(methods_count=Count("shipping_methods"))
...
class ShippingRateTableAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Table Configuration"),
            {
                ...
                "description": _(
                    "Basis type determines what value is used for tier lookup: weight, price, or quantity"
                ),
            },
        ),
        (
    ...
    def is_active_badge(self, obj):
        ...
        if obj.is_active:
            return format_html(
                '<span class="shipping-text-success-bold">&#10003; {}</span>', _("Active")
            )
        return format_html('<span class="shipping-text-muted">&#9675; {}</span>', _("Inactive"))
...
class ShippingPromotionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            _("Basic Information"),
            {"fields": ("name", "description", "is_active"), "classes": ("tab-basic",)},
        ),
        (
            _("Promotion Action"),
            {"fields": ("promotion_type", "promotion_value"), "classes": ("tab-action",)},
        ),
        (
            _("Cart Conditions"),
            {
        ...
        (_("Geographic Restrictions"), {"fields": ("zones",), "classes": ("tab-zones",)}),
        (
            _("Shipping Method Restrictions"),
            {"fields": ("shipping_methods",), "classes": ("tab-methods",)},
        ),
        (
        ...
        (_("Conditions Summary"), {"fields": ("conditions_summary",), "classes": ("tab-summary",)}),
        (
            _("Metadata"),
            {"fields": ("created_at", "updated_at", "created_by"), "classes": ("tab-metadata",)},
        ),
    )
    ...
    def promotion_type_badge(self, obj):
        ...
        return format_html(
            '<span class="{}">{}</span>', css_class, obj.get_promotion_type_display()
        )
    ...
    def time_valid_badge(self, obj):
        ...
        if obj.is_time_valid():
            return format_html(
                '<span class="shipping-text-success">&#10003; {}</span>', _("Valid now")
            )
        return format_html('<span class="shipping-text-danger">&#10007; {}</span>', _("Not valid"))
    ...
    def time_validity_display(self, obj):
        ...
        if obj.end_date:
            if now > obj.end_date:
                ...
                parts.append(
                    format_html(
                        '<span class="shipping-text-success">Valid until {}</span>', obj.end_date
                    )
                )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["free_shipping_count"] = queryset.filter(
            promotion_type="free_shipping"
        ).count()
        extra_context["discount_percentage_count"] = queryset.filter(
            promotion_type="discount_percentage"
        ).count()
        extra_context["discount_fixed_count"] = queryset.filter(
            promotion_type="discount_fixed"
        ).count()
        extra_context["override_cost_count"] = queryset.filter(
            promotion_type="override_cost"
        ).count()
        extra_context["surcharge_fixed_count"] = queryset.filter(
            promotion_type="surcharge_fixed"
        ).count()
        extra_context["surcharge_percentage_count"] = queryset.filter(
        ...
        extra_context["medium_priority_count"] = queryset.filter(
            priority__gte=0, priority__lt=50
        ).count()
...
class LocationAdmin(admin.ModelAdmin):
    ...
    def location_type_badge(self, obj):
        ...
        return format_html('<span class="{}">{}</span>', css_class, obj.get_location_type_display())
