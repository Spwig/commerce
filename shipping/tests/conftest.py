"""
Local conftest for shipping/tests/.

Provides autouse core-infrastructure fixtures the shared ``tests/conftest.py``
defines. This is required because the shipping tests use Django's ``TestCase``
(not pytest fixtures) but still hit middleware / views / signals that call
``SiteSettings.get_settings()`` — the shared fixtures are not autouse.

* ``_shipping_site_settings`` -- autouse: materialises a single-tenant
  ``SiteSettings`` row so currency/i18n middleware can resolve without
  triggering ``full_clean`` on an empty ``admin_email`` field.
* ``_shipping_django_site`` -- autouse: ensures the ``django.contrib.sites``
...
def _shipping_site_settings(db):
