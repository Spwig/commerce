...
class ShippingRateTableModelTest(TestCase):
    ...
    def test_applies_to_address_delegates_to_shipping_method(self):
        """Rate table zone restrictions are inherited from parent shipping method"""
        # ShippingRateTable.applies_to_address currently returns True and defers
        # actual zone filtering to the parent ShippingMethod. This test locks
        # in that documented behavior — do not add a zones M2M to the table
        # without updating this test and the model docstring.
        table = ShippingRateTable.objects.create(
            name="USA Rates",
            basis_type="weight",
            shipping_method=self.shipping_method,
        ...
        # Rate table always returns True — the shipping method enforces zones
    ...
    def test_applies_to_address_no_zone_restriction(self):
        """Test rate table applies to any address (zone check delegated to method)"""
        table = ShippingRateTable.objects.create(
...
class ShippingPromotionServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        # Create address
        self.address = {"country": "US", "state": "CA", "postal_code": "90210"}
    ...
    def test_calculate_rate_table_cost_weight_based(self):
        ...
        cost = ShippingRuleService.calculate_rate_table_cost(
            shipping_method=self.shipping_method, cart=self.cart, address=self.address
        )
...
class CartShippingIntegrationTest(TestCase):
    ...
    def test_calculate_shipping_with_method_and_rules(self):
        ...
        result = self.cart.calculate_shipping(
            shipping_address=self.address, shipping_method=self.shipping_method, user=self.user
        )
