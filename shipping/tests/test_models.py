...
class CarrierPresetModelTest(TestCase):
    ...
    def test_system_carrier_presets_loaded(self):
        """Test that system carrier presets were loaded via migration"""
        # Check for some known carriers
        dhl = CarrierPreset.objects.filter(slug='dhl-express').first()
        fedex = CarrierPreset.objects.filter(slug='fedex').first()
        ups = CarrierPreset.objects.filter(slug='ups').first()

        self.assertIsNotNone(dhl)
        self.assertIsNotNone(fedex)
        self.assertIsNotNone(ups)

        self.assertTrue(dhl.is_system)
        self.assertTrue(fedex.is_system)
        self.assertTrue(ups.is_system)
