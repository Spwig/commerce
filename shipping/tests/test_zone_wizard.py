...
class ZoneWizardStep2Test(TestCase):
    ...
    def test_step2_post_with_countries(self):
        """Test step 2 POST with country codes (dual-listbox format)"""
        # Step 2 view reads selected_countries[] as a list POST param
        # (dual-listbox multi-select), not a comma-separated string.
        data = {
    ...
    def test_step2_post_with_states_json(self):
        """Test step 2 POST with states (dual-listbox 'CC:SS' format)"""
        # New format: states arrive as selected_states[] with 'COUNTRY:STATE'
        # entries, then are grouped into a dict by country in the view.
        data = {
            "selected_countries[]": ["US", "CA"],
            "selected_states[]": ["US:CA", "US:OR", "US:WA", "CA:BC", "CA:ON"],
            "postal_patterns": "",
        }
        ...
        self.assertEqual(sorted(wizard_data["states"]["US"]), ["CA", "OR", "WA"])
        self.assertEqual(sorted(wizard_data["states"]["CA"]), ["BC", "ON"])

    def test_step2_post_with_postal_patterns(self):
        """Test step 2 POST with postal code patterns (newline-separated)"""
        # New format: postal_patterns is newline-separated, not comma-separated
        data = {
            "selected_countries[]": ["US"],
            "selected_states[]": [],
            "postal_patterns": "^10\\d{3}$\n^11\\d{3}$",
        }
...
class ZoneWizardNavigationTest(TestCase):
    ...
    def test_complete_wizard_flow(self):
        ...
        # Step 2 — new dual-listbox POST format
        step2_url = reverse("shipping:zone_wizard_step2")
        response = self.client.post(
            step2_url,
            {
                "selected_countries[]": ["US", "CA"],
                "selected_states[]": ["US:CA", "US:NY"],
