...
class DocumentService:
    ...
    def generate_commercial_invoice(shipment) -> str:
        ...
        buffer, doc = DocumentService._create_pdf_buffer(
            f"Commercial Invoice - {shipment.tracking_id}", pagesize=A4
        )
        styles = DocumentService._get_styles()
        story = []
        ...
        items_table = Table(
            items_data,
            colWidths=[2 * inch, 0.8 * inch, 0.6 * inch, 0.5 * inch, 0.8 * inch, 0.8 * inch],
        )
        items_table.setStyle(
            TableStyle(
                [
        ...
        if shipment.order.tax_amount and shipment.order.tax_amount.amount > 0:
            totals_data.append(
                [_("Tax:"), DocumentService._format_money(shipment.order.tax_amount)]
            )
        ...
        totals_data.append(
            [
                ...
                Paragraph(
                    "<b>" + DocumentService._format_money(shipment.order.total_amount) + "</b>",
                    styles["Normal_Right"],
                ),
            ]
        )
    ...
    def generate_customs_form(shipment, form_type: str = "CN22") -> str:
        ...
        buffer, doc = DocumentService._create_pdf_buffer(
            f"{form_type} - {shipment.tracking_id}", pagesize=pagesize
        )
        styles = DocumentService._get_styles()
        story = []
        ...
        # Form title
        story.append(
            Paragraph(f"<b>{form_type}</b> - " + _("Customs Declaration"), styles["DocumentTitle"])
        )
        story.append(Spacer(1, 0.1 * inch))
        ...
        items_data = [
            [_("Description"), _("Qty"), _("Weight (g)"), _("Value"), _("Origin"), _("HS Code")]
        ]
        ...
        items_table = Table(
            items_data,
            colWidths=[1.5 * inch, 0.5 * inch, 0.7 * inch, 0.6 * inch, 0.6 * inch, 0.8 * inch],
        )
        items_table.setStyle(
            TableStyle(
                [
    ...
    def generate_return_label(return_request):
        ...
        # Create PDF
        buffer, doc = DocumentService._create_pdf_buffer(
            f"Return_Label_{return_request.id}", pagesize=letter
        )
        ...
        rma_paragraph = Paragraph(
            ...
            ParagraphStyle(
                name="RMANumber", fontSize=16, textColor=colors.HexColor("#e74c3c"), spaceAfter=12
            ),
        )
        ...
        for item_data in return_request.items_json:
            try:
                ...
                logger.warning(
                    f"Could not retrieve order item {item_data.get('order_item_id')}: {e}"
                )
                continue
