...
class ShippingPromotionService:
    ...
    def evaluate_rules(
        ...
        rules = ShippingPromotion.objects.filter(
            is_active=True
        ).select_related(
            'created_by'
        ).prefetch_related(
    ...
    def get_rate_from_table(
        ...
        if address and not rate_table.applies_to_address(address):
            logger.debug(
                f"Rate table '{rate_table.name}' does not apply to address"
            )
            return None
        ...
        if rate:
            logger.info(
                f"Rate table '{rate_table.name}' ({rate_table.basis_type}): "
                f"{value} → {rate}"
            )
        else:
            logger.warning(
                f"No matching tier in rate table '{rate_table.name}' "
                f"for value {value}"
            )
    ...
    def calculate_rate_table_cost(
        ...
        if not rate_tables.exists():
            logger.warning(
                f"No active rate tables found for shipping method "
                f"'{shipping_method.name}'"
            )
            return None
        ...
        logger.info(
            f"No matching rates found in any table for shipping method "
            f"'{shipping_method.name}'"
        )
        return None
    ...
    def calculate_shipping_for_cart(
        ...
        logger.info(
            f"Calculating shipping for cart with method '{shipping_method.name}'"
        )
        ...
        if shipping_method.method_type == 'table_rate':
            ...
            if base_cost is None:
                ...
                base_cost = Money(
                    shipping_method.flat_rate_cost.amount
                    if shipping_method.flat_rate_cost
                    else 0,
                    currency
                )
        else:
        ...
        if calculation['rules_applied']:
            breakdown_parts.append(
                f"Promotions applied: {len(calculation['rules_applied'])}"
            )
        ...
        if calculation['total_discount'].amount > 0:
            breakdown_parts.append(
                f"Total discount: {calculation['total_discount']}"
            )

        if calculation['total_surcharge'].amount > 0:
            breakdown_parts.append(
                f"Total surcharge: {calculation['total_surcharge']}"
            )
