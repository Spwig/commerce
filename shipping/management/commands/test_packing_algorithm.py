...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, result in enumerate(results, 1):
            ...
            self.stdout.write(
                f"    Internal Dimensions: {result.package.length}×{result.package.width}×{result.package.height}cm (used for packing)"
            )
            self.stdout.write(
                f"    External Dimensions: {external_dims['length']}×{external_dims['width']}×{external_dims['height']}cm (sent to carriers)"
            )
            ...
            self.stdout.write(
                f"    Total Weight: {result.total_weight:.3f}kg (includes {result.package.tare_weight}kg package)"
            )
            self.stdout.write(
                f"    Weight Utilization: {result.weight_utilization:.1f}% ({result.total_weight:.3f}kg / {result.package.max_weight}kg)"
            )
        ...
        if items_packed == items_total:
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ All {items_total} items successfully packed!")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"  ⚠ Only {items_packed} of {items_total} items packed")
            )
