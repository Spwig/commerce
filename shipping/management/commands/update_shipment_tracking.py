...
from shipping.models import Shipment, TrackingEvent, ProviderAccount
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for shipment in shipments:
            ...
            try:
                ...
                logger.error(f"Failed to update tracking for shipment {shipment.id}: {e}", exc_info=True)
    ...
    def _update_shipment_tracking(self, shipment, dry_run=False):
        ...
        if old_status != new_status:
            self.stdout.write(
                f"[UPDATE] {shipment.tracking_id}: {old_status} -> {new_status}"
            )
            if not dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[OK] {shipment.tracking_id}: {old_status} (no change)"
                )
            )
        ...
        for event_data in tracking_info.get('events', []):
            ...
            if event_time and event_time not in existing_events:
                self.stdout.write(
                    f"  [NEW EVENT] {event_time}: {event_data['description']}"
                )
                if not dry_run:
