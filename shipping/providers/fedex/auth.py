...
class FedExOAuthClient:
    ...
    def __init__(self, api_key: str, api_secret: str, environment: str = "sandbox"):
        ...
        self.token_url = (
            self.PRODUCTION_TOKEN_URL if environment == "production" else self.SANDBOX_TOKEN_URL
        )
    ...
    def _acquire_token(self) -> str:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise ValueError(
                    _("Invalid FedEx API credentials. Please check your API Key and Secret.")
                )

            if response.status_code == 403:
                logger.error("FedEx OAuth authentication forbidden")
                raise ValueError(
                    _("Access forbidden. Please verify your FedEx account permissions.")
                )
            ...
            if response.status_code >= 400:
                ...
                raise ConnectionError(
                    _("Failed to acquire FedEx OAuth token: %(error)s") % {"error": error_msg}
                )
            ...
            raise ConnectionError(
                _("Unable to connect to FedEx API. Please check your network connection.")
            )
...
def create_oauth_client(credentials: dict[str, Any]) -> FedExOAuthClient:
    ...
    if missing_fields:
        raise ValueError(
            _("Missing required credentials: %(fields)s") % {"fields": ", ".join(missing_fields)}
        )
