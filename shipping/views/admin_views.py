...
from shipping.models import CarrierPreset, ProviderAccount, ShippingZone, ShippingPromotion
...
def delete_carrier(request, carrier_id):
    ...
    try:
        ...
        if carrier.is_system:
            return JsonResponse({
                'success': False,
                'message': _('System carriers cannot be deleted')
            }, status=403)
...
def bulk_action(request):
    ...
    try:
        ...
        if not action or not carrier_ids:
            return JsonResponse({
                'success': False,
                'message': _('Invalid action or no carriers selected')
            }, status=400)
...
def provider_bulk_action(request):
    ...
    try:
        ...
        if action == 'enable':
            ...
            if error_count > 0:
                message = _('{} provider(s) connected, {} failed').format(success_count, error_count)
            else:
...
def zone_bulk_action(request):
    ...
    try:
        ...
        if not action or not zone_ids:
            return JsonResponse({
                'success': False,
                'message': _('Invalid action or no zones selected')
            }, status=400)
...
def rule_bulk_action(request):
    ...
    try:
        ...
        if not action or not rule_ids:
            return JsonResponse({
                'success': False,
                'message': _('Invalid action or no promotions selected')
            }, status=400)
