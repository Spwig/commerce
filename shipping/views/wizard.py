...
class ProviderWizardStep1View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        providers = ComponentRegistry.objects.filter(component_type="shipping_provider").order_by(
            "name"
        )
        ...
        for component in providers:
            try:
                ...
                if provider_dir.exists():
                    ...
                    logger.warning(
                        "Component directory not found for %s: %s", component.name, provider_dir
                    )
...
class ProviderWizardStep5View(WizardSessionMixin, View):
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.success(
                request, _('Provider "%(name)s" connected successfully!') % {"name": display_name}
            )
