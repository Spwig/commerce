    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if wizard_data.get('min_cart_value'):
                rule.min_cart_value = Money(wizard_data.get('min_cart_value'), get_default_currency())
            if wizard_data.get('max_cart_value'):
                rule.max_cart_value = Money(wizard_data.get('max_cart_value'), get_default_currency())
            ...
            messages.error(
                request,
                _('Failed to create promotion: %(error)s') % {'error': str(e)}
            )
            return redirect('shipping:promotion_wizard_step5')
