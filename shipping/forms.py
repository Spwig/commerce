    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            if 'component' in self.fields:
                ...
                self.fields['component'].help_text = _('Component cannot be changed after creation.')
            ...
            try:
                ...
                if manifest:
                    ...
                    if self.instance.credentials_encrypted:
                        try:
                            decrypted_credentials = decrypt_credentials(self.instance.credentials_encrypted)
                        except Exception as e:
                    ...
                    for field_name, field_config in credential_schema.items():
                        ...
                        field_help = field_config.get('description', field_config.get('help_text', ''))

                        # Determine widget based on field type
                        if field_type == 'select':
                            # Handle choices - can be simple list or list of dicts with value/label
                            choices_config = field_config.get('choices', field_config.get('options', []))
                            if choices_config and isinstance(choices_config[0], dict):
                                # Format: [{"value": "x", "label": "X"}]
                                choices = [(choice['value'], choice['label']) for choice in choices_config]
                            else:
                                # Format: ["x", "y"] or already formatted tuples
                                choices = [(opt, opt) if not isinstance(opt, tuple) else opt for opt in choices_config]
                            ...
                            self.fields[f'credential_{field_name}'] = forms.ChoiceField(
                                ...
                                widget=widget,
                                initial=decrypted_credentials.get(field_name, field_config.get('default'))
                            )
                        elif field_secret:
            ...
            for field_name in self.fields.keys():
                if field_name.startswith('credential_'):
                    ...
                    if field_name in cleaned_data:
                        ...
                        credentials[credential_key] = self._original_credentials.get(credential_key, '')
                    else:
