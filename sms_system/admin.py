...
from .models import SMSOutbox, SMSProviderAccount, SMSTemplate
        ...
        def save(self, commit=True):
            ...
            for field_name in properties:
                ...
                if form_field_name in self.cleaned_data:
                    value = (
                        self.cleaned_data.get(form_field_name, "").strip()
                        if self.cleaned_data.get(form_field_name)
                        else ""
                    )
...
class SMSProviderAccountAdmin(admin.ModelAdmin):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if provider_key:
            ...
            if credential_schema:
                ...
                cred_fields = [f"cred_{name}" for name in properties]
                if cred_fields:
                    fieldsets.append(
                        (
            ...
            fieldsets.append(
                (
                    _("Provider Credentials"),
                    {
                        "fields": [],
                        "description": _(
                            "Select a provider above to see credential fields, or use the Setup Wizard for a guided experience."
                        ),
                        "classes": ["tab-credentials"],
                    },
                )
    ...
    def is_default_whatsapp_badge(self, obj):
        if obj.is_default_whatsapp:
            return format_html(
                '<span class="sms-badge-default-whatsapp">{}</span>', _("Default WhatsApp")
            )
        return "-"
    ...
    def connection_status_badge(self, obj):
        ...
        return format_html(
            '<span class="{}">{}</span>', css_class, obj.get_connection_status_display()
        )
    ...
    fieldsets = [
        ...
        (
            _("Message Content"),
            {
                "fields": ["message"],
                "description": _(
                    'Use {variable} syntax for placeholders. Example: "Hello {name}, your order #{order_number} is ready."'
                ),
            },
        ),
        (
