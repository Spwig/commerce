...
class EmailAccount(models.Model):
    ...
    provider_key = models.CharField(
        ...
        help_text=_("Provider identifier (e.g., 'builtin_smtp', 'gmail_api'). Required for built-in providers.")
    )
    ...
    reply_to = models.EmailField(
        ...
        help_text=_("Default reply-to email address")
    )
            ...
            if hasattr(e, 'error_dict'):
                ...
                for key, error_list in e.error_dict.items():
                    filtered = [
                        ...
                        if 'unique_default_email_account_per_site' not in
...
class EmailTemplate(SoftDeleteModel):
    ...
    html_content = models.TextField(
        ...
        help_text=_("HTML email content (MJML or plain HTML)")
    )
    ...
    def deactivate(self):
        ...
        if system_template and not EmailTemplate.objects.filter(
            site=self.site,
            template_type=self.template_type,
            is_active=True
        ).exists():
            ...
            logger.info(f"Activated system template as fallback")
    ...
    def get_active_template(cls, template_type, site=None, language_code='en'):
        ...
        for lang in languages:
            active = cls.objects.filter(
                ...
                language_code=lang,
                is_active=True
            ).order_by('is_system').first()  # Custom templates (is_system=False) first
    ...
    def create_version(self, user=None):
        ...
        logger.info(
            f"Created version {version_snapshot.version} of template {self.id}"
        )
    ...
    def revert_to_version(self, version_number):
        ...
        version_snapshot = EmailTemplate.objects.get(
...
class EmailOutbox(models.Model):
    ...
    cc = models.JSONField(
        ...
        blank=True,
        verbose_name=_("CC"),
        help_text=_("CC email addresses")
    )
    ...
    bcc = models.JSONField(
        ...
        blank=True,
        verbose_name=_("BCC"),
        help_text=_("BCC email addresses")
    )
    ...
    headers = models.JSONField(
        ...
        help_text=_("Custom email headers")
    )
    ...
    skip_reason = models.CharField(
        ...
        help_text=_("Reason why email was skipped (e.g., 'user_preference_disabled', 'unsubscribed')")
    )
...
class EmailDNSCheck(models.Model):
    ...
    domain = models.CharField(
        ...
        help_text=_("Domain being validated")
    )
    ...
    spf_record = models.TextField(
        blank=True,
        verbose_name=_("SPF record"),
        help_text=_("SPF TXT record value found")
    )

    spf_errors = models.TextField(
        blank=True,
        verbose_name=_("SPF errors"),
        help_text=_("SPF validation error messages")
    )
    ...
    dkim_record = models.TextField(
        blank=True,
        verbose_name=_("DKIM record"),
        help_text=_("DKIM TXT record value found")
    )
    ...
    dkim_errors = models.TextField(
        blank=True,
        verbose_name=_("DKIM errors"),
        help_text=_("DKIM validation error messages")
    )
    ...
    dmarc_record = models.TextField(
        blank=True,
        verbose_name=_("DMARC record"),
        help_text=_("DMARC TXT record value found")
    )

    dmarc_errors = models.TextField(
        blank=True,
        verbose_name=_("DMARC errors"),
        help_text=_("DMARC validation error messages")
    )
...
class EmailTemplateTranslation(models.Model):
    ...
    html_content = models.TextField(
        verbose_name=_("HTML content"),
        help_text=_("Translated MJML/HTML template")
    )

    text_content = models.TextField(
        verbose_name=_("text content"),
        help_text=_("Translated plain text version")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.html_content:
            ...
            try:
                ...
                raise ValidationError({
                    'html_content': f"Invalid MJML content: {e}"
                })
        super().save(*args, **kwargs)
