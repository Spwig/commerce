...
def filter_email_outbox(request):
    ...
    html = render_to_string(
        'admin/email_system/partials/emailoutbox_cards.html',
        {'emails': emails},
        request=request
    )
    ...
    try:
        with transaction.atomic():
            ...
            EmailAccount.objects.filter(
                site=account.site, is_default=True
        ...
        return JsonResponse({
            'success': True,
            'message': _('Default account updated to %(email)s') % {'email': account.from_email},
        })
    except EmailAccount.DoesNotExist:
        return JsonResponse({'success': False, 'message': _('Account not found')}, status=404)
    except Exception as e:
...
def test_account_connection(request, account_id):
    ...
    try:
        ...
        account.save(update_fields=['connection_status', 'connection_error', 'last_tested_at', 'updated_at'])
...
def delete_account(request, account_id):
    ...
    try:
        ...
        if account.is_default:
            return JsonResponse({
                'success': False,
                'message': _('Cannot delete the default email account. Set another account as default first.'),
            }, status=400)
    ...
    try:
        ...
        if action == 'enable':
            ...
            if default_in_selection:
                return JsonResponse({
                    'success': False,
                    'message': _('Cannot delete the default email account. Remove it from selection or change the default first.'),
                }, status=400)
            accounts.delete()
