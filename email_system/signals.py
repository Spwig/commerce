...
def _get_payment_method_display(order):
    ...
    display = PAYMENT_TYPE_DISPLAY.get(
        ...
        order.payment_method_type.replace('_', ' ').title()
    )
...
def send_order_confirmation_email(sender, instance, created, **kwargs):
    ...
    if meta.get('license_checkout') or meta.get('dev_license_purchase'):
        logger.debug(f"Skipping generic order confirmation for license order {instance.order_number}")
        return
    ...
    def _send():
        try:
            ...
            logger.info(f"Sent order confirmation email for order {instance.order_number} to {instance.email}")

        except Exception as e:
            logger.error(f"Failed to send order confirmation email for order {instance.order_number}: {e}", exc_info=True)
...
def send_order_status_emails(sender, instance, created, **kwargs):
    ...
    def _send():
        try:
            ...
            logger.error(f"Failed to send order status email for {instance.order_number}: {e}", exc_info=True)
...
def _send_shipping_confirmation(order):
    ...
    context = {
        ...
        'estimated_delivery': order.estimated_delivery_date.strftime('%B %d, %Y') if order.estimated_delivery_date else 'Soon',
...
def auto_translate_email_templates_on_language_change(sender, instance, created, **kwargs):
    ...
    if site_language not in ADMIN_LANGUAGES:
        ...
        from email_system.services.site_language_translation_service import SiteLanguageTranslationService
        ...
        result = translation_service.auto_translate_for_site_language(
            ...
            triggered_by='site_settings_change'
        )
...
def send_admin_new_order_notification(sender, instance, created, **kwargs):
    ...
    def _send():
        try:
            ...
            logger.error(f"Failed to send admin notification for order {instance.order_number}: {e}", exc_info=True)
