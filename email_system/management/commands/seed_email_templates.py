...
def get_english_base_files(template_dir):
    ...
    for filepath in all_md_files:
        ...
        has_lang_suffix = any(filename.endswith(f".{lang}.md") for lang in LANGUAGE_CODES)
        if has_lang_suffix:
            continue
...
class Command(SeedCommand):
    ...
    def seed(self) -> int:
        ...
        template_dir = os.path.join(settings.BASE_DIR, "email_templates_for_translation")

        if not os.path.isdir(template_dir):
            self.stderr.write(self.style.ERROR(f"Template directory not found: {template_dir}"))
            return 0

        # Discover English base template files
        base_files = get_english_base_files(template_dir)
        if not base_files:
            self.stderr.write(self.style.ERROR("No English base template files found"))
            return 0

        self.stdout.write(f"    Found {len(base_files)} English base template files")
        ...
        for english_file in base_files:
            ...
            if not is_hq and EmailTemplate.is_hq_only_type(english_data.get("template_type", "")):
                skipped_hq += 1
                continue
            ...
            try:
                ...
                errors.append(f"{filename}: Failed to upsert base template: {e}")
                continue

            # Process translations for all 16 languages
            for lang_code in LANGUAGE_CODES:
                translation_file = english_file.replace(".md", f".{lang_code}.md")
                ...
                if "error" in translation_data:
                    errors.append(f"{template_name}.{lang_code}.md: {translation_data['error']}")
                    continue
                ...
                try:
                    EmailTemplateTranslation.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            "text_content": translation_data.get("text_content", ""),
                    ...
                    errors.append(
                        f"{template_name}.{lang_code}.md: Failed to upsert translation: {e}"
                    )

        if skipped_hq:
            self.stdout.write(
                f"    Skipped {skipped_hq} HQ-only template(s) on non-HQ installation"
            )

        if errors:
            self.stderr.write(self.style.WARNING(f"    {len(errors)} issues encountered:"))
