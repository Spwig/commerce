...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        admin_languages = [lang[0] for lang in settings.LANGUAGES if lang[0] != 'en']  # Exclude English (already exists)

        if options['languages']:
            target_languages = options['languages']
            # Validate languages
            invalid = [l for l in target_languages if l not in admin_languages]
            if invalid:
        ...
        self.stdout.write(self.style.SUCCESS(f'Creating Admin Language Base Templates'))
        self.stdout.write(self.style.SUCCESS(f'{"="*80}\n'))
        self.stdout.write(f'Target Languages: {", ".join(target_languages)}')
        self.stdout.write(f'Template Types: {", ".join(template_types)}')
        self.stdout.write(f'Total to create: {len(template_types)} × {len(target_languages)} = {len(template_types) * len(target_languages)}\n')
        ...
        for template_type in template_types:
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING(f'✗ No English template for {template_type}, skipping'))
                continue
            ...
            for target_lang in target_languages:
                ...
                try:
                    self.stdout.write(f'  ⟳ {template_type} ({target_lang}) - translating...', ending='')
                    self.stdout.flush()

                    # Translate subject
                    translated_subject = client.translate(
                        text=base_template.subject,
                        source_lang='en',
                    ...
                    translated_html = client.translate(
                        text=base_template.html_content,
                        source_lang='en',
                        target_lang=target_lang
                    )
                    ...
                    translated_text = client.translate(
                        text=base_template.text_content,
                        source_lang='en',
                        target_lang=target_lang
                    )
                    ...
                    self.stdout.write(self.style.SUCCESS(f'  ✓ {template_type} ({target_lang}) - created'))
                    created_count += 1
                    ...
                    self.stdout.write('\r' + ' ' * 80 + '\r', ending='')  # Clear line
                    self.stdout.write(self.style.ERROR(f'  ✗ {template_type} ({target_lang}) - error: {str(e)[:50]}'))
                    error_count += 1
        ...
        self.stdout.write('those admin interface languages. Merchants can still use the AI Translation')
