...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        base_templates = EmailTemplate.objects.filter(
            language_code="en", is_system=True, template_type__in=template_types
        )
        ...
        for template in base_templates:
            ...
            if skip_existing:
                ...
                languages_to_translate = [
                    lang for lang in languages if lang not in existing_translations
                ]

                if len(languages_to_translate) < len(languages):
                    skipped = set(languages) - set(languages_to_translate)
                    self.stdout.write(
                        self.style.WARNING(f"   ⏭️  Skipping existing: {', '.join(sorted(skipped))}")
                    )
            ...
            try:
                result = translation_service.translate_template(
                    template=template, target_languages=languages_to_translate, user=None
                )
                    ...
                    if process_jobs and job_processor:
                        ...
                        for job_info in jobs:
                            ...
                            try:
                                ...
                                self.stdout.write(self.style.ERROR(f"      ✗ {lang}: {e}"))
                                stats["jobs_failed"] += 1
        ...
        if process_jobs:
            ...
            if stats["jobs_failed"] > 0:
                self.stdout.write(self.style.ERROR(f"✗ Jobs failed: {stats['jobs_failed']}"))
        else:
            self.stdout.write(
                self.style.WARNING(
                    "\n⚠️  Jobs created but not processed. "
                    "Run with --process-jobs to translate immediately."
                )
            )
