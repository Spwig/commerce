...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        translated_files = [
            f for f in json_files if not f.name.endswith("_en.json") and "EXAMPLE" not in f.name
        ]
        ...
        self.stdout.write(self.style.SUCCESS("Importing Translated Email Templates"))
        ...
        for json_file in sorted(translated_files):
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"  ✗ {json_file.name} - invalid JSON: {str(e)}")
                )
                error_count += 1
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"  ✗ {json_file.name} - error: {str(e)}"))
                error_count += 1
        ...
        if options["dry_run"]:
            ...
            if imported_count > 0:
                ...
                self.stdout.write(
                    "They can further customize them or use AI Translation Service for customer languages."
                )
