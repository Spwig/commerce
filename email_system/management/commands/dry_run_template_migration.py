...
class Command(BaseCommand):
    ...
    def __init__(self):
        super().__init__()
        self.template_dir = os.path.join(str(settings.BASE_DIR), "email_templates_for_translation")
        self.errors = []
        self.warnings = []
        self.stats = {
            "base_files_found": 0,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"   ✓ Successfully parsed {self.stats['base_files_parsed']}/{self.stats['base_files_found']} base templates\n"
        )
        ...
        for template_name, base_data in base_templates.items():
            ...
            for lang_code in self.LANGUAGE_CODES:
                ...
                validation_result = self.validate_translation(
                    trans_data, base_data, template_name, lang_code
                )
                if validation_result:
                    self.errors.append(validation_result)
                    self.stats["validation_errors"] += 1
        ...
        self.stdout.write(
            f"   ✓ Successfully parsed {self.stats['translations_parsed']} translations\n"
        )
    ...
    def validate_translation(self, trans_data, base_data, template_name, lang_code):
        ...
        base_vars = set(
            re.findall(r'\{\{[\s\w\.\|:\'"]+\}\}', base_data["subject"] + base_data["html_content"])
        )
        trans_vars = set(
            re.findall(
                r'\{\{[\s\w\.\|:\'"]+\}\}', trans_data["subject"] + trans_data["html_content"]
            )
        )
    ...
    def print_summary_report(self, check_colors):
        ...
        self.stdout.write(
            f"      ({self.stats['base_files_parsed']} base + {self.stats['translations_parsed']} translations)"
        )
        ...
        if self.errors:
            ...
            self.stdout.write(
                self.style.ERROR(f"   Found {len(self.errors)} critical errors that must be fixed")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"   All {self.stats['base_files_parsed']} base templates validated"
                )
            )
            self.stdout.write(
                self.style.SUCCESS(
                    f"   All {self.stats['translations_parsed']} translations validated"
                )
            )
            if self.warnings:
                self.stdout.write(
                    self.style.WARNING(f"   Found {len(self.warnings)} warnings (non-critical)")
                )
