    ...
    fieldsets = (
        ...
        (
            _("DKIM Configuration"),
            {
                ...
                "description": _(
                    'DKIM keys are used to digitally sign outgoing emails. Use "Regenerate DKIM Keys" action to create new keys.'
                ),
                "classes": ("tab-dkim",),
            },
        ),
        (
            _("Credentials"),
            {
                ...
                "description": _(
                    "Credentials are encrypted and managed through the provider setup. To update credentials, reconnect the provider."
                ),
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        for account in all_accounts:
            ...
            if account.provider_key == "builtin_smtp" and not account.component:
                ...
                provider_dir = (
                    INTEGRATIONS_DIR / "email_provider" / account.component.slug / version
                )
        ...
        component_counts = (
            all_accounts.filter(component__isnull=False)
            .values("component__slug", "component__name")
        ...
        builtin_count = all_accounts.filter(
            provider_key="builtin_smtp", component__isnull=True
        ).count()

        if builtin_count > 0:
            components_with_counts.insert(
                0,
                {"slug": "builtin_smtp", "name": _("Built-in SMTP Server"), "count": builtin_count},
            )
    ...
    def regenerate_dkim_keys(self, request, queryset):
        ...
        for account in queryset:
            if account.provider_key != "builtin_smtp":
                errors.append(
                    f"{account.name}: {_('Only built-in SMTP accounts can have DKIM keys regenerated')}"
                )
                continue
            ...
            try:
                ...
                self.message_user(
                    request, _(f"✓ DKIM keys regenerated for {account.name}"), messages.SUCCESS
                )
...
class EmailTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Template Information"), {"fields": ("site", "template_type", "language_code")}),
        (_("Content"), {"fields": ("subject", "html_content", "text_content")}),
        (_("Status"), {"fields": ("is_active", "is_system")}),
        (
            _("Version Control"),
            {"fields": ("version", "parent_template"), "classes": ("collapse",)},
        ),
        (
            _("Timestamps"),
    ...
    def clone_template(self, request, queryset):
        ...
        for template in queryset:
            try:
                template.clone(user=request.user, set_active=False)
                cloned_count += 1
                self.message_user(
                    request,
                    _(
                        f"Successfully cloned template: {template.get_template_type_display()} ({template.language_code})"
                    ),
                    messages.SUCCESS,
                )
            except Exception as e:
        ...
        if cloned_count > 0:
            self.message_user(
                request, _(f"Successfully cloned {cloned_count} template(s)"), messages.SUCCESS
            )
...
class EmailOutboxAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Content"), {"fields": ("subject", "html_body", "text_body", "reply_to")}),
        (_("Additional Recipients"), {"fields": ("cc", "bcc"), "classes": ("collapse",)}),
        (
            _("Metadata"),
            {
    ...
    def release_held_emails_action(self, request, queryset):
        ...
        updated = queryset.filter(status="held").update(status="queued", queued_at=timezone.now())
        if updated:
            self.message_user(
                request, _("Released %d email(s) for delivery.") % updated, messages.SUCCESS
            )
        else:
...
class EmailEventAdmin(admin.ModelAdmin):
    ...
    list_display = ["email", "event_type_badge", "bounce_type", "occurred_at", "created_at"]
    ...
    fieldsets = (
        (_("Event Information"), {"fields": ("email", "event_type", "occurred_at")}),
        (_("Event Data"), {"fields": ("event_data",), "classes": ("collapse",)}),
        (
            _("Bounce Details"),
            {"fields": ("bounce_type", "bounce_reason"), "classes": ("collapse",)},
        ),
