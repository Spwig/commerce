...
var _templateEditorEl = document.getElementById('template-editor-data');
var templateData = _templateEditorEl ? JSON.parse(_templateEditorEl.textContent) : {};
...
async function updatePreview() {
    ...
    if (!mjmlContent.trim()) {
        iframe.srcdoc = '<div style="padding: 40px; text-align: center; color: #999;">No content to preview</div>';
        return;
    }
    ...
    try {
        ...
        const response = await fetch(AdminUtils.buildAdminUrl('/admin/email-system/templates/preview-render/'), {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            const data = await response.json();
            iframe.srcdoc = data.html || '<div style="padding: 20px; color: red;">Preview generation failed</div>';
        } else {
