...
(function () {
  ...
  const configEl = document.getElementById('newsletter-send-config');
  const config = configEl ? JSON.parse(configEl.textContent) : { i18n: {} };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', async function (e) {
      ...
      const msg =
        (i18n.confirmSendPrefix || 'Send newsletter to ') +
        count +
        (i18n.confirmSendSuffix || ' recipients?');
