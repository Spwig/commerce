...
(function() {
    ...
    function applyFilters() {
        var connectionFilter = document.getElementById('connection-status-filter');
    ...
    function toggleAccountActive(accountId, button) {
        ...
        fetch(AdminUtils.buildAdminUrl('/admin/email-system/accounts/' + accountId + '/toggle-active/'), {
            method: 'POST',
            headers: {
    ...
    function testAccountConnection(accountId, button) {
        ...
        fetch(AdminUtils.buildAdminUrl('/admin/email-system/accounts/' + accountId + '/test-connection/'), {
            method: 'POST',
            headers: {
                'X-CSRFToken': AdminUtils.getCsrfToken(),
                'Content-Type': 'application/json',
    ...
    async function deleteAccount(accountId, button) {
        if (!await AdminModal.confirm({ message: 'Are you sure you want to delete this email account? This action cannot be undone.', danger: true, confirmText: 'Delete' })) {
            return;
        }
    ...
    async function applyBulkAction() {
        ...
        var accountIds = getSelectedAccounts();
        ...
        if (action === 'delete') {
            if (!await AdminModal.confirm({ message: 'Are you sure you want to delete ' + accountIds.length + ' account(s)? This action cannot be undone.', danger: true, confirmText: 'Delete' })) {
                return;
            }
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var selectAllCheckbox = document.getElementById('select-all-accounts');
        if (selectAllCheckbox) {
            selectAllCheckbox.addEventListener('change', function() {
                var checkboxes = document.querySelectorAll('.account-select');
        ...
        document.addEventListener('change', function(e) {
            if (e.target.classList.contains('account-select')) {
                ...
                var checkboxes = document.querySelectorAll('.account-select');
                var checkedBoxes = document.querySelectorAll('.account-select:checked');
                if (selectAll) {
