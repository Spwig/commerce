...
def oauth_initiate(request: HttpRequest, provider_key: str) -> HttpResponse:
    ...
    try:
        ...
        if not provider_class:
            messages.error(
                request, _("Email provider '%(provider)s' not found") % {"provider": provider_key}
            )
        ...
        if not hasattr(provider_class, "create_oauth_handler"):
            messages.error(
                request,
                _("Provider '%(provider)s' does not support OAuth") % {"provider": provider_key},
            )
            return redirect("admin:email_system_emailaccount_changelist")
        ...
        if request.method == "POST":
            ...
            if not client_id or not client_secret:
                messages.error(
                    request,
                    _("OAuth credentials not found. Please start the connection process again."),
                )
                return redirect("admin:email_system_emailaccount_changelist")
...
def oauth_callback(request: HttpRequest, provider_key: str) -> HttpResponse:
    ...
    try:
        ...
        if error:
            ...
            messages.error(
                request, _("OAuth authorization failed: %(error)s") % {"error": error_description}
            )
            return redirect("admin:email_system_emailaccount_changelist")
        ...
        if not all([client_id, client_secret, redirect_uri, stored_provider_key]):
            messages.error(
                request, _("OAuth session expired. Please start the connection process again.")
            )
            return redirect("admin:email_system_emailaccount_changelist")
        ...
        if wizard_data and wizard_data.get("component_slug") == provider_key:
            ...
            logger.info("OAuth callback from wizard - storing credentials and continuing wizard")
            ...
            messages.success(
                request,
                _("Successfully authorized %(provider)s. Continue configuring your account.")
        ...
        component = ComponentRegistry.objects.filter(
            component_type="email_provider", slug=provider_key
        ).first()
        ...
        messages.success(
            ...
            % {"provider": provider_class.provider_name, "email": account.from_email},
        )
...
def test_connection(request: HttpRequest, account_id: str) -> JsonResponse:
    ...
    try:
        ...
        return JsonResponse({"success": False, "message": str(e), "details": {}}, status=500)
