...
class ProviderBrowseView(View):
    ...
    def _get_builtin_provider_data(self):
        ...
        try:
            ...
            manifest_path = Path(settings.BASE_DIR) / 'email_system' / 'providers' / 'builtin' / 'manifest.json'
            with open(manifest_path, 'r') as f:
            ...
            return {
                ...
                'thumbnail_url': static(logo_path) if Path(settings.BASE_DIR / 'email_system' / 'static' / logo_path).exists() else '',
                'homepage_url': '',
                'documentation_url': '',
    ...
    def get(self, request):
        ...
        try:
            ...
            available_from_server = update_manager.list_available_components(component_type='email_provider')
        ...
        installed_db = {p.slug: p.current_version for p in ComponentRegistry.objects.filter(component_type='email_provider')}
        ...
        for provider in available_from_server:
            ...
            if is_installed:
                ...
                if local_manifest:
                    ...
                    if not manifest:
                        ...
                        for key in ('capabilities', 'setup_wizard', 'description',
                                    'name', 'translations', 'default_language',
                                    'homepage_url', 'documentation_url', 'api_docs_url'):
            ...
            provider_data = {
                ...
                'homepage_url': provider.get('homepage_url', '') or manifest.get('homepage_url', ''),
                'documentation_url': provider.get('documentation_url') or manifest.get('documentation_url', '') or manifest.get('api_docs_url', ''),
        ...
        transactional_count = sum(1 for p in all_providers if p['capabilities'].get('transactional'))
        ...
        for provider_data in all_providers:
            modal_data = {
                ...
                'translations': dict(provider_data.get('translations', {}), default_language=provider_data.get('default_language', 'en')),
...
def install_provider_ajax(request, provider_slug):
    ...
    try:
        ...
        available_providers = update_manager.list_available_components(component_type='email_provider')
