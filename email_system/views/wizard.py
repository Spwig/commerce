    ...
    def _ensure_smtp_server_running(self, request):
        ...
        try:
            result = subprocess.run(
                ["supervisorctl", "start", "smtp-server"], capture_output=True, text=True, timeout=5
            )
            ...
            if result.returncode == 0:
                ...
                if self._verify_smtp_server_listening():
                    logger.info(
                        "Built-in SMTP server started via supervisor and verified listening"
                    )
                    return True
                else:
                    logger.warning("SMTP server started but not yet listening on port 2525")
                    messages.warning(
                        request, _("SMTP server started but may need more time to initialize")
                    )
        ...
        try:
            ...
            if self._verify_smtp_server_listening():
                ...
                messages.warning(
                    request,
                    _(
                        "SMTP server started but may need more time. If test fails, wait a moment and try again."
                    ),
                )
...
class ProviderWizardStep1View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        if provider_slug:
            if provider_slug == "builtin_smtp":
                ...
                if get_license_manager().is_spwig_hosted():
                    messages.error(
                        request, _("Built-in SMTP is not available for Spwig-hosted installations.")
                    )
                ...
                if not get_license_manager().is_spwig_hosted():
                    messages.error(
                        request, _("Spwig Email is only available for Spwig-hosted installations.")
                    )
                    return redirect("email_system:wizard_step1")
                hosted_dir = Path(settings.BASE_DIR) / "email_system" / "providers" / "spwig_hosted"
                manifest_path = hosted_dir / "manifest.json"
        ...
        providers = ComponentRegistry.objects.filter(component_type="email_provider").order_by(
            "name"
        )
                ...
                if provider_dir.exists():
                    ...
                    if manifest:
                        ...
                        if logo_filename:
                            ...
                            if logo_path.exists():
                                logo_url = static(
                                    f"email_provider/{component.slug}/current/{logo_filename}"
                                )
                        ...
                        provider_data.append(
                            {
                                ...
                                "manifest": manifest,
                                "capabilities": manifest.get("capabilities", {}),
                                "requires_oauth": manifest.get("capabilities", {}).get(
                                    "oauth", False
                                ),
                            }
                        )
    ...
    def post(self, request):
        ...
        try:
            component = ComponentRegistry.objects.get(
                id=component_id, component_type="email_provider"
            )
        except ComponentRegistry.DoesNotExist:
...
class ProviderWizardStep2View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        wizard_data.get("is_builtin", False)
        ...
        try:
            ...
            if requires_oauth:
                ...
                if not client_id or not client_secret:
                    messages.error(
                        request,
                        _("Client ID and Client Secret are required for OAuth authentication."),
                    )
                    return redirect("email_system:wizard_step2")
                ...
                if credential_schema:
                    credentials, errors = validate_credential_fields(
                        credential_schema, request.POST
                    )
            ...
            messages.error(
                request, _("Error processing credentials: %(error)s") % {"error": str(e)}
            )
            return redirect("email_system:wizard_step2")
    ...
    def post(self, request):
        ...
        if component_id == "builtin_smtp":
            ...
            try:
                ...
                messages.success(
                    request,
                    _("✓ DKIM keys generated automatically for %(domain)s") % {"domain": domain},
                )
                ...
                messages.warning(
                    request,
                    _(
                        "Could not auto-generate DKIM keys. You may need to generate them manually later."
                    ),
                )
                ...
                if dns_requirements_file.exists():
                    ...
                    if not server_ip:
                        try:
                            ...
                            if server_ip and is_private_ip(server_ip):
                                logger.warning(
                                    f"Detected private IP {server_ip} - DNS records may not work for external email delivery"
                                )
                        except Exception:
                            server_ip = None
                    ...
                    try:
                        ...
                        if site_settings and site_settings.site_url:
                            ...
                            mx_hostname = (
                                parsed_url.netloc.split(":")[0] if parsed_url.netloc else domain
                            )
                            logger.info(f"Using MX hostname from site URL: {mx_hostname}")
                    except Exception as e:
                        logger.warning(
                            f"Could not extract hostname from site URL, using domain: {e}"
                        )
                    ...
                    with open(dns_requirements_file, encoding="utf-8") as f:
                    ...
                    context_data = Context(
                        {
                            ...
                            "dns_provider_display": dns_provider_info.get(
                    ...
                    if smtp_host and profiles:
                        ...
                        if not profile:
                            ...
                            for host_pattern, prof in profiles.items():
                                if (
                                    prof.get("host_match") == "contains"
                                    and host_pattern in smtp_host
                                ):
                                    profile = prof
                                    break
                        if profile:
        ...
        # Handle skip with warning
            ...
            if dns_check_results and dns_check_results != "pass":
                messages.warning(
                    request,
                    _(
                        "DNS validation incomplete. Your emails may be flagged as spam until DNS records are properly configured."
                    ),
                )
    ...
    def post(self, request):
        ...
        try:
                ...
                try:
                    ...
                    if "RUNNING" not in result.stdout:
                        ...
                        messages.info(
                            request,
                            _(
                                'For Docker: Check container logs. For development: Run "sudo postfix start"'
                            ),
                        )
                        return self.get(request)
                ...
                try:
                    ...
                    if existing_account and existing_account.credentials:
                        ...
                        messages.error(
                            request,
                            _(
                                "Spwig Email credentials have not been provisioned yet. Please contact support."
                            ),
                        )
                        return self.get(request)
                except Exception as e:
            ...
            if wizard_data.get("requires_oauth") and not credentials.get("access_token"):
                messages.error(
                    request, _("OAuth authentication not completed. Please go back to Step 2.")
                )
                return redirect("email_system:wizard_step2")
            ...
            # Build test message
            subject = _("Test Email from %(name)s") % {
                "name": wizard_data.get("from_name", "Spwig")
            }
...
"""
            ...
            if result.get("accepted"):
                messages.success(
                    request,
                    _("✓ Test email sent successfully to %(email)s! Check your inbox.")
                    % {"email": test_recipient},
                )

                if result.get("provider_message_id"):
                    messages.info(
                        request, _("Message ID: %(id)s") % {"id": result["provider_message_id"]}
                    )
                ...
                messages.error(
                    request, _("Failed to send test email: %(error)s") % {"error": error_msg}
                )
                return self.get(request)

        except Exception as e:
            logger.exception(f"Error sending test email: {e}")
            messages.error(request, _("Error sending test email: %(error)s") % {"error": str(e)})
            return self.get(request)
...
class ProviderWizardStep6View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        try:
            ...
            is_builtin = component_id in ("builtin_smtp", "spwig_hosted_mail")
        ...
        try:
            with transaction.atomic():
                ...
                if component_id == "builtin_smtp":
                    ...
                    dkim_keys = wizard_data.get("dkim_keys", {})
                    dkim_private_key = dkim_keys.get("private_key") or wizard_data.get(
                        "dkim_private_key", ""
                    )
                    dkim_public_key = dkim_keys.get("public_key") or wizard_data.get(
                        "dkim_public_key", ""
                    )
                    dkim_selector = dkim_keys.get("selector") or wizard_data.get(
                        "dkim_selector", "mail"
                    )
                ...
                if component_id == "builtin_smtp":
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            messages.success(
                                request,
                                _('✓ Email account "%(name)s" created successfully!')
                                % {"name": account.name},
                            )
                            messages.success(
                                request,
                            ...
                            logger.info(
                                f"Built-in SMTP server configured for account {account.id} (may already be running)"
                            )
                            messages.success(
                                request,
                                _('✓ Email account "%(name)s" created successfully!')
                            ...
                            messages.info(
                                request,
                                _(
                                    "The SMTP server should already be running. If emails don't send, please contact your system administrator."
                                ),
                            )
                        ...
                        logger.info(
                            f"Built-in SMTP server configured for account {account.id} (supervisor not available, should be running from Step 3)"
                        )
                        messages.success(
                            request,
                            _('✓ Email account "%(name)s" created successfully!')
                        ...
                        messages.info(
                            request, _("Your email system is configured and ready to use.")
                        )
                        ...
                        messages.info(
                            request,
                            _(
                                "Email system configured. If you experience issues sending emails, please contact your system administrator."
                            ),
                        )
                        ...
                        messages.info(
                            request,
                            _(
                                "Your email account is configured. The SMTP server should be running from the previous setup steps."
                            ),
                        )
                else:
                    messages.success(
