...
class SiteLanguageTranslationService:
    ...
    def auto_translate_for_site_language(
        self, target_language: str, triggered_by: str = "unknown"
    ) -> dict:
        """
        ...
        existing_count = EmailTemplate.objects.filter(
            language_code=target_language, is_system=True
        ).count()
        ...
        base_templates = EmailTemplate.objects.filter(
            language_code="en", is_system=True, template_type__in=self.TEMPLATE_TYPES
        )
        ...
        for template in base_templates:
            ...
            existing = EmailTemplate.objects.filter(
                template_type=template.template_type, language_code=target_language, is_system=True
            ).exists()

            if existing:
                logger.debug(
                    f"Base template already exists for {template.template_type} in {target_language}, skipping"
                )
                continue
    ...
    def bulk_translate_multiple_languages(self, target_languages: list[str], user=None) -> dict:
        """
        ...
        for language_code in target_languages:
            result = self.auto_translate_for_site_language(
                target_language=language_code, triggered_by="bulk_translation"
            )
