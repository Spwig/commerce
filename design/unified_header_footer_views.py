...
from .header_footer_models import HeaderTemplate, FooterTemplate, WidgetPlacement
...
def unified_footer_management_view(request):
    ...
    default_slug = default_footer.slug if default_footer else None

    # Get all footers with widget placement counts
    footers = FooterTemplate.objects.annotate(
        widget_count=Count('widget_placements')
    ).order_by('-is_default', '-is_active', 'name')
