    ...
    **Authentication:** Staff session required
    ...
    responses={
        200: OpenApiResponse(
            ...
            examples=[OpenApiExample("Success", value={"success": True, "updated_count": 3})],
        ),
        400: OpenApiResponse(description=_("Validation error")),
    },
)
...
@extend_schema(
    ...
    **Authentication:** Staff session required
    ...
    parameters=[
        ...
        OpenApiParameter(name="search", type=str, description=_("Search term to filter results")),
    ],
    responses={
        200: OpenApiResponse(
    ...
    responses={
        200: OpenApiResponse(
            ...
            examples=[
                OpenApiExample(
                    "Response",
                    value={"html": '<nav class="menu-preview">...</nav>', "item_count": 5},
                )
            ],
        ),
        404: OpenApiResponse(description=_("Menu not found")),
...
class MenuPreviewAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        if popup:
            try:
                html = render_to_string("design/menu_builder/preview_popup.html", context)
        ...
        try:
            html = render_to_string("design/menu_builder/partials/menu_preview.html", context)
        except Exception as e:
            html = f'<div class="menu-preview-error">Preview error: {str(e)}</div>'

        return Response({"html": html, "item_count": menu.items.filter(is_active=True).count()})
...
class MenuSaveStructureAPIView(APIView):
    ...
    def post(self, request, pk):
        ...
        return Response({"success": True, "menu_id": menu.id, **stats})
