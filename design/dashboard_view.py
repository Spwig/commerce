...
from .header_footer_models import FooterTemplate, HeaderTemplate, Menu, MenuItem, Widget
from .models import ComponentStyle, CustomCSS, DesignToken, GlobalDesignSettings
from .theme_models import Theme, ThemeBranding, ThemeInstallation
...
class DesignDashboardView(View):
    ...
    def get_recent_activity(self):
        ...
        recent_installations = ThemeInstallation.objects.select_related(
            "theme", "installed_by"
        ...
        activities.sort(
            key=lambda x: (
                x["timestamp"] if x["timestamp"] else timezone.now() - timedelta(days=365)
            ),
            reverse=True,
        )
        return activities[:5]
    ...
    def get_quick_action_urls(self):
        ...
        return {
            ...
            # Tools
            "regenerate_css": reverse("design:regenerate_css")
            if self.url_exists("design:regenerate_css")
            else "#",
            "clear_cache": reverse("design:clear_theme_cache")
            if self.url_exists("design:clear_theme_cache")
            else "#",
            "preview_site": "/",
        }
...
class ToggleForceLightModeView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse({"success": True, "force_light_mode": settings.force_light_mode})
        except Exception as e:
