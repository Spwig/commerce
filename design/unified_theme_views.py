...
def unified_theme_management_view(request):
    ...
    installed_themes = ComponentRegistry.objects.filter(
    ...
    theme_instances = {
        t.slug: t for t in Theme.objects.filter(slug__in=theme_slug_list)
        .annotate(bundled_count=Count('bundled_components'))
    }
    ...
    for theme_pkg in installed_themes:
        ...
        bundled_component_count = getattr(theme_instance, 'bundled_count', 0) if theme_instance else 0
        ...
        theme_data = {
            ...
            'has_update': theme_pkg.latest_version and theme_pkg.current_version != theme_pkg.latest_version,
            'is_installed': True,
    ...
    if filter_type != 'installed':
        try:
            ...
            messages.error(
                request,
                _('Failed to fetch available themes: %(error)s') % {'error': str(e)}
            )
...
def install_theme_ajax(request, slug):
    ...
    try:
        ...
        theme_pkg = ComponentRegistry.objects.filter(
            component_type='theme',
            slug=slug
        ).first()
        ...
        logger.info(f"Installing theme {slug} v{theme_pkg.latest_version or theme_pkg.current_version}")
...
def get_theme_detail_ajax(request, slug):
    ...
    try:
        ...
        data = {
            'success': True,
            'slug': slug,
            'name': (theme_pkg.name if theme_pkg else None) or (server_info or {}).get('name', slug),
            'description': (theme_pkg.description if theme_pkg else None) or (server_info or {}).get('description', ''),
...
@staff_member_required
def get_theme_components_ajax(request, slug):
    """
    AJAX endpoint to get all bundled components for a theme.
    Returns detailed component information including installation status.
    """
    try:
        from .theme_models import Theme
        ...
        bundled_components = ComponentStore.objects.filter(source_theme=theme)

        components_data = []
        for component in bundled_components:
            # Get installation status from ComponentRegistry
            ...
            component_data = {
                ...
                'allowed_tiers': component.allowed_tiers,
                'is_installed': registry_entry is not None,
                'installed_version': registry_entry.current_version if registry_entry else None,
                'package_file_url': component.package_file.url if component.package_file else None,
        ...
        logger.error(f"Failed to get components for theme {slug}: {e}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
...
@staff_member_required
def get_component_details_ajax(request, component_id):
    """
    AJAX endpoint to get detailed information about a specific bundled component.
    """
    try:
        from .models import ComponentStore

        component = ComponentStore.objects.filter(id=component_id).first()
        ...
        # Get source theme info if available
        theme_info = None
        if component.source_theme:
            theme_info = {
                'slug': component.source_theme.slug,
                'name': component.source_theme.name,
                'version': component.source_theme.version,
            }
        ...
        logger.error(f"Failed to get component details for {component_id}: {e}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
