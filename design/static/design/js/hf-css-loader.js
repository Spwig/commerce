...
(function () {
    ...
    if (!window.HFBuilderConfig) {
        ...
        if (configEl) {
            try { window.HFBuilderConfig = JSON.parse(configEl.textContent); } catch (e) {}
        }
    }
    ...
    function loadAndScopeCSS(url, id, layerName) {
        ...
        return fetch(url)
            ...
            .then(function (css) {
                ...
                var keyframesBlocks = [];
                var keyframesPlaceholder = '___KEYFRAMES_PLACEHOLDER_';
                var keyframesIndex = 0;

                var processedCSS = css.replace(/@keyframes\s+[\w-]+\s*\{(?:[^{}]*\{[^}]*\})*[^}]*\}/g, function (match) {
                ...
                var basePath = baseUrl.pathname.substring(0, baseUrl.pathname.lastIndexOf('/') + 1);

                processedCSS = processedCSS.replace(/url\(['"]?(?!data:)(?!http)([^'")\s]+)['"]?\)/g, function (match, relPath) {
                    if (relPath.startsWith('data:') || relPath.startsWith('http')) {
                        return match;
                    }
                    var absolutePath = new URL(relPath, baseUrl.origin + basePath).pathname;
                    return "url('" + absolutePath + "')";
                });

                // STEP 3: Scope remaining CSS selectors
                var scopedCSS = processedCSS.replace(/([^{}]+){/g, function (match, selector) {
                    ...
                    var selectors = selector.split(',').map(function (s) {
                        ...
                        if (trimmed.startsWith('.hf-content-preview')) { return trimmed; }
                        if (trimmed.includes(':root')) { return trimmed.replace(':root', '.hf-content-preview'); }
                        if (trimmed === 'html' || trimmed === 'body') { return '.hf-content-preview'; }
                        if (trimmed.startsWith('html ') || trimmed.startsWith('body ')) {
                            return '.hf-content-preview ' + trimmed.substring(5);
                        }
                        if (/^\[data-theme/.test(trimmed)) { return '.hf-content-preview' + trimmed; }
                ...
                scopedCSS = scopedCSS.replace(
                    ...
                    function (fullMatch, condition, innerCSS) {
                        var maxMatch = condition.match(/max-width\s*:\s*([\d.]+)/);
                        var minMatch = condition.match(/min-width\s*:\s*([\d.]+)/);
                        if (!maxMatch && !minMatch) { return fullMatch; } // not width-based, keep as-is

                        var maxW = maxMatch ? parseFloat(maxMatch[1]) : Infinity;
                        var minW = minMatch ? parseFloat(minMatch[1]) : 0;

                        // Determine which device preview modes this query covers.
                        // Simulated widths: mobile ~375px, tablet ~900px, desktop ~1200px
                        var devices = [];
                        if (375 >= minW && 375 <= maxW) { devices.push('mobile-preview'); }
                        if (900 >= minW && 900 <= maxW) { devices.push('tablet-preview'); }
                        if (1200 >= minW && 1200 <= maxW) { devices.push('desktop-preview'); }

                        if (devices.length === 0) { return ''; }
                        if (devices.length === 3) { return innerCSS; } // unconditional
                ...
                scopedCSS = scopedCSS.replace(
                    ...
                    function (match, index) { return keyframesBlocks[parseInt(index)]; }
                );
                ...
                var styleElement = document.getElementById('scoped-' + id);
                if (!styleElement) {
                ...
                console.log('✓ Loaded and scoped CSS: ' + id + (layerName ? ' (layer: ' + layerName + ')' : ''));
            })
    ...
    function loadThemeSystemCSS() {
        ...
        var themeBrandChain = (config.themeCssUrl
        ...
        var widgetFiles = cssFiles.widgets || [];
        var widgetLoads = widgetFiles.map(function (url) {
            var id = 'widget-' + url.split('/').pop().replace('.css', '');
