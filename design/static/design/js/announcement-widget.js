...
(function() {
    ...
    function initWidget(widget) {
        ...
        if (closeBtn) {
            closeBtn.addEventListener('click', function(e) {
                ...
                var zone = widget.closest('.header-zone--notification');
    ...
    function initHorizontalScroll(widget) {
        ...
        var configSpeed = parseFloat(widget.dataset.scrollSpeed);
        var originalWidth = track.scrollWidth / 2; // half = one set of items
        var duration;
    ...
    function openModal(announcementId, contentEl, backdrop) {
        ...
        fetch('/api/announcements/' + announcementId + '/detail/')
            ...
            .catch(function() {
                contentEl.innerHTML = '<p style="text-align:center;padding:2rem;">Could not load announcement.</p>';
            });
    }
    ...
    function buildModalContent(data) {
        ...
        if (data.image_url) {
            if (data.image_display_mode === 'background') {
                html += '<div class="announcement-modal-image-bg" style="background-image:url(' + escapeAttr(data.image_url) + ');--announcement-overlay-opacity:' + data.image_overlay_opacity + '">';
                ...
                if (data.link_url && data.link_text) {
                    html += '<a href="' + escapeAttr(data.link_url) + '" class="announcement-modal-link-btn">' + escapeHtml(data.link_text) + '</a>';
                }
                html += '</div></div>';
                return html;
            } else {
                html += '<img src="' + escapeAttr(data.image_url) + '" alt="" class="announcement-modal-image-banner">';
            }
        }
