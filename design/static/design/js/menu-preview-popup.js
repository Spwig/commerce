...
(function() {
    ...
    function applyDevice() {
        const presetKey = currentDevice === 'desktop' ? 'desktop' : `${currentDevice}-${currentOrientation}`;
        ...
        if (orientToggle) {
            ...
            var icon = orientToggle.querySelector('i');
            if (icon) {
    ...
    function fitToWrapper() {
        var bounds = frameWrapper.getBoundingClientRect();
        var padding = 60;
        var labelSpace = 80;

        var availableWidth = bounds.width - (padding * 2);
        var availableHeight = bounds.height - (padding * 2) - labelSpace;

        var scaleX = availableWidth / currentFrameWidth;
        var scaleY = availableHeight / currentFrameHeight;
        var autoScale = Math.min(scaleX, scaleY, 1);

        var scale = Math.max(autoScale * (manualZoom / 100), 0.15);
        ...
        var displayZoom = Math.round(scale * 100);
        var isConstrained = autoScale < 1;
        if (zoomValue) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            ...
            var hamburger = e.target.closest('[data-action="toggle-hamburger"]');
            if (hamburger) {
                hamburger.closest('.menu-preview') && hamburger.closest('.menu-preview').classList.toggle('menu-open');
            }
