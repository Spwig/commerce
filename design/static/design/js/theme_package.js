...
(function() {
    ...
    var msgEl = document.getElementById('package-messages');
    if (msgEl) {
        try { window.PackageMessages = JSON.parse(msgEl.textContent); } catch (e) {}
    }
    ...
    function validateTheme() {
        ...
        .then(data => {
            ...
            if (!data.success) {
                AdminModal.alert({message: `${window.PackageMessages.error}: ${data.error}`, type: 'error'});
                return;
            }
        ...
        .catch(error => {
            ...
            AdminModal.alert({message: `${window.PackageMessages.error}: ${error.message}`, type: 'error'});
        });
    }
    ...
    function displayValidationResults(data) {
        ...
        if (data.theme_info) {
            ...
            document.getElementById('theme-info-author').innerHTML = `<i class="fas fa-user"></i> ${data.theme_info.author}`;
    ...
    function packageTheme() {
        ...
        .then(data => {
            ...
            if (!data.success) {
                AdminModal.alert({message: `${window.PackageMessages.packagingFailed}: ${data.error}`, type: 'error'});
                return;
            }
    ...
    function displayPackageResults(data) {
        ...
        document.getElementById('package-size').textContent = formatBytes(data.package_info.package_size);
        document.getElementById('package-checksum').textContent = data.package_info.package_checksum.substring(0, 16) + '...';
