...
(function () {
    ...
    if (window._widgetCurrencyInit) { return; }
    window._widgetCurrencyInit = true;

    function initWidget(widget) {
        if (widget.dataset.currencyInitialized) { return; }
        widget.dataset.currencyInitialized = 'true';

        var currencyUrl = widget.dataset.currencyUrl || '';
        var showCurrentOnly = widget.dataset.showCurrentOnly === 'true';
        var meta = document.querySelector('meta[name="csrf-token"]');
        var csrfToken = (meta && meta.content) ? meta.content : '';
        if (!csrfToken) {
        ...
        if (showCurrentOnly) {
            var button = widget.querySelector('.currency-button');
            var dropdown = widget.querySelector('.currency-dropdown');
            var options = dropdown ? dropdown.querySelectorAll('.currency-option') : [];
            ...
            var select = widget.querySelector('.currency-select');
            if (!select) { return; }
    ...
    function changeCurrency(currencyCode, currencyUrl, csrfToken, widget) {
        var loading = widget.querySelector('.currency-loading');
        ...
        .then(function (response) {
            if (!response.ok) { throw new Error('Currency change failed'); }
