...
(function() {
    var msgEl = document.getElementById('theme-messages');
    if (msgEl) {
        try { window.ThemeMessages = JSON.parse(msgEl.textContent); } catch (e) {}
    }
...
(function() {
    ...
    async function activateTheme(slug, version) {
        const msg = window.ThemeMessages.activateConfirm || 'Activate this theme? All other themes will be deactivated.';
    ...
    async function uninstallTheme(slug) {
        const msg = window.ThemeMessages.uninstallConfirm || 'Uninstall this theme? This cannot be undone.';
    ...
    function purchaseTheme(slug) {
        ...
        .then(function(data) {
            if (data.purchase_url) {
                ...
                var errMsg = window.ThemeMessages.purchaseFailed || 'Could not generate purchase URL';
                AdminModal.alert({message: errMsg, type: 'error'});
            }
        })
        .catch(function(error) {
            var errMsg = window.ThemeMessages.error || 'Error';
            AdminModal.alert({message: errMsg + ': ' + error, type: 'error'});
        });
    }
    ...
    function showVersions(slug) {
        const msg = (window.ThemeMessages.versionHistoryFor || 'Version history for') + ' ' + slug + ' - ' + (window.ThemeMessages.toBeImplemented || 'To be implemented');
    ...
    function showThemeDetails(slug) {
        var overlay = document.getElementById('theme-detail-modal');
        var body = document.getElementById('modal-body');
        var footer = document.getElementById('modal-footer');
        var titleEl = document.getElementById('modal-theme-name');
        ...
        body.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--body-quiet-color);"><i class="fas fa-spinner fa-spin" style="font-size:2rem;"></i></div>';
        footer.innerHTML = '';
        ...
        .then(function(data) {
            if (!data.success) {
                body.innerHTML = '<p style="color:var(--error-fg);padding:1rem;">' + escapeHtml(data.error || 'Unknown error') + '</p>';
                return;
            }
        ...
        .catch(function() {
            body.innerHTML = '<p style="color:var(--error-fg);padding:1rem;">' + escapeHtml(window.ThemeMessages.detailsLoadFailed || 'Failed to load theme details') + '</p>';
        });
    }
    ...
    function buildScreenshotTitleMap(data) {
        var titleMap = {};
        var manifestScreenshots = (data.manifest && data.manifest.screenshots) || [];
        if (!manifestScreenshots.length || !data.preview_images) return titleMap;

        for (var i = 0; i < manifestScreenshots.length; i++) {
            var entry = manifestScreenshots[i];
            if (!entry) continue;
    ...
    function buildModalBody(data) {
        var html = '';
        var titleMap = buildScreenshotTitleMap(data);

        // Separate preview_images into desktop and mobile
        var desktopImages = [];
        var mobileImages = [];
        if (data.preview_images && data.preview_images.length > 0) {
            for (var i = 0; i < data.preview_images.length; i++) {
                var img = data.preview_images[i];
                var imgUrl = typeof img === 'string' ? img : (img.url || img.file || '');
                if (!imgUrl) continue;
                var filename = imgUrl.split('/').pop();
                var title = titleMap[filename] || (typeof img === 'object' ? (img.title || '') : '');
                var isMobile = filename.indexOf('mobile') !== -1 || (title && title.toLowerCase().indexOf('mobile') !== -1);
                var entry = { url: imgUrl, title: title, filename: filename };
                if (isMobile) {
        ...
        var firstPreviewUrl = (desktopImages.length > 0 ? desktopImages[0].url : null) || data.thumbnail_url || '';
        var firstPreviewTitle = (desktopImages.length > 0 ? desktopImages[0].title : '') || data.name || '';

        html += '<div class="admin-modal-section">';
        if (firstPreviewUrl) {
            html += '<div id="modal-preview-container" style="text-align:center;margin-bottom:1rem;background:var(--darkened-bg);border-radius:8px;padding:0.5rem;min-height:200px;display:flex;align-items:center;justify-content:center;">';
            html += '<img id="modal-preview-img" src="' + escapeHtml(firstPreviewUrl) + '" alt="' + escapeHtml(firstPreviewTitle) + '" style="max-width:100%;max-height:500px;border-radius:6px;object-fit:contain;">';
            html += '</div>';
        }
        ...
        if (desktopImages.length > 0) {
            ...
            for (var d = 0; d < desktopImages.length; d++) {
                var de = desktopImages[d];
                var isActive = (d === 0 && !data.thumbnail_url) || (firstPreviewUrl === de.url);
                html += '<img src="' + escapeHtml(de.url) + '" alt="' + escapeHtml(de.title || 'Screenshot ' + (d + 1)) + '"';
                html += ' data-preview-url="' + escapeHtml(de.url) + '" data-preview-title="' + escapeHtml(de.title || '') + '"';
                html += ' class="modal-screenshot-thumb' + (isActive ? ' active' : '') + '"';
                html += ' style="height:80px;min-width:120px;object-fit:cover;border-radius:6px;border:2px solid ' + (isActive ? 'var(--primary)' : 'var(--border-color)') + ';cursor:pointer;transition:border-color 0.2s;"';
        ...
        if (mobileImages.length > 0) {
            ...
            html += '<div style="display:flex;gap:0.5rem;overflow-x:auto;padding-bottom:0.5rem;">';
            for (var m = 0; m < mobileImages.length; m++) {
                var me = mobileImages[m];
                html += '<img src="' + escapeHtml(me.url) + '" alt="' + escapeHtml(me.title || 'Mobile ' + (m + 1)) + '"';
                html += ' data-preview-url="' + escapeHtml(me.url) + '" data-preview-title="' + escapeHtml(me.title || '') + '"';
                html += ' class="modal-screenshot-thumb"';
                html += ' style="height:80px;min-width:50px;object-fit:cover;border-radius:6px;border:2px solid var(--border-color);cursor:pointer;transition:border-color 0.2s;"';
        ...
        // Info section
        html += '<div class="admin-modal-section">';
        html += '<p style="color:var(--body-fg);line-height:1.6;margin:0 0 1rem;">' + escapeHtml(data.description || window.ThemeMessages.noDescription || 'No description available') + '</p>';

        // Meta info grid
        html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">';

        // Version
        html += '<div style="display:flex;align-items:center;gap:0.5rem;font-size:0.875rem;color:var(--body-quiet-color);">';
        ...
        html += '<span>' + escapeHtml(window.ThemeMessages.author || 'Author') + ': ' + escapeHtml(data.author_name || 'Unknown') + '</span></div>';
        ...
        if (data.is_installed) {
            ...
            html += '<span style="color:#4CAF50;">' + escapeHtml(window.ThemeMessages.installed || 'Installed') + '</span>';
            if (data.is_active) {
                html += ' <span style="background:#4CAF50;color:white;padding:0.125rem 0.5rem;border-radius:4px;font-size:0.75rem;margin-left:0.25rem;">Active</span>';
            }
        } else {
            html += '<i class="fas fa-circle" style="color:var(--body-quiet-color);"></i>';
            html += '<span style="color:var(--body-quiet-color);">' + escapeHtml(window.ThemeMessages.notInstalled || 'Not Installed') + '</span>';
        }
        html += '</div>';

        // Pricing
        if (!data.is_installed && data.pricing_model === 'paid') {
            html += '<div style="display:flex;align-items:center;gap:0.5rem;font-size:0.875rem;color:var(--body-fg);">';
        ...
        if (data.bundled_components && data.bundled_components.length > 0) {
            html += '<div class="admin-modal-section">';
            html += '<h4 style="margin:0 0 0.75rem;font-size:0.9375rem;color:var(--body-fg);">';
            html += '<i class="fas fa-puzzle-piece" style="color:var(--primary);margin-right:0.5rem;"></i>';
            ...
            for (var j = 0; j < data.bundled_components.length; j++) {
                var comp = data.bundled_components[j];
                ...
                html += '<div style="display:flex;align-items:center;gap:0.5rem;padding:0.5rem 0.75rem;background:var(--darkened-bg);border-radius:6px;font-size:0.875rem;">';
                html += '<i class="fas ' + iconClass + '" style="color:var(--primary);min-width:1rem;text-align:center;"></i>';
                html += '<span style="flex:1;color:var(--body-fg);font-weight:500;">' + escapeHtml(comp.display_name || comp.name || comp.slug) + '</span>';
                if (comp.version) {
                    html += '<span style="color:var(--body-quiet-color);font-size:0.75rem;font-family:monospace;">v' + escapeHtml(comp.version) + '</span>';
                }
                html += '</div>';
            }
            html += '</div>';
    ...
    function buildModalFooter(data) {
        ...
        if (data.is_installed) {
            if (!data.is_active) {
                html += '<button class="theme-btn primary" data-action="activate-theme" data-theme-slug="' + escapeHtml(data.slug) + '" data-theme-version="' + escapeHtml(data.current_version || '') + '">';
                html += '<i class="fas fa-power-off"></i> Activate</button>';
            }
            if (data.has_update) {
                html += '<button class="theme-btn warning" data-action="update-theme" data-theme-slug="' + escapeHtml(data.slug) + '">';
                html += '<i class="fas fa-download"></i> Update</button>';
            }
        } else {
            if (data.pricing_model === 'paid') {
                html += '<span class="theme-price-badge">&euro;' + escapeHtml(data.price_eur || '0.00') + '</span>';
                html += '<button class="theme-btn primary" data-action="purchase-theme" data-theme-slug="' + escapeHtml(data.slug) + '">';
                html += '<i class="fas fa-shopping-cart"></i> Purchase</button>';
            } else {
                html += '<button class="theme-btn primary" data-action="install-theme" data-theme-slug="' + escapeHtml(data.slug) + '">';
    ...
    function initScreenshotThumbs(container) {
        var thumbs = container.querySelectorAll('.modal-screenshot-thumb');
    ...
    function init() {
        ...
        var modalOverlay = document.getElementById('theme-detail-modal');
        if (modalOverlay) {
