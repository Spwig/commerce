...
(function () {
  const configEl = document.getElementById('menu-builder-config');
  if (configEl) {
    try {
      window.MenuBuilderConfig = JSON.parse(configEl.textContent);
    } catch (e) {
      window.MenuBuilderConfig = {};
    }
  }
})();
...
class MenuBuilder {
  ...
  bindEvents() {
    ...
    document.querySelectorAll('.device-toggle').forEach(btn => {
      btn.addEventListener('click', e =>
        this.setDevicePreview(e.target.closest('.device-toggle').dataset.device)
      );
    });
  ...
  bindModalEvents() {
    ...
    if (closeAddItemModal)
      closeAddItemModal.addEventListener('click', () => this.hideAddItemModal());
    ...
    if (closeSettingsModal)
      closeSettingsModal.addEventListener('click', () => this.hideMenuSettingsModal());
    if (cancelSettingsBtn)
      cancelSettingsBtn.addEventListener('click', () => this.hideMenuSettingsModal());
    ...
    if (closeNewMenuModal)
      closeNewMenuModal.addEventListener('click', () => this.hideNewMenuModal());
  ...
  bindItemEvents(element, item) {
    ...
    row.addEventListener('click', e => {
      if (
        !e.target.closest('.item-action-btn') &&
        !e.target.closest('.item-expand-toggle') &&
  ...
  initSortable() {
    ...
    const menuTree = document.getElementById('menu-tree');
    if (menuTree) {
      this.sortableInstances.push(new Sortable(menuTree, this.getSortableOptions(null)));
    }
    ...
    document.querySelectorAll('.menu-tree-children').forEach(container => {
      ...
      this.sortableInstances.push(new Sortable(container, this.getSortableOptions(parentId)));
    });
  }
  ...
  getMenuStyleHtml() {
    ...
    const customBadge = key =>
      isCustom(key) ? '<span class="token-custom-badge">Custom</span>' : '';
    ...
    const varHint = key =>
      `<span class="token-var-hint">Updates: <code>${getCssVarName(key)}</code></span>`;

    // Helper to check if token has device-specific values
    const hasDeviceValues = key => {
      const val = tokens[key];
      return (
        val && typeof val === 'object' && ('mobile' in val || 'tablet' in val || 'desktop' in val)
      );
    };

    // Badge for tokens with device-specific values
    const deviceBadge = key =>
      hasDeviceValues(key)
  ...
  async resetMenuTokens() {
    const confirmMsg =
      this.config.translations?.confirmResetStyles || 'Reset all menu styles to theme defaults?';
    if (
    ...
    this.showToast(
      this.config.translations?.menuStylesReset || 'Menu styles reset to defaults',
      'success'
    );
  }
  ...
  getEmptyPanelHtml(panelId) {
    ...
    const messages = {
      'item-properties-panel':
        this.config.translations.selectItem || 'Select a menu item to edit its properties',
  ...
  getItemPropertiesHtml(item) {
    ...
    if (item.item_type === 'category') {
      ...
      const selectedCategory = this.availableCategories.find(
        c => c.id === item.category_reference_id
      );
    ...
    if (item.item_type === 'widget') {
      ...
      const widgetOptions = widgetTypes
        .map(
          w =>
            `<option value="${w.value}" ${item.widget_config?.widget_type === w.value ? 'selected' : ''}>${w.label}</option>`
        )
        .join('');
      dynamicFields += `
                <div class="form-group">
    ...
    if (item.item_type === 'category_tree') {
      ...
      const categoryOptions = this.availableCategories
        .map(
          c =>
            `<option value="${c.id}" ${treeConfig.root_category_id === c.id ? 'selected' : ''}>${c.name}</option>`
        )
        .join('');
      ...
      const sortOptions = [
        ...
        .map(
          s =>
            `<option value="${s.value}" ${treeConfig.sort_by === s.value ? 'selected' : ''}>${s.label}</option>`
        )
        .join('');
  ...
  async deleteItem(item) {
    if (
      !(await AdminModal.confirm({
        message:
          this.config.translations.deleteConfirm ||
  ...
  populateQuickAddSources() {
    ...
    if (this.pagesUseAjax) {
      ...
      this.showSearchPrompt(
        'pages-list',
        this.config.translations?.typeToSearch || 'Type to search...'
      );
    } else {
  ...
  async searchPages(query) {
    ...
    if (!this.pagesUseAjax) {
      const filtered = this.availablePages.filter(
        p =>
    ...
    try {
      const response = await fetch(
        `${this.apiSourcesUrl}?type=pages&search=${encodeURIComponent(query)}`
      );
      const data = await response.json();
  ...
  async searchCategories(query) {
    ...
    try {
      const response = await fetch(
        `${this.apiSourcesUrl}?type=categories&search=${encodeURIComponent(query)}`
      );
      const data = await response.json();
  ...
  async showPropertyDropdown(type, query, dropdownId, item, refField) {
    ...
    dropdown.innerHTML =
      '<div class="searchable-select-loading"><i class="fas fa-spinner fa-spin"></i></div>';
    ...
    try {
      ...
      if (useAjax || localData.length === 0) {
        // AJAX search
        const response = await fetch(
          `${this.apiSourcesUrl}?type=${type}&search=${encodeURIComponent(query)}`
        );
        const data = await response.json();
  ...
  setupPreviewControls() {
    ...
    document
      .querySelectorAll('.preview-toolbar .device-buttons button[data-device]')
      .forEach(btn => {
        btn.addEventListener('click', () => {
  ...
  fitToWrapper() {
    ...
    if (this.zoomValue) {
      ...
      this.zoomValue.textContent = isConstrained ? displayZoom + '%*' : this.manualZoom + '%';
    }
  }
  ...
  renderPreviewItem(item, depth) {
    ...
    if (item.icon) {
      const iconStyle = item.style_config?.icon_color
        ? `style="color: ${item.style_config.icon_color}"`
        : '';
    ...
    while (i < css.length) {
      ...
      if (css[i] === '@') {
        ...
        if (ruleName === 'media' || ruleName === 'supports' || ruleName === 'layer') {
          ...
          let braceCount = 1;
          const ruleContent = css.substring(ruleStart, i + 1);
          i++;
          const innerStart = i;
          ...
          const innerContent = css.substring(innerStart, i - 1);
          const scopedInner = this.scopeCSS(innerContent, scopeSelector);
        ...
        // Scope the selector
        const scopedSelector = this.scopeSelectorList(selector, scopeSelector);
  ...
  handleKeyboard(e) {
    ...
    if (e.key === 'Escape') {
      ...
      this.selectedItem = null;
      document
        .querySelectorAll('.menu-tree-item-row.selected')
        ...
        if (previewVisible) {
          ...
          if (this.previewDevice !== 'desktop') {
            this.previewOrientation =
              this.previewOrientation === 'portrait' ? 'landscape' : 'portrait';
            this.applyDevice();
          }
        }
