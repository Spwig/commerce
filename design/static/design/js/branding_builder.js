...
class BrandingBuilder {
    constructor() {
        ...
        var _configEl = document.getElementById('branding-builder-config');
        var _cfg = _configEl ? JSON.parse(_configEl.textContent) : {};
    ...
    mergeTokens() {
        ...
        return {
            ...
            transitions: { ...(this.themeData.transitions || {}), ...(this.brandingData.transitions || {}) },
            ...
            'card-default': { ...(this.themeData['card-default'] || {}), ...(this.brandingData['card-default'] || {}) },
            'card-elevated': { ...(this.themeData['card-elevated'] || {}), ...(this.brandingData['card-elevated'] || {}) },
            'card-bordered': { ...(this.themeData['card-bordered'] || {}), ...(this.brandingData['card-bordered'] || {}) },
            'card-minimal': { ...(this.themeData['card-minimal'] || {}), ...(this.brandingData['card-minimal'] || {}) },
    ...
    loadTokenInputs() {
        ...
        tokenInputs.forEach(input => {
            ...
            if (effectiveValue !== undefined) {
                ...
                const needsResolution = resolveValueCategories.includes(category) ||
                                        layoutSections.includes(category);
        ...
        document.querySelectorAll('.bb-property-field').forEach(field => {
            ...
            if (!section || !property) {
                ...
                if (colorKey) {
                    ...
                    const tokenPath = field.dataset.token ||
                                     field.querySelector('[data-token]')?.dataset.token;
                    if (tokenPath) {
    ...
    resolveThemeVariable(varName, maxDepth = 5) {
        ...
        if (colorMatch && this.mergedData?.colors) {
            ...
            const colorValue = this.mergedData.colors[key] || this.mergedData.colors[key.replace(/-/g, '_')];
        ...
        if (fontFamilyMatch && this.mergedData?.typography) {
            ...
            const value = this.mergedData.typography[key] || this.mergedData.typography[key.replace(/-/g, '_')];
        ...
        if (transitionDurationMatch && this.mergedData?.transitions) {
            ...
            const value = this.mergedData.transitions[key] || this.mergedData.transitions[key.replace(/-/g, '_')];
            if (value) return maybeResolve(value);
        }
    ...
    setupColorInputs() {
        ...
        const colorFields = document.querySelectorAll('.color-field[data-color-key], .bb-property-field[data-color-key]');
            ...
            if (input) {
                ...
                if (section === 'typography') {
                    ...
                    if (inputId === 'typography-color-body') {
                        ...
                        themeDefault = this.themeData.elements?.body?.color ||
                        ...
                        themeDefault = this.themeData.elements?.heading?.[`${headingLevel}-color`] ||
                                      this.themeData.colors?.text;
                    } else {
    ...
    updateTypographyTooltip(type, tooltip) {
        ...
        const rows = [
            ...
            { label: 'Letter Spacing', value: settings.letterSpacing, computed: computedStyles.letterSpacing },
            { label: 'Text Transform', value: settings.textTransform, computed: computedStyles.textTransform },
        ];

        // Add non-default properties
        if (settings.wordSpacing && settings.wordSpacing !== 'normal') {
            rows.push({ label: 'Word Spacing', value: settings.wordSpacing, computed: computedStyles.wordSpacing });
        }
        if (settings.textDecoration && settings.textDecoration !== 'none') {
            rows.push({ label: 'Text Decoration', value: settings.textDecoration, computed: computedStyles.textDecoration });
        }
        if (settings.textAlign && settings.textAlign !== 'left') {
            rows.push({ label: 'Text Align', value: settings.textAlign, computed: computedStyles.textAlign });
        }

        tooltip.innerHTML = rows.map(row => {
    ...
    getTypographySettings(type) {
        ...
        if (type === 'menu') {
            ...
            return {
                ...
                textDecorationStyle: menuBranding['text-decoration-style'] || menuTheme['text-decoration-style'] || 'solid',
        ...
        const getValue = (headingKey, bodyKey, brandingKey) => {
            ...
            const brandingValue = typography[`${brandingKey}_${type}`] || typography[`font_${brandingKey}_${type}`];
            if (brandingValue) return brandingValue;

            // Check theme typography
            const themeValue = themeTypography[`${headingKey}-${type}`] || themeTypography[`${type}-${headingKey}`];
        ...
        return {
            ...
            textDecorationStyle: getValue('text-decoration-style', 'text-decoration-style', 'text_decoration_style') || 'solid',
    ...
    getBorderSettings(context) {
        ...
        const getValue = (key) => {
            return borders[`${context}-${key}`] || borders[`${context}_${key}`] ||
                   themeBorders[`${context}-${key}`] || themeBorders[`${context}_${key}`];
        };
    ...
    persistBorder(context, borderData, options = {}) {
        ...
        if (borderData.cornersLinked) {
            this.brandingData.borders[`${context}-radius`] = `${borderData.radius}${borderData.radiusUnit}`;
        } else {
    ...
    updateButtonTypographyTokens(settings, saveToServer) {
        ...
        if (settings.textTransform !== undefined) buttonTokens['text-transform'] = settings.textTransform;
    ...
    initializeCardShadowEditors() {
        ...
        cardTypes.forEach(cardType => {
            shadowProperties.forEach(prop => {
                const inputId = prop === 'shadow'
                    ? `card-${cardType}-shadow`
                    : `card-${cardType}-shadow-hover`;
    ...
    getCardTokenValue(cardType, property) {
        const tokenKey = `card-${cardType}`;
        return this.brandingData[tokenKey]?.[property]
            || this.themeData[tokenKey]?.[property]
            || '';
    }
    ...
    openShadowEditor(key) {
        ...
        const shadowConfigs = {
            'menu-dropdown': { inputId: 'menu-dropdown-shadow', section: 'menu', property: 'dropdown-shadow', shadowType: 'box' },
            'modal': { inputId: 'modal-shadow', section: 'elements.modal', property: 'shadow', shadowType: 'box' },
            'form-shadow': { inputId: 'form-shadow', section: 'elements.form', property: 'shadow', shadowType: 'box' },
            'form-focus-shadow': { inputId: 'form-focus-shadow', section: 'elements.form', property: 'focus-shadow', shadowType: 'box' },
            'search-dropdown-shadow': { inputId: 'search-dropdown-shadow', section: 'search', property: 'dropdown-shadow', shadowType: 'box' },
            'search-input-focus-shadow': { inputId: 'search-input-focus-shadow', section: 'search', property: 'input-focus-shadow', shadowType: 'box' }
        };
        ...
        if (!this.shadowEditors[key]) {
            ...
            if (config.section.startsWith('elements.')) {
                ...
                rawValue = this.brandingData.elements?.[elementType]?.[config.property] ||
                          this.themeData.elements?.[elementType]?.[config.property] || '';
            } else {
                rawValue = this.brandingData[config.section]?.[config.property] ||
    ...
    initializeSpacingEditors() {
        ...
        spacingConfigs.forEach(config => {
            ...
            const currentValue = this.brandingData[config.section]?.[config.property] ||
                                this.themeData[config.section]?.[config.property] || '';
            ...
            btn.addEventListener('click', (e) => {
                ...
                const latestValue = this.brandingData[config.section]?.[config.property] ||
                                   this.themeData[config.section]?.[config.property] || '';
    ...
    initializeShadowEditors() {
        ...
        const shadowInputs = [
            { key: 'menu-dropdown', inputId: 'menu-dropdown-shadow', section: 'menu', property: 'dropdown-shadow' },
            ...
            { key: 'form-focus-shadow', inputId: 'form-focus-shadow', section: 'elements.form', property: 'focus-shadow' },
            { key: 'search-dropdown-shadow', inputId: 'search-dropdown-shadow', section: 'search', property: 'dropdown-shadow' },
            { key: 'search-input-focus-shadow', inputId: 'search-input-focus-shadow', section: 'search', property: 'input-focus-shadow' }
        ];
    ...
    loadBrandingData() {
        ...
        if (this.brandingData.typography?.font_family_heading && !this.brandingData.typography?.font_family_h1) {
        ...
        typographyFields.forEach(field => {
            ...
            if (input && this.mergedData.typography) {
                // Try both formats
                const value = this.mergedData.typography[`font_family_${field}`] ||
    ...
    scopeCSSToPreview(css) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('{') && !line.trim().startsWith('#preview-frame')) {
                ...
                let selector = line.substring(0, braceIndex).trim();
    ...
    getTypographyInlineStyles(type) {
        ...
        if (type === 'h1' && Object.keys(typography).length > 0) {
            console.log('Typography data sample:', Object.keys(typography).filter(k => k.includes('h1') || k.includes('body')).slice(0, 10));
        }
    ...
    async resetToThemeDefault(section, property) {
        ...
        if (this.brandingData[section]) {
            ...
            if (section === 'typography' && property.startsWith('font_family_')) {
                ...
                const relatedProps = ['size', 'weight', 'style', 'line_height', 'letter_spacing', 'word_spacing', 'text_transform', 'text_align', 'text_decoration', 'text_decoration_style', 'text_indent', 'font_variant', 'vertical_align'];
        ...
        if (section === 'colors') {
            ...
            if (property.startsWith('color_')) {
                ...
                if (colorTarget === 'body') {
                    ...
                    themeDefault = this.themeData.elements?.heading?.[`${colorTarget}-color`] ||
                                  this.themeData.colors?.text || '';
                }
        ...
        if (input) {
            if (themeDefault) {
                ...
                else if (['header', 'footer', 'menu', 'search'].includes(section) &&
                ...
                else if ((section === 'elements' || section.startsWith('elements.')) &&
                         input.classList.contains('color-input')) {
                    // Resolve CSS variable to actual hex color for display
                    const resolvedColor = this.resolveCssVariable(themeDefault) || themeDefault;
                    input.value = resolvedColor;
                ...
                input.value = '';
                if (section === 'colors' ||
                    (['header', 'footer', 'menu', 'search'].includes(section) && input.classList.contains('color-input')) ||
                    ((section === 'elements' || section.startsWith('elements.')) && input.classList.contains('color-input'))) {
    ...
    cleanupElementTypographyTokens(type) {
        ...
        if (this.brandingData.elements[elementCategory]) {
            ...
            const typographyKeys = ['font-family', 'font-size', 'font-weight', 'font-style', 'line-height',
                                    'letter-spacing', 'word-spacing', 'text-transform', 'text-align',
                                    'text-decoration', 'text-decoration-style', 'text-indent',
                                    'vertical-align', 'font-variant'];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.resetToThemeDefault = (section, property) => window.brandingBuilder.resetToThemeDefault(section, property);
});