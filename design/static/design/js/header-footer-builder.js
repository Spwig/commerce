    ...
    groups: [
      {
        ...
        fields: [
          ...
          },
          {
            ...
            default: '/cart/',
            placeholder: '/cart/',
            help: 'URL for cart page (only used when Click Action is "Go to Cart Page")',
          },
        ],
      },
      {
        ...
        fields: [
          ...
          },
          {
            ...
            type: 'toggle',
            default: false,
            help: 'Display "Cart" text next to the icon',
          },
          {
            key: 'text',
            label: 'Button Text',
            type: 'text',
            placeholder: 'Cart',
            help: 'Custom text label (leave empty for default)',
          },
          {
            key: 'show_total',
            ...
            type: 'toggle',
            default: false,
            help: 'Display the cart total amount',
          },
        ],
      },
    ],
        ...
        fields: [
          {
            ...
            label: 'Account Menu',
            type: 'select',
            dynamicOptions: 'account_menus',
            placeholder: 'Use default account menu',
            help: 'Select which menu to display. Leave empty for default.',
          },
        ],
      },
      {
        ...
        fields: [
          {
            ...
            type: 'toggle',
            default: false,
            help: 'Display user avatar when logged in',
          },
          {
            key: 'show_name',
            label: 'Show Name',
            type: 'toggle',
            default: false,
            help: 'Display username when logged in',
          },
          {
            key: 'show_text',
            ...
            type: 'toggle',
            default: true,
            help: 'Show text labels on login/register buttons',
          },
          {
            key: 'show_register',
            ...
            type: 'toggle',
            default: true,
            help: 'Show register button for anonymous users',
          },
        ],
      },
    ],
    ...
    groups: [
      {
        ...
        fields: [
          ...
          {
            key: 'show_current_only',
            label: 'Compact Mode',
            type: 'toggle',
            default: false,
            help: 'Show only current currency',
          },
          {
    ...
    groups: [
      {
        ...
        fields: [
          ...
          },
          {
            ...
            label: 'Description',
            type: 'textarea',
            placeholder: 'Get updates on new products and promotions...',
          },
          {
            key: 'placeholder',
        ...
        fields: [
          {
            ...
            label: 'Widget ID',
            type: 'text',
            placeholder: 'custom-widget-1',
            help: 'Unique identifier for CSS/JS targeting',
          },
          {
            key: 'content',
            label: 'HTML Content',
...
class WidgetPropertyRenderer {
  ...
  render(widgetData) {
    ...
    const schema =
      this.builder.widgetSchemas?.[widgetData.widget_type] ||
  ...
  buildTypographySummaryFromObject(typo) {
    ...
    if (typo.fontFamily) {
      ...
      const fontName = typo.fontFamily
        .split(',')[0]
        .replace(/['"]/g, '')
        .replace('var(--theme-typography-font-family-', '')
        .replace(')', '');
      parts.push(fontName);
    }
    ...
    document.querySelectorAll('[data-color-trigger]').forEach(trigger => {
      ...
      if (window.ColorPickerUtility) {
        ...
        trigger.addEventListener('click', async () => {
          const color = await AdminModal.prompt({
            message: 'Enter color (hex, rgb, or CSS variable):',
    ...
    document.querySelectorAll('[data-typography-trigger]').forEach(trigger => {
      ...
      if (window.TypographyEditor) {
        ...
        trigger.addEventListener('click', async () => {
          ...
          const newValue = await AdminModal.prompt({
            message: 'Edit typography JSON:',
  ...
  selectIcon(targetId, icon) {
    const input = document.getElementById(
      targetId.startsWith('prop-') ? targetId : `prop-${targetId}`
    );
    const preview =
      document.getElementById(`icon-preview-${targetId}`) ||
      document.getElementById(`icon-preview-prop-${targetId}`);
...
class HeaderFooterBuilder {
  ...
  showZoneConfig(primaryZone) {
    ...
    const zoneConfig =
      this.currentData.zone_overrides?.[primaryZone] || this.getDefaultZoneConfig(primaryZone);
  ...
  getDefaultZoneConfig(zoneName = 'main-header') {
    ...
    const headerZoneDefaults = {
      'top-bar': {
        ...
        background: {
          type: 'theme',
          value:
            'var(--theme-header-zones-top-bar-background, var(--theme-color-background-secondary))',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-header-zones-top-bar-text-color, var(--theme-color-text-muted))',
        },
        visibility: { mobile: false, tablet: true, desktop: true },
        spacing: {
          padding: 'var(--theme-header-zones-top-bar-padding-y, var(--theme-space-2)) 0',
          margin: '0',
        },
      },
      'main-header': {
        ...
        background: {
          type: 'theme',
          value: 'var(--theme-header-zones-main-header-background, var(--theme-color-surface))',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-header-zones-main-header-text-color, var(--theme-color-text))',
        },
        visibility: { mobile: true, tablet: true, desktop: true },
        spacing: {
          padding: 'var(--theme-header-zones-main-header-padding-y, var(--theme-space-4)) 0',
          margin: '0',
        },
        sticky: true,
      ...
      'bottom-bar': {
        ...
        background: {
          type: 'theme',
          value:
            'var(--theme-header-zones-bottom-bar-background, var(--theme-color-background-secondary))',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-header-zones-bottom-bar-text-color, var(--theme-color-text))',
        },
        visibility: { mobile: false, tablet: true, desktop: true },
        spacing: {
          padding: 'var(--theme-header-zones-bottom-bar-padding-y, var(--theme-space-3)) 0',
          margin: '0',
        },
      },
      'mega-menu-bar': {
        ...
        background: {
          type: 'theme',
          value:
            'var(--theme-header-zones-mega-menu-bar-background, var(--theme-color-background-secondary))',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-header-zones-mega-menu-bar-text-color, var(--theme-color-text))',
        },
        visibility: { mobile: false, tablet: false, desktop: true },
        spacing: {
          padding: 'var(--theme-header-zones-mega-menu-bar-padding-y, var(--theme-space-4)) 0',
          margin: '0',
        },
      },
    };
    ...
    const footerZoneDefaults = {
      top: {
        ...
        background: {
          type: 'theme',
          value: 'var(--theme-footer-zones-top-background, transparent)',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-footer-text-color, var(--theme-color-text-inverse))',
        },
        visibility: { mobile: true, tablet: true, desktop: true },
        spacing: {
          padding: 'var(--theme-footer-zones-top-padding-y, var(--theme-space-12)) 0',
          margin: '0',
        },
      },
      main: {
        ...
        background: {
          type: 'theme',
          value: 'var(--theme-footer-zones-main-background, transparent)',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-footer-text-color, var(--theme-color-text-inverse))',
        ...
        visibility: { mobile: true, tablet: true, desktop: true },
        spacing: {
          padding: 'var(--theme-footer-zones-main-padding-y, var(--theme-space-8)) 0',
          margin: '0',
        },
      },
      bottom: {
        ...
        background: {
          type: 'theme',
          value: 'var(--theme-footer-zones-bottom-background, rgba(0, 0, 0, 0.2))',
        },
        text_color: {
          type: 'theme',
          value: 'var(--theme-footer-text-color, var(--theme-color-text-inverse))',
        ...
        visibility: { mobile: true, tablet: true, desktop: true },
        spacing: {
          padding: 'var(--theme-footer-zones-bottom-padding-y, var(--theme-space-6)) 0',
          margin: '0',
        },
      },
      'bottom-bar': {
    ...
    if (this.builderType === 'footer') {
      ...
      if (footerZoneDefaults[zoneName]) {
        ...
        zoneName.startsWith('col-') ||
        zoneName.startsWith('section-') ||
        zoneName === 'single'
      ) {
    ...
    return {
      ...
      background: zoneOverrides.background || {
        type: 'theme',
        value: 'var(--theme-header-zones-main-header-background, var(--theme-color-surface))',
      },
  ...
  async saveZoneConfig(primaryZone) {
    const config = {
      ...
      visibility: {
        mobile:
          document
            .querySelector('.device-visibility-btn[data-device="mobile"]')
            ?.classList.contains('active') !== false,
        tablet:
          document
            .querySelector('.device-visibility-btn[data-device="tablet"]')
            ?.classList.contains('active') !== false,
        desktop:
          document
            .querySelector('.device-visibility-btn[data-device="desktop"]')
            ?.classList.contains('active') !== false,
      },
      border_bottom: {
  ...
  renderPreview() {
    ...
    zones.forEach(zone => {
      ...
      if (primaryZone !== currentPrimaryZone) {
        ...
        if (
          this.builderType === 'header' &&
          this.currentDevice === 'mobile' &&
          primaryZone === 'main-header' &&
          menuPosition === 'left' &&
          !hamburgerInjected
        ) {
          html += `
  ...
  async fetchServerPreviews() {
    ...
    for (const widgetEl of widgetsNeedingPreview) {
      ...
      try {
        const previewHtml = await this.fetchWidgetPreview(
    ...
    switch (widget.widget_type) {
      ...
      case 'site_variable':
        ...
        if (config.typography) {
          try {
            const typo =
              typeof config.typography === 'string'
                ? JSON.parse(config.typography)
                : config.typography;
  ...
  async showPropertiesPanel(widgetData) {
    ...
    const selectedWrapper = document.querySelector(
      `.widget-wrapper[data-placement-id="${widgetData.id}"]`
    );
    if (selectedWrapper) {
  ...
  async deleteWidget(placementId) {
    if (
      !(await AdminModal.confirm({
        message:
          this.config.translations.confirmDelete || 'Are you sure you want to remove this widget?',
        danger: true,
        confirmText: 'Remove',
      }))
    ) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (config && config.forceLightMode) {
    ...
    if (previewEl) {
      previewEl.setAttribute('data-theme', 'light');
    }
  }
