...
(function () {
  ...
  const checkoutCards = JSON.parse(container.dataset.checkoutCards || '[]');
  const productCards = JSON.parse(container.dataset.productCards || '[]');
  const categoryCards = JSON.parse(container.dataset.categoryCards || '[]');
  const blogPostCards = JSON.parse(container.dataset.blogPostCards || '[]');
  const blogListCards = JSON.parse(container.dataset.blogListCards || '[]');
  ...
  function renderOptions(type, templateKey) {
    ...
    if (entries.length === 0) {
      gridEl.innerHTML =
        '<p style="color: var(--body-quiet-color); font-size: 0.875rem;">No configurable options for this template.</p>';
      return;
    }
  ...
  function createBadgeEditor(opts) {
    const badges = JSON.parse(container.dataset[opts.dataKey] || '[]');
    ...
    function render() {
      ...
      if (badges.length === 0) {
        ...
        listEl.innerHTML = badges
          .map((badge, index) => {
            const iconOptions = TRUST_BADGE_ICONS.map(
              ic =>
    ...
    function bindEvents() {
      ...
      listEl.querySelectorAll('.trust-badge-editor__icon-select').forEach(select => {
        select.addEventListener('change', function () {
          ...
          const preview = this.closest('.trust-badge-editor__item').querySelector(
            '.trust-badge-editor__icon-preview i'
          );
