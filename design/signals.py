...
def invalidate_cache_on_token_save(sender, instance, created, **kwargs):
    ...
    logger.info(
        f"DesignToken '{instance.name}' {action}, invalidating token caches"
    )
...
def invalidate_cache_on_token_delete(sender, instance, **kwargs):
    ...
    """
    logger.info(
        f"DesignToken '{instance.name}' deleted, invalidating token caches"
    )
    invalidate_token_cache()
    ...
    if created or previous_theme_id != current_theme_id:
        ...
        if instance.active_theme:
            try:
                ...
                created_count, updated_count, deleted_count = TokenSyncService.sync_theme_to_design_tokens(
...
def sync_branding_to_design_tokens(sender, instance, created, **kwargs):
    ...
    try:
        from .token_sync_service import TokenSyncService
        created_count, updated_count, deleted_count = TokenSyncService.sync_branding_to_design_tokens(
            instance
        )
        logger.info(
    ...
    if instance.pk:
        try:
            ...
            instance._tokens_changed = any(
                getattr(old_instance, f, None) != getattr(instance, f, None)
...
def regenerate_css_and_clear_cache(sender, instance, created, **kwargs):
    ...
    if tokens_changed or css_missing:
        try:
            ...
            logger.info(
                f"ThemeBranding CSS regenerated, new hash: {instance.css_hash}"
            )
