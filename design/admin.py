...
from . import (
    branding_admin,  # noqa: F401  # Enhanced branding admin replaces the basic one
    header_footer_admin,  # noqa: F401
)
from .models import (
    ComponentStore,
    ComponentStyle,
    ...
    PageTier,
    TierComponentPermission,
)
from .theme_models import Theme, ThemeAsset, ThemeInstallation
...
class DesignTokenAdmin(admin.ModelAdmin):
    ...
    actions = [
        "sync_theme_tokens",
        "sync_all_theme_tokens",
        "cleanup_orphaned_tokens",
        "delete_tokens_for_inactive_themes",
    ]

    fieldsets = (
        (_("Basic Information"), {"fields": ("name", "token_type", "value", "description")}),
        (
            _("Priority & Source"),
            {
    ...
    def sync_theme_tokens(self, request, queryset):
        ...
        self.message_user(
            request, _("Synced tokens from %(count)d theme(s)") % {"count": len(themes_synced)}
        )
    ...
    def sync_all_theme_tokens(self, request, queryset):
        ...
        self.message_user(
            request,
            _(
                "Synced %(themes)d theme(s): %(created)d created, %(updated)d updated, %(deleted)d deleted"
            )
            % {
                "themes": themes_count,
...
class PageTierAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("page_type", "tier", "display_name", "description")}),
        (_("Page Structure"), {"fields": ("schema", "locked_regions"), "classes": ("collapse",)}),
        (
            _("Security Settings"),
            {
                "fields": ("csp_policy", "max_external_scripts", "allows_custom_html"),
                "description": (
...
class ComponentStoreAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            _("Basic Information"),
            {"fields": ("component_type", "display_name", "description", "version", "author")},
        ),
        (
            _("Capabilities & Restrictions"),
            {"fields": ("capabilities", "allowed_tiers", "render_mode")},
        ),
        (
            _("Security Settings"),
            {
                "fields": ("external_domains", "script_budget_kb", "requires_sandbox"),
    ...
    def signature_status(self, obj):
        ...
        if obj.is_signed():
            ...
            if is_valid:
                return format_html('<span style="color: #28a745;">● {}</span>', _("Signed"))
            else:
                return format_html('<span style="color: #dc3545;">● {}</span>', _("Invalid"))
        else:
            return format_html('<span style="color: #6c757d;">○ {}</span>', _("Unsigned"))
    ...
    def sign_components(self, request, queryset):
        ...
        for component in queryset:
            ...
            if component.review_status != "approved":
                self.message_user(
                    request,
                    _("Skipped %(component)s - not approved") % {"component": component},
                    level="warning",
                )
                continue
            ...
            if success:
                ...
                self.message_user(
                    ...
                    % {"component": component, "error": message},
                    level="error",
                )
        ...
        if signed_count > 0:
            self.message_user(
                request,
                _("Successfully signed %(count)d component(s)") % {"count": signed_count},
                level="success",
            )
    ...
    def verify_components(self, request, queryset):
        ...
        for component in queryset:
            ...
            if is_valid:
                ...
                self.message_user(
                    request,
                    _("%(component)s: %(message)s") % {"component": component, "message": message},
                    level="error",
                )
        ...
        self.message_user(
            request,
            _("Verification complete: %(summary)s") % {"summary": ", ".join(summary_parts)},
    ...
    def validation_status(self, obj):
        ...
        if not latest_report:
            return format_html('<span style="color: #6c757d;">○ {}</span>', _("Not Validated"))
        ...
        if latest_report.is_valid:
            if latest_report.warning_count() > 0:
                ...
                return format_html('<span style="color: #28a745;">● {}</span>', _("Valid"))
        else:
            return format_html(
    ...
    def validate_components(self, request, queryset):
        ...
        for component in queryset:
            ...
            ComponentValidationReport.objects.create(
                component=component,
            ...
            if is_valid:
                ...
                if errors:
                    self.message_user(
                        ...
                        % {"component": component, "error": errors[0]},
                        level="error",
                    )
        ...
        self.message_user(
            ...
            % {"total": validated_count, "summary": ", ".join(summary)},
            level="success" if invalid_count == 0 else "warning",
        )
    ...
    def approve_components(self, request, queryset):
        ...
        for component in queryset:
            ...
            if not latest_report:
                self.message_user(
                    request,
                    _("%(component)s has not been validated") % {"component": component},
                    level="warning",
                )
                skipped_count += 1
                continue
            ...
            if not latest_report.is_valid:
                self.message_user(
                    request,
                    _("%(component)s has validation errors") % {"component": component},
                    level="error",
                )
                skipped_count += 1
                continue
            ...
            if not component.is_signed():
                self.message_user(
                    request,
                    _("%(component)s is not signed") % {"component": component},
                    level="warning",
                )
                skipped_count += 1
                continue
    ...
    def apply_system_permissions(self, request, queryset):
        ...
        self.message_user(
            request,
            _(
                "Applied system permissions to %(count)d component(s), "
                "created %(perms)d tier permissions"
            )
            % {"count": queryset.count(), "perms": total_permissions},
            level="success",
        )
    ...
    def apply_marketing_permissions(self, request, queryset):
        ...
        self.message_user(
            request,
            _(
                "Applied marketing permissions to %(count)d component(s), "
                "created %(perms)d tier permissions"
            )
            % {"count": queryset.count(), "perms": total_permissions},
            level="success",
    ...
    def apply_product_permissions(self, request, queryset):
        ...
        self.message_user(
            request,
            _(
                "Applied product permissions to %(count)d component(s), "
                "created %(perms)d tier permissions"
            )
            % {"count": queryset.count(), "perms": total_permissions},
            level="success",
    ...
    def remove_all_permissions(self, request, queryset):
        ...
        self.message_user(
            ...
            % {"count": total_deleted, "components": queryset.count()},
            level="success",
        )
...
class ComponentValidationReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Results"), {"fields": ("is_valid", "errors_display", "warnings_display")}),
    )
...
class ThemeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Package"), {"fields": ("package_file", "package_checksum", "extracted_path")}),
        (_("Configuration"), {"fields": ("manifest", "token_migrations", "preview_images")}),
        (
            _("Status"),
            {
                "fields": (
    ...
    def sync_tokens_for_selected(self, request, queryset):
        ...
        self.message_user(
            request,
            _(
                "Synced %(count)d theme(s): %(created)d created, %(updated)d updated, %(deleted)d deleted"
            )
            % {
...
class ThemeInstallationAdmin(admin.ModelAdmin):
    ...
    list_display = ["theme", "installed_at", "installed_by", "previous_theme", "has_migrations"]
...
class ThemeAssetAdmin(admin.ModelAdmin):
    ...
    list_display = ["path", "theme", "asset_type", "size", "is_critical", "route", "created_at"]
