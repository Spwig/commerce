...
def generate_tokens_css(
    tokens_source: Union[str, Path, dict],
    theme_name: str,
    ...
    if 'dark' in tokens and dark_mode_enabled:
        ...
        lines.extend(_generate_dark_mode_media_queries(
            tokens['dark'], original_colors, original_shadows
        ))
...
def _is_responsive_value(value) -> bool:
    ...
    if not isinstance(value, dict):
        return False
    return any(key in VALID_BREAKPOINTS for key in value.keys())
...
def _generate_colors_section(colors: dict) -> list:
    ...
    categories = {
        ...
        'Status': ['success', 'success-light', 'error', 'error-light',
                  'warning', 'warning-light', 'info', 'info-light'],
        'Overlay': ['overlay'],
    }
    ...
    remaining = {k: v for k, v in colors.items()
...
def _generate_dark_section(dark: dict) -> list:
    ...
    for category, keys in categories.items():
        category_tokens = {k: v for k, v in dark.items()
    ...
    remaining = {k: v for k, v in dark.items()
                 if k not in all_categorised_keys and v is not None}
    if remaining:
        lines.append('  /* Other */')
        for key, value in remaining.items():
            value = _transform_value_references(value)
...
def _generate_typography_section(typography: dict) -> list:
    ...
    # Group by type
    groups = {
        'Font Families': ['font-sans', 'font-serif', 'font-mono', 'font-family-body', 'font-family-heading'],
