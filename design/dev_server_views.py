...
def dev_server_enabled(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if not settings.DEBUG:
            return JsonResponse({
                'error': 'Dev server is only available in DEBUG mode'
            }, status=403)
...
def require_dev_token(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not token:
            return JsonResponse({
                'error': 'Missing X-Dev-Token header'
            }, status=401)
        ...
        if not session:
            return JsonResponse({
                'error': 'Invalid or expired dev token'
            }, status=401)
    ...
    for file_data in files:
        ...
        if encoding == 'base64':
            try:
                ...
                return JsonResponse({
                    'error': f"Failed to decode base64 for {processed['path']}"
                }, status=400)
        else:
