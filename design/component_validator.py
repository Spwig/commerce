...
class ComponentValidator:
    ...
    def _extract_package(self) -> bool:
        ...
        try:
            ...
            with zipfile.ZipFile(self.component.package_file.path, 'r') as zip_ref:
                ...
                for member in zip_ref.namelist():
                    ...
                    if member.startswith('/') or '..' in member:
                        self.errors.append(
                            str(_("Package contains invalid path: %(path)s") % {
                                'path': member
                            })
                        )
                        return False
            ...
            self.errors.append(
                str(_("Failed to extract package: %(error)s") % {'error': str(e)})
            )
            return False
    ...
    def validate_manifest(self):
        ...
        if manifest_size > self.MAX_MANIFEST_SIZE:
            self.errors.append(
                str(_("Manifest file too large: %(size).1f KB (max: %(max)d KB)") % {
                    'size': manifest_size,
        ...
        try:
            ...
            self.errors.append(
                str(_("Invalid JSON in manifest: %(error)s") % {'error': str(e)})
            )
            return
        except Exception as e:
            self.errors.append(
                str(_("Failed to read manifest: %(error)s") % {'error': str(e)})
            )
            return
        ...
        for field in required_fields:
            if field not in self.manifest_data:
                self.errors.append(
                    str(_("Missing required field in manifest: %(field)s") % {
                        'field': field
                    })
                )
        ...
        if 'version' in self.manifest_data:
            ...
            if not re.match(r'^\d+\.\d+\.\d+$', version):
                self.warnings.append(
                    str(_("Version should use semantic versioning (e.g., 1.0.0)"))
                )
        ...
        if 'component_type' in self.manifest_data:
            ...
            if not re.match(r'^[a-z_]+$', component_type):
                self.errors.append(
                    str(_("component_type must be lowercase with underscores only"))
                )
    ...
    def validate_template(self):
        ...
        if template_size > self.MAX_TEMPLATE_SIZE:
            self.errors.append(
                str(_("Template file too large: %(size).1f KB (max: %(max)d KB)") % {
                    'size': template_size,
                    'max': self.MAX_TEMPLATE_SIZE
                })
            )
        ...
        try:
            ...
            self.errors.append(
                str(_("Failed to read template: %(error)s") % {'error': str(e)})
            )
            return
        ...
        try:
            ...
            self.errors.append(
                str(_("Template syntax error: %(error)s") % {'error': str(e)})
            )
        ...
        for tag in self.DANGEROUS_TEMPLATE_TAGS:
            ...
            if re.search(pattern, template_content, re.IGNORECASE):
                self.errors.append(
                    str(_("Forbidden template tag: %(tag)s") % {'tag': tag})
                )
        ...
        if event_handlers:
            self.errors.append(
                str(_("Inline event handlers not allowed: %(handlers)s") % {
    ...
    def validate_assets(self):
        ...
        for css_file in assets_dir.glob('**/*.css'):
            ...
            if css_size > self.MAX_CSS_SIZE:
                self.warnings.append(
                    str(_("CSS file %(file)s is large: %(size).1f KB") % {
                        'file': css_file.name,
                        'size': css_size
                    })
                )
        ...
        for js_file in assets_dir.glob('**/*.js'):
            ...
            if js_size > self.MAX_JS_SIZE:
                self.warnings.append(
                    str(_("JS file %(file)s is large: %(size).1f KB") % {
                        'file': js_file.name,
                        'size': js_size
                    })
                )
        ...
        if self.component.script_budget_kb > 0:
            if total_script_size > float(self.component.script_budget_kb):
                self.errors.append(
                    str(_("Script size %(actual).1f KB exceeds budget %(budget).1f KB") % {
            ...
            try:
                ...
                for pattern in self.DANGEROUS_PATTERNS['xss']:
                    if re.search(pattern, content, re.IGNORECASE):
                        self.warnings.append(
                            str(_("Potential XSS pattern in %(file)s: %(pattern)s") % {
                                'file': file_path.name,
                                'pattern': pattern
                            })
                        )
                ...
                for pattern in self.DANGEROUS_PATTERNS['sql_injection']:
                    if re.search(pattern, content, re.IGNORECASE):
                        self.warnings.append(
                            str(_("Potential SQL injection pattern in %(file)s") % {
                                'file': file_path.name
                            })
                        )
                ...
                for pattern in self.DANGEROUS_PATTERNS['file_inclusion']:
                    if re.search(pattern, content):
                        self.errors.append(
                            str(_("Path traversal detected in %(file)s") % {
                                'file': file_path.name
                            })
                        )
    ...
    def validate_permissions(self):
        ...
        if not self.component.capabilities:
            self.warnings.append(
                str(_("Component does not declare any capabilities"))
            )

        # Check if component is allowed in at least one tier
        if not self.component.allowed_tiers:
            self.warnings.append(
                str(_("Component is not allowed in any tier"))
            )
        ...
        if 'external_scripts' in capabilities:
            if not self.component.external_domains:
                self.errors.append(
        ...
        if 'custom_html' in capabilities:
            if not self.component.requires_sandbox:
                self.warnings.append(
