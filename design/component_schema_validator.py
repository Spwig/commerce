...
class ComponentSchemaValidator:
    ...
    def _load_schema(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise ComponentValidationError(f"Manifest schema not found at {self.schema_path}")
        except json.JSONDecodeError as e:
            raise ComponentValidationError(f"Invalid JSON in manifest schema: {e}")

    def validate_component(self, component_dir: Path) -> tuple[bool, list[str]]:
        """
    ...
    def _validate_props_schema(self, schema_path: Path, manifest: dict[str, Any]) -> bool:
        ...
        if "props_schema" in manifest and props_schema != manifest["props_schema"]:
            self.errors.append("Props schema in schema.json does not match manifest.props_schema")
            return False

        # Validate it's a valid JSON Schema
        if jsonschema is None:
            logger.warning("jsonschema library not installed - skipping props schema validation")
            return True
    ...
    def _validate_assets(self, component_dir: Path, assets: dict[str, list[str]]) -> bool:
    ...
    def _validate_locales(self, component_dir: Path, locales: list[str]) -> bool:
    ...
    def _validate_preview(self, component_dir: Path, preview_filename: str) -> bool:
        ...
        if preview_path.stat().st_size > max_size:
            self.errors.append(f"Preview image is too large (max 5MB): {preview_filename}")
            return False
    ...
    def _compare_versions(self, v1: str, v2: str) -> int:
        ...
        # strict=False: version parts can differ in length (e.g. "1.2" vs "1.2.3"); shorter version treated as smaller
