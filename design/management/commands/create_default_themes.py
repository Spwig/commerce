...
class Command(BaseCommand):
        ...
        for theme_data in themes_to_create:
            ...
            if existing:
                if force:
                    existing.delete()
                    self.stdout.write(
                        self.style.WARNING(f"Deleted existing theme '{theme_data['slug']}'")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Theme '{theme_data['slug']}' already exists, skipping (use --force to recreate)"
                        )
                    )
                    continue
    ...
    def load_tokens_from_filesystem(self, slug: str) -> dict:
        ...
        components_dir = Path(
            getattr(settings, "SPWIG_COMPONENTS_DIR", settings.BASE_DIR / "components")
        )
    ...
    def _ensure_component_registry(self, theme_data, manifest):
        ...
        existing = ComponentRegistry.objects.filter(component_type="theme", slug=slug).first()
