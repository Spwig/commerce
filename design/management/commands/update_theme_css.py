...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for theme in themes:
            ...
            if installed_base.exists() and installed_css_dir.exists() and (installed_css_dir / 'tokens.css').exists():
            ...
            if not theme_dir.exists():
                self.stdout.write(
                    self.style.WARNING(f'CSS directory not found for {theme.slug} v{version}: {theme_dir}')
                )
                continue
            ...
            if tokens_path.exists():
                ...
                self.stdout.write(f"  - Skipped tokens.json (not found)")
    ...
    def _ensure_starter_theme_active(self):
        ...
        if not starter_theme:
            self.stdout.write(
                self.style.WARNING('Starter theme not found in database')
            )
            return
