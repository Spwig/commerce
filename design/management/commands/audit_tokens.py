...
class Command(BaseCommand):
    ...
    def _audit_report(self):
        ...
        # Orphaned tokens
        orphaned_theme = DesignToken.objects.filter(
        ...
        if orphaned_theme == 0 and orphaned_component == 0:
            ...
            if orphaned_theme > 0:
                self.stdout.write(self.style.WARNING(
                    f'  {orphaned_theme} theme token(s) with no theme FK'
                ))
    ...
    def _cleanup_orphans(self, dry_run):
        ...
        orphaned_theme_qs = DesignToken.objects.filter(
            source='theme', theme__isnull=True
        )
        ...
        if dry_run:
            ...
            if orphaned_theme_count:
                ...
                deleted += count
                self.stdout.write(self.style.SUCCESS(
                    f'  Deleted {count} orphaned theme token(s)'
                ))
            if orphaned_component_count:
                count, _ = orphaned_component_qs.delete()
                deleted += count
                self.stdout.write(self.style.SUCCESS(
                    f'  Deleted {count} orphaned component token(s)'
                ))
