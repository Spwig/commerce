...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not theme_slug and not process_all:
            self.stdout.write(self.style.ERROR("Please specify --theme <slug> or --all"))
            return
        ...
        for theme_dir in theme_dirs:
            if not theme_dir.exists():
                self.stdout.write(self.style.ERROR(f"Theme directory not found: {theme_dir}"))
                failed_count += 1
                continue
            ...
            if (theme_dir / "tokens.json").exists():
                ...
                self.stdout.write(
                    self.style.WARNING(f"No tokens.json found for {theme_name}, skipping")
                )
                continue
            ...
            try:
                ...
                if dry_run:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"✓ Generated: {tokens_css_path}"))

                generated_count += 1

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"✗ Failed to generate for {theme_name}: {e}"))
                failed_count += 1
