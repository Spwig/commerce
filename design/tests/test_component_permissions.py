...
def component_with_permissions(sample_component, tier_a, tier_b, tier_c):
    ...
    TierComponentPermission.objects.create(
        tier=tier_a, component=sample_component, allowed_regions=[], max_instances=-1
    )
    TierComponentPermission.objects.create(
        tier=tier_b, component=sample_component, allowed_regions=["header"], max_instances=2
    )
    ...
    def test_templates_have_required_fields(self):
        ...
        for template_name, template_config in PERMISSION_TEMPLATES.items():
            for field in required_fields:
                assert field in template_config, (
...
class TestApplyPermissionTemplate:
    ...
    def test_apply_template_with_override_max_instances(
        self, sample_component, tier_a, tier_b, tier_c
    ):
        """Test applying template with override settings."""
        permissions = apply_permission_template(
            sample_component, "system", override_settings={"max_instances": 5}
        )
    ...
    def test_apply_template_deletes_existing_permissions(
        self, component_with_permissions, tier_a, tier_b, tier_c
    ):
...
class TestCopyPermissions:
    ...
    def test_copy_permissions_preserves_settings(
        self, component_with_permissions, db, tier_a, tier_b
    ):
    ...
    def test_copy_permissions_deletes_existing_on_target(
        self, component_with_permissions, db, tier_a, tier_b, tier_c
    ):
        ...
        TierComponentPermission.objects.create(
            tier=tier_c, component=target_component, allowed_regions=[], max_instances=-1
        )
...
class TestGetAvailableTemplates:
    ...
    def test_returns_descriptions(self):
        ...
        for _template_name, description in templates.items():
...
class TestGetPermissionSummary:
    ...
    def test_summary_detects_region_restrictions(self, sample_component, tier_a):
        ...
        TierComponentPermission.objects.create(
            tier=tier_a, component=sample_component, allowed_regions=["header"], max_instances=-1
        )
    ...
    def test_summary_detects_max_instances_restrictions(self, sample_component, tier_a):
        ...
        TierComponentPermission.objects.create(
            tier=tier_a, component=sample_component, allowed_regions=[], max_instances=5
        )
    ...
    def test_summary_tiers_are_sorted(self, sample_component, tier_a, tier_b, tier_c):
        ...
        for tier in [tier_c, tier_a, tier_b]:
            TierComponentPermission.objects.create(
                tier=tier, component=sample_component, allowed_regions=[], max_instances=-1
            )
