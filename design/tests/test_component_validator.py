...
class TestManifestValidation:
    ...
    def test_validate_large_manifest(self, valid_component):
        ...
        zip_buffer.seek(0)
        valid_component.package_file.save('large_manifest.zip', ContentFile(zip_buffer.read()), save=True)
...
class TestTemplateValidation:
    ...
    def test_validate_template_syntax_error(self, valid_component, create_component_package):
        ...
        package = create_component_package(
            template='<div>{% invalid_tag %}</div>'
        )
        valid_component.package_file.save('syntax_error.zip', ContentFile(package.read()), save=True)
    ...
    def test_detect_dangerous_load_tag(self, valid_component, create_component_package):
        ...
        package = create_component_package(
            template='{% load custom_tags %}<div>Test</div>'
        )
        ...
        assert is_valid is False
        assert any('forbidden template tag' in err.lower() and 'load' in err.lower() for err in errors)

    def test_detect_dangerous_include_tag(self, valid_component, create_component_package):
        """Test that {% include %} tag is forbidden."""
        package = create_component_package(
            template='{% include "malicious.html" %}'
        )
        ...
        assert is_valid is False
        assert any('forbidden template tag' in err.lower() and 'include' in err.lower() for err in errors)
    ...
    def test_detect_inline_script_tags(self, valid_component, create_component_package):
        ...
        valid_component.package_file.save('inline_script.zip', ContentFile(package.read()), save=True)
    ...
    def test_detect_inline_event_handlers(self, valid_component, create_component_package):
        ...
        valid_component.package_file.save('event_handler.zip', ContentFile(package.read()), save=True)
...
class TestAssetValidation:
    ...
    def test_validate_script_budget_exceeded(self, valid_component, create_component_package):
        ...
        package = create_component_package(assets={'script.js': large_js})
        valid_component.package_file.save('budget_exceed.zip', ContentFile(package.read()), save=True)
...
class TestPermissionValidation:
    ...
    def test_external_scripts_requires_domains(self, db, create_component_package):
        ...
        assert is_valid is False
        assert any('external_scripts' in err.lower() and 'external_domains' in err.lower() for err in errors)
...
class TestConvenienceFunction:
    ...
    def test_validate_component_function(self, valid_component):
        ...
        assert is_valid is True or len(errors) == 0 or all('warning' in str(e).lower() for e in errors)
