...
def template_config_view(request):
    ...
    for key, meta in CHECKOUT_TEMPLATE_META.items():
        ...
        resolved = get_checkout_options(key, config.checkout_options if config.checkout_template == key else {})
    ...
    for key, meta in PRODUCT_TEMPLATE_META.items():
        ...
        resolved = get_product_options(key, config.product_options if config.product_template == key else {})
    ...
    for key, meta in CATEGORY_TEMPLATE_META.items():
        ...
        resolved = get_category_options(key, config.category_options if config.category_template == key else {})
    ...
    for key, meta in BLOG_POST_TEMPLATE_META.items():
        ...
        resolved = get_blog_post_options(key, config.blog_post_options if config.blog_post_template == key else {})
    ...
    for key, meta in BLOG_LIST_TEMPLATE_META.items():
        ...
        resolved = get_blog_list_options(key, config.blog_list_options if config.blog_list_template == key else {})
