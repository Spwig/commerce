...
class HeaderTemplate(models.Model):
    ...
    # Sticky behavior
    is_sticky = models.BooleanField(default=False, help_text=_("Header sticks to top when scrolling"))
    sticky_offset = models.IntegerField(default=0, help_text=_("Pixels from top before sticky activates"))
    ...
    is_preset = models.BooleanField(
        default=False,
        help_text=_("Mark as a preset template that users can clone")
    )
    ...
    # Metadata
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='created_headers')
    ...
    draft_data = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Current draft state being edited in the builder")
    )
    published_data = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Published state shown on the frontend")
    )
    published_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When the template was last published")
    )
    has_unpublished_changes = models.BooleanField(
        default=False,
        help_text=_("True if draft differs from published")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            HeaderTemplate.objects.filter(is_default=True).exclude(pk=self.pk).update(is_default=False)
    ...
    def _sync_widget_placements(self):
        ...
        with transaction.atomic():
            ...
            existing_placements = {
                p.id: p for p in self.widget_placements.all()
            }
...
class FooterTemplate(models.Model):
    ...
    zones = models.JSONField(
        default=dict,
        help_text=_("Widget zones configuration"),
        blank=True
    )
    ...
    bottom_bar_content = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Bottom bar widget configuration")
    )
    ...
    # Metadata
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='created_footers')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        # Ensure only one default
        if self.is_default:
            FooterTemplate.objects.filter(is_default=True).exclude(pk=self.pk).update(is_default=False)
        super().save(*args, **kwargs)
...
class Widget(models.Model):
    ...
    config = models.JSONField(
        default=dict,
        help_text=_("Widget configuration (varies by type)")
    )
...
class WidgetPlacement(models.Model):
    ...
    header = models.ForeignKey(HeaderTemplate, on_delete=models.CASCADE, null=True, blank=True, related_name='widget_placements')
    footer = models.ForeignKey(FooterTemplate, on_delete=models.CASCADE, null=True, blank=True, related_name='widget_placements')

    # Zone within the header/footer
    zone = models.CharField(
        max_length=50,
        help_text=_("Zone identifier (e.g., 'left', 'center', 'right', 'column_1')")
    )
    ...
    override_config = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Override widget config for this placement")
    )
...
class Menu(models.Model):
    ...
    translations = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Translations for menu-level content")
    )
...
class MenuItem(models.Model):
    ...
    # Content
    title = models.CharField(max_length=200, blank=True, help_text=_("Display title (auto-populated from reference if empty)"))
    url = models.CharField(max_length=500, blank=True, help_text=_("URL or path (for link/custom_url types)"))
    ...
    icon = models.CharField(max_length=100, blank=True, help_text=_("FontAwesome icon class (e.g., 'fas fa-home')"))

    # Badge (optional)
    badge_text = models.CharField(max_length=20, blank=True, help_text=_("e.g., 'NEW', 'SALE'"))
    badge_color = models.CharField(max_length=20, blank=True, help_text=_("Badge color (hex or CSS color name)"))
    ...
    mega_menu_content = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Configuration for mega menu layout")
    )
    ...
    translations = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Translations for menu item title")
    )
    ...
    def get_resolved_url(self):
        ...
        elif self.item_type in ('divider', 'header', 'category_tree'):
            return '#'
        elif self.item_type == 'widget':
            return '#'
        return self.url or '#'
        ...
        def build_tree(parent_category=None, current_depth=1):
            ...
            for cat in categories:
                item = {
                    ...
                    'children': build_tree(cat, current_depth + 1) if current_depth < max_depth else []
                }
    ...
    def should_show_for_user(self, user):
        ...
        for rule in self.visibility_rules:
            if rule.get('type') == 'user_status':
                ...
                if required_status == 'logged_in' and not user.is_authenticated:
                    return False
                elif required_status == 'logged_out' and user.is_authenticated:
                    return False
        return True