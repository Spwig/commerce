...
COMPUTED_FIELDS = {
    ...
    "catalog.Category": {
        "product_count": lambda c: str(
            c.products.filter(status="published").count() if hasattr(c, "products") else 0
        ),
    ...
    "catalog.Brand": {
        "product_count": lambda b: str(
            b.products.filter(status="published").count() if hasattr(b, "products") else 0
        ),
    ...
    "blog.BlogPost": {
        ...
        "category_name": lambda p: p.category.name if p.category else "",
        "tags_list": lambda p: (
            ", ".join(t.name for t in p.tags.all()) if hasattr(p, "tags") else ""
        ),
        "url": lambda p: p.get_absolute_url() if hasattr(p, "get_absolute_url") else "",
    },
}
...
def resolve_field_value(instance: Model, field_name: str, thumbnail_preset: str = "") -> str:
    ...
    model_key = f"{instance._meta.app_label}.{instance._meta.model_name}".replace(
        instance._meta.model_name, instance._meta.object_name
    )
...
def apply_bindings_to_element(element, instance: Model, bindings):
    ...
    for binding in element_bindings:
        value = resolve_field_value(instance, binding.model_field, binding.thumbnail_preset)
        if value:
