...
(function () {
  ...
  const _ebConfigEl = document.getElementById('element-builder-config');
  const config = _ebConfigEl
    ? JSON.parse(_ebConfigEl.textContent)
    : window.ElementBuilderConfig || {};
  ...
  async function searchModelItems(query) {
    ...
    try {
      ...
      dropdown.innerHTML =
        '<div class="preview-item-option preview-item-error">Error loading items</div>';
    }
  }
  ...
  function renderPreviewItemDropdown(items) {
    ...
    if (items.length === 0) {
      dropdown.innerHTML =
        '<div class="preview-item-option preview-item-empty">No items found</div>';
      return;
    }
  ...
  function renderElementNode(element) {
    ...
    el.querySelector('.node-icon').className =
      'node-icon ' + (typeIcons[element.element_type] || 'fas fa-cube');
  ...
  async function updatePropertiesPanel() {
    ...
    if (window.propertyRenderer && elementConfig) {
      ...
      window.propertyRenderer.renderProperties(
  ...
  function renderBasicPropertiesPanel(element) {
    ...
    let html =
      '<div class="properties-section"><h4 class="properties-section-title"><i class="fas fa-cog"></i> Content</h4><div class="properties-section-content">';
  ...
  async function handleBindingChange(element, modelField = null, thumbnailPreset = null) {
    ...
    try {
      ...
      if (modelField === '' || (modelField === null && !binding)) {
        ...
        if (binding) {
          await fetch(
            `${apiBase}/custom-elements/${state.customElementId}/bindings/?element_id=${element.id}&content_field=${contentField}`,
            {
              method: 'DELETE',
              headers: { 'X-CSRFToken': config.csrfToken },
            }
  ...
  async function deleteSelectedElement() {
    ...
    try {
      await fetch(
        `${apiBase}/custom-elements/${state.customElementId}/elements/${state.selectedElementId}/delete/`,
        {
          method: 'DELETE',
          headers: { 'X-CSRFToken': config.csrfToken },
        }
