...
def get_fields_by_type(model_key, field_type):
    ...
    return {name: config for name, config in fields.items() if config.get("type") == field_type}
...
def get_fields_by_group(model_key, group):
    ...
    fields = get_model_fields(model_key)
    return {name: config for name, config in fields.items() if config.get("group") == group}
