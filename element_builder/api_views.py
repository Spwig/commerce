...
**Authentication:** Admin session required
        ...
        parameters=[
            OpenApiParameter(
                name="pk", type=int, location=OpenApiParameter.PATH, description=_("Element ID")
            ),
        ],
        responses={
...
class ElementTreeUpdateAPI(APIView):
    ...
    def patch(self, request, pk, element_id):
        get_object_or_404(CustomElement, pk=pk)
...
class ElementBindingClearAPI(APIView):
    ...
    def delete(self, request, pk):
        ...
        deleted_count, _ = ElementBinding.objects.filter(custom_element=custom_element).delete()

        return Response(
            {"deleted": deleted_count, "message": f"Cleared {deleted_count} binding(s)"},
...
class ContainerLayoutAPI(APIView):
    ...
    def post(self, request, pk, element_id):
        ...
        if layout_type not in self.LAYOUT_CONFIGS:
            return Response(
                {"error": f"Invalid layout type: {layout_type}"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ElementMoveAPI(APIView):
    ...
    def _reorder_siblings(self, element):
        ...
        if element.parent_element:
            siblings = (
                Element.objects.filter(parent_element=element.parent_element)
            ...
            siblings = (
                Element.objects.filter(parent_element__isnull=True, page__isnull=True)
                .exclude(pk=element.pk)
                .order_by("order")
            )
    ...
    def _reorder_old_parent_siblings(self, old_parent):
        ...
        siblings = Element.objects.filter(parent_element=old_parent).order_by("order")
...
**Authentication:** Admin session required
    ...
    parameters=[
        OpenApiParameter(
            name="pk", type=int, location=OpenApiParameter.PATH, description=_("Custom element ID")
        ),
        OpenApiParameter(
            name="element_id",
            type=int,
    ...
    def get(self, request, pk, element_id=None):
        ...
        try:
            ...
            if hasattr(template, "origin") and template.origin:
                origin_info = (
                    str(template.origin.name)
                    if hasattr(template.origin, "name")
                    else str(template.origin)
                )
        ...
        return Response({"html": html, "element_id": element.id})
        ...
        if element_bindings:
            ...
            for binding in element_bindings:
                value = resolve_field_value(
                    model_instance, binding.model_field, binding.thumbnail_preset
                )
                if value:
