...
def editor_config(request, product_id):
    ...
    for surface in design_config.surfaces.filter(is_enabled=True).order_by('sort_order'):
        surfaces.append({
            ...
            'name': surface.name,
            'slug': surface.slug,
            'mockup_url': surface.mockup_image.original_file.url if surface.mockup_image else None,
    ...
    for cat in ClipartCategory.objects.filter(is_active=True).order_by('sort_order'):
        asset_count = ClipartAsset.objects.filter(
            category=cat,
            is_active=True,
        ).filter(
            Q(scope='global') | Q(scope='product', product=product)
        ).count()
        if asset_count > 0:
    ...
    for template in design_config.templates.filter(is_active=True).order_by('sort_order'):
        templates.append({
            ...
            'thumbnail_url': template.thumbnail.original_file.url if template.thumbnail else None,
            'design_data': template.design_data,
        })

    # Pricing rules
    pricing = {
        'base_design_fee': str(design_config.base_design_fee.amount) if design_config.base_design_fee else '0',
        'per_surface_fee': str(design_config.per_surface_fee.amount) if design_config.per_surface_fee else '0',
        'per_upload_fee': str(design_config.per_upload_fee.amount) if design_config.per_upload_fee else '0',
        'per_text_fee': str(design_config.per_text_fee.amount) if design_config.per_text_fee else '0',
        'currency': str(design_config.base_design_fee_currency) if design_config.base_design_fee else get_default_currency(),
    }
...
def clipart_list(request):
    ...
    queryset = ClipartAsset.objects.filter(is_active=True).select_related(
    ...
    if product_id:
        queryset = queryset.filter(
            Q(scope='global') | Q(scope='product', product_id=product_id)
        )
    else:
