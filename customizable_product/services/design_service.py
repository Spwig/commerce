...
class DesignPricingService:
    ...
    def calculate(design_config, design_data):
        ...
        for surface_slug, surface_data in surfaces.items():
        ...
        # Calculate fees
        base_fee = Decimal(str(design_config.base_design_fee.amount)) if design_config.base_design_fee else Decimal('0')
        surface_fee_rate = Decimal(str(design_config.per_surface_fee.amount)) if design_config.per_surface_fee else Decimal('0')
        upload_fee_rate = Decimal(str(design_config.per_upload_fee.amount)) if design_config.per_upload_fee else Decimal('0')
        text_fee_rate = Decimal(str(design_config.per_text_fee.amount)) if design_config.per_text_fee else Decimal('0')
