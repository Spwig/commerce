...
class DesignRenderService:
    ...
    def render_surface(surface, surface_data, output_dpi=300):
        ...
        if not source_width:
            source_width = float(surface.width) / 25.4 * 96 if surface.dimension_unit == 'mm' else float(surface.width) * 96 if surface.dimension_unit == 'in' else float(surface.width)
        if not source_height:
            source_height = float(surface.height) / 25.4 * 96 if surface.dimension_unit == 'mm' else float(surface.height) * 96 if surface.dimension_unit == 'in' else float(surface.height)
