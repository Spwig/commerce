...
class ProductDesignConfigAdmin(admin.ModelAdmin):
    list_display = ('product', 'editor_mode', 'is_enabled', 'surface_count', 'template_count', 'updated_at')
...
class CustomFontAdmin(admin.ModelAdmin):
    list_display = ('name', 'family', 'is_system_font', 'has_regular', 'has_bold', 'sort_order', 'is_active')
    ...
    fieldsets = (
        ...
        (_('Font Files'), {
            'description': _('Upload WOFF2 or TTF font files. At minimum, a Regular weight is needed for custom fonts.'),
...
class SavedDesignAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('user', 'product', 'name', 'design_data', 'thumbnails', 'created_at', 'updated_at')
...
class DesignSnapshotAdmin(admin.ModelAdmin):
    ...
    readonly_fields = (
        'order_item', 'design_data', 'rendered_images',
        'fulfillment_files', 'is_rendered', 'render_completed_at', 'created_at',
    )
