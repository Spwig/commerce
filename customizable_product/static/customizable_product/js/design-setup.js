...
(function () {
  ...
  // ─── State ──────────────────────────────────────────────────────────────
  const data = JSON.parse(document.getElementById('design-setup-data').textContent);
  ...
  // ─── Tabs ───────────────────────────────────────────────────────────────
  function initTabs() {
    const tabs = document.querySelectorAll('.design-setup-tab');
    tabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        ...
        const panels = document.querySelectorAll('.design-setup-panel');
  ...
  function positionCardZoneOverlays(container) {
    container.querySelectorAll('.design-setup-card__image img').forEach(function (img) {
      const positionOverlay = function () {
        const overlay = img.parentElement.querySelector('.design-setup-card__zone-overlay');
        if (!overlay) return;
        const bounds = getRenderedImageBounds(img);
        if (!bounds) return;
        const x = parseFloat(overlay.dataset.x) || 0;
        const y = parseFloat(overlay.dataset.y) || 0;
        const w = parseFloat(overlay.dataset.w) || 0;
        const h = parseFloat(overlay.dataset.h) || 0;
  ...
  function renderSurfaces() {
    ...
    surfaces.forEach(function (surface) {
      ...
      if (surface.mockup_url) {
        imageHtml =
          '<div class="design-setup-card__image">' +
          '<img src="' +
          escapeHtml(surface.mockup_url) +
          '" alt="' +
          escapeHtml(surface.name) +
          '">' +
          '<div class="design-setup-card__zone-overlay"' +
          ' data-x="' +
          surface.area_x_percent +
          '"' +
          ' data-y="' +
          surface.area_y_percent +
          '"' +
          ' data-w="' +
          surface.area_width_percent +
          '"' +
          ' data-h="' +
          surface.area_height_percent +
          '">' +
          '</div>' +
          '</div>';
      } else {
        imageHtml =
          '<div class="design-setup-card__image design-setup-card__image--empty">' +
      ...
      card.innerHTML =
        ...
        '<p class="design-setup-card__title">' +
        escapeHtml(surface.name) +
        '</p>' +
        ...
        surface.height +
        ' ' +
        surface.dimension_unit +
        ' · ' +
        surface.recommended_dpi +
        ' DPI' +
        '</p>' +
        '</div>' +
        '<div class="design-setup-card__actions">' +
        '<button type="button" class="button button-small btn-edit-surface" data-id="' +
        surface.id +
        '">' +
        '<i class="fas fa-edit"></i> Edit' +
        '</button>' +
        '<button type="button" class="button button-small button-danger btn-delete-surface" data-id="' +
        surface.id +
        '">' +
    ...
    grid.querySelectorAll('.btn-edit-surface').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        const surface = surfaces.find(function (s) {
          return s.id === id;
        });
  ...
  function deleteSurface(surfaceId) {
    const url = data.urls.deleteSurfaceBase + surfaceId + '/delete/';
    ajaxPost(url, {})
      .then(function () {
        surfaces = surfaces.filter(function (s) {
  ...
  function bindSurfaceModal() {
    ...
    document
      .getElementById('btn-save-surface-modal')
      .addEventListener('click', saveSurfaceFromModal);

    // Close handlers
    const modal = document.getElementById('modal-surface');
    ...
    document.getElementById('surface-name').addEventListener('input', function () {
      const slugField = document.getElementById('surface-slug');
  ...
  function openSurfaceModal(surface) {
    const modal = document.getElementById('modal-surface');
    const title = document.getElementById('modal-surface-title');
    ...
    if (surface) {
      ...
      document.getElementById('surface-allow-text').value =
        surface.allow_text === true ? 'true' : surface.allow_text === false ? 'false' : '';
      document.getElementById('surface-allow-image').value =
        surface.allow_image_upload === true
          ? 'true'
          : surface.allow_image_upload === false
            ? 'false'
            : '';
      document.getElementById('surface-allow-clipart').value =
        surface.allow_clipart === true ? 'true' : surface.allow_clipart === false ? 'false' : '';
      document.getElementById('surface-max-elements').value =
        surface.max_elements != null ? surface.max_elements : '';
  ...
  function saveSurfaceFromModal() {
    const editId = document.getElementById('surface-edit-id').value;
    const name = document.getElementById('surface-name').value.trim();
    const slug = document.getElementById('surface-slug').value.trim();
    ...
    const allowTextVal = document.getElementById('surface-allow-text').value;
    body.allow_text = allowTextVal === '' ? null : allowTextVal === 'true';
    const allowImageVal = document.getElementById('surface-allow-image').value;
    body.allow_image_upload = allowImageVal === '' ? null : allowImageVal === 'true';
    const allowClipartVal = document.getElementById('surface-allow-clipart').value;
    body.allow_clipart = allowClipartVal === '' ? null : allowClipartVal === 'true';
    const maxElVal = document.getElementById('surface-max-elements').value;
    body.max_elements = maxElVal ? parseInt(maxElVal) : null;

    if (editId) body.id = parseInt(editId);

    ajaxPost(data.urls.saveSurface, body)
      .then(function (response) {
        // Refresh surface list
        return fetch(data.urls.listSurfaces).then(function (res) {
          return res.json();
        });
      })
      .then(function (response) {
  ...
  function updateZonePreview(mockupUrl) {
    const container = document.getElementById('zone-preview-mockup');
  ...
  function updateZoneFromInputs() {
    const zone = document.getElementById('zone-preview-zone');
  ...
  function initZoneDragResize() {
    ...
    function syncInputsFromZone() {
      ...
      const x = pxToPercent(zoneRect.left - rect.left, rect.width);
      const y = pxToPercent(zoneRect.top - rect.top, rect.height);
      const w = pxToPercent(zoneRect.width, rect.width);
      const h = pxToPercent(zoneRect.height, rect.height);
    ...
    document.addEventListener('mousemove', function (e) {
      ...
      if (dragging) {
        const newLeft = clamp(startLeft + dx, 0, rect.width - zone.offsetWidth);
        const newTop = clamp(startTop + dy, 0, rect.height - zone.offsetHeight);
      ...
      if (resizing) {
        const minSize = 10; // min px
        let newL, newT, newW, newH;
    ...
    templates.forEach(function (template) {
      ...
      if (template.thumbnail_url) {
        imageHtml =
          '<div class="design-setup-card__image">' +
          '<img src="' +
          escapeHtml(template.thumbnail_url) +
          '" alt="' +
      ...
      card.innerHTML =
        ...
        '</div>' +
        '<div class="design-setup-card__actions">' +
        '<button type="button" class="button button-small btn-edit-template" data-id="' +
        template.id +
        '">' +
        '<i class="fas fa-edit"></i> Edit' +
        '</button>' +
        '<button type="button" class="button button-small button-danger btn-delete-template" data-id="' +
        template.id +
        '">' +
        '<i class="fas fa-trash"></i> Delete' +
        '</button>' +
  ...
  function deleteTemplate(templateId) {
    const url = data.urls.deleteTemplateBase + templateId + '/delete/';
    ajaxPost(url, {})
      .then(function () {
  ...
  function navigateToTemplateEditor(templateId) {
    let baseUrl = '/admin/customizable-product/product/' + data.productId + '/template-editor/';
    if (templateId) {
  ...
  function bindPricing() {
    document.getElementById('btn-save-pricing').addEventListener('click', function () {
      ...
      ajaxPost(data.urls.saveConfig, body)
        .then(function () {
  ...
  function bindSettings() {
    document.getElementById('btn-save-settings').addEventListener('click', function () {
      const uploadTypesRaw = document.getElementById('setting-upload-types').value;
      const uploadTypes = uploadTypesRaw
        ? uploadTypesRaw
            .split(',')
            .map(function (s) {
      ...
      const body = {
        ...
        max_uploads_per_surface:
          parseInt(document.getElementById('setting-max-uploads').value) || 5,
  ...
  function bindEnabledToggle() {
    document.getElementById('editor-enabled').addEventListener('change', function () {
      ajaxPost(data.urls.saveConfig, { is_enabled: this.checked })
        .then(function () {
          showNotification(
            'Editor ' + (document.getElementById('editor-enabled').checked ? 'enabled' : 'disabled')
          );
        })
        .catch(function (err) {
          showNotification(err.message, 'error');
  ...
  function setMockupPreview(imageUrl) {
    const preview = document.getElementById('surface-mockup-preview');
    const img = document.createElement('img');
    img.src = imageUrl;
    img.alt = 'Mockup';
