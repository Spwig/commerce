...
(function () {
  ...
  let dom = null; // Shared DOM refs from main module
  let state = null; // Shared state from main module
  ...
  function clearSnapGuides() {
    if (!dom.canvasZone) return;
    const guides = dom.canvasZone.querySelectorAll('[data-snap-guide]');
  ...
  function onMouseWheel(opt) {
    ...
    const newZoom = zoomLevel + (delta > 0 ? -ZOOM_STEP : ZOOM_STEP);
    setZoom(newZoom);
  ...
  function pasteClipboard() {
    ...
    clipboard
      .clone(['custom_type', 'custom_media_asset_id', 'custom_name'])
