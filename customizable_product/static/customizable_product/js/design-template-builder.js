...
(function () {
    ...
    // ─── State ──────────────────────────────────────────────────────────
    var editorData = JSON.parse(
        document.getElementById('template-editor-data').textContent
    );
    var surfaces = editorData.surfaces || [];
    var fonts = editorData.fonts || [];
    var clipartCategories = editorData.clipartCategories || [];
    var templateData = editorData.template || null;

    var csrfToken = editorData.csrfToken || '';
    if (!csrfToken) {
    ...
    function init() {
        ...
        var nameInput = document.getElementById('tmpl-name');
        var slugInput = document.getElementById('tmpl-slug');
    ...
    function initSurfaceTabs() {
        var container = document.getElementById('tmpl-surfaces');
        container.innerHTML = '';
    ...
    function switchSurface(index) {
        ...
        // Update tab styles
        var tabs = document.querySelectorAll('.tmpl-builder__surface-tab');
        tabs.forEach(function (t, i) {
            t.classList.toggle('active', i === index);
        });

        // Update mockup image
        var mockupImg = document.getElementById('tmpl-mockup-img');
        var mockupContainer = document.getElementById('tmpl-mockup-container');
        ...
        var dimLabel = document.getElementById('surface-dimensions');
        if (dimLabel) {
    ...
    function positionCanvasZone(surface, mockupImg) {
        var zone = document.getElementById('tmpl-canvas-zone');
        var container = document.getElementById('tmpl-mockup-container');
        ...
        var viewport = document.getElementById('tmpl-viewport');
        var maxW = viewport.clientWidth - 40;
        var maxH = viewport.clientHeight - 40;
        var scale = Math.min(1, maxW / mockupImg.naturalWidth, maxH / mockupImg.naturalHeight);
        ...
        var x = parseFloat(surface.area_x_percent) / 100;
        var y = parseFloat(surface.area_y_percent) / 100;
        var w = parseFloat(surface.area_width_percent) / 100;
        var h = parseFloat(surface.area_height_percent) / 100;
    ...
    function positionCanvasZoneNoMockup(surface) {
        ...
        var maxW = viewport.clientWidth - 40;
        var maxH = viewport.clientHeight - 40;
        var size = Math.min(500, maxW, maxH);
    ...
    function saveCurrentSurfaceState() {
        if (!state.canvas) return;
        var slug = surfaces[state.activeSurfaceIndex].slug;
        state.surfaceStates[slug] = {
            canvasJSON: state.canvas.toJSON([
                'custom_type', 'custom_name',
                'custom_lock_position', 'custom_lock_size',
                'custom_lock_rotation', 'custom_lock_content',
                'custom_lock_delete',
            ]),
    ...
    function restoreSurfaceState(slug) {
        if (!state.canvas) return;
        var surfaceState = state.surfaceStates[slug];
    ...
    function undo() {
        ...
        var current = state.canvas.toJSON([
            'custom_type', 'custom_name',
            'custom_lock_position', 'custom_lock_size',
            'custom_lock_rotation', 'custom_lock_content',
            'custom_lock_delete',
    ...
    function loadFonts() {
        fonts.forEach(function (font) {
            ...
            style.textContent =
                ...
                'src: url("' + font.regular_url + '"); font-weight: normal; font-style: normal; }';
            ...
            if (font.bold_url) {
                ...
                bs.textContent =
                    '@font-face { font-family: "' + font.family + '"; ' +
                    'src: url("' + font.bold_url + '"); font-weight: bold; font-style: normal; }';
            ...
            if (font.italic_url) {
                ...
                is.textContent =
                    '@font-face { font-family: "' + font.family + '"; ' +
                    'src: url("' + font.italic_url + '"); font-weight: normal; font-style: italic; }';
    ...
    function addImageToCanvas(url, name) {
        ...
        fabric.Image.fromURL(url, { crossOrigin: 'anonymous' }).then(function (img) {
            // Scale to fit canvas
            var maxW = state.canvas.getWidth() * 0.6;
            var maxH = state.canvas.getHeight() * 0.6;
    ...
    function bindClipartModal() {
        var modal = document.getElementById('modal-clipart');
        modal.querySelector('.tmpl-builder__modal-close').addEventListener('click', closeClipartModal);
        modal.querySelector('.tmpl-builder__modal-backdrop').addEventListener('click', closeClipartModal);

        // Build category pills
        var catContainer = document.getElementById('clipart-categories');
    ...
    function loadClipartAssets(categorySlug) {
        ...
        var url = editorData.urls.clipartApi +
            '?category=' + encodeURIComponent(categorySlug) +
            '&product_id=' + editorData.productId;
        ...
        fetch(url)
            ...
            .then(function (data) {
                ...
                if (assets.length === 0) {
                    grid.innerHTML = '<p style="font-size:0.85rem;color:#999;">No clipart in this category.</p>';
                    return;
                }
                assets.forEach(function (asset) {
                    var item = document.createElement('div');
                    item.className = 'tmpl-builder__clipart-item';
                    item.innerHTML = '<img src="' + escapeAttr(asset.url) + '" alt="' + escapeAttr(asset.name) + '">';
    ...
    function addClipartToCanvas(url, name) {
        pushHistory();
        fabric.Image.fromURL(url, { crossOrigin: 'anonymous' }).then(function (img) {
            var maxW = state.canvas.getWidth() * 0.4;
            var maxH = state.canvas.getHeight() * 0.4;
            var scale = Math.min(1, maxW / img.width, maxH / img.height);
            img.set({
                left: 40,
                top: 40,
    ...
    function bindProperties() {
        ...
        document.getElementById('prop-bring-front').addEventListener('click', function () {
            var obj = state.canvas.getActiveObject();
            if (obj) { state.canvas.bringObjectToFront(obj); pushHistory(); renderLayers(); }
        });
        document.getElementById('prop-bring-forward').addEventListener('click', function () {
            var obj = state.canvas.getActiveObject();
            if (obj) { state.canvas.bringObjectForward(obj); pushHistory(); renderLayers(); }
        });
        document.getElementById('prop-send-backward').addEventListener('click', function () {
            var obj = state.canvas.getActiveObject();
            if (obj) { state.canvas.sendObjectBackwards(obj); pushHistory(); renderLayers(); }
        });
        document.getElementById('prop-send-back').addEventListener('click', function () {
            var obj = state.canvas.getActiveObject();
            if (obj) { state.canvas.sendObjectToBack(obj); pushHistory(); renderLayers(); }
        });
    }
    ...
    function onSelectionChanged() {
        ...
        var isShape = obj.custom_type === 'shape';
    ...
    // ─── Lock Controls ──────────────────────────────────────────────────
    function bindLocks() {
        var lockIds = ['lock-position', 'lock-size', 'lock-rotation', 'lock-content', 'lock-delete'];
        var lockProps = [
            'custom_lock_position', 'custom_lock_size',
            'custom_lock_rotation', 'custom_lock_content',
            'custom_lock_delete',
    ...
    function renderLayers() {
        ...
        var activeObj = state.canvas.getActiveObject();
        ...
        for (var i = objects.length - 1; i >= 0; i--) {
            ...
            var name = obj.custom_name || getObjectLabel(obj);

            var lockIndicator = '';
            if (obj.custom_lock_position || obj.custom_lock_size ||
                obj.custom_lock_rotation || obj.custom_lock_content || obj.custom_lock_delete) {
                lockIndicator = ' <i class="fas fa-lock" style="font-size:0.65rem;color:var(--body-quiet-color,#999);"></i>';
            }

            item.innerHTML =
                '<span class="tmpl-builder__layer-icon"><i class="fas ' + icon + '"></i></span>' +
                '<span class="tmpl-builder__layer-name">' + escapeHtml(name) + lockIndicator + '</span>' +
                '<div class="tmpl-builder__layer-actions">' +
                    '<button type="button" class="tmpl-builder__layer-btn' + (obj.visible === false ? ' tmpl-builder__layer-btn--hidden' : '') + '" data-action="visibility" title="Toggle visibility">' +
    ...
    function deleteSelected() {
        ...
        if (obj.type === 'activeSelection') {
            obj.forEachObject(function (o) { state.canvas.remove(o); });
    ...
    function bindKeyboard() {
        document.addEventListener('keydown', function (e) {
            ...
            if (state.canvas && state.canvas.getActiveObject() && isTextObject(state.canvas.getActiveObject()) && state.canvas.getActiveObject().isEditing) return;
    ...
    function saveTemplate(closeAfter) {
        ...
        var slug = document.getElementById('tmpl-slug').value.trim();
        ...
        // Build design_data
        var designData = { version: 1, surfaces: {} };
        surfaces.forEach(function (surface) {
            ...
            designData.surfaces[surface.slug] = {
                ...
                canvas_width: ss && ss.canvasWidth ? ss.canvasWidth : (state.canvas ? state.canvas.getWidth() : 500),
                canvas_height: ss && ss.canvasHeight ? ss.canvasHeight : (state.canvas ? state.canvas.getHeight() : 500),
            };
        });
