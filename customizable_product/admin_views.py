...
def design_setup(request, product_id):
    ...
    for surface in surfaces:
        surfaces_data.append({
            ...
            'mockup_asset_id': str(surface.mockup_image_id) if surface.mockup_image_id else None,
    ...
    setup_data = {
        ...
        'urls': {
            ...
            'captureThumbnail': reverse('customizable_product:capture_thumbnail', args=[product_id]),
        },
...
def template_editor(request, product_id):
    ...
    if template_id:
        template = get_object_or_404(
            DesignTemplate, pk=template_id, design_config=design_config
        )
    ...
    for cat in ClipartCategory.objects.filter(is_active=True).order_by('sort_order'):
        ...
        count = ClipartAsset.objects.filter(
            category=cat, is_active=True,
        ).filter(Q(scope='global') | Q(scope='product', product=product)).count()
        if count > 0:
