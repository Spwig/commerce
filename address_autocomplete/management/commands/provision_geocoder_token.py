...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--license-key',
            type=str,
            help='Override license key from settings'
        )
        parser.add_argument(
            '--installation-uuid',
            type=str,
            help='Override installation UUID'
        )
        parser.add_argument(
            '--save-to-settings',
            action='store_true',
            help='Save token to settings file'
        )
        parser.add_argument(
            '--output-format',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if not license_key:
            self.stdout.write(
                self.style.ERROR('No license key provided. Use --license-key or set PLATFORM_LICENSE_KEY in settings')
            )
            return

        # Get or generate installation UUID
        installation_uuid = options.get('installation_uuid') or getattr(settings, 'INSTALLATION_UUID', None)
        if not installation_uuid:
            import uuid
            import platform
            hostname = platform.node()
            installation_uuid = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"{hostname}-{timezone.now().isoformat()}"))
        ...
        if not result['success']:
            self.stdout.write(
                self.style.ERROR(f"Failed to provision token: {result.get('error', 'Unknown error')}")
            )
            return
...
"""
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"Could not save to {settings_file}: {e}")
            )