...
(function () {
    ...
    var configEl = document.getElementById('address-autocomplete-config');
    if (!configEl) return;
    var config = JSON.parse(configEl.textContent);
    var shippingPrefix = config.shippingPrefix;
    var billingPrefix = config.billingPrefix;
    var separateBilling = config.separateBilling;
    ...
    function showValidationErrors(errors, prefix) {
        var input = document.querySelector('#' + prefix + 'search');
        if (!input) return;
        var container = input.parentElement;
        var errorDiv = container.querySelector('.address-validation-errors');
        if (!errorDiv) {
    ...
    function validateAddress(components, prefix) {
        var params = new URLSearchParams(components);
    ...
    function copyAddressFields(fromPrefix, toPrefix) {
        var fields = ['address1', 'address2', 'city', 'state', 'postal_code', 'country'];
        fields.forEach(function (field) {
            var fromField = document.getElementById('id_' + fromPrefix + field);
            var toField = document.getElementById('id_' + toPrefix + field);
            if (fromField && toField) {
                toField.value = fromField.value;
            }
        });
        ['latitude', 'longitude', 'normalized'].forEach(function (field) {
            var fromField = document.getElementById(fromPrefix + field);
            var toField = document.getElementById(toPrefix + field);
            if (fromField && toField) {
                toField.value = fromField.value;
            }
        });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        new AddressAutocomplete('#' + shippingPrefix + 'search', {
            ...
            onSelect: function (suggestion) {
                var el = document.getElementById(shippingPrefix + 'normalized');
        ...
        if (separateBilling && billingPrefix) {
            new AddressAutocomplete('#' + billingPrefix + 'search', {
                ...
                onSelect: function (suggestion) {
                    var el = document.getElementById(billingPrefix + 'normalized');
                    if (el) el.value = JSON.stringify(suggestion);
                    validateAddress(suggestion.components, billingPrefix);
                }
            });

            var sameAsShipping = document.getElementById('billing_same_as_shipping');
            if (sameAsShipping) {
