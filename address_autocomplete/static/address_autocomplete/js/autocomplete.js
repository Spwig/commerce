...
class AddressAutocomplete {
  ...
  handleSuggestionClick(suggestionData) {
    ...
    const suggestion =
      typeof suggestionData === 'string' ? JSON.parse(suggestionData) : suggestionData;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-address-autocomplete]').forEach(element => {
    ...
    if (element.dataset.maxSuggestions)
      options.maxSuggestions = parseInt(element.dataset.maxSuggestions);
    if (element.dataset.postalCodeFirst)
      options.postalCodeFirst = element.dataset.postalCodeFirst === 'true';
