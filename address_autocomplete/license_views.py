...
class ProvisionGeocoderTokenView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not license_key:
                return JsonResponse(
                    {"success": False, "error": "License key is required"}, status=400
                )
            ...
            try:
                result = loop.run_until_complete(
                    auth_manager.validate_license_and_get_token(
                        license_key, installation_uuid or self._generate_installation_uuid(request)
                    )
                )
            finally:
                loop.close()
            ...
            if result["success"]:
                ...
                return JsonResponse(
                    {"success": False, "error": result.get("error", "License validation failed")},
                    status=401,
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not current_token:
                return JsonResponse(
                    {"success": False, "error": "Current token is required"}, status=400
                )
...
class GeocoderTokenStatusView(View):
    ...
    def get(self, request):
        ...
        if not token:
            return JsonResponse(
                {"success": False, "error": "Token parameter is required"}, status=400
            )
        ...
        if is_valid:
            ...
            return JsonResponse(
                {"success": True, "valid": False, "error": payload.get("error", "Invalid token")}
            )
