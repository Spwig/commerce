...
(function () {
  ...
  const _configEl = document.getElementById('scene-setup-data');
  if (!_configEl) return;
  let D;
  try {
    D = JSON.parse(_configEl.textContent);
  } catch (e) {
    console.error('scene_setup: failed to parse config', e);
    return;
  }
  if (!D) return;
  ...
  // State
  let nodeTree = D.scene.node_tree || {};
  let mappings = D.mappings || [];
  let geometryAssets = D.geometry || [];
  let textureAssets = D.textures || [];
  ...
  function openModal(modalEl) {
    ...
    const focusable = modalEl.querySelector('input, select, button, textarea');
  ...
  function initViewer() {
    if (D.scene.base_model_url) {
      ...
      viewer.addEventListener('load', function () {
        ...
        const bloomStrength = parseFloat(D.scene.bloom_strength) || 0;
        if (bloomStrength > 0) {
    ...
    const dataBg = viewer.getAttribute('data-bg');
    if (dataBg) {
   ...
   */
  const DEFAULT_EMISSIVE_BOOST = 10.0;

  function applyEmissiveDefaults() {
    if (!viewer.model) return;
    const materials = viewer.model.materials;
    const treeMats = nodeTree.materials || {};
    const bloomStrength = parseFloat(D.scene.bloom_strength) || 0;

    for (let i = 0; i < materials.length; i++) {
      const mat = materials[i];
      const treeData = treeMats[mat.name];
      if (!treeData) continue;

      const ec = treeData.emissive_color;
      if (ec && (ec[0] > 0 || ec[1] > 0 || ec[2] > 0)) {
        ...
        const strength = treeData.emissive_strength || 0;
        if (strength > 0) {
  ...
  function setBloomEnabled(strength) {
    const existing = viewer.querySelector('effect-composer');
    if (strength > 0) {
      if (!existing) {
        const composer = document.createElement('effect-composer');
        ...
        const bloom = document.createElement('bloom-effect');
        ...
        const bloomEl = existing.querySelector('bloom-effect');
  ...
  function renderNodeTree() {
    ...
    if (!nodeTree.nodes || Object.keys(nodeTree.nodes).length === 0) {
      container.innerHTML =
        '<p class="scene-setup-tree__empty">' + escHtml(S.parseGlbEmpty) + '</p>';
      countEl.textContent = '0';
      return;
    }

    const nodeCount = Object.keys(nodeTree.nodes).length;
    countEl.textContent = nodeCount;

    let html = '';
    const roots = nodeTree.root_nodes || [];

    function renderNode(name, depth) {
      const node = nodeTree.nodes[name];
      if (!node) return;
      const icon = node.mesh ? 'fa-cube' : 'fa-folder';
      const mats =
        node.materials && node.materials.length
          ? ' <span class="scene-setup-tree__materials">[' +
            escHtml(node.materials.join(', ')) +
            ']</span>'
          : '';
      html +=
        '<div class="scene-setup-tree__node" data-node="' +
        escHtml(name) +
        '" style="--node-depth:' +
        depth +
  ...
  function renderMappings() {
    const container = document.getElementById('mappings-by-slot');
    const countEl = document.getElementById('mapping-count');
    countEl.textContent = mappings.length;

    if (mappings.length === 0) {
      container.innerHTML =
        '<p class="scene-setup-mappings__empty">' + escHtml(S.noMappings) + '</p>';
      return;
    }
    ...
    Object.keys(bySlot).forEach(function (slotName) {
      html +=
        '<div class="scene-setup-slot-group">' +
        '<div class="scene-setup-slot-group__title">' +
        escHtml(slotName) +
        '</div>';
      bySlot[slotName].forEach(function (m) {
        ...
        const detail = '&rarr; ' + escHtml(m.target_node);
        let colorSwatch = '';
        if (m.action_type === 'material_color' && m.action_data.color) {
          colorSwatch =
            '<span class="scene-setup-color-swatch" style="--swatch-color:' +
            escHtml(m.action_data.color) +
            '"></span>';
        }

        html +=
          '<div class="scene-setup-item" data-mapping-id="' +
          m.id +
          '">' +
          colorSwatch +
          '<div class="scene-setup-item__info">' +
          '<span class="scene-setup-item__label">' +
          escHtml(m.option_name) +
          '</span>' +
          '<span class="scene-setup-item__detail">' +
          '<span class="scene-setup-badge scene-setup-badge--' +
          badgeClass +
          '">' +
          escHtml(m.action_type.replace(/_/g, ' ')) +
          '</span>' +
          detail +
          '</span>' +
          '</div>' +
          '<div class="scene-setup-item__actions">' +
          '<button class="scene-setup-item__btn btn-edit-mapping" data-id="' +
          m.id +
          '" title="Edit"><i class="fas fa-edit"></i></button>' +
          '<button class="scene-setup-item__btn scene-setup-item__btn--danger btn-delete-mapping" data-id="' +
          m.id +
  ...
  function renderGeometryAssets() {
    ...
    const countEl = document.getElementById('geometry-count');
    ...
    geometryAssets.forEach(function (ga) {
      html +=
        '<div class="scene-setup-item" data-geometry-id="' +
        ga.id +
        '">' +
        '<div class="scene-setup-item__info">' +
        '<span class="scene-setup-item__label">' +
        ...
        '</span>' +
        '<span class="scene-setup-item__detail">' +
        escHtml(ga.target_node || S.noTarget) +
        ' &mdash; ' +
        escHtml(ga.media_asset_title) +
        '</span>' +
        '</div>' +
        '<div class="scene-setup-item__actions">' +
        '<button class="scene-setup-item__btn scene-setup-item__btn--danger btn-delete-geometry" data-id="' +
        ga.id +
        '" title="Delete"><i class="fas fa-trash"></i></button>' +
        '</div>' +
        '</div>';
    ...
    container.querySelectorAll('.btn-delete-geometry').forEach(function (btn) {
      btn.addEventListener('click', function () {
        confirmDelete(btn, function () {
          deleteGeometryAsset(parseInt(btn.dataset.id));
        });
      });
    });
  }
  ...
  function renderTextureAssets() {
    ...
    const countEl = document.getElementById('texture-count');
    ...
    textureAssets.forEach(function (ta) {
      html +=
        '<div class="scene-setup-item" data-texture-id="' +
        ta.id +
        '">' +
        '<div class="scene-setup-item__info">' +
        '<span class="scene-setup-item__label">' +
        ...
        '</span>' +
        '<span class="scene-setup-item__detail">' +
        '<span class="scene-setup-badge scene-setup-badge--texture">' +
        escHtml(ta.texture_type_display || ta.texture_type) +
        '</span>' +
        escHtml(ta.media_asset_title) +
        '</span>' +
        '</div>' +
        '<div class="scene-setup-item__actions">' +
        '<button class="scene-setup-item__btn scene-setup-item__btn--danger btn-delete-texture" data-id="' +
        ta.id +
        '" title="Delete"><i class="fas fa-trash"></i></button>' +
        '</div>' +
        '</div>';
    ...
    container.querySelectorAll('.btn-delete-texture').forEach(function (btn) {
      btn.addEventListener('click', function () {
        confirmDelete(btn, function () {
          deleteTextureAsset(parseInt(btn.dataset.id));
        });
      });
    });
  }
  ...
  function openMappingModal(mappingId) {
    ...
    if (editingMappingId) {
      ...
      const m = mappings.find(function (x) {
        return x.id === editingMappingId;
      });
      if (m) {
  ...
  function populateActionFields(actionType, data) {
    if (actionType === 'material_color') {
      ...
      document.getElementById('base-texture-name').textContent = data.base_color_url
        ? S.selectedLabel
        : S.noneLabel;
      document.getElementById('action-normal-texture-url').value = data.normal_url || '';
      document.getElementById('normal-texture-name').textContent = data.normal_url
        ? S.selectedLabel
        : S.noneLabel;
  ...
  function collectActionData(actionType) {
    if (actionType === 'material_color') {
      ...
      const emissiveColor = document.getElementById('action-emissive-color').value;
      if (emissiveColor !== '#000000') {
        result.emissive = emissiveColor;
        result.emissive_strength = parseFloat(
          document.getElementById('action-emissive-strength').value
        );
      }
      return result;
    } else if (actionType === 'material_texture') {
      ...
      const sel = document.getElementById('action-geometry-asset');
      return {
        glb_url: sel.value,
  ...
  async function saveMapping() {
    const slotOptionId = parseInt(document.getElementById('mapping-slot-option').value);
    const actionType = document.getElementById('mapping-action-type').value;
    const targetNode = document.getElementById('mapping-target-node').value;
    ...
    const result = await ajax(D.urls.saveMappings, 'POST', body);
    if (result.success) {
      closeMappingModal();
      const data = await ajax(D.urls.listMappings, 'GET');
  ...
  async function deleteMapping(id) {
    const url = buildDeleteUrl(D.urls.deleteMapping, id);
    const result = await ajax(url, 'POST');
    if (result.success) {
  ...
  async function saveGeometryFromModal() {
    const label = document.getElementById('geometry-label').value.trim();
    const targetNode = document.getElementById('geometry-target-node').value.trim();
    const mediaAssetId = document.getElementById('geometry-media-asset-id').value;
    ...
    const result = await ajax(D.urls.saveGeometry, 'POST', {
      label: label,
  ...
  async function deleteGeometryAsset(id) {
    const url = buildDeleteUrl(D.urls.deleteGeometry, id);
    const result = await ajax(url, 'POST');
    if (result.success) {
      geometryAssets = geometryAssets.filter(function (ga) {
        return ga.id !== id;
      });
      renderGeometryAssets();
  ...
  async function saveTextureFromModal() {
    const label = document.getElementById('texture-label').value.trim();
    const textureType = document.getElementById('texture-type').value;
    const mediaAssetId = document.getElementById('texture-media-asset-id').value;
    ...
    const result = await ajax(D.urls.saveTexture, 'POST', {
      label: label,
      media_asset_id: mediaAssetId,
  ...
  async function deleteTextureAsset(id) {
    const url = buildDeleteUrl(D.urls.deleteTexture, id);
    const result = await ajax(url, 'POST');
    if (result.success) {
      textureAssets = textureAssets.filter(function (ta) {
        return ta.id !== id;
      });
      renderTextureAssets();
  ...
  function openMediaPicker(callback, options) {
    const opts = options || {};
    let pickerFn = window.selectMediaFromLibrary;
  ...
  function initBackgroundPicker() {
    ...
    const bgGradientInput = document.getElementById('setting-bg-gradient');
    const preview = document.getElementById('bg-gradient-preview');
    const previewLabel = preview.querySelector('.scene-setup-bg-gradient-preview__label');
    const hexLabel = document.getElementById('bg-hex-value');
    const tabs = document.querySelectorAll('.scene-setup-bg-tab');
    const panels = document.querySelectorAll('.scene-setup-bg-panel');

    // Detect initial value
    const initialBg = D.scene.background_color || '#ffffff';
    ...
    tabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        ...
        tabs.forEach(function (t) {
          t.classList.remove('scene-setup-bg-tab--active');
        });
        tab.classList.add('scene-setup-bg-tab--active');
        panels.forEach(function (p) {
          p.classList.remove('scene-setup-bg-panel--active');
        });
        document
          .querySelector('.scene-setup-bg-panel--' + bgMode)
          .classList.add('scene-setup-bg-panel--active');
        // Live update viewer
    ...
    const gradBtn = document.getElementById('btn-open-gradient');
    gradBtn.addEventListener('click', function () {
      ...
      const GC = window.GradientCreatorUtility || window.GradientCreator;
  ...
  function bindSettingsControls() {
    const exposure = document.getElementById('setting-exposure');
    const shadow = document.getElementById('setting-shadow');
    const bloom = document.getElementById('setting-bloom');
    const autoRotate = document.getElementById('setting-auto-rotate');
    ...
    const shadowSoftness = document.getElementById('setting-shadow-softness');
    ...
    const toneMapping = document.getElementById('setting-tone-mapping');
  ...
  async function saveSettings() {
    ...
    // Environment image
    const envId = document.getElementById('btn-select-environment').dataset.envId || null;
    if (envId !== undefined) {
      body.environment_image_id = envId;
    }

    const result = await ajax(D.urls.saveScene, 'POST', body);
    if (result.success) {
  ...
  function selectBaseModel() {
    openMediaPicker(async function (asset) {
      ...
      const result = await ajax(D.urls.parseGlb, 'POST', { media_asset_id: asset.id });
      if (result.success) {
  ...
  async function captureThumbnail() {
    ...
    try {
      ...
      reader.onloadend = async function () {
        const result = await ajax(D.urls.captureThumbnail, 'POST', {
      ...
      if (mapping.action_type === 'material_color') {
        ...
        for (var i = 0; i < materials.length; i++) {
          if (materials[i].name === matName) {
            const hex = mapping.action_data.color;
            const r = parseInt(hex.slice(1, 3), 16) / 255;
            const g = parseInt(hex.slice(3, 5), 16) / 255;
            const b = parseInt(hex.slice(5, 7), 16) / 255;
            const alpha = mapping.action_data.alpha !== undefined ? mapping.action_data.alpha : 1;
            ...
            if (mapping.action_data.emissive) {
              const eHex = mapping.action_data.emissive;
              const er = parseInt(eHex.slice(1, 3), 16) / 255;
              const eg = parseInt(eHex.slice(3, 5), 16) / 255;
              const eb = parseInt(eHex.slice(5, 7), 16) / 255;
              ...
              if (mapping.action_data.emissive_strength !== undefined) {
                ...
                const eLum = 0.2126 * er + 0.7152 * eg + 0.0722 * eb;
                const adjStr =
                  eLum > 0.01
                    ? mapping.action_data.emissive_strength / eLum
                    : mapping.action_data.emissive_strength;
        ...
        for (var i = 0; i < materials.length; i++) {
          if (materials[i].name === matName) {
            if (mapping.action_data.base_color_url) {
              viewer
                .createTexture(mapping.action_data.base_color_url)
  ...
  function bindEnvironmentPicker() {
    const selectBtn = document.getElementById('btn-select-environment');
    const clearBtn = document.getElementById('btn-clear-environment');
    const nameEl = document.getElementById('environment-name');
  ...
  function init() {
    ...
    document.getElementById('btn-capture-camera').addEventListener('click', function () {
      let orbitStr = viewer.getAttribute('camera-orbit');
      let targetStr = viewer.getAttribute('camera-target');
      ...
      if (viewer.getCameraOrbit) {
        ...
        orbitStr =
          (orbit.theta * RAD2DEG).toFixed(1) +
          'deg ' +
          (orbit.phi * RAD2DEG).toFixed(1) +
          'deg ' +
          orbit.radius.toFixed(3) +
          'm';
      }
    ...
    document.getElementById('btn-reset-camera-default').addEventListener('click', function () {
      const orbit = D.scene.camera_orbit;
      const target = D.scene.camera_target;
    ...
    document
      .getElementById('mapping-modal')
      .querySelector('.scene-setup-modal__backdrop')
      .addEventListener('click', closeMappingModal);
    ...
    document
      .getElementById('geometry-modal')
      .querySelector('.scene-setup-modal__backdrop')
      .addEventListener('click', closeGeometryModal);
    ...
    document
      .getElementById('texture-modal')
      .querySelector('.scene-setup-modal__backdrop')
      .addEventListener('click', closeTextureModal);
    ...
    document.getElementById('btn-select-base-texture').addEventListener('click', function () {
      openMediaPicker(function (asset) {
        document.getElementById('action-base-texture-url').value =
          asset.url || asset.original_file_url || '';
    ...
    document.getElementById('btn-select-normal-texture').addEventListener('click', function () {
      openMediaPicker(function (asset) {
        document.getElementById('action-normal-texture-url').value =
          asset.url || asset.original_file_url || '';
