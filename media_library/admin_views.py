...
def filter_media_assets(request):
    ...
    # Build queryset
    queryset = (
        MediaAsset.objects.select_related("folder", "uploaded_by").prefetch_related("tags").all()
    )
...
def delete_thumbnail_size_api(request, size_id):
    ...
    try:
        ...
        if preset.is_system_preset:
            return JsonResponse(
                {"success": False, "error": "System presets cannot be deleted"}, status=403
            )
...
def regenerate_thumbnails_view(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Thumbnail regeneration started", "task_id": task.id}
        )
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
