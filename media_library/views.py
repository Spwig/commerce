...
class MediaAssetViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = MediaAsset.objects.select_related("folder", "uploaded_by").prefetch_related(
            "tags", "thumbnails"
        )
    ...
    def set_poster(self, request, pk=None):
        ...
        if not poster_file:
            return Response(
                {"error": _("No poster file provided")}, status=status.HTTP_400_BAD_REQUEST
            )

        asset.poster_image.save(
            f"{asset.id}_poster.{poster_file.name.split('.')[-1]}", poster_file, save=True
        )

        return Response({"success": True, "poster_url": asset.poster_image.url})
    ...
    def stream(self, request, pk=None):
        ...
        if not asset.is_video():
            return Response(
                {"error": _("This asset is not a video")}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not video_file:
            return Response({"error": _("Video file not found")}, status=status.HTTP_404_NOT_FOUND)
        ...
        content_type, _encoding = mimetypes.guess_type(video_file.name)
        if not content_type:
        ...
        try:
            ...
            if range_header:
                ...
                if range_match:
                    ...
                    response = FileResponse(
                        video_file.open("rb"), content_type=content_type, as_attachment=False
                    )
                    response["Accept-Ranges"] = "bytes"
            ...
            traceback.print_exc()
            return Response(
                {"error": _("Failed to stream video")}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def track_usage(self, request, pk=None):
        ...
        usage, created = MediaUsage.objects.get_or_create(
            media_asset=asset, content_type=content_type, object_id=object_id, field_name=field_name
        )
    ...
    def bulk_operations(self, request):
        ...
        try:
            if action == "delete":
                ...
                return Response(
                    {"message": _(f"Added tags to {assets.count()} assets successfully")}
                )
                ...
                return Response(
                    {"message": _(f"Removed tags from {assets.count()} assets successfully")}
                )
                ...
                return Response(
                    {
                        "message": _(
                            f"Toggled public status for {assets.count()} assets successfully"
                        )
                    }
                )
    ...
    def deleted(self, request):
        ...
        deleted_assets = (
            MediaAsset.all_objects.filter(is_deleted=True)
            .select_related("folder", "uploaded_by")
            .prefetch_related("tags", "thumbnails")
            .order_by("-deleted_at")
        )
        ...
        serializer = MediaAssetListSerializer(
            deleted_assets, many=True, context={"request": request}
        )
    ...
    def restore(self, request):
        ...
        if not assets.exists():
            return Response(
                {"error": _("No deleted assets found")}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def empty_recycle_bin(self, request):
        ...
        if not request.user.is_staff:
            return Response(
                {"error": _("Admin permission required")}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def stats(self, request):
        ...
        # Group by mime type
        by_type = (
            MediaAsset.objects.values("mime_type").annotate(count=Count("id")).order_by("-count")
        )
...
def upload_progress(request):
    ...
    return JsonResponse({"status": "processing", "progress": 100, "message": _("Upload complete")})
...
class MediaProcessingJobViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            return self.get_paginated_response({"stats": stats, "jobs": data})

        data = self.serialize_jobs(queryset)
        return Response({"stats": stats, "jobs": data})
    ...
    def active(self, request):
        ...
        queryset = (
            MediaProcessingJob.objects.filter(user=request.user)
            .filter(
                Q(
                    status__in=[
                        "pending",
                ...
                | Q(status="completed", completed_at__gte=thirty_seconds_ago)
                | Q(status="failed", completed_at__gte=thirty_seconds_ago)
            )
            .order_by("-created_at")
        )
    ...
    def serialize_job_detail(self, job):
        ...
        if job.media_asset:
            data["media_asset"] = {
                ...
                "thumbnail": job.media_asset.get_thumbnail("small")
                if job.media_asset.is_image()
                else None,
            }
...
class MediaFieldAutoSaveAPIView(APIView):
    ...
    @extend_schema(
        ...
        description=_(
            "Auto-save a MediaAsset ForeignKey field on any whitelisted model. "
        ...
        request=inline_serializer(
            ...
            fields={
                ...
                "model_name": drf_serializers.CharField(
                    help_text='Model name (e.g., "sitesettings")'
                ),
                ...
                "asset_id": drf_serializers.UUIDField(
                    required=False, allow_null=True, help_text="MediaAsset UUID or null to clear"
                ),
            },
        ),
        responses={
            ...
            400: OpenApiResponse(
                description=_("Missing required parameters or field is not FK to MediaAsset")
            ),
    ...
    def post(self, request):
        ...
        if field_name not in allowed_fields:
            ...
            return Response(
                {"error": _("Field not allowed for auto-save")}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        try:
            ...
            return Response({"error": _("Model not found")}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            return Response(
                {"error": _("Field not found on model")}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            ...
            return Response({"error": _("Instance not found")}, status=status.HTTP_404_NOT_FOUND)
        ...
        if asset_id:
            try:
                ...
                return Response(
                    {"error": _("Media asset not found")}, status=status.HTTP_404_NOT_FOUND
                )
...
def ckeditor_media_library_upload(request):
    ...
    try:
        ...
        return JsonResponse({"error": {"message": "Invalid image file."}}, status=400)
    ...
    try:
        ...
        serializer = MediaAssetCreateSerializer(
            data={"original_file": uploaded_file, "title": title}, context={"request": request}
        )
        ...
        if serializer.is_valid():
            ...
            return JsonResponse({"error": {"message": "Upload validation failed."}}, status=400)
    except Exception as e:
        logger.error(f"CKEditor media library upload failed: {e}")
        return JsonResponse({"error": {"message": "Upload failed. Please try again."}}, status=500)
