...
class MediaFolderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MediaFolder
        fields = ['id', 'name', 'slug', 'path', 'description', 'parent', 'asset_count', 'created_at']
...
class MediaAssetListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MediaAsset
        fields = [
            'id', 'title', 'alt_text', 'description', 'mime_type', 'width', 'height',
            'file_size', 'file_size_display', 'folder_name', 'tag_names',
            'thumbnail_url', 'display_url', 'poster_url', 'is_public', 'created_at', 'usage_count'
        ]
...
class MediaAssetDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MediaAsset
        fields = [
            'id', 'title', 'alt_text', 'description', 'mime_type', 
            'width', 'height', 'file_size', 'file_size_display', 'dimensions',
            'folder', 'tags', 'thumbnails', 'metadata', 
            'focal_point_x', 'focal_point_y', 'is_public',
            'uploaded_by_name', 'original_url', 'webp_url', 'display_url',
            'usage_count', 'last_used_at', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'file_size', 'width', 'height', 'mime_type', 'metadata',
            'uploaded_by_name', 'usage_count', 'last_used_at', 'created_at', 'updated_at'
        ]
...
class MediaAssetCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MediaAsset
        fields = [
            'id', 'title', 'alt_text', 'description', 'original_file', 'mime_type',
            'folder_id', 'tag_ids', 'focal_point_x', 'focal_point_y', 'is_public',
            'job_id', 'job_status', 'thumbnail_url', 'display_url', 'poster_image'
        ]
    ...
    def validate_original_file(self, value):
        ...
        try:
            ...
            try:
                ...
                raise serializers.ValidationError(
                    "Unable to verify file safety. Upload rejected."
                )
                        ...
                        try:
                            ...
                            if format == 'webm_av1':
                                converted_path = video_processor.convert_to_webm_av1(
                                    tmp_path, crf=crf, preset=preset,
...
class MediaAssetUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MediaAsset
        fields = [
            'title', 'alt_text', 'description', 'folder_id', 'tag_ids',
            'focal_point_x', 'focal_point_y', 'is_public'
        ]
...
class MediaProcessingJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'job_type', 'status', 'progress', 'status_message',
            'error_message', 'filename', 'file_size', 'user',
