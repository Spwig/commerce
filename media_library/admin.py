...
from .models import MediaAsset, MediaFolder, Tag, ImageSizePreset, MediaThumbnail, MediaUsage, MediaProcessingJob
...
class MediaAssetAdmin(admin.ModelAdmin):
    ...
    list_display = ['thumbnail_preview', 'title', 'folder', 'mime_type', 'file_size_display',
                    'dimensions', 'usage_count', 'uploaded_by', 'created_at']
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if obj and obj.is_video():
            ...
            return (
                ...
                (_('Image Files'), {
                    'fields': ('original_file', 'webp_file')
                }),
                (_('Organization'), {
                    'fields': ('folder', 'tags')
                }),
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        readonly = ['id', 'file_size', 'width', 'height', 'mime_type', 'metadata',
                   'usage_count', 'last_used_at', 'created_at', 'updated_at',
                   'original_file', 'webp_file', 'uploaded_by']

        if obj and obj.is_video():
            readonly.extend(['converted_video', 'poster_image', 'duration', 'frame_rate',
                           'bitrate', 'video_codec', 'audio_codec'])
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('gallery/', self.admin_site.admin_view(self.gallery_view), name='media_library_gallery'),
            path('upload/', self.admin_site.admin_view(self.upload_view), name='media_library_upload'),
            path('api/search/', self.admin_site.admin_view(self.api_search), name='media_library_api_search'),
        ]
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['thumbnail_regen_note'] = format_html(
            ...
            _('Configure thumbnail sizes and regenerate thumbnails for all product images in Site Settings.'),
            settings_url,
    ...
    def api_search(self, request):
        ...
        data = [{
            ...
            'poster_url': asset.poster_image.url if asset.is_video() and asset.poster_image else None,
...
class ImageSizePresetAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Size & Quality'), {
            'fields': ('width', 'height', 'crop_mode', 'quality')
        }),
        (_('Status'), {
            'fields': ('is_active', 'is_system_preset')
        }),
    )
    ...
    def delete_queryset(self, request, queryset):
        ...
        if system_presets.exists():
            self.message_user(
                request,
                _("System presets cannot be deleted. "
                  "Only custom presets were deleted."),
                level='warning'
            )
    ...
    def changelist_view(self, request, extra_context=None):
        """Add helpful info about system presets"""
        extra_context = extra_context or {}
        ...
        extra_context['system_presets_note'] = format_html(
            ...
            _('9 default image size presets (Thumbnail, Small, Medium, Large, Gallery, Hero, Banner, Card, Avatar) '
...
class MediaProcessingJobAdmin(admin.ModelAdmin):
    ...
    list_display = ['filename', 'job_type', 'status', 'progress_bar', 'user', 'created_at', 'duration_display']
    list_filter = ['status', 'job_type', 'created_at']
    search_fields = ['filename', 'user__username', 'error_message']
    readonly_fields = ['id', 'job_type', 'status', 'progress', 'status_message',
                      'filename', 'file_size', 'mime_type', 'media_asset',
    ...
    fieldsets = (
        ...
        (_('File Details'), {
            'fields': ('filename', 'file_size', 'mime_type', 'media_asset')
        }),
        (_('Timing'), {
    ...
    def progress_bar(self, obj):
        ...
        status_class = 'completed' if obj.is_complete else 'active' if obj.is_active else 'failed' if obj.is_failed else 'pending'
        return format_html(
    ...
    def cancel_jobs(self, request, queryset):
        ...
        active_jobs = queryset.filter(status__in=['pending', 'uploading', 'processing', 'converting', 'generating_thumbnails'])
