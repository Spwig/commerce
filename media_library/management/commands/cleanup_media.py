...
class Command(BaseCommand):
    ...
    def clean_unused_assets(self):
        ...
        unused_assets = MediaAsset.objects.annotate(usage_count_actual=Count("usages")).filter(
            usage_count_actual=0
        )
        ...
        with tqdm(total=total, desc="Cleaning unused assets") as pbar:
            for asset in unused_assets.iterator(chunk_size=self.batch_size):
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"Error deleting {asset.title}: {e}"))
