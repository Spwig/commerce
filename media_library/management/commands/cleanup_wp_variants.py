...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_variants == 0:
            self.stdout.write(self.style.SUCCESS('No WordPress size variants found. Nothing to clean up.'))
            return
        ...
        try:
            ...
            product_refs = ProductImage.objects.filter(
                media_asset__in=variant_assets
        ...
        try:
            ...
            blog_refs = BlogPost.objects.filter(
                featured_image__in=variant_assets
            ).values_list('featured_image_id', flat=True)
        ...
        usage_refs = MediaUsage.objects.filter(
            media_asset__in=variant_assets
        ).values_list('media_asset_id', flat=True)
