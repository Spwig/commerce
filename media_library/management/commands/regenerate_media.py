        ...
        with tqdm(total=total, desc="Processing assets") as pbar:
            for asset in queryset.iterator(chunk_size=self.batch_size):
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"Error processing {asset.title}: {e}"))

                pbar.update(1)

        self.stdout.write(
            self.style.SUCCESS(f"Completed! Processed: {processed}, Errors: {errors}")
        )
            ...
            try:
                ...
                if original_content:
                    ...
                    MediaThumbnail.objects.filter(
                        media_asset=asset, size_preset=preset.slug
                    ).delete()
