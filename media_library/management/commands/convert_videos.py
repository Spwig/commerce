...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--asset-id", type=str, help="Convert specific asset by ID")
        parser.add_argument(
            "--all", action="store_true", help="Convert all video assets without converted versions"
        )
        parser.add_argument(
            "--force",
            action="store_true",
        ...
        for asset in assets:
            ...
            try:
                ...
                if options["format"] == "webm_av1":
                    ...
                    converted_path = processor.convert_to_webm_vp9(tmp_path, crf=options["crf"])
                    output_ext = ".webm"
                else:  # mp4_h265
                    converted_path = processor.convert_to_mp4_h265(tmp_path, crf=options["crf"])
                    output_ext = ".mp4"
