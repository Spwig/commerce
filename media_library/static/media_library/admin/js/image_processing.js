...
(function () {
    ...
    fetch(buildUrl(urls.thumbnailSizes))
      ...
      .then(data => {
        ...
        if (data.has_db_sizes) {
          noteEl.innerHTML =
            '<i class="fas fa-database"></i> Sizes are managed in the database. <a href="#" class="toggle-manage-link">Edit sizes below</a>';
          ...
          noteEl.innerHTML =
            '<i class="fas fa-file-code"></i> These sizes are configured in <code>settings.py</code>';
        }

        if (entries.length === 0) {
          container.innerHTML =
            '<div class="images-empty-state">No thumbnail sizes configured</div>';
          return;
        }
      ...
      .catch(error => {
        ...
        container.innerHTML =
          '<div class="images-empty-state images-error">Error loading thumbnail configuration</div>';
      });
  }
                    ...
                    <tbody>
                        ${thumbnailSizesData
                          .map(size => {
                            ...
                            const readonlyAttr = isSystemPreset
                              ? 'readonly class="system-preset-field"'
                              : '';
  ...
  function saveThumbnailSizes(btn) {
    ...
    rows.forEach(row => {
      ...
      if (!data.name || !data.display_name || !data.width || !data.height) {
        AdminModal.alert({
          message: 'Please fill in all required fields (Name, Display Name, Width, Height)',
          type: 'warning',
        });
        return;
      }
    ...
    fetch(buildUrl(urls.saveThumbnailSizes), {
      ...
      .catch(error => {
        ...
        AdminModal.alert({
          message: 'Error saving thumbnail sizes: ' + error.message,
          type: 'error',
        });
      })
      .finally(() => {
  ...
  function deleteThumbnailSize(sizeId) {
    if (
      !confirm('Are you sure you want to delete this thumbnail size? This action cannot be undone.')
    ) {
      return;
    }
    ...
    fetch(buildUrl(deleteUrl), {
      ...
      .catch(error => {
        ...
        AdminModal.alert({
          message: 'Error deleting thumbnail size: ' + error.message,
          type: 'error',
        });
      });
  }
  ...
  function loadImageStats() {
    ...
    fetch(buildUrl(urls.imageStats))
      ...
      .catch(error => {
        ...
        container.innerHTML =
          '<div class="images-empty-state images-error">Error loading statistics</div>';
      });
  }
  ...
  function showRegenerationBanner(statusData) {
    ...
    if (banner) {
      ...
      if (message) {
        message.textContent =
          statusData.message ||
          `Processing ${statusData.processed || 0} of ${statusData.total || 0} images...`;
      }
      ...
      fetch(buildUrl(urls.regenerateThumbnailsStatus))
        ...
        .then(data => {
          if (data.complete) {
            ...
            if (message)
              message.textContent =
                data.message || `Successfully regenerated ${data.processed} images!`;
  ...
  function regenerateThumbnails() {
    ...
    if (
      !confirm(
        'This will regenerate all product image thumbnails. This process may take several minutes depending on the number of images. Continue?'
      )
    ) {
      return;
    }
    ...
    fetch(buildUrl(urls.regenerateThumbnails), {
            ...
            fetch(buildUrl(urls.regenerateThumbnailsStatus))
              ...
              .then(statusData => {
                ...
                if (statusData.complete) {
                  ...
                  setTimeout(() => {
                    btn.innerHTML =
                      '<i class="fas fa-sync-alt"></i> Regenerate All Product Thumbnails';
                    status.textContent = '';
      ...
      .catch(error => {
        ...
        hideRegenerationBanner();
        AdminModal.alert({
          message: 'Error regenerating thumbnails: ' + error.message,
          type: 'error',
        });
      });
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    const deleteBtn = e.target.closest('.delete-size-btn[data-size-id]');
    if (deleteBtn) {
