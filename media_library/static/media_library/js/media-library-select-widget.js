...
(function () {
    ...
    function getTranslations() {
        var i18nEl = document.querySelector('.media-widget-i18n');
    ...
    function initWidget(widget) {
        ...
        var i18n = getTranslations();
        var tSaved = i18n.saved || 'Saved';
        var tNoImage = i18n.noImage || 'No Image Selected, Drop Here';
        var tUploading = i18n.uploading || 'Uploading...';
        var tUploadFailed = i18n.uploadFailed || 'Upload failed';
        var tClear = i18n.clear || 'Clear';
        var tDropImage = i18n.dropImage || 'Please drop an image file';

        var selectBtn = widget.querySelector('.media-select-btn');
        var clearBtn = widget.querySelector('.media-clear-btn');
        var hiddenInput = widget.querySelector('.media-field-input');
        var preview = widget.querySelector('.media-preview');

        var autoSaveUrl = widget.dataset.autoSaveUrl;
        var autoSaveApp = widget.dataset.autoSaveApp;
        var autoSaveModel = widget.dataset.autoSaveModel;
        var autoSavePk = widget.dataset.autoSavePk;
        var autoSaveField = widget.dataset.autoSaveField;
        ...
        function autoSaveMediaField(assetId) {
            ...
            if (!csrfToken) { console.error('Auto-save: CSRF token not found'); return; }
            ...
            .then(function (data) {
                if (data.success) { showAutoSaveNotification(); }
        ...
        function showAutoSaveNotification() {
            var notification = widget.querySelector('.auto-save-notification');
            if (!notification) {
                notification = document.createElement('span');
                notification.className = 'auto-save-notification';
                notification.style.cssText = 'display:inline-block;margin-left:10px;color:var(--success-fg,#28a745);font-size:12px;opacity:0;transition:opacity 0.3s;';
        ...
        function makeNoImagePreview() {
            return '<div class="no-image-preview drop-zone" style="width:150px;height:150px;border:2px dashed var(--border-color);display:flex;align-items:center;justify-content:center;color:var(--body-quiet-color);border-radius:4px;text-align:center;cursor:pointer;transition:all 0.2s ease;"><span>' + tNoImage + '</span></div>';
        }

        function updatePreview(imageUrl) {
            if (imageUrl) {
                preview.innerHTML = '<img src="' + imageUrl + '" alt="Selected image" class="preview-image" style="max-width:150px;max-height:150px;object-fit:cover;border:1px solid var(--border-color);border-radius:4px;">';
                if (!widget.querySelector('.media-clear-btn')) {
                    ...
                    newClearBtn.style.cssText = 'background:var(--delete-button-bg);color:#fff;border:none;padding:8px 15px;border-radius:4px;cursor:pointer;margin-left:10px;';
                ...
                var cb = widget.querySelector('.media-clear-btn');
                if (cb) cb.remove();
        ...
        function uploadFile(file) {
            ...
            .then(function (data) {
                ...
                var imgUrl = data.thumbnail_url || data.display_url;
        ...
        function attachDragDropHandlers() {
            var dropZone = preview.querySelector('.drop-zone');
            if (!dropZone) return;
            ...
            dropZone.addEventListener('drop', function (e) {
                ...
                if (files && files.length > 0) {
                    var file = files[0];
                    if (!file.type.startsWith('image/')) { AdminModal.alert({message: tDropImage, type: 'warning'}); return; }
                    uploadFile(file);
                }
            }, false);
        }
