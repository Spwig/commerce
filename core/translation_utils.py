...
def get_primary_language():
    ...
    if primary is None:
        try:
            ...
            primary = SiteSettings.objects.values_list(
                'default_language', flat=True
            ).get(pk=1)
        except Exception:
...
def translate_menu_items(items, lang):
    ...
    for item in items:
        ...
        elif hasattr(item, '_prefetched_objects_cache') and 'children' in item._prefetched_objects_cache:
