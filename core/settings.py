...
SECRET_KEY = env('SPWIG_SECRET_KEY', default=env('DJANGO_SECRET_KEY', default='spwig-insecure-changeme-in-production'))

# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = env('DEBUG', default=False)

# ALLOWED_HOSTS: Check SPWIG_ALLOWED_HOSTS, ALLOWED_HOSTS, and DJANGO_ALLOWED_HOSTS for flexibility
ALLOWED_HOSTS = env.list('SPWIG_ALLOWED_HOSTS', default=None) or env.list('ALLOWED_HOSTS', default=None) or env.list('DJANGO_ALLOWED_HOSTS', default=['localhost', '127.0.0.1'])
...
SPWIG_COMPONENTS_DIR = env('SPWIG_COMPONENTS_DIR', default=str(BASE_DIR.parent / 'spwig-components'))
...
MIDDLEWARE = [
    ...
    *(['whitenoise.middleware.WhiteNoiseMiddleware'] if not DEBUG else []),  # Static files (production only)
...
MOBILE_API_SETTINGS = {
    ...
    'MOBILE_APP_API_KEY': env('MOBILE_APP_API_KEY', default=''),  # App-level key for pre-auth endpoints
    'IOS_APP_ID': env('IOS_APP_ID', default='RQ37N3FGPQ.com.spwig.Spwig'),  # Apple App ID for Universal Links
}
...
Public endpoints may be rate-limited. Authenticated requests have higher limits.'''),
        ...
        'TAGS': [
            ...
            {'name': 'Announcements', 'description': _('Store announcements for headers and footers')},
            ...
            {'name': 'Marketplace', 'description': _('Marketplace checkout and purchases (HQ only)')},
...
SANDBOX_ACTUAL_EMAIL_BACKEND = env('EMAIL_BACKEND', default='django.core.mail.backends.console.EmailBackend')
...
MEDIA_LIBRARY_SETTINGS = {
    ...
    'BLOCKED_MIME_TYPES': ['text/html', 'application/xhtml+xml', 'image/svg+xml'],  # Security: XSS risk
...
LICENSE_PATH = env(
    'LICENSE_PATH',
    default='/opt/shop-platform/license/license.json'
)
...
GEOIP_JWT_SECRET_KEY = env('GEOIP_JWT_SECRET_KEY', default=env('GEOCODER_JWT_SECRET_KEY', default=SECRET_KEY))
        ...
        'image': {
            ...
            'styles': {
                'options': ['inline', 'alignLeft', 'alignRight', 'alignCenter',
                            'alignBlockLeft', 'alignBlockRight',
                            'block', 'side']
            }
        },
        'table': {
