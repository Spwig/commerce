...
def _compute_badges():
    ...
    try:
        ...
        past_due_count = CustomerSubscription.objects.filter(
            status='past_due'
        ).count()
        if past_due_count > 0:
    ...
    try:
        ...
        translations_failed_count = TranslationJob.objects.filter(
            status='failed'
        ).count()
        ...
        return {
            'license_status': {
                ...
                'is_account_suspended': (
                    ...
                    and license_manager.get_account_status() in ('suspended', 'read_only', 'cancelled')
                ),
            },
...
def ssl_status(request):
    ...
    try:
        ...
        config = DomainConfiguration.objects.filter(pk=1).values(
            'ssl_mode', 'domain'
        ).first()
        if not config:
        ...
        return {
            'ssl_not_configured': (
                config['ssl_mode'] == 'none' and bool(config['domain'])
            ),
        }
    except Exception:
        return {}
