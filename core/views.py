...
def switch_admin_theme(request):
    ...
    try:
        ...
        if theme not in ['light', 'dark']:
            ...
            return JsonResponse({
                'success': False, 
                'error': _(f'Invalid theme: {theme}. Must be light or dark.')
            }, status=400)
        ...
        response = JsonResponse({
            'success': True, 
            'theme': theme,
            'message': _(f'Theme switched to {theme}')
        })
        ...
        return JsonResponse({
            'success': False,
            'error': _(f'Invalid JSON: {str(e)}')
        }, status=400)
    except Exception as e:
        logger.error(f"Unexpected error in theme switch: {e}")
        return JsonResponse({
            'success': False,
            'error': _(f'Server error: {str(e)}')
        }, status=500)
...
def community_redirect(request):
    ...
    if not user.email:
        messages.error(request, _('You must have an email address configured to access the Community.'))
    ...
    if not EmailAddress.objects.filter(user=user, email=user.email, verified=True).exists():
        messages.warning(
            request,
            _('Please verify your email address before accessing the Community. '
    ...
    if not site_settings.community_merchant_id or not site_settings.community_client_secret:
        ...
        logger.info(f"First community access - registering with SSO broker")
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return HttpResponse(
                    f"Failed to register with community services: {error_msg}",
                    status=500
                )
            ...
            logger.info(f"Successfully registered with SSO broker: merchant_id={result['merchant_id']}")

        except requests.RequestException as e:
            logger.error(f"Failed to connect to SSO broker: {e}")
            return HttpResponse(
                "Failed to connect to community services. Please try again later.",
                status=503
            )
        except Exception as e:
            import traceback
    ...
    try:
        ...
        logger.info(f"Community redirect: user={user.username} ({user.email}), merchant={merchant_id}")
...
def security_txt(request):
    ...
    try:
        ...
        contact_email = site_settings.contact_email if site_settings and site_settings.contact_email else 'security@spwig.com'
    except Exception:
...
def sso_redirect(request):
    ...
    if not client_id:
        return JsonResponse({'error': 'missing_client_id', 'message': 'client_id parameter is required'}, status=400)

    if not redirect_uri:
        return JsonResponse({'error': 'missing_redirect_uri', 'message': 'redirect_uri parameter is required'}, status=400)
    ...
    if client_id not in allowed_clients:
        ...
        return JsonResponse({'error': 'invalid_client', 'message': f'Unknown client: {client_id}'}, status=400)

    # Validate redirect_uri
    client_config = allowed_clients[client_id]
    if redirect_uri not in client_config['redirect_uris']:
        logger.warning(f"SSO redirect: invalid redirect_uri '{redirect_uri}' for client '{client_id}'")
        return JsonResponse({
    ...
    if not EmailAddress.objects.filter(user=user, email=user.email, verified=True).exists():
        messages.warning(
            request,
            _('Please verify your email address before using SSO services. '
              'Check your inbox for a verification email or request a new one below.')
        )
        return HttpResponseRedirect(reverse('account_email'))
    ...
    if not site_settings.community_merchant_id or not site_settings.community_client_secret:
        ...
        try:
            ...
            logger.info(f"Auto-registered with SSO broker via sso_redirect: merchant_id={result['merchant_id']}")
            ...
            import traceback
            logger.error(f"SSO registration failed during sso_redirect: {e}\n{traceback.format_exc()}")
            return JsonResponse({
    ...
    try:
        ...
        logger.info(f"SSO redirect: user={user.username}, merchant={merchant_id}, client={client_id}")
...
def filter_users(request):
    ...
    html = render_to_string(
        'admin/auth/user/partials/user_cards.html',
...
def admin_style_guide(request):
    ...
    """
    return render(request, 'admin/style_guide.html', {
