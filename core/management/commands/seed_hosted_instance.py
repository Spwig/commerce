...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Hosted instance seeded: {store_name} ({domain})"))
    ...
    def _enable_languages(self, default_language):
        ...
        try:
            ...
            self.stdout.write(
                f"  Enabled {len(target_codes)} languages (default: {default_language})"
            )
        except Exception as e:
    ...
    def _extract_email_config(self, options, store_name):
        ...
        if provision_json:
            try:
                ...
                self.stderr.write(self.style.WARNING(f"  Could not parse --provision-data: {e}"))
