...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if service.is_accepted():
            ...
            self.stdout.write(self.style.WARNING(
                "License agreement has been updated. Re-acceptance required."
            ))
        ...
        if not license_text:
            self.stderr.write(self.style.ERROR(
                "LICENSE.txt not found. Cannot proceed with license acceptance."
            ))
            return
        ...
        auto_accept = options['auto_accept'] or os.environ.get(
            'SPWIG_ACCEPT_LICENSE', ''
        ).lower() == 'true'
    ...
    def _interactive_accept(self, service, license_text, license_version):
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\nLicense v{license_version} accepted. "
            f"You may now proceed with setup."
        ))

    def _auto_accept(self, service, license_version):
        """Auto-accept license (unattended mode)."""
        self.stdout.write(
            f"Auto-accepting license v{license_version}..."
        )

        service.record_acceptance(accepted_via='env')

        self.stdout.write(self.style.SUCCESS(
            f"License v{license_version} auto-accepted. "
