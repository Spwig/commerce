...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--app',
            type=str,
            help='Specific app to generate translations for'
        )
        parser.add_argument(
            '--locale',
            action='append',
        ...
        parser.add_argument(
            '--all',
            action='store_true',
            help='Generate translations for all apps'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        local_apps = [
            ...
            'marketplace', 'marketplace_checkout', 'migration',
        ...
        if options['app']:
            ...
            self.stdout.write(
                self.style.ERROR(
                    "Please specify either --app <app_name> or --all"
                )
            )
            return
    ...
    def process_app(self, app_name, locales, base_dir, dry_run=False):
        ...
        for locale in locales:
            ...
            cmd = [
                str(python_path), str(manage_py_path), 'makemessages',
                '--locale', locale,
                '--ignore', 'shop_venv/*',
                '--ignore', 'static/*',
                '--ignore', 'media/*',
            ...
            try:
                ...
                if po_file.exists():
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"No translations found for {app_name} in {locale}"
                        )
                    )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Unexpected error processing {app_name} ({locale}): {str(e)}"
                    )
                )
