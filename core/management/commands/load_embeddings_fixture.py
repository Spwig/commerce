...
class Command(BaseCommand):
        ...
        if options['run_async'] and is_jsonl:
            ...
            if dispatched:
                self.stdout.write(self.style.SUCCESS(
                    "Embeddings loading dispatched to background worker"
                ))
                return
            self.stdout.write(self.style.WARNING(
                "Celery not available, falling back to synchronous loading"
            ))
    ...
    def _load_jsonl_sync(self, fixture_path, force, batch_size, throttle):
        ...
        if skipped:
            self.stdout.write(self.style.WARNING(
                f"  Skipped: {skipped} chunks (topic not found in database)"
            ))
