...
REVIEW_TEMPLATES = [
    (
        5,
        "Amazing quality!",
        "Exceeded my expectations. The quality is outstanding and delivery was fast.",
    ),
    ...
    (
        4,
        "Very good",
        "Great product overall. Minor packaging issue but the item itself is excellent.",
    ),
    ...
    (
        4,
        "Happy customer",
        "Very satisfied with the product. Shipping was a bit slow but worth the wait.",
    ),
...
UTM_CAMPAIGNS = [
    {
        "source": "google",
        "medium": "cpc",
        "campaign": "spring_collection_2025",
        "term": "fashion online",
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--reset", action="store_true", help="Delete existing showcase data first"
        )
    ...
    def handle(self, *args, **options):
        ...
        for i, _p in enumerate(self.products):
    ...
    def _reset(self):
        ...
        showcase_users = User.objects.filter(
            email__startswith=SHOWCASE_EMAIL_PREFIX, email__endswith=f"@{SHOWCASE_EMAIL_DOMAIN}"
        )
        affiliate_users = User.objects.filter(
            email__startswith=AFFILIATE_EMAIL_PREFIX, email__endswith=f"@{SHOWCASE_EMAIL_DOMAIN}"
        )
        all_showcase_users = User.objects.filter(
            models.Q(email__startswith=SHOWCASE_EMAIL_PREFIX)
            | models.Q(email__startswith=AFFILIATE_EMAIL_PREFIX),
        ...
        try:
            ...
            ReferralEvent.objects.filter(
                referrer_identity__customer__in=all_showcase_users
            ).delete()
            ...
            visitors.append(
                VisitorLocation(
                    ...
                    page_views=random.choices(
                        range(1, 21),
                        weights=[15, 20, 18, 12, 8, 6, 5, 4, 3, 2, 2, 1, 1, 1, 1, 1, 1, 1, 1, 1],
                        k=1,
                    )[0],
    ...
    def _create_vouchers(self):
        ...
        for vdef in VOUCHER_DEFINITIONS:
            ...
            end_date = (
                self.now + timedelta(days=90) if vdef["active"] else self.now - timedelta(days=30)
            )
    ...
    def _create_affiliates(self):
        ...
        affiliate_idx = 0
        referral_orders = [
            o for o in self.showcase_orders if o.status in ("delivered", "shipped", "processing")
        ]
        ...
        for tier_count, tier_name, min_comm, max_comm in affiliate_tiers:
            for _ in range(tier_count):
                ...
                AffiliateProgramMembership.objects.get_or_create(
                    affiliate=affiliate, program=program, defaults={"status": "approved"}
                )
                ...
                # Clicks
                click_count = random.randint(
                    50 if tier_name == "top" else 10 if tier_name == "mid" else 1,
                    2000 if tier_name == "top" else 200 if tier_name == "mid" else 50,
                )
                clicks = []
                ...
                for _ in range(min(num_commissions, len(referral_orders))):
                    ...
                    commission_amount = (
                        (order.total_amount.amount * Decimal("0.10")).quantize(Decimal("0.01"))
                        if hasattr(order.total_amount, "amount")
                        else Decimal("10.00")
                    )
                    comm_status = _weighted_choice(
                        [("approved", 70), ("pending", 20), ("paid", 10)]
                    )
                    comm = Commission.objects.create(
                        ...
                        approved_at=order.created_at + timedelta(days=3)
                        if comm_status in ("approved", "paid")
                        else None,
                        paid_at=order.created_at + timedelta(days=30)
                        if comm_status == "paid"
                        else None,
                    )
    ...
    def _create_referrals(self):
        ...
        with (
            disable_auto_now(ReferralAttribution, ["created_at"]),
            ...
            for i in range(min(35, len(referees), len(orders_for_referral))):
                ...
                referee = (
                    referees[i] if i < len(referees) else random.choice(self.showcase_users[20:])
                )
    ...
    def _create_reviews(self):
        ...
        seen_pairs = set(
            ProductReview.objects.filter(user__in=self.showcase_users).values_list(
                "product_id", "user_id"
            )
        )
    ...
    def _create_abandoned_carts(self):
        ...
        with (
            disable_auto_now(Cart, ["created_at", "updated_at"]),
    ...
    def _create_customer_messages(self):
        ...
        with disable_auto_now(CustomerMessage, ["created_at", "updated_at"]):
            for month_idx, target in enumerate(monthly_targets):
                ...
                for _ in range(target):
                    template = CUSTOMER_MESSAGE_TEMPLATES[
                        template_idx % len(CUSTOMER_MESSAGE_TEMPLATES)
                    ]
                    template_idx += 1
                    ...
                    if (
                        msg_type == "order"
                        and hasattr(self, "showcase_orders")
                        and self.showcase_orders
                    ):
        ...
        replied = CustomerMessage.objects.filter(
            user__in=self.showcase_users, status="replied"
        ).count()
        unread = CustomerMessage.objects.filter(
            user__in=self.showcase_users, status="unread"
        ).count()
        self.stdout.write(
            f"  Created {messages_created} customer messages ({replied} replied, {unread} unread)"
        )
