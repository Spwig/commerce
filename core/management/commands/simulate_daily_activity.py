...
class Command(BaseCommand):
    ...
    def _load_context(self):
        ...
        self.product_weights = [3 if i < bestseller_count else 1 for i in range(len(self.products))]
    ...
    def _create_new_orders(self):
        ...
        with disable_auto_now(Order, ["created_at"]):
            for seq in range(1, count + 1):
                ...
                selected = random.choices(self.products, weights=self.product_weights, k=num_items)
                seen = set()
                unique_products = []
                for p in selected:
                    if p.pk not in seen:
                ...
                total_amount = (subtotal + tax_amount + shipping_cost - discount_amount).quantize(
                    Decimal("0.01")
                )
                if total_amount < 0:
                ...
                order = Order(
                    ...
                    shipping_address2=(
                        f"Apt {random.randint(1, 500)}" if random.random() < 0.3 else ""
                    ),
    ...
    def _progress_order_lifecycle(self):
        ...
        for order in to_ship:
            ...
            TrackingEvent.objects.create(
                ...
                occurred_at=self.sim_datetime + timedelta(hours=random.randint(2, 8)),
            )
        ...
        if random.random() < 0.15:
            cancel_candidates = list(
                Order.objects.filter(
                    order_number__startswith="SC", status="pending", created_at__lt=cutoff_2d
                )[:10]
            )
        ...
        if random.random() < 0.05:
            refund_candidates = list(
                Order.objects.filter(
                    order_number__startswith="SC", status="delivered", created_at__lt=cutoff_5d
                )[:20]
            )
        ...
        for i in range(count):
            ...
            visitors.append(
                VisitorLocation(
                    ...
                    ip_address=(
                        f"{random.randint(10, 220)}.{random.randint(1, 255)}"
    ...
    def _create_abandoned_carts(self):
        ...
        with disable_auto_now(Cart, ["created_at", "updated_at"]):
            for i in range(count):
                ...
                timestamp = self.sim_datetime + timedelta(hours=hour, minutes=random.randint(0, 59))
        ...
        if random.random() < 0.30:
            ...
            for ac in unrecovered:
                ac.recovered = True
                ac.recovered_at = self.sim_datetime + timedelta(hours=random.randint(10, 20))
                ac.save(update_fields=["recovered", "recovered_at"])
                recover_count = 1

        self.stdout.write(f"  Abandoned Carts: created {count}, recovered {recover_count}")
    ...
    def _update_stock(self):
        ...
        self.stdout.write(f"  Stock: restocked {restocked}, decreased {decreased}")
    ...
    def _progress_shipments(self):
        ...
        for shipment in random.sample(in_transit, k=sample_size) if in_transit else []:
            ...
            TrackingEvent.objects.create(
                ...
                occurred_at=self.sim_datetime + timedelta(hours=random.randint(6, 14)),
            )
            progressed += 1
        ...
        for shipment in random.sample(ofd, k=sample_size) if ofd else []:
            ...
            TrackingEvent.objects.create(
                ...
                occurred_at=self.sim_datetime + timedelta(hours=random.randint(10, 18)),
            )
            delivered += 1

        self.stdout.write(f"  Shipments: {progressed} out for delivery, {delivered} delivered")
    ...
    def _create_customer_messages(self):
        ...
        with disable_auto_now(CustomerMessage, ["created_at", "updated_at"]):
            for _ in range(count):
                ...
                if msg_type == "order":
                    sample_order = (
                        Order.objects.filter(order_number__startswith="SC").order_by("?").first()
                    )
                    if sample_order:
                        order_ref = sample_order
                        body = body.replace("{order_number}", sample_order.order_number)
                    else:
                ...
                if random.random() < 0.70:
                    ...
                    status = "read"
                    read_at = timestamp + timedelta(hours=random.randint(1, 4))
                    read_by = admin_user
    ...
    def _process_loyalty_activity(self):
        ...
        for order in self.todays_orders:
            if not order.user:
                continue
            member = LoyaltyMember.objects.filter(customer=order.user, is_active=True).first()
            if not member:
                continue
                ...
                if members_with_points:
                    ...
                    if member.balance.available_points >= reward.points_cost:
                        LoyaltyRedemption.objects.create(
                            member=member,
                            reward=reward,
                            redemption_code=(f"LOYALTY-SCD-{secrets.token_hex(5).upper()}"),
                        ...
                        member.balance.save(update_fields=["available_points", "lifetime_redeemed"])
                        redeemed = 1

        self.stdout.write(f"  Loyalty: {earned} earn transactions, {redeemed} redemptions")
    ...
    def _process_affiliate_activity(self):
        ...
        for _ in range(click_count):
            ...
            link = Link.objects.filter(affiliate=affiliate, is_active=True).first()
            if not link:
                continue
            Click.objects.create(
                ...
                user_agent=random.choice(USER_AGENTS["desktop"] + USER_AGENTS["mobile"]),
                referrer="",
                session_id=(
                    f"aff_daily_{self.sim_date.strftime('%Y%m%d')}_{random.randint(1000, 9999)}"
                ),
        ...
        self.stdout.write(f"  Affiliates: {clicks_created} clicks, {commission_count} commissions")
