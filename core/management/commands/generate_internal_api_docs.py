...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['format'] in ['json', 'both']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'JSON documentation generated in {output_dir}')
            )
    ...
    def collect_api_data(self):
        ...
        for app_config in apps.get_app_configs():
            ...
            try:
                ...
                try:
                    api_module = __import__(f'{app_name}.api', fromlist=[''])
                except ImportError:
                    api_module = None
            ...
            for app_name, app_apis in api_data['apps'].items():
                ...
                if app_apis['viewsets']:
                    ...
                    for viewset in app_apis['viewsets']:
                        f.write(f'- **{viewset["name"]}**: {viewset["docstring"].split("\\n")[0]}\n')
                        if viewset['methods']:
                            f.write('  - Actions: ' + ', '.join(m['name'] for m in viewset['methods']) + '\n')
                    f.write('\n')
                ...
                if app_apis['api_views']:
                    ...
                    for view in app_apis['api_views']:
                        ...
                        if view['methods']:
                            f.write('  - Methods: ' + ', '.join(m['name'] for m in view['methods']) + '\n')
                    f.write('\n')
    ...
    def generate_json_docs(self, api_data, output_dir):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'OpenAPI schema generated: {openapi_path}')
            )
        except ImportError:
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.WARNING(f'Could not generate OpenAPI schema: {str(e)}')
            )