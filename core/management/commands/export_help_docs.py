...
URL_SANITIZE_RULES = [
    ...
    (
        r"/admin/catalog/category/(\d+)/change/",
        "Navigate to **Products > Categories > Edit Category**",
    ),
    ...
    # Payment
    (
        r"/admin/payment_providers/paymentprovideraccount/",
    ...
    (
        r"/admin/(\w+)/(\w+)/",
        lambda m: f"Navigate to **{m.group(1).title()} > {m.group(2).replace('_', ' ').title()}**",
    ),
]
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--output", type=str, required=True, help="Output directory for exported files"
        )
        parser.add_argument(
            "--doc-version",
            type=str,
        ...
        parser.add_argument(
            "--sanitize-urls", action="store_true", help="Replace admin URLs with navigation hints"
        )
        parser.add_argument(
            "--copy-images", action="store_true", help="Copy help images to output directory"
        )
        parser.add_argument(
            "--languages",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("\nExporting topics...")
        topics = (
            HelpTopic.objects.filter(is_published=True)
        ...
        self.stdout.write(
            f"   Total files: {topics_exported + translations_exported + len(categories_data) + 2}"
        )
...
"""
    ...
    def _copy_help_images(self, output_dir: Path) -> int:
        ...
        help_images_source = (
            Path(settings.BASE_DIR) / "core" / "static" / "core" / "admin" / "img" / "help"
        )
        help_images_dest = output_dir / "images"

        if not help_images_source.exists():
            self.stdout.write(
                self.style.WARNING(f"  ⚠ Help images source not found: {help_images_source}")
            )
            return 0
    ...
    def _markdown_to_html_placeholder(self, markdown_content: str) -> str:
        ...
        if not markdown:
            self.stdout.write(
                self.style.WARNING(
                    "  ⚠ markdown library not available, translations will lack formatting"
                )
            )
