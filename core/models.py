...
class SoftDeleteModel(models.Model):
    ...
    is_deleted = models.BooleanField(
        default=False, db_index=True, help_text=_("Whether this object has been soft deleted")
    )

    deleted_at = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text=_("When this object was soft deleted")
    )
...
class SiteSettings(models.Model):
    ...
    site_logo = models.ForeignKey(
        ...
        help_text=_(
            "Select a logo from the media library. Recommended: PNG or SVG format. Horizontal logos work best (max 400px wide). SVG files scale perfectly to any size."
        ),
    )
    ...
    phone_number = models.CharField(
        max_length=20, blank=True, help_text="Store contact phone number"
    )
    ...
    address_line_2 = models.CharField(
        max_length=255, blank=True, help_text="Apartment, suite, etc."
    )
    ...
    default_timezone = models.CharField(
        max_length=50, default="UTC", help_text="Default timezone for displaying dates and times."
    )
    ...
    enable_multi_currency = models.BooleanField(
        ...
        help_text=_(
            "Allow customers to view prices and checkout in different currencies. Requires exchange rate provider configuration."
        ),
    )
    ...
    supported_currencies = models.JSONField(
        ...
        help_text=_(
            "List of currency codes enabled for customer selection (e.g., ['USD', 'EUR', 'GBP']). Leave empty to support all 308 currencies."
        ),
    )
    ...
    show_currency_switcher = models.BooleanField(
        ...
        help_text=_(
            "Display currency switcher widget in storefront (only when multi-currency is enabled)"
        ),
    )
    ...
    show_exchange_rate_info = models.BooleanField(
        ...
        help_text=_(
            "Display exchange rate and last update time to customers (transparency feature)"
        ),
    )
    ...
    enable_locale_formatting = models.BooleanField(
        ...
        help_text=_(
            "Format currency amounts according to customer's locale (e.g., '$1,234.56' for US, '1.234,56 €' for Germany)"
        ),
    )
    ...
    exchange_rate_markup_enabled = models.BooleanField(
        ...
        help_text=_(
            "Add a percentage markup to exchange rates to cover conversion fees and currency risk"
        ),
    )
    ...
    exchange_rate_markup_percentage = models.DecimalField(
        ...
        help_text=_(
            "Percentage to add to exchange rates (e.g., 2.50 for 2.5% markup). Only applied when markup is enabled."
        ),
    )
    ...
    exchange_rate_selection_strategy = models.CharField(
        ...
        help_text=_(
            "When multiple providers are active: 'Primary' always uses rates from your primary provider (consistent pricing), 'Latest' uses the most recently synced rate from any active provider (freshest data)."
        ),
    )
    ...
    exchange_rate_sync_interval = models.CharField(
        ...
        help_text=_(
            "How often to automatically sync exchange rates from providers. "
    ...
    allow_guest_checkout = models.BooleanField(
        default=True, help_text="Allow customers to checkout without creating an account"
    )

    require_phone_for_checkout = models.BooleanField(
        default=False, help_text="Require phone number during checkout"
    )
    ...
    auto_approve_reviews = models.BooleanField(
        default=False, help_text="Automatically approve product reviews without moderation"
    )
    ...
    enable_order_confirmation_emails = models.BooleanField(
        default=True, help_text="Send order confirmation emails to customers"
    )

    enable_shipping_notification_emails = models.BooleanField(
        default=True, help_text="Send shipping notification emails to customers"
    )

    enable_low_stock_alerts = models.BooleanField(
        default=True, help_text="Send low stock alerts to admin email"
    )

    low_stock_threshold = models.PositiveIntegerField(
        default=10, help_text="Stock level at which to send low stock alerts"
    )
    ...
    meta_title = models.CharField(
        max_length=60, blank=True, help_text="Default meta title for pages (60 characters max)"
    )
    ...
    meta_keywords = models.CharField(
        max_length=255, blank=True, help_text="Default meta keywords for pages (comma-separated)"
    )
    ...
    maintenance_mode = models.BooleanField(
        default=False, help_text="Enable maintenance mode (site will show maintenance page)"
    )
    ...
    maintenance_page = models.ForeignKey(
        ...
        help_text=_(
            "Custom page displayed when maintenance mode is enabled. Edit this page in Page Builder to customize the maintenance experience."
        ),
    )
    ...
    error_reporting_enabled = models.BooleanField(
        ...
        help_text=_(
            "Automatically send anonymized error reports to Spwig to help improve the platform. No personal data is ever transmitted."
        ),
    )
    error_reporting_include_js = models.BooleanField(
        ...
        help_text=_(
            "Also capture and report JavaScript errors from the storefront and admin interface."
        ),
    )
    ...
    community_registered_at = models.DateTimeField(
        null=True, blank=True, help_text="When this store was registered with community services"
    )
    ...
    staff_2fa_grace_period_days = models.PositiveIntegerField(
        ...
        help_text=_(
            "Days allowed for staff to set up 2FA after enforcement is enabled. Set to 0 for immediate enforcement."
        ),
    )
    ...
    admin_password_login_enabled = models.BooleanField(
        ...
        help_text=_(
            "Allow password-based login on the admin page. Disable to enforce SSO-only access."
        ),
    )
    ...
    max_devices_per_user = models.PositiveIntegerField(
        ...
        help_text=_(
            "Maximum number of mobile devices each staff member can have registered simultaneously. "
    ...
    cookie_consent_mode = models.CharField(
        ...
        help_text=_(
            "'Simple' shows Accept/Reject buttons only. "
    ...
    cookie_analytics_description = models.TextField(
        ...
        help_text=_(
            "Describe what analytics cookies do (shown in granular mode). Supports translations."
        ),
    )
    ...
    cookie_marketing_description = models.TextField(
        ...
        help_text=_(
            "Describe what marketing cookies do (shown in granular mode). Supports translations."
        ),
    )
    ...
    cookie_functional_description = models.TextField(
        ...
        help_text=_(
            "Describe what functional cookies do (shown in granular mode). Supports translations."
        ),
    )
    ...
    default_image_resize_mode = models.CharField(
        ...
        help_text=_(
            "How images are resized when generating thumbnails. 'Cover' crops to fill dimensions, 'Contain' fits within dimensions, 'Pad' preserves all content with padding."
        ),
    )
    ...
    default_padding_color = models.CharField(
        ...
        help_text=_(
            "Background color for padding when using 'Pad' resize mode. Transparent works best for logos."
        ),
    )
    ...
    logo_padding_color = models.CharField(
        ...
        help_text=_(
            "Background color for logo padding. Transparent preserves the logo background."
        ),
    )
    ...
    enable_double_opt_in = models.BooleanField(
        ...
        help_text=_(
            "Require customers to verify their email address before receiving marketing communications (GDPR compliance)"
        ),
    )
    ...
    default_marketing_opt_in = models.BooleanField(
        ...
        help_text=_(
            "Default opt-in state for marketing emails during registration (False = opt-out per GDPR best practices)"
        ),
    )
    ...
    preference_center_enabled = models.BooleanField(
        ...
        help_text=_(
            "Allow customers to manage their communication preferences via account dashboard"
        ),
    )
    ...
    show_unsubscribe_reasons = models.BooleanField(
        ...
        help_text=_(
            "Ask customers why they're unsubscribing (helps improve communication strategy)"
        ),
    )
    ...
    geocoder_installation_uuid = models.UUIDField(
        ...
        help_text=_(
            "Unique identifier for this installation used for geocoder service authentication"
        ),
    )
    ...
    allow_backorders_by_default = models.BooleanField(
        ...
        help_text=_(
            "Allow customers to order products even when out of stock (new products inherit this setting)"
        ),
    )
    ...
    safety_stock_multiplier = models.FloatField(
        ...
        help_text=_(
            "Multiplier applied to velocity for safety stock calculation (e.g., 1.5 means 50%% buffer)"
        ),
    )
    ...
    invoice_footer_text = models.TextField(
        ...
        help_text=_(
            "Custom text displayed at the bottom of generated invoices (e.g., payment terms, thank you message)"
        ),
    )
    ...
    packing_slip_footer_text = models.TextField(
        ...
        help_text=_(
            "Custom text displayed at the bottom of packing slips (e.g., return instructions)"
        ),
    )
    ...
    document_logo_width = models.PositiveIntegerField(
        ...
        help_text=_(
            "Width of the logo in generated PDF documents (invoices, packing slips). Height scales proportionally."
        ),
    )
    ...
    def clean(self):
        ...
        if not self.enable_multi_warehouse:
            ...
            if non_main_warehouses.exists():
                ...
                for warehouse in non_main_warehouses:
                    ...
                    stock_items = warehouse.stock_items.filter(on_hand__gt=0).select_related(
                        "product"
                    )
                ...
                if warehouses_with_stock:
                    ...
                    for wh in warehouses_with_stock[:5]:  # Show max 5
                        ...
                        warehouse_details.append(
                            f"{escape(wh.name)} ({escape(wh.code)}): {stock_count} product(s)"
                        )
                    ...
                    raise ValidationError({"enable_multi_warehouse": error_msg})

                # Has non-main warehouses but no stock - still require deletion
                else:
                    warehouse_names = [
                        f"{escape(wh.name)} ({escape(wh.code)})" for wh in non_main_warehouses[:5]
                    ]
        ...
        if not self.enable_multi_currency:
            ...
            if multi_currency_orders.exists():
                ...
                currencies_used = [
                    c
                    for c in multi_currency_orders.values_list(
                        "customer_currency", flat=True
                    ).distinct()[:5]
                    if c
                ]
        ...
        if self.enable_multi_currency:
            ...
            active_providers = ExchangeRateProviderAccount.objects.filter(site=site, is_active=True)
            has_manual_rates = ManualExchangeRate.objects.filter(site=site, is_active=True).exists()
    ...
    def effective_account_creation_message(self):
        ...
        default = _(
            "Create an account to track your order, save addresses, and enjoy faster checkout next time!"
        )
    ...
    def get_page_for_role(self, role):
        ...
        return Page.objects.filter(
            page_type=page_type, is_default_for_type=True, status="published"
        ).first()
    ...
    def get_legal_pages(self):
        ...
        return {role: self.get_page_for_role(role) for role in legal_roles}
    ...
    def get_cookie_consent_context(self, language_code="en"):
        ...
        if self.cookie_page:
            ...
            try:
                policy_url = reverse(
                    "page_builder:page_detail", kwargs={"slug": self.cookie_page.slug}
                )
            except Exception:
                policy_url = ""
...
class LicenseRevocation(models.Model):
    ...
    acknowledged = models.BooleanField(
        default=False, help_text="Merchant dismissed the warning banner"
    )
...
class HelpTopic(models.Model):
    ...
    category = models.ForeignKey(
        HelpCategory, on_delete=models.CASCADE, related_name="topics", verbose_name=_("Category")
    )
    ...
    keywords = models.JSONField(
        default=list, help_text=_("Search keywords (JSON array)"), verbose_name=_("Keywords")
    )
    ...
    translations = models.JSONField(
        ...
        blank=True,
        help_text=_(
            "Translated content by language code, e.g. {'es': {'title': '...', 'content': '...'}}"
        ),
        verbose_name=_("Translations"),
    )
...
class HelpFeedback(models.Model):
    ...
    topic = models.ForeignKey(
        HelpTopic, on_delete=models.CASCADE, related_name="feedback", verbose_name=_("Topic")
    )
    user = models.ForeignKey(
        get_user_model(), on_delete=models.SET_NULL, null=True, blank=True, verbose_name=_("User")
    )
    helpful = models.BooleanField(
        help_text=_("True = helpful, False = not helpful"), verbose_name=_("Helpful")
    )
...
class HelpView(models.Model):
    ...
    topic = models.ForeignKey(
        HelpTopic, on_delete=models.CASCADE, related_name="views", verbose_name=_("Topic")
    )
    session_id = models.CharField(
        max_length=100, help_text=_("Anonymous session identifier"), verbose_name=_("Session ID")
    )
...
class HelpSearchIndex(models.Model):
    ...
    indexed_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Indexed At"), db_index=True
    )
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("Updated At"))
...
class APIToken(models.Model):
    ...
    name = models.CharField(
        max_length=200,
        help_text=_(
            "Descriptive name for this token (e.g., 'Help System API', 'Zapier Integration')"
        ),
        verbose_name=_("Name"),
    )
...
class PlatformSecrets(models.Model):
    ...
    geoip_jwt_secret = models.CharField(
        max_length=128, blank=True, help_text=_("JWT secret for GeoIP service authentication")
    )
    ...
    installation_uuid = models.UUIDField(
        null=True, blank=True, help_text=_("Installation UUID assigned by license server")
    )

    license_server_token = models.TextField(
        blank=True, help_text=_("Current access token for license server API")
    )

    license_server_refresh_token = models.TextField(
        blank=True, help_text=_("Refresh token for license server API")
    )

    token_expires_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When the current access token expires")
    )

    # Timestamps
    secrets_fetched_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When secrets were last fetched from license server")
    )
...
class TrustedDevice(models.Model):
    ...
    expires_at = models.DateTimeField(
        verbose_name=_("Expires At"), help_text=_("When this trusted device designation expires")
    )

    last_used_at = models.DateTimeField(null=True, blank=True, verbose_name=_("Last Used At"))

    is_revoked = models.BooleanField(default=False, db_index=True, verbose_name=_("Is Revoked"))

    revoked_at = models.DateTimeField(null=True, blank=True, verbose_name=_("Revoked At"))

    revoked_reason = models.CharField(
        max_length=255, blank=True, verbose_name=_("Revocation Reason")
    )
    ...
    def validate_token(cls, user, raw_token):
        ...
        try:
            device = cls.objects.get(user=user, device_token_hash=token_hash, is_revoked=False)
            if device.is_valid:
                device.record_usage()
                return device
...
class ErrorReport(models.Model):
    ...
    def __str__(self):
        exc_type = (
            self.error_data.get("exception_type", "Unknown") if self.error_data else "Unknown"
        )
...
class SalesBellEvent(models.Model):
    ...
    def log_refund(cls, refund_transaction):
        ...
        return cls.objects.create(
            ...
            amount=raw_amount,
            currency=str(refund_transaction.amount_currency)
            if refund_transaction.amount_currency
            else "EUR",
        )
