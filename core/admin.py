...
from .models import SiteSettings, LicenseStatus, APIToken, ErrorReport, BugReport, CookieConsentLog
...
class SiteSettingsForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'site_name': TranslatableFieldWidget(
                base_widget=widgets.TextInput(attrs={'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'})
            ),
            'site_tagline': TranslatableFieldWidget(
                base_widget=widgets.TextInput(attrs={'style': 'width: 100%; max-width: 100%; box-sizing: border-box;', 'placeholder': 'e.g., Quality Products, Unbeatable Prices'})
            ),
            'site_description': TranslatableFieldWidget(
                base_widget=widgets.Textarea(attrs={'rows': 3, 'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'})
            ),
            # Non-translatable fields
            'address_line_1': widgets.TextInput(attrs={'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'}),
            'address_line_2': widgets.TextInput(attrs={'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'}),
            'meta_title': widgets.TextInput(attrs={'maxlength': 60, 'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'}),
            'meta_description': widgets.Textarea(attrs={'rows': 3, 'maxlength': 160, 'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'}),
            'meta_keywords': widgets.TextInput(attrs={'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'}),
            'maintenance_message': widgets.Textarea(attrs={'rows': 4, 'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'}),
            # Account creation settings
            'account_creation_message': TranslatableFieldWidget(
                base_widget=widgets.Textarea(attrs={
                    'rows': 3,
            ...
            'cookie_banner_title': TranslatableFieldWidget(
                base_widget=widgets.TextInput(attrs={'style': 'width: 100%; max-width: 100%; box-sizing: border-box;', 'placeholder': 'e.g., We use cookies'})
            ),
            'cookie_banner_text': TranslatableFieldWidget(
                base_widget=widgets.Textarea(attrs={'rows': 3, 'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'})
            ),
            'cookie_analytics_description': TranslatableFieldWidget(
                base_widget=widgets.Textarea(attrs={'rows': 2, 'style': 'width: 100%; max-width: 100%; box-sizing: border-box;'})
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['default_language'].widget = SearchableSelectWidget(
            icon_callback=get_language_icon,
            attrs={'data-placeholder': 'Search languages...'}
        )
        ...
        self.fields['default_timezone'].widget = SearchableSelectWidget(
            icon_callback=get_timezone_icon,
            attrs={'data-placeholder': 'Search timezones...'}
        )
...
class SiteSettingsAdmin(TranslatableAdminMixin, admin.ModelAdmin):
    ...
    translatable_fields = [
        ...
        'cookie_analytics_description', 'cookie_marketing_description', 'cookie_functional_description',
    ...
    readonly_fields = ('created_at', 'updated_at', 'payment_methods_summary', 'oauth_providers_summary', 'email_accounts_summary')
    ...
    def response_change(self, request, obj):
        ...
        self.message_user(
            request, 
            _(f"Site settings for '{obj.site_name}' have been updated successfully.")
        )
        return response
    ...
    def response_add(self, request, obj, post_url_continue=None):
        ...
        self.message_user(
            request,
            _(f"Site settings for '{obj.site_name}' have been created successfully.")
        )
        return response
    ...
    def payment_methods_summary(self, obj):
        ...
        try:
            ...
            if total_providers == 0:
                ...
                status_text = str(_("{} provider(s) configured but none active")).format(total_providers)
            else:
                status_class = "status-success"
                status_text = str(_("{} of {} provider(s) active")).format(active_providers, total_providers)
            ...
            if active_providers > 0:
                active_provider_list = PaymentProviderAccount.objects.filter(is_active=True).select_related('component')
                provider_badges = []
    ...
    def _oauth_providers_summary_old(self, obj):
        ...
        try:
            ...
            configured_providers = OAuthProviderSettings.objects.filter(is_configured=True).count()
            ...
            if not site_settings.site_url or site_settings.site_url in ('http://example.com', 'https://example.com', ''):
                summary_parts.append(f'''
                    <div style="
                        background: #fff3cd;
                        border: 1px solid #ffc107;
    ...
    def email_accounts_summary(self, obj):
        ...
        try:
            ...
            return format_html(
                ...
                _('Email system not installed')
            )
        except Exception as e:
            return format_html(
                '<p style="color: #dc3545;">{}: {}</p>',
                _('Error loading email accounts'),
                str(e)
            )
    ...
    def _get_2fa_compliance_data(self):
        ...
        if total_staff > 0:
            compliance_percent = round((staff_with_2fa / total_staff) * 100)
        else:
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            ...
            if (old_obj.staff_2fa_enforcement != 'required' and
                obj.staff_2fa_enforcement == 'required' and
                not obj.staff_2fa_enforcement_date):
        ...
        if not obj.admin_password_login_enabled:
            try:
                ...
                if not obj.admin_sso_enabled or not sso_config.is_configured:
                    ...
                    messages.warning(
                        request,
                        _("Password login was re-enabled because SSO is not configured. "
    ...
    class Media:
        css = {
            'all': ('core/admin/css/site_settings.css', 'media_library/css/media-library.css')
        }
...
class LicenseStatusAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        return render(request, self.change_list_template or [
            'admin/%s/%s/change_list.html' % (self.model._meta.app_label, self.model._meta.model_name),
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('api/status/', self.admin_site.admin_view(self.license_status_api), name='core_licensestatus_api'),
            path('upload/', self.admin_site.admin_view(self.upload_license_view), name='core_licensestatus_upload'),
            path('activate/', self.admin_site.admin_view(self.activate_license_view), name='core_licensestatus_activate'),
            path('clear-cache/', self.admin_site.admin_view(self.clear_cache_view), name='core_licensestatus_clear_cache'),
            path('refresh/', self.admin_site.admin_view(self.refresh_license_view), name='core_licensestatus_refresh'),
        ]
        ...
        if request.method == 'POST' and request.FILES.get('license_file'):
            try:
                ...
                if not license_manager.verify_signature(license_data):
                    return JsonResponse({
                        'success': False,
                        'error': _('License signature verification failed. This license may be tampered or invalid.')
                    }, status=400)
                ...
                return JsonResponse({
                    'success': False,
                    'error': _('Invalid JSON format in license file.')
                }, status=400)
            except Exception as e:
                return JsonResponse({
                    'success': False,
    ...
    def activate_license_view(self, request):
        ...
        if not license_key:
            return JsonResponse({
                'success': False,
                'error': _('License key is required')
            }, status=400)
        ...
        try:
            ...
            if not license_manager.verify_signature(license_data_obj):
                return JsonResponse({
                    'success': False,
                    'error': _('License signature verification failed.')
                }, status=400)
            ...
            return JsonResponse({
                'success': False,
                'error': _('Could not connect to update server. Please check your internet connection.')
            }, status=503)
        except Exception as e:
            return JsonResponse({
                'success': False,
                'error': _(f'Activation error: {str(e)}')
            }, status=500)
    ...
    def refresh_license_view(self, request):
        ...
        try:
            ...
            if result.get('error'):
                return JsonResponse({
                    'success': False,
                    'error': _('License refresh failed: %s') % result['error']
                }, status=502)
            ...
            return JsonResponse({
                ...
                'message': _('License updated successfully!') if result.get('refreshed')
    ...
    def clear_cache_view(self, request):
        ...
        try:
            ...
            return JsonResponse({
                'success': True,
                'message': _('License cache cleared and reloaded successfully!')
            })
        except Exception as e:
            return JsonResponse({
                'success': False,
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    list_display = ['username', 'email', 'email_verified', 'display_roles', 'first_name', 'last_name', 'is_staff', 'is_active']
    ...
    def display_roles(self, obj):
        ...
        if obj.is_superuser and not roles:
            return format_html('<span style="background: var(--primary); color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 11px;">Owner</span>')
        if not roles:
        ...
        for role in roles:
            ...
            badges.append(f'<span style="background: {bg}; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 11px; margin-right: 4px;">{role.display_name}</span>')
    ...
    def email_verified(self, obj):
        ...
        if not obj.email:
            return format_html(
                '<span style="color: var(--body-quiet-color);" title="{}">—</span>',
                _('No email')
            )
...
class APITokenAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        (_('Token Value'), {
            ...
            'description': _('Copy this token now - it will be masked in the list view for security.')
        }),
...
class ErrorReportAdmin(admin.ModelAdmin):
    list_display = (
        'error_type_badge', 'exception_summary', 'occurrence_count',
        'status_badge', 'last_seen',
    )
    list_filter = ('error_type', 'status')
    search_fields = ('error_data',)
    readonly_fields = (
        'error_type', 'fingerprint', 'occurrence_count', 'error_data',
        'first_seen', 'last_seen', 'sent_at',
    )
...
class BugReportAdmin(admin.ModelAdmin):
    list_display = ('category', 'severity_badge', 'description_short', 'status_badge', 'submitted_by', 'created_at')
    list_filter = ('category', 'severity', 'status')
    readonly_fields = (
        'category', 'description', 'severity', 'browser_data', 'consent_flags',
        'contact_name', 'contact_email', 'contact_consent',
        'page_url', 'admin_section', 'submitted_by', 'created_at', 'sent_at',
    )
...
class CookieConsentLogAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        'timestamp', 'action', 'consent_data', 'ip_address',
        'user_agent', 'user', 'session_key',
    ]
    list_per_page = 50
    ordering = ['-timestamp']
    ...
    def export_consent_csv(self, request, queryset):
        ...
        writer.writerow([
            'Timestamp', 'Action', 'IP Address', 'User Agent',
            'User Email', 'Session Key', 'Analytics', 'Marketing', 'Functional',
        ])
