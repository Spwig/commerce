...
(function () {
  ...
  const configEl = document.getElementById('translation-editor-init-config');
  if (!configEl) return;
  ...
  function initializeTranslationEditor() {
    ...
    document.querySelectorAll('.translate-field-btn').forEach(function (button) {
      ...
      const fieldInput = document.querySelector('#id_' + fieldName);
      if (!fieldInput) {
      ...
      button.addEventListener('click', async function (e) {
        ...
        try {
          ...
          const saveResponse = await fetch(fieldEndpoints.saveField, {
            method: 'POST',
  ...
  async function openTranslationEditor(fieldName, fieldInput, endpoints, availableLanguages) {
    const translateBtn = document.querySelector(
      '.translate-field-btn[data-field-name="' + fieldName + '"]'
    );
    const fieldType = (translateBtn && translateBtn.getAttribute('data-field-type')) || 'text';
    const isRichText = fieldType === 'richtext';
    const ckeditorConfig =
      (translateBtn && translateBtn.getAttribute('data-ckeditor-config')) || 'default';
    let ckeditorEditorConfig = null;

    let currentValue;
    if (isRichText && typeof ClassicEditor !== 'undefined') {
      const editorElement = fieldInput.closest('.django-ckeditor-5');
    ...
    const fieldLabel = fieldName.replace(/_/g, ' ').replace(/\b\w/g, function (l) {
    ...
    body.innerHTML =
      ...
      '<label style="font-weight: bold; display: block; margin-bottom: 8px; color: var(--body-fg);">Source Text ' +
      (isRichText ? '(Preview)' : '') +
      ':</label>' +
    ...
    const lockEndpoint = config.lockEndpoint;
    ...
    availableLanguages.forEach(function (lang) {
      ...
      const isLocked = langLockedFields.includes(fieldName);
      ...
      const escapedTranslation = existingTranslation
        .replace(/"/g, '&quot;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;');
      const lockBtnStyle =
      ...
      const savedBadge =
        isTranslated && !isLocked
          ? '<span class="translation-status-badge" data-lang="' +
            lang.code +
            '" style="padding: 2px 8px; border-radius: 3px; font-size: 0.75em; font-weight: bold; background: var(--success-bg, #d4edda); color: var(--success-color, #28a745); border: 1px solid var(--success-color, #28a745);">' +
            TRANS.saved +
            '</span>'
          : '';
      let editField = '';
      if (isTranslated && !isLocked) {
        if (isRichText) {
          editField =
            '<div class="translation-edit-field-richtext" data-lang="' +
            lang.code +
            '" data-original-value="' +
            escapedTranslation +
            '" id="editor_' +
    ...
    languagesList.querySelectorAll('.translation-lock-btn').forEach(function (btn) {
      btn.addEventListener('click', async function (e) {
        ...
        try {
          ...
          if (data.success) {
            modal.remove();
            openTranslationEditor(fieldName, fieldInput, endpoints, availableLanguages);
          }
        } catch (err) {
    ...
    const sourceTextDisplay = body.querySelector('#source-text-display');
    if (isRichText) {
      sourceTextDisplay.innerHTML = currentValue;
    } else {
      sourceTextDisplay.textContent = currentValue;
    }

    if (isRichText && typeof ClassicEditor !== 'undefined') {
      const baseConfig =
        (window.django_ckeditor_5_configs &&
          (window.django_ckeditor_5_configs[ckeditorConfig] ||
            window.django_ckeditor_5_configs['default'])) ||
        {};
      ...
      availableLanguages.forEach(function (lang) {
        ...
        if (existingTranslation) {
          const editorEl = document.getElementById('editor_' + lang.code);
          if (editorEl) {
            ClassicEditor.create(editorEl, ckeditorEditorConfig)
              .then(function (editor) {
                editorInstances[lang.code] = editor;
                const unescapedContent = existingTranslation
                  .replace(/&quot;/g, '"')
                  .replace(/&lt;/g, '<')
                  .replace(/&gt;/g, '>');
                ...
                editor.model.document.on('change:data', function () {
                  ...
                  const badge = languagesList.querySelector(
                    '.translation-status-badge[data-lang="' + lang.code + '"]'
                  );
                  if (badge) {
              ...
              .catch(function (error) {
                ...
                editorEl.outerHTML =
                  ...
                  '" data-original-value="' +
                  existingTranslation +
                  '" style="width: 100%; padding: 8px; border: 1px solid var(--hairline-color); border-radius: 4px; background: var(--body-bg); color: var(--body-fg); font-family: inherit; font-size: 0.9em; min-height: 100px; resize: vertical;">' +
                  existingTranslation +
                  '</textarea>';
              });
          }
    ...
    const recommendationEl = body.querySelector('#recommendation');
    ...
    function updateSaveButtonState() {
      let hasUnsaved = Array.from(languagesList.querySelectorAll('.translation-edit-field')).some(
        function (t) {
      ...
      if (!hasUnsaved && isRichText) {
        hasUnsaved = Object.values(editorInstances).some(function (editor) {
          return (
            editor &&
            editor.originalValue !== undefined &&
            editor.getData() !== editor.originalValue
          );
        });
      }
    ...
    selectAllBtn.addEventListener('click', function () {
      languagesList.querySelectorAll('input[type="checkbox"]').forEach(function (cb) {
        cb.checked = true;
      });
    });
      ...
      if (isRichText) {
        Object.entries(editorInstances).forEach(function (entry) {
          ...
          if (editor && editor.originalValue !== undefined) {
            const newValue = editor.getData().trim();
            if (newValue && newValue !== (editor.originalValue || '')) {
              editedTranslations[lang] = newValue;
              hasEdits = true;
            }
          }
          ...
          result.saved_languages.forEach(function (lang) {
            ...
            if (textarea) {
              textarea.setAttribute(
                'data-original-value',
                textarea.value.replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
              );
              existingTranslations[lang] = textarea.value;
            }
            if (isRichText && editorInstances[lang]) {
              const currentData = editorInstances[lang].getData();
              editorInstances[lang].originalValue = currentData;
              existingTranslations[lang] = currentData;
            }
          });
      ...
      try {
        ...
        if (response.status === 202 && result.recommend_schedule) {
          recommendationEl.innerHTML =
            ...
            '</em><div style="display: flex; gap: 8px; margin-top: 8px;"><button data-action="translate-anyway" style="padding: 6px 12px; background: var(--button-bg); color: var(--button-fg); border: none; border-radius: 4px; cursor: pointer;"><i class="fas fa-bolt"></i> Translate Anyway</button><button data-action="schedule-translation" style="padding: 6px 12px; background: var(--primary); color: #fff; border: none; border-radius: 4px; cursor: pointer;"><i class="fas fa-calendar-plus"></i> Schedule for Later</button></div></div>';
          recommendationEl
            .querySelector('[data-action="translate-anyway"]')
            .addEventListener('click', function () {
              const m = this.closest('.utility-translation-modal');
              if (m) m.querySelector('.btn-apply').click();
            });
          const schedBtn = recommendationEl.querySelector('[data-action="schedule-translation"]');
          if (schedBtn) {
            schedBtn.addEventListener('click', async function () {
              ...
              try {
                const csrfToken = AdminUtils.getCsrfToken();
                const schedResponse = await fetch(endpoints.translate, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrfToken },
                  body: JSON.stringify({
                    text: currentValue,
                ...
                if (schedResult.success && schedResult.scheduled) {
                  recommendationEl.innerHTML =
                    '<div style="padding: 12px; background: var(--success-bg, #d4edda); border-left: 3px solid var(--success-color, #28a745); color: var(--body-fg); border-radius: 4px;"><strong><i class="fas fa-check-circle"></i> Translations Scheduled</strong><br>' +
                    schedResult.job_count +
                    ' translation job(s) queued for background processing. Translations will appear automatically once completed.</div>';
                  applyBtn.disabled = false;
                  applyBtn.innerHTML = '<i class="fas fa-language"></i> Translate';
                  applyBtn.dataset.forceImmediate = 'false';
                } else {
                  throw new Error(schedResult.error || 'Scheduling failed');
                }
              } catch (err) {
                console.error('Schedule error:', err);
                recommendationEl.innerHTML +=
                  '<div style="padding: 8px; background: var(--error-bg, #f8d7da); color: var(--error-fg, #842029); border-radius: 4px; margin-top: 8px;">Scheduling failed: ' +
                  err.message +
                  '</div>';
                schedBtn.disabled = false;
                schedBtn.innerHTML = '<i class="fas fa-calendar-plus"></i> Schedule for Later';
              }
            });
          }
          applyBtn.disabled = false;
        ...
        if (result.success) {
          ...
          availableLanguages.forEach(function (lang) {
            ...
            langItem.innerHTML =
              '<div style="display: flex; align-items: center; gap: 12px;"><input type="checkbox" id="lang_' +
              lang.code +
              '" value="' +
              lang.code +
              '" ' +
              (isTranslated ? '' : 'checked') +
              ' style="width: 18px; height: 18px; flex-shrink: 0;"><label for="lang_' +
              lang.code +
              '" style="flex: 1; margin: 0; color: var(--body-fg);"><strong>' +
              lang.name +
              '</strong>' +
          ...
          if (isRichText && typeof ClassicEditor !== 'undefined' && ckeditorEditorConfig) {
            availableLanguages.forEach(function (lang) {
              const translation = allTranslations[lang.code];
              if (translation) {
                const editorEl = document.getElementById('editor_new_' + lang.code);
                if (editorEl && !editorInstances[lang.code]) {
                  ClassicEditor.create(editorEl, ckeditorEditorConfig)
                    .then(function (editor) {
                      editorInstances[lang.code] = editor;
                      const unescapedContent = translation
                        .replace(/&quot;/g, '"')
                        .replace(/&lt;/g, '<')
                        .replace(/&gt;/g, '>');
                      editor.setData(unescapedContent);
  ...
  function showAdminMessage(message, type) {
    ...
    const contentDiv = document.querySelector('.content') || document.querySelector('main');
    if (contentDiv) {
