...
(function () {
  ...
  const BADGE_API_URL = '/api/core/badges/';
  const INITIAL_DELAY = 5000; // 5 seconds after page load
  ...
  function updateBadges(badges) {
    document.querySelectorAll('[data-badge-key]').forEach(function (menuItem) {
      const key = menuItem.getAttribute('data-badge-key');
      const count = badges[key];
      let badgeEl = menuItem.querySelector('.menu-badge:not([data-rollup])');

      if (count && count > 0) {
        if (!badgeEl) {
          badgeEl = document.createElement('span');
          const severity = BADGE_SEVERITY[key] || 'primary';
