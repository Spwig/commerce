...
const HelpSystem = {
    ...
    async submitFeedback(helpful) {
        ...
        try {
            const response = await fetch(this.apiUrl(`/help/topics/${this.currentTopic.slug}/feedback/`), {
                method: 'POST',
                headers: this.getHeaders(),
                credentials: 'same-origin',
                body: JSON.stringify({
    ...
    renderCategories() {
        ...
        if (this.categories.length === 0) {
            container.innerHTML = '<p style="text-align:center;color:var(--text-muted);">No categories available</p>';
            return;
        }
    ...
    showLoading(containerId) {
        const container = document.getElementById(containerId);
        if (container) {
            container.innerHTML = '<div class="help-loading"><i class="fas fa-spinner fa-spin"></i> Loading...</div>';
        }
    },
    ...
    showNoResults(containerId) {
        const container = document.getElementById(containerId);
        if (container) {
            container.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:2rem 1rem;">No results found. Try different keywords.</p>';
        }
    },
    ...
    showFeedbackSuccess() {
        ...
        if (feedbackSection) {
            feedbackSection.innerHTML = '<p style="color:#28a745;text-align:center;padding:1rem;">Thank you for your feedback!</p>';
        }
    },
    ...
    renderMarkdown(markdown) {
        ...
        html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="help-img" loading="lazy">');
        ...
        // Paragraphs
        html = html.split('\n\n').map(para => {
            if (para.startsWith('<h') || para.startsWith('<ul') || para.startsWith('<pre') || para.startsWith('<table')) {
                return para;
            }
            return `<p>${para}</p>`;
        }).join('\n');
