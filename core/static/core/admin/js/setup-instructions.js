...
(function () {
    ...
    document.addEventListener('click', function (e) {
        var b = e.target.closest('[data-action="copy-to-clipboard"][data-copy-id]');
        if (b) {
            var el = document.getElementById(b.dataset.copyId);
            var text = el ? el.textContent.trim() : '';
            navigator.clipboard.writeText(text).then(function () {
                var orig = b.textContent; b.textContent = 'Copied!'; b.classList.add('success');
                setTimeout(function () { b.textContent = orig; b.classList.remove('success'); }, 2000);
            });
            return;
        }
        ...
        if (b) {
            ...
            var container = b.closest('.provider-tabs, .tabs, .registrar-tabs');
            if (container) {
                var section = container.parentElement || document;
                section.querySelectorAll('.provider-content, .tab-content').forEach(function (el) { el.classList.remove('active'); });
                container.querySelectorAll('[data-action="switch-tab"]').forEach(function (el) { el.classList.remove('active'); });
            } else {
                document.querySelectorAll('.provider-content, .tab-content').forEach(function (el) { el.classList.remove('active'); });
                document.querySelectorAll('[data-action="switch-tab"]').forEach(function (el) { el.classList.remove('active'); });
            }
            var content = document.getElementById(tab + '-content') || document.getElementById(tab + '-tab') || document.getElementById(tab);
