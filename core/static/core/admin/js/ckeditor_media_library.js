...
(function() {
    ...
    function openImageLibrary(editorInstance) {
        if (typeof window.selectImageFromLibrary !== 'function') {
            AdminModal.alert({message: 'Media library is not available. Please reload the page.', type: 'error'});
            return;
        }
    ...
    function openVideoLibrary(editorInstance) {
        ...
        window.selectMediaFromLibrary(function(media) {
            if (!media || !media.url) return;

            var mime = media.mime_type || 'video/mp4';
            var videoHtml = '<figure class="media-video">' +
                '<video controls preload="metadata">' +
                '<source src="' + media.url + '" type="' + mime + '">' +
                '</video>' +
                '</figure>';
    ...
    async function insertExternalVideo(editorInstance) {
        var url = await AdminModal.prompt('Enter a video URL (YouTube, Vimeo, etc.):');
    ...
    function showEmbedNotice(editorInstance) {
        var editable = editorInstance.ui.getEditableElement();
        if (!editable) return;
        var ckEditor = editable.closest('.ck-editor');
        if (!ckEditor) return;

        // Don't stack multiple notices
        var existing = ckEditor.parentNode.querySelector('.ck-embed-notice');
    ...
    function initCKEditorMediaIntegration() {
        var checkInterval = setInterval(function() {
            ...
            editables.forEach(function(editable) {
                var editorInstance = editable.ckeditorInstance;
                ...
                var isRichEditor = !!ckEditor.querySelector('.ck-file-dialog-button') ||
                ...
                var hasMediaEmbed = editorInstance.plugins.has('MediaEmbed');
    ...
    function replaceImageButton(ckEditor, editorInstance) {
        var fileDialogBtn = ckEditor.querySelector('.ck-file-dialog-button');
        if (!fileDialogBtn) return;

        // Clone the button to strip all CKEditor event listeners
        var newBtn = fileDialogBtn.cloneNode(true);
        ...
        var fileInput = fileDialogBtn.parentElement.querySelector('input[type="file"]');
    ...
    function showVideoMenu(anchorBtn, editorInstance, hasMediaEmbed) {
        ...
        var existing = document.querySelector('.ck-video-menu');
        if (existing) {
            existing.remove();
            return;
        }
        ...
        var libOption = document.createElement('button');
        ...
        if (hasMediaEmbed) {
            var extOption = document.createElement('button');
