...
(function() {
    ...
    // =========================================================================
    var consoleBuffer = window.__spwig_console_buffer || [];
    var navBreadcrumbs = window.__spwig_nav_breadcrumbs || [];
    ...
    function showStep(step) {
        ...
        // Hide all steps
        els.steps.forEach(function(s) { if (s) s.setAttribute('data-visible', 'false'); });
    ...
    function initStep1Listeners() {
        ...
        if (els.categoryGrid) {
            els.categoryGrid.addEventListener('click', function(e) {
                var card = e.target.closest('.bug-category-card');
                if (!card) return;
                // Deselect all
        ...
        if (els.severityOptions) {
            els.severityOptions.addEventListener('click', function(e) {
                var card = e.target.closest('.bug-severity-card');
                if (!card) return;
    ...
    function shakeElement(el) {
        ...
        setTimeout(function() { el.classList.remove('bug-shake'); }, 500);
    }
    ...
    function initStep2Listeners() {
        ...
        ['consentConsole', 'consentPageUrl', 'consentBrowser', 'consentNavigation'].forEach(function(id) {
    ...
    function updateConsentPreviews() {
        ...
        var consoleContent = document.getElementById('consolePreviewContent');
        if (consoleContent) {
        ...
        // Page URL preview
        var pageUrlContent = document.getElementById('pageUrlPreviewContent');
        if (pageUrlContent) {
            var breadcrumbs = document.querySelector('.breadcrumbs');
            pageUrlContent.textContent =
                'URL: ' + window.location.href + '\n' +
                'Section: ' + (breadcrumbs ? breadcrumbs.textContent.trim() : '(unknown)');
        }

        // Browser info preview
        var browserContent = document.getElementById('browserPreviewContent');
        if (browserContent) {
        ...
        // Navigation preview
        var navContent = document.getElementById('navPreviewContent');
        if (navContent) {
    ...
    function collectBrowserData() {
        var data = {};
        var flags = formData.consentFlags;
    ...
    function captureContactInfo() {
        var nameEl = document.getElementById('bugContactName');
        var emailEl = document.getElementById('bugContactEmail');
        var consentEl = document.getElementById('bugContactConsent');
    ...
    function buildReview() {
        ...
        html += '<button type="button" class="bug-review-edit" data-goto="1"><i class="fas fa-pencil-alt"></i> Edit</button>';
        html += '</div>';
        html += '<div class="bug-review-field"><span class="bug-review-label">Category:</span> <span class="bug-review-value">' + escapeHtml(categoryLabels[formData.category] || formData.category) + '</span></div>';
        html += '<div class="bug-review-field"><span class="bug-review-label">Severity:</span> <span class="bug-review-value bug-severity-badge bug-severity-' + escapeHtml(formData.severity) + '">' + escapeHtml(severityLabels[formData.severity] || formData.severity) + '</span></div>';
        html += '<div class="bug-review-field"><span class="bug-review-label">Description:</span></div>';
        html += '<div class="bug-review-description">' + escapeHtml(formData.description) + '</div>';
        html += '</div>';

        // Browser data section
        var browserKeys = Object.keys(formData.browserData);
        html += '<div class="bug-review-section">';
        html += '<div class="bug-review-section-header">';
        html += '<h4><i class="fas fa-laptop"></i> Browser Data</h4>';
        html += '<button type="button" class="bug-review-edit" data-goto="2"><i class="fas fa-pencil-alt"></i> Edit</button>';
        html += '</div>';
        if (browserKeys.length === 0) {
            ...
            browserKeys.forEach(function(key) {
                var val = formData.browserData[key];
                var displayVal;
                ...
                html += '<div class="bug-review-field"><span class="bug-review-label">' + escapeHtml(key.replace(/_/g, ' ')) + ':</span> <span class="bug-review-value">' + displayVal + '</span></div>';
            });
        }
        html += '</div>';
        ...
        html += '<button type="button" class="bug-review-edit" data-goto="3"><i class="fas fa-pencil-alt"></i> Edit</button>';
        html += '</div>';
        if (!formData.contactName && !formData.contactEmail) {
            html += '<div class="bug-review-field"><em>Submitting anonymously</em></div>';
        } else {
            if (formData.contactName) {
                html += '<div class="bug-review-field"><span class="bug-review-label">Name:</span> <span class="bug-review-value">' + escapeHtml(formData.contactName) + '</span></div>';
            }
            if (formData.contactEmail) {
                html += '<div class="bug-review-field"><span class="bug-review-label">Email:</span> <span class="bug-review-value">' + escapeHtml(formData.contactEmail) + '</span></div>';
            }
            html += '<div class="bug-review-field"><span class="bug-review-label">OK to contact:</span> <span class="bug-review-value">' + (formData.contactConsent ? 'Yes' : 'No') + '</span></div>';
        }
        html += '</div>';
    ...
    function openWizard() {
        ...
        var helpDrawer = document.getElementById('helpDrawer');
        var helpOverlay = document.getElementById('helpDrawerOverlay');
    ...
    function resetWizard() {
        ...
        var consentIds = ['consentConsole', 'consentPageUrl', 'consentBrowser'];
        consentIds.forEach(function(id) {
            var el = document.getElementById(id);
            if (el) el.checked = true;
        });
        var navConsent = document.getElementById('consentNavigation');
        if (navConsent) navConsent.checked = false;

        // Reset contact fields
        var nameEl = document.getElementById('bugContactName');
        var emailEl = document.getElementById('bugContactEmail');
        var contactConsentEl = document.getElementById('bugContactConsent');
    ...
    // =========================================================================
    var drag = { active: false, startX: 0, startY: 0, modalX: 0, modalY: 0 };

    function initDrag() {
        getEls();
        if (!els.overlay) return;

        var header = els.overlay.querySelector('.admin-modal-header');
        var modal = els.overlay.querySelector('.admin-modal');
