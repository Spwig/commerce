...
class AdminLinkSelector {
    ...
    update() {
        var selected = this.linkTypeField.value;
        var hasLink = selected && selected !== 'none';
        var isEntityType = hasLink && selected !== 'custom_url' && this.fieldMap[selected];

        // Show/hide custom URL container
        var customEl = this.customUrlContainerId ? document.getElementById(this.customUrlContainerId) : null;
        if (customEl) {
        ...
        var opts = this.linkOptionsSelector ? document.querySelector(this.linkOptionsSelector) : null;
        if (opts) {
            if (hasLink) {
        ...
        if (hasLink) {
            for (var type in this.fieldMap) {
                if (type !== selected) {
                    var input = document.getElementById(this.fieldMap[type].inputId);
    ...
    renderSearch(type) {
        ...
        var hiddenInput = document.getElementById(fc.inputId);
    ...
    doSearch(fc, query, resultsDiv, searchDiv, previewDiv) {
        var self = this;
        resultsDiv.innerHTML = '<div class="als-loading"><i class="fas fa-spinner"></i> Searching...</div>';
        resultsDiv.style.display = 'block';

        fetch(this.apiUrl + '?type=' + encodeURIComponent(fc.apiType) + '&search=' + encodeURIComponent(query) + '&limit=10')
            .then(function(r) { return r.json(); })
            .then(function(data) {
                var results = data[fc.resultKey] || [];
                if (results.length === 0) {
    ...
    renderResults(items, fc, resultsDiv, searchDiv, previewDiv) {
        ...
        items.forEach(function(item) {
            ...
            row.innerHTML =
                '<div class="als-result-thumb">' + thumbHtml + '</div>' +
                '<div class="als-result-info">' +
                    '<div class="als-result-title">' + self._escapeHtml(item.title || item.name) + '</div>' +
                    '<div class="als-result-url">' + self._escapeHtml(item.url || '') + '</div>' +
                '</div>';
        ...
        previewDiv.innerHTML =
            '<div class="als-preview-content">' +
                '<div class="als-preview-info">' +
                    '<span class="als-preview-type">' + self._escapeHtml(typeLabels[self._currentType] || self._currentType) + '</span>' +
                    '<span class="als-preview-title">' + self._escapeHtml(item.title || item.name || '') + '</span>' +
                    '<span class="als-preview-url">' + self._escapeHtml(item.url || '') + '</span>' +
                '</div>' +
    ...
    loadInitialData() {
        ...
        var type = this.initialData.type;
        var fc = this.fieldMap[type];
        if (!fc) return;
        ...
        var searchDiv = this.searchContainer ? this.searchContainer.querySelector('.als-search') : null;
        var previewDiv = this.searchContainer ? this.searchContainer.querySelector('.als-preview') : null;
...
class LinkSelector {
    ...
    update() {
        ...
        if (selected && selected !== 'none' && this.fieldMap[selected]) {
            var active = document.querySelector(this.fieldMap[selected]);
