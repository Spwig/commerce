...
(function () {
  ...
  function init() {
    const configEl = document.getElementById('license-status-config');
    if (configEl) {
      try {
    ...
    document.addEventListener('click', function (e) {
      let btn = e.target.closest('[data-action="clear-license-cache"]');
      if (btn) {
        clearLicenseCache();
        return;
      }
      ...
      if (btn) {
        const targetId = btn.dataset.target || 'license-file-input';
  ...
  function activateLicenseByKey() {
    ...
    const environmentType = document.getElementById('environment-type-select').value;
    ...
    if (!licenseKey) {
      ...
      resultDiv.innerHTML =
        '<strong>\u2717 ' +
        (config.i18n.enterLicenseKey || 'Please enter a license key') +
        '</strong>';
      return;
    }
    ...
    resultDiv.innerHTML =
      '\u23F3 ' + (config.i18n.activatingLicense || 'Activating license... Please wait');
    ...
    fetch(config.urls.activate, {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          resultDiv.innerHTML =
            ...
            data.message +
            '</strong><br>' +
            (config.i18n.owner || 'Owner') +
            ': ' +
            data.owner_name +
            '<br>' +
            (config.i18n.type || 'Type') +
            ': ' +
            data.license_type +
            '<br>' +
            (config.i18n.environment || 'Environment') +
            ': ' +
            data.environment_type +
            '<br>' +
            (config.i18n.installations || 'Installations') +
            ': ' +
            data.active_installations +
            '/' +
            data.max_installations +
            '<br><br>' +
            '<em>' +
            (config.i18n.pageWillReload || 'Page will reload automatically...') +
            '</em>';
          setTimeout(function () {
          ...
          resultDiv.innerHTML =
            '<strong>\u2717 ' +
            (config.i18n.activationFailed || 'Activation Failed') +
            '</strong><br>' +
            errorMsg;
        }
      })
      .catch(function (error) {
        ...
        resultDiv.innerHTML =
          '<strong>\u2717 ' +
          (config.i18n.connectionError || 'Connection Error') +
          '</strong><br>' +
          error.message;
      });
  }
  ...
  function handleFileUpload(input) {
    ...
    resultDiv.style.color = 'var(--body-fg)';
    resultDiv.innerHTML =
      '\u23F3 ' + (config.i18n.uploadingLicense || 'Uploading and validating license...');
      ...
      .then(function (data) {
        if (data.success) {
          ...
          resultDiv.innerHTML =
            ...
            data.message +
            '</strong><br>' +
            (config.i18n.licenseKey || 'License Key') +
            ': ' +
            data.license_key +
            '<br>' +
            (config.i18n.type || 'Type') +
          ...
          resultDiv.innerHTML =
            '<strong>\u2717 ' +
            (config.i18n.uploadFailed || 'Upload Failed') +
            '</strong><br>' +
            data.error;
        }
      })
    ...
    fetch(config.urls.refreshLicense, {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          AdminModal.alert({
            message: (config.i18n.refreshFailed || 'License Refresh Failed') + ': ' + data.error,
            type: 'error',
          });
          btn.disabled = false;
      ...
      .catch(function (error) {
        AdminModal.alert({
          message: (config.i18n.error || 'Error') + ': ' + error.message,
          type: 'error',
        });
        btn.disabled = false;
        btn.textContent = originalText;
  ...
  async function clearLicenseCache() {
    if (
      !(await AdminModal.confirm({
        message:
          config.i18n.confirmClearCache || 'Are you sure you want to clear the license cache?',
        danger: true,
        confirmText: 'Clear',
      }))
    ) {
    ...
    fetch(config.urls.clearCache, {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          AdminModal.alert({
            message: (config.i18n.error || 'Error') + ': ' + data.error,
            type: 'error',
          });
        }
      })
  ...
  function activatePOSLicense() {
    const keyInput = document.getElementById('pos-license-key-input');
    const resultDiv = document.getElementById('pos-activation-result');
    ...
    if (!/^POS-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(licenseKey)) {
      ...
      resultDiv.textContent =
        config.i18n.invalidPosFormat || 'Invalid format. Expected: POS-XXXX-XXXX-XXXX-XXXX';
      return;
    }
    ...
    fetch(config.urls.activatePos, {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          resultDiv.className = 'pos-activation-result error';
          resultDiv.textContent =
            data.message || config.i18n.activationFailed || 'Activation failed.';
        }
      })
      .catch(function (err) {
        resultDiv.className = 'pos-activation-result error';
