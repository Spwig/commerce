  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function addCharacterCount(fieldId, maxLength) {
      ...
      if (field.length) {
        ...
        function updateCount() {
          ...
          counter.html(
            `<span style="color: ${color}">${field.val().length}/${maxLength} characters</span>`
          );
        }
      ...
      if (field.length) {
        ...
        field.on('blur', function () {
          ...
          if (email && !validateEmail(email)) {
            validationDiv.html(
              '<span style="color: #dc3545;">⚠ Please enter a valid email address</span>'
            );
          } else {
            validationDiv.empty();
          }
      ...
      if (field.length) {
        ...
        field.on('blur', function () {
          ...
          if (url && !validateURL(url)) {
            validationDiv.html(
              '<span style="color: #dc3545;">⚠ Please enter a valid URL (including http:// or https://)</span>'
            );
          } else {
            validationDiv.empty();
          }
    ...
    if (maintenanceField.length) {
      maintenanceField.on('change', function () {
        if (this.checked) {
          const warning = $(
            '<div class="maintenance-warning" style="background: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; margin: 10px 0; border-radius: 4px;"><strong>⚠ Warning:</strong> Enabling maintenance mode will make your site inaccessible to visitors. Only staff members will be able to access the site.</div>'
          );
    ...
    if (adminEmailField.length && supportEmailField.length) {
      adminEmailField.on('blur', async function () {
        if (!supportEmailField.val().trim() && validateEmail(adminEmailField.val())) {
          if (
            await AdminModal.confirm('Would you like to use the same email for customer support?')
          ) {
    ...
    if (currencyField.length) {
      const currencyHelp = $(
        '<div class="field-help" style="font-size: 11px; color: #666; margin-top: 5px;">This will be the default currency for all product prices. You can still set individual product prices in different currencies.</div>'
      );
    ...
    if (timezoneField.length) {
      const timezoneHelp = $(
        '<div class="field-help" style="font-size: 11px; color: #666; margin-top: 5px;">This affects order timestamps, scheduled tasks, and email notifications.</div>'
      );
    ...
    $('form').on('submit', function (e) {
        ...
        if (email && !validateEmail(email)) {
          ...
          AdminModal.alert({
            message:
              ...
              $(this).closest('.form-row').find('label').text(),
            type: 'error',
          });
          return false;
        }
      ...
      $('.required input[type="url"]').each(function () {
        ...
        if (url && !validateURL(url)) {
          ...
          $(this).focus();
          AdminModal.alert({
            message: 'Please correct the URL: ' + $(this).closest('.form-row').find('label').text(),
            type: 'error',
          });
          return false;
        }
    ...
    if (maintenanceField.length && maintenanceField.prop('checked')) {
      const warning = $(
        '<div class="maintenance-active" style="background: #f8d7da; border: 1px solid #f5c6cb; color: #721c24; padding: 15px; margin: 15px 0; border-radius: 4px; font-weight: bold;"><strong>🚧 Maintenance Mode is Currently Active</strong><br>Your site is in maintenance mode. Visitors will see a maintenance page.</div>'
      );
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const dataEl = document.getElementById('site-settings-license-data');
    if (!dataEl) {
      return;
    }
    ...
    fetch(apiUrl)
      ...
      .then(function (d) {
        ...
        if (d.is_valid) {
          ...
          if (featuresEl) {
            ...
            setFeature(
              'feature-payment',
              d.features.payment_processing,
              t.paymentProcessing || 'Payment Processing',
              t
            );
            setFeature(
              'feature-support',
              d.features.priority_support,
      ...
      .catch(function () {
        const badge = document.getElementById('license-badge');
        if (badge) {
          badge.textContent = '? ' + (t.unknown || 'Unknown');
        }
      });
  });
