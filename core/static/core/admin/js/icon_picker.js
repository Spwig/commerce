...
(function() {
    ...
    class IconPicker {
        ...
        _bindEvents() {
            ...
            if (this.priorityGrid) {
                this.priorityGrid.addEventListener('click', function(e) {
                    var item = e.target.closest('.icon-picker-grid-item');
        ...
        _updatePrioritySelection(iconClass) {
            ...
            var items = this.priorityGrid.querySelectorAll('.icon-picker-grid-item');
        ...
        _dispatchChange(value, iconClass, iconStyle, iconLabel) {
            ...
            this.root.dispatchEvent(new CustomEvent('icon-picker:change', {
                bubbles: true,
                detail: {
                    value: value,
                    iconClass: iconClass,
        ...
        _buildModal() {
            ...
            title.innerHTML = '<i class="fas fa-icons"></i> ' +
                (trans.chooseIcon || 'Choose an Icon');
        ...
        _renderModalIcons(searchTerm, category) {
            ...
            // Update count
            var countWord = trans.iconCount || 'icons';
        ...
        _createModalIconBtn(icon) {
            ...
            // Check if selected
            var fullValue = icon.style + ' ' + icon['class'];
        ...
        _updateModalSelection() {
            ...
            for (var i = 0; i < items.length; i++) {
                var fullVal = items[i].dataset.iconStyle + ' ' + items[i].dataset.iconClass;
                var bareVal = items[i].dataset.iconClass;
                var isSelected = (
                    this.selectedValue === fullVal || this.selectedValue === bareVal
                );
    ...
    function initAll() {
        var widgets = document.querySelectorAll('[data-icon-picker]');
