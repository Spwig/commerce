    ...
    var AdminTabs = {
        ...
        init: function(options) {
            ...
            if (!restored && !this._activeTab) {
                ...
                var activeBtn = this._container.querySelector('.admin-tab-btn.active');
                if (activeBtn) {
        ...
        switchTo: function(tabName) {
            ...
            var button = this._container.querySelector(
                '.admin-tab-btn[data-tab="' + tabName + '"]'
            );
            var panel = this._findPanel(tabName);
        ...
        addErrorBadge: function(tabName, count) {
            ...
            var btn = this._container.querySelector(
                '.admin-tab-btn[data-tab="' + tabName + '"]'
            );
            if (!btn) return;

            // Remove existing badge
            var existing = btn.querySelector('.error-badge');
        ...
        clearErrorBadges: function() {
            ...
            var badges = this._container.querySelectorAll('.error-badge');
        ...
        scanForErrors: function() {
            ...
            this._buttons.forEach(function(btn) {
                ...
                var panel = self._findPanel(tabName);
                if (!panel) return;

                var errors = panel.querySelectorAll(
                    '.errorlist li, .errors, p.error'
                );
                if (errors.length > 0) {
        ...
        _detectPattern: function() {
            var firstBtn = this._buttons[0];
            if (!firstBtn) return null;
            var testName = firstBtn.getAttribute('data-tab');
            ...
            if (document.querySelector(
                '.admin-tab-content[data-tab="' + testName + '"]'
            )) {
            ...
            var idTabEl = document.getElementById('tab-' + testName);
        ...
        _getAllPanels: function() {
            var self = this;
            return this._buttons.map(function(btn) {
        ...
        _deriveStorageKey: function() {
            ...
            var adminIdx = parts.indexOf('admin');
            if (adminIdx >= 0 && parts.length > adminIdx + 2) {
                var app = parts[adminIdx + 1];
                var model = parts[adminIdx + 2];
                var nextPart = parts[adminIdx + 3];
                var id = (nextPart && /^\d+$/.test(nextPart)) ? nextPart : 'new';
