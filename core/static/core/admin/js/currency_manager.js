...
class CurrencyManager {
  ...
  async loadCurrencies() {
    try {
      ...
      console.log(
        `Available: ${this.availableCurrencies.length}, Active: ${this.activeCurrencies.length}`
      );
    } catch (error) {
  ...
  renderCurrencies() {
    ...
    if (this.availableCurrencies.length === 0) {
      this.availableContainer.innerHTML =
        '<div class="empty-state"><i class="fas fa-check-circle"></i><p>All currencies are active</p></div>';
    } else {
    ...
    if (this.activeCurrencies.length === 0) {
      this.activeContainer.innerHTML =
        '<div class="empty-state"><i class="fas fa-info-circle"></i><p>No active currencies. Drag currencies from the left to activate them.</p></div>';
    } else {
  ...
  createCurrencyItem(currency, isActive) {
    ...
    if (isActive) {
      if (currency.protected) {
        ...
        html +=
          '<button type="button" class="remove-btn" title="Deactivate"><i class="fas fa-times"></i></button>';
      }
    }
  ...
  deactivateCurrency(code) {
    ...
    if (index !== -1) {
      ...
      if (currency.protected) {
        this.showError(
          `Cannot deactivate ${code} - it is the default currency. Change the default currency in the Locale tab first.`
        );
        return;
      }
  ...
  showMessage(message, type) {
    ...
    if (!messagesDiv) {
      ...
      document
        ...
        .insertBefore(messagesDiv, document.querySelector('.content').firstChild);
    }
