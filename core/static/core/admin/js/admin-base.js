...
class SearchableSelect {
  constructor(selectElement, options = {}) {
    this.selectElement =
      typeof selectElement === 'string' ? document.querySelector(selectElement) : selectElement;
    ...
    this.options = {
      placeholder:
        options.placeholder || selectElement.getAttribute('data-placeholder') || 'Search...',
  ...
  renderOptions(searchTerm = '') {
    ...
    this.allOptions.forEach(group => {
      const filteredOptions = group.options.filter(
        opt => opt.text.toLowerCase().includes(term) || opt.value.toLowerCase().includes(term)
      );
...
class SlugGenerator {
  constructor(slugFieldSelector, sourceFieldSelector = null) {
    this.slugField =
      typeof slugFieldSelector === 'string'
    ...
    if (!sourceFieldSelector) {
      ...
      this.sourceField =
        typeof sourceFieldSelector === 'string'
...
function initializeBadgeRollup() {
  document.querySelectorAll('.menu-item-group').forEach(function (group) {
      ...
      childBadges.forEach(function (badge) {
        ...
        for (let i = 0; i < BADGE_SEVERITY_ORDER.length; i++) {
          if (badge.classList.contains('menu-badge-' + BADGE_SEVERITY_ORDER[i])) {
            if (
              !highestSeverity ||
              BADGE_SEVERITY_ORDER.indexOf(BADGE_SEVERITY_ORDER[i]) <
                BADGE_SEVERITY_ORDER.indexOf(highestSeverity)
            ) {
    ...
    const observer = new MutationObserver(updateRollup);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('img[data-img-fallback-src]').forEach(function (img) {
    const fallbackSrc = img.dataset.imgFallbackSrc;
