...
(function($) {
    ...
    function discoverFieldTabMappingFromDOM() {
        $('.tab-panel').each(function() {
            ...
            $(this).find('.form-row[class*="field-"]').each(function() {
    ...
    function populateErrorTabIndicators() {
        $('.error-item[data-field]').each(function() {
            ...
            if (tabName) {
                $(this).find('.error-tab-indicator').text(' (' + tabName + ' ' + gettext('tab') + ')');
            }
        });
    }
        ...
        counters.forEach(function(counter) {
            ...
            if ($field.length && $counter.length) {
                function updateCounter() {
                    ...
                    const text = interpolate(gettext('%(count)s/%(max)s characters'), {
                        count: length,
    ...
    function initializeMaintenanceWarning() {
        ...
        if ($checkbox.length) {
            $checkbox.on('change', async function() {
                if ($(this).is(':checked')) {
                    const message = gettext('Warning: Enabling maintenance mode will make your site inaccessible to customers. Are you sure?');
    ...
    function initializeMultiCurrencyToggle() {
        ...
        $('#multi-currency-help-link').on('click', function(e) {
            e.preventDefault();
            var helpBtn = document.querySelector('[data-action="toggle-help"]');
