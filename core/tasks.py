...
def regenerate_thumbnails_task(self):
    ...
    product_image_assets = MediaAsset.objects.filter(product_uses__isnull=False).distinct()
    ...
    try:
        for asset in product_image_assets:
            try:
                ...
                self.update_state(
                    state="PROGRESS",
                    meta={"processed": processed, "total": total, "progress": progress},
                )
        ...
        cache.set(
            "thumbnail_last_regeneration",
            {"timestamp": completion_time.isoformat(), "processed": processed, "total": total},
            timeout=2592000,
        )  # 30 days

        return {"status": "complete", "processed": processed, "total": total}
...
def cleanup_expired_trusted_devices():
    ...
    try:
        ...
        old_revoked = TrustedDevice.objects.filter(is_revoked=True, created_at__lt=revoked_cutoff)
...
def load_help_embeddings_async(
    self, fixture_path, force=False, batch_size=200, throttle_seconds=0.1
):
    """
    ...
    try:
        ...
        if (
            ...
            and current.get("task_id") != self.request.id
        ):
