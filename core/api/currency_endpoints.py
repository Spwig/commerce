...
def list_all_currencies(request):
    ...
    try:
        ...
        active_currencies = {curr.code: curr for curr in SupportedCurrency.objects.all()}
...
def list_active_currencies(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "currencies": currencies, "count": len(currencies)})
...
def activate_currencies(request):
    ...
    try:
        ...
        if not codes:
            return JsonResponse(
                {"success": False, "error": "No currency codes provided"}, status=400
            )
...
def reorder_currencies(request):
    ...
    try:
        ...
        if not isinstance(codes, list):
            return JsonResponse({"success": False, "error": "codes must be an array"}, status=400)
        ...
        return JsonResponse(
            {"success": True, "updated": updated, "message": "Currency order updated"}
        )
...
def update_currency_settings(request, code):
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": f"Currency {code} not found"}, status=404)
    except json.JSONDecodeError:
        return JsonResponse({"success": False, "error": "Invalid JSON data"}, status=400)
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
...
def bulk_update_currencies(request):
    ...
    try:
        ...
        if not isinstance(updates, list):
            return JsonResponse({"success": False, "error": "updates must be an array"}, status=400)
        ...
        with transaction.atomic():
            for update in updates:
                ...
                try:
                    currency, created = SupportedCurrency.objects.get_or_create(
                        code=code, defaults={"is_active": update.get("is_active", False)}
                    )
        ...
        return JsonResponse({"success": len(errors) == 0, "updated": updated, "errors": errors})
