...
class MessageSubmitSerializer(serializers.Serializer):
    ...
    message_type = serializers.ChoiceField(
        choices=["general", "support", "order", "product", "other"], default="general"
    )
...
@extend_schema(
    ...
    description=_(
        "Submit a new message through the contact form. Rate limited to 5/hour for anonymous users."
    ),
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "status", str, description=_("Filter by status: unread, read, replied, archived")
            ),
            OpenApiParameter(
                "type",
                str,
                description=_("Filter by type: general, support, order, product, other"),
            ),
        ],
