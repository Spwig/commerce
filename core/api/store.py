...
@extend_schema(
    ...
    description=_(
        "Get all public store information including basic info, contact details, address, and social links. Cached for 5 minutes."
    ),
    responses={
...
def get_store_info(request):
    ...
    if not settings:
        return Response(
            {"success": False, "message": _("Store settings not configured")},
    ...
    data = {
        "basic": {
            ...
            "favicon": request.build_absolute_uri(settings.get_favicon_url())
            if settings.get_favicon_url()
            else None,
        },
        "contact": {
...
def get_store_social(request):
    ...
    if settings.facebook_url:
        social_links.append(
            {"platform": "facebook", "url": settings.facebook_url, "icon": "fab fa-facebook-f"}
        )

    if settings.twitter_url:
        social_links.append(
            {"platform": "twitter", "url": settings.twitter_url, "icon": "fab fa-twitter"}
        )

    if settings.instagram_url:
        social_links.append(
            {"platform": "instagram", "url": settings.instagram_url, "icon": "fab fa-instagram"}
        )

    if settings.linkedin_url:
        social_links.append(
            {"platform": "linkedin", "url": settings.linkedin_url, "icon": "fab fa-linkedin-in"}
        )

    return Response(
        {"success": True, "data": {"links": social_links, "has_social": len(social_links) > 0}}
    )
...
def get_store_payment_methods(request):
    ...
    try:
        ...
        return Response(
            {
                "success": True,
                "data": {"methods": payment_methods, "has_payments": len(payment_methods) > 0},
            }
        )

    except Exception:
        # If payment_providers app not configured, return empty
        return Response({"success": True, "data": {"methods": [], "has_payments": False}})
...
def get_store_shipping_info(request):
    ...
    try:
        ...
        active_carriers = ProviderAccount.objects.filter(is_active=True).values_list(
...
@extend_schema(
    ...
    description=_(
        "Get all active currencies that the store accepts. Use this to populate a currency switcher on the storefront."
    ),
...
def list_currencies(request):
    ...
    return Response({"success": True, "currencies": currencies, "count": len(currencies)})
...
@extend_schema(
    ...
    description=_(
        "Switch the session's active currency. The currency code must be a valid, supported ISO 4217 code."
    ),
    request={
        "application/json": {
            "type": "object",
            "properties": {"currency": {"type": "string", "example": "EUR"}},
            "required": ["currency"],
        }
    },
    responses={
...
def set_currency_api(request):
    ...
    if currency_code not in MONEYED_CURRENCIES:
        return Response(
            {"success": False, "error": f"Invalid currency code: {currency_code}"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if (
        settings
        and settings.supported_currencies
        and currency_code not in settings.supported_currencies
    ):
        return Response(
            {"success": False, "error": f"Currency not supported: {currency_code}"},
            status=status.HTTP_400_BAD_REQUEST,
        )
