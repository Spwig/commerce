...
## Subject
✅ Ваша подписка на {{ plan_name }} активна - {{ shop_name }}
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Сумма:</strong> {{ subscription_amount }} / {{ billing_cycle }}
              </mj-text>

              {% if trial_period %}
              <mj-text font-size="14px" color="{{ theme.color.success|default:'#10b981' }}" padding="5px 0">
                <strong>Период пробного использования:</strong> {{ trial_period }} дней (БЕСПЛАТНО)
              </mj-text>
              <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="5px 0">
                <strong>Первый платеж:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Следующий платеж:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% endif %}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Немедленно получите доступ к вашим товарам и преимуществам
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Управляйте своей подпиской в любое время в вашем аккаунте
        </mj-text>
      </mj-column>
    </mj-section>
...
ДЕТАЛИ ПОДПИСКИ:
План: {{ plan_name }}
{% if product_name %}Товар: {{ product_name }}
{% endif %}Сумма: {{ subscription_amount }} / {{ billing_cycle }}
{% if trial_period %}Период пробного использования: {{ trial_period }} дней (БЕСПЛАТНО)
Первый платеж: {{ next_billing_date|date:"F d, Y" }}
{% else %}Следующий платеж: {{ next_billing_date|date:"F d, Y" }}
{% endif %}Способ оплаты: {{ payment_method }}
