...
## Subject
ใบเสร็จจาก {{ store_name }} - หมายเลขคำสั่งซื้อ #{{ order_number }}
    ...
    <mj-section background-color="{{ theme.color.warning_light|default:'#fef3c7' }}" padding="15px 20px">
      <mj-column>
        <mj-text font-size="14px" color="#92400e" align="center">
          <strong>+{{ loyalty_points_earned }} คะแนนสะสม!</strong>
        </mj-text>
      </mj-column>
    </mj-section>
    {% endif %}
...
Order #{{ order_number }}
Date: {{ order_date }}
{% if cashier_name %}Served by: {{ cashier_name }}{% endif %}
...
--------------------------------
ยอดรวมยังไม่รวมภาษี:    {{ subtotal }}
{% if discount_amount %}ส่วนลด:   -{{ discount_amount }}{% endif %}
{% if tax_amount %}ภาษี:         {{ tax_amount }}{% endif %}
--------------------------------
ยอดรวม:       {{ total }}
--------------------------------
...
{% endfor %}
{% if change_given %}เงินทอน: {{ change_given }}{% endif %}

{% if loyalty_points_earned %}
+{{ loyalty_points_earned }} คะแนนสะสม!
{% endif %}
...
{% if receipt_url %}ดูใบเสร็จออนไลน์: {{ receipt_url }}{% endif %}
