...
## Subject
🎉 Seu amigo {{ referee_name }} acabou de se inscrever!
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.success|default:'#10b981' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.background|default:'#ffffff' }}" align="center" padding-top="10px">
          Seu referral agora é um membro
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          Boas notícias! {{ referee_name }} acabou de se inscrever usando seu link de referral.
        </mj-text>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          Quando eles fizerem sua primeira compra, vocês dois receberão recompensas! Enviamos outro e-mail quando isso acontecer.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}">
          1. {{ referee_name }} faz sua primeira compra<br/>
          2. Você e {{ referee_name }} recebem suas recompensas automaticamente<br/>
          3. Você pode usar sua recompensa em qualquer compra futura
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="18px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center" padding-bottom="15px">
          Continue compartilhando para ganhar mais!
        </mj-text>
        <mj-text
...
O que acontece em seguida?
1. {{ referee_name }} faz sua primeira compra
2. Você e {{ referee_name }} recebem suas recompensas automaticamente
3. Você pode usar sua recompensa em qualquer compra futura

Continue compartilhando para ganhar mais:
{{ referral_link }}

Ver seus referrals: {{ referral_dashboard_url }}
