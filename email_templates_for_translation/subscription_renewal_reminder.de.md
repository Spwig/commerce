...
## Subject
🔔 Ihre {{ plan_name }} erneuert sich in {{ days_until_renewal }} Tagen - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Ihre Abonnement erneuert sich bald
        </mj-text>
      </mj-column>
    </mj-section>
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Betrag:</strong> {{ subscription_amount }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Erneuerungsdatum:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Keine Aktion erforderlich. Ihr Abonnement wird am {{ next_billing_date|date:"F d, Y" }} automatisch erneuert.
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="15px 20px 0 20px" line-height="1.6" align="center">
          Zahlungsmethode aktualisieren oder Änderungen vornehmen? Besuchen Sie Ihr Konto-Dashboard.
        </mj-text>
      </mj-column>
    </mj-section>
...
Ihr Abonnement erneuert sich bald

BEVORSTEHENDE GEBÜHR:
Plan: {{ plan_name }}
Betrag: {{ subscription_amount }}
Erneuerungsdatum: {{ next_billing_date|date:"F d, Y" }}
