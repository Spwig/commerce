  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="13px" color="{{ theme.color.success|default:'#10b981' }}" padding="10px 0 5px 0">
                ✓ 您仍可在 {{ access_until|date:"F d, Y" }} 前繼續使用您的權益
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Status:</strong> 立即取消
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          您可在 {{ access_until|date:"F d, Y" }} 前繼續使用您的權益
        </mj-text>
        {% else %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
...
這代表什麼：
• 您將不再被收費
{% if access_until %}• 您可在 {{ access_until|date:"F d, Y" }} 前繼續使用您的權益
{% else %}• 您的訪問權限已立即終止
{% endif %}• 您隨時可以重新啟用
