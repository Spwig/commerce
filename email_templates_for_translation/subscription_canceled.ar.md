...
## Subject
❌ تم إلغاء اشتراكك في {{ plan_name }} - {{ shop_name }}
...
<mjml>
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
        ...
        <mj-wrapper background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px" border="2px solid {{ theme.color.text_muted|default:'#6b7280' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>تم الإلغاء في:</strong> {{ cancellation_date|date:"F d, Y" }}
              </mj-text>

              {% if access_until %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>الوصول حتى:</strong> {{ access_until|date:"F d, Y" }}
              </mj-text>
              <mj-text font-size="13px" color="{{ theme.color.success|default:'#10b981' }}" padding="10px 0 5px 0">
                ✓ لا يزال لديك الوصول إلى مزاياك حتى {{ access_until|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>الحالة:</strong> تم الإلغاء فورًا
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          لن يتم تحصيل أي مبالغ منك مرة أخرى
        </mj-text>

        {% if access_until %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          يمكنك مواصلة استخدام مزاياك حتى {{ access_until|date:"F d, Y" }}
        </mj-text>
        {% else %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          انتهت مدة الوصول فورًا
        </mj-text>
        {% endif %}
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px 15px 20px" line-height="1.6" align="center">
          نحن نرغب في سماع آرائك لمساعدتنا على التحسين.
        </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px 30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px 15px 20px" line-height="1.6" align="center">
          تغيرت رأيك؟ يمكنك إعادة تفعيل اشتراكك في أي وقت.
        </mj-text>
...
ملخص الإلغاء:
الخطة: {{ plan_name }}
تم الإلغاء في: {{ cancellation_date|date:"F d, Y" }}
{% if access_until %}الوصول حتى: {{ access_until|date:"F d, Y" }}

✓ لا يزال لديك الوصول إلى مزاياك حتى {{ access_until|date:"F d, Y" }}
{% else %}الحالة: تم الإلغاء فورًا
{% endif %}

ما يعنيه هذا:
• لن يتم تحصيل أي مبالغ منك مرة أخرى
{% if access_until %}• يمكنك مواصلة استخدام مزاياك حتى {{ access_until|date:"F d, Y" }}
{% else %}• انتهت مدة الوصول فورًا
{% endif %}• يمكنك إعادة تفعيل الاشتراك في أي وقت

نرغب في سماع آرائك لمساعدتنا على التحسين.
مشاركة التعليقات: {{ feedback_url }}

تغيرت رأيك؟ يمكنك إعادة تفعيل اشتراكك في أي وقت.
إعادة تفعيل الاشتراك: {{ reactivate_url }}
