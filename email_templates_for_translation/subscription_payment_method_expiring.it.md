...
## Subject
💳 Aggiornamento richiesto: Metodo di pagamento in scadenza - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        <mj-text font-size="32px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          💳 Metodo di pagamento in scadenza
        </mj-text>
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Azione richiesta per evitare interruzioni della sottoscrizione
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Sottoscrizione:</strong> {{ plan_name }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>Scade:</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Prossima fatturazione:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ Aggiorna il tuo metodo di pagamento prima di {{ expiration_date|date:"m/Y" }} per evitare interruzioni del servizio.
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Aggiungi un nuovo metodo di pagamento o aggiorna la tua carta esistente
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">2.</span>
          Assicurati di aggiornare prima della tua prossima data di fatturazione
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">3.</span>
          La tua sottoscrizione continuerà senza interruzioni
        </mj-text>
      </mj-column>
    </mj-section>
...
Scade: {{ expiration_date|date:"m/Y" }}
Prossima fatturazione: {{ next_billing_date|date:"F d, Y" }}

⚠️ Aggiorna il tuo metodo di pagamento prima di {{ expiration_date|date:"m/Y" }} per evitare interruzioni del servizio.

Cosa devi fare:
1. Aggiungi un nuovo metodo di pagamento o aggiorna la tua carta esistente
2. Assicurati di aggiornare prima della tua prossima data di fatturazione
3. La tua sottoscrizione continuerà senza interruzioni

Aggiorna il metodo di pagamento: {{ update_payment_url }}
Gestisci la sottoscrizione: {{ manage_subscription_url }}
