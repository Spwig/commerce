...
## Subject
Chúc mừng! Bạn đã được nâng cấp lên {{ new_tier }}
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          Chúc mừng! Sự trung thành của bạn đã được đền đáp. Bạn đã được nâng cấp lên {{ new_tier }}!
        </mj-text>

        <!-- Bonus Points (if any) -->
        {% if bonus_points %}
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="20px">
          <strong>🎁 Upgrade Bonus: {{ bonus_points }} điểm!</strong>
        </mj-text>
        {% endif %}
      </mj-column>
    </mj-section>
...
{% if bonus_points %}Upgrade Bonus: {{ bonus_points }} điểm!{% endif %}

Lợi ích mới của bạn:
{{ tier_benefits }}

Xem phần thưởng của bạn: {{ account_url }}
