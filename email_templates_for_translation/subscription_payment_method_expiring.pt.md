...
## Subject
💳 Atualização Necessária: Método de Pagamento Expirando em Breve - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Ação necessária para evitar interrupção da assinatura
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>Expires:</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Próxima Cobrança:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ Por favor, atualize seu método de pagamento antes de {{ expiration_date|date:"m/Y" }} para evitar interrupção do serviço.
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
...
Expires: {{ expiration_date|date:"m/Y" }}
Próxima Cobrança: {{ next_billing_date|date:"F d, Y" }}

⚠️ Por favor, atualize seu método de pagamento antes de {{ expiration_date|date:"m/Y" }} para evitar interrupção do serviço.

O Que Você Precisa Fazer:
1. Adicione um novo método de pagamento ou atualize seu cartão existente
2. Certifique-se de atualizar antes da data de cobrança seguinte
3. Sua assinatura continuará sem interrupções
