...
## Subject
💳 Cập nhật cần thiết: Phương thức thanh toán sắp hết hạn - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        <mj-text font-size="32px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          💳 Phương thức thanh toán sắp hết hạn
        </mj-text>
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Hành động cần thiết để tránh gián đoạn đăng ký
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Subscription:</strong> {{ plan_name }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>Hết hạn:</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Lần thanh toán tiếp theo:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ Vui lòng cập nhật phương thức thanh toán trước {{ expiration_date|date:"m/Y" }} để tránh gián đoạn dịch vụ.
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
...
Hết hạn: {{ expiration_date|date:"m/Y" }}
Lần thanh toán tiếp theo: {{ next_billing_date|date:"F d, Y" }}

⚠️ Vui lòng cập nhật phương thức thanh toán trước {{ expiration_date|date:"m/Y" }} để tránh gián đoạn dịch vụ.

Điều bạn cần làm:
1. Thêm phương thức thanh toán mới hoặc cập nhật thẻ hiện tại
2. Đảm bảo cập nhật trước ngày thanh toán tiếp theo của bạn
3. Đăng ký của bạn sẽ tiếp tục không gián đoạn

Cập nhật phương thức thanh toán: {{ update_payment_url }}
Quản lý đăng ký: {{ manage_subscription_url }}
