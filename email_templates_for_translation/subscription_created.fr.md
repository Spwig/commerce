...
## Subject
✅ Votre abonnement {{ plan_name }} est actif - {{ shop_name }}
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              {% if product_name %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Produit:</strong> {{ product_name }}
              </mj-text>
              {% endif %}

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Montant:</strong> {{ subscription_amount }} / {{ billing_cycle }}
              </mj-text>

              {% if trial_period %}
              <mj-text font-size="14px" color="{{ theme.color.success|default:'#10b981' }}" padding="5px 0">
                <strong>Période d'essai:</strong> {{ trial_period }} jours (GRATUIT)
              </mj-text>
              <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="5px 0">
                <strong>Premier prélèvement:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Prochain prélèvement:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% endif %}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Accédez immédiatement à vos produits et avantages
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Gérez votre abonnement à tout moment depuis votre compte
        </mj-text>
      </mj-column>
    </mj-section>
...
DÉTAILS DE L'ABONNEMENT:
Plan: {{ plan_name }}
{% if product_name %}Produit: {{ product_name }}
{% endif %}Montant: {{ subscription_amount }} / {{ billing_cycle }}
{% if trial_period %}Période d'essai: {{ trial_period }} jours (GRATUIT)
Premier prélèvement: {{ next_billing_date|date:"F d, Y" }}
{% else %}Prochain prélèvement: {{ next_billing_date|date:"F d, Y" }}
{% endif %}Méthode de paiement: {{ payment_method }}
