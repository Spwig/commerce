...
## Subject
⚠️ مشكلة توافق: {{ component_name }} و{{ conflicting_component }}
    ...
    <mj-section>
      <mj-column>
        <mj-text font-size="18px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          تم اكتشاف تعارض في الإصدار
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          تم اكتشاف مشكلة توافق بين مكونات متجر Spwig الخاص بك.
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="20px">
          <mj-column>
            ...
            <mj-text color="{{ theme.color.text|default:'#4b5563' }}">
              <strong>المكون 1:</strong> {{ component_name }} v{{ component_version }}<br/>
              <strong>المكون 2:</strong> {{ conflicting_component }} v{{ conflicting_version }}<br/>
        ...
        <mj-text font-size="13px" color="{{ theme.color.text_secondary|default:'#6b7280' }}" align="center">
          لا يزال متجرك يعمل، لكننا نوصي باصلاح هذا التعارض قريبًا.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
...
تفاصيل التعارض:
- المكون 1: {{ component_name }} v{{ component_version }}
- المكون 2: {{ conflicting_component }} v{{ conflicting_version }}
...
{% if update_url %}حل التعارض: {{ update_url }}{% endif %}
