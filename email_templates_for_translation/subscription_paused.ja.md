...
## Subject
⏸️ {{ plan_name }} サブスクリプションが一時停止されました - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          {{ plan_name }} サブスクリプションが現在一時停止されています
        </mj-text>
      </mj-column>
    </mj-section>
        ...
        <mj-wrapper background-color="#fff7ed" padding="30px" border="2px solid {{ theme.color.warning|default:'#f59e0b' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>一時停止日:</strong> {{ pause_date|date:"F d, Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>ステータス:</strong> 一時停止
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          サブスクリプションの請求が一時停止されています - 支払いは行われません
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">•</span>
          サブスクリプションの特典へのアクセスが制限される可能性があります
        </mj-text>
...
## Text Content
⏸️ サブスクリプションが一時停止されました

{{ plan_name }} サブスクリプションが現在一時停止されています

一時停止情報:
プラン: {{ plan_name }}
一時停止日: {{ pause_date|date:"F d, Y" }}
ステータス: 一時停止
...
サブスクリプションを再開: {{ resume_subscription_url }}
