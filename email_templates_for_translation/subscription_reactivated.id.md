...
## Subject
Selamat Kembali! Langganan {{ plan_name }} Anda Telah Dihidupkan Kembali - {{ shop_name }}
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Langganan Anda telah dihidupkan kembali
        </mj-text>
      </mj-column>
    </mj-section>
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Dihidupkan Kembali Pada:</strong> {{ reactivation_date|date:"F d, Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Pembayaran Berikutnya:</strong> {{ new_billing_date|date:"F d, Y" }}
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.success|default:'#10b981' }}; font-size: 18px; margin-right: 8px;">&#10003;</span>
          Akses penuh ke semua manfaat Anda kembali pulih
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.success|default:'#10b981' }}; font-size: 18px; margin-right: 8px;">&#10003;</span>
          Pembayaran pertama Anda akan dilakukan pada {{ new_billing_date|date:"F d, Y" }}
        </mj-text>
      </mj-column>
    </mj-section>
...
DETAIL LANGGANAN:
Plan: {{ plan_name }}
Dihidupkan Kembali Pada: {{ reactivation_date|date:"F d, Y" }}
Pembayaran Berikutnya: {{ new_billing_date|date:"F d, Y" }}
