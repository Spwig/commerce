...
## Subject
🏆 Relatório de Produtos Top - {{ report_period }}
    ...
    <mj-section>
      <mj-column>
        <mj-text font-size="18px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          Melhores Vendedores - {{ report_period }}
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="12px">
          <mj-column>
            <mj-text font-size="14px" color="{{ theme.color.text|default:'#4b5563' }}">
              ...
              Receita: {{ product.revenue }} | Vendas: {{ product.units }}
            </mj-text>
          </mj-column>
        </mj-section>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="12px">
          <mj-column>
            <mj-text font-size="14px" color="{{ theme.color.text|default:'#4b5563' }}">
              <strong>{{ product.rank }}. {{ product.name }}</strong><br/>
              Unidades: {{ product.units }} | Receita: {{ product.revenue }}
            </mj-text>
          </mj-column>
        </mj-section>
        <mj-spacer height="8px" />
...
Ver relatorio completo: {{ full_report_url }}