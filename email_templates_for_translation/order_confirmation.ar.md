...
## Subject
تأكيد الطلب #{{ order_number }} - شكرًا!
    ...
    <mj-section background-color="{{ theme.color.info_light|default:'#dbeafe' }}" padding="20px">
      <mj-column>
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          <strong>تقدير التسليم:</strong> {{ estimated_delivery_start }} - {{ estimated_delivery_end }}
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px 20px 10px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="5px">
          تتبع طلباتك، اختر بسرعة في المرة القادمة، وقم بإدارة تفضيلاتك.
        </mj-text>
      </mj-column>
    </mj-section>
...
تقدير التسليم: {{ estimated_delivery_start }} - {{ estimated_delivery_end }}

العناصر المطلوبة:
{% for item in items %}
- {{ item.name }} (الكمية: {{ item.quantity }}) - {{ item.subtotal }}
{% endfor %}
