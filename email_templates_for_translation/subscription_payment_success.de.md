...
## Subject
✅ Zahlung für {{ plan_name }} - {{ shop_name }} erhalten
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Bezahlter Betrag:</strong> {{ amount_paid }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Zahlungsdatum:</strong> {{ payment_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Nächster Rechnungsdatum:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Ihre Abonnement bleibt aktiv und Sie haben weiterhin vollen Zugriff auf alle Vorteile.
        </mj-text>
      </mj-column>
    </mj-section>
...
Zahlungsdatum: {{ payment_date|date:"F d, Y" }}
Zahlungsmethode: {{ payment_method }}
{% if transaction_id %}Transaktions-ID: {{ transaction_id }}
{% endif %}
Nächster Rechnungsdatum: {{ next_billing_date|date:"F d, Y" }}

Ihr Abonnement bleibt aktiv und Sie haben weiterhin vollen Zugriff auf alle Vorteile.

Abonnement ansehen: {{ manage_subscription_url }}
Rechnung herunterladen: {{ download_receipt_url }}
