...
## Subject
예약 확인 - {{ product_name }} on {{ booking_date }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          안녕하세요 {{ customer_name }}, 예약이 확인되었습니다.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="10px">
          <strong>총액:</strong> {{ total_cost }}
        </mj-text>
        {% endif %}
        {% if deposit_amount %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}">
          <strong>예치금 지불:</strong> {{ deposit_amount }}
        </mj-text>
        {% endif %}
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px">
      <mj-column>
        <mj-text font-size="13px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center">
          변경이 필요하신가요? 계정에서 <a href="{{ reschedule_url }}" style="color: {{ theme.color.primary|default:'#2563eb' }};">예약을 다시 조정</a>하거나 <a href="{{ cancel_url }}" style="color: {{ theme.color.primary|default:'#2563eb' }};">예약을 취소</a>하실 수 있습니다.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if total_cost %}총액: {{ total_cost }}{% endif %}
{% if deposit_amount %}예치금 지불: {{ deposit_amount }}{% endif %}

{% if ical_url %}달력에 추가: {{ ical_url }}{% endif %}

변경이 필요하신가요? 계정에서 예약을 다시 조정하거나 예약을 취소하려면 다음 링크를 방문하세요.
{{ reschedule_url }}
{{ cancel_url }}