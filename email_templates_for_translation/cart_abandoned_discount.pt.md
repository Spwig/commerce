...
## Subject
Oferta Exclusiva de {{ discount_percentage }}% no seu carrinho! - {{ shop_name }}
    ...
    <mj-section background-color="#dcfce7">
      <mj-column>
        ...
        <mj-text font-size="18px" color="#047857" align="center">
          {{ discount_percentage }}% DE DESCONTO NO SEU CARRINHO
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section>
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          Queremos facilitar isso, {{ customer_name }}
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          Conclua sua compra agora e economize {{ discount_percentage }}% com o código <strong>{{ discount_code }}</strong>
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="15px">
          <mj-column>
            ...
            <mj-text align="right">
              <span style="color: {{ theme.color.text_secondary|default:'#6b7280' }};">Desconto ({{ discount_percentage }}%):</span> <span style="color: #059669; font-weight: bold;">-{{ discount_amount }}</span>
            </mj-text>
            <mj-text font-size="20px" font-weight="bold" align="right" color="{{ theme.color.text|default:'#1f2937' }}">
              Novo Total: {{ discounted_total }}
            </mj-text>
          </mj-column>
        </mj-section>
        ...
        <mj-button href="{{ cart_url }}" background-color="#059669" color="#ffffff" font-size="18px" font-weight="bold" border-radius="6px" padding="18px 50px">
          Reclame seu desconto de {{ discount_percentage }}%
        </mj-button>

        <mj-spacer height="20px" />

        <mj-text font-size="13px" color="#dc2626" align="center" font-style="italic">
          Oferta válida até {{ discount_expiry }} - Não perca!
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
...
Conclua sua compra agora e economize {{ discount_percentage }}% com o código {{ discount_code }}
...
Subtotal: {{ cart_total }}
Desconto ({{ discount_percentage }}%): -{{ discount_amount }}
NOVO TOTAL: {{ discounted_total }}

Reclame seu {{ discount_percentage }}% de desconto: {{ cart_url }}
