...
## Subject
⚠️ 您的 {{ current_tier }} 等級將於不久後到期 - {{ shop_name }}
...
不要失去您的 {{ current_tier }} 優惠！

您好 {{ customer_name }}，

除非您保持活動水平，否則您的 {{ current_tier }} 等級狀態將在 {{ expiry_date }} 到期。

目前狀態：
- 當前等級：{{ current_tier }}
- 到期日：{{ expiry_date }}（{{ days_remaining }} 天）
- 下一等級：{{ next_tier }}

如何保持您的 {{ current_tier }} 等級：
您需要在 {{ expiry_date }} 前 {{ requirement_type }}：

{{ requirement_description }}
目前：{{ current_progress }} | 需要：{{ required_amount }}
...
立即購物以保持您的等級：{{ shop_url }}
查看完整詳情：{{ loyalty_dashboard_url }}