...
## Subject
수정 요청: {{ component_name }} v{{ version }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="20px">
          저희 검토 팀이 성분이 승인되기 전에 몇 가지 변경이 필요하다고 요청했습니다.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="25px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-bottom="8px">
          <strong>성분:</strong> {{ component_name }}
        </mj-text>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-bottom="8px">
...
{% if review_notes %}요청된 변경 사항:
{{ review_notes }}{% endif %}

피드백을 검토하고 업데이트된 버전을 제출해 주세요.

제출 세부 정보 보기: {{ submission_url }}
