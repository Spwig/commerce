...
## Subject
🔔 Gia hạn {{ plan_name }} trong {{ days_until_renewal }} ngày - {{ shop_name }}
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Kế hoạch:</strong> {{ plan_name }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Ngày gia hạn:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Không cần hành động. Thuê bao của bạn sẽ tự động gia hạn vào {{ next_billing_date|date:"F d, Y" }}.
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="15px 20px 0 20px" line-height="1.6" align="center">
          Cần cập nhật phương thức thanh toán hoặc thay đổi? Vui lòng truy cập bảng điều khiển tài khoản của bạn.
        </mj-text>
      </mj-column>
    </mj-section>
...
Thuê bao của bạn sẽ được gia hạn sớm

GIA HẠN SẮP TỚI:
Kế hoạch: {{ plan_name }}
Số tiền: {{ subscription_amount }}
Ngày gia hạn: {{ next_billing_date|date:"F d, Y" }}
...
Quản lý thuê bao: {{ manage_subscription_url }}
