...
## Subject
❌ Gói {{ plan_name }} Của Bạn Đã Bị Hủy - {{ shop_name }}
...
<mjml>
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        <mj-text font-size="32px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          Chúng Tôi Rất Tiếc Khi Bạn Ra Đi
        </mj-text>
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Gói của bạn đã bị hủy
        </mj-text>
      </mj-column>
    </mj-section>
        ...
        <mj-wrapper background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px" border="2px solid {{ theme.color.text_muted|default:'#6b7280' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Ngày Hủy:</strong> {{ cancellation_date|date:"F d, Y" }}
              </mj-text>

              {% if access_until %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Truy cập đến:</strong> {{ access_until|date:"F d, Y" }}
              </mj-text>
              <mj-text font-size="13px" color="{{ theme.color.success|default:'#10b981' }}" padding="10px 0 5px 0">
                ✓ Bạn vẫn có thể truy cập các quyền lợi của mình đến {{ access_until|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Tình trạng:</strong> Hủy ngay lập tức
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Bạn sẽ không bị tính phí nữa
        </mj-text>

        {% if access_until %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          Bạn có thể tiếp tục sử dụng các quyền lợi của mình đến {{ access_until|date:"F d, Y" }}
        </mj-text>
        {% else %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          Truy cập của bạn đã kết thúc ngay lập tức
        </mj-text>
        {% endif %}
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px 15px 20px" line-height="1.6" align="center">
          Chúng tôi rất mong nhận được phản hồi của bạn để giúp chúng tôi cải thiện.
        </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px 30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px 15px 20px" line-height="1.6" align="center">
          Đã thay đổi ý định? Bạn có thể kích hoạt lại gói của mình bất kỳ lúc nào.
        </mj-text>
...
✓ Bạn vẫn có thể truy cập các quyền lợi của mình đến {{ access_until|date:"F d, Y" }}
{% else %}Status: Hủy ngay lập tức
{% endif %}

Điều Này Có Nghĩa Là Gì:
• Bạn sẽ không bị tính phí nữa
{% if access_until %}• Bạn có thể tiếp tục sử dụng các quyền lợi của mình đến {{ access_until|date:"F d, Y" }}
{% else %}• Truy cập của bạn đã kết thúc ngay lập tức
{% endif %}• Bạn có thể kích hoạt lại bất kỳ lúc nào

Chúng tôi rất mong nhận được phản hồi của bạn để giúp chúng tôi cải thiện.
Chia Sẻ Phản Hồi: {{ feedback_url }}

Đã thay đổi ý định? Bạn có thể kích hoạt lại gói của mình bất kỳ lúc nào.
Kích Hoạt Lại Gói: {{ reactivate_url }}
