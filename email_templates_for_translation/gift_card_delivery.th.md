...
## Subject
🎁 คุณได้รับบัตรของขวัญจาก {{ sender_name }} - {{ shop_name }}
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" align="center" padding-top="10px">
                ⏰ หมดอายุ: {{ gift_card.expires_at|date:"F d, Y" }}
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="20px">
      <mj-column>
        <mj-text font-size="11px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" line-height="1.5">
          บัตรของขวัญไม่สามารถคืนเงินได้และไม่สามารถแลกเป็นเงินสดได้。
          {% if not gift_card.expires_at %}บัตรของขวัญนี้ไม่มีวันหมดอายุ{% endif %}。
          บัตรของขวัญไม่สามารถใช้เพื่อซื้อบัตรของขวัญอื่นได้。
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if gift_card.sender_name %}จาก: {{ gift_card.sender_name }}

{% endif %}{% if gift_card.message %}ข้อความส่วนตัว:
"{{ gift_card.message }}"

{% endif %}รหัสบัตรของขวัญของคุณ:
{{ gift_card.code }}

ยอดคงเหลือบัตรของขวัญ:
{{ gift_card.current_balance.amount }} {{ gift_card.current_balance.currency }}

{% if gift_card.expires_at %}หมดอายุ: {{ gift_card.expires_at|date:"F d, Y" }}
{% endif %}

วิธีใช้บัตรของขวัญ:
1. ค้นหาสินค้าและเพิ่มสินค้าลงในตะกร้า
2. ดำเนินการชำระเงินและใส่รหัสบัตรของขวัญของคุณ
3. ยอดเงินในบัตรของขวัญจะถูกนำไปใช้ในคำสั่งซื้อของคุณ
...
ข้อกำหนดของบัตรของขวัญ:
บัตรของขวัญไม่สามารถคืนเงินได้และไม่สามารถแลกเป็นเงินสดได้。
{% if not gift_card.expires_at %} บัตรของขวัญนี้ไม่มีวันหมดอายุ。
{% endif %} บัตรของขวัญไม่สามารถใช้เพื่อซื้อบัตรของขวัญอื่นได้。
