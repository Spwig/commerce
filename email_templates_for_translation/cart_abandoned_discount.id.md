...
## Subject
Diskon Eksklusif {{ discount_percentage }}% untuk keranjang belanja Anda! - {{ shop_name }}
    ...
    <mj-section background-color="#dcfce7">
      <mj-column>
        ...
        <mj-text font-size="18px" color="#047857" align="center">
          {{ discount_percentage }}% OFF Keranjang Belanja Anda
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section>
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          Kami ingin membuat ini mudah, {{ customer_name }}
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          Lengkapi pembelian Anda sekarang dan hemat {{ discount_percentage }}% dengan kode <strong>{{ discount_code }}</strong>
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="15px">
          <mj-column>
            ...
            <mj-text align="right">
              <span style="color: {{ theme.color.text_secondary|default:'#6b7280' }};">Diskon ({{ discount_percentage }}%):</span> <span style="color: #059669; font-weight: bold;">-{{ discount_amount }}</span>
            </mj-text>
            <mj-text font-size="20px" font-weight="bold" align="right" color="{{ theme.color.text|default:'#1f2937' }}">
              Total Baru: {{ discounted_total }}
            </mj-text>
          </mj-column>
        </mj-section>
        ...
        <mj-button href="{{ cart_url }}" background-color="#059669" color="#ffffff" font-size="18px" font-weight="bold" border-radius="6px" padding="18px 50px">
          Klaim Diskon {{ discount_percentage }}% Anda
        </mj-button>

        <mj-spacer height="20px" />

        <mj-text font-size="13px" color="#dc2626" align="center" font-style="italic">
          Penawaran berakhir {{ discount_expiry }} - Jangan lewatkan!
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
...
Lengkapi pembelian Anda sekarang dan hemat {{ discount_percentage }}% dengan kode {{ discount_code }}
...
Subtotal: {{ cart_total }}
Diskon ({{ discount_percentage }}%): -{{ discount_amount }}
TOTAL BARU: {{ discounted_total }}

Klaim diskon {{ discount_percentage }}% Anda: {{ cart_url }}
