---
template_type: wishlist_item_added_confirmation
category: Wishlist
---

# Email Template: wishlist_item_added_confirmation

## Subject
✓ {{ product_name }} added to your wishlist - {{ shop_name }}

## HTML Content
<mjml>
  <mj-body>
    <mj-section>
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          ❤️ Added to Your Wishlist!
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          Hi {{ customer_name }},
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          You've successfully added {{ product_name }} to your wishlist. We'll keep an eye on it for you!
        </mj-text>

        <mj-spacer height="30px" />

        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="20px">
          <mj-column width="35%">
            <mj-image src="{{ product_image }}" alt="{{ product_name }}" border-radius="8px" />
          </mj-column>
          <mj-column width="65%">
            <mj-text font-weight="bold" font-size="18px" color="{{ theme.color.text|default:'#1f2937' }}">
              {{ product_name }}
            </mj-text>
            <mj-text font-size="16px" font-weight="bold" color="{{ theme.color.primary|default:'#2563eb' }}">
              {{ product_price }}
            </mj-text>
            {% if product_in_stock %}
            <mj-text font-size="13px" color="#059669">
              ✓ In Stock
            </mj-text>
            {% else %}
            <mj-text font-size="13px" color="#dc2626">
              ⚠️ Out of Stock - We'll notify you when it's back!
            </mj-text>
            {% endif %}
          </mj-column>
        </mj-section>

        <mj-spacer height="30px" />

        <mj-section background-color="#f0fdf4" border-radius="8px" padding="15px">
          <mj-column>
            <mj-text font-size="14px" color="#065f46" align="center">
              💡 <strong>We'll notify you about:</strong><br/>
              • Price drops<br/>
              • Back in stock alerts<br/>
              • Limited-time sales
            </mj-text>
          </mj-column>
        </mj-section>

        <mj-spacer height="30px" />

        <mj-button href="{{ wishlist_url }}" background-color="{{ theme.color.primary|default:'#2563eb' }}" color="#ffffff" font-size="16px" font-weight="bold" border-radius="6px" padding="15px 40px">
          View My Wishlist
        </mj-button>

        {% if product_in_stock %}
        <mj-spacer height="10px" />
        <mj-button href="{{ product_url }}" background-color="#059669" color="#ffffff" font-size="14px" border-radius="6px" padding="10px 25px">
          Buy Now
        </mj-button>
        {% endif %}
      </mj-column>
    </mj-section>
  </mj-body>
</mjml>

## Text Content
❤️ ADDED TO YOUR WISHLIST!

Hi {{ customer_name }},

You've successfully added {{ product_name }} to your wishlist. We'll keep an eye on it for you!

{{ product_name }}
Price: {{ product_price }}
{% if product_in_stock %}✓ In Stock{% else %}⚠️ Out of Stock - We'll notify you when it's back!{% endif %}

💡 WE'LL NOTIFY YOU ABOUT:
• Price drops
• Back in stock alerts
• Limited-time sales

View my wishlist: {{ wishlist_url }}
{% if product_in_stock %}Buy now: {{ product_url }}{% endif %}

## Variables

| Variable | Description | Example |
|----------|-------------|---------|
| customer_name | Customer's first name | Sarah |
| product_name | Product name | Wireless Headphones |
| product_price | Current price | $79.99 |
| product_image | Image URL | https://shop.com/media/headphones.jpg |
| product_in_stock | Stock status boolean | true |
| wishlist_url | Wishlist page | https://shop.com/en/account/wishlist |
| product_url | Product page | https://shop.com/en/products/wireless-headphones |
| shop_name | Store name | Amazing Shop |

## Notes

- Confirmation email after adding to wishlist
- Marketing email - respects preferences
- Sets expectations for future notifications
- Encourages purchase if in stock
- Sent immediately after wishlist addition
