...
## Subject
Pengingat: {{ product_name }} - {{ booking_date }}
  ...
  <mj-body background-color="{{ theme.color.background_secondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Halo {{ customer_name }}, ini adalah pengingat mengenai pemesanan Anda yang segera datang.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        {% if resource_name %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}">
          <strong>Sumber:</strong> {{ resource_name }}
        </mj-text>
        {% endif %}
        {% if persons_display %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px">
      <mj-column>
        <mj-text font-size="13px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center">
          Perlu membuat perubahan? Anda dapat <a href="{{ reschedule_url }}" style="color: {{ theme.color.primary|default:'#2563eb' }};">mengatur ulang</a> atau <a href="{{ cancel_url }}" style="color: {{ theme.color.primary|default:'#2563eb' }};">membatalkan</a> pemesanan Anda dari akun Anda.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if resource_name %}Sumber: {{ resource_name }}{% endif %}
{% if persons_display %}Tamu: {{ persons_display }}{% endif %}

{% if ical_url %}Tambah ke Kalender: {{ ical_url }}{% endif %}

Perlu membuat perubahan? Kunjungi akun Anda untuk mengatur ulang atau membatalkan.
{{ reschedule_url }}
{{ cancel_url }}