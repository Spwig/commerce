...
<mjml>
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="20px">
          您的 Spwig 开发者账户已被暂停。在此期间，您已发布的组件仍然可用，但您无法提交新的组件或更新。
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          如果您认为这是错误的，请通过 <a href="mailto:{{ support_email }}" style="color: {{ theme.color.primary|default:'#2563eb' }}; text-decoration: none;">{{ support_email }}</a> 联系我们。
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if suspension_reason %}原因：{{ suspension_reason }}{% endif %}

如果您认为这是错误的，请联系 {{ support_email }}。
