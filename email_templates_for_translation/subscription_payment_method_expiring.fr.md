...
## Subject
💳 Mise à jour requise : Méthode de paiement expirant bientôt - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        <mj-text font-size="32px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          💳 Méthode de paiement expirant
        </mj-text>
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Action requise pour éviter une interruption de souscription
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Souscription :</strong> {{ plan_name }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>Expire le :</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Prochaine facturation :</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ Veuillez mettre à jour votre méthode de paiement avant {{ expiration_date|date:"m/Y" }} pour éviter une interruption du service.
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Ajoutez une nouvelle méthode de paiement ou mettez à jour votre carte existante
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">2.</span>
          Assurez-vous de mettre à jour avant votre date de facturation suivante
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">3.</span>
          Votre souscription continuera sans interruption
        </mj-text>
      </mj-column>
    </mj-section>
...
Souscription : {{ plan_name }}
Méthode de paiement : {{ payment_method }}
Expire le : {{ expiration_date|date:"m/Y" }}
Prochaine facturation : {{ next_billing_date|date:"F d, Y" }}

⚠️ Veuillez mettre à jour votre méthode de paiement avant {{ expiration_date|date:"m/Y" }} pour éviter une interruption du service.

Ce que vous devez faire :
1. Ajoutez une nouvelle méthode de paiement ou mettez à jour votre carte existante
2. Assurez-vous de mettre à jour avant votre date de facturation suivante
3. Votre souscription continuera sans interruption

Mettre à jour la méthode de paiement : {{ update_payment_url }}
Gérer la souscription : {{ manage_subscription_url }}
