...
## Subject
⚠️ Терминал POS отключен: {{ terminal_name }}
...
- Последнее видение: {{ last_seen }}
- Отключен с: {{ offline_since }}
- Продолжительность: {{ offline_duration }}

ОБЩИЕ ПРИЧИНЫ:
• Проблемы с сетевым подключением
• Терминал выключен или перезагружен
• Сбой или зависание программного обеспечения
• Проблемы с интернет-сервисом

РЕКОМЕНДУЕМЫЕ ДЕЙСТВИЯ:
1. Проверьте питание терминала и сетевое подключение
2. Перезагрузите устройство терминала
3. Проверьте подключение к интернету
4. Проверьте настройки брандмауэра и безопасности

{% if active_shift %}
⚠️ АКТИВНЫЙ СМЕНА ПРЕДУПРЕЖДЕНИЕ:
Этот терминал имеет активную смену. Данные о продажах могут не синхронизироваться до тех пор, пока терминал не будет подключен.
{% endif %}

Просмотреть статус терминала: {{ admin_terminals_url }}

Вы получите еще одно уведомление, когда терминал переподключится.