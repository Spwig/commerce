---
template_type: digital_product_download_expired
category: Digital Products
---

# Email Template: digital_product_download_expired

## Subject
Lien de téléchargement expiré - Commande #{{ order_number }}

## HTML Content
<mjml>
  <mj-head>
    <mj-attributes>
      <mj-all font-family="'Helvetica Neue', Helvetica, Arial, sans-serif" />
      <mj-text font-size="14px" color="#333333" line-height="20px" />
      <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px" />
    </mj-attributes>
  </mj-head>
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    <!-- Header -->
    <mj-section background-color="{{ theme.color.error|default:'#ef4444' }}" padding="20px">
      <mj-column>
        <mj-text font-size="24px" font-weight="bold" color="{{ theme.color.background|default:'#ffffff' }}" align="center">
          Lien de téléchargement expiré
        </mj-text>
      </mj-column>
    </mj-section>

    <!-- Main Content -->
    <mj-section>
      <mj-column>
        <mj-text font-size="16px">
          Bonjour {{ customer_name }},
        </mj-text>
        <mj-text>
          Votre lien de téléchargement pour <strong>{{ product_name }}</strong> depuis la commande #{{ order_number }} a expiré.
        </mj-text>
      </mj-column>
    </mj-section>

    <!-- Expired Information -->
    <mj-section background-color="#fef2f2" padding="20px" border-radius="8px">
      <mj-column>
        <mj-text font-size="14px" color="#991b1b">
          Les liens de téléchargement expirent {{ expiration_days }} jours après l'achat pour des raisons de sécurité.
        </mj-text>
      </mj-column>
    </mj-section>

    <!-- Request New Link -->
    <mj-section>
      <mj-column>
        <mj-text font-size="16px" font-weight="bold">
          Besoin d'un nouveau lien de téléchargement ?
        </mj-text>
        <mj-text>
          Vous pouvez demander un nouveau lien de téléchargement en vous connectant à votre compte ou en contactant notre équipe de support.
        </mj-text>
        <mj-button href="{{ account_url }}" background-color="{{ theme.color.primary|default:'#2563eb' }}" color="{{ theme.color.background|default:'#ffffff' }}" font-size="16px" padding="15px 30px" border-radius="6px">
          Accéder à Mon Compte
        </mj-button>
      </mj-column>
    </mj-section>

    <!-- Support -->
    <mj-section>
      <mj-column>
        <mj-divider border-color="{{ theme.color.border|default:'#e5e7eb' }}" />
        <mj-text font-size="12px" color="#9ca3af" align="center">
          Questions ? Contactez {{ support_email }}
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
</mjml>

## Text Content
Lien de téléchargement expiré

Bonjour {{ customer_name }},

Votre lien de téléchargement pour {{ product_name }} depuis la commande #{{ order_number }} a expiré.

Les liens de téléchargement expirent {{ expiration_days }} jours après l'achat pour des raisons de sécurité.

Besoin d'un nouveau lien de téléchargement ?
Vous pouvez demander un nouveau lien de téléchargement en vous connectant à votre compte ou en contactant notre équipe de support.

Accéder à Mon Compte: {{ account_url }}

Questions ? Contactez {{ support_email }}