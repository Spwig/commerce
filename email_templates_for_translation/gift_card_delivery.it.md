...
## Subject
🎁 Carta Regalo da {{ sender_name }} - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Qualcuno speciale ha pensato a te
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              <mj-text font-size="14px" color="#0c4a6e" align="center" text-transform="uppercase" letter-spacing="1px" font-weight="600" padding-bottom="10px">
                Il Codice della Tua Carta Regalo
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" align="center" padding-top="10px">
                ⏰ Scade: {{ gift_card.expires_at|date:"F d, Y" }}
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="20px">
      <mj-column>
        <mj-text font-size="11px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" line-height="1.5">
          Le carte regalo non sono rimborsabili e non possono essere scambiate in contanti.
          {% if not gift_card.expires_at %}Questa carta regalo non scade mai.{% endif %}
          Le carte regalo non possono essere utilizzate per acquistare altre carte regalo.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if gift_card.sender_name %}Da: {{ gift_card.sender_name }}

{% endif %}{% if gift_card.message %}Messaggio Personale:
"{{ gift_card.message }}"

{% endif %}IL CODICE DELLA TUA CARTA REGALO:
{{ gift_card.code }}

Saldo della Carta Regalo:
{{ gift_card.current_balance.amount }} {{ gift_card.current_balance.currency }}

{% if gift_card.expires_at %}Scade: {{ gift_card.expires_at|date:"F d, Y" }}
{% endif %}

Come Utilizzare la Tua Carta Regalo:
1. Sfoglia i nostri prodotti e aggiungi gli articoli al carrello
2. Procedi al checkout e inserisci il codice della tua carta regalo
3. Il saldo della carta regalo verrà applicato al tuo ordine
...
Termini della Carta Regalo:
Le carte regalo non sono rimborsabili e non possono essere scambiate in contanti.{% if not gift_card.expires_at %} Questa carta regalo non scade mai.{% endif %} Le carte regalo non possono essere utilizzate per acquistare altre carte regalo.
