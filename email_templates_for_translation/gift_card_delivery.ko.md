...
## Subject
🎁 {{ sender_name }}님으로부터의 선물 카드 - {{ shop_name }}
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" align="center" padding-top="10px">
                ⏰ 유효 기한: {{ gift_card.expires_at|date:"F d, Y" }}
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="20px">
      <mj-column>
        <mj-text font-size="11px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" line-height="1.5">
          선물 카드는 환불이 불가능하며 현금으로 교환할 수 없습니다.
          {% if not gift_card.expires_at %}이 선물 카드는 유효 기한이 없습니다.{% endif %}
          선물 카드는 다른 선물 카드를 구매하는 데 사용할 수 없습니다.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if gift_card.sender_name %}보낸 사람: {{ gift_card.sender_name }}

{% endif %}{% if gift_card.message %}개인 메시지:
"{{ gift_card.message }}"
...
{% if gift_card.expires_at %}유효 기한: {{ gift_card.expires_at|date:"F d, Y" }}
{% endif %}

선물 카드 사용 방법:
1. 상품을 살펴보고 장바구니에 상품을 추가하세요
2. 결제로 이동하여 선물 카드 코드를 입력하세요
3. 선물 카드 잔액이 주문에 적용됩니다
...
선물 카드 이용 약관:
선물 카드는 환불이 불가능하며 현금으로 교환할 수 없습니다.{% if not gift_card.expires_at %} 이 선물 카드는 유효 기한이 없습니다.{% endif %} 선물 카드는 다른 선물 카드를 구매하는 데 사용할 수 없습니다.
