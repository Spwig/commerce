...
## Subject
💳 결제 수단 업데이트 필요: 곧 만료됩니다 - {{ shop_name }}
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>만료일:</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>다음 결제일:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ {{ expiration_date|date:"m/Y" }} 이전에 결제 수단을 업데이트하여 서비스 중단을 피해 주세요.
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
...
만료일: {{ expiration_date|date:"m/Y" }}
다음 결제일: {{ next_billing_date|date:"F d, Y" }}

⚠️ {{ expiration_date|date:"m/Y" }} 이전에 결제 수단을 업데이트하여 서비스 중단을 피해 주세요.

해야 할 일:
1. 새 결제 수단을 추가하거나 기존 카드를 업데이트하세요
2. 다음 결제일 이전에 업데이트를 완료하세요
3. 구독은 중단 없이 계속됩니다
