...
## Subject
✅ Pagamento Recebido para {{ plan_name }} - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Data do Pagamento:</strong> {{ payment_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Data da Próxima Cobrança:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Sua assinatura permanece ativa e você continua a ter acesso completo a todos os benefícios.
        </mj-text>
      </mj-column>
    </mj-section>
...
Data do Pagamento: {{ payment_date|date:"F d, Y" }}
Método de Pagamento: {{ payment_method }}
{% if transaction_id %}ID da Transação: {{ transaction_id }}
{% endif %}
Data da Próxima Cobrança: {{ next_billing_date|date:"F d, Y" }}

Sua assinatura permanece ativa e você continua a ter acesso completo a todos os benefícios.

Ver Assinatura: {{ manage_subscription_url }}
Baixar Comprovante: {{ download_receipt_url }}
