...
## Subject
⏱️ Ihre {{ plan_name }}-Abonnement ist abgelaufen - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        <mj-text font-size="32px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          ⏱️ Abonnement abgelaufen
        </mj-text>
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Ihr {{ plan_name }}-Abonnement ist beendet
        </mj-text>
      </mj-column>
    </mj-section>
        ...
        <mj-wrapper background-color="#fef2f2" padding="30px" border="2px solid {{ theme.color.error|default:'#ef4444' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Ablaufdatum:</strong> {{ expiration_date|date:"F d, Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0">
                <strong>Status:</strong> Ablauf
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Ihr Zugriff auf die Vorteile des Abonnements ist beendet
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.error|default:'#ef4444' }}; font-size: 18px; margin-right: 8px;">•</span>
          Sie werden künftig nicht mehr belastet
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.error|default:'#ef4444' }}; font-size: 18px; margin-right: 8px;">•</span>
          Sie können das Abonnement jederzeit erneuern, um den Zugriff wiederherzustellen
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px 30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="0 20px 20px 20px" line-height="1.6" align="center">
          Erneuern Sie Ihr Abonnement, um den Zugriff auf alle Vorteile wiederherzustellen
        </mj-text>
...
ABONNEMENT-INFO:
Plan: {{ plan_name }}
Ablaufdatum: {{ expiration_date|date:"F d, Y" }}
Status: Ablauf
