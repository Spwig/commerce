  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          Boa notícia! Você acabou de ganhar {{ points }} pontos {{ reason }}.
        </mj-text>

        <!-- Balance -->
        <mj-divider border-color="{{ theme.color.border|default:'#e5e7eb' }}" padding="20px 0" />
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          <strong>Total de Pontos:</strong> {{ total_points }}
        </mj-text>
        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}">
          Disponíveis para resgatar agora
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if suggested_reward %}Você pode agora resgatar: {{ suggested_reward }}{% endif %}

Explorar recompensas: {{ rewards_url }}
