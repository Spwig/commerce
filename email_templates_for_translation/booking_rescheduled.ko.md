    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          안녕하세요 {{ customer_name }}, 예약 일정이 변경되었습니다.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="13px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding-top="15px">
          <em>이전: {{ old_date }}에 {{ old_time_start }} - {{ old_time_end }}</em>
        </mj-text>
        {% endif %}
      </mj-column>
    </mj-section>
...
{% if old_date %}이전: {{ old_date }}에 {{ old_time_start }} - {{ old_time_end }}{% endif %}

{% if ical_url %}캘린더 업데이트: {{ ical_url }}{% endif %}