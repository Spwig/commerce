...
## Subject
💳 Aktualisierung erforderlich: Zahlungsmethode läuft bald ab - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Aktion erforderlich, um Unterbrechung der Abonnement zu vermeiden
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              <mj-text font-size="20px" font-weight="600" color="#9a3412" align="center" padding-bottom="15px">
                Läufende Zahlungsmethode
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Abonnement:</strong> {{ plan_name }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>Läuft ab:</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Nächste Abrechnung:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ Bitte aktualisieren Sie Ihre Zahlungsmethode vor {{ expiration_date|date:"m/Y" }}, um eine Unterbrechung des Dienstes zu vermeiden.
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Fügen Sie eine neue Zahlungsmethode hinzu oder aktualisieren Sie Ihre bestehende Karte
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">2.</span>
          Stellen Sie sicher, dass Sie dies vor Ihrem nächsten Abrechnungstermin aktualisieren
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.warning|default:'#f59e0b' }}; font-size: 18px; margin-right: 8px;">3.</span>
          Ihr Abonnement wird ohne Unterbrechung weiterlaufen
        </mj-text>
      </mj-column>
    </mj-section>
...
Aktion erforderlich, um Unterbrechung des Abonnements zu vermeiden

LÄUFTENDE ZAHLUNGSMETHODE:
Abonnement: {{ plan_name }}
Zahlungsmethode: {{ payment_method }}
Läuft ab: {{ expiration_date|date:"m/Y" }}
Nächste Abrechnung: {{ next_billing_date|date:"F d, Y" }}

⚠️ Bitte aktualisieren Sie Ihre Zahlungsmethode vor {{ expiration_date|date:"m/Y" }}, um eine Unterbrechung des Dienstes zu vermeiden.

Was Sie tun müssen:
1. Fügen Sie eine neue Zahlungsmethode hinzu oder aktualisieren Sie Ihre bestehende Karte
2. Stellen Sie sicher, dass Sie dies vor Ihrem nächsten Abrechnungstermin aktualisieren
3. Ihr Abonnement wird ohne Unterbrechung weiterlaufen
