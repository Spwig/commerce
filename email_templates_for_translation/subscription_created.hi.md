...
## Subject
✅ आपका {{ plan_name }} सदस्यता सक्रिय है - {{ shop_name }}
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              {% if product_name %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>उत्पाद:</strong> {{ product_name }}
              </mj-text>
              {% endif %}

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>राशि:</strong> {{ subscription_amount }} / {{ billing_cycle }}
              </mj-text>

              {% if trial_period %}
              <mj-text font-size="14px" color="{{ theme.color.success|default:'#10b981' }}" padding="5px 0">
                <strong>परीक्षण अवधि:</strong> {{ trial_period }} दिन (मुफ्त)
              </mj-text>
              <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="5px 0">
                <strong>पहला भुगतान:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          तुरंत अपने उत्पादों और लाभों के अनुभव करें
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          अपने खाते में कभी भी अपनी सदस्यता का प्रबंधन करें
        </mj-text>
      </mj-column>
    </mj-section>
...
सदस्यता विवरण:
योजना: {{ plan_name }}
{% if product_name %}उत्पाद: {{ product_name }}
{% endif %}राशि: {{ subscription_amount }} / {{ billing_cycle }}
{% if trial_period %}परीक्षण अवधि: {{ trial_period }} दिन (मुफ्त)
पहला भुगतान: {{ next_billing_date|date:"F d, Y" }}
{% else %}अगला भुगतान: {{ next_billing_date|date:"F d, Y" }}
{% endif %}भुगतान विधि: {{ payment_method }}
...
सदस्यता का प्रबंधन करें: {{ manage_subscription_url }}
