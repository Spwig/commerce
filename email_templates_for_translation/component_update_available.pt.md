---
template_type: component_update_available
category: Component Updates
---

# Email Template: component_update_available

## Subject
Atualização Disponível: {{ component_name }} v{{ new_version }}

## HTML Content
<mjml>
  <mj-body>
    <mj-section background-color="#eff6ff">
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="#1e40af" align="center">
          📦 Atualização Disponível
        </mj-text>
      </mj-column>
    </mj-section>

    <mj-section>
      <mj-column>
        <mj-text font-size="18px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          Nova Versão Disponível
        </mj-text>

        <mj-text font-size="15px" color="{{ theme.color.text|default:'#4b5563' }}">
          Uma nova versão do {{ component_name }} está disponível para sua loja Spwig.
        </mj-text>

        <mj-spacer height="20px" />

        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="20px">
          <mj-column>
            <mj-text font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
              Detalhes da Atualização:
            </mj-text>
            <mj-text color="{{ theme.color.text|default:'#4b5563' }}">
              <strong>Componente:</strong> {{ component_name }}<br/>
              <strong>Versão Atual:</strong> {{ current_version }}<br/>
              <strong>Nova Versão:</strong> {{ new_version }}<br/>
              <strong>Data de Lançamento:</strong> {{ release_date }}
            </mj-text>
          </mj-column>
        </mj-section>

        <mj-spacer height="30px" />

        <mj-text font-size="16px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          O que há de novo:
        </mj-text>

        <mj-text font-size="15px" color="{{ theme.color.text|default:'#4b5563' }}" line-height="1.8">
          {{ changelog }}
        </mj-text>

        {% if breaking_changes %}
        <mj-spacer height="20px" />
        <mj-section background-color="#fef3c7" border-radius="8px" padding="15px">
          <mj-column>
            <mj-text font-size="14px" color="#92400e" font-weight="bold">
              ⚠️ Alterações que quebram a compatibilidade
            </mj-text>
            <mj-text font-size="14px" color="#92400e" line-height="1.6">
              {{ breaking_changes }}
            </mj-text>
          </mj-column>
        </mj-section>
        {% endif %}

        <mj-spacer height="30px" />

        <mj-button href="{{ update_url }}" background-color="{{ theme.color.primary|default:'#2563eb' }}" color="#ffffff" font-size="16px" font-weight="bold" border-radius="6px" padding="15px 40px">
          Instalar Atualização
        </mj-button>

        <mj-spacer height="20px" />

        <mj-text font-size="14px" color="{{ theme.color.text_secondary|default:'#6b7280' }}" align="center">
          <a href="{{ changelog_url }}" style="color: {{ theme.color.primary|default:'#2563eb' }};">
            Ver Registro Completo de Alterações
          </a>
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
</mjml>

## Text Content
📦 ATUALIZAÇÃO DISPONÍVEL

Nova Versão Disponível

Uma nova versão do {{ component_name }} está disponível para sua loja Spwig.

DETALHES DA ATUALIZAÇÃO:
- Componente: {{ component_name }}
- Versão Atual: {{ current_version }}
- Nova Versão: {{ new_version }}
- Data de Lançamento: {{ release_date }}

O QUE HÁ DE NOVO:
{{ changelog }}

{% if breaking_changes %}
⚠️ ALTERAÇÕES QUE QUEBRAM A COMPATIBILIDADE:
{{ breaking_changes }}
{% endif %}

Instalar atualização: {{ update_url }}
Ver registro completo de alterações: {{ changelog_url }}