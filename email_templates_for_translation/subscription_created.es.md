...
## Subject
✅ Tu suscripción a {{ plan_name }} está activa - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Monto:</strong> {{ subscription_amount }} / {{ billing_cycle }}
              </mj-text>

              {% if trial_period %}
              <mj-text font-size="14px" color="{{ theme.color.success|default:'#10b981' }}" padding="5px 0">
                <strong>Periodo de prueba:</strong> {{ trial_period }} días (GRATIS)
              </mj-text>
              <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="5px 0">
                <strong>Primer cobro:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Tu suscripción ahora está activa
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Accede inmediatamente a tus productos y beneficios
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Gestiona tu suscripción en cualquier momento desde tu cuenta
        </mj-text>
      </mj-column>
    </mj-section>
...
DETALLES DE LA SUSCRIPCIÓN:
Plan: {{ plan_name }}
{% if product_name %}Producto: {{ product_name }}
{% endif %}Monto: {{ subscription_amount }} / {{ billing_cycle }}
{% if trial_period %}Periodo de prueba: {{ trial_period }} días (GRATIS)
Primer cobro: {{ next_billing_date|date:"F d, Y" }}
{% else %}Próximo cobro: {{ next_billing_date|date:"F d, Y" }}
{% endif %}Método de pago: {{ payment_method }}
...
Gestiona la Suscripción: {{ manage_subscription_url }}
