...
## Subject
⏰ Ihr {{ plan_name }}-Test endet in {{ days_remaining }} Tagen - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Ihr {{ plan_name }}-Test endet am {{ trial_end_date|date:"F d, Y" }}
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Test endet:</strong> {{ trial_end_date|date:"F d, Y at g:i A" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Erste Abbuchung:</strong> {{ subscription_amount }} am {{ trial_end_date|date:"F d, Y" }}
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Ihre Abonnement wird nach Ablauf des Tests automatisch fortgesetzt
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">•</span>
          Wir werden {{ subscription_amount }} von Ihrer {{ payment_method }} abheben
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">•</span>
          Sie können das Abonnement jederzeit vor {{ trial_end_date|date:"F d, Y" }} ohne Kosten kündigen
        </mj-text>
      </mj-column>
    </mj-section>
...
TEST-INFO:
{{ days_remaining }} Tage verbleiben

Test endet: {{ trial_end_date|date:"F d, Y at g:i A" }}
Erste Abbuchung: {{ subscription_amount }} am {{ trial_end_date|date:"F d, Y" }}
Zahlungsmethode: {{ payment_method }}

Was passiert als Nächstes?
• Ihr Abonnement wird nach Ablauf des Tests automatisch fortgesetzt
• Wir werden {{ subscription_amount }} von Ihrer {{ payment_method }} abheben
• Sie können das Abonnement jederzeit vor {{ trial_end_date|date:"F d, Y" }} ohne Kosten kündigen

Abonnement verwalten: {{ manage_subscription_url }}
Vor Ablauf des Tests kündigen: {{ cancel_subscription_url }}
