...
## Subject
✅ {{ plan_name }} Aboneliğiniz Aktif - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              {% if product_name %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Ürün:</strong> {{ product_name }}
              </mj-text>
              {% endif %}

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Tutar:</strong> {{ subscription_amount }} / {{ billing_cycle }}
              </mj-text>

              {% if trial_period %}
              <mj-text font-size="14px" color="{{ theme.color.success|default:'#10b981' }}" padding="5px 0">
                <strong>Başlangıç Dönemi:</strong> {{ trial_period }} gün (ÜCRETSİZ)
              </mj-text>
              <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="5px 0">
                <strong>Birinci Ödeme:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Sonraki Ödeme:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% endif %}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Abonelik artık aktif
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Ürünlerinize ve faydalarınıza hemen erişim sağlayın
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">✓</span>
          Hesabınızda her zaman aboneliğinizi yönetin
        </mj-text>
      </mj-column>
    </mj-section>
...
ABONELİK DETAYLARI:
Plan: {{ plan_name }}
{% if product_name %}Ürün: {{ product_name }}
{% endif %}Tutar: {{ subscription_amount }} / {{ billing_cycle }}
{% if trial_period %}Başlangıç Dönemi: {{ trial_period }} gün (ÜCRETSİZ)
Birinci Ödeme: {{ next_billing_date|date:"F d, Y" }}
{% else %}Sonraki Ödeme: {{ next_billing_date|date:"F d, Y" }}
{% endif %}Ödeme Yöntemi: {{ payment_method }}
