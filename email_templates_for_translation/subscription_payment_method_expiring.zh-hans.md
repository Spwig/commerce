          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.warning|default:'#f59e0b' }}" padding="5px 0">
                <strong>过期时间:</strong> {{ expiration_date|date:"m/Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>下次账单:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" padding="5px 0" font-weight="600">
                ⚠️ 请在 {{ expiration_date|date:"m/Y" }} 之前更新您的支付方式，以避免服务中断。
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
...
订阅: {{ plan_name }}
支付方式: {{ payment_method }}
过期时间: {{ expiration_date|date:"m/Y" }}
下次账单: {{ next_billing_date|date:"F d, Y" }}
