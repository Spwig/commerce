...
## Subject
新 {{ form_name }} 提交來自 {{ submitter_name }}
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="20px">
          <mj-column>
            ...
            <mj-text color="{{ theme.color.text|default:'#4b5563' }}">
              <strong>表單：</strong> {{ form_name }}<br/>
              <strong>提交者：</strong> {{ submitter_name }}<br/>
              <strong>電子郵件：</strong> {{ submitter_email }}<br/>
              <strong>提交時間：</strong> {{ submission_date }}<br/>
              <strong>參考編號：</strong> {{ submission_id }}
            </mj-text>
          </mj-column>
        </mj-section>
...
- 表單： {{ form_name }}
- 提交者： {{ submitter_name }}
- 電子郵件： {{ submitter_email }}
- 提交時間： {{ submission_date }}
- 參考編號： {{ submission_id }}
...
在管理員中查看： {{ admin_submission_url }}
{% if reply_to_email %}回覆提交者： mailto:{{ reply_to_email }}{% endif %}