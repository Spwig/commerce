---
template_type: blog_comment_approved
category: Blog
---

# Email Template: blog_comment_approved

## Subject
Your comment on "{{ post_title }}" has been approved - {{ blog_name }}

## HTML Content
<mjml>
  <mj-body>
    <mj-section background-color="#f0fdf4">
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="#059669" align="center">
          ✓ Comment Approved
        </mj-text>
      </mj-column>
    </mj-section>

    <mj-section>
      <mj-column>
        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          Hi {{ commenter_name }},
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          Great news! Your comment on "{{ post_title }}" has been approved and is now visible to other readers.
        </mj-text>

        <mj-spacer height="20px" />

        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="20px">
          <mj-column>
            <mj-text font-size="14px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
              Your Comment:
            </mj-text>
            <mj-text font-size="14px" color="{{ theme.color.text|default:'#4b5563' }}" line-height="1.6">
              {{ comment_text }}
            </mj-text>
          </mj-column>
        </mj-section>

        <mj-spacer height="30px" />

        <mj-button href="{{ comment_url }}" background-color="{{ theme.color.primary|default:'#2563eb' }}" color="#ffffff" font-size="16px" border-radius="6px" padding="12px 30px">
          View Your Comment
        </mj-button>

        <mj-spacer height="20px" />

        <mj-text font-size="14px" color="{{ theme.color.text_secondary|default:'#6b7280' }}">
          We'll notify you when someone replies to your comment.
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-body>
</mjml>

## Text Content
✓ COMMENT APPROVED

Hi {{ commenter_name }},

Great news! Your comment on "{{ post_title }}" has been approved and is now visible to other readers.

YOUR COMMENT:
{{ comment_text }}

View your comment: {{ comment_url }}

We'll notify you when someone replies to your comment.

## Variables

| Variable | Description | Example |
|----------|-------------|---------|
| commenter_name | Commenter's name | John |
| post_title | Blog post title | 10 Tips for Better Sleep |
| comment_text | Comment content | Great article! These tips really helped me... |
| comment_url | Direct comment link | https://shop.com/en/blog/sleep-tips#comment-123 |
| blog_name | Blog name | Health Blog |

## Notes

- Transactional notification
- Sent when comment approved by moderator
- Sets expectation for reply notifications
