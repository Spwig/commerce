...
## Subject
Yêu cầu chỉnh sửa: {{ component_name }} v{{ version }}
...
<mjml>
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.info|default:'#3b82f6' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.background|default:'#ffffff' }}" align="center" padding-top="10px">
          Một số thay đổi cần thực hiện trước khi được phê duyệt
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="20px">
          Đội ngũ đánh giá của chúng tôi đã yêu cầu một số thay đổi đối với thành phần của bạn trước khi có thể được phê duyệt.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="25px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          Vui lòng xem xét phản hồi và gửi lại phiên bản đã cập nhật.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if review_notes %}Yêu cầu chỉnh sửa:
{{ review_notes }}{% endif %}

Vui lòng xem xét phản hồi và gửi lại phiên bản đã cập nhật.

Xem chi tiết gửi nộp: {{ submission_url }}
