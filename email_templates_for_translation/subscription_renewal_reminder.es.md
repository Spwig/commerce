...
## Subject
🔔 Tu suscripción {{ plan_name }} se renueva en {{ days_until_renewal }} días - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Fecha de renovación:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          No se requiere ninguna acción. Tu suscripción se renovará automáticamente el {{ next_billing_date|date:"F d, Y" }}.
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="15px 20px 0 20px" line-height="1.6" align="center">
          ¿Necesitas actualizar tu método de pago o hacer cambios? Visita tu panel de control de cuenta.
        </mj-text>
      </mj-column>
    </mj-section>
...
Fecha de renovación: {{ next_billing_date|date:"F d, Y" }}
