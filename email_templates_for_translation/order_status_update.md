...
## Subject
Order #{{ order_number }} - Status Update: {{ new_status_display }}
  ...
  <mj-body>
    ...
    <mj-section>
      <mj-column>
        ...
        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          Your order <strong>#{{ order_number }}</strong> status has been updated.
        </mj-text>
...
| old_status_display | Human-readable previous status | Processing |
| new_status_display | Human-readable new status | On Hold |
| order_url | Link to the order status page | https://shop.com/orders/ORD-12345/ |

## Notes

- Transactional email - sent when merchant manually changes order status with "Send notification" checked
- Generic template covering all status transitions
- The signal-based emails (shipping_confirmation, delivery_confirmation, refund_notification) fire automatically on specific transitions; this template is for manual admin-triggered notifications
