...
## Subject
Как прошёл ваш покупка? Оставьте отзыв
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="10px" line-height="1.8">
          Надеемся, что вы наслаждаетесь вашей последней покупкой! Ваше мнение помогает нам улучшаться и помогает другим покупателям принимать обоснованные решения.
        </mj-text>
      </mj-column>
    </mj-section>
...
- {{ item.name }} (Количество: {{ item.quantity }})
  Написать отзыв: {{ item.review_url }}
{% endfor %}

{% if incentive_offered %}
🎁 Бонусное вознаграждение
{{ incentive_details }}
{% if incentive_code %}Код: {{ incentive_code }}{% endif %}
{% endif %}
