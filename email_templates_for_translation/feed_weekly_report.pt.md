...
## Subject
📊 Relatório Semanal de Alimentação de Produtos - {{ week_range }}
...
## Text Content
📊 PERFORMANCE SEMANAL DE FEED

Resumo do desempenho do feed

Aqui está como os seus feeds de produtos se saíram de {{ week_range }}.

ESTATÍSTICAS GERAIS:
- Total de Feeds: {{ total_feeds }}
- Feeds Ativos: {{ active_feeds }}
- Total de Sincronizações: {{ total_syncs }}
- Sincronizações Bem-sucedidas: {{ successful_syncs }} ({{ success_rate }}%)
- Sincronizações com Falha: {{ failed_syncs }}

DESEMPENHO DE FEED POR FEED:
{% for feed in feed_stats %}
{{ feed.name }}
Plataforma: {{ feed.platform }}
Sincronizações: {{ feed.sync_count }} ({{ feed.success_count }} bem-sucedidas)
Produtos: {{ feed.product_count }}
{% if feed.errors > 0 %}Erros: {{ feed.errors }}{% endif %}
...
{% for error in top_errors %}
{{ error.type }}: {{ error.count }} ocorrências
{% endfor %}
{% endif %}
...
Ver painel de feeds: {{ feeds_dashboard_url }}