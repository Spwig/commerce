...
## Subject
专属 {{ discount_percentage }}% 折扣！- {{ shop_name }}
    ...
    <mj-section background-color="#dcfce7">
      <mj-column>
        ...
        <mj-text font-size="18px" color="#047857" align="center">
          您的购物车可享 {{ discount_percentage }}% 折扣
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section>
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          我们想让这变得简单，{{ customer_name }}
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          立即完成购买，使用代码 <strong>{{ discount_code }}</strong> 可节省 {{ discount_percentage }}%
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="15px">
          <mj-column>
            <mj-text align="right">
              <span style="color: {{ theme.color.text_secondary|default:'#6b7280' }};">小计：</span> <span style="text-decoration: line-through; color: #9ca3af;">{{ cart_total }}</span>
            </mj-text>
            <mj-text align="right">
...
## Text Content
🎉 专属于您的特别优惠！
{{ discount_percentage }}% 折扣，您的购物车

我们想让这变得简单，{{ customer_name }}

立即完成购买，使用代码 {{ discount_code }} 可节省 {{ discount_percentage }}%
...
领取您的 {{ discount_percentage }}% 折扣：{{ cart_url }}
