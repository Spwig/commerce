...
<mjml>
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="20px">
          あなたのSpwig開発者アカウントが一時停止されました。この期間中、公開されたコンポーネントは引き続き利用可能ですが、新しいコンポーネントや更新の提出はできません。
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          エラーでこの処理が行われたとお考えの場合、<a href="mailto:{{ support_email }}" style="color: {{ theme.color.primary|default:'#2563eb' }}; text-decoration: none;">
          {{ support_email }}
        </a>までご連絡ください。
        </mj-text>
      </mj-column>
...
{% if suspension_reason %}理由: {{ suspension_reason }}{% endif %}

エラーでこの処理が行われたとお考えの場合、{{ support_email }}までご連絡ください。
