...
## Subject
カートから{{ discount_percentage }}%割引！- {{ shop_name }}
    ...
    <mj-section>
      <mj-column>
        <mj-text font-size="20px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}">
          あなたのために簡単にしています、{{ customer_name }}
        </mj-text>

        <mj-text font-size="16px" line-height="1.6" color="{{ theme.color.text|default:'#4b5563' }}">
          今すぐ購入を完了して、コード<strong>{{ discount_code }}</strong>で{{ discount_percentage }}%節約してください
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="15px">
          <mj-column>
            ...
            <mj-text align="right">
              <span style="color: {{ theme.color.text_secondary|default:'#6b7280' }};">割引 ({{ discount_percentage }}%):</span> <span style="color: #059669; font-weight: bold;">-{{ discount_amount }}</span>
            </mj-text>
            <mj-text font-size="20px" font-weight="bold" align="right" color="{{ theme.color.text|default:'#1f2937' }}">
              新しい合計: {{ discounted_total }}
            </mj-text>
          </mj-column>
        </mj-section>
...
今すぐ購入を完了して、コード{{ discount_code }}で{{ discount_percentage }}%節約してください
...
小計: {{ cart_total }}
割引 ({{ discount_percentage }}%): -{{ discount_amount }}
NEW TOTAL: {{ discounted_total }}

{{ discount_percentage }}%割引を獲得: {{ cart_url }}
