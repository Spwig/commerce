...
## Subject
✓ Feed Produk Telah Dibuat: {{ feed_name }}
    ...
    <mj-section>
      <mj-column>
        ...
        <mj-text font-size="15px" color="{{ theme.color.text|default:'#4b5563' }}">
          Feed produk {{ feed_name }} Anda telah berhasil dibuat dan siap digunakan.
        </mj-text>
        ...
        <mj-section background-color="{{ theme.color.surface|default:'#f9fafb' }}" border-radius="8px" padding="20px">
          <mj-column>
            ...
            <mj-text color="{{ theme.color.text|default:'#4b5563' }}">
              ...
              <strong>Produk:</strong> {{ product_count }}<br/>
              <strong>Dibuat:</strong> {{ generated_at }}<br/>
              <strong>Ukuran File:</strong> {{ file_size }}
            </mj-text>
          </mj-column>
        </mj-section>
        ...
        <mj-section background-color="#fef3c7" border-radius="8px" padding="15px">
          <mj-column>
            <mj:text font-size="14px" color="#92400e" font-weight="bold">
              ⚠️ {{ warnings_count }} Peringatan{{ warnings_count|pluralize }}
            </mj:text>
            <mj-text font-size="14px" color="#92400e">
              Beberapa produk memiliki masalah kualitas data. Periksa peringatan di panel admin.
            </mj-text>
          </mj-column>
        </mj-section>
        {% endif %}
...
- Feed: {{ feed_name }}
- Format: {{ feed_format }}
- Produk: {{ product_count }}
- Dibuat: {{ generated_at }}
- Ukuran File: {{ file_size }}

{% if warnings_count > 0 %}
⚠️ {{ warnings_count }} PERINGATAN{{ warnings_count|pluralize|upper }}:
Beberapa produk memiliki masalah kualitas data. Periksa peringatan di panel admin.
{% endif %}
...
Unduh feed: {{ download_url }}
Lihat di admin: {{ admin_feed_url }}