...
## Subject
✅ Paiement reçu pour {{ plan_name }} - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Date de paiement:</strong> {{ payment_date|date:"F d, Y" }}
              </mj-text>
              ...
              {% if transaction_id %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Identifiant de transaction:</strong> {{ transaction_id }}
              </mj-text>
              {% endif %}
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Date de prochaine facturation:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Votre abonnement reste actif et vous continuez à avoir accès à l'ensemble des avantages.
        </mj-text>
      </mj-column>
    </mj-section>
...
Date de paiement: {{ payment_date|date:"F d, Y" }}
Méthode de paiement: {{ payment_method }}
{% if transaction_id %}Identifiant de transaction: {{ transaction_id }}
{% endif %}Date de prochaine facturation: {{ next_billing_date|date:"F d, Y" }}

Votre abonnement reste actif et vous continuez à avoir accès à l'ensemble des avantages.

Voir l'abonnement: {{ manage_subscription_url }}
Télécharger le reçu: {{ download_receipt_url }}
