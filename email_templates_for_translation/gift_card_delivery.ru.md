...
## Subject
🎁 Подарочный сертификат от {{ sender_name }} - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        <mj-text font-size="32px" font-weight="bold" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          🎁 Вы получили подарочный сертификат!
        </mj-text>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              <mj-text font-size="14px" color="#0c4a6e" align="center" text-transform="uppercase" letter-spacing="1px" font-weight="600" padding-bottom="10px">
                Ваш код подарочного сертификата
              </mj-text>
              ...
              <mj-text font-size="13px" color="{{ theme.color.error|default:'#ef4444' }}" align="center" padding-top="10px">
                ⏰ Истекает: {{ gift_card.expires_at|date:"F d, Y" }}
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Ознакомьтесь с нашими товарами и добавьте товары в корзину
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">2.</span>
          Перейдите к оформлению заказа и введите код подарочного сертификата
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">3.</span>
          Баланс подарочного сертификата будет применён к вашему заказу
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="20px">
      <mj-column>
        <mj-text font-size="11px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" line-height="1.5">
          Подарочные сертификаты не подлежат возврату и не могут быть обменены на наличные.
          {% if not gift_card.expires_at %}Этот подарочный сертификат никогда не истекает.{% endif %}
          Подарочные сертификаты не могут быть использованы для покупки других подарочных сертификатов.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if gift_card.sender_name %}От: {{ gift_card.sender_name }}

{% endif %}{% if gift_card.message %}Личное сообщение:
"{{ gift_card.message }}"

{% endif %}ВАШ КОД ПОДАРОЧНОГО СЕРТИФИКАТА:
{{ gift_card.code }}

Баланс подарочного сертификата:
{{ gift_card.current_balance.amount }} {{ gift_card.current_balance.currency }}

{% if gift_card.expires_at %}Истекает: {{ gift_card.expires_at|date:"F d, Y" }}
{% endif %}

Как использовать подарочный сертификат:
1. Ознакомьтесь с нашими товарами и добавьте товары в корзину
2. Перейдите к оформлению заказа и введите код подарочного сертификата
3. Баланс подарочного сертификата будет применён к вашему заказу
...
Подарочные сертификаты не подлежат возврату и не могут быть обменены на наличные.{% if not gift_card.expires_at %} Этот подарочный сертификат никогда не истекает.{% endif %} Подарочные сертификаты не могут быть использованы для покупки других подарочных сертификатов.
