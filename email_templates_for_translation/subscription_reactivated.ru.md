...
## Subject
Добро пожаловать обратно! Ваша подписка на {{ plan_name }} восстановлена - {{ shop_name }}
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Восстановлено:</strong> {{ reactivation_date|date:"F d, Y" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Следующая оплата:</strong> {{ new_billing_date|date:"F d, Y" }}
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.success|default:'#10b981' }}; font-size: 18px; margin-right: 8px;">&#10003;</span>
          Полный доступ ко всем вашим преимуществам восстановлен
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.success|default:'#10b981' }}; font-size: 18px; margin-right: 8px;">&#10003;</span>
          Ваша первая оплата будет {{ new_billing_date|date:"F d, Y" }}
        </mj-text>
      </mj-column>
    </mj-section>
...
ДЕТАЛИ ПОДПИСКИ:
План: {{ plan_name }}
Восстановлено: {{ reactivation_date|date:"F d, Y" }}
Следующая оплата: {{ new_billing_date|date:"F d, Y" }}
