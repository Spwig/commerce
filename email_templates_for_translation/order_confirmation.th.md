...
## Subject
ยืนยันการสั่งซื้อ #{{ order_number }} - ขอบคุณ!
    ...
    <mj-section background-color="{{ theme.color.info_light|default:'#dbeafe' }}" padding="20px">
      <mj-column>
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          <strong>ระยะเวลาการจัดส่งที่คาดการณ์:</strong> {{ estimated_delivery_start }} - {{ estimated_delivery_end }}
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px 20px 10px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="5px">
          ติดตามคำสั่งซื้อของคุณ ชำระเงินเร็วขึ้นในครั้งต่อไป และจัดการตัวเลือกของคุณ
        </mj-text>
      </mj-column>
    </mj-section>
...
ระยะเวลาการจัดส่งที่คาดการณ์: {{ estimated_delivery_start }} - {{ estimated_delivery_end }}

รายการสินค้า:
{% for item in items %}
- {{ item.name }} (จำนวน: {{ item.quantity }}) - {{ item.subtotal }}
{% endfor %}
...
ดูสถานะการสั่งซื้อ: {{ order_url }}
{% if activation_url %}

สร้างบัญชีของคุณ
ติดตามคำสั่งซื้อของคุณ ชำระเงินเร็วขึ้นในครั้งต่อไป และจัดการตัวเลือกของคุณ
สร้างบัญชี: {{ activation_url }}
{% endif %}
