  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          グレートニュース！{{ reason }} により、あなたは {{ points }} ポイントを獲得しました。
        </mj-text>

        <!-- Balance -->
        <mj-divider border-color="{{ theme.color.border|default:'#e5e7eb' }}" padding="20px 0" />
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}">
          <strong>総ポイント：</strong> {{ total_points }}
        </mj-text>
...
{% if suggested_reward %}今から交換可能です： {{ suggested_reward }}{% endif %}

リワードを確認する： {{ rewards_url }}
