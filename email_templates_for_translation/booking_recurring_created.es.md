...
## Subject
Serie de Reserva Recurrente Creada - {{ product_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="16px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          Hola {{ customer_name }}, tu serie de reservas recurrentes ha sido configurada.
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}">
          <strong>Programa:</strong> {{ recurrence_description }}
        </mj-text>
        {% endif %}
        {% if first_date %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}">
          <strong>Primera Reserva:</strong> {{ first_date }}
        </mj-text>
        {% endif %}
        {% if total_bookings %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}">
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding-top="10px">
          <strong>Costo por Reserva:</strong> {{ total_cost }}
        </mj-text>
        {% endif %}
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px">
      <mj-column>
        <mj-text font-size="13px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center">
          Puedes gestionar reservas individuales de esta serie desde tu cuenta.
        </mj-text>
      </mj-column>
    </mj-section>
...
{% if recurrence_description %}Programa: {{ recurrence_description }}{% endif %}
{% if first_date %}Primera Reserva: {{ first_date }}{% endif %}
{% if total_bookings %}Total de Reservas: {{ total_bookings }}{% endif %}
{% if resource_name %}Recurso: {{ resource_name }}{% endif %}
{% if total_cost %}Costo por Reserva: {{ total_cost }}{% endif %}
