...
## Subject
⏰ {{ plan_name }} 시험 기간이 {{ days_remaining }}일 남았습니다 - {{ shop_name }}
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="40px 20px">
      <mj-column>
        ...
        <mj-text font-size="18px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="10px">
          {{ plan_name }} 시험 기간은 {{ trial_end_date|date:"F d, Y" }}에 끝납니다
        </mj-text>
      </mj-column>
    </mj-section>
          ...
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>시험 기간 종료:</strong> {{ trial_end_date|date:"F d, Y at g:i A" }}
              </mj-text>

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>첫 결제:</strong> {{ subscription_amount }} {{ trial_end_date|date:"F d, Y" }}에
              </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          시험 기간 후 자동으로 구독이 이어집니다
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">•</span>
          {{ subscription_amount }}을 {{ payment_method }}로 자동으로 결제합니다
        </mj-text>

        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.info|default:'#3b82f6' }}; font-size: 18px; margin-right: 8px;">•</span>
          {{ trial_end_date|date:"F d, Y" }} 전에는 아무 비용 없이 언제든지 취소할 수 있습니다
        </mj-text>
      </mj-column>
    </mj-section>
...
시험 기간 종료: {{ trial_end_date|date:"F d, Y at g:i A" }}
첫 결제: {{ subscription_amount }} {{ trial_end_date|date:"F d, Y" }}에
...
시험 기간 종료 전에 취소하기: {{ cancel_subscription_url }}
