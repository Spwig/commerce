...
## Subject
❌ Langganan {{ plan_name }} Anda Dibatalkan - {{ shop_name }}
...
<mjml>
  ...
  <mj-body background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}">
        ...
        <mj-wrapper background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px" border="2px solid {{ theme.color.text_muted|default:'#6b7280' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Dibatalkan Pada:</strong> {{ cancellation_date|date:"F d, Y" }}
              </mj-text>

              {% if access_until %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Akses Sampai:</strong> {{ access_until|date:"F d, Y" }}
              </mj-text>
              <mj-text font-size="13px" color="{{ theme.color.success|default:'#10b981' }}" padding="10px 0 5px 0">
                ✓ Anda masih memiliki akses ke manfaat Anda sampai {{ access_until|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Status:</strong> Dibatalkan segera
              </mj-text>
              {% endif %}
            </mj-column>
          </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          ...
          Anda tidak akan dikenakan biaya lagi
        </mj-text>

        {% if access_until %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          Anda masih dapat terus menggunakan manfaat Anda sampai {{ access_until|date:"F d, Y" }}
        </mj-text>
        {% else %}
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="8px 20px" line-height="1.6">
          <span style="color: {{ theme.color.text_muted|default:'#6b7280' }}; font-size: 18px; margin-right: 8px;">•</span>
          Akses Anda telah berakhir segera
        </mj-text>
        {% endif %}
    ...
    <mj-section background-color="{{ theme.color.background_secondaryondary|default:'#f9fafb' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px 15px 20px" line-height="1.6" align="center">
          Kami ingin mendengar masukan Anda untuk membantu kami meningkatkan.
        </mj-text>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="10px 20px 30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px 15px 20px" line-height="1.6" align="center">
          Berubah pikiran? Anda dapat mengaktifkan kembali langganan Anda kapan saja.
        </mj-text>
...
RINGKASAN PEMBATALAN:
Plan: {{ plan_name }}
Dibatalkan Pada: {{ cancellation_date|date:"F d, Y" }}
{% if access_until %}Akses Sampai: {{ access_until|date:"F d, Y" }}

✓ Anda masih memiliki akses ke manfaat Anda sampai {{ access_until|date:"F d, Y" }}
{% else %}Status: Dibatalkan segera
{% endif %}

Artinya Ini:
• Anda tidak akan dikenakan biaya lagi
{% if access_until %}• Anda masih dapat terus menggunakan manfaat Anda sampai {{ access_until|date:"F d, Y" }}
{% else %}• Akses Anda telah berakhir segera
{% endif %}• Anda dapat mengaktifkan kembali kapan saja

Kami ingin mendengar masukan Anda untuk membantu kami meningkatkan.
Beri Masukan: {{ feedback_url }}

Berubah pikiran? Anda dapat mengaktifkan kembali langganan Anda kapan saja.
Aktifkan Kembali Langganan: {{ reactivate_url }}
