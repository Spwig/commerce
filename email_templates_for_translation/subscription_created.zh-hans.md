        ...
        <mj-wrapper background-color="#f0f9ff" padding="30px" border="2px solid #0ea5e9" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              {% if product_name %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>产品：</strong> {{ product_name }}
              </mj-text>
              {% endif %}

              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>金额：</strong> {{ subscription_amount }} / {{ billing_cycle }}
              </mj-text>

              {% if trial_period %}
              <mj-text font-size="14px" color="{{ theme.color.success|default:'#10b981' }}" padding="5px 0">
                <strong>试用期：</strong> {{ trial_period }} 天 (免费)
              </mj-text>
              <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" padding="5px 0">
                <strong>首次收费：</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% else %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>下次收费：</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
              {% endif %}
...
订阅详情：
计划： {{ plan_name }}
{% if product_name %}产品： {{ product_name }}
{% endif %}金额： {{ subscription_amount }} / {{ billing_cycle }}
{% if trial_period %}试用期： {{ trial_period }} 天 (免费)
首次收费： {{ next_billing_date|date:"F d, Y" }}
{% else %}下次收费： {{ next_billing_date|date:"F d, Y" }}
{% endif %}支付方式： {{ payment_method }}
