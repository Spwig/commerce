...
## Subject
✅ Pembayaran Diterima untuk {{ plan_name }} - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Tanggal Pembayaran:</strong> {{ payment_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Tanggal Tagihan Berikutnya:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Langganan Anda tetap aktif dan Anda terus memiliki akses penuh ke semua manfaat.
        </mj-text>
      </mj-column>
    </mj-section>
...
Tanggal Pembayaran: {{ payment_date|date:"F d, Y" }}
Cara Pembayaran: {{ payment_method }}
{% if transaction_id %}ID Transaksi: {{ transaction_id }}
{% endif %}
Tanggal Tagihan Berikutnya: {{ next_billing_date|date:"F d, Y" }}

Langganan Anda tetap aktif dan Anda terus memiliki akses penuh ke semua manfaat.

Lihat Langganan: {{ manage_subscription_url }}
Unduh Struk: {{ download_receipt_url }}
