...
## Subject
Xác nhận đơn hàng #{{ order_number }} - Cảm ơn bạn!
    ...
    <mj-section background-color="{{ theme.color.info_light|default:'#dbeafe' }}" padding="20px">
      <mj-column>
        <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" align="center">
          <strong>Thời gian giao hàng dự kiến:</strong> {{ estimated_delivery_start }} - {{ estimated_delivery_end }}
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="20px 20px 10px">
      <mj-column>
        ...
        <mj-text font-size="14px" color="{{ theme.color.text_muted|default:'#6b7280' }}" align="center" padding-top="5px">
          Theo dõi đơn hàng, thanh toán nhanh hơn lần sau và quản lý sở thích của bạn.
        </mj-text>
      </mj-column>
    </mj-section>
...
Thời gian giao hàng dự kiến: {{ estimated_delivery_start }} - {{ estimated_delivery_end }}

Đơn hàng:
{% for item in items %}
- {{ item.name }} (Số lượng: {{ item.quantity }}) - {{ item.subtotal }}
{% endfor %}
