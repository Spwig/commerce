...
## Subject
✅ Recibo de pago para {{ plan_name }} - {{ shop_name }}
        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Fecha de pago:</strong> {{ payment_date|date:"F d, Y" }}
              </mj-text>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>Próxima fecha de cobro:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
    ...
    <mj-section background-color="{{ theme.color.background|default:'#ffffff' }}" padding="30px 20px">
      <mj-column>
        <mj-text font-size="16px" color="{{ theme.color.text|default:'#1f2937' }}" padding="0 20px" line-height="1.6" align="center">
          Su suscripción sigue activa y continúa teniendo acceso completo a todos los beneficios.
        </mj-text>
      </mj-column>
    </mj-section>
...
Fecha de pago: {{ payment_date|date:"F d, Y" }}
Método de pago: {{ payment_method }}
{% if transaction_id %}ID de transacción: {{ transaction_id }}
{% endif %}
Próxima fecha de cobro: {{ next_billing_date|date:"F d, Y" }}

Su suscripción sigue activa y continúa teniendo acceso completo a todos los beneficios.

Ver suscripción: {{ manage_subscription_url }}
Descargar recibo: {{ download_receipt_url }}
