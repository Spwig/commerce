        ...
        <mj-wrapper background-color="#f0fdf4" padding="30px" border="2px solid {{ theme.color.success|default:'#10b981' }}" border-radius="12px">
          <mj-section background-color="transparent">
            <mj-column>
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>已付款金额:</strong> {{ amount_paid }}
              </mj-text>
              ...
              {% if transaction_id %}
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>交易编号:</strong> {{ transaction_id }}
              </mj-text>
              {% endif %}
              ...
              <mj-text font-size="14px" color="{{ theme.color.text|default:'#1f2937' }}" padding="5px 0">
                <strong>下次计费日期:</strong> {{ next_billing_date|date:"F d, Y" }}
              </mj-text>
            </mj-column>
          </mj-section>
        </mj-wrapper>
...
付款方式: {{ payment_method }}
{% if transaction_id %}交易编号: {{ transaction_id }}
{% endif %}
下次计费日期: {{ next_billing_date|date:"F d, Y" }}

您的订阅仍然有效，并且您可以继续享受所有权益。

查看订阅: {{ manage_subscription_url }}
下载收据: {{ download_receipt_url }}
