...
def domain_ssl_status(request):
    ...
    return JsonResponse({
        ...
        'cert': {
            ...
            'expires_at': config.cert_expires_at.isoformat() if config.cert_expires_at else None,
            'obtained_at': config.cert_obtained_at.isoformat() if config.cert_obtained_at else None,
...
def configure_domain_view(request):
    ...
    try:
        ...
        logger.warning(
            'Celery unavailable (%s), running domain config synchronously', e,
        )
...
def upload_cert_view(request):
    ...
    if not cert_pem or not key_pem:
        return JsonResponse({
            'error': 'Both cert_pem and key_pem are required'
        }, status=400)

    # Basic PEM validation
    if '-----BEGIN CERTIFICATE-----' not in cert_pem:
        return JsonResponse({
            'error': 'Invalid certificate format (expected PEM)'
        }, status=400)
    if '-----BEGIN' not in key_pem or 'KEY-----' not in key_pem:
        return JsonResponse({
            'error': 'Invalid key format (expected PEM)'
        }, status=400)
        ...
        if cert_info:
            ...
            if config.domain and cert_domain:
                if cert_domain.startswith('*.'):
                    ...
                    domain_mismatch = not config.domain.endswith(
                        f'.{wildcard_base}'
        ...
        if config.domain and config.ssl_mode in _LOCAL_CERT_MODES:
            try:
                nginx_service.write_ssl_config(
                    config.domain, ssl_mode=config.ssl_mode,
                )
                logger.info(
                ...
                logger.warning(
                    'Failed to write SSL NGINX config after cert upload: %s', e,
                )
...
def custom_domain_verify(request):
    ...
    response, error = _update_server_request(
        'POST', f'instances/{store_slug}/domain/verify/',
...
def custom_domain_add(request):
    ...
    response, error = _update_server_request(
        'POST', f'instances/{store_slug}/domain/',
        json_data={'domain': domain},
    )
    if error:
        return JsonResponse({'error': error}, status=502)
...
def custom_domain_remove(request):
    ...
    response, error = _update_server_request(
        'DELETE', f'instances/{store_slug}/domain/',
    )
    if error:
        return JsonResponse({'error': error}, status=502)
