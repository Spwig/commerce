...
(function () {
  ...
  function init() {
    const dataEl = document.getElementById('domain-ssl-config-data');
    if (!dataEl) return;
    ...
    const domainInput = document.getElementById('domain-ssl-domain');
    if (domainInput) {
      if (domainInput.dataset.currentDomain) {
        ...
        try {
          const hostname = new URL(domainInput.dataset.siteUrl).hostname;
          if (
            hostname &&
            hostname !== 'localhost' &&
            hostname !== '127.0.0.1' &&
    ...
    const emailInput = document.getElementById('domain-ssl-email');
  ...
  function bindEvents() {
    // DNS check button
    const dnsBtn = document.getElementById('domain-ssl-check-dns');
    if (dnsBtn) {
      dnsBtn.addEventListener('click', checkDns);
    }

    // SSL mode radio cards
    const modeCards = document.querySelectorAll('.domain-ssl-mode-card');
    ...
    const radios = document.querySelectorAll('input[name="domain-ssl-mode"]');
    ...
    // Configure button
    const configBtn = document.getElementById('domain-ssl-configure');
    if (configBtn) {
  ...
  function onModeChange(mode) {
    ...
    document.querySelectorAll('.domain-ssl-conditional').forEach(function (el) {
      ...
      const modes = showFor ? showFor.split(',') : [];
  ...
  function loadStatus() {
    const statusEl = document.getElementById('domain-ssl-status');
    if (!statusEl) return;
  ...
  function renderStatus(data) {
    ...
    const domain = data.domain || t.noDomain || 'No domain configured';
    const sslLabel = data.ssl_mode_display || t.noSsl || 'None';
    let statusBadge = 'none';
    ...
    if (data.cert && data.cert.has_valid_cert) {
      certHtml =
        ...
        '<div class="domain-ssl-status-label">' +
        (t.certIssuer || 'Issued by') +
        '</div>' +
        '<div class="domain-ssl-status-value">' +
        escapeHtml(data.cert.issuer || '—') +
        '</div>' +
        '</div>' +
        '<div class="domain-ssl-status-item">' +
        '<div class="domain-ssl-status-label">' +
        (t.certExpires || 'Expires') +
        '</div>' +
        '<div class="domain-ssl-status-value">' +
        (data.cert.days_remaining != null
          ? data.cert.days_remaining + ' ' + (t.daysRemaining || 'days remaining')
          : '—') +
        '</div></div>';
    }
    ...
    let errorHtml = '';
    if (data.last_error) {
      const isWarning = data.last_error.indexOf('Warning:') === 0;
      const badgeClass = isWarning ? 'warning' : 'error';
      const labelText = isWarning ? t.warning || 'Warning' : t.error || 'Error';
      errorHtml =
        '<div class="domain-ssl-status-item" style="grid-column: 1 / -1;">' +
        '<div class="domain-ssl-status-label">' +
        labelText +
        '</div>' +
        '<div class="domain-ssl-status-value"><span class="status-badge ' +
        badgeClass +
        '">' +
    ...
    statusEl.innerHTML =
      '<div class="domain-ssl-status-grid">' +
      '<div class="domain-ssl-status-item">' +
  ...
  function checkDns() {
    ...
    if (!domain) {
      showDnsResult(
        ...
        '<ul class="messagelist"><li class="error">' +
          (t.domainRequired || 'Please enter a domain name') +
          '</li></ul>'
      );
      return;
    }

    const btn = document.getElementById('domain-ssl-check-dns');
    btn.disabled = true;
    showDnsResult(
      'info',
    ...
    fetchJSON(config.validateDnsUrl, {
      ...
      .then(function (data) {
        ...
        if (data.valid) {
          let items =
            '<li class="success"><i class="fas fa-check-circle"></i> ' +
            (t.validDns || 'DNS resolves correctly') +
            '</li>';

          if (data.resolved_ips && data.resolved_ips.length) {
            items +=
              '<li class="info"><i class="fas fa-info-circle"></i> IP: ' +
              escapeHtml(data.resolved_ips.join(', ')) +
              '</li>';
          }

          if (data.ip_match) {
            items +=
              '<li class="success"><i class="fas fa-check"></i> ' +
              (t.ipMatch || 'IP matches this server') +
              '</li>';
          } else if (data.server_ip) {
            items +=
              '<li class="warning"><i class="fas fa-exclamation-triangle"></i> ' +
  ...
  function showDnsResult(type, html) {
    const el = document.getElementById('domain-ssl-dns-result');
  ...
  function startConfiguration() {
    const domain = getDomain();
    const sslMode = getSelectedMode();
    const email = (document.getElementById('domain-ssl-email') || {}).value || '';
  ...
  function uploadCertThenConfigure(domain, sslMode, email) {
    const certPem = (document.getElementById('domain-ssl-cert-pem') || {}).value || '';
    const keyPem = (document.getElementById('domain-ssl-key-pem') || {}).value || '';
      ...
      .then(function (data) {
        ...
        if (data.is_complete) {
          ...
          if (
            configuredDomain &&
            configuredSslMode &&
            configuredSslMode !== 'none' &&
            configuredSslMode !== 'managed_externally'
          ) {
            const newUrl = 'https://' + configuredDomain + window.location.pathname;
            showResult(
              'success',
              '<i class="fas fa-check-circle"></i> ' +
                ...
                escapeHtml(newUrl) +
                '">' +
                (t.redirecting || 'Redirecting to secure site...') +
                '</a>'
            );
            setTimeout(function () {
          ...
          showResult(
            'success',
            '<i class="fas fa-check-circle"></i> ' + (t.success || 'Configuration complete!')
          );
          if (data.last_error) {
      ...
      .catch(function () {
        ...
        if (
          ...
          showResult(
            ...
            '<i class="fas fa-check-circle"></i> ' +
              ...
              escapeHtml(newUrl) +
              '">' +
              (t.openSecureSite || 'Open your secure site') +
              ' →</a>'
          );
          return;
        }
        ...
        showResult(
          ...
          '<i class="fas fa-exclamation-triangle"></i> ' +
            (t.connectionLost ||
              'Lost connection to the server. The configuration may have completed.') +
            ' <a href="javascript:location.reload()">' +
            (t.reloadPage || 'Reload this page') +
            '</a>'
        );
      });
  }
  ...
  function getDomain() {
    const el = document.getElementById('domain-ssl-domain');
    return el ? el.value.trim().toLowerCase() : '';
  }

  function getSelectedMode() {
    const radio = document.querySelector('input[name="domain-ssl-mode"]:checked');
    return radio ? radio.value : 'none';
  }

  function showProgress(status) {
    const el = document.getElementById('domain-ssl-progress');
    const fill = document.getElementById('domain-ssl-progress-fill');
    const text = document.getElementById('domain-ssl-progress-text');
  ...
  function showResult(type, html) {
    const el = document.getElementById('domain-ssl-result');
    if (!el) return;
  ...
  function showWarning(message) {
    ...
    let el = document.getElementById('domain-ssl-warning');
    if (!el) {
      // Create warning element after result
      const result = document.getElementById('domain-ssl-result');
      if (!result) return;
  ...
  function hideWarning() {
    const el = document.getElementById('domain-ssl-warning');
    if (el) {
  ...
  function disableActions() {
    const btn = document.getElementById('domain-ssl-configure');
