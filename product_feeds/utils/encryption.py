"""
Encryption utilities for securing product feed provider credentials.

Uses Django's SECRET_KEY for symmetric encryption via Fernet.
Credentials are encrypted before storing in FeedProviderAccount.credentials.
...
def _is_secret_field(field_name: str) -> bool:
    """
    Determine if a field should be treated as secret based on its name.

    Args:
        field_name: Credential field name

    Returns:
        True if field should be encrypted/masked
    """
    secret_keywords = [
        'key', 'secret', 'token', 'password', 'credential',
        'client_id', 'client_secret', 'api_key', 'access_token',
        'refresh_token', 'private_key', 'app_id', 'service_account',
        'json_key'
    ]
