...
def sync_all_feeds(self, site_id=None, force=False):
    ...
    try:
        ...
        site = Site.objects.get(pk=site_id) if site_id else Site.objects.get(pk=1)

        # Get accounts that need syncing
        now = timezone.now()

        accounts = FeedProviderAccount.objects.filter(site=site, is_active=True)

        if not force:
            # Only sync accounts that are due
            accounts = accounts.filter(next_sync_at__lte=now) | accounts.filter(
                next_sync_at__isnull=True
            )
...
def sync_feed(self, account_id, full_sync=False):
    ...
    try:
        ...
        # Generate feed
        feed_content = service.generate_feed(format=feed_format, save_to_db=True)
        ...
        try:
            ...
            account.save(
                update_fields=["sync_status", "sync_error_message", "last_error_at", "updated_at"]
            )
        except Exception:
            pass
...
def cleanup_expired_feeds(days_old=7):
    ...
    feeds_to_delete = ProductFeed.objects.filter(expires_at__lt=now) | ProductFeed.objects.filter(
...
def validate_provider_credentials(account_id):
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Provider credential validation failed: {account.name} - {result.get('message')}"
            )
...
def generate_feed_preview(account_id, format="xml", limit=10):
    ...
    try:
        ...
        product_ids = list(
            Product.objects.filter(is_active=True, status="published").values_list("id", flat=True)[
                :limit
            ]
        )
        ...
        feed_content = service.generate_feed(
            format=format, product_ids=product_ids, save_to_db=False
        )
