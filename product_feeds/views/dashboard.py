...
def product_feeds_dashboard(request):
    ...
    total_products = accounts.filter(is_active=True).aggregate(
        total=Sum('products_in_feed')
    )['total'] or 0
    ...
    attention_needed = accounts.filter(
        Q(sync_status='error') |
        Q(last_sync_at__isnull=True, is_active=True)
    ).select_related('component').order_by('-last_error_at', '-created_at')[:5]
    ...
    products_synced_24h = syncs_24h.filter(status='success').aggregate(
        total=Sum('products_synced')
    )['total'] or 0
