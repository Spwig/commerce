...
class ProviderBrowseView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            available_from_server = update_manager.list_available_components(component_type='product_feed_provider')
        ...
        installed_db = {p.slug: p.current_version for p in ComponentRegistry.objects.filter(component_type='product_feed_provider')}
...
class WizardStep1View(WizardSessionMixin, View):
                ...
                if provider_dir.exists():
                    ...
                    if manifest:
                        ...
                        if logo_filename:
                            ...
                            if logo_path.exists():
                                logo_url = static(f'product_feed_provider/{component.slug}/current/{logo_filename}')
    ...
    def post(self, request):
        ...
        try:
            component = ComponentRegistry.objects.get(id=component_id, component_type='product_feed_provider')
        except ComponentRegistry.DoesNotExist:
...
class WizardStep3View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        try:
            ...
            feed_format_choices = [(fmt, format_labels.get(fmt, fmt.upper())) for fmt in feed_formats]
    ...
    def post(self, request):
        ...
        for field_name, field_config in credential_schema.items():
            ...
            if field_config.get('required', False) and not value:
                errors.append(_('%(field)s is required.') % {'field': field_config.get('label', field_name)})
            ...
            if action == 'test':
                ...
                try:
                    ...
                    return JsonResponse({
                        ...
                        'redirect_url': reverse('admin:product_feeds_feedprovideraccount_change', args=[provider_account.id])
                    })
...
def download_feed(request, account_id):
    ...
    response['Content-Disposition'] = f'attachment; filename="{account.component.slug}_feed.{feed.feed_format}"'
...
def set_primary_ajax(request, account_id):
    ...
    FeedProviderAccount.objects.filter(
        site=account.site,
        is_primary=True
...
def bulk_action_ajax(request):
    ...
    if not action or not provider_ids:
        return JsonResponse({'success': False, 'message': 'Missing action or provider_ids'}, status=400)
    ...
    if action == 'enable':
        ...
        if count == 1:
            ...
            return JsonResponse({'success': False, 'message': 'Can only set one primary'}, status=400)
        ...
        names = list(accounts.values_list('name', flat=True))
        accounts.delete()
...
def install_provider_ajax(request, provider_slug):
    ...
    try:
        ...
        available_providers = update_manager.list_available_components(component_type='product_feed_provider')
