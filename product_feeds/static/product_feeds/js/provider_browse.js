    ...
    class ProviderInstaller {
        async install(providerSlug, providerName) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/product-feeds/providers/install/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
        ...
        updateCardToInstalled(providerSlug) {
            ...
            if (installBtn) {
                ...
                configureBtn.href = AdminUtils.buildAdminUrl('/admin/product-feeds/wizard/step1/');
    ...
    class ProviderUpdater {
        async update(providerSlug, providerVersion) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/product-feeds/providers/update/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
                        'X-CSRFToken': AdminUtils.getCsrfToken(),
                        'Content-Type': 'application/json',
        ...
        detailsButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                try {
                    ...
                    providerData.provider_type = 'product_feed';
