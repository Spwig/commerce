...
(function () {
  ...
  async function handleBulkAction() {
    ...
    if (action === 'delete') {
      if (
        !(await AdminModal.confirm({
          message: `Are you sure you want to delete ${providerIds.length} provider(s)?`,
          danger: true,
          confirmText: 'Delete',
        }))
      ) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl('/admin/product-feeds/admin/bulk-action/'),
        {
          method: 'POST',
          headers: {
  ...
  async function handleSync(e) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/product-feeds/admin/${providerId}/sync/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async function handleToggle(e) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/product-feeds/admin/${providerId}/toggle/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (data.success) {
        showNotification(
          data.message || `Provider ${isActive ? 'disabled' : 'enabled'}`,
          'success'
        );
  ...
  async function handleSetPrimary(e) {
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/product-feeds/admin/${providerId}/set-primary/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async function handleDelete(e) {
    ...
    const providerName = card
      ? card.querySelector('.provider-name a')?.textContent?.trim()
      : 'this provider';
    ...
    try {
      const response = await fetch(
        AdminUtils.buildAdminUrl(`/admin/product-feeds/admin/${providerId}/delete/`),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
