...
from product_feeds.models import FeedProviderAccount, ProductFeed, FeedSyncLog
...
class FeedProviderAccountAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('product_feeds/css/admin_provider_list.css',)
        }
    ...
    fieldsets = (
        ...
        (_('Configuration'), {
            'fields': ('is_active', 'is_primary', 'priority', 'config')
        }),
        (_('Credentials'), {
        ...
        (_('Sync Status'), {
            'fields': ('sync_status', 'sync_error_message', 'last_sync_at', 'next_sync_at', 'last_error_at'),
            'classes': ('collapse',)
        }),
        (_('Feed Statistics'), {
            'fields': ('products_in_feed', 'feed_url'),
            'classes': ('collapse',)
        }),
        (_('Timestamps'), {
    ...
    def products_count(self, obj):
        ...
        if obj.products_in_feed > 0:
            return format_html(
                '<span class="product-count-value">{:,}</span>',
                obj.products_in_feed
            )
        return '-'
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        for provider in all_providers:
            ...
            provider_dir = INTEGRATIONS_DIR / 'product_feed_provider' / provider.component.slug / version
            ...
            if manifest_path.exists():
                with open(manifest_path) as f:
                    ...
                    if logo_filename:
                        ...
                        if logo_path.exists():
                            logo_url = static(f'product_feed_provider/{provider.component.slug}/current/{logo_filename}')
        ...
        extra_context['total_products'] = all_providers.aggregate(total=Sum('products_in_feed'))['total'] or 0
...
class ProductFeedAdmin(admin.ModelAdmin):
    ...
    list_display = ['account', 'feed_format', 'product_count', 'file_size_display', 'generated_at', 'expires_at', 'expired_indicator', 'download_count']
    list_filter = ['feed_format', 'generated_at', 'account']
    search_fields = ['account__name', 'account__component__name']
    readonly_fields = ['generated_at', 'file_size', 'product_count', 'download_count', 'last_downloaded_at']
...
class FeedSyncLogAdmin(admin.ModelAdmin):
    ...
    list_display = ['account', 'status_badge', 'sync_type', 'products_synced', 'products_failed', 'duration_display', 'started_at']
    list_filter = ['status', 'sync_type', 'started_at', 'account']
    search_fields = ['account__name', 'error_message']
    readonly_fields = ['started_at', 'completed_at', 'duration_seconds', 'products_synced', 'products_failed', 'products_skipped', 'error_details']
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['total_products_synced'] = all_logs.aggregate(
            total=Sum('products_synced'))['total'] or 0

        # Accounts for filter
        extra_context['feed_accounts'] = FeedProviderAccount.objects.values('id', 'name')

        # Sync types for filter
        extra_context['sync_types'] = all_logs.values_list(
            'sync_type', flat=True).distinct()
