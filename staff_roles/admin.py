...
class StaffRoleAdmin(admin.ModelAdmin):
    list_display = ['display_name_with_icon', 'description_short', 'access_badges', 'member_count_display', 'is_predefined_badge']
    ...
    def display_name_with_icon(self, obj):
        return format_html(
            '<span class="role-icon-name"><i class="{}"></i> {}</span>',
            obj.icon, obj.display_name
        )
    ...
    def access_badges(self, obj):
        ...
        if obj.can_access_admin:
            badges.append(('role-access-badge role-access-badge--admin', 'fas fa-desktop', str(_('Admin'))))
        if obj.can_access_pos:
            badges.append(('role-access-badge role-access-badge--pos', 'fas fa-cash-register', str(_('POS'))))
        if badges:
