...
class UpdateManager:
    ...
    def _ensure_authenticated(self) -> bool:
        ...
        try:
            from core.license import is_sandbox_mode, get_license_manager
            ...
            self.config.jwt_expires_at = timezone.now() + timedelta(seconds=data.get('expires_in', 3600))
            self.config.save()
            ...
            if pos_license_key and pos_license_key != self.config.pos_license_key:
                ...
                self.config.save(update_fields=[
                    'pos_license_key', 'pos_license_status',
    ...
    def _store_service_secrets(self, service_secrets: Dict, installation_uuid: Optional[str] = None):
        """
        ...
        try:
            ...
            secrets.save()
            logger.info(f"✅ Platform secrets updated: geoip={bool(secrets.geoip_jwt_secret)}, push={bool(secrets.push_jwt_secret)}, sso={bool(secrets.sso_jwt_secret)}")
    ...
    def refresh_license(self, force_write: bool = False) -> Dict:
        ...
        """
        from core.license import get_license_manager, reload_license_manager, is_sandbox_mode
        ...
        try:
            ...
            response = self.session.post(
                f"{self.config.server_url}/api/v1/licenses/refresh/",
                json=request_body,
                timeout=30
            )
            ...
            if response.status_code == 403:
                ...
                if error_code in ('license_revoked', 'license_expired'):
                    logger.warning(
                        f"License refresh: {error_code} — {data.get('message', '')}"
                    )
            ...
            if current_data and current_data.get('license'):
                ...
                for key in ('maintenance_active', 'maintenance_expires_at', 'is_active',
                            'license_type', 'product_type', 'max_installations', 'entitlements',
                            'features', 'expires_at'):
            ...
            if changes or force_write or not current_data:
                ...
                logger.info(
                    f"License refreshed — changes: {changes or ['force_write']}"
                )
            else:
    ...
    def _handle_license_revocation(self, error_code: str, message: str):
        ...
        if not LicenseRevocation.objects.filter(grace_expires_at__gt=timezone.now()).exists():
            ...
            logger.warning(
                f"License revocation recorded: {error_code}. "
    ...
    def list_available_components(
        ...
        try:
            ...
            response = self.session.get(
                f"{self.config.server_url}/api/v1/components/",
                params=params,
                timeout=30
            )
            response.raise_for_status()
    ...
    def check_for_updates(self, component: Optional[ComponentRegistry] = None) -> Dict:
        """
        ...
        if component:
            ...
            components = ComponentRegistry.objects.filter(locked=False)
        ...
        try:
            ...
            for update_info in updates_list:
                ...
                if comp:
                    ...
                    logger.debug(f"🔍 Update info for {comp.slug}: checksum={checksum[:16]}..." if checksum else f"🔍 Update info for {comp.slug}: NO CHECKSUM")
                    ...
                    logger.debug(f"💾 Saved {comp.slug}: checksum={comp.latest_version_checksum[:16]}..." if comp.latest_version_checksum else f"💾 Saved {comp.slug}: checksum is EMPTY")
                    ...
                    updates_found.append({
                        ...
                        'critical': update_info.get('security_update', update_info.get('critical', False)),
                        'checksum': update_info.get('package_checksum', ''),
    ...
    def download_component(self, component: ComponentRegistry, version: str) -> Path:
        ...
        try:
            ...
            if expected_checksum:
                logger.info(f"🔍 Verifying package checksum...")
                ...
                logger.warning(f"⚠️ No checksum available for verification")
    ...
    @transaction.atomic
    def install_update(self, component: ComponentRegistry, version: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if not health_passed:
                logger.warning(f"⚠️ Health check failed for {component.name} v{target_version}, attempting rollback...")
                try:
                    # Find previous active version
                    previous_version = ComponentVersion.objects.filter(
                        component=component,
                        is_active=False
                    ).exclude(id=new_version.id).order_by('-installed_at').first()
                    ...
                    if previous_version:
                        ...
                        raise UpdateInstallError(f"Health check failed, rolled back to v{previous_version.version}")
                    else:
                        logger.error("❌ No previous version available for rollback")
                        raise UpdateInstallError("Health check failed and no previous version to rollback to")

                except Exception as rollback_error:
                    logger.error(f"❌ Rollback failed: {rollback_error}")
                    raise UpdateInstallError(f"Health check failed and rollback failed: {rollback_error}")
    ...
    @transaction.atomic
    def install_component(self, component_type: str, slug: str, version: Optional[str] = None) -> Dict:
        """
        ...
        try:
            ...
            response = self.session.get(
                f"{self.config.server_url}/api/v1/component/{slug}/",
                timeout=30
            )
            response.raise_for_status()
            ...
            component = ComponentRegistry.objects.create(
                ...
                author=component_info.get('author', {}).get('name', 'Unknown') if isinstance(component_info.get('author'), dict) else component_info.get('author', 'Unknown'),
            )
    ...
    def update_component(self, component_type: str, slug: str, version: Optional[str] = None) -> Dict:
        """
    ...
    def _install_package(self, component: ComponentRegistry, package_path: Path, version: str):
        ...
        try:
            ...
            if manifest_slug != component_slug:
                raise UpdateInstallError(f"Package slug mismatch: expected {component.slug}, got {manifest.get('slug')}")
    ...
    def _install_theme_package(self, component: ComponentRegistry, extract_dir: Path, manifest: Dict):
        """
    ...
    def _install_utility_package(self, component: ComponentRegistry, extract_dir: Path, manifest: Dict):
        """
        ...
        try:
            ...
            if not activate_result['success']:
                raise UpdateInstallError(f"Failed to activate utility: {activate_result.get('error')}")
    ...
    def _install_element_package(self, component: ComponentRegistry, extract_dir: Path, manifest: Dict):
    ...
    def _install_generic_provider_package(self, component: ComponentRegistry, extract_dir: Path, manifest: Dict):
        """
    ...
    def _install_template_package(self, component: ComponentRegistry, extract_dir: Path, manifest: Dict):
    ...
    def _install_language_pack_package(self, component: ComponentRegistry, extract_dir: Path, manifest: Dict):
        """
    ...
    def _collect_static_files(self):
        ...
        try:
            ...
            logger.warning(f"collect_component_statics failed, falling back to full collectstatic: {e}")
            try:
    ...
    def _health_check(self, component: ComponentRegistry, version: ComponentVersion) -> bool:
        ...
        try:
            ...
            if component.component_type == 'theme':
                ...
                from .utility_version_manager import UtilityVersionManager, UTILITIES_STATIC
    ...
    def _cleanup_old_versions(self, component: ComponentRegistry):
        ...
        """
        versions = ComponentVersion.objects.filter(
            component=component
    ...
    @transaction.atomic
    def rollback(self, component: ComponentRegistry, target_version: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"✅ Rolled back {component.name} from v{current_version} to v{version.version}")
            return True
    ...
    def check_dependencies(self, component: ComponentRegistry, version: str) -> Tuple[bool, List[str]]:
        """
        ...
        """
        dependencies = ComponentDependency.objects.filter(
            component=component,
            version=version
        )
        ...
        for dep in dependencies:
            ...
            required = ComponentRegistry.objects.filter(
                component_type=dep.required_component_type,
                slug=dep.required_slug
            ).first()
...
class PlatformUpdateService:
    ...
    def verify_package(self, package_path: Path, expected_checksum: str, update_record=None) -> bool:
        """
        ...
        if expected_checksum and actual_checksum != expected_checksum:
            ...
            raise PlatformUpdateError(
                f"Checksum verification failed. Package may be corrupted or tampered with."
            )
    ...
    def run_pre_checks(self, staging_dir: Path, update_record=None) -> bool:
        ...
        logger.info(f"🔍 Running pre-update checks...")
        ...
        if pre_update_script.exists():
            try:
                ...
                logger.info(f"✅ Pre-update checks passed")
    ...
    def report_update_status(self, from_version: str, to_version: str, status: str,
                            duration_seconds: int = None, downtime_seconds: int = None,
                            error_message: str = '', error_stage: str = '',
                            was_rolled_back: bool = False, rollback_reason: str = ''):
        """
    ...
    def _estimate_remaining_time(self, update) -> Optional[int]:
    ...
    def get_rollback_info(self) -> Dict:
        ...
        last_successful = PlatformUpdate.objects.filter(
            status='completed',
            rollback_available=True
        ...
        return {
            ...
            'updated_at': last_successful.completed_at.isoformat() if last_successful.completed_at else None
        }
