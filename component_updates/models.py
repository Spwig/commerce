...
class UpdateChannel(models.Model):
    ...
    name = models.CharField(
        ...
        unique=True,
        help_text=_("Update channel name")
    )
    description = models.TextField(blank=True)
    priority = models.IntegerField(
        default=0,
        help_text=_("Lower numbers = higher priority. Security=99")
    )
...
class ComponentRegistry(models.Model):
    ...
    slug = models.SlugField(
        max_length=200,
        db_index=True,
        help_text=_("Unique component identifier")
    )
    name = models.CharField(
        max_length=200,
    ...
    installed_at = models.DateTimeField(
        ...
        help_text=_("When this component was first installed")
    )
    ...
    last_checked = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("Last time we checked for updates")
    )

    # Update management
    update_available = models.BooleanField(
        default=False,
        help_text=_("Whether an update is available")
    )
    latest_version = models.CharField(
        max_length=20,
        blank=True,
        help_text=_("Latest available version")
    )
    latest_version_checksum = models.CharField(
        max_length=64,
        blank=True,
        help_text=_("SHA256 checksum of latest version package")
    )
    auto_update = models.BooleanField(
        default=False,
        help_text=_("Automatically install updates")
    )
    ...
    locked = models.BooleanField(
        default=False,
        help_text=_("Prevent any updates to this component")
    )
    lock_reason = models.TextField(
        blank=True,
        help_text=_("Why this component is locked")
    )
    ...
    thumbnail_url = models.URLField(
        blank=True,
        help_text=_("URL to component thumbnail image - 600x800px for themes")
    )
    preview_images = models.JSONField(
        default=list,
        blank=True,
        help_text=_("Array of preview image URLs")
    )
    ...
    requires_platform_version = models.CharField(
        max_length=20,
        blank=True,
        help_text=_("Required platform version (e.g., '1.x', '2.x')")
    )
...
class ComponentVersion(models.Model):
    ...
    rollback_available = models.BooleanField(
        default=True,
        help_text=_("Whether this version can be rolled back to")
    )
    ...
    health_details = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Detailed health check results")
    )
    ...
    package_checksum = models.CharField(
        max_length=64,
        blank=True,
        help_text=_("SHA256 checksum of package")
    )
    ...
    installation_log = models.TextField(
        blank=True,
        help_text=_("Installation output and logs")
    )
...
class ComponentDependency(models.Model):
    ...
    version_constraint = models.CharField(
        max_length=50,
        help_text=_("Semantic version constraint")
    )

    # Dependency type
    is_required = models.BooleanField(
        default=True,
        help_text=_("Whether this dependency is required")
    )
...
class UpdateLog(models.Model):
    ...
    duration_seconds = models.IntegerField(
        null=True,
        blank=True,
        help_text=_("How long the operation took")
    )
    ...
    performed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='component_update_logs'
    )

    # Automatic vs manual
    is_automatic = models.BooleanField(
        default=False,
        help_text=_("Whether this was an automatic update")
    )
...
class UpdateServerConfig(models.Model):
    ...
    server_url = models.URLField(
        default='https://updates.spwig.com',
        help_text=_("Update server base URL")
    )
    api_version = models.CharField(
        max_length=10,
        default='v1',
        help_text=_("API version to use")
    )
    ...
    license_key = models.CharField(
        max_length=200,
        blank=True,
        help_text=_("Platform license key")
    )
    jwt_token = models.TextField(
        blank=True,
        help_text=_("Current JWT authentication token")
    )
    jwt_expires_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When the JWT token expires")
    )

    # Update checking
    check_interval_hours = models.IntegerField(
        default=24,
        help_text=_("How often to check for updates (hours)")
    )
    last_check = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("Last successful update check")
    )

    # Update behavior
    auto_download = models.BooleanField(
        default=True,
        help_text=_("Automatically download available updates")
    )
    auto_install_security = models.BooleanField(
        default=True,
        help_text=_("Automatically install security updates")
    )

    # Telemetry
    send_telemetry = models.BooleanField(
        default=True,
        help_text=_("Send anonymous usage statistics")
    )
    ...
    pos_license_expires_at = models.DateTimeField(
        null=True, blank=True,
        help_text=_("When the POS license subscription expires")
    )
    ...
    is_connected = models.BooleanField(
        default=False,
        help_text=_("Whether we can connect to update server")
    )
...
class PlatformUpdate(models.Model):
    ...
    from_version = models.CharField(
        max_length=20,
        help_text=_("Platform version before update")
    )
    to_version = models.CharField(
        max_length=20,
        help_text=_("Target platform version")
    )
    channel = models.CharField(
        max_length=20,
        default='stable',
        help_text=_("Update channel")
    )
    ...
    progress_percent = models.IntegerField(
        default=0,
        help_text=_("Overall update progress (0-100)")
    )
    current_step = models.CharField(
        max_length=100,
        blank=True,
        help_text=_("Current step description")
    )
    ...
    steps = models.JSONField(
        default=list,
        blank=True,
        help_text=_("List of update steps with status")
    )
    log_lines = models.JSONField(
        default=list,
        blank=True,
        help_text=_("Recent log output lines")
    )
    ...
    package_checksum = models.CharField(
        max_length=64,
        blank=True,
        help_text=_("SHA256 checksum of the downloaded package")
    )
    package_size_bytes = models.BigIntegerField(
        default=0,
        help_text=_("Package file size")
    )
    bytes_downloaded = models.BigIntegerField(
        default=0,
        help_text=_("Bytes downloaded so far")
    )

    # Release info (from update server)
    changelog = models.TextField(
        blank=True,
        help_text=_("Changelog for this update")
    )
    release_notes = models.TextField(
        blank=True,
        help_text=_("User-facing release notes")
    )
    requires_migration = models.BooleanField(
        default=False,
        help_text=_("Whether database migrations are required")
    )
    migration_estimate_seconds = models.IntegerField(
        default=30,
        help_text=_("Estimated migration time")
    )
    breaking_changes = models.BooleanField(
        default=False,
        help_text=_("Whether this update has breaking changes")
    )
    security_update = models.BooleanField(
        default=False,
        help_text=_("Whether this is a security update")
    )

    # Rollback capability
    rollback_available = models.BooleanField(
        default=False,
        help_text=_("Whether rollback to previous version is available")
    )
    rollback_version = models.CharField(
        max_length=20,
        blank=True,
        help_text=_("Version to rollback to if needed")
    )

    # Error tracking
    error_message = models.TextField(
        blank=True,
        help_text=_("Error message if update failed")
    )
    error_stage = models.CharField(
        max_length=50,
        blank=True,
        help_text=_("Stage where error occurred")
    )
    error_traceback = models.TextField(
        blank=True,
        help_text=_("Full error traceback")
    )
    ...
    started_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When the update process started")
    )
    completed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When the update process completed")
    )
    duration_seconds = models.IntegerField(
        null=True,
        blank=True,
        help_text=_("Total update duration")
    )
    downtime_seconds = models.IntegerField(
        null=True,
        blank=True,
        help_text=_("Actual downtime during update")
    )
    ...
    celery_task_id = models.CharField(
        max_length=255,
        blank=True,
        help_text=_("Celery task ID for this update")
    )
    ...
    def get_current_update(cls):
        ...
        return cls.objects.filter(
            status__in=['checking', 'downloading', 'verifying', 'extracting',
                       'pre_checks', 'migrating', 'building', 'deploying',
