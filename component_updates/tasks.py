...
@shared_task(
    bind=True,
    name="component_updates.perform_platform_update",
...
def perform_platform_update(self, update_id: str):
    ...
    from .services import PlatformUpdateError, PlatformUpdateService
    ...
    try:
        ...
        service.report_update_status(
            from_version=update.from_version, to_version=update.to_version, status="started"
        )
        ...
        try:
            ...
            if not preflight_data.get("all_passed"):
                ...
                raise PlatformUpdateError(f"Preflight checks failed: {', '.join(failed_checks)}")
        ...
        try:
            ...
            if not start_data.get("success"):
                raise PlatformUpdateError(start_data.get("error", "Failed to start upgrade"))
        except requests.RequestException as e:
        ...
        while polls < max_polls:
            ...
            try:
                ...
                if operation.get("completed"):
                    ...
                    update.add_log_line(f"Platform updated successfully to v{update.to_version}")
                    update.save()
                    ...
                    logger.info(
                        f"Platform update completed: v{update.from_version} -> v{update.to_version}"
                    )
                    return {
                        "status": "success",
                        "from_version": update.from_version,
        ...
        update.mark_failed(
            error_message=str(e), error_stage=update.current_step, traceback=traceback.format_exc()
        )
...
def refresh_license():
    ...
    try:
        ...
        if result.get("refreshed"):
            logger.info(f"License refreshed successfully — changes: {result.get('changes', [])}")
...
def check_hotfixes():
    ...
    try:
        ...
        if result.get("hotfix_available"):
            ...
            logger.info(f"No hotfixes available for v{result.get('current_version')}")
