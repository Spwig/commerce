...
class Command(BaseCommand):
    ...
    THEME_SLUGS_TO_BUNDLE = [
        'starter', 'elegant-shop', 'modern-shop', 'modern-dark',
        'tech-theme', 'apparel-theme', 'space-theme',
        'botanica', 'artisan', 'nature', 'bold', 'vivid',
    ]
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--components-dir',
            default=getattr(settings, 'SPWIG_COMPONENTS_DIR', str(Path(settings.BASE_DIR) / 'components')),
    ...
    def _build_theme_packages(self, output_dir: Path) -> list:
        ...
        for slug in self.THEME_SLUGS_TO_BUNDLE:
            ...
            self.stdout.write(self.style.SUCCESS(f"  + theme: {slug} v{version} ({zip_path.stat().st_size // 1024}KB)"))
    ...
    def _build_utility_packages(self, output_dir: Path) -> list:
        ...
        for util_dir in sorted(utilities_src.iterdir()):
            ...
            if not manifest_path.exists():
                self.stdout.write(self.style.WARNING(f"  ! {util_dir.name}: no manifest.json, skipping"))
                continue
