...
class Command(BaseCommand):
    ...
    def _collect_existing_po_files(
        self, lang_code: str, locale_output: Path, base_dir: Path
    ) -> int:
    ...
    def _export_ui_strings_template(self, lang_code: str, output: Path):
        ...
        strings = {}
        for key, _english_source in sorted(UI_STRING_REGISTRY.items()):
        ...
        data = {
            ...
            "strings": strings,
            "_reference": dict(sorted(UI_STRING_REGISTRY.items())),
        }
    ...
    def _compile_po_files(self, locale_dir: Path) -> int:
        ...
        for po_file in locale_dir.rglob("*.po"):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING("  msgfmt not found, .mo files must be compiled manually")
                )
                return count
    ...
    def _calculate_coverage(self, source: Path, lang_code: str) -> dict:
        ...
        po_apps = 0
        if locale_dir.is_dir():
            po_apps = sum(1 for d in locale_dir.iterdir() if d.is_dir() and any(d.rglob("*.po")))

        # Email templates
        email_dir = source / "email_templates"
        email_count = 0
        if email_dir.is_dir():
            email_count = sum(1 for _ in email_dir.glob(f"*.{lang_code}.md"))
