...
from component_updates.services import PlatformUpdateService, PlatformUpdateError
...
class Command(BaseCommand):
    ...
    def _show_status(self):
        ...
        if hotfix_num > 0:
            self.stdout.write(
                self.style.WARNING(f"Applied hotfix:   hf{hotfix_num}")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS("No hotfixes applied (running base image code)")
            )
    ...
    def _check_and_optionally_apply(self, apply=False):
        ...
        self.stdout.write(
            f"Checking hotfixes for v{version} (current: hf{current_hf})...\n"
        )
        ...
        if not data.get('hotfix_available'):
            if data.get('maintenance_required'):
                self.stdout.write(
                    self.style.WARNING(
                        "Hotfixes available but require active maintenance."
                    )
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS("No new hotfixes available.")
                )
            return
    ...
    def _apply_via_upgrader(self, version, hotfix_number):
        ...
        try:
            ...
            if data.get('success'):
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Hotfix apply started. "
                        f"Containers will restart to apply the patch."
                    )
                )
            else:
                self.stderr.write(
                    self.style.ERROR(f"Upgrader error: {data.get('error')}")
                )

        except requests.ConnectionError:
            self.stderr.write(
                self.style.ERROR(
                    "Could not connect to upgrader service. "
                    "Is the upgrader container running?"
                )
            )
    ...
    def _rollback_hotfix(self):
        ...
        try:
            ...
            if data.get('success'):
                self.stdout.write(
                    self.style.SUCCESS(
                        "Hotfix rollback started. "
                        "Containers will restart with base image code."
                    )
                )
            else:
                self.stderr.write(
