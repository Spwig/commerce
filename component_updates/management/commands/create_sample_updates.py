...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, component in enumerate(components):
            ...
            component.save()
            self.stdout.write(
                f"  ✓ {component.name}: v{component.current_version} → v{component.latest_version}"
            )
        ...
        self.stdout.write("\n📊 Summary:")
        self.stdout.write(
            f"  Components with updates: {ComponentRegistry.objects.filter(update_available=True).count()}"
        )
        self.stdout.write(
            f"  Locked components: {ComponentRegistry.objects.filter(locked=True).count()}"
        )
