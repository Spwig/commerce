...
def _install_site_language(extract_dir: Path, language_code: str):
    ...
    if not meta_file.exists():
        ...
        SiteLanguage.objects.update_or_create(code=language_code, defaults={"is_active": True})
        return
...
def _install_ui_strings(ui_strings_file: Path, language_code: str) -> int:
    ...
    if not site_lang:
        raise InstallError(
            f"SiteLanguage '{language_code}' not found — install language_meta first"
        )
    ...
    override.verified_count = sum(1 for m in existing_meta.values() if m.get("verified"))
    override.save()
...
def _install_email_templates(email_dir: Path, language_code: str) -> int:
    ...
    for pattern in patterns:
        for md_file in email_dir.glob(pattern):
            ...
            if "error" in parsed:
                logger.warning(f"  Skipping {md_file.name}: {parsed['error']}")
                continue
            ...
            if not base_template:
                logger.debug(
                    f"  Skipping {md_file.name}: no base template for type '{template_type}'"
                )
                continue
            ...
            if existing and existing.translation_job_id:
                ...
                logger.debug(f"  Skipping {md_file.name}: merchant translation exists")
                continue
...
def _update_packs_config(
    language_code: str, name: str = "", version: str = "", action: str = "add"
):
    """
...
def _remove_pack_ui_strings(language_code: str):
    ...
    override.verified_count = sum(
        1 for m in meta.values() if isinstance(m, dict) and m.get("verified")
    )
    override.save()
