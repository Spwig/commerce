...
class ComponentRegistryAdmin(admin.ModelAdmin):
    ...
    list_filter = ["component_type", "update_available", "auto_update", "locked", "update_channel"]
    ...
    fieldsets = (
        ...
        (_("Lock & Freeze"), {"fields": ("locked", "lock_reason"), "classes": ("collapse",)}),
        (
            _("Platform Compatibility"),
            {"fields": ("engine_min_version", "engine_max_version"), "classes": ("collapse",)},
        ),
        (_("Links"), {"fields": ("homepage_url", "support_url"), "classes": ("collapse",)}),
        (
            _("Timestamps"),
            {"fields": ("installed_at", "last_updated", "last_checked"), "classes": ("collapse",)},
        ),
        (_("Rollback"), {"fields": ("rollback_options",), "classes": ("collapse",)}),
    )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "filter/",
                self.admin_site.admin_view(self.filter_components),
                name="component_updates_componentregistry_filter",
            ),
            path(
                "check-updates/",
                self.admin_site.admin_view(self.check_all_updates),
                name="component_updates_componentregistry_check_updates",
            ),
            path(
                "<int:pk>/install-update/",
                self.admin_site.admin_view(self.install_single_update),
                name="component_updates_componentregistry_install_update",
            ),
            path(
                "<int:pk>/toggle-lock/",
                self.admin_site.admin_view(self.toggle_lock),
                name="component_updates_componentregistry_toggle_lock",
            ),
            path(
                "version-history/<slug:slug>/",
                self.admin_site.admin_view(self.get_version_history),
                name="component_updates_componentregistry_version_history",
            ),
        ]
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        updates_available = ComponentRegistry.objects.filter(
    ...
    def install_single_update(self, request, pk):
        ...
        try:
            ...
            return JsonResponse(
                {"success": True, "message": f"Successfully updated {component.name}"}
            )
    ...
    def check_for_updates(self, request, queryset):
        ...
        try:
            ...
            if updates_found > 0:
                self.message_user(
                    request, f"Found {updates_found} update(s) available", messages.WARNING
                )
            else:
                self.message_user(
                    request, f"All {queryset.count()} component(s) are up to date", messages.SUCCESS
                )
        except UpdateAuthenticationError as e:
            self.message_user(request, f"Authentication failed: {e}", messages.ERROR)
        except Exception as e:
            self.message_user(request, f"Update check failed: {e}", messages.ERROR)
    ...
    def install_updates(self, request, queryset):
        ...
        for component in queryset:
            ...
            try:
                ...
                failed += 1
                self.message_user(
                    request, f"Failed to update {component.name}: {e}", messages.ERROR
                )

        if installed > 0:
            self.message_user(
                request, f"Successfully installed {installed} update(s)", messages.SUCCESS
            )
        if skipped > 0:
            self.message_user(
                request, f"Skipped {skipped} component(s) (locked or no updates)", messages.WARNING
            )
...
class ComponentVersionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Version Information"), {"fields": ("component", "version", "is_active")}),
        (
            _("Installation"),
            {"fields": ("installed_at", "install_method", "installed_by", "rollback_available")},
        ),
        (_("Health Status"), {"fields": ("health_status", "health_checked_at", "health_details")}),
        (
            _("Package Info"),
            {
                "fields": ("package_url", "package_checksum", "package_size_bytes"),
                "classes": ("collapse",),
            },
        ),
        (
            _("Release Notes"),
            {
                "fields": ("release_notes", "breaking_changes", "security_update"),
                "classes": ("collapse",),
            },
        ),
    )
    ...
    def activate_version(self, request, queryset):
        ...
        if queryset.count() != 1:
            self.message_user(
                request, "Please select exactly one version to activate", level="error"
            )
            return
...
class UpdateLogAdmin(admin.ModelAdmin):
    ...
    list_filter = ["action", "status", "is_automatic", "started_at"]
    ...
    fieldsets = (
        (
            _("Operation Details"),
            {"fields": ("component", "action", "status", "old_version", "new_version")},
        ),
        (_("Timing"), {"fields": ("started_at", "completed_at", "duration_seconds")}),
        (_("User & Automation"), {"fields": ("performed_by", "is_automatic")}),
        (_("Error Information"), {"fields": ("error_display",), "classes": ("collapse",)}),
    ...
    def error_display(self, obj):
        ...
        return format_html(
            '<div class="cu-detail-box cu-detail-box-error"><strong>Error:</strong><br>{}</div>',
            obj.error_message,
        )
...
class UpdateServerConfigAdmin(admin.ModelAdmin):
    list_display = ["server_url", "api_version", "connection_status", "jwt_status", "last_check"]
    readonly_fields = [
        "installation_uuid",
        "jwt_expires_at",
        "last_check",
    ...
    fieldsets = (
        (_("Server Configuration"), {"fields": ("server_url", "api_version")}),
        (
            _("Authentication"),
            {"fields": ("installation_uuid", "license_key", "jwt_token", "jwt_expires_at")},
        ),
        (
            _("Update Behavior"),
            {"fields": ("check_interval_hours", "auto_download", "auto_install_security")},
        ),
        (_("Telemetry"), {"fields": ("send_telemetry",)}),
        (
            _("Connection Status"),
            {
                "fields": (
...
class ComponentDependencyAdmin(admin.ModelAdmin):
    list_display = ["component", "depends_on", "version_constraint", "is_required", "created_at"]
    list_filter = ["is_required", "created_at"]
    search_fields = ["component__name", "depends_on__name", "version_constraint"]
...
class PlatformUpdateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Progress"), {"fields": ("progress_display",)}),
        (
            _("Release Details"),
            {
                "fields": (
        ...
        (_("Logs"), {"fields": ("log_display",), "classes": ("collapse",)}),
        (_("Error Information"), {"fields": ("error_display",), "classes": ("collapse",)}),
    )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "check/",
                self.admin_site.admin_view(self.check_for_update),
                name="component_updates_platformupdate_check",
            ),
            path(
                "start/",
                self.admin_site.admin_view(self.start_update),
                name="component_updates_platformupdate_start",
            ),
            path(
                "<uuid:pk>/status/",
                self.admin_site.admin_view(self.get_status),
                name="component_updates_platformupdate_status",
            ),
            path(
                "<uuid:pk>/cancel/",
                self.admin_site.admin_view(self.cancel_update),
                name="component_updates_platformupdate_cancel",
            ),
            path(
                "check-hotfix/",
                self.admin_site.admin_view(self.check_for_hotfix_api),
                name="component_updates_platformupdate_check_hotfix",
            ),
            path(
                "apply-hotfix/",
                self.admin_site.admin_view(self.apply_hotfix_api),
                name="component_updates_platformupdate_apply_hotfix",
            ),
            path(
                "rollback-hotfix/",
                self.admin_site.admin_view(self.rollback_hotfix_api),
                name="component_updates_platformupdate_rollback_hotfix",
            ),
            path(
                "hotfix-status/",
                self.admin_site.admin_view(self.hotfix_status_api),
                name="component_updates_platformupdate_hotfix_status",
            ),
            # Hosted upgrade scheduling
            path(
                "hosted-upgrade/",
                self.admin_site.admin_view(self.hosted_upgrade_view),
                name="component_updates_hosted_upgrade",
            ),
            path(
                "hosted-upgrade/api/check/",
                self.admin_site.admin_view(self.hosted_upgrade_check_api),
                name="component_updates_hosted_upgrade_check",
            ),
            path(
                "hosted-upgrade/api/schedule/",
                self.admin_site.admin_view(self.hosted_upgrade_schedule_api),
                name="component_updates_hosted_upgrade_schedule",
            ),
            path(
                "hosted-upgrade/api/status/",
                self.admin_site.admin_view(self.hosted_upgrade_status_api),
                name="component_updates_hosted_upgrade_status",
            ),
            path(
                "hosted-upgrade/api/snooze/",
                self.admin_site.admin_view(self.hosted_upgrade_snooze_api),
                name="component_updates_hosted_upgrade_snooze",
            ),
            path(
                "hosted-upgrade/api/cancel/",
                self.admin_site.admin_view(self.hosted_upgrade_cancel_api),
                name="component_updates_hosted_upgrade_cancel",
            ),
        ]
        return custom_urls + urls
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if current_update and current_update.started_at:
            ...
            if elapsed > 1800:
                try:
                    ...
                    _params = (
                        {"instance": getattr(django_settings, "FLEET_INSTANCE_NAME", "")}
                    ...
                    upgrader_done = op.get("type") != "upgrade" or op.get("status") in (
                        "idle",
                        "completed",
                        "failed",
                    )
        ...
        missing_hf_numbers = [
            hf["hotfix_number"] for hf in hotfix_status.get("missing_hotfixes", [])
        ]
        ...
        try:
            ...
            if (
                upgrader_data.get("type") in ("hotfix", "hotfix_rollback")
    ...
    def version_change(self, obj):
        ...
        return format_html(
            '<span class="cu-version-mono">v{} <span class="cu-version-arrow">→</span> v{}</span>',
            obj.from_version,
            obj.to_version,
        )
    ...
    def status_badge(self, obj):
        ...
        css_class = f"cu-badge cu-badge-{obj.status}"
    ...
    def channel_badge(self, obj):
        ...
        return format_html('<span class="{}">{}</span>', css_class, obj.channel.upper())
    ...
    def start_update(self, request):
        ...
        try:
            ...
            if not PlatformUpdate.can_start_update():
                return JsonResponse({"error": "An update is already in progress"}, status=400)
            ...
            if not update_info.get("update_available"):
                return JsonResponse({"error": "No update available"}, status=400)

            # Create update record
            service = PlatformUpdateService()
            update = service.create_update_record(
                to_version=update_info["latest_version"], update_info=update_info, user=request.user
            )
            ...
            return JsonResponse({"success": True, "update_id": str(update.id), "task_id": task.id})
    ...
    def get_status(self, request, pk):
        ...
        try:
            ...
            if (
                status.get("status") in ("deploying", "health_check", "switching")
    ...
    def cancel_update(self, request, pk):
        ...
        try:
            ...
            if update.status not in ["checking", "downloading"]:
                return JsonResponse(
                    {"error": "Update cannot be cancelled at this stage"}, status=400
                )
    ...
    def apply_hotfix_api(self, request):
        ...
        try:
            ...
            if not version or not hotfix_numbers:
                return JsonResponse({"error": "version and hotfix_numbers required"}, status=400)
            ...
            return JsonResponse(
                {"error": "Could not connect to upgrader service. Is it running?"}, status=503
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
    ...
    def hosted_upgrade_view(self, request):
        ...
        return render(
            request,
            "admin/component_updates/platformupdate/hosted_upgrade.html",
            {
