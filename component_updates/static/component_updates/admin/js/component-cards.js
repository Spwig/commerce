...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const island = document.getElementById('component-cards-i18n');
    if (island) {
      try {
  ...
  async function installUpdate(componentId) {
    ...
    const url =
      langPrefix + '/admin/component_updates/componentregistry/' + componentId + '/install-update/';
  ...
  async function toggleLock(componentId, lock) {
    ...
    const langPrefix = AdminUtils.getLanguagePrefix();
    const url =
      langPrefix + '/admin/component_updates/componentregistry/' + componentId + '/toggle-lock/';
  ...
  function openChangelogModal(slug, name) {
    ...
    body.innerHTML =
      '<p class="version-history-loading"><i class="fas fa-spinner fa-spin"></i> Loading...</p>';

    // Fetch and render
    if (typeof VersionHistory !== 'undefined') {
      VersionHistory.fetch(slug)
