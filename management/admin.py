...
from .models import (
    DatabaseBackup, QueryHistory, SystemMetrics, AccessLog, FileOperation,
    DeploymentBackup, BackupSchedule, SystemUpgrade, SystemRestore, SystemStatus,
    LogEntry, LogViewerSettings, RemoteStorageDestination
)
from .utils import DatabaseManager, SystemMonitor, FileManager, DataExporter
...
from .views.oauth import storage_oauth_initiate, storage_oauth_callback
...
def _check_not_hosted(request):
    ...
    if get_license_manager().is_spwig_hosted():
        return HttpResponseForbidden('This feature is not available for Spwig-hosted installations.')
    return None


@admin.register(DatabaseBackup)
class DatabaseBackupAdmin(admin.ModelAdmin):
    list_display = ['name', 'backup_type', 'file_size_display', 'status', 'created_at', 'created_by']
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('create-backup/', self.admin_site.admin_view(self.create_backup_view), name='management_backup_create'),
            path('download/<int:backup_id>/', self.admin_site.admin_view(self.download_backup), name='management_backup_download'),
        ]
    ...
    def create_backup_view(self, request):
        ...
        return render(request, 'admin/management/create_backup.html', {
...
class QueryHistoryAdmin(admin.ModelAdmin):
    list_display = ['query_preview', 'user', 'executed_at', 'execution_time', 'rows_affected', 'success']
    list_filter = ['success', 'executed_at', 'user']
    search_fields = ['query', 'user__username']
    readonly_fields = ['query', 'executed_at', 'execution_time', 'rows_affected', 'success', 'error_message']
    change_list_template = 'admin/management/change_list.html'
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('query-runner/', self.admin_site.admin_view(self.query_runner_view), name='management_query_runner'),
            path('database-browser/', self.admin_site.admin_view(self.database_browser_view), name='management_database_browser'),
            path('browse-table/<str:table_name>/', self.admin_site.admin_view(self.browse_table_view), name='management_browse_table'),
            path('execute-query/', self.admin_site.admin_view(self.execute_query), name='management_execute_query'),
        ]
        return custom_urls + urls
    ...
    def query_runner_view(self, request):
        ...
        return render(request, 'admin/management/query_runner.html', {
            'title': _('SQL Query Runner'),
            'opts': self.model._meta,
...
class SystemMetricsAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path('shop-dashboard/', self.admin_site.admin_view(shop_dashboard.shop_dashboard_view), name='management_shop_dashboard'),
            path('shop-dashboard-api/', self.admin_site.admin_view(shop_dashboard.shop_dashboard_api), name='management_shop_dashboard_api'),
            path('shop-dashboard/geography-cities/<str:country_code>/', self.admin_site.admin_view(shop_dashboard.get_geography_cities), name='management_geography_cities'),
            path('dashboard/', self.admin_site.admin_view(self.dashboard_view), name='management_system_dashboard'),
            path('metrics-api/', self.admin_site.admin_view(self.metrics_api), name='management_metrics_api'),
            path('collect-metrics/', self.admin_site.admin_view(self.collect_metrics), name='management_collect_metrics'),
            # Deployment Dashboard URLs
            path('deployment-status/', self.admin_site.admin_view(self.deployment_status_api), name='management_deployment_status'),
            path('run-diagnostics/', self.admin_site.admin_view(self.run_diagnostics_view), name='management_run_diagnostics'),
            path('toggle-maintenance/', self.admin_site.admin_view(self.toggle_maintenance_view), name='management_toggle_maintenance'),
            # Full Backup URLs
            path('create-full-backup/', self.admin_site.admin_view(self.create_full_backup_view), name='management_create_full_backup'),
            path('backup-progress/<int:backup_id>/', self.admin_site.admin_view(self.backup_progress_api), name='management_backup_progress'),
            path('backup-schedule/', self.admin_site.admin_view(self.backup_schedule_view), name='management_backup_schedule'),
            # Restore URLs
            path('restore/', self.admin_site.admin_view(self.restore_list_view), name='management_restore'),
            path('restore/<int:backup_id>/', self.admin_site.admin_view(self.restore_confirm_view), name='management_restore_confirm'),
            path('restore/<int:backup_id>/execute/', self.admin_site.admin_view(self.restore_execute_view), name='management_restore_execute'),
            path('restore-progress/<int:restore_id>/', self.admin_site.admin_view(self.restore_progress_api), name='management_restore_progress'),
            # Upgrade URLs
            path('check-updates/', self.admin_site.admin_view(self.check_updates_view), name='management_check_updates'),
            path('upgrade/', self.admin_site.admin_view(self.upgrade_view), name='management_upgrade'),
            path('upgrade/start/', self.admin_site.admin_view(self.upgrade_start_view), name='management_upgrade_start'),
            path('upgrade/<int:upgrade_id>/execute/', self.admin_site.admin_view(self.upgrade_execute_view), name='management_upgrade_execute'),
            path('upgrade-progress/<int:upgrade_id>/', self.admin_site.admin_view(self.upgrade_progress_api), name='management_upgrade_progress'),
            # Remote Storage URLs
            path('remote-storage/', self.admin_site.admin_view(storage_destinations_view), name='management_storage_destinations'),
            path('remote-storage/add/step1/', self.admin_site.admin_view(StorageWizardStep1View.as_view()), name='management_storage_wizard_step1'),
            path('remote-storage/add/step2/', self.admin_site.admin_view(StorageWizardStep2View.as_view()), name='management_storage_wizard_step2'),
            path('remote-storage/add/step3/', self.admin_site.admin_view(StorageWizardStep3View.as_view()), name='management_storage_wizard_step3'),
            path('remote-storage/test-connection/', self.admin_site.admin_view(test_connection_view), name='management_storage_test_connection'),
            path('remote-storage/delete/<uuid:destination_id>/', self.admin_site.admin_view(delete_destination_view), name='management_storage_delete'),
            path('remote-storage/retest/<uuid:destination_id>/', self.admin_site.admin_view(retest_destination_view), name='management_storage_retest'),
            path('remote-storage/oauth/initiate/', self.admin_site.admin_view(storage_oauth_initiate), name='management_storage_oauth_initiate'),
            path('remote-storage/oauth/callback/', storage_oauth_callback, name='management_storage_oauth_callback'),
        ]
        return custom_urls + urls
    ...
    def dashboard_view(self, request):
        ...
        if not needs_refresh and not system_status.db_healthy and not system_status.redis_healthy and not system_status.celery_healthy:
            needs_refresh = True
        ...
        running_restore = SystemRestore.objects.filter(
            status__in=['pending', 'backup', 'downloading', 'restoring_db', 'restoring_media', 'restoring_config']
        ).first()
        running_upgrade = SystemUpgrade.objects.filter(
            status__in=['pending', 'preflight', 'backup', 'draining', 'maintenance', 'pulling', 'upgrading', 'migrating']
        ).first()
        ...
        return render(request, 'admin/management/system_dashboard.html', {
            'title': _('System Dashboard'),
            'opts': self.model._meta,
    ...
    def metrics_api(self, request):
        ...
        metrics = SystemMetrics.objects.filter(
            ...
            'active_sessions', 'network_bytes_sent', 'network_bytes_recv'
        )
    ...
    def run_diagnostics_view(self, request):
        ...
        return render(request, 'admin/management/diagnostics.html', {
    ...
    def toggle_maintenance_view(self, request):
        ...
        try:
            ...
            if success:
                ...
                return JsonResponse({'success': False, 'error': _('Failed to toggle maintenance mode')})
        except Exception as e:
            return JsonResponse({'success': False, 'error': str(e)})
    ...
    def create_full_backup_view(self, request):
        ...
        if request.method == 'POST':
            ...
            backup = DeploymentBackup.objects.create(
                name=request.POST.get('name', '').strip() or f"Manual backup {timezone.now().strftime('%Y%m%d_%H%M%S')}",
                backup_type=backup_type,
                status='pending',
        ...
        return render(request, 'admin/management/create_full_backup.html', {
    ...
    def backup_schedule_view(self, request):
        ...
        selected_dest_ids = set(
            schedule.remote_destinations.values_list('pk', flat=True)
        )
        # Annotate each destination with a selected flag for the template
        for dest in destinations:
            dest.is_selected = dest.pk in selected_dest_ids
        return render(request, 'admin/management/backup_schedule.html', {
    ...
    def restore_list_view(self, request):
        ...
        return render(request, 'admin/management/restore_list.html', {
    ...
    def restore_confirm_view(self, request, backup_id):
        ...
        return render(request, 'admin/management/restore_confirm.html', {
    ...
    def upgrade_view(self, request):
        ...
        return render(request, 'admin/management/upgrade.html', {
    ...
    def upgrade_execute_view(self, request, upgrade_id=None):
        ...
        messages.info(request, _('Upgrade started. The system will be in maintenance mode during the upgrade.'))
        return redirect(reverse('admin:management_system_dashboard'))
    ...
    def upgrade_progress_api(self, request, upgrade_id):
        ...
        try:
            ...
            return JsonResponse({
                ...
                'error_message': upgrade.error_message if upgrade.status in ('failed', 'rolled_back') else None,
            })
...
class FileOperationAdmin(admin.ModelAdmin):
    list_display = ['operation_type', 'file_name', 'user', 'timestamp', 'success', 'file_size_display']
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('file-manager/', self.admin_site.admin_view(self.file_manager_view), name='management_file_manager'),
            path('upload-file/', self.admin_site.admin_view(self.upload_file), name='management_upload_file'),
            path('delete-file/', self.admin_site.admin_view(self.delete_file), name='management_delete_file'),
        ]
        return custom_urls + urls
    ...
    def file_manager_view(self, request):
        ...
        context = {
            ...
            'files': files,
            'parent_path': os.path.dirname(current_path) if current_path != settings.MEDIA_ROOT else None,
            'has_permission': True,
            'user': request.user,
            'site_url': getattr(settings, 'SITE_URL', '/'),
            'site_title': self.admin_site.site_title,
...
class LogEntryAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['container_name', 'level', 'message', 'timestamp', 'raw_line', 'source', 'archived_at']
    ...
    def level_badge(self, obj):
        return format_html(
            '<span class="mgmt-level-badge mgmt-level-badge-{}">{}</span>',
            obj.level, obj.level
        )
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('log-viewer/', self.admin_site.admin_view(self.log_viewer_view), name='management_logentry_log_viewer'),
            path('log-viewer/api/logs/', self.admin_site.admin_view(self.logs_api), name='management_logentry_logs_api'),
            path('log-viewer/api/stats/', self.admin_site.admin_view(self.stats_api), name='management_logentry_stats_api'),
            path('log-viewer/api/containers/', self.admin_site.admin_view(self.containers_api), name='management_logentry_containers_api'),
            path('log-viewer/api/export/', self.admin_site.admin_view(self.export_api), name='management_logentry_export_api'),
        ]
        return custom_urls + urls
    ...
    def export_api(self, request):
        ...
        if export_format == 'csv':
            ...
            response['Content-Disposition'] = f'attachment; filename="logs_export_{timestamp_str}.csv"'
            ...
            response['Content-Disposition'] = f'attachment; filename="logs_export_{timestamp_str}.json"'
            return response
...
class LogViewerSettingsAdmin(admin.ModelAdmin):
    ...
    list_display = ['__str__', 'stream_enabled', 'redis_retention_minutes', 'db_retention_days', 'auto_refresh_interval']
    ...
    fieldsets = (
        ...
        (_('Security'), {
            ...
            'description': _('Regex patterns to redact from logs (e.g., passwords, tokens). Enter as a JSON array: ["pattern1", "pattern2"]')
        }),
        (_('Display'), {
