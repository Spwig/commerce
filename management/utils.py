...
class DatabaseManager:
    ...
    def execute_query(query, fetch_results=True, read_only=False):
        ...
        if read_only:
            import re
            stripped = re.sub(r'^(/\*.*?\*/|--[^\n]*\n|\s)+', '', query.strip(), flags=re.DOTALL).upper()
    ...
    def _find_db_container():
        ...
        try:
            result = subprocess.run(
                ['docker', 'ps', '--filter', f'name=^/{expected_name}$',
                 '--format', '{{.Names}}'],
                capture_output=True, text=True, timeout=5,
            )
    ...
    def create_backup(backup_name, backup_type='full', compression='gzip'):
        ...
        try:
            ...
            if use_docker:
                cmd = ['docker', 'exec', '-e', f'PGPASSWORD={db_settings["PASSWORD"]}',
                       docker_container, 'pg_dump'] + pg_dump_args
            ...
            if compression == 'gzip':
                with open(backup_path, 'wb') as f:
                    p1 = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=cmd_env)
...
class SystemMonitor:
    ...
    def get_process_info():
        ...
        return {
            ...
            'create_time': timezone.make_aware(datetime.fromtimestamp(current_process.create_time())),
