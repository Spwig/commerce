...
(function() {
    ...
    async function fetchLogs(append = false) {
        ...
        if (!append) {
            state.offset = 0;
            elements.logEntries.innerHTML = '<div class="log-loading"><i class="fas fa-spinner fa-spin"></i> Loading logs...</div>';
        }
        ...
        try {
            ...
            elements.logSourceInfo.textContent = data.source === 'redis' ?
                '(from Redis)' : '(from Database)';
    ...
    function renderLogs() {
        ...
        if (state.logs.length === 0) {
            elements.logEntries.innerHTML = '<div class="log-empty"><i class="fas fa-inbox"></i> No logs found</div>';
            return;
        }
    ...
    async function fetchContainers() {
        try {
            ...
            data.containers.forEach(container => {
                const badge = elements.containerBadges.querySelector(`[data-container="${container.name}"]`);
                if (badge) {
    ...
    elements.filterSearch.addEventListener('input', debounce(() => fetchLogs(), 300));
