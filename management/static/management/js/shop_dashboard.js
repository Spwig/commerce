...
(function () {
          ...
          tooltip: {
            ...
            callbacks: {
              label: function (context) {
                ...
                const dropOff =
                  stage.drop_off > 0 ? ` (${stage.drop_off.toFixed(1)}% drop-off)` : '';
                return [`Count: ${count}`, `${percentage}% of total${dropOff}`];
              },
            },
          },
        },
          ...
          tooltip: {
            ...
            callbacks: {
              label: function (context) {
                ...
                return [`${context.label}: ${count} visitors`, `${percentage}% of total traffic`];
              },
            },
          },
        },
  ...
  function refreshDashboard() {
    ...
    if (period === 'custom' && startDateInput && endDateInput) {
      ...
      if (!startDate || !endDate) {
        AdminModal.alert({
          message: gettext('Please select both start and end dates'),
          type: 'warning',
        });
        return;
      }
    ...
    fetch(apiUrl)
      ...
      .catch(error => {
        ...
        AdminModal.alert({
          message: gettext('Failed to refresh dashboard. Please try again.'),
          type: 'error',
        });
        if (dashboard) {
          dashboard.classList.remove('dashboard-loading');
  ...
  function updateProfitMetrics(data) {
    ...
    const profitSection = Array.from(document.querySelectorAll('.section-header h2')).find(
      h =>
        h.textContent.includes('Profit Analysis') ||
        h.textContent.includes(gettext('Profit Analysis'))
    );
    ...
    if (metricCards[1]) {
      ...
      const changeEl = metricCards[1].querySelector('.metric-change');
      if (valueEl)
        valueEl.textContent = parseFloat(data.current.margin_percentage).toFixed(1) + '%';
      if (changeEl && data.changes && data.changes.margin_percentage !== null) {
        ...
        const textNode = Array.from(changeEl.childNodes).find(
    ...
    if (data.products && data.products.length > 0) {
      const profitTable = profitSection
        .closest('.dashboard-section')
        .querySelector('.profit-table tbody');
      if (profitTable) {
        profitTable.innerHTML = data.products
          .map(
            product => `
                    <tr>
  ...
  function updateTopProducts(products) {
    ...
    if (products.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="empty-state"><i class="fas fa-inbox"></i>' +
        gettext('No sales data for this period') +
        '</td></tr>';
      return;
    }
  ...
  function updateMostViewedProducts(products) {
    ...
    if (products.length === 0) {
      productList.innerHTML =
        '<li class="empty-state"><i class="fas fa-inbox"></i>' +
        gettext('No view data available') +
        '</li>';
      return;
    }
  ...
  function updateVisitorGeography(geography) {
    ...
    if (geography.length === 0) {
      geoList.innerHTML =
        '<li class="empty-state"><i class="fas fa-inbox"></i>' +
        gettext('No geography data available') +
        '</li>';
      return;
    }

    geoList.innerHTML = geography
      .map(item => {
      ...
      .then(data => {
        if (data.cities && data.cities.length > 0) {
          ...
          citiesList.innerHTML =
            '<li style="padding: 10px; color: #999; font-size: 13px; text-align: center;">No city data available</li>';
        }
      })
      .catch(error => {
        console.error('Error loading cities:', error);
        citiesList.innerHTML =
          '<li style="padding: 10px; color: #dc3545; font-size: 13px; text-align: center;">Failed to load cities</li>';
      });
  }
  ...
  function updateReferrerStats(referrers) {
    ...
    referrerList.innerHTML = referrers
      .map(referrer => {
        return `<li class="data-list-item">
  ...
  function updateChannelCharts(data) {
    ...
    if (channelRevenueChart) {
      ...
      channelRevenueChart.data.datasets[0].data = data.by_channel.map(
        channel => parseFloat(channel.revenue) || 0
      );
      channelRevenueChart.data.datasets[0].backgroundColor = data.by_channel.map(
        channel => channelColors[channel.source] || '#95a5a6'
      );
    ...
    if (channelOrdersChart) {
      ...
      channelOrdersChart.data.datasets[0].data = data.by_channel.map(
        channel => parseFloat(channel.orders) || 0
      );
      channelOrdersChart.data.datasets[0].backgroundColor = data.by_channel.map(
        channel => channelColors[channel.source] || '#95a5a6'
      );
