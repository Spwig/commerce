...
(function () {
  ...
  function readConfig() {
    const el = document.getElementById('backup-config');
  ...
  function pollBackupProgress() {
    ...
    const url = config.progressUrl ? config.progressUrl.replace('0', backupId) : null;
    if (!url) return;
    fetch(url)
      .then(function (r) {
        return r.json();
      ...
      .then(function (data) {
        ...
        if (data.status === 'completed') {
          ...
          AdminModal.alert({
            message:
              (config.msgFailed || 'Backup failed') +
              ': ' +
  ...
  document.addEventListener('click', function (e) {
    const cardEl = e.target.closest('.backup-type-card');
    if (cardEl) {
      document.querySelectorAll('.backup-type-card').forEach(function (c) {
        ...
        fetch(config.backupUrl, {
          ...
          .then(function (data) {
            if (data.success) {
              ...
              AdminModal.alert({
                message:
                  (config.msgFailedStart || 'Failed to start backup') +
                  ': ' +
              ...
              btn.disabled = false;
              btn.innerHTML =
                '<i class="fas fa-save"></i> ' + (config.msgStartBackup || 'Start Backup');
            }
          })
          .catch(function (error) {
            console.error('Error:', error);
            AdminModal.alert({
              message: config.msgFailedStart || 'Failed to start backup',
              type: 'error',
            });
            document.getElementById('backup-progress').classList.remove('active');
            btn.disabled = false;
