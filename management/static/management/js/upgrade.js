...
(function () {
  ...
  function readConfig() {
    const el = document.getElementById('upgrade-config');
  ...
  function runPreflightChecks() {
    ...
    btn.disabled = true;
    btn.innerHTML =
      "<i class='fas fa-spinner fa-spin'></i> " + (config.msgChecking || 'Checking...');
    const checks = document.querySelectorAll('.preflight-item');
    let checkIndex = 0;
    function runNextCheck() {
      if (checkIndex >= checks.length) {
        btn.innerHTML =
          "<i class='fas fa-check'></i> " + (config.msgChecksComplete || 'Checks Complete');
        const allPassed = Array.from(checks).every(function (c) {
          return c.classList.contains('passed');
        });
    ...
    fetch(config.checkUpdatesUrl, {
      ...
      .then(function (data) {
        if (data.update_available) {
          ...
          AdminModal.alert(
            config.msgNoUpdates || 'No updates available. You are running the latest version.'
          );
          btn.disabled = false;
  ...
  function showReconnecting() {
    ...
    const headerIcon = document.querySelector('#upgrade-progress h3 i');
    if (headerIcon) {
  ...
  function pollUpgradeProgress() {
    if (!upgradeId) return;
    const config = readConfig();
    const url = config.upgradeProgressUrl
      ? config.upgradeProgressUrl.replace('0', upgradeId)
      : null;
    if (!url) return;
    fetch(url)
      .then(function (r) {
      ...
      .then(function (data) {
        ...
        if (data.status === 'completed') {
          AdminModal.toast(
            config.msgUpgradeCompleted ||
              'Upgrade completed successfully! The page will now reload.',
            'success'
          );
          setTimeout(function () {
            location.reload();
          ...
          AdminModal.alert({
            message:
              (config.msgUpgradeFailed || 'Upgrade failed') +
              ': ' +
              (data.error_message || config.msgUnknown || 'Unknown error'),
            type: 'error',
          });
          setTimeout(function () {
            location.reload();
          ...
          setTimeout(pollUpgradeProgress, 2000);
        }
      })
      .catch(function (error) {
        consecutiveErrors++;
        ...
        const delay = Math.min(3000 + consecutiveErrors * 2000, 10000);
    ...
    if (upgradeForm) {
      upgradeForm.addEventListener('submit', async function (e) {
        ...
        if (!preflightPassed) {
          AdminModal.alert({
            message:
              config.msgRunChecksFirst ||
              'Please run and pass all pre-flight checks before upgrading.',
            type: 'warning',
          });
          return;
        }
        const confirmMsg =
          config.msgConfirmUpgrade ||
          'Are you sure you want to start the upgrade? A backup will be created first.';
        if (!(await AdminModal.confirm(confirmMsg))) return;
        const btn = document.getElementById('upgrade-btn');
        btn.disabled = true;
        btn.innerHTML =
          "<i class='fas fa-spinner fa-spin'></i> " + (config.msgStarting || 'Starting...');
        ...
        fetch(config.upgradeStartUrl, {
          ...
          .then(function (data) {
            if (data.success) {
              upgradeId = data.upgrade_id;
              pollUpgradeProgress();
            } else {
              AdminModal.alert({
                message:
                  (config.msgFailedStart || 'Failed to start upgrade') +
                  ': ' +
              ...
              btn.innerHTML =
                ...
                (config.msgStartUpgrade || 'Start Upgrade');
            }
          })
          .catch(function (error) {
            console.error('Error:', error);
            AdminModal.alert({
              message: config.msgFailedStart || 'Failed to start upgrade',
              type: 'error',
            });
            document.getElementById('upgrade-progress').classList.remove('active');
            btn.disabled = false;
