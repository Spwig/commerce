...
class SystemStatusService:
    ...
    @classmethod
    def get_cached_status(cls) -> dict[str, Any] | None:
    ...
    def check_redis(cls) -> dict[str, Any]:
        ...
        try:
            ...
            result["host"] = (
                ...
                if "@" in redis_url
                else redis_url.replace("redis://", "").split("/")[0]
            )
        ...
        try:
            ...
            if config.ssl_mode == DomainConfiguration.SSLMode.MANAGED_EXTERNALLY:
                ...
                if info.get("expires_at"):
                    ...
                    result["days_remaining"] = (info["expires_at"] - datetime.now(UTC)).days
                if not valid:
            ...
            try:
                ...
                for line in output.strip().split("\n"):
                    if line.startswith("subject="):
                        ...
                        content = line.split("=", 1)[1]
                        kvs = dict(_kv_re.findall(content))
                        result["issuer"] = kvs.get("O", "").strip() or kvs.get("CN", "").strip()
