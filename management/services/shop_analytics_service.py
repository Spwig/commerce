...
class ShopAnalyticsService:
    ...
    def get_date_range_for_period(
        ...
        if period == "today":
            ...
            if quarter == 0:
                ...
                start = now.replace(
                    year=now.year - 1, month=10, day=1, hour=0, minute=0, second=0, microsecond=0
                )
    ...
    @staticmethod
    def calculate_percentage_change(current: Decimal, previous: Decimal) -> float | None:
        """
    ...
    def get_action_cards(cls) -> dict:
        ...
        data = {
            ...
            "abandoned_carts": AbandonedCart.objects.filter(recovered=False).count(),
            ...
            "out_of_stock_products": Product.objects.filter(track_inventory=True)
            .exclude(stock_items__on_hand__gt=F("stock_items__allocated"))
            .distinct()
            .count(),
    ...
    def get_sales_performance(
        cls, start_date: datetime, end_date: datetime, compare: bool = True
    ) -> dict:
        """
        ...
        if compare:
            ...
            result["changes"] = {
                "revenue": cls.calculate_percentage_change(
                    result["current"]["revenue"], result["previous"]["revenue"]
                ),
                "orders": cls.calculate_percentage_change(
                    Decimal(result["current"]["orders"]), Decimal(result["previous"]["orders"])
                ),
                "aov": cls.calculate_percentage_change(
                    result["current"]["aov"], result["previous"]["aov"]
                ),
            }
    ...
    def get_refund_metrics(
        ...
        current_refunds = Refund.objects.filter(created_at__gte=start_date, created_at__lt=end_date)
        ...
        if compare:
            ...
            if prev_total_orders > 0:
                prev_refund_rate = (
                    Decimal(prev_stats["refund_count"]) / Decimal(prev_total_orders)
                ) * 100
            ...
            result["changes"] = {
                "total_amount": cls.calculate_percentage_change(
                    result["current"]["total_amount"], result["previous"]["total_amount"]
                ),
                ...
                "refund_rate": cls.calculate_percentage_change(
                    result["current"]["refund_rate"], result["previous"]["refund_rate"]
                ),
            }
    ...
    def get_sales_by_source(
        ...
        if compare:
            ...
            prev_by_source = {}
            for source_code, _source_label in Order.SOURCE_CHOICES:
            ...
            for source in result["by_source"]:
                ...
                source["changes"] = {
                    "revenue": cls.calculate_percentage_change(source["revenue"], prev["revenue"]),
                    "orders": cls.calculate_percentage_change(
                        Decimal(source["orders"]), Decimal(prev["orders"])
                    ),
                }
    ...
    @classmethod
    def get_utm_campaign_performance(
        cls, start_date: datetime, end_date: datetime, compare: bool = True
    ) -> dict:
        """
        ...
        # Group by campaign
        campaigns = []
        campaign_groups = current_visitors.values(
        ...
        if compare:
            ...
            result["comparison"] = {
                ...
                "change_pct": cls.calculate_percentage_change(
                    Decimal(current_visitors.count()), Decimal(prev_visitors.count())
                ),
            }
    ...
    def get_traffic_sources(
        ...
        utm_sources = []
        utm_groups = (
            current_visitors.exclude(utm_source="")
            .values("utm_source")
            .annotate(visitor_count=Count("id"))
            ...
            utm_sources.append(
                {
                    ...
                    "visitors": group["visitor_count"],
                    "percentage": (
                        Decimal(group["visitor_count"]) / Decimal(current_visitors.count()) * 100
                    )
            ...
            referrer_domains.append(
                {
                    "domain": domain,
                    "visitors": count,
                    "percentage": (Decimal(count) / Decimal(current_visitors.count()) * 100)
                    if current_visitors.count() > 0
                    else Decimal("0.00"),
                }
            )
        ...
        if compare:
            ...
            result["comparison"] = {
                ...
                "direct_change_pct": cls.calculate_percentage_change(
                    Decimal(direct_traffic), Decimal(prev_direct)
                ),
            }
    ...
    def get_traffic_by_device(
        ...
        if compare:
            ...
            prev_breakdown = {}
            for device_code, _device_label in VisitorLocation.DEVICE_TYPE_CHOICES:
            ...
            for device in device_breakdown:
                ...
                result["comparison"]["device_changes"][device_code] = (
                    cls.calculate_percentage_change(
                        Decimal(device["visitors"]), Decimal(prev_count)
                    )
                )
    ...
    def get_conversion_by_device(
        ...
        for device_code, device_label in VisitorLocation.DEVICE_TYPE_CHOICES:
            ...
            if device_visitors > 0:
                ...
                device_orders = (
                    int(current_orders.count() * (device_visitors / current_visitors.count()))
                    if current_visitors.count() > 0
                    else 0
                )

                conversion_rate = (
                    (Decimal(device_orders) / Decimal(device_visitors) * 100)
                    if device_visitors > 0
                    else Decimal("0.00")
                )
        ...
        if compare:
            ...
            result["comparison"] = {
                ...
                "order_change_pct": cls.calculate_percentage_change(
                    Decimal(current_orders.count()), Decimal(prev_orders.count())
                ),
            }
    ...
    def get_revenue_by_device(
        ...
        for device_code, device_label in VisitorLocation.DEVICE_TYPE_CHOICES:
            ...
            if device_visitors > 0:
                ...
                visitor_ratio = (
                    Decimal(device_visitors) / Decimal(current_visitors.count())
                    if current_visitors.count() > 0
                    else Decimal("0.00")
                )
                ...
                device_revenue.append(
                    {
                        ...
                        "visitors": device_visitors,
                        "revenue_per_visitor": device_revenue_amount / Decimal(device_visitors)
                        if device_visitors > 0
                        else Decimal("0.00"),
        ...
        if compare:
            ...
            result["comparison"] = {
                ...
                "revenue_change_pct": cls.calculate_percentage_change(total_revenue, prev_revenue),
            }
    ...
    def get_profit_metrics(
        ...
        order_items = OrderItem.objects.filter(order__in=current_orders).select_related("product")
        ...
        refunds_for_orders = Refund.objects.filter(
            order__in=current_orders, status="completed"
        ...
        for product_data in product_profits.values():
            ...
            product_data["margin_pct"] = (
                (product_data["profit"] / product_data["revenue"] * 100)
        ...
        if compare:
            ...
            prev_refunds_for_orders = Refund.objects.filter(
                order__in=prev_orders, status="completed"
            ).select_related("order")
            ...
            prev_margin_percentage = (
                (prev_profit / prev_revenue * 100) if prev_revenue > 0 else Decimal("0.00")
            )
            ...
            result["changes"] = {
                ...
                "margin_percentage": prev_margin_percentage - margin_percentage
                if prev_revenue > 0
                else Decimal("0.00"),
            }
    ...
    @classmethod
    def get_abandoned_cart_metrics(
        cls, start_date: datetime, end_date: datetime, compare: bool = True
    ) -> dict:
        """
        ...
        current_abandoned = AbandonedCart.objects.filter(
            abandoned_at__gte=start_date, abandoned_at__lt=end_date
        )
        ...
        total_value_stats = current_abandoned.aggregate(
            total=Coalesce(
                Sum("total_value", output_field=DecimalField()),
                Value(0, output_field=DecimalField()),
            )
        )
        ...
        recovery_rate = (
            (recovered_count / total_abandoned * 100) if total_abandoned > 0 else Decimal("0.00")
        )

        # Calculate average cart value
        avg_cart_value = (
            (total_potential_value / total_abandoned) if total_abandoned > 0 else Decimal("0.00")
        )
        ...
        recent_carts = (
            current_abandoned.filter(recovered=False)
            .select_related("user", "cart")
            .prefetch_related("cart__items__product")[:20]
        )
        ...
        if compare:
            ...
            prev_abandoned = AbandonedCart.objects.filter(
                abandoned_at__gte=prev_start, abandoned_at__lt=prev_end
            )
            ...
            prev_recovery_rate = (
                (prev_recovered_count / prev_total_abandoned * 100)
                if prev_total_abandoned > 0
                else Decimal("0.00")
            )

            prev_avg_cart_value = (
                (prev_total_potential_value / prev_total_abandoned)
                if prev_total_abandoned > 0
                else Decimal("0.00")
            )
            ...
            result["changes"] = {
                "total_abandoned": cls.calculate_percentage_change(
                    total_abandoned, prev_total_abandoned
                ),
                "recovered_count": cls.calculate_percentage_change(
                    recovered_count, prev_recovered_count
                ),
                "unrecovered_count": cls.calculate_percentage_change(
                    unrecovered_count, prev_unrecovered_count
                ),
                "total_potential_value": cls.calculate_percentage_change(
                    total_potential_value, prev_total_potential_value
                ),
                "recovered_value": cls.calculate_percentage_change(
                    recovered_value, prev_recovered_value
                ),
                "unrecovered_value": cls.calculate_percentage_change(
                    unrecovered_value, prev_unrecovered_value
                ),
                "recovery_rate": prev_recovery_rate - recovery_rate
                if prev_total_abandoned > 0
                else Decimal("0.00"),
                "avg_cart_value": cls.calculate_percentage_change(
                    avg_cart_value, prev_avg_cart_value
                ),
            }
    ...
    def get_voucher_performance(
        ...
        current_usages = VoucherUsage.objects.filter(
            used_at__gte=start_date, used_at__lt=end_date
        ...
        discount_stats = current_usages.aggregate(
            total_discount=Coalesce(
                Sum("discount_amount", output_field=DecimalField()),
                Value(0, output_field=DecimalField()),
            ),
            total_revenue=Coalesce(
                Sum("cart_total", output_field=DecimalField()),
                Value(0, output_field=DecimalField()),
            ),
        )
        ...
        discount_rate = (
            (total_discount / total_revenue * 100) if total_revenue > 0 else Decimal("0.00")
        )
        ...
        top_by_usage = (
            current_usages.values("voucher__code", "voucher__name", "voucher__discount_type")
            .annotate(
                use_count=Count("id"),
                total_discount=Coalesce(
                    Sum("discount_amount", output_field=DecimalField()),
        ...
        for voucher_data in top_by_usage:
            discount_rate_pct = (
                (voucher_data["total_discount"] / voucher_data["total_revenue"] * 100)
                if voucher_data["total_revenue"] > 0
                else Decimal("0.00")
            )
        ...
        if compare:
            ...
            prev_usages = VoucherUsage.objects.filter(used_at__gte=prev_start, used_at__lt=prev_end)
            ...
            prev_avg_discount = (
                (prev_total_discount / prev_total_uses) if prev_total_uses > 0 else Decimal("0.00")
            )
            prev_discount_rate = (
                (prev_total_discount / prev_total_revenue * 100)
            ...
            result["changes"] = {
                ...
                "total_discount": cls.calculate_percentage_change(
                    total_discount, prev_total_discount
                ),
                ...
                "discount_rate": prev_discount_rate - discount_rate
                if prev_total_revenue > 0
                else Decimal("0.00"),
            }
    ...
    def get_customer_segmentation(cls) -> dict:
        ...
        for segment in segments:
            ...
            for metrics in all_metrics:
                if segment.matches_user(metrics):
                    ...
                    segment_ltv += (
                        Decimal(str(metrics.lifetime_value.amount))
                        if metrics.lifetime_value
                        else Decimal("0.00")
                    )
                    segment_aov += (
                        Decimal(str(metrics.average_order_value.amount))
                        if metrics.average_order_value
                        else Decimal("0.00")
                    )
        ...
        for segment in segment_data:
            segment["percentage"] = (
                (segment["customer_count"] / total_customers * 100) if total_customers > 0 else 0.0
            )
    ...
    def get_top_products(
        cls, start_date: datetime, end_date: datetime, limit: int = 10
    ) -> list[dict]:
        """
    ...
    def get_visitor_analytics(
        ...
        all_visitors = VisitorLocation.objects.filter(
            first_seen__gte=start_date, first_seen__lt=end_date
        )

        # Real human storefront visitors only (exclude bots and admin traffic)
        current_visitors = all_visitors.filter(is_bot=False, is_admin_traffic=False)
        ...
        total_page_views = current_visitors.aggregate(total=Coalesce(Sum("page_views"), Value(0)))[
            "total"
        ]
        ...
        if compare:
            ...
            prev_page_views = prev_visitors.aggregate(total=Coalesce(Sum("page_views"), Value(0)))[
                "total"
            ]
            ...
            prev_conversion_rate = (
                (prev_orders / prev_unique_visitors * 100) if prev_unique_visitors > 0 else 0
            )
            ...
            result["changes"] = {
                "unique_visitors": cls.calculate_percentage_change(
                    Decimal(unique_visitors), Decimal(prev_unique_visitors)
                ),
                "page_views": cls.calculate_percentage_change(
                    Decimal(total_page_views), Decimal(prev_page_views)
                ),
                "new_customers": cls.calculate_percentage_change(
                    Decimal(new_customers), Decimal(prev_new_customers)
                ),
                "conversion_rate": cls.calculate_percentage_change(
                    Decimal(str(conversion_rate)), Decimal(str(prev_conversion_rate))
                ),
            }
    ...
    def get_visitor_geography(
        ...
        if country_code:
            geography = (
                base_query.filter(resolved_country=country_code)
                .exclude(resolved_city="")
                .values("resolved_city", "resolved_region")
        ...
        # Country-level view
        geography = (
            base_query.exclude(resolved_country="")
    ...
    def determine_grouping(
        start_date: datetime, end_date: datetime, grouping: str | None = None
    ) -> str:
        """
    ...
    def get_sales_over_time(
        ...
        # Convert to dict for easy lookup
        current_dict = {
            item["period"]: {"revenue": item["revenue"], "orders": item["orders"]}
        ...
        while current < end_date:
            if actual_grouping == "month":
                ...
                period_key = (current - timedelta(days=current.weekday())).replace(
            ...
            data = current_dict.get(
                period_key.date() if actual_grouping == "day" else period_key,
    ...
    @classmethod
    def get_views_over_time_grouped(
        cls,
        start_date: datetime,
        end_date: datetime,
        compare: bool = True,
        ...
        current_views = (
            ...
            .annotate(period=trunc_func("first_seen"))
            .values("period")
            .annotate(views=Sum("page_views"))
            .order_by("period")
        )
        ...
        while current < end_date:
            ...
            result["current"]["data"].append(
                current_dict.get(period_key.date() if actual_grouping == "day" else period_key, 0)
            )
        ...
        if compare:
            ...
            while current < prev_end:
                ...
                result["previous"]["labels"].append(label)
                result["previous"]["data"].append(
                    prev_dict.get(period_key.date() if actual_grouping == "day" else period_key, 0)
                )
    ...
    def get_loyalty_summary(cls, start_date: datetime, end_date: datetime) -> dict:
        ...
        points_redeemed = (
            LoyaltyTransaction.objects.filter(
                created_at__gte=start_date, created_at__lt=end_date, transaction_type="redeem"
            ).aggregate(
                total=Coalesce(
                    Sum("points", output_field=DecimalField()),
                    Value(0, output_field=DecimalField()),
        ...
        for tier in tiers:
            member_count = LoyaltyMember.objects.filter(is_active=True, current_tier=tier).count()
        ...
        no_tier_count = LoyaltyMember.objects.filter(
            is_active=True, current_tier__isnull=True
        ).count()
    ...
    @classmethod
    def get_email_campaign_roi(
        cls, start_date: datetime, end_date: datetime, template_id=None
    ) -> dict:
        """
        ...
        emails_query = EmailOutbox.objects.filter(
            sent_at__gte=start_date, sent_at__lt=end_date, status="sent"
        )
        ...
        if template_id:
            conversions_query = conversions_query.filter(
                attributed_email__template_type=template_id
            )
        ...
        # Calculate rates
        open_rate = (
            (Decimal(total_opened) / Decimal(total_sent) * 100)
            if total_sent > 0
            else Decimal("0.00")
        )
        click_rate = (
            (Decimal(total_clicked) / Decimal(total_sent) * 100)
            if total_sent > 0
            else Decimal("0.00")
        )
        conversion_rate = (
            (Decimal(total_conversions) / Decimal(total_sent) * 100)
            if total_sent > 0
            else Decimal("0.00")
        )
        revenue_per_email = (
            (total_revenue / Decimal(total_sent)) if total_sent > 0 else Decimal("0.00")
        )
        ...
        if not template_id:  # Only show breakdown if not filtering by specific template
            ...
            for template_type in template_types:
                ...
                roi = (
                    (template_revenue / Decimal(sent_count)) if sent_count > 0 else Decimal("0.00")
                )
    ...
    def get_customer_messages(cls, limit: int = 10) -> list[dict]:
        ...
        for note in customer_notes:
            ...
            customer_name = (
                customer.get_full_name() if customer else note.order.shipping_name or "Guest"
            )
    ...
    def get_shipment_summary(cls, start_date: datetime, end_date: datetime) -> dict:
        ...
        delivered_today_count = Shipment.objects.filter(
            status="delivered", updated_at__gte=today_start, updated_at__lt=today_end
        ).count()
    ...
    @classmethod
    def get_sales_channel_performance(
        ...
        if compare:
            ...
            for current_channel in current_data["by_channel"]:
                ...
                current_channel["revenue_change"] = cls.calculate_percentage_change(
                    current_channel["revenue"], prev_channel["revenue"]
                )

                current_channel["order_change"] = cls.calculate_percentage_change(
                    current_channel["orders"], prev_channel["orders"]
                )

                current_channel["aov_change"] = cls.calculate_percentage_change(
                    current_channel["aov"], prev_channel["aov"]
                )
            ...
            total_orders_change = cls.calculate_percentage_change(
                current_data["total_orders"], previous_data["total_orders"]
            )
    ...
    def get_conversion_funnel(cls, start_date: datetime, end_date: datetime) -> dict:
        ...
        payment_initiated_count = CheckoutSession.objects.filter(
            created_at__gte=start_date, created_at__lte=end_date, step_completed__in=payment_steps
        ).count()
    ...
    def get_traffic_source_analytics(cls, start_date: datetime, end_date: datetime) -> dict:
        ...
        for visitor in visitors_with_referrer.only("referrer_url", "session_key"):
            try:
                ...
                if domain not in referrer_data:
                    referrer_data[domain] = {"visitors": 0, "session_keys": []}
        ...
        for domain, data in referrer_data.items():
            ...
            conversion_rate = (
                (Decimal(total_orders) / Decimal(total_visitors) * 100)
                if total_visitors > 0
                else Decimal("0.00")
            )
            estimated_conversions = (
                int((Decimal(visitors_count) / Decimal(total_visitors)) * Decimal(total_orders))
                if total_visitors > 0
                else 0
            )
            estimated_revenue = (
                (total_revenue / Decimal(total_visitors)) * Decimal(visitors_count)
                if total_visitors > 0
                else Decimal("0.00")
            )
            ...
            top_referrers.append(
                {
                    ...
                    "percentage": (Decimal(visitors_count) / Decimal(total_visitors) * 100)
                    if total_visitors > 0
                    else Decimal("0.00"),
                    "conversion_rate": conversion_rate,
                }
            ...
            utm_campaigns.append(
                {
                    ...
                    "conversion_rate": conversion_rate,
                    "revenue": estimated_revenue,
                    "revenue_per_visitor": estimated_revenue / Decimal(visitors_count)
                    if visitors_count > 0
                    else Decimal("0.00"),
                }
            )
        ...
        device_breakdown = []
        device_groups = visitors.values("device_type").annotate(visitor_count=Count("id"))
        ...
        traffic_by_type = [
            {
                ...
                "percentage": (Decimal(direct_traffic) / Decimal(total_visitors) * 100)
                if total_visitors > 0
                else Decimal("0.00"),
                "color": "#3498db",
            },
            {
                "type": "UTM Campaigns",
                "count": utm_traffic,
                "percentage": (Decimal(utm_traffic) / Decimal(total_visitors) * 100)
                if total_visitors > 0
                else Decimal("0.00"),
                "color": "#2ecc71",
            },
            {
                ...
                "percentage": (Decimal(organic_traffic) / Decimal(total_visitors) * 100)
                if total_visitors > 0
                else Decimal("0.00"),
                "color": "#9b59b6",
            },
            {
                ...
                "percentage": (
                    Decimal(referral_traffic - organic_traffic) / Decimal(total_visitors) * 100
                )
                if total_visitors > 0
                else Decimal("0.00"),
                "color": "#e74c3c",
        ...
        traffic_trends = list(
            visitors.annotate(date=TruncDate("first_seen"))
            .values("date")
            .annotate(visitors=Count("id"))
            .order_by("date")
        )
        ...
        revenue_per_visitor = (
            total_revenue / Decimal(total_visitors) if total_visitors > 0 else Decimal("0.00")
        )
        ...
        return {
            "summary": {
                ...
                "utm_traffic_percentage": (Decimal(utm_traffic) / Decimal(total_visitors) * 100)
                if total_visitors > 0
                else Decimal("0.00"),
                "direct_traffic_percentage": (
                    Decimal(direct_traffic) / Decimal(total_visitors) * 100
                )
                if total_visitors > 0
                else Decimal("0.00"),
            },
