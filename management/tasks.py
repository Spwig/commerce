...
def run_full_backup(self, backup_id, backup_type='full', encrypt=False, destination_ids=None):
    ...
    from .models import DeploymentBackup, BackupRemoteUpload
...
def run_restore(self, restore_id):
    ...
    import os
    from django.conf import settings
    from .models import SystemRestore, DeploymentBackup
    ...
    try:
        ...
        if not backup_path or not os.path.exists(backup_path):
            ...
            remote_upload = BackupRemoteUpload.objects.filter(
            ...
            if not remote_upload:
                raise Exception(
                    "No backup file available — not on local disk or remote storage"
                )
            ...
            provider = StorageProviderRegistry.create_from_destination(
                remote_upload.destination
            )
            ...
            if not success:
                raise Exception(
                    f"Failed to download backup from {remote_upload.destination.name}"
                )
    ...
    try:
        ...
        while polls < max_polls:
            ...
            try:
                ...
                if was_in_progress and status == 'idle' and not operation.get('completed') and not operation.get('failed'):
                    error_msg = (
                    ...
                    logger.error(f"Upgrade {upgrade_id} failed: upgrader crash detected (in_progress -> idle)")
        ...
        while polls < max_polls:
            ...
            try:
                ...
                if was_in_progress and status == 'idle' and not operation.get('completed') and not operation.get('failed'):
                    ...
                    upgrade.error_message = error_msg
                    upgrade.save(update_fields=['status', 'error_message'])
                    logger.error(f"Rollback {upgrade_id} failed: upgrader crash detected (in_progress -> idle)")
                    return {"success": False, "error": error_msg}
...
def run_scheduled_backup():
    ...
    try:
        ...
        if schedule.next_run and now < schedule.next_run:
            return {"success": True, "message": "Backup not yet due", "next_run": schedule.next_run.isoformat()}
        ...
        # Run backup
        result = run_full_backup.apply(args=[
            backup.id,
...
def collect_docker_logs():
    ...
    try:
        ...
        for container in DockerLogService.CONTAINER_SERVICES.keys():
            try:
...
def cleanup_old_logs():
    ...
    try:
        ...
        logger.info(f"Cleaned up {deleted_count} old log entries (retention: {retention_days} days)")
...
def upload_backup_to_remote(self, upload_id):
    ...
    try:
        upload = BackupRemoteUpload.objects.select_related(
            'backup', 'destination'
        ).get(pk=upload_id)
    ...
    upload.save(update_fields=[
        'status', 'started_at', 'task_id', 'file_size', 'updated_at',
    ])
    ...
    try:
        ...
        def on_progress(bytes_transferred, total_bytes):
            if total_bytes > 0:
                ...
                if pct - last_pct[0] >= 5 or pct >= 100:
                    last_pct[0] = pct
                    BackupRemoteUpload.objects.filter(pk=upload_id).update(
        ...
        if result.success:
            ...
            upload.save(update_fields=[
                'status', 'remote_path', 'progress_percent',
            ...
            destination.save(update_fields=[
                'last_upload_at', 'last_upload_size',
                'total_uploads', 'total_bytes_uploaded', 'updated_at',
            ])

            logger.info(
                f"Upload {upload_id}: {filename} → {destination.name} completed "
                f"({file_size} bytes)"
            )
...
def cleanup_remote_backups():
    ...
    for dest in destinations:
        try:
            ...
            logger.info(
                f"Remote cleanup: {dest.name} — deleted {deleted_count} old file(s)"
            )
            results.append({
