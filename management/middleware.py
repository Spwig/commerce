...
class DatabaseSecurityMixin(ManagementSecurityMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (
            hasattr(settings, "MANAGEMENT_DISABLE_DB_ACCESS")
            and settings.MANAGEMENT_DISABLE_DB_ACCESS
        ):
...
class FileSecurityMixin(ManagementSecurityMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (
            hasattr(settings, "MANAGEMENT_DISABLE_FILE_OPS")
            and settings.MANAGEMENT_DISABLE_FILE_OPS
        ):
...
class RateLimitMixin:
    ...
    def check_rate_limit(self, request, operation_type):
        ...
        # Increment counter
        timeout = (
            3600 if operation_type == "backup" else 60
        )  # 1 hour for backup, 1 minute for others
