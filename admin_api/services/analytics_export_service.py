...
class AnalyticsExportService:
    ...
    def _csv_products(cls, writer, start_date, end_date):
        ...
        data = AnalyticsService.get_product_analytics(start_date, end_date, page_size=10000)
        currency = data["currency"]

        writer.writerow([f"Product Analytics Report ({start_date} to {end_date})"])
        writer.writerow(
            [
                f"Currency: {currency}",
    ...
    def _csv_customers(cls, writer, start_date, end_date):
        ...
        data = AnalyticsService.get_customer_analytics(start_date, end_date, page_size=10000)
        currency = data["currency"]
        summary = data["summary"]

        writer.writerow([f"Customer Analytics Report ({start_date} to {end_date})"])
        writer.writerow(
            [
        ...
        writer.writerow(
            [
                ...
                "Email",
                "Segment",
                f"Total Spent ({currency})",
                "Total Orders",
                f"Period Spent ({currency})",
                "Period Orders",
                "Joined",
            ]
        )
    ...
    def _csv_categories(cls, writer, start_date, end_date):
        ...
        writer.writerow([f"Category Analytics Report ({start_date} to {end_date})"])
        writer.writerow(
            [
                f"Currency: {currency}",
                f"Total Revenue: {cls._format_decimal(data['summary']['total_revenue'])}",
    ...
    def _csv_brands(cls, writer, start_date, end_date):
        ...
        writer.writerow([f"Brand Analytics Report ({start_date} to {end_date})"])
        writer.writerow(
            [
                f"Currency: {currency}",
                f"Total Revenue: {cls._format_decimal(data['summary']['total_revenue'])}",
    ...
    def _csv_orders(cls, writer, start_date, end_date):
        ...
        writer.writerow([f"Orders Report ({start_date} to {end_date})"])
    ...
    def _csv_summary(cls, writer, start_date, end_date):
        ...
        writer.writerow([f"Analytics Summary ({start_date} to {end_date})"])
        writer.writerow([f"Currency: {currency}"])
        writer.writerow([])
    ...
    def export_pdf(cls, report_type: str, start_date: date, end_date: date) -> BytesIO:
        ...
        story.append(
            Paragraph(f"{start_date.isoformat()} to {end_date.isoformat()}", styles["SmallText"])
        )
        ...
        # Footer
        story.append(Spacer(1, 0.3 * inch))
        story.append(Paragraph("Generated by Spwig Analytics", styles["SmallText"]))
    ...
    def _pdf_products(cls, story, styles, start_date, end_date):
        ...
        data = AnalyticsService.get_product_analytics(start_date, end_date, page_size=100)
        currency = data["currency"]
        summary = data["summary"]
        ...
        story.append(
            cls._build_table(summary_rows[0], summary_rows[1:], col_widths=[2.5 * inch, 3.5 * inch])
        )
    ...
    def _pdf_customers(cls, story, styles, start_date, end_date):
        ...
        data = AnalyticsService.get_customer_analytics(start_date, end_date, page_size=50)
        currency = data["currency"]
        summary = data["summary"]
        ...
        ltv_rows = [[b["label"], str(b["count"])] for b in data["ltv_distribution"]]
        if ltv_rows:
            story.append(cls._build_table(ltv_header, ltv_rows, col_widths=[3 * inch, 3 * inch]))
        story.append(Spacer(1, 0.2 * inch))
        ...
        if data["geo_breakdown"]:
            ...
            story.append(
                cls._build_table(
                    geo_header, geo_rows, col_widths=[2 * inch, 1 * inch, 1.5 * inch, 1 * inch]
                )
            )
    ...
    def _pdf_orders(cls, story, styles, start_date, end_date):
        ...
        story.append(
            Paragraph(f"Showing {min(200, total_count)} of {total_count} orders", styles["Normal"])
        )
        story.append(Spacer(1, 0.2 * inch))
    ...
    def _pdf_summary(cls, story, styles, start_date, end_date):
        ...
        kpi_rows = [
            ...
            [
                "Average Order Value",
                f"{currency} {cls._format_decimal(kpi['average_order_value'])}",
            ],
        ]
        story.append(
            cls._build_table(kpi_rows[0], kpi_rows[1:], col_widths=[2.5 * inch, 3.5 * inch])
        )
        story.append(Spacer(1, 0.2 * inch))
        ...
        if cat_rows:
            story.append(
                cls._build_table(
                    cat_header, cat_rows, col_widths=[2 * inch, 1.5 * inch, 1 * inch, 1 * inch]
                )
            )
        story.append(Spacer(1, 0.2 * inch))
        ...
        if brand_rows:
            story.append(
                cls._build_table(
                    brand_header, brand_rows, col_widths=[2 * inch, 1.5 * inch, 1 * inch, 1 * inch]
                )
            )
