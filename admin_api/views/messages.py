...
from admin_api.models import CustomerMessage, MessageReply, MessageReadReceipt
    ...
    **Deep Linking:**
    ...
    parameters=[
        ...
        OpenApiParameter(
            ...
            type=str,
            location=OpenApiParameter.QUERY,
            description=_("Sort order: '-activity' (latest activity, default), '-created_at' (newest), 'created_at' (oldest), 'status' (group by status)"),
            required=False,
            default='-activity'
        ),
        OpenApiParameter(
...
def message_list(request):
    ...
    if source_filter in ('all', 'order_note'):
        on_queryset = OrderNote.objects.filter(
...
def message_detail(request, source, message_id):
    ...
    if source == 'contact_form':
        ...
        try:
            note = OrderNote.objects.filter(
                is_customer_note=True
            ).select_related('order').get(id=message_id)
...
def unread_count(request):
    ...
    on_all_ids = set(
        OrderNote.objects.filter(is_customer_note=True).values_list('id', flat=True)
    )
    on_read_ids = set(
        MessageReadReceipt.objects.filter(
            source='order_note',
