...
@extend_schema(
    ...
    **Rate Limit:** 300 requests per minute
    ...
    parameters=[
        ...
        OpenApiParameter(name='parent_id', type=int, required=False, description=_('Filter by parent category ID')),
        OpenApiParameter(name='is_active', type=str, required=False, description=_('Filter by active status (all/true/false)')),
        ...
        OpenApiParameter(name='page_size', type=int, required=False, description=_('Items per page (max 100)')),
    ],
...
def category_detail(request, category_id):
    ...
    try:
        category = Category.objects.select_related('parent').annotate(
            product_count=Count('products', filter=Q(products__is_deleted=False))
        ).get(id=category_id)
...
@extend_schema(
    ...
    **Rate Limit:** 30 requests per minute (sensitive operation)
    ...
    responses={201: AdminCategoryDetailSerializer, 400: ErrorResponseSerializer, 409: ErrorResponseSerializer}
)
@api_view(['POST'])
@permission_classes([IsStaffWithWritePermission])
...
@extend_schema(
    ...
    **Rate Limit:** 30 requests per minute (sensitive operation)
    ...
    responses={200: AdminCategoryDetailSerializer, 400: ErrorResponseSerializer, 404: ErrorResponseSerializer}
)
@api_view(['PATCH'])
@permission_classes([IsStaffWithWritePermission])
@throttle_classes([AdminSensitiveOperationThrottle])
def update_category(request, category_id):
    ...
    for field in ['name', 'description', 'icon', 'is_active', 'is_featured',
                  'sort_order', 'products_per_page', 'page_template',
...
def delete_category(request, category_id):
    ...
    if product_count > 0:
        return Response({
            ...
            'error': {
                'code': 409,
                'message': _('Cannot delete category with %(count)d associated product(s). Reassign or delete products first.') % {'count': product_count},
                'reference': generate_error_reference()
            }
        }, status=status.HTTP_409_CONFLICT)
    ...
    if child_count > 0:
        return Response({
            ...
            'error': {
                'code': 409,
                'message': _('Cannot delete category with %(count)d child category(ies). Delete or reassign children first.') % {'count': child_count},
                'reference': generate_error_reference()
            }
        }, status=status.HTTP_409_CONFLICT)
...
def _upload_category_asset(request, category_id, field_name):
    ...
    try:
        ...
        old_asset = getattr(category, field_name)
