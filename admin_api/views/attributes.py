...
from catalog.models import Product, ProductAttribute, AttributeValue, ProductAttributeAssignment
...
@extend_schema(
    ...
    **Rate Limit:** 30 requests per minute (sensitive operation)
    ...
    responses={201: AdminAttributeListSerializer, 400: ErrorResponseSerializer, 409: ErrorResponseSerializer}
)
@api_view(['POST'])
@permission_classes([IsStaffWithWritePermission])
...
def assign_product_attributes(request, product_id):
    ...
    for item in assignments_data:
        ...
        valid_value_ids = set(
            AttributeValue.objects.filter(
                id__in=value_ids, attribute=attribute
    ...
    return Response({
        'success': True,
        'message': _('Attributes assigned successfully. %(created)d created, %(updated)d updated.') % {
