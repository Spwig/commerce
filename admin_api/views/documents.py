...
def _generate_invoice_pdf(order, site_settings=None):
    ...
    items_table = Table(
        items_data,
        colWidths=[1 * inch, 2.5 * inch, 0.5 * inch, 1 * inch, 1 * inch]
    )
...
def _generate_packing_slip_pdf(order, site_settings=None):
    ...
    items_table = Table(
        items_data,
        colWidths=[1.2 * inch, 2.5 * inch, 1.3 * inch, 1 * inch]
    )
    items_table.setStyle(TableStyle([
        # Header
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#34495e')),
    ...
    if packing_footer:
        ...
        story.append(Paragraph(
            _("Please verify all items are included before shipping."),
            styles['SmallText']
        ))
...
def _generate_pick_list_pdf(order, site_settings=None):
    ...
    for warehouse_name, items in items_by_warehouse.items():
        ...
        pick_table = Table(
            pick_data,
            colWidths=[1.2 * inch, 2.3 * inch, 1.2 * inch, 0.6 * inch, 0.7 * inch]
        )
    ...
    story.append(Paragraph(
        _("Picked by: ____________________  Date: ____________________"),
        styles['Normal']
    ))
...
def order_packing_slip_pdf(request, order_number):
    ...
    try:
        ...
        logger.error(f"Packing slip PDF generation failed for order {order_number}: {e}", exc_info=True)
        return Response({
            'success': False,
            'error': {
                'code': 500,
...
def order_pick_list_pdf(request, order_number):
    ...
    try:
        ...
        logger.error(f"Pick list PDF generation failed for order {order_number}: {e}", exc_info=True)
        return Response({
            'success': False,
            'error': {
                'code': 500,
