...
def role_create(request):
    ...
    with transaction.atomic():
        # Create Django Group
        group = Group.objects.create(name=f"role_{name.lower().replace(' ', '_')}_{secrets.token_hex(3)}")
...
@extend_schema(
    ...
    description=_("Update name, description, or permissions of a custom role. Cannot modify built-in roles."),
