...
def register_device(request):
    ...
    return Response({
        'success': True,
        'message': _('Device registered successfully.') if created else _('Device updated successfully.'),
...
def list_devices(request):
    ...
    return Response({
        'success': True,
        'data': DeviceListSerializer(devices, many=True).data
    }, status=status.HTTP_200_OK)
...
def active_sessions(request):
    ...
    device_registrations = {
        d.device_id: d.platform
        for d in DeviceRegistration.objects.filter(user=request.user)
    }
...
def revoke_session(request, device_id):
    ...
    MobileAuthToken.revoke_all_for_device(
        request.user,
        device_id,
        reason='Revoked by user from another device'
    )
...
def available_languages(request):
    ...
    return Response({
        'success': True,
        'data': {
            'current': settings.LANGUAGE_CODE,
            'available': languages
        }
    }, status=status.HTTP_200_OK)
