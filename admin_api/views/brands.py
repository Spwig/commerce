...
@extend_schema(
    ...
    **Rate Limit:** 30 requests per minute (sensitive operation)
    ...
    responses={201: AdminBrandDetailSerializer, 400: ErrorResponseSerializer, 409: ErrorResponseSerializer}
)
@api_view(['POST'])
@permission_classes([IsStaffWithWritePermission])
...
@extend_schema(
    ...
    **Rate Limit:** 30 requests per minute (sensitive operation)
    ...
    responses={200: AdminBrandDetailSerializer, 400: ErrorResponseSerializer, 404: ErrorResponseSerializer}
)
@api_view(['PATCH'])
@permission_classes([IsStaffWithWritePermission])
@throttle_classes([AdminSensitiveOperationThrottle])
def update_brand(request, brand_id):
    ...
    for field in ['name', 'description', 'website', 'show_brand_page', 'brand_story',
...
def delete_brand(request, brand_id):
    ...
    if product_count > 0:
        return Response({
            ...
            'error': {
                'code': 409,
                'message': _('Cannot delete brand with %(count)d associated product(s). Reassign or delete products first.') % {'count': product_count},
                'reference': generate_error_reference()
            }
        }, status=status.HTTP_409_CONFLICT)
