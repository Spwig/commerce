...
def bulk_stock_adjust(request):
    ...
    for idx, adj in enumerate(adjustments):
        ...
        try:
            ...
            logger.error(
                f"Bulk stock adjustment failed for product {product_id}: {e}",
                exc_info=True
            )
            results.append({
                'index': idx,
...
def bulk_price_update(request):
    ...
    for product_id in product_ids:
        try:
            ...
            logger.error(
                f"Bulk price update failed for product {product_id}: {e}",
                exc_info=True
            )
            results.append({
                'product_id': product_id,
...
def bulk_assign_category(request):
    ...
    for product_id in product_ids:
        try:
            ...
            logger.error(
                f"Bulk category assignment failed for product {product_id}: {e}",
                exc_info=True
            )
            results.append({
                'product_id': product_id,
...
def bulk_assign_tags(request):
    ...
    for product_id in product_ids:
        try:
            ...
            logger.error(
                f"Bulk tag assignment failed for product {product_id}: {e}",
                exc_info=True
            )
            results.append({
                'product_id': product_id,
        ...
        try:
            with transaction.atomic():
                ...
                old_values = {
                    ...
                    'sale_start_date': (
                        product.sale_start_date.isoformat()
                        if product.sale_start_date else None
                    ),
                    'sale_end_date': (
                        product.sale_end_date.isoformat()
                        if product.sale_end_date else None
                    ),
                }
                ...
                new_values = {
                    ...
                    'sale_start_date': (
                        sale_start_date.isoformat() if sale_start_date else None
                    ),
                    'sale_end_date': (
                        sale_end_date.isoformat() if sale_end_date else None
                    ),
                }
            ...
            logger.error(
                f"Bulk sale update failed for product {product_id}: {e}",
                exc_info=True
            )
            results.append({
                'product_id': product_id,
