...
def sso_mobile_callback(request):
    ...
    if not code or not state:
        return HttpResponseRedirect(
            f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?error=missing_params'
        )
    ...
    if not state_data:
        ...
        return HttpResponseRedirect(
            f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?error=invalid_state'
        )
    ...
    if not enabled:
        return HttpResponseRedirect(
            f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?error=sso_disabled'
        )
    ...
    if not email:
        ...
        return HttpResponseRedirect(
            f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?error=no_email'
        )
    ...
    if not user:
        logger.warning("SSO mobile callback: no user found for email %s***@%s",
                        email[:2], email.split('@')[1] if '@' in email else '?')
        return HttpResponseRedirect(
            f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?error=user_not_found'
        )
    ...
    if config.restrict_to_staff and not user.is_staff:
        logger.warning("SSO mobile callback: user %s***@%s is not staff",
                        email[:2], email.split('@')[1] if '@' in email else '?')
        return HttpResponseRedirect(
            f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?error=unauthorized'
        )
    ...
    logger.info("SSO mobile: generated one-time code for user_id=%s (device %s)", user.id, device_id[:8])
    return HttpResponseRedirect(
        f'{MOBILE_SSO_CALLBACK_SCHEME}://callback?code={one_time_code}'
    )
...
def sso_mobile_token(request):
    ...
    if not code or not device_id or len(device_id) > 255 or not re.match(r'^[a-zA-Z0-9\-_.:]+$', device_id):
        return Response({
            'success': False,
            'error': {
                'code': 400,
    ...
    if code_data['device_id'] != device_id:
        logger.warning("SSO mobile token: device_id mismatch (expected %s, got %s)",
                        code_data['device_id'][:8], device_id[:8])
        return Response({
            'success': False,
            'error': {
                'code': 400,
    ...
    if not is_existing_device and existing_devices >= max_devices:
        return Response({
            ...
            'error': {
                'code': 403,
                'message': _('Maximum device limit reached. Please logout from another device.'),
                'reference': _generate_error_reference(),
            }
        }, status=status.HTTP_403_FORBIDDEN)
