...
def _verify_totp_or_recovery(user, code):
    ...
    try:
        ...
        if len(code) == 6 and code.isdigit():
            try:
                totp_auth = Authenticator.objects.get(user=user, type=Authenticator.Type.TOTP)
                totp = totp_auth.wrap()
...
def staff_login(request):
    ...
    if max_devices > 0:
        ...
        existing_devices = (
            MobileAuthToken.objects.filter(user=user, is_revoked=False, token_type="refresh")
            .values("device_id")
            .distinct()
            .count()
        )
            ...
            return Response(
                {
                    ...
                    "error": {
                        "code": 403,
                        "message": _(
                            "Maximum device limit reached. Please remove a device from your profile or ask an administrator to increase the limit."
                        ),
                        "reference": generate_error_reference(),
                    },
                },
    ...
    AuditService.log_login(
        user=user, device_id=device_id, device_name=device_name, request=request, success=True
    )
...
def refresh_token(request):
    ...
    try:
        with transaction.atomic():
            ...
            try:
                token = (
                    MobileAuthToken.objects.select_for_update(nowait=True)
                    .select_related("user")
                    .get(**query)
                )
...
def _perform_token_refresh(request, token, user, device_id):
    ...
    MobileAuthToken.objects.filter(
        user=user, device_id=device_id, token_type="access", is_revoked=False
    ).update(is_revoked=True, revoked_at=timezone.now(), revoked_reason="Token refreshed")
...
def staff_logout(request):
    ...
    if logout_all:
        ...
        AuditService.log_logout(
            user=request.user, device_id=device_id or "", logout_all=True, request=request
        )
        return Response(
            {"success": True, "message": _("Logged out from all devices.")},
            status=status.HTTP_200_OK,
        )
    elif device_id:
        MobileAuthToken.revoke_all_for_device(request.user, device_id, reason="Device logout")
        AuditService.log_logout(
            user=request.user, device_id=device_id, logout_all=False, request=request
        )
        return Response(
            {"success": True, "message": _("Logged out successfully.")}, status=status.HTTP_200_OK
        )
    else:
        return Response(
            {
...
def staff_profile(request):
    ...
    """
    return Response(
        {"success": True, "data": StaffProfileSerializer(request.user).data},
        status=status.HTTP_200_OK,
    )
...
def verify_2fa(request):
    ...
    try:
        pending_token = MobileAuthToken.objects.select_related("user").get(
            token=pending_token_str, token_type="2fa_pending", device_id=device_id
        )
    except MobileAuthToken.DoesNotExist:
        AuditService.log(
            user=None,
    ...
    if not success:
        ...
        return Response(
            {
                "success": False,
                "error": {"code": 400, "message": error, "reference": generate_error_reference()},
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if trust_device:
        TrustedDevice.trust_device(
            user=user, device_id=device_id, device_name=pending_token.device_name
        )
        AuditService.log(
            user=user,
    ...
    if max_devices > 0:
        ...
        # Check if this is a new device
        is_existing_device = MobileAuthToken.objects.filter(
            user=user, device_id=device_id, token_type="refresh", is_revoked=False
        ).exists()
...
@extend_schema(
    ...
    description=_(
        "Request a password reset email for a staff account. Always returns success to prevent email enumeration. Rate limited to 5 requests per minute."
    ),
    ...
    return Response(
        {
            "success": True,
            "message": _(
                "If a staff account with that email exists, a password reset link has been sent."
            ),
        },
        status=status.HTTP_200_OK,
    )
