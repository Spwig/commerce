...
class StaffLoginSerializer(serializers.Serializer):
    ...
    email = serializers.EmailField(
        required=False,
        help_text=_('Staff user email address')
    )
    username = serializers.CharField(
        required=False,
        help_text=_('Staff username or email (alternative to email field)')
    )
...
class TokenResponseSerializer(serializers.Serializer):
    ...
    expires_in = serializers.IntegerField(
        read_only=True,
        help_text=_('Access token expiry time in seconds')
    )
...
class RefreshTokenSerializer(serializers.Serializer):
    ...
    refresh_token = serializers.CharField(
        required=True,
        help_text=_('The refresh token to use')
    )
    device_id = serializers.CharField(
        required=False,
        max_length=255,
...
class LogoutSerializer(serializers.Serializer):
    ...
    logout_all = serializers.BooleanField(
        required=False,
        default=False,
        help_text=_('Logout from all devices')
    )
...
class AdminDataResponseSerializer(serializers.Serializer):
    ...
    data = serializers.DictField(
        help_text=_('Response payload (structure varies by endpoint)')
    )
    error = serializers.CharField(
        allow_null=True,
        default=None,
        help_text=_('Null on success')
    )
...
class BulkOperationResponseSerializer(serializers.Serializer):
    ...
    success = serializers.BooleanField(
        help_text=_('True if at least one item succeeded')
    )
    data = serializers.DictField(
        help_text=_(
            'Contains total, succeeded, failed counts and per-item results array'
        )
    )
    error = serializers.CharField(
        allow_null=True,
...
class TwoFactorVerifySerializer(serializers.Serializer):
    ...
    pending_token = serializers.CharField(
        required=True,
        help_text=_('The 2FA pending token received from login')
    )
    ...
    device_id = serializers.CharField(
        required=True,
        max_length=255,
        help_text=_('Device identifier (must match login request)')
    )
...
class TwoFactorRequiredResponseSerializer(serializers.Serializer):
    ...
    pending_token = serializers.CharField(
        read_only=True,
        help_text=_('Token to use for 2FA verification')
    )
    expires_in = serializers.IntegerField(
        read_only=True,
        help_text=_('Seconds until pending token expires')
    )
...
class StaffPasswordResetConfirmSerializer(serializers.Serializer):
    ...
    uid = serializers.CharField(
        required=True,
        help_text=_('Base64-encoded user ID from the reset link')
    )
    token = serializers.CharField(
        required=True,
        help_text=_('Password reset token from the reset link')
    )
