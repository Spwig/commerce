...
class BulkOrderFulfillItemSerializer(serializers.Serializer):
    ...
    order_number = serializers.CharField(
        max_length=32,
        help_text=_("Order number to fulfill.")
    )
    ...
    tracking_url = serializers.URLField(
        ...
        allow_blank=True,
        default='',
        help_text=_("Tracking URL for the shipment.")
    )
...
class BulkOrderFulfillSerializer(serializers.Serializer):
    ...
    notify_customers = serializers.BooleanField(
        default=True,
        help_text=_("Whether to send shipping notification emails to customers.")
    )
...
class StockAdjustmentItemSerializer(serializers.Serializer):
    ...
    product_id = serializers.IntegerField(
        help_text=_("Product ID to adjust stock for.")
    )
    ...
    quantity = serializers.IntegerField(
        help_text=_("Stock quantity: for 'set' this is the new value; for 'adjust' this is the delta (can be negative).")
    )
...
class BulkPriceUpdateSerializer(serializers.Serializer):
    ...
    value = serializers.DecimalField(
        max_digits=10,
        decimal_places=2,
        help_text=_("Price value: absolute amount or percentage change (e.g., -10 for 10%% decrease).")
    )
    apply_to = serializers.ChoiceField(
        choices=['price'],
        default='price',
        help_text=_("Which price field to update.")
    )
...
class BulkAssignCategorySerializer(serializers.Serializer):
    ...
    category_id = serializers.IntegerField(
        help_text=_("Target category ID to assign products to.")
    )
...
class BulkAssignTagsSerializer(serializers.Serializer):
    ...
    mode = serializers.ChoiceField(
        ...
        help_text=_("'add': add tags. 'replace': replace all tags. 'remove': remove specified tags.")
    )
...
class BulkSaleUpdateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if sale_value < 0:
            raise serializers.ValidationError({
                'sale_value': _('sale_value must not be negative.')
            })
