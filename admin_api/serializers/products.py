...
class ProductFilterSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=[("all", "All")] + Product.STATUS_CHOICES, required=False, default="all"
    )
    ...
    category_id = serializers.IntegerField(required=False, help_text=_("Filter by category ID"))
    brand_id = serializers.IntegerField(required=False, help_text=_("Filter by brand ID"))
...
class ProductImageUploadSerializer(serializers.Serializer):
    ...
    alt_text = serializers.CharField(
        max_length=255, required=False, allow_blank=True, help_text=_("Alt text for accessibility")
    )
    is_primary = serializers.BooleanField(
        default=False, help_text=_("Set as primary/featured image")
    )
...
class ProductUpdateSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(choices=[c[0] for c in Product.STATUS_CHOICES], required=False)
...
class BulkProductUpdateSerializer(serializers.Serializer):
    ...
    def validate_products(self, value):
        ...
        for item in value:
            if "id" not in item:
                raise serializers.ValidationError(_("Each product must have an 'id' field."))
        return value
