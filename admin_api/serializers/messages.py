...
class UnifiedMessageListSerializer(serializers.Serializer):
    ...
    reply_count = serializers.IntegerField(
        default=0, help_text=_("Number of replies in this thread")
    )
    last_reply_at = serializers.DateTimeField(
        allow_null=True, help_text=_("Timestamp of last reply")
    )
    ...
    read_by = ReadBySerializer(
        many=True, help_text=_("List of staff members who have read this message")
    )
    ...
    message_id = serializers.IntegerField(
        allow_null=True, help_text=_("CustomerMessage ID (for contact_form source)")
    )
    order_id = serializers.IntegerField(
        allow_null=True, help_text=_("Order ID (for order_note source)")
    )
    order_number = serializers.CharField(
        allow_null=True, help_text=_("Order number (for order_note source)")
    )
...
class UnifiedMessageDetailSerializer(serializers.Serializer):
    ...
    message_type = serializers.CharField(
        help_text=_("Message type (general, support, order, product, other)")
    )
    ...
    reply_text = serializers.CharField(
        allow_null=True, allow_blank=True, help_text=_("Latest staff reply text (backward compat)")
    )
    replied_at = serializers.DateTimeField(
        allow_null=True, help_text=_("When the latest staff reply was sent")
    )
    replied_by_name = serializers.CharField(
        allow_null=True, help_text=_("Name of staff member who last replied")
    )

    # Thread replies (full conversation history)
    replies = ThreadReplySerializer(
        many=True, help_text=_("All replies in this thread, chronologically ordered")
    )
    reply_count = serializers.IntegerField(
        default=0, help_text=_("Number of replies in this thread")
    )
...
class MessageReplyInputSerializer(serializers.Serializer):
    ...
    reply_text = serializers.CharField(max_length=5000, help_text=_("Reply message content"))
    send_email = serializers.BooleanField(
        default=True, help_text=_("Whether to send the reply via email to the customer")
    )
