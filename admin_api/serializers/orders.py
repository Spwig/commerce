...
class AdminOrderItemSerializer(serializers.ModelSerializer):
    ...
    unit_price = serializers.DecimalField(max_digits=10, decimal_places=2, source='unit_price.amount')
    total_price = serializers.DecimalField(max_digits=10, decimal_places=2, source='total_price.amount')
...
class AdminOrderListSerializer(serializers.ModelSerializer):
    ...
    total_amount = serializers.DecimalField(max_digits=10, decimal_places=2, source='total_amount.amount')
...
class AdminOrderDetailSerializer(serializers.ModelSerializer):
    ...
    tax_amount = serializers.DecimalField(max_digits=10, decimal_places=2, source='tax_amount.amount')
    shipping_cost = serializers.DecimalField(max_digits=10, decimal_places=2, source='shipping_cost.amount')
    discount_amount = serializers.DecimalField(max_digits=10, decimal_places=2, source='discount_amount.amount')
    total_amount = serializers.DecimalField(max_digits=10, decimal_places=2, source='total_amount.amount')
    amount_paid = serializers.DecimalField(max_digits=10, decimal_places=2, source='amount_paid.amount')
    amount_refunded = serializers.DecimalField(max_digits=10, decimal_places=2, source='amount_refunded.amount')
    currency = serializers.CharField(source='total_amount.currency.code', read_only=True)
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    payment_status_display = serializers.CharField(source='get_payment_status_display', read_only=True)
    item_count = serializers.IntegerField(source='total_item_quantity', read_only=True)
...
class OrderRefundSerializer(serializers.Serializer):
    ...
    def validate_amount(self, value):
        ...
        if value and value > max_refundable:
            raise serializers.ValidationError(
                f"Refund amount cannot exceed {max_refundable}."
            )
...
class OrderNoteCreateSerializer(serializers.Serializer):
    ...
    notify_customer = serializers.BooleanField(
        default=False,
        help_text=_("Whether to send email notification to customer")
    )
...
class OrderFilterSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=[('all', 'All')] + Order.STATUS_CHOICES,
        required=False,
        default='all'
    )
    ...
    date_from = serializers.DateField(
        required=False,
        help_text=_('Filter orders created on or after this date (YYYY-MM-DD)')
    )
    date_to = serializers.DateField(
        required=False,
        help_text=_('Filter orders created on or before this date (YYYY-MM-DD)')
    )
