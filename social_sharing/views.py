...
from .models import SocialShare, ShareCount
...
def social_sharing_dashboard(request):
    ...
    # Shares today
    shares_today = SocialShare.objects.filter(
        shared_at__gte=today_start
    ).count()
    ...
    # Top platform
    top_platform_data = ShareCount.objects.values('platform').annotate(
    ...
    top_product = None
    if product_ct:
        top_product_data = ShareCount.objects.filter(
        ...
        if top_product_data:
            ...
            try:
                ...
                top_product = {
                    ...
                    'thumbnail': product.images.first().thumbnail_small if product.images.exists() else None
                }
    ...
    top_sharer_data = SocialShare.objects.filter(
        user__isnull=False,
        shared_at__gte=start_date
    ).values('user', 'user__email', 'user__first_name', 'user__last_name').annotate(
        share_count=Count('id')
    ).order_by('-share_count').first()

    if top_sharer_data:
        full_name = f"{top_sharer_data['user__first_name']} {top_sharer_data['user__last_name']}".strip()
        most_active_sharer = {
    ...
    daily_shares = SocialShare.objects.filter(
        shared_at__gte=start_date, shared_at__lte=end_date
    ).annotate(
        date=TruncDate('shared_at')
    ...
    for platform_code, platform_name in platforms:
        count = ShareCount.objects.filter(
    ...
    top_sharers = []
    top_sharer_list = SocialShare.objects.filter(
        user__isnull=False,
        shared_at__gte=start_date
    ).values('user', 'user__email', 'user__first_name', 'user__last_name').annotate(
        share_count=Count('id')
    ...
    top_platforms = []
    platform_counts = ShareCount.objects.values('platform').annotate(
        total_count=Sum('count')
    ).order_by('-total_count')[:5]

    for platform_data in platform_counts:
        platform_name = dict(SocialShare.PLATFORM_CHOICES).get(platform_data['platform'], platform_data['platform'])
    ...
    recent_shares = SocialShare.objects.select_related(
        'user',
        'content_type'
    ).order_by('-shared_at')[:10]

    # Top 5 most shared content
    top_shared_content = ShareCount.objects.select_related(
        'content_type'
    ).order_by('-count')[:5]
...
def filter_shares(request):
    ...
    # Build query
    shares = SocialShare.objects.select_related(
        'user',
        'content_type'
    ).order_by('-shared_at')
    ...
    html = render_to_string('admin/social_sharing/partials/share_cards.html', {
        'shares': shares,
    })
...
def filter_sharecounts(request):
    ...
    # Build query
    sharecounts = ShareCount.objects.select_related(
        'content_type'
    )
    ...
    # Render results as HTML
    html = render_to_string('admin/social_sharing/partials/sharecount_cards.html', {
        'sharecounts': sharecounts,
    })

    return JsonResponse({
        'html': html,
        'count': sharecounts.count()
    })
