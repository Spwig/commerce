...
class SocialShareAdmin(admin.ModelAdmin):
    ...
    def user_info(self, obj):
        ...
        return format_html(
            '<span class="social-admin-anonymous">{}</span>',
            _('Anonymous')
        )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        shares = SocialShare.objects.select_related(
            'user',
            'content_type'
        ).order_by('-shared_at')[:50]  # Limit initial display to 50 most recent
...
class ShareCountAdmin(admin.ModelAdmin):
    ...
    def count_display(self, obj):
        ...
        css = 'social-admin-count-high' if obj.count > 1000 else (
              'social-admin-count-medium' if obj.count > 100 else 'social-admin-count-normal')
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        sharecounts = ShareCount.objects.select_related(
            'content_type'
        ).order_by('-count', '-last_updated')[:50]  # Top 50 by count
