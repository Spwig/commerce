...
(function () {
  ...
  function initSocialDashboard() {
    ...
    const trendEl = document.getElementById('shares-trend-data');
    const distEl = document.getElementById('platform-dist-data');
    const i18nEl = document.getElementById('social-dashboard-i18n');

    const sharesTrendData = trendEl ? JSON.parse(trendEl.textContent) : [];
    const platformDistributionData = distEl ? JSON.parse(distEl.textContent) : [];
          ...
          datasets: [
            {
              ...
              data: sharesTrendData.map(function (d) {
                return d.count;
              }),
              borderColor: '#667eea',
              ...
              callbacks: {
                label: function (context) {
                  ...
                  return (
                    n +
                    ' ' +
                    (n !== 1 ? i18n.shares_plural || 'shares' : i18n.share_singular || 'share')
                  );
                },
              },
            },
    ...
    if (platformDistCtx && platformDistributionData && platformDistributionData.length > 0) {
      new Chart(platformDistCtx, {
        type: 'doughnut',
        data: {
          labels: platformDistributionData.map(function (d) {
            return d.name;
          }),
          datasets: [
            {
              data: platformDistributionData.map(function (d) {
                return d.count;
              }),
              backgroundColor: platformDistributionData.map(function (d) {
                return d.color;
              }),
