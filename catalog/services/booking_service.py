...
class BookingAvailabilityService:
    ...
    def get_available_dates(
        ...
        for day_num in range(1, days_in_month + 1):
            ...
            # Skip past dates
            if datetime.combine(check_date, time(23, 59), tzinfo=UTC) < now:
                results.append(
                    {
    ...
    def check_availability(
        ...
        if config.booking_type == "accommodation":
            ...
            if persons:
                for _ptype, count in persons.items():
    ...
    def calculate_booking_price(
        ...
        if persons:
            person_types = {pt.name: pt for pt in product.booking_person_types.all()}
    ...
    def calculate_accommodation_price(
        ...
        # Load person types
        person_type_objs = {pt.name: pt for pt in product.booking_person_types.all()}
        ...
        if resource_id:
            cost_rules = [
                r for r in cost_rules if r.resource_id is None or r.resource_id == resource_id
            ]
        else:
        ...
        for day_offset in range(num_nights):
            ...
            for rule in nightly_rules:
                ...
                if (
                    rule.lead_time_min_days is not None
                    and days_until_checkin < rule.lead_time_min_days
                ):
                    continue
                if (
                    rule.lead_time_max_days is not None
                    and days_until_checkin > rule.lead_time_max_days
                ):
                    continue
            ...
            if rule.length_of_stay_min and num_nights >= rule.length_of_stay_min:
                if rule.length_of_stay_discount_percent:
                    discount_amount = subtotal * (
                        rule.length_of_stay_discount_percent / Decimal("100")
                    )
                    los_discount = {
    ...
    def create_slot_reservation(
        ...
        persons: dict | None = None,
        channel: str = "web",
    ) -> tuple[bool, str, BookingSlotReservation | None]:
        """
        ...
        resource = None
        if resource_id:
            resource = BookingResource.objects.filter(pk=resource_id, product=product).first()
    ...
    def confirm_booking(
        ...
        if raw_breakdown:
            ...
            price_breakdown = json.loads(json.dumps(raw_breakdown, default=str))
    ...
    def cancel_booking(
        ...
        BookingLifecycleService.add_note(
            booking,
            f"Booking cancelled: {reason}" if reason else "Booking cancelled",
    ...
    def join_waitlist(
        ...
        desired_date: date,
        customer_name: str = "",
        desired_time_start: time | None = None,
        desired_time_end: time | None = None,
        desired_persons: dict | None = None,
        customer=None,
    ) -> tuple[bool, str, BookingWaitlist | None]:
    ...
    def _within_advance_window(config: BookingConfig, booking_date: date) -> bool:
        ...
        if datetime.combine(booking_date, time(23, 59), tzinfo=UTC) < earliest:
            return False
        ...
        return not datetime.combine(booking_date, time(0, 0), tzinfo=UTC) > latest
    ...
    def _rule_matches_date(rule: BookingAvailabilityRule, booking_date: date) -> bool:
        ...
        if rule.scope == "all_dates":
            ...
            if rule.start_date and booking_date < rule.start_date:
                return False
            return not (rule.end_date and booking_date > rule.end_date)
...
class BookingLifecycleService:
    ...
    def cancel_booking(
        ...
        cls.add_note(
            booking,
            f"Booking cancelled by {initiated_by}" + (f": {reason}" if reason else ""),
            author=author,
            note_type="status_change",
        )
    ...
    def send_booking_email(
        ...
        try:
            ...
            from email_system.utils.language import (
                _get_site_default_language,
                get_order_email_language,
            ...
            if booking.order_id:
                ...
                booking.customer_id and hasattr(booking.customer, "user") and booking.customer.user
            ):
    ...
    def send_admin_email(
        ...
        try:
            ...
            if not admin_email:
                logger.info(
                    f"No admin email configured, skipping {template_type} for booking #{booking.pk}"
                )
                return
