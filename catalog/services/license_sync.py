...
class LicenseProviderService:
    ...
    def create_license(self, license_key, product, order):
        ...
        if not self.adapter:
            ...
            sync.error_message = f"No adapter available for provider type: {self.provider.provider_type}"
            sync.save()
            return sync
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Successfully synced license {license_key.key} to {self.provider.name}")
            else:
                sync.sync_status = 'failed'
    ...
    def sync_activation(self, activation):
        ...
        try:
            ...
            if success:
                logger.info(f"Synced activation for {activation.license_key.key} to {self.provider.name}")
                return True
            else:
    ...
    def sync_deactivation(self, activation):
        ...
        try:
            ...
            if success:
                logger.info(f"Synced deactivation for {activation.license_key.key} to {self.provider.name}")
                return True
            else:
        ...
        for sync in failed_syncs:
            try:
                ...
                if self.adapter:
                    ...
                    product = license_key.digital_asset.product if license_key.digital_asset else None
