...
class LicenseKeyGenerator:
    ...
    def generate(self, template, context: Dict) -> str:
        ...
        if len(key) < template.min_length:
            raise ValidationError(
                f"Generated key is too short: {len(key)} < {template.min_length}"
            )
        if len(key) > template.max_length:
            raise ValidationError(
                f"Generated key is too long: {len(key)} > {template.max_length}"
            )
    ...
    def validate(self, key: str, template) -> bool:
        ...
        if '{CHECKSUM:' in template.pattern:
            if not self._verify_checksum(key, template):
                return False
    ...
    def _verify_checksum(self, key: str, template) -> bool:
        ...
        regex_pattern = re.sub(
            r'\\{CHECKSUM:(\d+)\\}', checksum_replacer, regex_pattern
        )
