...
def variation_builder(request, product_id):
    ...
    assignments = (
        product_obj.attribute_assignments.select_related("attribute")
...
def generate_variations(request, product_id):
    ...
    try:
        ...
        for attr_id, value_ids in selected_attributes.items():
            attr_values_map[attr_id] = list(
                AttributeValue.objects.filter(
                    id__in=value_ids, attribute_id=attr_id
        ...
        with transaction.atomic():
            ...
            # Success message
            if created_count > 0:
                messages.success(request, _(f"Successfully created {created_count} variant(s)."))
            if skipped_count > 0:
                messages.info(request, _(f"Skipped {skipped_count} variant(s) that already exist."))
...
def _find_existing_variant(product, combination):
    ...
    for variant in product.variants.all():
        ...
        combination_ids = {av.id for av in combination}
...
def digital_products_analytics_dashboard(request):
    ...
    recent_downloads = DigitalDownload.objects.filter(downloaded_at__gte=thirty_days_ago).count()
    recent_license_keys = LicenseKey.objects.filter(issued_at__gte=thirty_days_ago).count()
    ...
    for i in range(30, -1, -1):
        ...
        downloads_count = DigitalDownload.objects.filter(
            downloaded_at__gte=date_start, downloaded_at__lt=date_end
        ).count()

        download_trends.append({"date": date_start.strftime("%Y-%m-%d"), "count": downloads_count})
    ...
    top_products = (
        DigitalAsset.objects.annotate(download_count=Count("downloads"))
    ...
    failed_downloads_by_asset = (
        DigitalDownload.objects.filter(status="failed")
        .values("digital_asset__filename", "digital_asset__product__name", "digital_asset_id")
    ...
    total_activations = sum(lic.current_activations for lic in LicenseKey.objects.all())

    # Average activations per license
    avg_activations = LicenseKey.objects.aggregate(avg=Avg("current_activations"))["avg"] or 0

    # Licenses at capacity (100% activated)
    licenses_at_capacity = LicenseKey.objects.filter(
        max_activations__isnull=False, current_activations__gte=F("max_activations")
    ).count()
    ...
    this_week_downloads = DigitalDownload.objects.filter(downloaded_at__gte=seven_days_ago).count()

    last_week_start = seven_days_ago - timedelta(days=7)
    last_week_downloads = DigitalDownload.objects.filter(
        downloaded_at__gte=last_week_start, downloaded_at__lt=seven_days_ago
    ).count()
    ...
    try:
        ...
        with transaction.atomic():
            ...
            attribute, attribute_created = ProductAttribute.objects.get_or_create(
                slug=attribute_slug, defaults={"name": attribute_name, "type": attribute_type}
            )

            # Get the highest existing sort_order for this attribute's values
            existing_max_order = (
                AttributeValue.objects.filter(attribute=attribute).aggregate(
            ...
            for idx, value_name in enumerate(values_list):
                ...
                existing_value = AttributeValue.objects.filter(
                    attribute=attribute, slug=value_slug
                ).first()
        ...
        all_attribute_values = AttributeValue.objects.filter(attribute=attribute).order_by(
            "sort_order", "value"
        )
        ...
        return JsonResponse(
            {
                ...
                "created_values": [
                    {"id": v.id, "value": v.value, "slug": v.slug} for v in created_values
                ],
                "all_values": [
                    {"id": v.id, "value": v.value, "slug": v.slug} for v in all_attribute_values
                ],
        ...
        return JsonResponse(
            {"success": False, "errors": [gettext("Invalid JSON data")]}, status=400
        )
...
def save_product_mapping(request, provider_id):
    ...
    try:
        ...
        if not product_id:
            return JsonResponse(
                {"success": False, "error": gettext("Product ID is required")}, status=400
            )
        ...
        return JsonResponse({"success": False, "error": gettext("Invalid JSON data")}, status=400)
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
        ...
        if provider_id:
            ...
            try:
                ...
                if provider.product_mapping:
                    mapped_ids = [int(pid) for pid in provider.product_mapping]
...
def autocomplete_component_products(request):
    ...
    if term:
        queryset = queryset.filter(models.Q(name__icontains=term) | models.Q(sku__icontains=term))
...
def delete_product_variant(request, variant_id):
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": gettext("Variant not found.")}, status=404)
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
...
def variant_form_context(request, product_id):
    ...
    assignments = (
        product.attribute_assignments.select_related("attribute")
        .prefetch_related("allowed_values")
        .order_by("sort_order")
    )
...
def variant_detail(request, variant_id):
    ...
    variant = get_object_or_404(
        ProductVariant.objects.select_related(
            "product", "preferred_shipping_package"
        ).prefetch_related(
            "selected_attributes__attribute",
            "images__media_asset",
            "stock_items__warehouse",
    ...
    # Selected attributes
    selected_attrs = []
    for val in variant.selected_attributes.select_related("attribute").order_by(
        "attribute__sort_order"
    ):
...
def create_variant(request, product_id):
    ...
    if not name:
        return JsonResponse(
            {"success": False, "error": gettext("Variant name is required.")}, status=400
        )
    if not sku:
    ...
    if ProductVariant.objects.filter(sku=sku).exists():
        return JsonResponse(
            {"success": False, "error": gettext('SKU "%(sku)s" already exists.') % {"sku": sku}},
            status=400,
        )
    ...
    try:
        with transaction.atomic():
            ...
            if "sku" in data:
                ...
                if ProductVariant.objects.filter(sku=new_sku).exclude(pk=variant.pk).exists():
                    return JsonResponse(
                        {
                            "success": False,
                            "error": gettext('SKU "%(sku)s" already exists.') % {"sku": new_sku},
                        },
                        status=400,
                    )
                variant.sku = new_sku
            ...
            if "preferred_shipping_package_id" in data:
                variant.preferred_shipping_package_id = (
                    data["preferred_shipping_package_id"] or None
                )
        ...
        return JsonResponse(
            {
                "success": True,
                "variant": card_data,
                "message": gettext('Variant "%(name)s" updated successfully.')
                % {"name": variant.name},
            }
        )
...
def update_attribute_color(request, value_id):
    ...
    if color_hex and not (color_hex.startswith("#") and len(color_hex) == 7):
        return JsonResponse(
            {"success": False, "error": gettext("Invalid color hex format. Use #RRGGBB.")},
            status=400,
        )
...
def filter_gift_card_transactions(request):
    ...
    queryset = GiftCardTransaction.objects.select_related("gift_card", "order", "created_by").all()

    # Search filter
    if search:
        queryset = queryset.filter(
            Q(gift_card__code__icontains=search) | Q(notes__icontains=search)
        )
...
def slot_options_manager(request, slot_id):
    ...
    slot = get_object_or_404(ConfigurationSlot.objects.select_related("product"), pk=slot_id)
    ...
    if request.method == "POST":
        ...
        if action == "add_option":
            ...
            if product_id:
                ...
                option_variant = None
                if variant_id:
                    option_variant = get_object_or_404(
                        ProductVariant, pk=variant_id, product=option_product
                    )
            ...
            if option_id:
                try:
                    ...
                    try:
                        from decimal import Decimal as D  # noqa: N817 — short local alias
    ...
    options = slot.options.select_related("option_product", "option_variant").order_by(
        "sort_order", "pk"
    )
    ...
    if request.method == "POST":
        ...
        if action == "save_rule":
            ...
            if created_count > 0:
                messages.success(
                    request,
                    _("Generated {} compatibility rule(s) from tags.").format(created_count),
                )
            else:
                messages.info(
                    request,
                    _(
                        "No new rules generated. Ensure options have matching compatibility tags across different slots."
                    ),
                )
            return redirect("catalog_admin:compatibility_matrix", product_id=product_obj.pk)
...
def _generate_rules_from_tags(product):
    ...
    for _tag, options_list in tag_map.items():
        # Group by slot
        by_slot = {}
...
def list_slots(request, product_id):
    ...
    slots = (
        ConfigurationSlot.objects.filter(product=product_obj)
        .annotate(option_count=models.Count("options"))
...
def create_slot(request, product_id):
    ...
    max_order = (
        ConfigurationSlot.objects.filter(product=product_obj).aggregate(m=Max("sort_order"))["m"]
        or 0
    )
...
def update_slot(request, slot_id):
    ...
    if "slug" in data and data["slug"].strip():
        ...
        if (
            ConfigurationSlot.objects.filter(product=slot.product, slug=new_slug)
            .exclude(pk=slot.pk)
            .exists()
        ):
            return JsonResponse(
                {"success": False, "error": gettext("Slug already exists.")}, status=400
            )
        slot.slug = new_slug
...
def list_presets(request, product_id):
    ...
    product_obj = get_object_or_404(Product, pk=product_id, product_type="configurable")
    presets = ConfigurationPreset.objects.filter(product=product_obj).order_by("sort_order", "pk")

    presets_data = []
    for preset in presets:
        # Count selections
        selection_count = (
            sum(len(opts) for opts in preset.selections.values()) if preset.selections else 0
        )
...
def create_preset(request, product_id):
    ...
    max_order = (
        ConfigurationPreset.objects.filter(product=product_obj).aggregate(m=Max("sort_order"))["m"]
        or 0
    )
...
def preset_detail(request, preset_id):
    ...
    slots_with_options = []
    slots = (
        ConfigurationSlot.objects.filter(product=preset.product)
        .prefetch_related("options__option_product")
        .order_by("sort_order")
    )
...
def update_preset(request, preset_id):
    ...
    if "slug" in data and data["slug"].strip():
        new_slug = data["slug"].strip()
        if (
            ConfigurationPreset.objects.filter(product=preset.product, slug=new_slug)
...
def filter_warehouses(request):
    ...
    queryset = Warehouse.objects.select_related("region").annotate(stock_count=Count("stock_items"))
    ...
    if request.method == "POST":
        ...
        if action == "restore":
            ...
            if errors:
                messages.error(
                    request,
                    "Some products have orders and cannot be permanently deleted: "
                    + ", ".join(errors),
                )
            ...
            if count:
                messages.success(
                    request, f"Recycle bin emptied ({count} products permanently deleted)"
                )
            if errors:
                messages.warning(
                    request,
...
def filter_bookings(request):
    ...
    queryset = Booking.objects.select_related("product", "resource", "customer").all()
    ...
    html = render_to_string(
        "admin/catalog/booking/partials/booking_cards.html", {"bookings": bookings}, request=request
    )
...
def _serialize_availability_rule(rule):
    ...
    return {
        ...
        "length_of_stay_discount_percent": str(rule.length_of_stay_discount_percent)
        if rule.length_of_stay_discount_percent is not None
        else "",
...
def booking_config_detail(request, product_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "exists": True, "config": _serialize_booking_config(config)}
        )
...
def list_booking_availability_rules(request, product_id):
    ...
    if err:
        return err
    rules = (
        BookingAvailabilityRule.objects.filter(product=product_obj)
    ...
    resources = BookingResource.objects.filter(product=product_obj, is_active=True).order_by(
        "sort_order", "name"
    )
    return JsonResponse(
        {
...
def update_booking_availability_rule(request, rule_id):
    ...
    if "resource_id" in data:
        if data["resource_id"]:
            rule.resource = get_object_or_404(
                BookingResource, pk=data["resource_id"], product=rule.product
            )
        else:
            rule.resource = None
    ...
    if "specific_dates" in data:
        rule.specific_dates = (
            data["specific_dates"] if isinstance(data["specific_dates"], list) else []
        )
...
def list_booking_recurrence_rules(request, product_id):
    ...
    if err:
        return err
    rules = BookingRecurrenceRule.objects.filter(product=product_obj).order_by(
...
def create_booking_recurrence_rule(request, product_id):
    ...
    if not start_time or not end_time or not start_date:
        return JsonResponse(
            {"error": "start_time, end_time, and start_date are required"}, status=400
        )
    ...
    rule = BookingRecurrenceRule.objects.create(
        ...
        if data.get("day_of_month") not in (None, "")
        else None,
...
def update_booking_recurrence_rule(request, rule_id):
    ...
    if "day_of_week" in data:
        rule.day_of_week = (
            int(data["day_of_week"]) if data["day_of_week"] not in (None, "") else None
        )
    if "day_of_month" in data:
        rule.day_of_month = (
            int(data["day_of_month"]) if data["day_of_month"] not in (None, "") else None
        )
    if "start_time" in data:
...
def booking_check_reschedule(request, booking_id):
    ...
    try:
        new_start = dt.fromisoformat(f"{date_str}T{start_str}").replace(tzinfo=UTC)
        new_end = dt.fromisoformat(f"{date_str}T{end_str}").replace(tzinfo=UTC)
    except (ValueError, TypeError):
