...
class CustomizationOptionSerializer(serializers.ModelSerializer):
    ...
    price_amount = serializers.DecimalField(
        source='price_amount.amount',
        max_digits=10,
        decimal_places=2,
        read_only=True
    )
    price_currency = serializers.CharField(
        source='price_amount.currency.code',
        read_only=True
    )
...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    def get_image_url(self, obj) -> str | None:
        ...
        if first_img and first_img.media_asset and first_img.media_asset.original_file:
            return first_img.media_asset.get_thumbnail('medium') or first_img.media_asset.get_display_url()
    ...
    def get_component_variant(self, obj) -> dict | None:
        ...
        return {
            ...
            'attributes': AttributeValueSerializer(
                variant.selected_attributes.all(),
                many=True
            ).data
        }
    ...
    def get_images(self, obj) -> list:
        ...
        for img in images:
            if img.media_asset:
                result.append({
                    'id': img.id,
                    'thumbnail_url': img.media_asset.get_thumbnail('small') if hasattr(img.media_asset, 'get_thumbnail') else img.media_asset.get_display_url(),
                    'image_url': img.media_asset.get_display_url(),
                    'url': img.media_asset.original_file.url if img.media_asset.original_file else '',
...
class ProductDetailSerializer(TranslatedFieldsMixin, TranslationAwareSerializer, serializers.ModelSerializer):
    ...
    def get_related_products(self, obj) -> list:
        ...
        related = Product.objects.filter(
            ...
            status='published'
        ).exclude(id=obj.id).exclude(sales_channel='pos_only')[:6]
    ...
    def get_stock_availability(self, obj) -> list:
        ...
        stock_items = obj.stock_items.select_related('warehouse').filter(
        ...
        for stock_item in stock_items:
            availability.append({
                ...
                'low_stock_threshold': stock_item.low_stock_threshold or obj.low_stock_threshold,
            })
    ...
    def get_regional_stock(self, obj) -> dict | None:
        ...
        if not region:
            ...
            total_available = sum(
                item.available for item in obj.stock_items.filter(
                    warehouse__is_active=True
                )
            )
            return {
                ...
                'message': f'{total_available} available' if total_available > 0 else 'Out of Stock'
            }

        # Calculate regional stock
        regional_stock = obj.stock_items.filter(
        ...
        return {
            ...
            'available': total_available > 0,
            'quantity': total_available,
            'message': f'{total_available} available in {region.name}' if total_available > 0 else f'Currently unavailable in {region.name}'
        }
    ...
    def get_pickup_locations(self, obj) -> list:
        ...
        stock_items = obj.stock_items.select_related(
            'warehouse',
            'warehouse__shipping_location'
        ).filter(
            warehouse__is_active=True,
...
class GiftCardTransactionSerializer(serializers.ModelSerializer):
    ...
    @extend_schema_field({'type': 'object', 'properties': {'amount': {'type': 'string'}, 'currency': {'type': 'string'}}})
...
class GiftCardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            ...
            'created_at', 'issued_at', 'first_used_at', 'transactions'
        ]
...
class GiftCardBalanceCheckSerializer(serializers.Serializer):
    ...
    code = serializers.CharField(
        max_length=50,
        required=True,
        help_text=_("Gift card code to check")
    )
...
class GiftCardPurchaseSerializer(serializers.Serializer):
    ...
    denomination = serializers.DecimalField(
        ...
        decimal_places=2,
        required=False,
        help_text=_("Fixed denomination choice")
    )
    recipient_email = serializers.EmailField(required=True, help_text=_("Email to send gift card to"))
    ...
    message = serializers.CharField(required=False, allow_blank=True, help_text=_("Personal message"))
...
class ConfigurationSlotOptionSerializer(TranslationAwareSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'product_name', 'product_slug', 'product_sku', 'product_image',
            'variant_name', 'variant_id', 'allow_variant_selection',
            'effective_price', 'price_adjustment',
            'is_default', 'is_popular', 'sort_order',
            'compatibility_tags', 'quantity', 'in_stock',
        ]
...
class CompatibilityRuleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'rule_type', 'source_option', 'target_slot',
...
class ConfigurationPresetSerializer(TranslationAwareSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'selections', 'is_featured', 'sort_order', 'calculated_price',
        ]
        translated_fields = ['name', 'description']
    ...
    def get_calculated_price(self, obj):
        ...
        for slot_id_str, option_ids in (obj.selections or {}).items():
