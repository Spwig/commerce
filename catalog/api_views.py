...
class ProductAttributeViewSet(HeadlessAPIMixin, viewsets.ReadOnlyModelViewSet):
    ...
    queryset = (
        ProductAttribute.objects.all().prefetch_related("values").order_by("sort_order", "name")
    )
...
@extend_schema_view(
    ...
    get_variant_by_attributes=extend_schema(
        tags=["Catalog"], summary=_("Find variant by attribute selection")
    ),
    customization_options=extend_schema(
        tags=["Catalog"], summary=_("Get product customization options")
    ),
    validate_customizations=extend_schema(
        tags=["Catalog"], summary=_("Validate customization values")
    ),
...
class ProductViewSet(HeadlessAPIMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def search(self, request):
        ...
        if in_stock and in_stock.lower() == "true":
            products = products.filter(
                Q(track_inventory=False) | Q(track_inventory=True, quantity__gt=0)
            )
    ...
    def recently_viewed(self, request):
        ...
        ordered_products = [
            products_dict[pid] for pid in recently_viewed_ids if pid in products_dict
        ]
    ...
    def track_view(self, request, slug=None):
        ...
        return Response({"success": True, "views_count": product.views_count})
    ...
    def customization_options(self, request, slug=None):
        ...
        if not product.allow_customization:
            return Response({"allow_customization": False, "options": []})
        ...
        return Response({"allow_customization": True, "options": serializer.data})
    ...
    def validate_customizations(self, request, slug=None):
        ...
        if not product.allow_customization:
            return Response(
                {"valid": False, "errors": ["This product does not support customization"]},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not is_valid:
            return Response(
                {"valid": False, "errors": [error_msg]}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for _option_id, customization_data in validated_customizations.items():
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "limit", int, description=_("Number of products (default: 12, max: 50)")
            ),
        ],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                "period", str, description=_("Time period: all, month, week (default: all)")
            ),
        ],
    )
    @action(detail=False, methods=["get"])
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                "days", int, description=_("Products added in last N days (default: 30)")
            ),
        ],
    )
    ...
    def new_arrivals(self, request):
        ...
        products = (
            self.get_queryset().filter(created_at__gte=cutoff_date).order_by("-created_at")[:limit]
        )
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "limit", int, description=_("Number of products (default: 24, max: 100)")
            ),
        ],
    )
    ...
    @extend_schema(
        ...
        description=_(
            "Get products related to a specific product (same category, brand, or attributes)."
        ),
        parameters=[
            OpenApiParameter(
                "limit", int, description=_("Number of products (default: 8, max: 20)")
            ),
        ],
    )
    @action(detail=True, methods=["get"])
    ...
    def similar(self, request, slug=None):
        ...
        similar = (
            self.get_queryset()
            .filter(category=product.category, price__gte=price_min, price__lte=price_max)
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "limit", int, description=_("Number of products (default: 4, max: 10)")
            ),
        ],
    )
    ...
    def frequently_bought_together(self, request, slug=None):
        ...
        co_purchased = (
            ...
            .values("product")
            .annotate(frequency=Count("product"))
        ...
        if product_ids:
            ...
            products = (
                self.get_queryset().filter(category=product.category).exclude(id=product.id)[:limit]
            )
    ...
    def upsells(self, request, slug=None):
        ...
        upsells = (
            self.get_queryset()
            .filter(category=product.category, price__gt=product.price)
            .exclude(id=product.id)
    ...
    def configurator(self, request, slug=None):
        ...
        if product.product_type != "configurable":
            return Response(
                {"error": "This product is not configurable."}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        currency = (
            base_price.currency if hasattr(base_price, "currency") else get_default_currency()
        )
        ...
        data = {
            ...
            "slots": ConfigurationSlotSerializer(
    ...
    def configurator_validate(self, request, slug=None):
        ...
        for option, variant in resolved_options:
            ...
            if product.configurator_pricing_strategy == "base_plus_adjustments":
                ...
                p = variant.get_price() if variant else option.option_product.price
                amount = (
                    p.amount if p and hasattr(p, "amount") else Decimal("0.00")
                ) * option.quantity

            slot_subtotals[slot_id] = str(Decimal(slot_subtotals.get(slot_id, "0.00")) + amount)
...
@extend_schema(
    ...
    description=_(
        "Retrieve catalog-wide statistics including total products, featured products, products in stock, category count, brand count, and collection count. Useful for dashboard displays and analytics."
    ),
    responses={
        200: OpenApiResponse(
            description=_("Catalog statistics"),
            response={
                "products": {"total": 150, "featured": 20, "in_stock": 130},
                "categories": 15,
                "brands": 25,
                "collections": 8,
            },
                    ...
                    "products": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "url": {
                                    "type": "string",
                                    "example": "/en/product/premium-diffuser/",
                                },
                                "image_url": {
                                    "type": "string",
                                    "nullable": True,
                                    "example": "/media/products/diffuser.jpg",
                                },
        ...
        result.append(
            {
                ...
                "name": product.name,
                "url": reverse(
                    "page_builder:product_detail", kwargs={"product_slug": product.slug}
                ),
                "image_url": image_url,
                "price": str(price),
...
def catalog_stats(request):
    ...
    stats = {
        "products": {
            ...
            "in_stock": Product.objects.filter(status="published")
            .filter(Q(track_inventory=False) | Q(track_inventory=True, quantity__gt=0))
            .count(),
        },
...
@extend_schema(
    ...
    description=_(
        "Retrieve all available filter options for product filtering including categories, brands, price ranges, and product types. Used to populate filter UI components."
    ),
    responses={
        200: OpenApiResponse(
...
def available_filters(request):
    ...
    product_types = (
        Product.objects.filter(status="published").values_list("product_type", flat=True).distinct()
    )
...
@extend_schema(
    ...
    description=_(
        "Check real-time stock availability for a product at specific warehouse locations. Optionally calculate distance from customer location. Returns on-hand, allocated, and available quantities per warehouse."
    ),
    parameters=[
        OpenApiParameter(
            ...
            location=OpenApiParameter.QUERY,
            description=_(
                "Comma-separated list of warehouse codes to check (e.g., 'MAIN-WH,WC-WH')"
            ),
            required=False,
        ),
        OpenApiParameter(
...
def check_warehouse_stock(request, product_slug):
    ...
    stock_items = StockItem.objects.filter(
        product=product, warehouse__in=warehouses_query
        ...
        availability.append(
            {
                ...
                or product.low_stock_threshold,
                "is_in_stock": stock_item.available > 0,
                "is_low_stock": stock_item.available
                <= (stock_item.low_stock_threshold or product.low_stock_threshold)
                if stock_item.low_stock_threshold or product.low_stock_threshold
                else False,
            }
        )
...
@extend_schema(
    ...
    description=_(
        "Get all warehouse locations that support customer pickup (buy online, pick up in store). Optionally filter by product availability and calculate distances from customer location."
    ),
    parameters=[
        OpenApiParameter(
            name="product_slug",
...
def pickup_locations(request):
    ...
    if product_slug:
        ...
        stock_items = StockItem.objects.filter(
            product=product, warehouse__in=warehouses, on_hand__gt=0
        ).select_related("warehouse")
        ...
        return Response(
            {"product": {"name": product.name, "slug": product.slug}, "locations": locations}
        )

    # No product filter - return all pickup locations
    locations = [{"warehouse": WarehouseSerializer(wh).data} for wh in warehouses]
...
@extend_schema(
    ...
    description=_(
        "Check the balance of a gift card without requiring authentication. Public endpoint."
    ),
    request={
        "application/json": {
            "type": "object",
...
def product_availability(request, product_slug):
    ...
    if settings.show_ships_from and warehouse:
        ...
        if warehouse_display_name:
            ships_from = {"display_name": warehouse_display_name, "warehouse_code": warehouse.code}
    ...
    backorder_available = (
        out_of_stock_action == "allow_backorder" and stock_status == "out_of_stock"
    )

    # Pre-order info
    preorder_info = {
        "available": product.is_preorder,
        "release_date": product.preorder_release_date.isoformat()
    ...
    return Response(
        {
            ...
            "show_exact_quantity": settings.show_exact_quantity
            ...
            "show_estimated_delivery": bool(settings.show_estimated_delivery)
            if product.track_inventory
            else False,
            ...
            "preorder_release_date": product.preorder_release_date.isoformat()
            if product.preorder_release_date
            else None,
...
def stock_notification_subscribe(request, product_slug):
    ...
    StockNotification.objects.create(email=email, product=product, variant=variant)
...
@extend_schema(
    ...
    description=_(
        "Returns all attributes assigned to a product with their allowed values and full value sets. **Admin only.**"
    ),
...
@extend_schema(
    ...
    description=_(
        "Returns attributes not yet assigned to the product, filtered by optional search query. **Admin only.**"
    ),
    parameters=[
...
@extend_schema(
    ...
    description=_(
        "Creates a new attribute assignment for the product. All attribute values are enabled by default. **Admin only.**"
    ),
    request={
        "application/json": {
            "type": "object",
            "properties": {"attribute_id": {"type": "integer"}},
...
def add_product_attribute(request, product_id):
    ...
    if not attribute_id:
        return Response(
            {"success": False, "error": "attribute_id is required"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if ProductAttributeAssignment.objects.filter(product=product, attribute=attribute).exists():
        return Response(
            {"success": False, "error": "Attribute already assigned to this product"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
@extend_schema(
    ...
    description=_(
        "Deletes an attribute assignment. Returns count of variants that used this attribute. **Admin only.**"
    ),
    responses={
...
@extend_schema(
    ...
    description=_(
        "Replaces the set of allowed values for an attribute assignment. **Admin only.**"
    ),
    request={
        "application/json": {
            "type": "object",
            "properties": {"allowed_value_ids": {"type": "array", "items": {"type": "integer"}}},
            "required": ["allowed_value_ids"],
        }
    },
    responses={
...
def update_attribute_values(request, product_id, assignment_id):
    ...
    if invalid:
        return Response(
            {"success": False, "error": f"Invalid value IDs for this attribute: {list(invalid)}"},
            status=status.HTTP_400_BAD_REQUEST,
        )
