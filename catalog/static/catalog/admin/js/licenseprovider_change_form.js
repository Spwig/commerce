...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const configEl = document.getElementById('licenseprovider-config');
    if (!configEl) return;
    ...
    // URLs
    const getMappingsUrl =
      '/' + languageCode + '/admin/catalog/license-provider/' + providerId + '/mappings/';
    const saveMappingUrl =
      '/' + languageCode + '/admin/catalog/license-provider/' + providerId + '/mappings/save/';
    const deleteMappingUrl =
      '/' + languageCode + '/admin/catalog/license-provider/' + providerId + '/mappings/delete/';
    const searchProductsUrl =
      '/' + languageCode + '/admin/catalog/license-provider/search-products/';
    ...
    document.addEventListener('click', function (e) {
      const btn = e.target.closest('[data-action="delete-mapping"]');
      if (btn) {
        ...
        .then(function (data) {
          if (data.success) {
            ...
            mappingsContainer.innerHTML =
              '<div class="mappings-empty"><p>' + data.error + '</p></div>';
          }
        })
        .catch(function (error) {
          console.error('Error loading mappings:', error);
          mappingsContainer.innerHTML =
            '<div class="mappings-empty"><p>' + config.strings.errorLoadingMappings + '</p></div>';
        });
    }
      ...
      fetch(searchProductsUrl + '?q=' + encodeURIComponent(query) + '&provider_id=' + providerId, {
        ...
        .then(function (data) {
          if (data.success && data.products.length > 0) {
            productSearchResults.innerHTML = data.products
              .map(function (product) {
                return (
                  '<div class="product-search-item" data-id="' +
                  product.id +
                  '" data-name="' +
                  escapeHtml(product.name) +
                  '">' +
                  (product.thumbnail
                    ? '<img src="' + product.thumbnail + '" class="product-thumb" alt="">'
            ...
            productSearchResults.innerHTML =
              '<div class="product-search-item no-results">' +
              config.strings.noProductsFound +
              '</div>';
            productSearchResults.classList.add('active');
          }
        })
    ...
    async function deleteMapping(productId) {
      if (
        !(await AdminModal.confirm({
          message: config.strings.confirmDelete,
          danger: true,
          confirmText: 'Delete',
        }))
      )
    ...
    function renderMappings(mappings) {
      ...
      const rows = mappings
        .map(function (mapping) {
          return (
            ...
            '</div>' +
            (mapping.product_sku
              ? '<div class="product-sku">SKU: ' + escapeHtml(mapping.product_sku) + '</div>'
              : '') +
            '</div></div></td>' +
            '<td><span class="external-id">' +
            escapeHtml(mapping.external_id) +
            '</span></td>' +
            '<td class="actions-cell">' +
            '<button type="button" class="delete-btn" data-action="delete-mapping" data-product-id="' +
            mapping.product_id +
            '" title="' +
            config.strings.removeMapping +
            '">' +
      ...
      mappingsContainer.innerHTML =
        ...
        '<th>' +
        config.strings.product +
        '</th>' +
        '<th>' +
        config.strings.externalId +
        '</th>' +
        '<th></th></tr></thead>' +
        '<tbody>' +
        rows +
