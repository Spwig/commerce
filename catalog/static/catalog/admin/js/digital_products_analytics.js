...
(function () {
  ...
  const configEl = document.getElementById('digital-analytics-config');
  if (!configEl) return;
  const config = JSON.parse(configEl.textContent);

  document.addEventListener('DOMContentLoaded', function () {
    // Download Trends Chart
    const downloadTrendsEl = document.getElementById('downloadTrendsChart');
    if (downloadTrendsEl) {
    ...
    const licenseStatsEl = document.getElementById('licenseStatsChart');
    if (licenseStatsEl) {
      new Chart(licenseStatsEl.getContext('2d'), {
        ...
        data: {
          labels: [
            ...
            config.strings.expired,
            config.strings.revoked,
            config.strings.suspended,
          ],
          datasets: [
            {
              data: [
    ...
    const downloadStatsEl = document.getElementById('downloadStatsChart');
    if (downloadStatsEl) {
      new Chart(downloadStatsEl.getContext('2d'), {
        type: 'doughnut',
        data: {
          labels: [
            config.strings.completed,
            config.strings.failed,
            config.strings.inProgress,
