...
(function () {
  ...
  function initInlineForm(inlineForm) {
    ...
    const requiresLicenseCheckbox = inlineForm.querySelector(
      '.field-requires_license input[type="checkbox"]'
    );
  ...
  function handleFileSelection(event, fileInput, filenameInput, inlineForm) {
    ...
    if (file.size > maxSize) {
      AdminModal.alert({
        message: `File size (${formatFileSize(file.size)}) exceeds maximum allowed size (2GB). Please choose a smaller file.`,
        type: 'warning',
      });
      fileInput.value = '';
      return;
    ...
    [downloadLimitInput, expirationDaysInput].forEach(input => {
      ...
      input.addEventListener('blur', function () {
        ...
        if (this.value !== '' && (isNaN(value) || value < 0)) {
          AdminModal.alert({
            message: 'Please enter a positive number or leave blank for unlimited.',
            type: 'warning',
          });
          this.value = '';
          this.focus();
    ...
    form.addEventListener('submit', function (e) {
      ...
      inlineForms.forEach(function (inlineForm) {
        ...
        if (fileInput && fileInput.files.length > 0 && filenameInput && !filenameInput.value) {
          AdminModal.alert({
            message: 'Please provide a filename for the uploaded file.',
            type: 'warning',
          });
          filenameInput.focus();
          hasErrors = true;
