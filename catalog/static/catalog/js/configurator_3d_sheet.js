...
(function () {
  ...
  // DOM references
  let sheet, handle, content, sheetFooter;
  let mobileViewer, desktopLayout, desktopRight, desktopViewerContainer;
  let viewer, loading;
  ...
  function handleViewportChange(mql) {
    const matches = typeof mql.matches !== 'undefined' ? mql.matches : mql;
    if (matches) {
  ...
  function observeHeaderChanges() {
    ...
    const announceClose = document.querySelector(
      '.announcement-close, .announcement-bar__close, .announcement__close, [data-announcement-close]'
    );
    if (announceClose) {
      announceClose.addEventListener(
        'click',
        function () {
          setTimeout(function () {
            measureHeaderHeight();
  ...
  function updateViewerPosition() {
    ...
    const sheetVh = SNAP[currentSnap];
  ...
  function findNearestSnap(currentVh) {
    ...
    let minDist = Math.abs(currentVh - nearest.vh);
    for (let i = 1; i < points.length; i++) {
      const dist = Math.abs(currentVh - points[i].vh);
      if (dist < minDist) {
        minDist = dist;
        nearest = points[i];
      }
  ...
  function onDragMove(e) {
    ...
    const minH = viewportHeight * 0.2;
    const maxH = viewportHeight * 0.92;
  ...
  function onDragEnd(e) {
    ...
    if (lastTime) {
      ...
      if (timeDelta > 0 && timeDelta < 300) {
        const pixelDelta = startY - touch.clientY;
  ...
  function onContentTouchMove(e) {
    ...
    if (contentDragging) {
      e.preventDefault();
      // Move the sheet
      const sheetDelta = startY - currentY;
      let newHeight = startHeight + sheetDelta;
      const viewportHeight = window.innerHeight;
      const minH = viewportHeight * 0.2;
      const maxH = viewportHeight * 0.92;
      newHeight = Math.max(minH, Math.min(maxH, newHeight));
