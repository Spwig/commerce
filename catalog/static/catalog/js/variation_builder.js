...
(function () {
  ...
  function handleAttributeChange(e) {
    ...
    if (checkbox.checked) {
      ...
      state.selectedAttributes[attrId] = state.selectedAttributes[attrId].filter(
        id => id !== valueId
      );
  ...
  async function handleFormSubmit(e) {
    ...
    if (combinations.length === 0) {
      AdminModal.alert({
        message: 'Please select at least one value for each attribute to generate variations.',
        type: 'warning',
      });
      return false;
    }
    ...
    if (['fixed_add', 'fixed_subtract', 'percentage_add'].includes(state.priceStrategy)) {
      if (!state.priceAmount || state.priceAmount <= 0) {
        AdminModal.alert({
          message: 'Please enter a valid price amount for the selected pricing strategy.',
          type: 'warning',
        });
        return false;
      }
