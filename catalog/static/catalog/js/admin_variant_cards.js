...
(function() {
    ...
    let listEl, loadingEl, countBadge, addBtn, overlay, modal, modalTitle, modalBody, saveBtn, cancelBtn, closeBtn;
    ...
    async function loadVariants() {
        try {
            ...
            if (loadingEl) loadingEl.innerHTML = `<span class="text-error"><i class="fas fa-exclamation-triangle"></i> Failed to load variants</span>`;
        }
    }
    ...
    function renderRow(v) {
        ...
        const stockClass = v.stock_status === 'in_stock' ? 'stock-in' : v.stock_status === 'low_stock' ? 'stock-low' : 'stock-out';
        const stockLabel = v.stock_status === 'in_stock' ? 'In Stock' : v.stock_status === 'low_stock' ? 'Low Stock' : 'Out of Stock';

        // Attribute pills
        let pillsHtml = '';
        if (v.attribute_pills && v.attribute_pills.length) {
            pillsHtml = v.attribute_pills.map(p => {
                if (p.color_hex) {
    ...
    async function openEditModal(variantId) {
        ...
        modalBody.innerHTML = '<div class="variant-modal-loading"><i class="fas fa-spinner fa-spin"></i> Loading...</div>';
        openModal();
    ...
    function renderModalContent(data) {
        ...
        if (ctx.attributes && ctx.attributes.length) {
            ...
            ctx.attributes.forEach(attr => {
                ...
                attr.values.forEach(val => {
                    ...
                    if (attr.type === 'color') {
                        const colorStyle = val.color_hex ? `background-color: ${val.color_hex}` : 'background-color: #ccc';
                        html += `
        ...
        const priceAmount = data?.price_amount !== null && data?.price_amount !== undefined ? data.price_amount : '';
        ...
        if (addImageBtn) {
            addImageBtn.addEventListener('click', () => {
                if (window.selectMultipleMedia) {
                    window.selectMultipleMedia((selectedMedia) => {
            ...
            toggle.addEventListener('change', (e) => {
                if (e.target.checked) {
                    ...
                    if (header && !header.querySelector('.primary-badge')) {
                        ...
                        handle?.insertAdjacentHTML('afterend', '<span class="primary-badge"><i class="fas fa-star"></i> Primary</span>');
                    }
                } else {
                    const card = e.target.closest('.variant-modal-image-card');
    ...
    function collectModalData() {
        ...
        if (data.pricing_strategy === 'custom') {
            ...
            data.price_currency = (formContext && formContext.default_currency) || window.__shopCurrency || 'USD';
        }
    ...
    async function saveVariant() {
        ...
        try {
            ...
            if (stockData.length > 0) {
                await postJSON(`${BASE}/variant/${currentVariantId}/stock/update/`, { stock_items: stockData });
            }
    ...
    async function deleteVariant(variantId) {
        ...
        if (!await AdminModal.confirm({ message: `Are you sure you want to delete "${name}"? This action cannot be undone.`, danger: true, confirmText: 'Delete' })) return;
