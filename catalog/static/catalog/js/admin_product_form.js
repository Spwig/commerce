...
(function () {
    ...
    if (dataEl) {
      try {
        ...
        if (cfg.translationsApiUrlTemplate) {
          const productId = cfg.productId || '0';
          cfg.translationsApiUrl = cfg.translationsApiUrlTemplate.replace(
            '/0/',
            '/' + productId + '/'
          );
        }
  ...
  function initProductTypeVisibility() {
    ...
    const giftCardFieldset = document
      .querySelector('.form-row.field-gift_card_denomination_type')
      ?.closest('.module');
    const customizationFieldset = document
      .querySelector('.form-row.field-allow_customization')
      ?.closest('.module');
    ...
    function updateFieldsetVisibility() {
      ...
      if (physicalFieldset) {
        physicalFieldset.style.display = ['digital', 'gift_card', 'booking'].includes(selectedType)
          ? 'none'
          : 'block';
      }

      // Inventory - hide for digital, gift_card, and booking
      const inventoryFieldset = document
        .querySelector('.form-row.field-track_inventory')
        ?.closest('.module');
      if (inventoryFieldset) {
        inventoryFieldset.style.display = ['digital', 'gift_card', 'booking'].includes(selectedType)
          ? 'none'
          : 'block';
      }
    }
    ...
    function updateTabVisibility() {
      ...
      if (digitalTab && digitalPanel) {
        const showDigital =
          ...
          (selectedType === 'digital' || (isDigitalCheckbox && isDigitalCheckbox.checked));
        if (showDigital) {
      ...
      if (customizationTab && customizationPanel) {
        const showCustomization =
          selectedType === 'customizable' ||
          (allowCustomizationCheckbox && allowCustomizationCheckbox.checked);
  ...
  async function loadProductImages() {
    ...
    try {
      const response = await fetch(
        `/api/products/${window.PRODUCT_ADMIN_CONFIG.productId}/images/`
      );
      if (response.ok) {
  ...
  function initStickyHeader() {
    ...
    const getStickyStart = () =>
      placeholder.getBoundingClientRect().top + getScrollPosition() - stickyOffset;
  ...
  if (imagesGrid) {
    imagesGrid.addEventListener('click', async function (e) {
      ...
      if (action === 'set-primary') {
        ...
        if (confirmed) {
          productImages = productImages.filter(function (img) {
