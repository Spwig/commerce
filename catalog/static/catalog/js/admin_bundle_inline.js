...
(function ($) {
  ...
  function updateVariantVisibility(card) {
    ...
    if (customerChooses) {
      ...
      if (
        productInfo &&
        productInfo.product_type === 'variable' &&
  ...
  function getProductInput(card) {
    ...
    return card
      .find('select[name$="-component_product"], input[name$="-component_product"]')
      .first();
  }
  ...
  function updateCardUI(card, productInfo) {
    ...
    if (productInfo) {
      ...
      nameContainer.html('<span class="product-name-text placeholder">Select a product...</span>');
    }
