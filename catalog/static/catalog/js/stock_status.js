...
class StockStatusManager {
  ...
  updateBadge(data) {
    ...
    this.elements.badge.innerHTML = badgeIcon
    ...
    if (data.is_preorder) {
      ...
      if (buttonText) {
        buttonText.textContent =
          data.allow_backorders && !data.in_stock ? 'Backorder' : 'Add to Cart';
      }
    } else {
  ...
  showNotifyError(message) {
    ...
    const existingError = this.elements.notifyContainer?.querySelector(
      '.stock-status__notify-error'
    );
