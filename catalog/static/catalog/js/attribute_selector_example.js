(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const widget = document.querySelector('.attribute-selector-widget');
    const addToCartBtn = document.getElementById('add-to-cart-btn');

    if (widget && widget.attributeSelector) {
      const selector = widget.attributeSelector;
      ...
      addToCartBtn.addEventListener('click', function () {
        ...
        const selectedVariant = selector.getSelectedVariant();
        if (selectedVariant) {
          AdminModal.toast(
            'Added to cart: ' + selectedVariant.name + ' (' + selectedVariant.sku + ')',
            'success'
          );
        }
      });
