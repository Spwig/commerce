...
(function () {
  ...
  function initViewer() {
    ...
    setViewerProp(
      'shadowIntensity',
      'shadow-intensity',
      parseFloat(scene3d.shadow_intensity) || 0.5
    );
  ...
  function applyMaterialColor(mapping) {
    ...
    const matName = data.material_name;
    if (!matName) return;

    const materials = viewer.model.materials;
    for (let i = 0; i < materials.length; i++) {
      if (materials[i].name === matName) {
        const rgb = hexToRgb(data.color || '#ffffff');
        const alpha = data.alpha !== undefined ? data.alpha : 1;
        ...
        if (data.emissive) {
          const emRgb = hexToRgb(data.emissive);
          ...
          if (data.emissive_strength !== undefined) {
            ...
            const lum = rgbLuminance(emRgb);
            const adjStrength = lum > 0.01 ? data.emissive_strength / lum : data.emissive_strength;
    ...
    for (let i = 0; i < materials.length; i++) {
      if (materials[i].name === matName) {
        if (data.base_color_url) {
          try {
            const texture = await viewer.createTexture(data.base_color_url);
        ...
        if (data.normal_url) {
          try {
            const normalTex = await viewer.createTexture(data.normal_url);
  ...
  async function applyGeometrySwap(mapping) {
    ...
    try {
      ...
      let loaded = geometryCache[glbUrl];
      if (!loaded) {
  ...
  function applySelections(selections) {
    ...
    Object.keys(selections).forEach(function (slotId) {
      const opts = selections[slotId];
      if (Array.isArray(opts)) {
        opts.forEach(function (id) {
          activeOptionIds.add(String(id));
        });
      }
    });
    ...
    activeOptionIds.forEach(function (optionId) {
      const optionMappings = mappings[optionId];
  ...
  function applyEmissiveDefaults() {
    if (!viewer.model) return;
    const treeMats = (scene3d.node_tree || {}).materials || {};
    const materials = viewer.model.materials;
    const bloomStrength = parseFloat(scene3d.bloom_strength) || 0;

    for (let i = 0; i < materials.length; i++) {
      const treeData = treeMats[materials[i].name];
  ...
  viewer.addEventListener('load', function () {
    ...
    const configDataEl = document.getElementById('configurator-data');
    if (configDataEl) {
      try {
  ...
  function initFullscreenToggle() {
    ...
    const section = document.querySelector('.configurator-3d-section');
    if (!btn || !section) return;

    const FS_CLASS = 'configurator-3d-section--fullscreen';
    ...
    function toggle() {
      const isFs = section.classList.toggle(FS_CLASS);
