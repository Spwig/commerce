...
(function() {
    ...
    function renderCard(assignment) {
        ...
        const valuesHtml = assignment.values.map(v => {
            ...
            if (isColor) {
                ...
                const dotStyle = hasColor
                    ? `background:${escapeHtml(v.color_hex)}`
                    : '';
    ...
    async function updateColor(input) {
        ...
        try {
            ...
            for (const a of assignments) {
                ...
                if (val) { val.color_hex = newColor; break; }
            }
    ...
    async function toggleValue(pill) {
        ...
        try {
            await postJSON(`${API_BASE}/products/${productId}/attribute-assignments/${assignmentId}/values/`, {
    ...
    async function removeAttribute(btn) {
        ...
        if (!await AdminModal.confirm({ message: `Remove "${name}" from this product?\n\nVariants using this attribute will not be deleted, but their attribute selections may become orphaned.`, danger: true, confirmText: 'Remove' })) {
            return;
        }

        try {
            const data = await postJSON(`${API_BASE}/products/${productId}/attribute-assignments/${assignmentId}/remove/`, {});
    ...
    async function searchAttributes(query) {
        ...
        try {
            ...
            resultsEl.innerHTML = attrs.map(attr => {
                const icon = TYPE_ICONS[attr.type] || 'fa-tag';
                return `
    ...
    function init() {
        ...
        // Modal close buttons
        document.querySelectorAll('.attr-modal-close, .attr-modal-footer .button[data-modal]').forEach(btn => {
