...
(function () {
    ...
    try {
      ...
      if (slotListEl)
        slotListEl.innerHTML =
          '<div class="config-list-error"><i class="fas fa-exclamation-triangle"></i> Failed to load slots</div>';
    }
  }
  ...
  function renderSlotList(slots) {
    ...
    if (!slots.length) {
      slotListEl.innerHTML = `
            ...
            `;
      const addSlotTrigger = slotListEl.querySelector('[data-action="trigger-add-slot"]');
      if (addSlotTrigger) {
  ...
  function renderSlotModalContent(data) {
    const d = data || {};
    const iconOptions = ICON_OPTIONS.map(
      o =>
  ...
  async function deleteSlot(slotId, slotName) {
    if (
      !(await AdminModal.confirm({
        message: `Delete slot "${slotName || 'this slot'}"?\n\nThis will also delete all options and compatibility rules for this slot.`,
        danger: true,
        confirmText: 'Delete',
      }))
    ) {
    ...
    try {
      ...
      if (presetListEl)
        presetListEl.innerHTML =
          '<div class="config-list-error"><i class="fas fa-exclamation-triangle"></i> Failed to load presets</div>';
    }
  }
  ...
  function renderPresetList(presets) {
    ...
    if (!presets.length) {
      presetListEl.innerHTML = `
            ...
            `;
      const addPresetTrigger = presetListEl.querySelector('[data-action="trigger-add-preset"]');
      if (addPresetTrigger) {
  ...
  async function openPresetModal(presetId) {
    ...
    if (presetId) {
      ...
      try {
        const slotData = await fetchJSON(
          `${BASE}/product/${PRODUCT_ID}/slots/list/?include_options=1`
        );
                      ...
                      .map(slot => {
                        ...
                        return `<div class="config-preset-slot-group">
                            ...
                            <div class="config-preset-options" data-slot-id="${slot.id}" data-max="${slot.max_selections}">
                                ${options
                                  .map(opt => {
                                    const checked = slotSelections.includes(opt.id)
                                      ? 'checked'
                                      : '';
                                    const label = opt.variant_name
                                      ? `${escapeHtml(opt.name)} - ${escapeHtml(opt.variant_name)}`
                                      : escapeHtml(opt.name);
  ...
  async function deletePreset(presetId, presetName) {
    if (
      !(await AdminModal.confirm({
        message: `Delete preset "${presetName || 'this preset'}"?`,
        danger: true,
        confirmText: 'Delete',
      }))
    ) {
  ...
  function init() {
    ...
    if (slotOverlay) {
      slotOverlay
        .querySelector('.config-slot-modal-close')
        ?.addEventListener('click', closeSlotModal);
      document
        .getElementById('config-slot-modal-cancel')
        ?.addEventListener('click', closeSlotModal);
      slotOverlay.addEventListener('click', e => {
        if (e.target === slotOverlay) closeSlotModal();
      });
    }
    ...
    if (presetOverlay) {
      presetOverlay
        .querySelector('.config-preset-modal-close')
        ?.addEventListener('click', closePresetModal);
      document
        .getElementById('config-preset-modal-cancel')
        ?.addEventListener('click', closePresetModal);
