...
class LicenseValidationRequestSerializer(serializers.Serializer):
    ...
    key = serializers.CharField(
        max_length=255,
        required=True,
        help_text=_("License key to validate")
    )
    product_id = serializers.IntegerField(
        required=False,
        help_text=_("Optional product ID to validate key against")
    )
...
class LicenseActivationRequestSerializer(serializers.Serializer):
    ...
    key = serializers.CharField(
        max_length=255,
        required=True,
        help_text=_("License key to activate")
    )
...
class LicenseDeactivationRequestSerializer(serializers.Serializer):
    ...
    key = serializers.CharField(
        max_length=255,
        required=True,
        help_text=_("License key")
    )
    device_fingerprint = serializers.CharField(
        max_length=255,
        required=True,
        help_text=_("Device fingerprint to deactivate")
    )
...
class BulkGenerateRequestSerializer(serializers.Serializer):
    ...
    pool_id = serializers.IntegerField(
        required=True,
        help_text=_("License pool ID to generate keys for")
    )
...
class BulkGenerateResponseSerializer(serializers.Serializer):
    ...
    task_id = serializers.CharField(
        required=False,
        help_text=_("Celery task ID for background generation (if async)")
    )
...
class LicenseInfoRequestSerializer(serializers.Serializer):
    ...
    key = serializers.CharField(
        max_length=255,
        required=True,
        help_text=_("License key to get information for")
    )
