...
from core.api.api_descriptions import INVALID_REQUEST
...
def validate_license(request):
    ...
    if not serializer.is_valid():
        return Response(
            {"valid": False, "message": "Invalid request", "errors": serializer.errors},
    ...
    try:
        ...
        license_key = LicenseKey.objects.select_related(
            "order_item__product", "digital_asset", "user"
        ).get(key=key)
        ...
        return Response({"valid": False, "message": "License key not found", "license_info": None})
...
def activate_license(request):
    ...
    try:
        ...
        license_key.save(
            update_fields=["current_activations", "first_activated_at", "last_activated_at"]
        )

        # Dispatch webhook event
        try:
            LicenseWebhookDispatcher.dispatch(
                LicenseWebhookEvents.LICENSE_ACTIVATED, license_key, activation=activation
            )
        ...
        try:
            ...
            active_providers = LicenseProvider.objects.filter(
                is_active=True, sync_on_activation=True
            )
...
def deactivate_license(request):
    ...
    try:
        ...
        try:
            LicenseWebhookDispatcher.dispatch(
                LicenseWebhookEvents.LICENSE_DEACTIVATED, license_key, activation=activation
            )
        except Exception as e:
        ...
        try:
            ...
            active_providers = LicenseProvider.objects.filter(
                is_active=True, sync_on_deactivation=True
            )
...
def list_license_pools(request):
    ...
    return Response({"success": True, "pools": serializer.data})
...
def get_license_pool(request, pool_id):
    ...
    return Response({"success": True, "pool": serializer.data})
...
def bulk_generate_licenses(request):
    ...
    if count <= 0:
        return Response(
            {"success": False, "message": "Pool already has all keys generated"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        return Response(
            {"success": False, "message": f"Bulk generation failed: {str(e)}"},
