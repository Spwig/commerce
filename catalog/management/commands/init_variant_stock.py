...
from catalog.models import ProductVariant, Warehouse, StockItem
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if warehouse_count == 0:
            self.stdout.write(self.style.ERROR('No warehouses found! Please create at least one warehouse first.'))
            return
            ...
            for warehouse in warehouses:
                ...
                if stock_item:
                    ...
                    if not dry_run:
                        ...
                        self.stdout.write(self.style.SUCCESS(f'  ✓ Created stock item for {warehouse.code}'))
                    else:
                        self.stdout.write(self.style.SUCCESS(f'  → Would create stock item for {warehouse.code}'))
                    created_count += 1
        ...
        if dry_run:
            self.stdout.write('')
            self.stdout.write(self.style.WARNING('DRY RUN - Run without --dry-run to actually create records'))
