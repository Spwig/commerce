...
from catalog.models import (
    ...
    BookingAvailabilityRule, BookingRecurrenceRule, Booking,
)
...
STAFF_NAMES = [
    ('Emma', 'Rodriguez'), ('Liam', 'Chen'), ('Sophia', 'Williams'),
    ('Noah', 'Patel'), ('Olivia', 'Kim'), ('James', 'Martinez'),
    ('Ava', 'Johnson'), ('Lucas', 'Thompson'), ('Mia', 'Davis'),
    ('Ethan', 'Wilson'), ('Isabella', 'Brown'), ('Mason', 'Garcia'),
    ('Charlotte', 'Lee'), ('Logan', 'Anderson'), ('Amelia', 'Taylor'),
    ('Alexander', 'Thomas'), ('Harper', 'Jackson'), ('Benjamin', 'White'),
]

CUSTOMER_NAMES = [
    ('Alex', 'Morgan'), ('Jordan', 'Baker'), ('Taylor', 'Reed'),
    ('Casey', 'Brooks'), ('Morgan', 'Foster'), ('Riley', 'Price'),
    ('Quinn', 'Hayes'), ('Avery', 'Russell'), ('Jamie', 'Griffin'),
    ('Drew', 'Palmer'), ('Skyler', 'Ward'), ('Robin', 'Barnes'),
    ('Hayden', 'Ross'), ('Parker', 'Stewart'), ('Reese', 'Collins'),
    ('Charlie', 'Murphy'), ('Kendall', 'Cooper'), ('Dakota', 'Bell'),
    ('Emery', 'Diaz'), ('Finley', 'Hughes'), ('Sage', 'Perry'),
    ('Rowan', 'Butler'), ('Blair', 'Long'), ('Cameron', 'Scott'),
    ('Peyton', 'Mitchell'), ('Addison', 'Wood'), ('Elliot', 'Gray'),
    ('Jessie', 'Bennett'), ('Marley', 'Nelson'), ('Sam', 'Carter'),
]
...
BOOKINGS = {
    ...
    'farm_tour': {
        ...
        'person_types': [
            {'name': 'Adult', 'cost_adjustment': 0, 'min_persons': 1, 'max_persons': 10,
             'is_counted_for_capacity': True, 'is_per_night': False},
            {'name': 'Child (3-12)', 'cost_adjustment': -20, 'min_persons': 0, 'max_persons': 8,
             'is_counted_for_capacity': True, 'is_per_night': False},
            {'name': 'Infant (0-2)', 'cost_adjustment': -45, 'min_persons': 0, 'max_persons': 3,
             'is_counted_for_capacity': False, 'is_per_night': False},
        ],
        'availability_rules': [
            {
                'rule_type': 'available',
    ...
    'farm_stay': {
        ...
        'person_types': [
            {'name': 'Adult', 'cost_adjustment': 0, 'min_persons': 1, 'max_persons': 4,
             'is_counted_for_capacity': True, 'is_per_night': True},
            {'name': 'Child', 'cost_adjustment': -50, 'min_persons': 0, 'max_persons': 3,
             'is_counted_for_capacity': True, 'is_per_night': True},
        ],
        'availability_rules': [
    ...
    'birthday_party': {
        ...
        'person_types': [
            {'name': 'Child', 'cost_adjustment': 15, 'min_persons': 5, 'max_persons': 25,
    ...
    'pottery_workshop': {
        ...
        'person_types': [
            {'name': 'Adult', 'cost_adjustment': 0, 'min_persons': 1, 'max_persons': 6,
             'is_counted_for_capacity': True},
            {'name': 'Student (16-21)', 'cost_adjustment': -20, 'min_persons': 0, 'max_persons': 6,
             'is_counted_for_capacity': True},
        ],
        'availability_rules': [
            {
...
class Command(BaseCommand):
    ...
    def _create_booking(self, spec):
        ...
        if Product.all_objects.filter(sku=sku).exists():
            self.stderr.write(self.style.ERROR(
                f'Product {sku} already exists. Use --delete first to recreate.'
            ))
            return
        ...
        self.stdout.write(f'  Config: {config.booking_type} / {config.duration}{config.duration_unit[0]}')
        ...
        if count > 0:
            ...
            booking_count = self._generate_bookings(
                product, config, resources, person_types, spec,
            )
        ...
        self.stdout.write(f'  Bookings: http://localhost:8000/en/admin/catalog/booking/?product__id__exact={product.id}')
        self.stdout.write(f'  Calendar: http://localhost:8000/en/admin/catalog/booking/?view=calendar')
    ...
    def _generate_bookings(self, product, config, resources, person_types, spec):
        ...
        while bookings_created < count and attempts < max_attempts:
            ...
            valid_windows = [
                w for w in time_windows if dow in w['days']
            ]
            ...
            start_dt = timezone.make_aware(
                datetime(random_date.year, random_date.month, random_date.day,
    ...
    def _generate_accommodation_bookings(self, product, config, resources, person_types, spec):
        ...
        while bookings_created < count and attempts < max_attempts:
            ...
            start_dt = timezone.make_aware(
                datetime(check_in_date.year, check_in_date.month, check_in_date.day,
                         check_in.hour, check_in.minute),
            )
            end_dt = timezone.make_aware(
                datetime(check_out_date.year, check_out_date.month, check_out_date.day,
                         check_out.hour, check_out.minute),
            )
