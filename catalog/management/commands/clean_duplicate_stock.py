...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        duplicates = (
            StockItem.objects.values("product_id", "warehouse_id", "variant_id")
        ...
        self.stdout.write(
            self.style.WARNING(f"Found {duplicates.count()} sets of duplicate records:\n")
        )
        ...
        for dup in duplicates:
            ...
            stock_items = StockItem.objects.filter(
                product_id=product_id, warehouse_id=warehouse_id, variant_id=variant_id
            ).order_by("id")
