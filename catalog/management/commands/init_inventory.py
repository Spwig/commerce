...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--country", type=str, required=True, help="Two-letter country code (e.g., US, NZ, SG)"
        )
        parser.add_argument(
            "--currency",
            type=str,
        ...
        parser.add_argument("--address", type=str, required=True, help="Warehouse address line 1")
        parser.add_argument("--city", type=str, required=True, help="City")
        parser.add_argument("--postal-code", type=str, required=True, help="Postal/ZIP code")
        parser.add_argument("--state", type=str, default="", help="State or province (optional)")
        parser.add_argument(
            "--region-name",
            type=str,
        ...
        parser.add_argument("--email", type=str, help="Contact email for warehouse")
        parser.add_argument("--phone", type=str, help="Contact phone for warehouse")
    ...
    def handle(self, *args, **options):
        ...
        if region_created:
            self.stdout.write(
                self.style.SUCCESS(f"✓ Created SalesRegion: {region.name} ({region.code})")
            )
        else:
        ...
        if warehouse_created:
            self.stdout.write(
                self.style.SUCCESS(f"✓ Created Warehouse: {warehouse.name} ({warehouse.code})")
            )
        else:
        ...
        self.stdout.write(
            "  2. Or import stock from CSV: ./manage.py import_stock --input stock.csv"
        )
