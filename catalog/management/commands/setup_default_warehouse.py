...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ Created sales region: {region.name} ({region.code})")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"  ⚠ Sales region already exists: {region.name} ({region.code})"
                )
            )
        ...
        if created:
            ...
            self.stdout.write(
                self.style.WARNING("  ⚠ Please update the warehouse address in the admin interface")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"  ⚠ Warehouse already exists: {warehouse.name} ({warehouse.code})"
                )
            )
        ...
        if not skip_migration:
            ...
            if total_products == 0:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "  Please use the Stock Items admin to set actual stock levels."
                    )
                )
        else:
