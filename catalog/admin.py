...
from core.admin_mixins import TranslatableAdminMixin, MoneyFieldCurrencyMixin
from core.widgets import TranslatableFieldWidget, KeyValueWidget, IconPickerWidget
...
import json
from .models import (
    Category, Brand, Product, ProductImage, ProductVariantImage, ProductPrice, ProductVariant,
    ProductReview, Collection, Promotion, PriceCharmingRule,
    SalesRegion, Warehouse, StockItem, StockMovement, ProductRegionVisibility,
    ProductAttribute, AttributeValue, ProductAttributeAssignment,
    DigitalAsset, LicenseKey, LicenseKeyTemplate, LicenseProvider, ExternalLicenseSync,
    WebhookSubscription, DigitalDownload, LicenseActivation, LicensePool,
    BundleItem, ProductDependency, GiftCard, GiftCardTransaction,
    CustomizationOption, CustomizationValue,
    StockDisplaySettings, StockNotification,
    ConfigurationSlot, ConfigurationSlotOption, CompatibilityRule, ConfigurationPreset,
    BookingConfig, BookingResource, BookingPersonType,
    BookingAvailabilityRule, BookingRecurrenceRule,
    Booking, BookingWaitlist, BookingSlotReservation,
    ProductTag,
)
from .forms import ProductVariantForm, CommaSeparatedDecimalField
...
class DigitalAssetInline(admin.StackedInline):
    ...
    fields = [
        'file', 'filename', 'version', 'file_metadata',
        'requires_license', 'license_key_preview',
        'download_limit', 'expiration_days',
        'is_active'
    ]
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_digital_asset.css',)
        }
    ...
    def file_metadata(self, obj):
        ...
        if obj.pk and obj.file:
            ...
            size_display = obj.get_file_size_display() if hasattr(obj, 'get_file_size_display') else f"{obj.file_size / (1024*1024):.2f} MB"

            # File type
            file_type = obj.file_type or _('Unknown')

            # Upload date
            upload_date = date_format(obj.created_at, 'SHORT_DATETIME_FORMAT') if hasattr(obj, 'created_at') else _('N/A')
...
class ProductVariantInline(admin.StackedInline):
    ...
    fields = [
        'name', 'sku', 'is_active',
        'selected_attributes',
        'pricing_strategy', 'price',
        'weight', 'length', 'width', 'height',
        'barcode', 'preferred_shipping_package'
    ]
    readonly_fields = ['attributes_display', 'effective_price', 'stock_summary']
    filter_horizontal = ['selected_attributes']  # Use filter_horizontal to respect queryset filtering
    ordering = ['name']

    class Media:
        css = {
            'all': ['catalog/css/hide_selected_attributes_add.css']
        }
            ...
            if product:
                ...
                if assigned_attrs:
                    ...
                    kwargs['queryset'] = AttributeValue.objects.filter(
                        attribute_id__in=assigned_attrs
                    ).select_related('attribute').order_by('attribute__name', 'sort_order')
                else:
    ...
    def effective_price(self, obj):
        ...
        if obj.pk:
            ...
            if obj.pricing_strategy == 'inherit':
                return format_html('<span title="Inherited from product">{} (inherited)</span>', price)
            return str(price)
        return '-'
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_variant_inline.css',)
        }
        js = (
...
class BundleItemInline(admin.StackedInline):
    ...
    fields = ['component_product', 'allow_variant_selection', 'component_variant', 'quantity', 'sort_order', 'is_optional']
    ordering = ['sort_order', 'id']
    template = 'admin/catalog/edit_inline/bundle_stacked.html'

    class Media:
        css = {
            'all': ('catalog/css/admin_bundle.css',)
        }
...
class CustomizationOptionInline(admin.TabularInline):
    ...
    fields = [
        'name', 'option_type', 'is_required', 'sort_order',
        'pricing_type', 'price_amount', 'max_length',
        'min_value', 'max_value', 'choices_display'
    ]
        ...
        if db_field.name == 'icon':
            ...
            return forms.CharField(
                widget=IconPickerWidget(
                    priority_icons=[
                        'fa-microchip', 'fa-gear', 'fa-sliders',
                        'fa-puzzle-piece', 'fa-wrench', 'fa-cube',
                        'fa-palette', 'fa-memory',
                    ],
                    style_prefix=True,
                ),
                required=False,
...
class ConfigurationPresetInline(admin.TabularInline):
    ...
    fields = [
        'name', 'slug', 'description', 'is_featured', 'sort_order',
    ]
    prepopulated_fields = {'slug': ('name',)}
    ordering = ['sort_order', 'name']
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_customization.css',)
        }
...
class BookingConfigInline(admin.StackedInline):
    ...
    fieldsets = (
        (_('Booking Type & Duration'), {
            'fields': (
                'booking_type', 'duration_type', 'duration', 'duration_unit',
        ...
        (_('Scheduling'), {
            'fields': (
                ...
                'min_advance', 'min_advance_unit',
                'max_advance', 'max_advance_unit',
        ...
        (_('Confirmation & Cancellation'), {
            'fields': (
                ...
                'cancellation_allowed', 'cancellation_deadline', 'cancellation_deadline_unit',
            ),
        }),
        (_('Display & UX'), {
            'fields': (
                'calendar_display', 'customer_timezone_enabled',
            ),
        }),
        (_('Deposits'), {
            'fields': (
...
class BookingResourceInline(admin.TabularInline):
    ...
    fields = [
        'name', 'resource_type', 'quantity', 'assignment_type',
        'base_cost_adjustment', 'email', 'is_active', 'sort_order',
    ]
    ordering = ['sort_order', 'name']
...
class BookingPersonTypeInline(admin.TabularInline):
    ...
    fields = [
        'name', 'cost_adjustment', 'min_persons', 'max_persons',
        'is_counted_for_capacity', 'sort_order',
    ]
    ordering = ['sort_order', 'name']
...
class BookingAvailabilityRuleInline(admin.TabularInline):
    ...
    fields = [
        'rule_type', 'scope', 'start_date', 'end_date',
        'start_time', 'end_time', 'days_of_week',
        'cost_override', 'cost_adjustment', 'priority',
    ]
...
class BookingRecurrenceRuleInline(admin.TabularInline):
    ...
    fields = [
        ...
        'auto_create_days_ahead', 'is_active',
    ]
...
class AttributeValueInline(admin.TabularInline):
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_attribute.css',)
        }
...
class ProductAttributeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('name', 'slug', 'type', 'is_required', 'sort_order')
        }),
        (_('Translations'), {
            'fields': ('translations',),
        }),
    ...
    class Meta:
        ...
        widgets = {
            ...
            'icon': IconPickerWidget(
                priority_icons=[
                    'fa-bag-shopping', 'fa-store', 'fa-tag', 'fa-folder',
                    'fa-shirt', 'fa-laptop', 'fa-house', 'fa-utensils',
                    'fa-heart', 'fa-star',
                ],
                style_prefix=True,
            ),
...
class CategoryAdmin(CustomFieldsAdminMixin, SEOGeneratorAdminMixin, TranslatableAdminMixin, admin.ModelAdmin):
    """
    ...
    fieldsets = (
        (_('Basic Information'), {
            ...
            'description': _('Click the globe icon to translate name and description into other languages.')
        }),
        (_('Media'), {
            'fields': ('image_asset', 'banner_asset', 'icon'),
            'description': _('Select images from your media library or choose an icon for this category.')
        }),
        (_('Display Options'), {
            'fields': ('page_template', 'products_per_page', 'show_subcategories')
        }),
        (_('Design & Theme'), {
            'fields': ('template_variant', 'css_classes', 'layout_config', 'style_overrides',
                      'cascade_theme_to_children', 'cascade_theme_to_products', 'inherit_parent_theme'),
            'classes': ('collapse',)
        }),
        (_('SEO'), {
            'fields': ('meta_title', 'meta_description', 'seo_auto_generated'),
            'description': _('Click the globe icon to translate SEO fields. Use "Regenerate SEO" to auto-generate.')
        }),
        (_('Status'), {
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['parent_categories'] = Category.objects.filter(
            parent__isnull=True
        ).order_by('name')
...
class BrandAdmin(SEOGeneratorAdminMixin, TranslationAdmin):
    ...
    fieldsets = (
        ...
        (_('Brand Assets'), {
            'fields': ('logo', 'banner_image', 'brand_story')
        }),
        (_('SEO'), {
            'fields': ('meta_title', 'meta_description', 'seo_auto_generated')
        }),
        (_('Status'), {
            'fields': ('is_active', 'is_featured', 'show_brand_page')
        })
    )
...
class ProductForm(forms.ModelForm):
    ...
    gift_card_denominations = CommaSeparatedDecimalField(
        ...
        help_text=_("Enter amounts separated by commas (e.g., 25, 50, 100). Currency symbol not needed."),
        required=False,
...
class ProductAdmin(CustomFieldsAdminMixin, SEOGeneratorAdminMixin, TranslatableAdminMixin, MoneyFieldCurrencyMixin, admin.ModelAdmin):
    ...
    translatable_fields = ['name', 'short_description', 'full_description', 'meta_title', 'meta_description']
    ...
    class Media:
        js = TranslatableAdminMixin.Media.js + SEOGeneratorAdminMixin.Media.js
        css = {
            **TranslatableAdminMixin.Media.css,
            **SEOGeneratorAdminMixin.Media.css
        }
    ...
    list_display = ['name', 'sku', 'category', 'brand', 'price', 'pricing_strategy', 'status', 'is_featured', 'international_ready']
    list_filter = ['status', 'is_featured', 'pricing_strategy', 'category', 'brand', 'product_type', 'country_of_origin']
    search_fields = ['name', 'sku', 'hs_code']
    prepopulated_fields = {'slug': ('name',)}
    inlines = [ProductImageInline, ProductAttributeAssignmentInline, ProductVariantInline, ProductPriceInline, DigitalAssetInline, StockItemInline, ProductRegionVisibilityInline]
    actions = ['mark_international_ready', 'export_customs_data', 'delete_selected_products', 'restore_selected_products']
    ...
    fieldsets = (
        ...
        (_('Product Descriptions'), {
            ...
            'description': _('Short description appears in product listings. Full description shows on the product page.')
        }),
        (_('SEO'), {
            'fields': ('meta_title', 'meta_description', 'seo_auto_generated'),
            'description': _('Search engine optimization fields for better visibility in search results.')
        }),
        (_('Product Details'), {
            'fields': ('features', 'specifications')
        }),
        (_('Pricing'), {
            'fields': ('pricing_strategy', 'price', 'cost', 'sale_type', 'sale_value', 'sale_start_date', 'sale_end_date'),
            'description': _('Use "Dynamic" pricing to auto-convert prices using exchange rates, or "Fixed" to set custom prices per currency below.')
        }),
        (_('Bundle Pricing'), {
            'fields': ('bundle_pricing_strategy', 'bundle_discount_percentage'),
            'description': _('Configure how the bundle price is calculated. Choose "Fixed Price" to use the price field above, "Percentage Discount" to discount component sum, or "Sum of Components" to show total of all components.'),
            'classes': ('collapse',)
        }),
        (_('Gift Card Configuration'), {
            'fields': ('gift_card_denomination_type', 'gift_card_denominations', 'gift_card_min_amount', 'gift_card_max_amount', 'gift_card_expires_days', 'gift_card_currency'),
            'description': _('Configure gift card options. Choose fixed denominations, custom amounts, or both. Set expiration period (0 = never expires). Optionally set a specific currency for multi-currency stores.'),
            'classes': ('collapse',)
        }),
        (_('Customization Configuration'), {
            'fields': ('allow_customization', 'customization_preview_template'),
            'description': _('Enable product customization to allow customers to personalize this product with text, images, or selections. Customization options are managed in the section below.'),
            'classes': ('collapse',)
        }),
        (_('Subscription Configuration'), {
            'fields': ('is_subscription_enabled', 'subscription_plans', 'allow_one_time_purchase', 'subscription_default'),
            'description': _('Enable subscription billing for this product. Customers can purchase with recurring payments. Configure available subscription plans and whether one-time purchase is also allowed.'),
            'classes': ('collapse',)
        }),
        (_('Configurator Configuration'), {
            'fields': ('configurator_pricing_strategy', 'configurator_base_price'),
            'description': _('Configure pricing for build-to-order products. '
                           'Slots and options are managed in the sections below.'),
            'classes': ('collapse',)
        }),
        (_('Inventory'), {
            'fields': ('track_inventory', 'low_stock_threshold', 'allow_backorders', 'out_of_stock_action_override'),
            'description': _('Inventory is tracked per warehouse. Manage stock levels in the Stock Items section below. Out of stock action overrides site-wide and category settings.')
        }),
        (_('Pre-Order'), {
            'fields': ('is_preorder', 'preorder_release_date', 'preorder_message'),
            'description': _('Enable pre-orders for products not yet available. Pre-order products are purchasable even when out of stock.'),
            'classes': ('collapse',)
        }),
        (_('Physical Attributes'), {
            'fields': ('weight', 'length', 'width', 'height', 'preferred_shipping_package')
        }),
        (_('Product Identifiers'), {
            'fields': ('gtin', 'ean', 'upc', 'isbn', 'asin', 'mpn')
        }),
        (_('International Shipping / Customs'), {
            'fields': ('hs_code', 'country_of_origin', 'unit_price_for_customs',
                      'export_license_number', 'export_license_expiry'),
            'description': _('Required for international shipments. '
        ...
        (_('Product Page Design'), {
            'fields': ('page_template', 'gallery_type', 'show_related_products', 'show_reviews', 'show_specifications'),
            'description': _('Leave "Page Template" empty to use the site default. Configure site-wide defaults under Design > Page Templates.'),
        }),
        (_('Status'), {
            'fields': ('status', 'is_featured', 'is_digital', 'hide_from_storefront')
        }),
        (_('Licensing'), {
            'fields': ('requires_license', 'license_generation_trigger', 'license_template',
                      'default_license_type', 'default_max_activations',
                      'default_validity_days', 'license_provider', 'external_product_id'),
            ...
            for i, (name, options) in enumerate(fieldsets):
                if name == _('Inventory'):
                    ...
                    options = {
                        ...
                        'description': _('<strong>Stock is managed at the variant level.</strong> '
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if 'gift_card_currency' in form.base_fields:
            ...
            gc_currency_choices.extend(
                (code, label) for code, label in currency_choices
            )
    ...
    def get_queryset(self, request):
        ...
        if has_digital_assets == 'with':
            qs = qs.annotate(
                digital_asset_count=Count('digital_assets')
            ).filter(digital_asset_count__gt=0)
        elif has_digital_assets == 'without':
            qs = qs.annotate(
                digital_asset_count=Count('digital_assets')
            ).filter(digital_asset_count=0)
    ...
    def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
        ...
        if request.method == 'POST':
            ...
            print(f'[ProductAdmin.changeform_view] POST keys (first 30): {list(request.POST.keys())[:30]}')
            print(f'[ProductAdmin.changeform_view] product_images_data present: {"product_images_data" in request.POST}')
            ...
            formsets, inline_instances = self._create_formsets(request, self.get_object(request, object_id), change=bool(object_id))
            ...
            for i, formset in enumerate(formsets):
                ...
                print(f'[ProductAdmin.changeform_view] Formset {i} ({inline_name}) is_valid: {is_valid}')
                if not is_valid:
                    print(f'[ProductAdmin.changeform_view] Formset {i} errors: {formset.errors}')
                    print(f'[ProductAdmin.changeform_view] Formset {i} non_form_errors: {formset.non_form_errors()}')
            print('='*80 + '\n')
    ...
    def save_model(self, request, obj, form, change):
        ...
        print(f'[ProductAdmin.save_model] POST keys: {list(request.POST.keys())[:20]}')  # Show first 20 keys
        print(f'[ProductAdmin.save_model] product_images_data in POST: {"product_images_data" in request.POST}')
        ...
        if not settings.enable_multi_warehouse and obj.product_type != 'variable' and 'simple_stock_quantity' in request.POST:
            try:
                from catalog.models import Warehouse, StockItem, SalesRegion
                ...
                if default_warehouse:
                    ...
                    if not created and stock_item.on_hand != quantity:
                        ...
                        print(f'[ProductAdmin.save_model] Updated simple stock: {old_quantity} -> {quantity}')
                        ...
                        print(f'[ProductAdmin.save_model] Created simple stock item with on_hand: {quantity}')
            except Exception as e:
                print(f'[ProductAdmin.save_model] Error updating simple stock quantity: {e}')

        print(f'[ProductAdmin.save_model] Completed')
    ...
    def save_formset(self, request, form, formset, change):
        ...
        if formset.model == ProductVariant:
            from catalog.models import Warehouse, StockItem

            print(f'[ProductAdmin.save_formset] Processing variant stock updates...')

            # Debug: Print all POST keys related to stock
            stock_keys = [k for k in request.POST.keys() if 'stock_item_' in k]
            print(f'[ProductAdmin.save_formset] Found {len(stock_keys)} stock-related POST keys: {stock_keys[:5]}...')
            ...
            print(f'[ProductAdmin.save_formset] Processing stock for {len(all_variants)} total variants')

            for instance in all_variants:
                print(f'[ProductAdmin.save_formset] Processing variant: {instance.name} (ID: {instance.pk})')
                ...
                for warehouse in warehouses:
                    ...
                    if created:
                        print(f'[ProductAdmin.save_formset] Created stock item for variant {instance.name} in warehouse {warehouse.code}')

                # Look for stock item updates in request.POST
                stock_items = instance.stock_items.all()
                print(f'[ProductAdmin.save_formset] Found {stock_items.count()} stock items for this variant')

                for stock_item in stock_items:
                    on_hand_key = f'stock_item_{stock_item.id}_on_hand'
                    threshold_key = f'stock_item_{stock_item.id}_low_stock_threshold'

                    print(f'[ProductAdmin.save_formset] Looking for keys: {on_hand_key}, {threshold_key}')
                    print(f'[ProductAdmin.save_formset] Keys in POST: {on_hand_key in request.POST}, {threshold_key in request.POST}')

                    if on_hand_key in request.POST:
                        print(f'[ProductAdmin.save_formset] Found {on_hand_key} = {request.POST[on_hand_key]}')
                    ...
                    if on_hand_key in request.POST:
                        try:
                            ...
                            if stock_item.on_hand != new_on_hand:
                                ...
                                logger.info(f'Updated stock for variant {instance.name} in {stock_item.warehouse.code}: {old_on_hand} -> {new_on_hand}')
                                print(f'[ProductAdmin.save_formset] Updated stock: {old_on_hand} -> {new_on_hand}')
                                ...
                                try:
                                    ...
                                    print(f'[ProductAdmin.save_formset] Created stock movement record')
                                except Exception as e:
                                    logger.error(f'Error creating stock movement: {e}')
                                    print(f'[ProductAdmin.save_formset] Error creating stock movement: {e}')
                    ...
                    if threshold_key in request.POST:
                        try:
                            ...
                            if stock_item.low_stock_threshold != new_threshold:
                                ...
                                updated = True
                                logger.info(f'Updated low stock threshold for variant {instance.name} in {stock_item.warehouse.code}: {new_threshold}')
                                print(f'[ProductAdmin.save_formset] Updated threshold: {new_threshold}')
                        except (ValueError, TypeError) as e:
                    ...
                    if updated:
                        ...
                        print(f'[ProductAdmin.save_formset] No changes for stock item ID: {stock_item.id}')
    ...
    def save_related(self, request, form, formsets, change):
        ...
        print(f'[ProductAdmin.save_related] product_images_data present: {bool(product_images_data)}')
        if product_images_data:
            print(f'[ProductAdmin.save_related] product_images_data length: {len(product_images_data)}')
            try:
                images_data = json.loads(product_images_data)
                print(f'[ProductAdmin.save_related] Successfully parsed {len(images_data)} images from JSON')
                ...
                for image_data in images_data:
                    ...
                    if image_id and not str(image_id).startswith('new_'):
                        ...
                        try:
                            product_image = ProductImage.objects.get(id=image_id, product=form.instance)
                    ...
                    if not image_id or str(image_id).startswith('new_'):
                        # Create new image
                        print(f'[ProductAdmin.save_related] Creating new image for media_asset_id: {media_asset_id}')
                        ...
                        try:
                            ...
                            updated_image_ids.add(product_image.id)
                            print(f'[ProductAdmin.save_related] Created new ProductImage with id: {product_image.id}')
                            logger.info(f'Created new ProductImage {product_image.id}')
                        except MediaAsset.DoesNotExist:
                            print(f'[ProductAdmin.save_related] ERROR: MediaAsset {media_asset_id} not found')
                ...
                if images_to_delete:
                    ...
                    print(f'[ProductAdmin.save_related] Deleted {len(images_to_delete)} ProductImages')
                    logger.info(f'Deleted {len(images_to_delete)} ProductImages')

                print(f'[ProductAdmin.save_related] Finished processing images. Updated: {len(updated_image_ids)}, Deleted: {len(images_to_delete)}')
            ...
            print('[ProductAdmin.save_related] No product_images_data in POST, skipping image processing')
            ...
            if formset.model == ProductVariant:
                for form_obj in formset.forms:
                    if form_obj.instance.pk:  # Only process saved variants
                        ...
                        print(f'[ProductAdmin.save_related] Variant {variant.id} ({variant.name}): images_data present: {bool(variant_images_data)}')

                        if variant_images_data:
                            try:
                                images_data = json.loads(variant_images_data)
                                print(f'[ProductAdmin.save_related] Processing {len(images_data)} images for variant {variant.id}')
                                logger.info(f'Processing variant {variant.id} images: {len(images_data)} images')
                                ...
                                for image_data in images_data:
                                    ...
                                    if not media_asset_id:
                                        logger.warning(f'Skipping variant image with no media_asset_id: {image_data}')
                                        continue
                                    ...
                                    if image_id and not str(image_id).startswith('new_'):
                                        ...
                                        try:
                                            ...
                                            logger.info(f'Updated ProductVariantImage {variant_image.id}')
                                        except ProductVariantImage.DoesNotExist:
                                            logger.warning(f'ProductVariantImage {image_id} not found, creating new one')
                                            # Fall through to create new one
                                            image_id = None

                                    if not image_id or str(image_id).startswith('new_'):
                                        # Create new image
                                        print(f'[ProductAdmin.save_related] Creating new variant image for media_asset_id: {media_asset_id}')
                                        from media_library.models import MediaAsset
                                        try:
                                            media_asset = MediaAsset.objects.get(id=media_asset_id)
                                            print(f'[ProductAdmin.save_related] Found MediaAsset: {media_asset.id}')
                                            ...
                                            updated_image_ids.add(variant_image.id)
                                            print(f'[ProductAdmin.save_related] Created new ProductVariantImage with id: {variant_image.id}')
                                            logger.info(f'Created new ProductVariantImage {variant_image.id}')
                                        except MediaAsset.DoesNotExist:
                                            print(f'[ProductAdmin.save_related] ERROR: MediaAsset {media_asset_id} not found')
                                            logger.error(f'MediaAsset {media_asset_id} not found')
                                            continue
                                ...
                                if images_to_delete:
                                    ...
                                    print(f'[ProductAdmin.save_related] Deleted {len(images_to_delete)} ProductVariantImages for variant {variant.id}')
                                    logger.info(f'Deleted {len(images_to_delete)} ProductVariantImages for variant {variant.id}')

                                print(f'[ProductAdmin.save_related] Finished processing variant {variant.id} images. Updated: {len(updated_image_ids)}, Deleted: {len(images_to_delete)}')

                            except json.JSONDecodeError as e:
                                print(f'[ProductAdmin.save_related] ERROR: Failed to parse variant images JSON: {e}')
                                logger.error(f'Failed to parse variant images data for variant {variant.id}: {e}')
                            except Exception as e:
                                print(f'[ProductAdmin.save_related] ERROR: Exception processing variant images: {e}')
                                logger.error(f'Error processing variant {variant.id} images: {e}', exc_info=True)
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if not settings.enable_multi_warehouse and object_id:
            try:
                from catalog.models import Warehouse, StockItem, Product
        ...
        if object_id:
            try:
                ...
                if product_obj.product_type == 'customizable':
                    extra_context['has_design_config'] = ProductDesignConfig.objects.filter(
        ...
        if object_id:
            try:
                ...
                if product.product_type == 'configurable':
                    from catalog.models import ConfigurationSlot, CompatibilityRule
        ...
        if object_id:
            try:
                ...
                product = Product.objects.get(pk=object_id)
                slot_options = ConfigurationSlotOption.objects.filter(
                    option_product=product
                ).select_related('slot__product').distinct()
    ...
    def mark_international_ready(self, request, queryset):
        ...
        if ready_count > 0:
            messages.success(request, f'{ready_count} product(s) are ready for international shipping.')
    ...
    def export_customs_data(self, request, queryset):
        ...
        writer.writerow([
            'SKU', 'Name', 'HS Code', 'Country of Origin',
            'Customs Unit Price', 'Export License', 'License Expiry',
            'International Ready'
        ])
    ...
    def delete_selected_products(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{count} product(s) moved to recycle bin.',
            messages.SUCCESS
        )
    ...
    def restore_selected_products(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                f'{count} product(s) restored from recycle bin.',
                messages.SUCCESS
            )
        ...
        if errors:
            self.message_user(
                request,
                'Some products could not be restored: ' + '; '.join(errors),
                messages.ERROR
            )
    ...
    def bulk_action_view(self, request):
        ...
        try:
            ...
            if not action or not product_ids:
                return JsonResponse({
                    'success': False,
                    'message': _('Action and product IDs required')
                }, status=400)
...
class CollectionAdmin(TranslationAdmin):
    ...
    fieldsets = (
        ...
        (_('Design'), {
            'fields': ('template_variant', 'inherit_parent_theme', 'css_classes',
                       'layout_config', 'style_overrides', 'responsive_config'),
            'classes': ('tab-design',),
        }),
        (_('Status'), {
            'fields': ('is_active', 'is_featured', 'sort_order'),
...
class PromotionAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'discount_type', 'discount_value', 'apply_to', 'is_active', 'start_date', 'end_date', 'priority']
    ...
    fieldsets = (
        ...
        (_('Advanced Settings'), {
            'fields': ('priority', 'can_stack_with_product_sales'),
            'classes': ('collapse',)
        })
    )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['expired_count'] = queryset.filter(
            end_date__lt=now
        ).count()
        ...
        if selector_type == 'categories':
            ...
            for cat in categories:
                items.append({
                    ...
                    'meta': f'{cat.products.count()} products' if hasattr(cat, 'products') else None
                })
            ...
            for brand in brands:
                items.append({
                    ...
                    'meta': f'{brand.products.count()} products' if hasattr(brand, 'products') else None
                })
            ...
            from .models import Product
            products = Product.objects.filter(status='published').select_related('category', 'brand').prefetch_related('images').order_by('name')
            for product in products:
...
class PriceCharmingRuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Charming Rule'), {
            ...
            'description': _('Select the type of price charming to apply. For custom endings, specify the decimal value (e.g., 0.99 for .99 endings).')
        }),
        (_('Settings'), {
            'fields': ('min_price_threshold', 'apply_to_sale_prices', 'is_active')
        }),
    )
...
class BundleItemAdmin(admin.ModelAdmin):
    ...
    list_display = ['bundle', 'component_product', 'component_variant', 'quantity', 'sort_order', 'is_optional']
    ...
    fieldsets = (
        ...
        (_('Item Details'), {
            'fields': ('quantity', 'sort_order', 'is_optional')
        }),
    )
...
class SalesRegionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('name', 'code', 'is_active', 'priority')
        }),
        (_('Geographic Settings'), {
            'fields': ('countries', 'default_currency'),
            'description': _('Enter ISO country codes as a JSON array (e.g., ["NZ", "FJ", "AU"])')
        }),
    )
        ...
        if object_id:
            try:
                ...
                if obj:
                    ...
                    extra_context['country_count'] = len(countries) if isinstance(countries, list) else 0
            except Exception:
                pass
        return super().change_view(request, object_id, form_url, extra_context=extra_context)
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Add filter options
        extra_context['currencies'] = SalesRegion.objects.values_list('default_currency', flat=True).distinct().order_by('default_currency')
...
class WarehouseAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'code', 'region', 'city', 'country', 'is_active', 'fulfillment_priority', 'stock_item_count']
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['countries'] = Warehouse.objects.values_list('country', flat=True).distinct().order_by('country')
...
class StockMovementInline(admin.TabularInline):
    ...
    extra = 0
    can_delete = False
    fields = ['created_at', 'movement_type', 'quantity', 'previous_quantity', 'new_quantity', 'reason', 'user']
    readonly_fields = ['created_at', 'movement_type', 'quantity', 'previous_quantity', 'new_quantity', 'reason', 'user']
    ordering = ['-created_at']
...
class StockItemAdmin(admin.ModelAdmin):
    ...
    list_display = ['product_link', 'sku', 'warehouse', 'on_hand', 'allocated', 'available', 'low_stock_threshold', 'stock_status_icon', 'updated_at']
    ...
    def stock_status_icon(self, obj):
        ...
        if available <= 0:
            ...
            # Adequate stock
            return format_html(
                '<span style="color: #28a745;" title="In Stock">✅ OK</span>'
            )
    ...
    def adjust_stock_action(self, request, queryset):
        ...
        class StockAdjustmentForm(forms.Form):
            ...
            quantity = forms.IntegerField(
                ...
                initial=0,
                label=_('Quantity'),
                help_text=_('For "Add" and "Remove": amount to change. For "Set": new on_hand value.')
            )
        ...
        if 'apply' in request.POST:
            ...
            if form.is_valid():
                ...
                messages.success(request, f'Successfully adjusted stock for {updated_count} item(s)')
                return redirect(request.get_full_path())
        else:
            form = StockAdjustmentForm()

        # Render intermediate page with form
        return render(request, 'admin/catalog/stock_adjustment.html', {
            'form': form,
    ...
    def mark_for_reorder(self, request, queryset):
        ...
        if low_stock_items:
            ...
            items_list = ', '.join([f"{item.product.name} ({item.warehouse.code})" for item in low_stock_items])
            messages.warning(
                request,
...
class StockMovementAdmin(admin.ModelAdmin):
    ...
    list_display = ['created_at', 'stock_item', 'movement_type', 'quantity', 'previous_quantity', 'new_quantity', 'user', 'order']
    list_filter = ['movement_type', 'created_at', 'stock_item__warehouse']
    search_fields = ['stock_item__product__name', 'stock_item__product__sku', 'reason', 'order__order_number']
    readonly_fields = ['created_at', 'stock_item', 'movement_type', 'quantity', 'previous_quantity', 'new_quantity', 'reason', 'order', 'user']
...
class ProductRegionVisibilityAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Visibility'), {
            'fields': ('is_visible',),
            'description': _('Controls whether this product appears in this region. If no rules exist for a product, it is visible in all regions by default.')
        }),
    )
...
class LicenseKeyAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'key_display', 'digital_asset_link', 'user_link', 'order_link',
        'key_type', 'status_badge', 'activation_status', 'issued_at'
    ]
    list_filter = ['status', 'key_type', 'issued_at', 'expires_at']
    search_fields = [
        'key', 'user__email', 'user__username',
        'order_item__order__order_number',
        'digital_asset__filename', 'digital_asset__product__name'
    ]
    readonly_fields = [
        'key', 'order_item', 'digital_asset', 'user',
        'current_activations', 'issued_at',
        'activation_list_display'
    ]
    date_hierarchy = 'issued_at'
    ordering = ['-issued_at']
    actions = [
        'activate_licenses', 'deactivate_licenses', 'revoke_licenses',
        'export_license_keys', 'regenerate_license_keys', 'resend_license_email'
    ]

    fieldsets = (
        (_('License Key'), {
            'fields': ('key', 'key_type', 'status')
        }),
        (_('Associated Records'), {
            'fields': ('digital_asset', 'order_item', 'user')
        }),
        (_('Activation Limits'), {
    ...
    def digital_asset_link(self, obj):
        ...
        if obj.digital_asset:
            ...
            return format_html(
                ...
                url, obj.digital_asset.filename, obj.digital_asset.product.name
            )
        return '-'
    ...
    def activation_status(self, obj):
        ...
        percentage = (obj.current_activations / obj.max_activations * 100) if obj.max_activations > 0 else 0
        ...
        return format_html(
            ...
            color, icon, obj.current_activations, obj.max_activations
        )
    ...
    def activation_list_display(self, obj):
        ...
        if obj.pk:
            ...
            if not activations.exists():
                return format_html('<em style="color: #999;">{}</em>', _('No active device activations'))
    ...
    def activate_licenses(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f'{updated} license key(s) activated successfully.')
        )
    ...
    def deactivate_licenses(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f'{updated} license key(s) suspended successfully.')
        )
    ...
    def revoke_licenses(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f'{updated} license key(s) revoked and all device activations removed.')
        )
    ...
    def export_license_keys(self, request, queryset):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="license_keys_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'

        writer = csv.writer(response)
        writer.writerow([
            'License Key', 'Product', 'Asset', 'Customer Email',
        ...
        for license_key in queryset.select_related(
            'digital_asset', 'digital_asset__product',
            'user', 'order_item__order'
        ):
            writer.writerow([
                license_key.key,
                ...
                license_key.order_item.order.order_number if license_key.order_item and license_key.order_item.order else '',
                ...
                license_key.expires_at.strftime('%Y-%m-%d') if license_key.expires_at else 'Never'
            ])
    ...
    def regenerate_license_keys(self, request, queryset):
        ...
        messages.success(
            request,
            _(f'{regenerated} license key(s) regenerated. Old keys have been revoked.')
        )
    ...
    def resend_license_email(self, request, queryset):
        ...
        for license_key in queryset.select_related(
            ...
            context = {
                ...
                'product_name': license_key.digital_asset.product.name if license_key.digital_asset else 'Digital Product',
                'order_number': license_key.order_item.order.order_number if license_key.order_item and license_key.order_item.order else '',
                ...
                'expiration_date': license_key.expires_at.strftime('%B %d, %Y') if license_key.expires_at else None,
            }
            ...
            try:
                ...
                if outbox and outbox.status == 'queued':
                    ...
                    logger.info(f"License email skipped for {license_key.key}: {outbox.skip_reason}")
                    failed_count += 1
        ...
        if sent_count > 0:
            messages.success(
                request,
                _(f'Successfully sent license key email to {sent_count} customer(s).')
            )
        if failed_count > 0:
            messages.warning(
                request,
                _(f'Failed to send email for {failed_count} license key(s). Check that customers have valid email addresses.')
            )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        digital_products = Product.objects.filter(
            digital_assets__requires_license=True
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_license_key.css',)
        }
...
class LicenseKeyTemplateAdmin(admin.ModelAdmin):
    ...
    def products_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:catalog_product_changelist') + f'?license_template__id__exact={obj.pk}'
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_license_template.css',)
        }
...
class LicenseProviderAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_license_provider.css',)
        }
    ...
    fieldsets = (
        ...
        (_('API Configuration'), {
            ...
            'description': _('API credentials for connecting to external license provider. Keep these secure!')
        }),
        (_('Synchronization Settings'), {
            'fields': ('sync_on_order', 'sync_on_activation', 'sync_on_deactivation', 'sync_bidirectional')
        }),
        (_('Webhook Configuration'), {
            'fields': ('webhook_secret', 'webhook_events'),
            'classes': ('collapse',)
        }),
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        for provider in all_providers:
            ...
            if provider_class:
                ...
                provider_meta = next(
                    (p for p in available_providers if p['key'] == provider.provider_type),
                    None
                )
        ...
        extra_context['not_tested_count'] = all_providers.filter(connection_status='not_tested').count()
...
class ExternalLicenseSyncAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('External Reference'), {
            'fields': ('external_id', 'external_data_display')
        }),
        (_('Error Details'), {
...
class WebhookSubscriptionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('name', 'url', 'secret', 'is_active')
        }),
        (_('Event Configuration'), {
            'fields': ('events',),
            'description': _('Select which license events to receive')
        }),
        (_('Filtering'), {
    ...
    def delivery_stats(self, obj):
        ...
        if obj.total_deliveries > 0:
            ...
            return format_html(
                ...
                color, obj.successful_deliveries, obj.total_deliveries, success_rate
            )
...
class LicensePoolAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        'keys_generated', 'keys_distributed', 'status', 'progress_display',
        'available_keys_display', 'generation_started_at', 'generation_completed_at',
    ...
    fieldsets = (
        ...
        (_('License Configuration'), {
            'fields': (
                'license_template', 'key_type', 'max_activations',
                'expires_after_days', 'pool_expires_at'
            )
        }),
        ...
        (_('Progress'), {
            'fields': ('progress_display', 'available_keys_display'),
            'classes': ('collapse',)
        }),
        (_('External Sync'), {
            'fields': ('sync_to_provider',),
            'classes': ('collapse',)
        }),
        (_('Generation Status'), {
            'fields': (
    ...
    def generate_pool_keys(self, request, queryset):
        ...
        if generated_total > 0:
            self.message_user(
                request,
                f'Successfully generated {generated_total} license keys',
                messages.SUCCESS
            )
    ...
    def export_pool_keys(self, request, queryset):
        ...
        writer = csv.writer(response)
        writer.writerow(['Pool Name', 'License Key', 'Key Type', 'Max Activations', 'Status', 'Is Distributed'])
    ...
    def reset_pool_status(self, request, queryset):
        ...
        self.message_user(
            request,
            f'Reset {updated} pool(s) to ready status',
            messages.SUCCESS
        )
...
class DigitalAssetAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'filename_display', 'product_link', 'version', 'file_info',
        'download_stats', 'license_stats', 'is_active_badge', 'created_at'
    ]
    ...
    readonly_fields = [
        ...
        'download_analytics', 'license_key_analytics'
    ]
    date_hierarchy = 'created_at'
    ordering = ['-created_at']
    ...
    fieldsets = (
        ...
        (_('License Settings'), {
            ...
            'description': _('If enabled, license keys will be generated automatically on order completion.')
        }),
        (_('Download Settings'), {
            'fields': ('download_limit', 'expiration_days', 'download_analytics'),
            'description': _('Leave blank for unlimited downloads. Expiration is counted from order date.')
        }),
        (_('Status'), {
            'fields': ('is_active',)
        }),
    ...
    def product_link(self, obj):
        ...
        if obj.product:
            ...
            return format_html(
                ...
                url, obj.product.name, obj.product.sku
            )
        return '-'
    product_link.short_description = _('Product')
    ...
    def file_info(self, obj):
        ...
        size = obj.get_file_size_display() if hasattr(obj, 'get_file_size_display') else f"{obj.file_size / (1024*1024):.2f} MB"
        file_type = obj.file_type or _('Unknown')
    ...
    def download_stats(self, obj):
        ...
        if obj.pk:
            ...
            return format_html(
                ...
                '</div>',
                total_downloads, color, completed_downloads, success_rate
            )
        return '-'
    ...
    def download_analytics(self, obj):
        ...
        if obj.pk:
            ...
            if not downloads.exists():
                return format_html('<em style="color: #999;">{}</em>', _('No download activity yet'))
            ...
            return format_html(
                ...
                '</table>'
                '</div>',
                total, completed, failed, in_progress, unique_users, recent_downloads
            )
        return '-'
    ...
    def license_key_analytics(self, obj):
        ...
        if obj.pk:
            ...
            return format_html(
                ...
                '</table>'
                '</div>',
                total, active, expired, revoked, suspended, total_activations
            )
        return '-'
    ...
    def activate_assets(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f'{updated} digital asset(s) activated successfully.')
        )
    ...
    def deactivate_assets(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f'{updated} digital asset(s) deactivated successfully.')
        )
    ...
    def export_asset_report(self, request, queryset):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="digital_assets_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'

        writer = csv.writer(response)
        writer.writerow([
            'Filename', 'Product', 'SKU', 'Version', 'File Size (MB)',
            'File Type', 'Requires License', 'Download Limit', 'Expiration Days',
            'Is Active', 'Total Downloads', 'License Keys Generated', 'Created'
        ])

        for asset in queryset.select_related('product'):
            # Get statistics
            total_downloads = DigitalDownload.objects.filter(digital_asset=asset).count()
            license_count = LicenseKey.objects.filter(digital_asset=asset).count() if asset.requires_license else 0
...
class DigitalDownloadAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'digital_asset_link', 'user_link', 'status_badge',
        'ip_address', 'downloaded_at', 'file_size_display'
    ]
    list_filter = ['status', 'downloaded_at']
    search_fields = [
        'user__email', 'user__username',
        'digital_asset__filename', 'digital_asset__product__name',
        'order_item__order__order_number', 'ip_address'
    ]
    readonly_fields = [
        'digital_asset', 'order_item', 'user', 'ip_address', 'user_agent',
        'downloaded_at', 'status', 'error_message', 'file_version'
    ]
    ...
    fieldsets = (
        (_('Download Information'), {
            'fields': ('digital_asset', 'order_item', 'user', 'status')
        }),
        (_('Client Information'), {
            'fields': ('ip_address', 'user_agent')
        }),
        (_('Timing'), {
            'fields': ('downloaded_at', 'file_version')
        }),
        (_('Error Details'), {
            'fields': ('error_message',),
            'classes': ('collapse',)
...
class GiftCardTransactionInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['transaction_type', 'amount', 'balance_after', 'order_link', 'notes', 'created_at', 'created_by']
    readonly_fields = ['transaction_type', 'amount', 'balance_after', 'order_link', 'notes', 'created_at', 'created_by']
    ordering = ['-created_at']
    can_delete = False
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_gift_card.css',)
        }
...
class GiftCardAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'code', 'status_badge', 'balance_display', 'recipient_email',
        'recipient_name', 'issued_at', 'expires_at', 'created_at'
    ]
    list_filter = [
        'is_active', 'expires_at', 'issued_at', 'created_at',
        ('product', admin.RelatedOnlyFieldListFilter)
    ]
    search_fields = ['code', 'recipient_email', 'recipient_name', 'sender_name']
    readonly_fields = [
        'code', 'product_link', 'order_item_link', 'initial_value', 'current_balance',
        'balance_display', 'redemption_progress', 'is_expired', 'is_valid',
        'created_at', 'issued_at', 'first_used_at'
    ]
    fieldsets = [
        ...
        (_('Balance'), {
            'fields': ['initial_value', 'current_balance', 'balance_display', 'redemption_progress'],
            'classes': ['tab-balance'],
        }),
        (_('Recipient'), {
    ...
    class Media:
        css = {
            'all': ('catalog/css/admin_gift_card.css',
                    'catalog/css/giftcard_change_form.css')
        }
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        active_count = all_gift_cards.filter(
            is_active=True
        ).exclude(
            expires_at__lt=now
        ).count()
        ...
        total_balance = all_gift_cards.filter(
            is_active=True
        ).aggregate(
    ...
    def balance_display(self, obj):
        ...
        return format_html(
            ...
            color, obj.current_balance, obj.initial_value
        )
    ...
    def redemption_progress(self, obj):
        ...
        return format_html(
            ...
            '</div>',
            bar_color, percentage, percentage
        )
    ...
    def send_gift_card_email(self, request, queryset):
        ...
        self.message_user(request, _('{count} gift card email(s) sent').format(count=queryset.count()))
...
class GiftCardTransactionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'gift_card_code', 'transaction_type_badge', 'amount_display',
        'balance_after', 'order_link', 'created_at', 'created_by'
    ]
    list_filter = ['transaction_type', 'created_at', ('gift_card', admin.RelatedOnlyFieldListFilter)]
    search_fields = ['gift_card__code', 'notes']
    readonly_fields = [
        'gift_card', 'transaction_type', 'amount', 'balance_after',
        'order', 'notes', 'created_at', 'created_by'
    ]
    fieldsets = [
        ...
        (_('Details'), {
            'fields': ['order', 'notes', 'created_at', 'created_by']
        }),
    ]
    date_hierarchy = 'created_at'
    ordering = ['-created_at']
    ...
    def transaction_type_badge(self, obj):
        ...
        return format_html(
            '<span style="display: inline-flex; align-items: center; gap: 4px; '
            'color: {}; font-weight: 500;"><i class="fas {}"></i> {}</span>',
            color, icon_class, obj.get_transaction_type_display()
        )
...
class CustomizationOptionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'product', 'option_type', 'is_required',
        'pricing_type', 'price_amount', 'sort_order'
    ]
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('product', 'name', 'description', 'sort_order')
        }),
        (_('Option Configuration'), {
            'fields': ('option_type', 'is_required')
        }),
        (_('Validation Rules'), {
            'fields': (
                'max_length', 'allowed_file_types', 'max_file_size_mb',
                'min_value', 'max_value', 'choices'
            ),
            'description': _('Configure validation rules based on option type. Text/textarea uses max_length, file uses allowed_file_types and max_file_size_mb, number uses min/max_value, select/color uses choices.')
        }),
        (_('Pricing'), {
            'fields': ('pricing_type', 'price_amount'),
            'description': _('Configure additional pricing for this customization. For select options, you can also set per-choice price modifiers in the choices field.')
        })
    )
...
class StockDisplaySettingsAdmin(admin.ModelAdmin):
    ...
    list_display = ['__str__', 'out_of_stock_action', 'allow_backorders', 'show_stock_status', 'updated_at']

    fieldsets = (
        (_('Stock Status Display'), {
            'fields': ('show_stock_status', 'show_low_stock_warning', 'low_stock_threshold', 'show_exact_quantity'),
            'description': _('Configure how stock status is displayed on product pages.')
        }),
        (_('Out of Stock Behavior'), {
            'fields': ('out_of_stock_action', 'out_of_stock_message', 'allow_backorders', 'backorder_message'),
            'description': _('Configure default behavior when products are out of stock. These can be overridden at category and product level.')
        }),
        (_('Warehouse Display'), {
            'fields': ('show_ships_from', 'show_estimated_delivery'),
            'description': _('Configure warehouse and delivery information shown to customers.')
        }),
    )
...
class StockNotificationAdmin(admin.ModelAdmin):
    ...
    list_display = ['email', 'product_name', 'variant_name', 'created_at', 'notified_status', 'notified_at']
    ...
    fieldsets = (
        ...
        (_('Product'), {
            'fields': ('product', 'variant', 'preferred_warehouse')
        }),
    ...
    def send_notifications(self, request, queryset):
        ...
        if sent_count > 0:
            ...
            messages.info(request, _('All {} notification(s) skipped due to user preferences.').format(skipped_count))
        else:
            messages.warning(request, _('No notifications to send (products still out of stock or already notified).'))
...
class ConfigurationSlotAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'product', 'is_required', 'min_selections', 'max_selections', 'sort_order']
...
class BookingAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'booking_id', 'product_link', 'resource', 'customer_display',
        'start_datetime', 'end_datetime', 'status_badge', 'total_cost',
    ]
    list_filter = ['status', 'product', 'resource', 'is_recurring']
    search_fields = [
        'customer_name', 'customer_email', 'customer_phone',
        'product__name', 'resource__name',
    ]
    date_hierarchy = 'start_datetime'
    ordering = ['-start_datetime']
    readonly_fields = [
        'ical_uid', 'recurrence_group_id', 'created_at', 'updated_at',
        'reminder_sent_at', 'duration_display',
    ]
    list_per_page = 50
    ...
    fieldsets = (
        ...
        (_('Customer'), {
            'fields': (
                ...
                'customer_phone', 'customer_notes', 'customer_timezone',
            ),
        }),
        (_('Person Counts'), {
            'fields': ('persons',),
    ...
    def mark_confirmed(self, request, queryset):
        updated = queryset.filter(
            status__in=['pending_confirmation']
        ...
        if object_id:
            ...
            try:
                booking = Booking.objects.select_related(
                    'product', 'resource', 'order', 'customer',
                ).get(pk=object_id)
                ...
                extra_context.update({
                    ...
                    'ical_url': (
                        ...
                        if booking.ical_uid and booking.product else None
                    ),
    ...
    def _handle_booking_action(self, request, object_id):
        ...
        if action == 'confirm':
            ...
            ok, msg = BookingLifecycleService.cancel_booking(
                booking, author=author, reason=reason, initiated_by='admin',
            )
            ...
            ok, msg = BookingLifecycleService.change_status(
                booking, new_status, author=author,
            )
            ...
            ok, msg = BookingLifecycleService.reschedule_booking(
                booking, new_start, new_end, author=author,
            )
            ...
            BookingLifecycleService.add_note(
                booking, note_text, author=author,
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if view_mode == 'calendar':
            ...
            last_day = datetime(year, month, last_day_num, 23, 59, 59, tzinfo=dt_timezone.utc)

            bookings = Booking.objects.filter(
                start_datetime__gte=first_day,
                start_datetime__lte=last_day,
            ).select_related('product', 'resource', 'customer').order_by('start_datetime')
            ...
            start_dt = datetime.combine(week_start, datetime.min.time()).replace(tzinfo=dt_timezone.utc)
            end_dt = datetime.combine(week_end, datetime.max.time()).replace(tzinfo=dt_timezone.utc)
            ...
            start_dt = datetime.combine(target_date, datetime.min.time()).replace(tzinfo=dt_timezone.utc)
            end_dt = datetime.combine(target_date, datetime.max.time()).replace(tzinfo=dt_timezone.utc)
...
class BookingWaitlistAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'customer_email', 'customer_name', 'product',
        'desired_date', 'time_range', 'status_badge', 'created_at',
    ]
    ...
    fieldsets = (
        (None, {
            'fields': (
                'product', 'customer', 'customer_email', 'customer_name',
                'desired_date', 'desired_time_start', 'desired_time_end',
                'desired_persons', 'status', 'notified_at', 'created_at',
            ),
        }),
    )
