...
class Category(CustomFieldsMixin, DesignMixin):
    ...
    page_template = models.CharField(
        ...
        help_text=_('Override the site default category page template. Leave empty to use the site default from Design settings.')
    )

    # Theme inheritance for child categories and products
    cascade_theme_to_children = models.BooleanField(
        default=True,
        help_text="Apply this category's design to child categories"
    )
    cascade_theme_to_products = models.BooleanField(
        default=True,
        help_text="Apply this category's design to products"
    )
    ...
    allow_backorders_override = models.BooleanField(
        ...
        null=True,
        help_text=_('Override site-wide backorder setting for products in this category (None = inherit from site)')
    )
    ...
    seo_auto_generated = models.BooleanField(
        default=False,
        help_text="Automatically regenerate SEO content when category is saved"
    )
    ...
    def get_card_image_url(self):
        ...
        product = (
            self.products
            .filter(status='published', images__isnull=False)
            .only('id')
            .first()
        )
        if product:
...
class Brand(DesignMixin):
    ...
    external_id = models.CharField(
        ...
        blank=True,
        db_index=True,
        help_text="Original ID from source platform"
    )
    imported_meta = models.JSONField(
        default=dict,
        blank=True,
        help_text="Metadata from import"
    )
    ...
    seo_auto_generated = models.BooleanField(
        default=False,
        help_text="Automatically regenerate SEO content when brand is saved"
    )
...
class ProductQuerySet(models.QuerySet):
    ...
    def with_optimized_stock_prefetch(self):
        ...
        """
        from catalog.models import StockItem, Warehouse  # Import here to avoid circular imports
...
class Product(CustomFieldsMixin, DesignMixin):
    ...
    meta_title = models.CharField(
        max_length=255,
        blank=True,
        help_text="SEO title for search engines (primary language)"
    )
    ...
    seo_auto_generated = models.BooleanField(
        default=False,
        help_text="Automatically regenerate SEO content when product is saved"
    )
    ...
    pricing_strategy = models.CharField(
        ...
        help_text=_("How to calculate prices in different currencies. Dynamic: Auto-convert using exchange rates. Fixed: Set custom prices per currency.")
    )
    ...
    bundle_pricing_strategy = models.CharField(
        ...
        help_text=_("How to calculate bundle price. Fixed: Use the price field. Percentage Discount: Apply discount to component sum. Components Sum: Total of all components.")
    )
    ...
    bundle_discount_percentage = models.DecimalField(
        ...
        help_text=_("Discount percentage off component total (e.g., 15.00 for 15% off). Only used with 'Percentage Discount' strategy.")
    )
    ...
    gift_card_denomination_type = models.CharField(
        ...
        help_text=_("How customers select gift card value. Fixed: Choose from preset amounts. Custom: Enter any amount. Both: Preset amounts + custom option.")
    )
    ...
    gift_card_denominations = models.JSONField(
        ...
        help_text=_("List of preset amounts (e.g., [25, 50, 100, 200]). Currency symbol not needed.")
    )
    ...
    gift_card_currency = models.CharField(
        ...
        help_text=_("Currency for issued gift cards. Leave blank for store's base currency. "
    ...
    configurator_pricing_strategy = models.CharField(
        ...
        help_text=_("How the final price is calculated. Sum of Components: total of all selected options. "
    ...
    configurator_base_price = MoneyField(
        ...
        help_text=_("Starting price for 'Base Price + Adjustments' strategy. "
    ...
    is_subscription_enabled = models.BooleanField(
        ...
        help_text=_("Allow customers to purchase this product as a subscription with recurring billing")
    )
    subscription_plans = models.ManyToManyField(
        ...
        help_text=_("Available subscription plans for this product. Customers can choose from these plans.")
    )
    allow_one_time_purchase = models.BooleanField(
        default=True,
        verbose_name=_("Allow One-Time Purchase"),
        help_text=_("Allow customers to buy this product without a subscription. If disabled, subscription is required.")
    )
    subscription_default = models.BooleanField(
        default=False,
        verbose_name=_("Default to Subscription"),
        help_text=_("Pre-select subscription option in the product interface (one-time purchase must also be allowed)")
    )
    ...
    preferred_shipping_package = models.ForeignKey(
        ...
        help_text=_("Default package to use for shipping this product. If set, package dimensions will be used instead of product dimensions for shipping calculations.")
    )
    ...
    hs_code = models.CharField(
        ...
        help_text=_("Harmonized System code (6-12 digits) for customs classification. "
    ...
    country_of_origin = models.CharField(
        ...
        help_text=_("ISO 3166-1 alpha-2 country code where product was manufactured. "
    ...
    unit_price_for_customs = models.DecimalField(
        ...
        help_text=_("Declared value per unit for customs purposes. May differ from retail price. "
    ...
    export_license_number = models.CharField(
        ...
        help_text=_("Required for controlled/restricted items (ITAR, EAR). Leave blank for most products.")
    )
    ...
    page_template = models.CharField(
        ...
        default='',
        verbose_name=_('Page Template'),
        help_text=_('Override the site default product page template. Leave empty to use the site default from Design settings.')
    )
    ...
    product_sections = models.JSONField(
        default=list, 
        blank=True,
        help_text="Custom content sections for product page"
    )
    ...
    is_digital = models.BooleanField(
        ...
        help_text=_("Check if this product includes digital downloads (e.g., files, licenses). Can be combined with any product type for scenarios like variable digital products (software with Basic/Pro editions) or customizable digital products (custom designs).")
    )
    hide_from_storefront = models.BooleanField(
        default=False,
        verbose_name=_("Hide from Storefront"),
        help_text=_("Hide this product from catalog listings and search results. "
    ...
    license_template = models.ForeignKey(
        ...
        help_text=_("Custom license key format for digital products. If not set, uses default format.")
    )
    ...
    default_validity_days = models.PositiveIntegerField(
        ...
        help_text=_("Default license validity in days from issue date. Leave blank for never expires.")
    )
    ...
    requires_license = models.BooleanField(
        ...
        help_text=_("Generate a license key when this product is purchased. Enable for software licenses, API keys, or activation codes.")
    )
    ...
    is_deleted = models.BooleanField(
        default=False,
        db_index=True,
        help_text=_("Whether this product has been soft deleted")
    )
    deleted_at = models.DateTimeField(
        null=True,
        blank=True,
        db_index=True,
        help_text=_("When this product was soft deleted")
    )
    ...
    def variant_stock_summary(self):
        ...
        totals = self.stock_items.filter(
    ...
    def get_variant_stock_status(self, variant):
        ...
        available = variant.stock_items.aggregate(
            total=Sum(F('on_hand') - F('allocated'))
        )['total'] or 0
    ...
    def is_on_sale(self):
        ...
        # Check end date
        if self.sale_end_date and self.sale_end_date < now:
            return False
    ...
    def get_effective_bundle_price(self):
        ...
        if self.bundle_pricing_strategy == 'fixed':
            ...
            discount_multiplier = (Decimal('100.00') - self.bundle_discount_percentage) / Decimal('100.00')
    ...
    def validate_bundle_inventory(self, warehouse=None):
        ...
        for item in self.bundle_items.all():
            if not item.check_stock_availability(warehouse):
                return False
        return True
    ...
    def is_bundle_digital(self):
        ...
        for item in self.bundle_items.all():
            if not item.component_product.is_digital:
                return False
        return True
    ...
    def is_international_shipping_ready(self):
        ...
        return bool(
            self.hs_code and
            self.country_of_origin and
            self.unit_price_for_customs
        )
    ...
    def clean(self):
        ...
        if self.hs_code:
            if not re.match(r'^\d{6,12}$', self.hs_code):
                raise ValidationError({
        ...
        if self.country_of_origin:
            ...
            if not re.match(r'^[A-Z]{2}$', self.country_of_origin.upper()):
                raise ValidationError({
                    'country_of_origin': _('Country code must be 2-letter ISO code. Example: US, CA, DE')
                })
            # Auto-uppercase
        ...
        if self.hs_code and not self.unit_price_for_customs:
            raise ValidationError({
                'unit_price_for_customs': _('Customs unit price is required when HS code is specified')
            })
        ...
        if self.gift_card_currency:
            if self.product_type != 'gift_card':
                raise ValidationError({
                    'gift_card_currency': _('Gift card currency can only be set on gift card products.')
                })
            ...
            elif settings.supported_currencies and self.gift_card_currency not in settings.supported_currencies:
                raise ValidationError({
                    'gift_card_currency': _('Currency "%(currency)s" is not in the store\'s supported currencies.')
...
class BundleItem(models.Model):
    ...
    quantity = models.PositiveIntegerField(
        default=1,
        help_text=_("How many of this component are included")
    )
    sort_order = models.IntegerField(
        default=0,
        help_text=_("Display order in bundle")
    )
    is_optional = models.BooleanField(
        default=False,
        help_text=_("Can customers exclude this item? (for customizable bundles)")
    )
    allow_variant_selection = models.BooleanField(
        default=False,
        help_text=_("If True, customer chooses variant at purchase time. "
    ...
    def clean(self):
        ...
        if self.bundle_id and self.component_product_id and self.bundle_id == self.component_product_id:
        ...
        if self.component_product and self.component_product.product_type == 'bundle':
            raise ValidationError(
                _("Bundle products cannot be used as components. Only simple and variable products are allowed.")
            )
            ...
            if self.component_variant.product_id != self.component_product_id:
                raise ValidationError(
                    _("Variant %(variant)s does not belong to product %(product)s.") % {
                        'variant': self.component_variant,
                        'product': self.component_product
                    }
                )

        # Validate simple products don't have variant
        if self.component_product and self.component_product.product_type == 'simple' and self.component_variant:
...
class ProductDependency(models.Model):
    ...
    def __str__(self):
        return f"{self.product.name} {self.get_dependency_type_display()} {self.required_product.name}"

    def clean(self):
        if self.product_id and self.required_product_id and self.product_id == self.required_product_id:
...
class ProductImage(models.Model):
    ...
    alt_text = models.CharField(
        max_length=255,
        blank=True,
        help_text="Override media asset alt text if needed"
    )
...
class ProductAttribute(models.Model):
    ...
    slug = models.SlugField(
        max_length=100,
        unique=True,
        help_text="URL-safe version of name"
    )
    ...
    is_required = models.BooleanField(
        default=True,
        help_text="Must customers select this attribute?"
    )
    sort_order = models.IntegerField(
        default=0,
        help_text="Display order in variation selector"
    )
    translations = models.JSONField(
        default=dict,
        blank=True,
...
class AttributeValue(models.Model):
    ...
    value = models.CharField(
        max_length=100,
        help_text="Value label (e.g., 'Small', 'Red', 'Cotton')"
    )
    slug = models.SlugField(
        max_length=100,
        help_text="URL-safe version of value"
    )
    color_hex = models.CharField(
        max_length=7,
        blank=True,
        help_text="Hex color code for color swatches (e.g., '#FF0000')"
    )
    sort_order = models.IntegerField(
        default=0,
        help_text="Display order within attribute"
    )
    translations = models.JSONField(
        default=dict,
        blank=True,
...
class ProductAttributeAssignment(models.Model):
    ...
    sort_order = models.IntegerField(
        default=0,
        help_text="Display order for this attribute on product page"
    )
...
class ProductVariant(DesignMixin):
    ...
    quantity = models.IntegerField(
        default=0,
        help_text="DEPRECATED: Use StockItem model for multi-warehouse inventory"
    )
    ...
    color_swatch = models.CharField(max_length=7, blank=True, help_text="Hex color code for color variants")
    barcode = models.CharField(
        _('barcode'),
        max_length=50,
        blank=True,
...
class ProductVariantImage(models.Model):
    ...
    is_primary = models.BooleanField(
        default=False,
        help_text="Primary image shown first in variant gallery"
    )
    ...
    show_in_gallery = models.BooleanField(
        default=True,
        help_text="Show in product detail page gallery"
    )
    show_in_listing = models.BooleanField(
        default=True,
        help_text="Show in product listing pages"
    )
...
class Collection(DesignMixin):
    ...
    auto_criteria = models.JSONField(
        default=dict, 
        blank=True,
        help_text="Criteria for automatic product selection"
    )
...
class Promotion(models.Model):
    ...
    description = models.TextField(
        blank=True,
        help_text="Internal notes about this promotion"
    )
    ...
    start_date = models.DateTimeField(
        ...
        help_text="When this promotion becomes active"
    )
    ...
    priority = models.PositiveIntegerField(
        default=0,
        help_text="Higher priority promotions are applied first (0 is lowest)"
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text="Enable or disable this promotion"
    )
...
class SalesRegion(models.Model):
    ...
    default_currency = models.CharField(
        _('default currency'),
        max_length=3,
        help_text=_('ISO currency code (e.g., "NZD", "SGD")')
    )
    is_active = models.BooleanField(
        _('active'),
        default=True,
        help_text=_('Whether this region is currently active')
    )
    priority = models.IntegerField(
        _('priority'),
        default=0,
        help_text=_('Higher priority regions are checked first for stock')
    )
...
class Warehouse(models.Model):
    ...
    is_active = models.BooleanField(
        _('active'),
        default=True,
        help_text=_('Whether this warehouse is currently operational')
    )
    ...
    display_name = models.CharField(
        ...
        max_length=100,
        blank=True,
        help_text=_('Customer-facing name (e.g., "Ships from Australia"). Leave blank to use warehouse name.')
    )
...
class StockItem(models.Model):
    ...
    def allocate(self, quantity):
        ...
        if quantity > self.available:
            raise ValueError(
                f"Cannot allocate {quantity} units. Only {self.available} available."
            )
    ...
    def fulfill(self, quantity):
        ...
        if quantity > self.allocated:
            raise ValueError(
                f"Cannot fulfill {quantity} units. Only {self.allocated} allocated."
            )
        self.allocated = F('allocated') - quantity
...
class StockMovement(models.Model):
    ...
    quantity = models.IntegerField(
        _('quantity'),
        help_text=_('Change in quantity (positive or negative)')
    )
    previous_quantity = models.IntegerField(
        _('previous quantity'),
        help_text=_('Stock level before this movement')
    )
    ...
    reason = models.TextField(
        _('reason'),
        blank=True,
        help_text=_('Explanation for this movement')
    )
...
class ProductRegionVisibility(models.Model):
    ...
    is_visible = models.BooleanField(
        _('visible'),
        default=True,
        help_text=_('Whether product is visible in this region')
    )
...
class DigitalAsset(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.file.field.storage or self.file.field.storage.__class__.__name__ != 'MinIODigitalAssetsStorage':
...
class DigitalDownload(models.Model):
    ...
    error_message = models.TextField(
        ...
        help_text=_('Error details if download failed')
    )
    ...
    @classmethod
    def create_download_record(cls, digital_asset, order_item, user=None, ip_address=None, user_agent=''):
        """
...
class LicenseActivation(models.Model):
    ...
    def deactivate(self, reason=''):
        ...
        self.license_key.current_activations = self.license_key.activations.filter(is_active=True).count()
...
class LicenseKeyTemplate(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        help_text=_("Template name (e.g., 'Standard App License')")
    )
    description = models.TextField(
        blank=True,
        help_text=_("Description of this template and when to use it")
    )
    is_active = models.BooleanField(
        default=True,
        help_text=_("Inactive templates cannot be used for new products")
    )
    ...
    prefix = models.CharField(
        max_length=20,
        blank=True,
        help_text=_("Static prefix for all keys (e.g., 'MYAPP')")
    )

    suffix = models.CharField(
        max_length=20,
        blank=True,
        help_text=_("Static suffix for all keys")
    )

    separator = models.CharField(
        max_length=5,
        default='-',
        help_text=_("Character(s) used to separate key segments")
    )
    ...
    min_length = models.IntegerField(
        default=20,
        help_text=_("Minimum total key length (including separators)")
    )

    max_length = models.IntegerField(
        default=50,
        help_text=_("Maximum total key length (including separators)")
    )
    ...
    def validate_pattern(self):
        ...
        valid_placeholders = [
            'RANDOM', 'CHECKSUM', 'PREFIX', 'SUFFIX',
            'ORDER_ID', 'PRODUCT_SKU', 'DATE'
        ]
...
class LicenseProvider(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        help_text=_("Provider name (e.g., 'Keygen Production')")
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=_("Inactive providers will not sync licenses")
    )

    # API Configuration
    api_endpoint = models.URLField(
        max_length=500,
        help_text=_("API base URL (e.g., 'https://api.keygen.sh/v1')")
    )
    api_key = models.CharField(
        max_length=500,
        help_text=_("API key or token for authentication. NOTE: Store securely - consider encryption in production.")
    )
    api_secret = models.CharField(
        max_length=500,
        blank=True,
        help_text=_("API secret (if required by provider)")
    )

    # Sync Configuration
    sync_on_order = models.BooleanField(
        default=True,
        help_text=_("Automatically sync license to provider when order is completed")
    )
    sync_on_activation = models.BooleanField(
        default=True,
        help_text=_("Sync device activations to provider")
    )
    sync_on_deactivation = models.BooleanField(
        default=True,
        help_text=_("Sync device deactivations to provider")
    )
    sync_bidirectional = models.BooleanField(
        default=False,
        help_text=_("Allow provider to update Spwig licenses via webhooks")
    )
    ...
    webhook_secret = models.CharField(
        max_length=255,
        blank=True,
        help_text=_("Shared secret for webhook signature verification")
    )
    webhook_events = models.JSONField(
        default=list,
        blank=True,
        help_text=_("List of webhook events to receive from provider")
    )
    ...
    product_mapping = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Map Spwig product IDs to external license policies/products. Format: {product_id: external_policy_id}")
    )
    ...
    connection_error = models.TextField(
        blank=True,
        help_text=_("Error message if connection test failed")
    )
    last_tested_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("Last time connection was tested")
    )
...
class ExternalLicenseSync(models.Model):
    ...
    external_id = models.CharField(
        max_length=255,
        db_index=True,
        help_text=_("License ID in external provider system")
    )
    external_data = models.JSONField(
        default=dict,
        blank=True,
        help_text=_("Full response data from external provider")
    )
    ...
    next_retry_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When to retry failed sync")
    )
...
class WebhookSubscription(models.Model):
    ...
    secret = models.CharField(
        max_length=255,
        help_text=_("Shared secret for HMAC signature verification")
    )
    is_active = models.BooleanField(
        default=True,
        help_text=_("Inactive webhooks will not receive events")
    )

    # Event Configuration
    events = models.JSONField(
        default=list,
        help_text=_("List of event types to receive (e.g., ['license.generated', 'license.activated'])")
    )
    ...
    retry_delay_seconds = models.IntegerField(
        default=300,
        help_text=_("Delay in seconds between retry attempts")
    )

    # Statistics
    total_deliveries = models.IntegerField(
        default=0,
        help_text=_("Total webhook deliveries attempted")
    )
    successful_deliveries = models.IntegerField(
        default=0,
        help_text=_("Successful webhook deliveries")
    )
    failed_deliveries = models.IntegerField(
        default=0,
        help_text=_("Failed webhook deliveries")
    )
...
class LicensePool(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        help_text=_("Descriptive name for this license pool")
    )
    description = models.TextField(
        blank=True,
        help_text=_("Internal notes about this pool")
    )
    ...
    total_keys = models.IntegerField(
        default=100,
        help_text=_("Total number of keys to generate")
    )

    keys_generated = models.IntegerField(
        default=0,
        help_text=_("Number of keys successfully generated")
    )

    keys_distributed = models.IntegerField(
        default=0,
        help_text=_("Number of keys that have been distributed/used")
    )
    ...
    max_activations = models.IntegerField(
        default=1,
        help_text=_("Maximum device activations per key")
    )
...
class CustomizationOption(models.Model):
    ...
    description = models.TextField(
        ...
        help_text=_("Help text shown to customers")
    )
    ...
    def validate_value(self, value):
        ...
        if self.option_type in ('text', 'textarea'):
            if self.max_length and len(str(value)) > self.max_length:
                return False, _("Text exceeds maximum length of {max} characters").format(max=self.max_length)
...
class CustomizationValue(models.Model):
    ...
    text_value = models.TextField(
        ...
        help_text=_("Text/textarea input value")
    )
...
class GiftCard(models.Model):
    ...
    recipient_email = models.EmailField(
        ...
        help_text=_("Email address to send gift card to")
    )
    ...
    issued_at = models.DateTimeField(
        ...
        blank=True,
        verbose_name=_("Issued At"),
        help_text=_("When sent to recipient")
    )
    ...
    first_used_at = models.DateTimeField(
        ...
        help_text=_("When first redeemed")
    )
    ...
    def is_valid(self):
        ...
        return (
            ...
            not self.is_fully_redeemed and
    ...
    def can_redeem(self, amount):
        ...
        if isinstance(amount, Decimal) or isinstance(amount, (int, float)):
        ...
        if amount.currency.code != self.current_balance.currency.code:
            return False, _("Currency mismatch. Gift card is in {card_currency}, order is in {order_currency}").format(
...
class ConfigurationSlot(models.Model):
    ...
    max_selections = models.PositiveIntegerField(
        ...
        help_text=_("Maximum options allowed. Set >1 for multi-select (e.g., add-ons, extra storage).")
    )
...
class ConfigurationSlotOption(models.Model):
    ...
    price_adjustment = MoneyField(
        ...
        help_text=_("For 'Base + Adjustments' pricing: amount added to base price when selected. "
...
class CompatibilityRule(models.Model):
    ...
    compatible_options = models.ManyToManyField(
        ...
        help_text=_("Options in target slot affected by this rule. "
    ...
    def clean(self):
        ...
        if self.source_option_id and self.target_slot_id:
            if self.source_option.slot == self.target_slot:
                raise ValidationError({
                    'target_slot': _("Target slot must be different from the source option's slot.")
                })
...
class ConfigurationPreset(models.Model):
    ...
    is_featured = models.BooleanField(
        default=False,
        verbose_name=_("Featured Preset")
    )
    sort_order = models.IntegerField(default=0, verbose_name=_("Sort Order"))
    ...
    def get_total_price(self):
        ...
        for slot_id_str, option_ids in self.selections.items():
...
class BookingResourceImage(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.is_primary:
            BookingResourceImage.objects.filter(
