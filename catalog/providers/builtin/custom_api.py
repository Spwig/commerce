...
class CustomAPIAdapter(BaseLicenseProviderAdapter):
    ...
    def _get_endpoint(self, operation: str, **kwargs) -> str:
        ...
        endpoint = endpoint_mapping.get(operation, "")
    ...
    def create_license(self, license_key, product, order) -> tuple[bool, str, dict]:
        ...
        if success:
            ...
            external_id = (
                response.get("id")
                or response.get("license_id")
                or response.get("external_id")
                or license_key.key
            )
            logger.info(
                f"Successfully created license {license_key.key} in custom API (ID: {external_id})"
            )
    ...
    def validate_license(self, key: str) -> tuple[bool, dict]:
        ...
        logger.error("Failed to validate license in custom API")
        return False, response
    ...
    def activate_device(
        ...
        if success:
            ...
            activation_id = (
                response.get("activation_id") or response.get("id") or device_fingerprint
            )
            logger.info(
                f"Successfully activated device {device_fingerprint} for license {license_key.key}"
            )
    ...
    def deactivate_device(self, license_key, device_fingerprint: str) -> tuple[bool, dict]:
        ...
        if success:
            logger.info(
                f"Successfully deactivated device {device_fingerprint} for license {license_key.key}"
            )
            return True, response
    ...
    def test_connection(self) -> dict:
        ...
        try:
            ...
            success, response = self._make_request(
                "GET", test_endpoint, headers=self._get_auth_headers()
            )
            ...
            logger.exception("Connection test failed for custom API")
