...
class CryptlexAdapter(BaseLicenseProviderAdapter):
    ...
    def create_license(self, license_key, product, order) -> tuple[bool, str, dict]:
        ...
        if success:
            ...
            logger.info(
                f"Successfully created license {license_key.key} in Cryptlex (ID: {external_id})"
            )
    ...
    def validate_license(self, key: str) -> tuple[bool, dict]:
        ...
        if success:
            ...
            is_valid = (
                not response.get("revoked", False) and response.get("suspended", False) is False
            )
        ...
        logger.error("Failed to validate license in Cryptlex")
        return False, response
    ...
    def activate_device(
        ...
        if success:
            ...
            logger.info(
                f"Successfully activated device {device_fingerprint} in Cryptlex (ID: {activation_id})"
            )
    ...
    def test_connection(self) -> dict:
        ...
        try:
            ...
            logger.exception("Connection test failed for Cryptlex")
