...
class TestAdminPageHealth:
    ...
    def test_all_admin_pages_return_200(self):
        ...
        assert not errors, f"{len(errors)} admin pages returned HTTP errors:\n" + "\n".join(
            f"  [{r.status_code}] {r.label}: {r.url}" for r in errors
        )

    def test_no_admin_console_errors(self):
        """No JavaScript console errors on admin pages."""
        pages = self.report.error_pages
        assert not pages, f"{len(pages)} admin pages have console errors:\n" + "\n".join(
    ...
    def test_no_admin_404_resources(self):
        ...
        assert not pages, f"{len(pages)} admin pages have missing resources:\n" + "\n".join(
    ...
    def test_no_slow_admin_pages(self):
        ...
        assert not slow, f"{len(slow)} admin pages are slow:\n" + "\n".join(
    ...
    def test_no_navigation_failures(self):
        ...
        assert not failures, f"{len(failures)} admin pages failed to load:\n" + "\n".join(
