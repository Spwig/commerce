...
from tests.factories import (
    ProductFactory, CategoryFactory, ProductVariantFactory,
)
...
def bundle_product_scenario():
    ...
    comp_a = ProductFactory(
        name='Bundle Component A', slug='bundle-comp-a',
        category=category, price=Decimal('15.00'), status='published',
    )
    comp_b = ProductFactory(
        name='Bundle Component B', slug='bundle-comp-b',
        category=category, price=Decimal('20.00'), status='published',
    )
    bundle = ProductFactory(
        name='Starter Bundle', slug='starter-bundle',
        category=category, price=Decimal('30.00'),
        product_type='bundle', status='published',
        bundle_pricing_strategy='fixed',
    )
    BundleItem.objects.create(
        bundle=bundle, component_product=comp_a, quantity=1, sort_order=0,
    )
    BundleItem.objects.create(
        bundle=bundle, component_product=comp_b, quantity=2, sort_order=1,
    )
    return {
        'bundle': bundle,
...
def gift_card_product_scenario():
    ...
    product = ProductFactory(
        name='Store Gift Card', slug='store-gift-card',
        category=category, price=Decimal('50.00'),
        product_type='gift_card', status='published',
    )
...
def customizable_product_scenario():
    ...
    product = ProductFactory(
        name='Personalized Mug', slug='personalized-mug',
        category=category, price=Decimal('19.99'),
        product_type='customizable', status='published',
        allow_customization=True,
    )
    option = CustomizationOption.objects.create(
        product=product, name='Engraving Text', slug='engraving-text',
        option_type='text', is_required=True,
        sort_order=0,
    )
...
def configurable_product_scenario():
    ...
    product = ProductFactory(
        ...
        category=category, price=Decimal('999.99'),
        product_type='configurable', status='published',
        configurator_pricing_strategy='components_sum',
    )
    cpu_slot = ConfigurationSlot.objects.create(
        product=product, name='Processor', slug='processor',
        is_required=True, min_selections=1, max_selections=1, sort_order=0,
    )
    ram_slot = ConfigurationSlot.objects.create(
        product=product, name='Memory', slug='memory',
        is_required=True, min_selections=1, max_selections=1, sort_order=1,
    )
    # Option products
    cpu_option = ProductFactory(
        name='Intel i7', slug='intel-i7', category=category,
        price=Decimal('350.00'), status='published',
    )
    ram_option = ProductFactory(
        name='16GB RAM', slug='16gb-ram', category=category,
        price=Decimal('120.00'), status='published',
    )
    ConfigurationSlotOption.objects.create(
        slot=cpu_slot, option_product=cpu_option, sort_order=0,
    )
    ConfigurationSlotOption.objects.create(
        slot=ram_slot, option_product=ram_option, sort_order=0,
    )
    return {
        'product': product,
