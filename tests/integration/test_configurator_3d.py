...
class TestNodeMappingModel:
    ...
    def test_ordering(self, scene_config, slot_and_option):
        ...
        from configurator_3d.models import NodeMapping
        _, option = slot_and_option
        m1 = NodeMappingFactory(scene_config=scene_config, slot_option=option,
                                target_node='A', sort_order=2)
        m2 = NodeMappingFactory(scene_config=scene_config, slot_option=option,
                                target_node='B', sort_order=1)
        m3 = NodeMappingFactory(scene_config=scene_config, slot_option=option,
                                target_node='C', sort_order=1)
...
class TestIDORProtection:
    ...
    def test_delete_geometry_asset_wrong_product_returns_404(self, staff_client,
                                                              geometry_asset,
                                                              configurable_product):
        """Deleting a geometry asset using a different product_id returns 404."""
        other_product = ProductFactory(product_type='configurable')
        url = _url('delete_geometry_asset',
                    product_id=other_product.pk,
                    asset_id=geometry_asset.pk)
        resp = staff_client.post(url)
        assert resp.status_code == 404

    def test_delete_geometry_asset_correct_product_succeeds(self, staff_client,
                                                             geometry_asset,
                                                             configurable_product):
    ...
    def test_delete_texture_asset_wrong_product_returns_404(self, staff_client,
                                                             texture_asset,
                                                             configurable_product):
        """Deleting a texture asset using a different product_id returns 404."""
        other_product = ProductFactory(product_type='configurable')
        url = _url('delete_texture_asset',
                    product_id=other_product.pk,
                    asset_id=texture_asset.pk)
        resp = staff_client.post(url)
        assert resp.status_code == 404

    def test_delete_texture_asset_correct_product_succeeds(self, staff_client,
                                                            texture_asset,
                                                            configurable_product):
...
class TestSceneSetupView:
    ...
    def test_scene_setup_creates_scene_if_not_exists(self, staff_client,
                                                      configurable_product,
                                                      site_settings):
    ...
    def test_scene_setup_404_for_simple_product(self, staff_client,
                                                  simple_product_non_configurable):
    ...
    def test_scene_setup_context_has_json_keys(self, staff_client,
                                                configurable_product,
                                                scene_config,
                                                slot_and_option,
                                                mapping,
        ...
        assert resp.status_code == 200
        ctx = resp.context
        for key in ['slots_json', 'mappings_json', 'geometry_json',
                     'texture_json', 'scene_json']:
            assert key in ctx, f'Missing context key: {key}'
            # Verify it is valid JSON
            parsed = json.loads(ctx[key])
            assert parsed is not None

    def test_scene_setup_slots_json_structure(self, staff_client,
                                               configurable_product,
                                               slot_and_option,
                                               site_settings):
...
class TestSaveSceneConfigView:
    ...
    def test_save_scene_config_partial_update(self, staff_client, configurable_product):
        ...
        url = _url('save_scene_config', product_id=configurable_product.pk)
        resp = staff_client.post(
            url, data=json.dumps({'exposure': 3.0}),
            content_type='application/json')
        assert resp.status_code == 200
    ...
    def test_save_scene_config_with_environment_image(self, staff_client,
                                                       configurable_product):
    ...
    def test_save_scene_config_clear_environment_image(self, staff_client, scene_config,
                                                        configurable_product):
...
class TestListMappingsView:
    ...
    def test_list_mappings_with_data(self, staff_client, configurable_product,
...
class TestSaveMappingView:
    ...
    def test_create_mapping(self, staff_client, configurable_product,
                            slot_and_option):
    ...
    def test_update_mapping(self, staff_client, configurable_product,
                            mapping):
    ...
    def test_create_mapping_missing_fields(self, staff_client, configurable_product):
        ...
        url = _url('save_mapping', product_id=configurable_product.pk)
        resp = staff_client.post(
            url, data=json.dumps({'action_type': 'visibility'}),
            content_type='application/json')
        assert resp.status_code == 400

    def test_create_mapping_invalid_action_type(self, staff_client,
                                                 configurable_product,
                                                 slot_and_option):
    ...
    def test_update_mapping_wrong_scene_returns_404(self, staff_client,
                                                     configurable_product,
                                                     slot_and_option):
        ...
        other_mapping = NodeMappingFactory(
            scene_config=other_scene, slot_option=option,
            target_node='X', action_type='visibility')
...
class TestDeleteMappingView:
    ...
    def test_delete_mapping_nonexistent_returns_404(self, staff_client,
                                                     configurable_product):
    ...
    def test_delete_mapping_requires_post(self, staff_client, configurable_product,
                                           mapping):
...
class TestSaveGeometryAssetView:
    ...
    @patch('configurator_3d.admin_views.parse_glb_from_media_asset')
    def test_create_geometry_asset(self, mock_parse, staff_client,
                                    configurable_product):
    ...
    @patch('configurator_3d.admin_views.parse_glb_from_media_asset')
    def test_update_geometry_asset(self, mock_parse, staff_client,
                                    configurable_product, geometry_asset):
    ...
    def test_create_geometry_asset_missing_media_id(self, staff_client,
                                                     configurable_product):
        """save_geometry_asset returns 400 when media_asset_id is missing."""
        url = _url('save_geometry_asset', product_id=configurable_product.pk)
        resp = staff_client.post(
            url, data=json.dumps({'label': 'Test'}),
            content_type='application/json')
        assert resp.status_code == 400

    def test_create_geometry_asset_nonexistent_media(self, staff_client,
                                                      configurable_product):
    ...
    @patch('configurator_3d.admin_views.parse_glb_from_media_asset')
    def test_update_geometry_asset_wrong_scene_returns_404(self, mock_parse,
                                                            staff_client,
                                                            configurable_product):
...
class TestDeleteGeometryAssetView:
    ...
    def test_delete_geometry_asset_success(self, staff_client, configurable_product,
                                            geometry_asset):
    ...
    def test_delete_geometry_asset_nonexistent_returns_404(self, staff_client,
                                                            configurable_product):
    ...
    def test_delete_geometry_asset_requires_post(self, staff_client,
                                                  configurable_product,
                                                  geometry_asset):
...
class TestListTexturesView:
    ...
    def test_list_textures_with_data(self, staff_client, configurable_product,
                                      scene_config, texture_asset):
...
class TestSaveTextureAssetView:
    ...
    def test_update_texture_asset(self, staff_client, configurable_product,
                                   texture_asset):
    ...
    def test_create_texture_asset_missing_media_id(self, staff_client,
                                                     configurable_product):
        """save_texture_asset returns 400 when media_asset_id is missing."""
        url = _url('save_texture_asset', product_id=configurable_product.pk)
        resp = staff_client.post(
            url, data=json.dumps({'label': 'No Media'}),
            content_type='application/json')
        assert resp.status_code == 400

    def test_create_texture_asset_invalid_type(self, staff_client,
                                                configurable_product):
    ...
    def test_create_texture_asset_nonexistent_media(self, staff_client,
                                                      configurable_product):
    ...
    def test_update_texture_asset_wrong_scene_returns_404(self, staff_client,
                                                           configurable_product):
    ...
    def test_save_texture_asset_default_type_is_base_color(self, staff_client,
                                                            configurable_product):
...
class TestDeleteTextureAssetView:
    ...
    def test_delete_texture_asset_success(self, staff_client, configurable_product,
                                           texture_asset):
    ...
    def test_delete_texture_asset_nonexistent_returns_404(self, staff_client,
                                                           configurable_product):
    ...
    def test_delete_texture_asset_requires_post(self, staff_client,
                                                 configurable_product,
                                                 texture_asset):
...
class TestCaptureThumbnailView:
    ...
    def test_capture_thumbnail_success(self, staff_client,
                                        configurable_product, admin_user):
    ...
    def test_capture_thumbnail_missing_image_data(self, staff_client,
                                                    configurable_product):
    ...
    def test_capture_thumbnail_size_limit(self, staff_client,
                                           configurable_product, settings):
    ...
    def test_capture_thumbnail_strips_data_url_prefix(self, staff_client,
                                                       configurable_product):
        ...
        with patch('PIL.Image.open') as mock_open:
            ...
            resp = staff_client.post(
                url, data=json.dumps(payload_with_prefix),
                content_type='application/json')
            assert resp.status_code == 200

    def test_capture_thumbnail_404_for_simple_product(self, staff_client,
                                                       simple_product_non_configurable):
        """capture_thumbnail returns 404 for non-configurable products."""
        url = _url('capture_thumbnail',
                    product_id=simple_product_non_configurable.pk)
        resp = staff_client.post(
            url, data=json.dumps({'image_data': 'test'}),
            content_type='application/json')
        assert resp.status_code == 404
...
class TestSerializeScene3D:
    ...
    def test_serialize_returns_none_for_scene_without_base_model(self,
                                                                   configurable_product):
    ...
    def test_serialize_full_scene_structure(self, scene_config, mapping,
                                             geometry_asset, texture_asset):
    ...
    def test_serialize_multiple_mappings_same_option(self, scene_config, slot_and_option):
        ...
        from configurator_3d.serializers import serialize_scene_3d
        _, option = slot_and_option
        NodeMappingFactory(scene_config=scene_config, slot_option=option,
                           action_type='material_color', target_node='Body')
        NodeMappingFactory(scene_config=scene_config, slot_option=option,
                           action_type='visibility', target_node='Lid')

        result = serialize_scene_3d(scene_config)
        option_id_str = str(option.pk)
        assert len(result['mappings'][option_id_str]) == 2

    def test_serialize_geometry_assets_keyed_by_media_id(self, scene_config,
                                                          geometry_asset):
...
class TestGlbParserService:
    ...
    def test_parse_glb_missing_pygltflib(self):
        ...
        from configurator_3d.services.glb_parser import parse_glb
