...
class TestCustomFieldsMixin:
    ...
    def test_get_custom_field_value_returns_stored_value(
        self, site_settings, category, text_field
    ):
    ...
    def test_get_custom_field_value_returns_none_for_unknown_slug(
        self, site_settings, category
    ):
    ...
    def test_json_field_queryable(self, site_settings, category, text_field):
        ...
        results = Product.objects.filter(
            custom_fields__external_id='EXT-100'
        )
...
class TestCustomFieldModels:
    ...
    def test_same_slug_different_content_type(
        self, field_group, product_ct, category_ct
    ):
...
class TestCustomFieldsAPI:
    ...
    def test_list_definitions_authenticated(
        self, admin_client, text_field, number_field
    ):
    ...
    def test_filter_by_model(
        self, admin_client, text_field, category_ct
    ):
    ...
    def test_definition_detail(self, admin_client, text_field):
        ...
        resp = admin_client.get(
            f'/api/custom-fields/definitions/{text_field.pk}/'
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_definition_includes_choices_for_select(
        self, admin_client, select_field
    ):
        """Select field definitions include choices in the response."""
        resp = admin_client.get(
            f'/api/custom-fields/definitions/{select_field.pk}/'
        )
        data = resp.json()
...
class TestAdminManagement:
    ...
    def test_create_field_via_ajax(self, admin_user, field_group, product_ct):
        ...
        data = resp.json()
        assert data['success'] is True
        assert CustomFieldDefinition.objects.filter(
            slug='test_ajax_field'
        ).exists()

    def test_delete_group_soft_deletes(self, admin_user, field_group):
        """Deleting a group sets is_active=False."""
        from django.test import Client
        client = Client()
        client.force_login(admin_user)
        resp = client.post(
        ...
        assert resp.status_code == 200
        field_group.refresh_from_db()
        assert field_group.is_active is False
