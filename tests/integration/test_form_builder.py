...
class TestRecycleBin:
    ...
    def test_restore_form(self, staff_client, form, admin_user):
        ...
        resp = staff_client.post(
            "/en/admin/form_builder/forms/recycle-bin/",
            {
                "action": "restore",
                "form_ids": [form.pk],
            },
...
class TestFormAdminActions:
    ...
    def test_duplicate_form(self, staff_client, form_with_fields):
        resp = staff_client.post(f"/en/admin/form_builder/forms/{form_with_fields.pk}/duplicate/")
        assert resp.status_code == 302
    ...
    def test_export_responses(self, staff_client, form_with_fields):
        ...
        resp = staff_client.get(f"/en/admin/form_builder/responses/{form_with_fields.pk}/export/")
        assert resp.status_code == 200
...
class TestFormActionsExecution:
    ...
    def test_email_notification_action(self, mock_send, form_with_fields):
        # EmailNotificationAction lazy-imports EmailSendingService inside its
        # execute() method, so patch at the source module.
        mock_send.return_value = MagicMock(status="queued")
