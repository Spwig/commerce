...
class TestConvertGuestToFullAccount:
    def test_successful_conversion(self):
        ...
        Service now stores the full email (lowercased) as the username to avoid
        collisions with real usernames.
        """
        from accounts.services.account_creation_service import AccountCreationService
        ...
        assert guest.username == "convert@example.com"  # full email as username
        assert guest.has_usable_password()
        assert guest.check_password("securepass123")

    def test_username_uses_full_email_lowercased(self):
        """Even when a stem-username collides, service uses full email lowercased.

        The old collision-counter behavior was replaced by using the full email
        address so no name-stem collision is possible.
        """
        from accounts.services.account_creation_service import AccountCreationService

        # Occupy the stem-only 'taken' username with a real user
        UserFactory(username="taken", email="other@example.com")

        guest = _make_guest("Taken@Example.com")

        success, _msg = AccountCreationService.convert_guest_to_full_account(
            user=guest,
        ...
        guest.refresh_from_db()
        assert success is True
        # Full email, lowercased, becomes the username
        assert guest.username == "taken@example.com"
...
class TestActivateGuestAccountView:
    ...
    def test_invalid_token_redirects(self, mock_redirect, django_site, guest_view_settings):
        ...
        client = Client()
        url = reverse(
            "accounts:activate_guest_account",
            kwargs={
                "uidb64": uid,
                "token": "invalid-token",
            },
        ...
        mock_redirect.assert_called_with("page_builder:home")
...
class TestGuestOrderLookupView:
    ...
    def test_post_with_matching_guest_sends_email(
        self, mock_email_service, django_site, guest_view_settings
    ):
    ...
    @patch("email_system.services.email_sender.EmailSendingService")
    def test_post_guest_without_orders_does_not_send(
        self, mock_email_service, django_site, guest_view_settings
    ):
...
class TestOrderConfirmationSignal:
    ...
    @patch("email_system.services.email_sender.EmailSendingService")
    def test_guest_order_includes_activation_url(
        self, mock_email_service, django_site, site_settings
    ):
        """Order confirmation for a guest order includes activation_url in context.

        transaction=True is required because the signal uses transaction.on_commit
        which only fires after the outer transaction commits.
        """
        guest = _make_guest("signal@example.com")

        OrderFactory(user=guest, email="signal@example.com")

        # transaction.on_commit fires now
    ...
    @pytest.mark.django_db(transaction=True)
    @patch("email_system.services.email_sender.EmailSendingService")
    def test_registered_order_no_activation_url(
        self, mock_email_service, django_site, site_settings
    ):
        ...
        OrderFactory(user=registered, email="siguser@example.com")
...
class TestAccountTypeDisplay:
    def test_guest_badge(self):
        ...
        Badge markup was refactored to use CSS classes (CSP-safe) instead of inline colors.
        """
        from accounts.models import CustomerProfile
        ...
        # CSS class-based styling replaces inline color hex
        assert "customer-badge-guest" in html_str
    ...
    def test_registered_badge(self):
        ...
        html_str = str(html)
        assert "Registered" in html_str
        # CSS class-based styling replaces inline color hex
        assert "customer-badge-registered" in html_str
...
class TestMergeDuplicateGuestsCommand:
    ...
    def test_execute_actually_merges(self):
        ...
        remaining = User.objects.filter(
            email__iexact="exec@example.com", username__startswith="guest_"
        )
...
class TestDashboardUnreadReplyCount:
    def test_dashboard_includes_unread_reply_count(
        self, customer_client, customer_user, site_settings
    ):
