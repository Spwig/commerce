...
class TestCategoryRouting:
    ...
    def test_category_list_uses_correct_template(self, client, site_settings):
        resp = client.get("/en/category/")
        # "default" template alias resolves to the grid template.
        assert "page_builder/category/grid.html" in [t.name for t in resp.templates]
...
class TestCardImageFallback:
    ...
    def test_product_fallback_shows_product_image(
        self, client, site_settings, category_with_products
    ):
    ...
    def test_dedicated_image_used_in_placeholder_mode(
        self, client, site_settings, category_with_image
    ):
    ...
    def test_empty_category_shows_placeholder_in_product_mode(
        self, client, site_settings, empty_category
    ):
...
class TestCategoryPagination:
    ...
    def test_invalid_page_string_falls_back_to_1(
        self, client, site_settings, many_products_category
    ):
...
class TestProductFiltering:
    ...
    def test_descendant_category_products_included(
        self, client, site_settings, parent_category, child_category
    ):
...
class TestSubcategories:
    ...
    def test_subcategories_shown(
        self, client, site_settings, parent_category, child_category, another_child
    ):
        resp = client.get(f"/en/category/{parent_category.slug}/")
        content = resp.content.decode()
    ...
    def test_subcategories_hidden_when_disabled(
        self, client, site_settings, parent_category, child_category
    ):
    ...
    def test_category_list_shows_only_top_level(
        self, client, site_settings, parent_category, child_category
    ):
...
class TestTemplateRegistry:
    ...
    def test_get_category_options_returns_all_keys(self):
        from design.template_registry import CATEGORY_TEMPLATE_OPTIONS, get_category_options

        # "default" is a backward-compat alias for "grid" in the registry.
        opts = get_category_options("default")
        schema = CATEGORY_TEMPLATE_OPTIONS["grid"]
    ...
    def test_get_category_template_path_default(self):
        ...
        # "default" is an alias that resolves to the grid template.
        assert get_category_template_path("default") == "page_builder/category/grid.html"

    def test_get_category_template_path_fallback(self):
        from design.template_registry import get_category_template_path

        assert get_category_template_path("nonexistent") == "page_builder/category/grid.html"

    def test_category_template_meta_exists(self):
        from design.template_registry import CATEGORY_TEMPLATE_META

        # Registry uses "grid" as the canonical key; "default" is a legacy alias.
        assert "grid" in CATEGORY_TEMPLATE_META
        assert "name" in CATEGORY_TEMPLATE_META["grid"]
        assert "icon" in CATEGORY_TEMPLATE_META["grid"]
...
class TestDisplayOptions:
    ...
    def test_category_description_hidden_when_disabled(
        self, client, site_settings, parent_category
    ):
    ...
    def test_product_count_hidden_when_disabled(
        self, client, site_settings, category_with_products
    ):
...
class TestEdgeCases:
    ...
    def test_multiple_sort_params_uses_last(self, client, site_settings, category_with_products):
        ...
        resp = client.get(
            f"/en/category/{category_with_products.slug}/?sort=price_low&sort=name_az"
        )
