...
from tests.factories import (
    ...
    SalesRegionFactory, WarehouseFactory, OrderFactory, OrderItemFactory,
    CategoryFactory, ShippingMethodFactory, ShippingZoneFactory,
)
...
class TestWarehouseAllocation:
    ...
    def test_digital_product_allocated_anywhere(self, fulfillment_svc, merchant):
        ...
        digital = ProductFactory(
            name='E-Book', slug='e-book', digital=True, track_inventory=False,
        )
        product_a = merchant['products']['a']
...
class TestVariantStockFiltering:
    ...
    def test_check_availability_filters_by_variant(self, fulfillment_svc, merchant):
        ...
        result_red = fulfillment_svc.check_stock_availability(
            variable, 10, region=us_region, variant=variant_red,
        )
        result_blue = fulfillment_svc.check_stock_availability(
            variable, 10, region=us_region, variant=variant_blue,
        )
    ...
    def test_allocate_stock_with_variant(self, fulfillment_svc, merchant):
        ...
        order = OrderFactory(shipping_country='US')
        oi = OrderItemFactory(
            order=order, product=variable, variant=variant_red,
            quantity=5, product_name='T-Shirt Red', sku='TSHIRT-RED',
        )
    ...
    def test_allocate_stock_no_variant_simple_product(self, fulfillment_svc, merchant):
        ...
        order = OrderFactory(shipping_country='US')
        oi = OrderItemFactory(
            order=order, product=product_a,
            quantity=3, product_name='Widget A', sku=product_a.sku,
        )
    ...
    def test_allocate_stock_wrong_variant_raises(self, fulfillment_svc, merchant):
        ...
        order = OrderFactory(shipping_country='US')
        oi = OrderItemFactory(
            order=order, product=variable, variant=variant_blue,
            quantity=1, product_name='T-Shirt Blue', sku='TSHIRT-BLUE',
        )
    ...
    def test_fulfill_stock_with_variant(self, fulfillment_svc, merchant):
        ...
        StockItem.objects.filter(
            product=variable, warehouse=wh_us, variant=variant_red,
        ).update(allocated=10)

        order = OrderFactory(shipping_country='US')
        oi = OrderItemFactory(
            order=order, product=variable, variant=variant_red,
            quantity=10, product_name='T-Shirt Red', sku='TSHIRT-RED',
        )
...
class TestCurrencyHandling:
    ...
    def test_calculate_shipping_uses_method_currency(self, site_settings):
        ...
        from shipping.services.rule_service import ShippingRuleService
        ...
        currency = ShippingRuleService._resolve_currency(method)
    ...
    def test_apply_rules_to_cost_uses_correct_currency(self, site_settings):
        ...
        result = ShippingRuleService.apply_rules_to_cost(
