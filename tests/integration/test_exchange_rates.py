...
from tests.factories import ComponentRegistryFactory, UserFactory
...
class TestExchangeRateProviderAccountModel:
    ...
    def test_ordering(self, site, exchange_rate_component, exchange_rate_component_2):
        ...
        p1 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component,
            name='Zebra', credentials=b'd', is_primary=False, is_active=True, priority=5,
        )
        p2 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component_2,
            name='Alpha', credentials=b'd', is_primary=True, is_active=True, priority=10,
        )
        p3 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component,
            name='Beta', credentials=b'd', is_primary=False, is_active=False, priority=0,
        )
    ...
    def test_unique_primary_constraint(self, site, exchange_rate_component, exchange_rate_component_2):
        """Only one provider per site can be primary."""
        ExchangeRateProviderAccount.objects.all().delete()

        ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component,
            name='First Primary', credentials=b'd', is_primary=True,
        )
        ...
        with pytest.raises(IntegrityError):
            with transaction.atomic():
                ExchangeRateProviderAccount.objects.create(
                    site=site, component=exchange_rate_component_2,
                    name='Second Primary', credentials=b'd', is_primary=True,
                )

    def test_multiple_non_primary_allowed(self, site, exchange_rate_component, exchange_rate_component_2):
        """Multiple non-primary providers for the same site are allowed."""
        ExchangeRateProviderAccount.objects.all().delete()

        p1 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component,
            name='Non-Primary 1', credentials=b'd', is_primary=False,
        )
        p2 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component_2,
            name='Non-Primary 2', credentials=b'd', is_primary=False,
        )
...
class TestExchangeRateHistoryModel:
    ...
    def test_ordering(self, db):
        ...
        h1 = ExchangeRateHistory.objects.create(
            ...
            rate=Decimal('0.85'), provider_name='P1',
        )
        h2 = ExchangeRateHistory.objects.create(
            base_currency='USD', target_currency='EUR',
            rate=Decimal('0.86'), provider_name='P2',
        )
    ...
    def test_related_name_exchange_rate_snapshots(self, db):
        ...
        h = ExchangeRateHistory.objects.create(
            base_currency='EUR', target_currency='JPY',
            rate=Decimal('162.30'), provider_name='Fixer',
            order=order,
        )
...
class TestExchangeRateProviderAccountAdmin:
    ...
    def test_display_name_or_component_with_name(self, provider_account):
        ...
        admin_obj = ExchangeRateProviderAccountAdmin(
            ExchangeRateProviderAccount, AdminSite()
        )
...
class TestFilterExchangeRateProviders:
    ...
    @patch('exchange_rates.views.admin_views.render_to_string', return_value='<div>card</div>')
    def test_count_matches_providers(self, mock_render, staff_client, provider_account, primary_provider):
    ...
    @patch('exchange_rates.views.admin_views.render_to_string', return_value='<div>card</div>')
    def test_filter_by_sync_status(self, mock_render, staff_client, provider_account, primary_provider):
    ...
    @patch('exchange_rates.views.admin_views.render_to_string', return_value='<div>card</div>')
    def test_filter_by_component(self, mock_render, staff_client, provider_account, primary_provider):
...
class TestProviderBulkAction:
    ...
    def test_bulk_set_primary_multiple_fails(self, staff_client, provider_account, primary_provider):
        """Bulk set_primary fails when multiple providers selected."""
        response = self._post_bulk(
            staff_client, 'set_primary',
...
class TestCSPCompliance:
    ...
    def test_no_inline_style_attributes(self):
        ...
        assert violations == [], (
            f"Inline style= attributes found in templates: {violations}"
        )
    ...
    def test_no_onclick_handlers_in_templates(self):
        ...
        assert violations == [], (
            f"onclick handlers found in templates: {violations}"
        )
    ...
    def test_no_onclick_handlers_in_js_files(self):
        ...
        assert violations == [], (
            f"onclick handlers found in JS files: {violations}"
        )
...
class TestStaticFileCopyright:
    ...
    def test_css_files_have_copyright(self):
        ...
        for path in css_files:
            ...
            if 'Spwig' not in first_line or '\u00a9' not in first_line:
                missing.append(str(path.relative_to(PROJECT_ROOT)))

        assert missing == [], (
            f"CSS files missing copyright header: {missing}"
        )

    def test_js_files_have_copyright(self):
        """All JS files start with copyright header."""
        ...
        assert missing == [], (
            f"JS files missing copyright header: {missing}"
        )
...
class TestI18nCompliance:
    ...
    def test_provider_account_field_verbose_names(self):
        ...
        field_names = [
            'site', 'component', 'name', 'credentials', 'is_active',
            'is_primary', 'priority', 'settings', 'last_sync_at',
    ...
    def test_history_field_verbose_names(self):
        ...
        field_names = [
            ...
            'provider_name', 'created_at', 'order',
        ]
        for field_name in field_names:
...
class TestEdgeCases:
    ...
    def test_provider_with_empty_settings(self, site, exchange_rate_component):
        ...
        acct = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component,
            name='Empty Settings', credentials=b'd',
        )
        assert acct.settings == {}
    ...
    def test_multiple_rates_different_providers(
        ...
        p1 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component,
            name='P1', credentials=b'd',
        )
        p2 = ExchangeRateProviderAccount.objects.create(
            site=site, component=exchange_rate_component_2,
            name='P2', credentials=b'd',
        )
        r1 = ExchangeRate.objects.create(
            provider_account=p1, base_currency='USD',
            target_currency='EUR', rate=Decimal('0.85'),
        )
        r2 = ExchangeRate.objects.create(
            provider_account=p2, base_currency='USD',
            target_currency='EUR', rate=Decimal('0.86'),
        )
        assert r1.pk is not None
        assert r2.pk is not None
    ...
    def test_history_without_order(self, db):
        ...
        h = ExchangeRateHistory.objects.create(
            base_currency='AUD', target_currency='NZD',
            rate=Decimal('1.07'), provider_name='Test',
        )
