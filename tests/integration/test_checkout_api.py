...
class TestCheckoutSession:
    ...
    def test_session_recalculates_on_fetch(
        ...
        resp = auth_client.get("/api/checkout/")
        session = resp.json()["session"]
        assert Decimal(session["subtotal"]) == Decimal("25.00")
        ...
        assert Decimal(session["subtotal"]) == Decimal("75.00")
...
class TestShippingAddress:
    ...
    def test_address_change_clears_shipping_method(self, auth_client, cart_with_item):
        ...
        resp = auth_client.get("/api/checkout/")
        assert resp.json()["session"]["selected_shipping_method"] is not None
...
class TestShippingMethods:
    ...
    @pytest.mark.skip(
        reason="ShippingMethod.min_order_value and 'free_shipping' method_type were "
        "removed; threshold-gated free shipping is no longer a model-level feature."
    )
    ...
    def test_free_shipping_above_threshold(self, auth_client, cart_with_expensive_item):
...
class TestTotalsRecalculation:
    def test_address_change_resets_shipping_cost(self, auth_client, cart_with_item):
        ...
        resp = auth_client.get("/api/checkout/")
        assert Decimal(resp.json()["session"]["shipping_cost"]) == Decimal("12.00")
        ...
        resp = auth_client.get("/api/checkout/")
        assert Decimal(resp.json()["session"]["shipping_cost"]) == Decimal("0.00")
    ...
    def test_cart_mutation_syncs_session(
        ...
        # Create session
        resp = auth_client.get("/api/checkout/")
        assert Decimal(resp.json()["session"]["subtotal"]) == Decimal("25.00")
...
class TestCheckoutValidation:
    ...
    def test_validate_empty_cart(self, auth_client, customer_user, site_settings):
...
class TestCheckoutSecurity:
    def test_checkout_requires_auth(self, api_client, site_settings):
        ...
        site_settings.admin_email = (
            site_settings.admin_email or "admin@test.com"
        )  # Ensure admin_email is set
        site_settings.allow_guest_checkout = False
        site_settings.account_creation_timing = (
            "before_checkout"  # Compatible with no guest checkout
        )
        site_settings.save()
