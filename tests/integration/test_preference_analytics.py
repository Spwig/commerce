...
def test_get_action_cards_pending_verifications(users_with_varied_preferences):
    """Test pending verifications count"""
    cards = PreferenceAnalyticsService.get_action_cards()

    # Should match unverified users (marketing enabled but not verified)
...
def test_get_opt_in_metrics_calculations(users_with_varied_preferences):
    ...
    # SMS opted in: 2
    assert metrics['sms_opted_in']['count'] == 2
    assert metrics['sms_opted_in']['percentage'] == pytest.approx(13.33, rel=0.1)
...
def test_get_opt_in_metrics_with_comparison(users_with_varied_preferences):
    ...
    metrics = PreferenceAnalyticsService.get_opt_in_metrics(
        start_date, end_date, compare=True
    )
...
def test_get_app_preference_breakdown_counts():
    ...
    # Create users with specific app preferences
    for i in range(3):
        user = UserFactory()
        prefs = CommunicationPreference.get_or_create_for_user(user)[0]
    ...
    assert breakdown['blog'] == 3
    assert breakdown['loyalty'] == 2
