...
from tests.factories import (
    ProductFactory, CategoryFactory, StockItemFactory,
    WarehouseFactory, SalesRegionFactory,
)
...
class TestStockLevels:
    ...
    def test_only_terminal_warehouse(
        self, pos_client, product_with_stock, warehouse, category, site_settings,
    ):
...
class TestCrossLocationStock:
    ...
    def test_all_locations(
        self, pos_client, product_with_stock, warehouse, site_settings,
    ):
...
class TestAdjustStock:
    ...
    def test_adjust_stock_increase(
        self, pos_manager_client, product_with_stock, site_settings,
    ):
        """Increase stock by a positive amount via receive adjustment."""
        response = pos_manager_client.post(ADJUST_STOCK_URL, {
    ...
    def test_requires_permission(
        self, pos_client, product_with_stock, site_settings,
    ):
        """Cashier without pos_stock_adjustment permission gets 403."""
        response = pos_client.post(ADJUST_STOCK_URL, {
            'product_id': product_with_stock.id,
            'adjustment_type': 'receive',
            'quantity': 10,
...
class TestStockMovements:
    ...
    def test_list_movements(
        ...
        self._create_adjustment(
            pos_manager_client, product_with_stock, 'receive', 5, 'Restock',
        )
        self._create_adjustment(
            pos_manager_client, product_with_stock, 'damage', 2, 'Broken item',
        )
    ...
    def test_filter_by_product(
        self, pos_manager_client, product_with_stock, warehouse, category, site_settings,
    ):
        ...
        self._create_adjustment(
            pos_manager_client, product_with_stock, 'receive', 5, 'Restock main',
        )
        self._create_adjustment(
            pos_manager_client, other_product, 'receive', 3, 'Restock other',
        )

        response = pos_manager_client.get(
            MOVEMENTS_URL, {'product_id': product_with_stock.id},
        )
        data = assert_pos_success(response)
    ...
    def test_filter_by_date(
        ...
        self._create_adjustment(
            pos_manager_client, product_with_stock, 'receive', 10, 'Shipment',
        )
        self._create_adjustment(
            pos_manager_client, product_with_stock, 'damage', 2, 'Broken',
        )
    ...
    def test_includes_movement_details(
        ...
        self._create_adjustment(
            pos_manager_client, product_with_stock, 'receive', 7, 'Weekly restock',
        )
