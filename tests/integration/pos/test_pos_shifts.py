...
from pos_app.models import POSShift, CashMovement, POSPayment
...
class TestOpenShift:
    ...
    def test_blocks_different_cashier(
        self, pos_manager_client, open_shift, site_settings
    ):
...
class TestCashMovement:
    ...
    def test_cash_in(self, pos_manager_client, manager_open_shift, site_settings):
        ...
        response = pos_manager_client.post(CASH_MOVEMENT_URL, {
            'movement_type': 'in',
            'amount': '50.00',
            'reason': 'Float top-up',
        })
