...
class TestProductList:
    ...
    def test_search_by_barcode(
        self, pos_client, product_with_barcode, site_settings,
    ):
        """?q= filters by barcode."""
        data = assert_pos_success(
            pos_client.get(PRODUCTS_URL, {'q': '1234567890123'})
        )
    ...
    def test_filter_by_category(
        self, pos_client, category, product_with_stock, site_settings,
    ):
        ...
        data = assert_pos_success(
            pos_client.get(PRODUCTS_URL, {'category': category.id})
        )
    ...
    def test_pagination(self, pos_client, site_settings):
        ...
        data = assert_pos_success(
            pos_client.get(PRODUCTS_URL, {'page': 1, 'page_size': 2})
        )
...
class TestProductDetail:
    ...
    def test_simple_product_detail(
        ...
        data = assert_pos_success(
            pos_client.get(_product_detail_url(product_with_stock.id))
        )
    ...
    def test_includes_variants(
        ...
        data = assert_pos_success(
            pos_client.get(_product_detail_url(variable.id))
        )
...
class TestBarcodeLookup:
    ...
    def test_lookup_by_product_barcode(
        ...
        data = assert_pos_success(
            pos_client.get(_barcode_url('1234567890123'))
        )
    ...
    def test_lookup_by_sku_fallback(
        ...
        data = assert_pos_success(
            pos_client.get(_barcode_url('FALLBACK-SKU-42'))
        )
...
class TestCategoryList:
    ...
    def test_includes_parent_id(
        ...
        child_data = next(
            (c for c in data['categories'] if c['id'] == child.id), None
        )
        assert child_data is not None
        assert child_data['parent_id'] == category.id

        parent_data = next(
            (c for c in data['categories'] if c['id'] == category.id), None
        )
