...
from tests.factories import (
    ...
    UserFactory, OrderFactory, OrderItemFactory,
    POSTerminalFactory, POSShiftFactory,
)
...
class TestProductDeltaSync:
    ...
    def test_since_timestamp(
        self, pos_client, category, warehouse, site_settings,
    ):
        ...
        data = assert_pos_success(
            pos_client.get(SYNC_PRODUCTS_URL, {'since': since})
        )
    ...
    def test_all_without_since(
        self, pos_client, category, site_settings,
    ):
    ...
    def test_includes_stock(
        ...
        matching = [
            p for p in data['results'] if p['id'] == product_with_stock.id
        ]
    ...
    def test_pagination(self, pos_client, category, site_settings):
        ...
        data = assert_pos_success(
            pos_client.get(SYNC_PRODUCTS_URL, {'page': 1, 'page_size': 2})
        )
...
class TestCustomerSync:
    ...
    def test_since_timestamp(self, pos_client, site_settings):
        ...
        data = assert_pos_success(
            pos_client.get(SYNC_CUSTOMERS_URL, {'since': since})
        )
...
class TestUploadOfflineTransactions:
    ...
    def test_single_transaction(
        ...
        product_with_stock, open_shift, site_settings,
    ):
        """Upload one offline transaction creates an order."""
        from orders.models import Order

        txn = self._make_transaction(
            product_with_stock, pos_terminal, pos_staff_user,
        )
    ...
    def test_idempotency_by_local_id(
        ...
        assert Order.objects.filter(
            external_id='idempotent-txn-001', channel='pos'
        ).count() == 1
...
class TestOrderSync:
    ...
    def test_returns_recent_pos_orders(
        self, pos_client, pos_terminal, pos_staff_user,
        product_with_stock, site_settings,
    ):
        """Returns POS orders for offline caching."""
        order = self._create_pos_order(
            pos_terminal, pos_staff_user, product_with_stock,
        )
...
class TestStockAdjustmentSync:
    ...
    def test_upload_offline_adjustments(
        self, pos_manager_client, pos_terminal, warehouse,
        product_with_stock, site_settings,
    ):
        ...
        stock = StockItem.objects.get(
            product=product_with_stock, warehouse=warehouse,
        )
