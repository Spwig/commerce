...
from tests.factories import (
    OrderFactory, OrderItemFactory, ProductFactory, StockItemFactory,
    UserFactory,
)
from tests.helpers import assert_pos_success, assert_pos_error
...
def _create_pos_payment(order, shift, **kwargs):
    ...
    defaults = dict(
        ...
        shift=shift,
        method='cash',
        amount=order.total_amount.amount if hasattr(order.total_amount, 'amount') else order.total_amount,
    )
    defaults.update(kwargs)
...
class TestOrderList:
    ...
    def test_list_pos_orders(
        self, pos_client, pos_terminal, pos_staff_user, open_shift, site_settings,
    ):
    ...
    def test_filter_by_status(
        ...
        processing = _create_pos_order(
            pos_terminal, pos_staff_user, site_settings, status='processing',
        )
        _create_pos_payment(processing, open_shift)

        delivered = _create_pos_order(
            pos_terminal, pos_staff_user, site_settings, status='delivered',
        )
        _create_pos_payment(delivered, open_shift)

        data = assert_pos_success(
            pos_client.get(ORDERS_URL, {'status': 'processing'})
        )
    ...
    def test_filter_by_cashier(
        ...
        other_cashier = UserFactory(
            username='other_cashier', is_staff=True,
            first_name='Other', last_name='Cashier',
        )
        my_order = _create_pos_order(
            pos_terminal, pos_staff_user, site_settings,
        )
        _create_pos_payment(my_order, open_shift)

        other_order = _create_pos_order(
            pos_terminal, other_cashier, site_settings,
        )
        _create_pos_payment(other_order, open_shift)

        data = assert_pos_success(
            pos_client.get(ORDERS_URL, {'cashier_id': pos_staff_user.id})
        )
    ...
    def test_search_by_order_number(
        ...
        data = assert_pos_success(
            pos_client.get(ORDERS_URL, {'search': 'SRCH-9988'})
        )
    ...
    def test_pagination(
        ...
        data = assert_pos_success(
            pos_client.get(ORDERS_URL, {'page': 1, 'page_size': 2})
        )
...
class TestOrderReceipt:
    ...
    def _make_receipt_order(
        self, pos_terminal, pos_staff_user, open_shift, site_settings,
    ):
    ...
    def test_receipt_data(
        ...
        order = self._make_receipt_order(
            pos_terminal, pos_staff_user, open_shift, site_settings,
        )
...
class TestRefundOrder:
    ...
    def _make_refundable_order(
        self, pos_terminal, cashier, shift, site_settings,
        total=Decimal('50.00'),
    ):
        """Create a completed POS order ready for refunding."""
        order = _create_pos_order(
            pos_terminal, cashier, site_settings,
            status='processing',
            payment_status='paid',
            total_amount=total,
            subtotal=total,
    ...
    def test_full_refund(
        self, pos_manager_client, pos_terminal, pos_manager_user,
        manager_open_shift, site_settings,
    ):
        """Full refund sets order status to refunded and records the amount."""
        order, _ = self._make_refundable_order(
            pos_terminal, pos_manager_user, manager_open_shift, site_settings,
        )
    ...
    def test_return_to_stock(
        self, pos_manager_client, pos_terminal, pos_manager_user,
        manager_open_shift, site_settings, warehouse, category,
    ):
        ...
        stock = StockItemFactory(
            product=product, warehouse=warehouse, on_hand=10,
        )

        order = _create_pos_order(
            pos_terminal, pos_manager_user, site_settings,
    ...
    def test_requires_permission(
        self, pos_client, pos_terminal, pos_staff_user,
        open_shift, site_settings,
    ):
...
class TestVoidOrder:
    ...
    def test_reverses_shift_totals(
        ...
        manager_open_shift.save(update_fields=['total_sales', 'total_transactions', 'total_refunds'])
    ...
    def test_returns_stock(
        ...
        stock = StockItemFactory(
            product=product, warehouse=warehouse, on_hand=8,
        )
