...
@pytest.fixture
def sms_account(db, django_site):
    """Mock SMS provider account."""
    account = SMSProviderAccount.objects.create(
        site=django_site,
        provider_key='twilio',
        account_label='Test Twilio',
        is_default_sms=True,
        is_active=True,
...
def user_with_sms_opted_in(db):
    """User with SMS transactional and marketing enabled."""
    user = UserFactory()
    ...
    CustomerProfile.objects.filter(user=user).update(phone='+15555555555')
...
def user_without_sms(db):
    ...
    # Create CustomerProfile with phone
    CustomerProfile.objects.filter(user=user).update(phone='+15555555556')
...
def test_send_sms_transactional_sent_when_opted_in(db, sms_service, sms_account, user_with_sms_opted_in, mock_sms_provider):
    """Transactional SMS sent when user opted in."""
    phone = user_with_sms_opted_in.customer_profile.phone
...
def test_send_sms_skipped_when_opted_out(db, sms_service, sms_account, user_without_sms):
    """SMS skipped when user has SMS disabled."""
    phone = user_without_sms.customer_profile.phone
...
def test_send_sms_marketing_requires_opt_in(db, sms_service, sms_account, mock_sms_provider):
    ...
    CustomerProfile.objects.filter(user=user).update(phone='+15555555557')
...
def test_send_sms_marketing_sent_when_opted_in(db, sms_service, sms_account, user_with_sms_opted_in, mock_sms_provider):
...
def test_send_sms_guest_user_sent_normally(db, sms_service, sms_account, mock_sms_provider):
...
def test_send_template_sms_passes_message_type(db, sms_service, sms_account, user_without_sms):
...
def test_send_sms_without_message_type_bypasses_check(db, sms_service, sms_account, user_without_sms, mock_sms_provider):
...
def test_skipped_status_recorded_in_sms_outbox(db, sms_service, sms_account, user_without_sms):
...
def test_skipped_sms_not_sent_to_provider(db, sms_service, sms_account, user_without_sms):
    """Skipped SMS are not sent to SMS provider."""
    phone = user_without_sms.customer_profile.phone

    with patch('sms_system.services.sms_sender.SMSSendingService._get_provider') as mock_get_provider:
        provider = Mock()
...
def test_master_sms_toggle_blocks_all_sms(db, sms_service, sms_account):
    ...
    CustomerProfile.objects.filter(user=user).update(phone='+15555555558')
    phone = user.customer_profile.phone
...
def test_sms_to_user_without_customer_profile(db, sms_service, sms_account, mock_sms_provider):
    """SMS to user without CustomerProfile is sent normally."""
    user = UserFactory()
    # Don't create CustomerProfile
...
def test_sms_lookup_by_phone_number(db, sms_service, sms_account):
    ...
    phone = '+15555551111'
    CustomerProfile.objects.filter(user=user).update(phone=phone)
...
def test_sms_provider_error_still_creates_outbox(db, sms_service, sms_account):
    ...
    CustomerProfile.objects.filter(user=user).update(phone='+15555551112')
    phone = user.customer_profile.phone
