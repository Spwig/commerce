...
class TestGeoLocationModel:
    ...
    def test_to_dict_returns_expected_keys(self, db):
        ...
        d = loc.to_dict()
        expected_keys = {
            'ip', 'country', 'country_name', 'region', 'region_name',
            'city', 'postal_code', 'lat', 'lon', 'asn', 'isp',
            'source', 'confidence', 'is_proxy', 'is_vpn', 'is_tor',
            'is_mobile', 'resolved_at',
        }
    ...
    def test_security_flags(self, db):
        ...
        loc = GeoLocationFactory(
            ...
            is_vpn=True, is_proxy=True, is_tor=True, is_mobile=True,
        )
...
class TestGeoIPProviderModel:
    ...
    def test_provider_type_choices(self, db):
        ...
        valid_types = ['spwig', 'maxmind', 'dbip', 'ip2location',
                       'edge_header', 'browser_hint', 'custom']
...
class TestReportCorrectionAPI:
    ...
    def test_report_creates_correction(self, anon_client, db):
        """report_correction creates a correction for current session visitor."""
        # Middleware creates VisitorLocation on first request
        anon_client.get('/api/geoip/v1/resolve/')
        resp = anon_client.post(
            '/api/geoip/v1/report/',
    ...
    def test_report_updates_visitor_location(self, anon_client, db):
        ...
        if session_key:
            # Use the visitor created by middleware
            visitor = VisitorLocation.objects.filter(session_key=session_key).first()
