...
class TestCustomerPasswordReset:
    ...
    def test_calls_send_template_email_with_correct_template_type(
        self, mock_send, site_settings, django_site
    ):
    ...
    def test_inactive_user_not_sent(self, mock_send, site_settings, django_site):
        ...
        response = client.post(
            "/api/accounts/password-reset/", {"email": "inactive@test.spwig.com"}
        )
        assert response.status_code == 200
        mock_send.assert_not_called()
...
class TestFormBuilderEmailNotification:
    ...
    def test_calls_send_template_email_with_correct_type(self, mock_send, django_site):
        ...
        action, form_response = _mock_form_action_and_response(to_emails=["admin@store.com"])
    ...
    def test_all_recipients_fail(self, mock_send, django_site):
        ...
        action, form_response = _mock_form_action_and_response(to_emails=["a@store.com"])
        executor = EmailNotificationAction(action, form_response)
        result = executor.execute()
...
class TestStaffPasswordResetFallbackRemoval:
    ...
    def test_staff_reset_uses_email_sending_service(
        self, mock_audit, mock_send, site_settings, django_site
    ):
    ...
    @patch("email_system.services.email_sender.EmailSendingService.send_template_email")
    @patch("admin_api.services.audit_service.AuditService.log")
    def test_staff_reset_non_staff_user_not_sent(
        self, mock_audit, mock_send, site_settings, django_site
    ):
        ...
        client = APIClient()
        response = client.post(
            "/api/admin/auth/password-reset/", {"email": "regular@test.spwig.com"}
        )
        assert response.status_code == 200
        mock_send.assert_not_called()
    ...
    @patch("email_system.services.email_sender.EmailSendingService.send_template_email")
    @patch("admin_api.services.audit_service.AuditService.log")
    def test_staff_reset_error_does_not_crash(
        self, mock_audit, mock_send, site_settings, django_site
    ):
        ...
        client = APIClient()
        response = client.post(
            "/api/admin/auth/password-reset/", {"email": "stafferr@test.spwig.com"}
        )
        assert response.status_code == 200
