...
class TestMediaAssetAPI:
    ...
    def test_search_media_assets(self, auth_client, admin_user, site_settings):
        ...
        specific = MediaAssetFactory(
            title='Unique Product Photo',
            alt_text='Product shot',
            uploaded_by=admin_user
        )
        generic = MediaAssetFactory(
            title='Generic Image',
            uploaded_by=admin_user
        )
...
class TestMediaFolderAPI:
    ...
    def test_create_folder(self, auth_client, site_settings):
        ...
        resp = auth_client.post('/api/media/folders/', {
            'name': 'Product Images',
...
class TestTagAPI:
    ...
    def test_create_tag(self, auth_client, site_settings):
        """Can create new tags."""
        resp = auth_client.post('/api/media/tags/', {
            'name': 'product-photos'
        })
        if resp.status_code != 201:
            print(f"Validation error: {resp.json()}")
...
class TestMediaUsage:
    ...
    def test_mark_asset_in_use(self, auth_client, admin_user, site_settings):
        ...
        resp = auth_client.patch(f'/api/media/assets/{asset.id}/', {
            'title': 'Updated Title'
        })
        assert resp.status_code == 200
    ...
    def test_filter_unused_assets(self, auth_client, admin_user, site_settings):
        ...
        in_use = MediaAssetFactory(usage_count=5, uploaded_by=admin_user)  # usage_count > 0 means in use
        not_in_use = MediaAssetFactory(usage_count=0, uploaded_by=admin_user)  # usage_count = 0 means not in use
