...
from tests.factories import (
    ProductFactory, CategoryFactory, SiteLanguageFactory, PageFactory,
)
...
def target_languages(db):
    ...
    SiteLanguage.objects.update_or_create(
        code='en',
        defaults={'name': 'English', 'native_name': 'English', 'is_active': True, 'is_default': True, 'flag': ''},
    )
    es, _ = SiteLanguage.objects.update_or_create(
        code='es',
        defaults={'name': 'Spanish', 'native_name': 'Español', 'is_active': True, 'is_default': False, 'flag': ''},
    )
    fr, _ = SiteLanguage.objects.update_or_create(
        code='fr',
        defaults={'name': 'French', 'native_name': 'Français', 'is_active': True, 'is_default': False, 'flag': ''},
    )
    return [es, fr]
...
class TestCoverageService:
    ...
    def test_empty_source_fields_skipped(
        self, target_languages, category, site_settings
    ):
    ...
    def test_overall_percentage_calculation(
        ...
        if result['total_fields'] > 0:
            expected_pct = round(
                result['translated_fields'] / result['total_fields'] * 100, 1
            )
    ...
    def test_invalidate_cache(self, target_languages, site_settings):
        ...
        from translations.coverage_service import (
            TranslationCoverageService, invalidate_coverage_cache, COVERAGE_CACHE_KEY,
        )
    ...
    def test_ui_string_coverage_included(self, target_languages, site_settings):
        ...
        UITranslationOverride.objects.create(
            language=es_lang,
        ...
        assert ui_ct['by_language']['es']['translated'] == 50
...
class TestCoverageAPI:
    ...
    def test_coverage_api_returns_200(
        self, staff_client, target_languages, site_settings
    ):
        resp = staff_client.get('/en/admin/translations/api/coverage/')
    ...
    def test_coverage_api_requires_staff(self, target_languages, site_settings):
        client = Client()
        resp = client.get('/en/admin/translations/api/coverage/')
    ...
    def test_coverage_refresh_invalidates_cache(
        ...
        resp = staff_client.post('/en/admin/translations/api/coverage/refresh/')
        assert resp.status_code == 200
        data = resp.json()
        assert data['success'] is True
    ...
    def test_coverage_refresh_requires_post(
        self, staff_client, target_languages, site_settings
    ):
        resp = staff_client.get('/en/admin/translations/api/coverage/refresh/')
        assert resp.status_code == 405

    def test_coverage_detail_page_renders(
        self, staff_client, target_languages, site_settings
    ):
...
class TestTranslateAllEstimate:
    ...
    def test_estimate_returns_field_counts(
        ...
        resp = staff_client.get('/en/admin/translations/api/translate-all/estimate/')
        assert resp.status_code == 200
        data = resp.json()
        assert data['success'] is True
    ...
    def test_estimate_is_large_flag(
        self, staff_client, target_languages, site_settings
    ):
    ...
    def test_estimate_requires_staff(self, target_languages, site_settings):
        client = Client()
        resp = client.get('/en/admin/translations/api/translate-all/estimate/')
...
class TestTranslateAllAPI:
    ...
    def test_creates_translation_jobs(
        self, mock_ui_task, mock_job_task,
        staff_client, target_languages, product_untranslated, site_settings,
    ):
        ...
        resp = staff_client.post(
            '/en/admin/translations/api/translate-all/',
    ...
    def test_batch_splitting(
        self, mock_ui_task, mock_job_task,
        staff_client, target_languages, site_settings,
    ):
    ...
    def test_requires_post(self, staff_client, target_languages, site_settings):
        resp = staff_client.get('/en/admin/translations/api/translate-all/')
        assert resp.status_code == 405
...
class TestTranslateAllStatus:
    ...
    def test_status_counts_completed(
        self, staff_client, target_languages, site_settings
    ):
        from translations.models import TranslationJob
        ...
        resp = staff_client.get(
            f'/en/admin/translations/api/translate-all/status/?job_ids={ids_str}'
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_status_empty_job_ids(self, staff_client, target_languages, site_settings):
        resp = staff_client.get(
            '/en/admin/translations/api/translate-all/status/'
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_status_requires_staff(self, target_languages, site_settings):
        client = Client()
        resp = client.get('/en/admin/translations/api/translate-all/status/')
        assert resp.status_code == 302

    def test_status_with_failed_jobs(
        self, staff_client, target_languages, site_settings
    ):
        from translations.models import TranslationJob
...
class TestTaskProcessorModelContent:
    ...
    def test_get_model_content_skips_empty_fields(
        self, target_languages, category, site_settings
    ):
        """Product with empty short_description — only name extracted."""
        p = ProductFactory(
            name='Empty Desc', slug='empty-desc',
            short_description='', category=category,
        )
    ...
    def test_get_model_content_invalid_registry_key(
        self, target_languages, site_settings
    ):
        processor = self._make_processor()
        from translations.models import TranslationJob

        job = TranslationJob.objects.create(
            job_type='bulk', source_language='en',
            target_languages=['es'], content_type='fake.model',
            translated_data={
                'object_ids': [999],
                'language': 'es',
    ...
    def test_save_model_translations_invalidates_cache(
        ...
        processor._save_model_translations(
            job, {f"{product_untranslated.pk}:name": "Test"}
        )
...
class TestTranslationLocks:
    ...
    def test_get_locked_fields_filters(self, target_languages, admin_user, site_settings):
        ...
        TranslationMeta.objects.create(
            content_type='catalog.product', object_id=10, field_name='name',
            language='es', is_locked=True, locked_by=admin_user
        )
        TranslationMeta.objects.create(
            content_type='catalog.product', object_id=10, field_name='description',
    ...
    def test_ui_save_skips_locked(
        ...
        client.force_login(admin_user)
        resp = client.post(
            '/en/admin/translations/api/ui-translations/es/save/',
    ...
    def test_lock_toggle_api(self, target_languages, admin_user, site_settings, django_site):
        ...
        # Lock
        resp = client.post(
            '/en/admin/translations/api/lock/toggle/',
            data=json.dumps({
                'content_type': 'catalog.product',
                'object_id': 999,
                'field_name': 'name',
    ...
    def test_ui_lock_toggle_api(self, target_languages, admin_user, site_settings, django_site):
        ...
        # Lock
        resp = client.post(
            '/en/admin/translations/api/ui-translations/es/lock/',
    ...
    def test_lock_toggle_requires_staff(self, target_languages, customer_user, site_settings, django_site):
