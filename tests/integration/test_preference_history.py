...
def test_preference_history_link_from_preferences_page(client, user_with_history):
...
def test_preference_history_end_to_end(client, db):
...
def test_preference_history_query_performance(client, user_with_history, django_assert_num_queries):
    """Test preference history doesn't generate excessive queries"""
    client.force_login(user_with_history)

    url = reverse('accounts:preference_history')

    # Should be efficient - limited queries even with pagination
    with django_assert_num_queries(10):  # Adjust based on actual needs
        response = client.get(url)
