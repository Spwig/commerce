...
class TestEmailTemplateModel:
    ...
    def test_deactivate_restores_system_fallback(self, django_site):
        ...
        EmailTemplate.all_objects.filter(
            site=django_site, template_type=tt
        ).delete()
...
class TestEmailAccountAJAXEndpoints:
    ...
    def test_test_connection(self, staff_client, email_account):
        ...
        with patch.object(
            type(email_account), 'get_provider_instance', return_value=mock_provider
        ):
...
class TestTrackOpen:
    ...
    def test_track_open_creates_event(self, anon_client, outbox_entry):
        ...
        event = EmailEvent.objects.filter(
            email=outbox_entry, event_type='opened'
        ).first()
...
class TestTrackClick:
    ...
    def test_track_click_creates_event(self, anon_client, outbox_entry):
        ...
        event = EmailEvent.objects.filter(
            email=outbox_entry, event_type='clicked'
        ).first()
        assert event is not None
...
class TestEmailSendingService:
    ...
    def test_queue_email_preference_check_skips_marketing(
        self, default_email_account, django_site
    ):
    ...
    def test_queue_email_transactional_always_sends(
        self, default_email_account, django_site
    ):
...
class TestEmailTemplateTranslation:
    ...
    def test_clone_includes_translations(self, django_site, staff_user):
        ...
        EmailTemplateTranslation.objects.create(
            template=original, language_code='es',
            subject='S', html_content='<p>H</p>', text_content='T',
        )
        EmailTemplateTranslation.objects.create(
            template=original, language_code='fr',
            subject='S', html_content='<p>H</p>', text_content='T',
        )
