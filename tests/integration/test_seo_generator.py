...
from seo_generator.api.endpoints import (
    MODEL_MAP, extract_content_from_object, get_model_class,
)
from tests.factories import (
    UserFactory, ProductFactory, CategoryFactory, PageFactory,
...
class TestSEOGenerateAPI:
    ...
    def test_custom_provider_key(self, admin_client, product, site_settings):
        ...
        response = admin_client.post(
            url,
            data=json.dumps({'provider': 'deterministic'}),
...
class TestSEOProviderAccount:
    ...
    def test_ordering(self, site):
        ...
        c = SEOProviderAccount.objects.create(
            site=site, provider_key='c', name='B Provider', priority=0,
        )
        assert c.is_primary is True  # Auto-set as first
        b = SEOProviderAccount.objects.create(
            site=site, provider_key='b', name='A Provider', is_primary=False, priority=5,
        )
        a = SEOProviderAccount.objects.create(
            site=site, provider_key='a', name='C Provider', is_primary=False, priority=10,
        )
...
class TestEncryptionUtils:
    ...
    def test_encrypt_decrypt_roundtrip(self):
        ...
        from seo_generator.utils.encryption import encrypt_credentials, decrypt_credentials
...
class TestCSPCompliance:
    ...
    def test_no_inline_style_attributes(self):
        """Templates must not use inline style= attributes."""
        for html_file in self._get_html_files():
            content = html_file.read_text()
            # Match style= but not {% static or CSS file references
            matches = re.findall(r'\bstyle\s*=\s*["\']', content)
            assert len(matches) == 0, (
                f"Inline style= found in {html_file.relative_to(PROJECT_ROOT)}"
            )
...
class TestStaticFiles:
    ...
    def test_js_copyright_headers(self):
        ...
        for js_file in self._get_js_files():
            ...
            assert '© 2025 Spwig' in content or '©' in content, (
...
class TestI18n:
    ...
    def test_field_verbose_names(self):
        ...
        fields = {f.name: f for f in SEOProviderAccount._meta.get_fields() if hasattr(f, 'verbose_name')}
