...
class TestFormModel:
    ...
    def test_response_count_excludes_draft(self, form, user):
        FormResponse.objects.create(
            form=form, user=user, data={}, status='draft'
        )
...
class TestFormFieldModel:
    ...
    def test_all_field_types(self, form):
        ...
        field_types = [
            ...
            'date', 'time', 'datetime', 'select', 'checkbox',
            'checkbox_group', 'radio', 'file', 'product_select',
            'rating_stars', 'rating_likert', 'rating_nps',
            'heading', 'paragraph', 'divider', 'hidden',
        ]
        for i, ft in enumerate(field_types):
            FormField.objects.create(
                form=form, field_name=f'field_{ft}',
                field_type=ft, label=f'Test {ft}', order=i,
            )
    ...
    def test_width_choices(self, form):
        for width in ['full', 'half', 'third']:
            f = FormField.objects.create(
                form=form, field_name=f'w_{width}',
                field_type='text', label='W', width=width, order=0,
            )
    ...
    def test_rating_config_json(self, form):
        field = FormField.objects.create(
            form=form, field_name='stars',
            field_type='rating_stars', label='Rating',
    ...
    def test_file_config_json(self, form):
        field = FormField.objects.create(
            form=form, field_name='upload',
            field_type='file', label='Upload',
...
class TestFormResponseModel:
    ...
    def test_str_anonymous(self, form):
        r = FormResponse.objects.create(
            form=form, data={}, status='submitted'
        )
    ...
    def test_action_results(self, form):
        r = FormResponse.objects.create(
            form=form, data={}, status='completed',
            action_results={
                'email_notification': {'success': True, 'sent_to': 'admin@test.com'}
            },
        )
...
class TestFormConditionalRuleModel:
    ...
    def source_field(self, form, step):
        return FormField.objects.create(
            form=form, step=step, field_name='rating',
            field_type='number', label='Rating', order=10,
        )

    @pytest.fixture
    def target_field(self, form, step):
        return FormField.objects.create(
            form=form, step=step, field_name='feedback',
            field_type='textarea', label='Feedback', order=11,
        )
    ...
    def test_str_without_name(self, form, source_field, target_field):
        rule = FormConditionalRule.objects.create(
            form=form, source_field=source_field,
            operator='equals', value={'value': 'test'},
            action='show_field', target_field=target_field,
        )
    ...
    def test_clean_field_action_requires_target_field(self, form, source_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field,
            operator='equals', value={'value': '1'},
            action='show_field', target_field=None,
        )
    ...
    def test_clean_step_action_requires_target_step(self, form, source_field, step):
        rule = FormConditionalRule(
            form=form, source_field=source_field,
            operator='equals', value={'value': '1'},
            action='skip_to_step', target_step=None,
        )
        with pytest.raises(ValidationError):
            rule.clean()
    ...
    def test_evaluate_equals(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='equals',
            value={'value': 'hello'}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_not_equals(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='not_equals',
            value={'value': 'hello'}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_contains(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='contains',
            value={'value': 'world'}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_not_contains(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='not_contains',
            value={'value': 'world'}, action='show_field', target_field=target_field,
        )
        assert rule.evaluate('hello') is True
    ...
    def test_evaluate_starts_with(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='starts_with',
            value={'value': 'hel'}, action='show_field', target_field=target_field,
        )
        assert rule.evaluate('hello') is True
        assert rule.evaluate('world') is False
    ...
    def test_evaluate_ends_with(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='ends_with',
            value={'value': 'lo'}, action='show_field', target_field=target_field,
        )
        assert rule.evaluate('hello') is True
        assert rule.evaluate('world') is False
    ...
    def test_evaluate_greater_than(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='greater_than',
            value={'value': '5'}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_less_than(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='less_than',
            value={'value': '5'}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_greater_than_or_equal(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='greater_than_or_equal',
            value={'value': '5'}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_less_than_or_equal(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='less_than_or_equal',
            value={'value': '5'}, action='show_field', target_field=target_field,
        )
        assert rule.evaluate('5') is True
        assert rule.evaluate('4') is True
    ...
    def test_evaluate_is_empty(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='is_empty',
            value={}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_is_not_empty(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='is_not_empty',
            value={}, action='show_field', target_field=target_field,
        )
    ...
    def test_evaluate_in_list(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='in_list',
    ...
    def test_evaluate_not_in_list(self, form, source_field, target_field):
        rule = FormConditionalRule(
            form=form, source_field=source_field, operator='not_in_list',
...
class TestFormActionModel:
    ...
    def test_action_types(self, form):
        for at in ['email_notification', 'auto_reply', 'webhook']:
            a = FormAction.objects.create(
                form=form, action_type=at,
                name=f'Action {at}', order=0,
            )
    ...
    def test_ordering(self, form):
        a2 = FormAction.objects.create(
            form=form, action_type='webhook', name='Second', order=2,
        )
        a1 = FormAction.objects.create(
            form=form, action_type='email_notification', name='First', order=1,
        )
    ...
    def test_cascade_delete(self, form):
        FormAction.objects.create(
            form=form, action_type='webhook', name='Test', order=0,
        )
