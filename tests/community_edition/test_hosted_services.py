...
def test_get_usage_snapshot_returns_none_when_cache_cold():
    ...
    from core.hosted_services import get_usage_snapshot, usage

    cache.delete(usage.CACHE_KEY)
    with (
        patch.object(usage, "_fetch_geoip_usage") as f1,
...
def test_snapshot_aggregates_pct_and_over_flags(settings):
    ...
    with (
        ...
        patch(
            "core.hosted_services.tiers.get_tier_config",
            return_value={
                "services": {},
...
def test_tier_config_returns_empty_on_error(settings):
    ...
    with patch("core.hosted_services.tiers.requests.get", side_effect=Exception("no dns")):
...
def test_quota_email_fires_once_per_month(settings):
    ...
    with (
        ...
        patch("core.hosted_services.refresh_usage_snapshot", return_value=snapshot_over_90),
