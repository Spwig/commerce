...
class AutocompleteViewTests(TestCase):
    ...
    def test_postcode_detection_patterns(self):
        ...
        for query, country, expected_is_postcode in test_cases:
            ...
            self.assertEqual(result, expected_is_postcode, f"Failed for {query} in {country}")
    ...
    def test_country_based_sorting(self, mock_autocomplete):
        ...
        response = self.client.get(
            "/api/address/autocomplete/", {"q": "Main Street", "country": "SG"}
        )
    ...
    def test_increased_result_limit(self):
        ...
        response = self.client.get(
            "/api/address/autocomplete/", {"q": "Main Street", "limit": "10"}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_minimum_query_length(self):
        ...
        response = self.client.get("/api/address/autocomplete/", {"q": "ab"})
        self.assertEqual(response.status_code, 200)
        data = response.json()
