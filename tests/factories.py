...
class LocationFactory(factory.django.DjangoModelFactory):
    ...
    code = factory.Sequence(lambda n: f"LOC-TEST-{n:04d}")
    location_type = "store"
    address1 = "789 Store Street"
    city = "New York"
    state = "NY"
    postal_code = "10001"
    country = "US"
    phone = "+1-555-0300"
    email = factory.Sequence(lambda n: f"location{n}@test.spwig.com")
    is_active = True
...
class ShippingCountryFactory(factory.django.DjangoModelFactory):
    ...
    site = factory.LazyFunction(
        lambda: __import__(
            "django.contrib.sites.models", fromlist=["Site"]
        ).Site.objects.get_or_create(id=1, defaults={"domain": "localhost", "name": "Test"})[0]
    )
    country_code = "US"
    is_active = True
    priority = 0
...
class OrderFactory(factory.django.DjangoModelFactory):
    ...
    shipping_name = factory.LazyAttribute(
        lambda o: f"{o.user.first_name} {o.user.last_name}" if o.user else "Guest Customer"
    )
    ...
    class Params:
        ...
        with_tracking = factory.Trait(
            tracking_number=factory.LazyFunction(
                lambda: f"1Z999AA1{uuid.uuid4().hex[:10].upper()}"
            ),
...
class OrderItemFactory(factory.django.DjangoModelFactory):
    ...
    unit_price_base = factory.LazyAttribute(
        lambda o: (
            o.unit_price.amount if hasattr(o.unit_price, "amount") else Decimal(str(o.unit_price))
        )
    )
    total_price_base = factory.LazyAttribute(
        lambda o: (
            o.total_price.amount
            if hasattr(o.total_price, "amount")
            else Decimal(str(o.total_price))
        )
    )
    ...
    # Stock tracking (BooleanField flags on the model)
    ...
    class Params:
        ...
        bundle_component = factory.Trait(
            parent_bundle=factory.SubFactory(
                "tests.factories.OrderItemFactory", bundle_parent=True
            ),
        ...
        with_customizations = factory.Trait(
            customizations=factory.LazyFunction(
                lambda: {
...
class RefundFactory(factory.django.DjangoModelFactory):
    ...
    # Base-currency equivalents
    total_amount_base = factory.LazyAttribute(
        lambda o: (
            o.total_amount.amount
            if hasattr(o.total_amount, "amount")
            else Decimal(str(o.total_amount))
        )
    )
    shipping_refund_amount_base = factory.LazyAttribute(
        lambda o: (
            o.shipping_refund_amount.amount
            if hasattr(o.shipping_refund_amount, "amount")
            else Decimal(str(o.shipping_refund_amount))
        )
    )
    tax_refund_amount_base = factory.LazyAttribute(
        lambda o: (
            o.tax_refund_amount.amount
            if hasattr(o.tax_refund_amount, "amount")
            else Decimal(str(o.tax_refund_amount))
        )
    )
...
class ReturnRequestFactory(factory.django.DjangoModelFactory):
    ...
    items_json = factory.LazyFunction(
        lambda: [
            {
                "order_item_id": 1,
                "quantity": 1,
    ...
    class Params:
        ...
        label_sent = factory.Trait(
            ...
            return_tracking_number=factory.LazyFunction(
                lambda: f"RET-{uuid.uuid4().hex[:12].upper()}"
            ),
        )
...
class MediaFolderFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        nested = factory.Trait(parent=factory.SubFactory("tests.factories.MediaFolderFactory"))
...
class ImageSizePresetFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        small = factory.Trait(name="test_small", width=300, height=300)
        medium = factory.Trait(name="test_medium", width=600, height=600)
        large = factory.Trait(name="test_large", width=1200, height=1200)
...
class MediaAssetFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        ...
        svg = factory.Trait(mime_type="image/svg+xml", width=None, height=None)

        # Video types
        video = factory.Trait(mime_type="video/mp4", width=1920, height=1080)
        webm = factory.Trait(mime_type="video/webm", width=1920, height=1080)

        # 3D model
        model_3d = factory.Trait(mime_type="model/gltf-binary", width=None, height=None)
        ...
        # With folder
        in_folder = factory.Trait(folder=factory.SubFactory(MediaFolderFactory))
...
class ThumbnailFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        small = factory.Trait(size_preset="small", width=300, height=300)
        medium = factory.Trait(size_preset="medium", width=600, height=600)
        large = factory.Trait(size_preset="large", width=1200, height=1200)
...
class MediaProcessingJobFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        ...
        failed = factory.Trait(status="failed", error_message="Test error message")
...
class PaymentProviderAccountFactory(factory.django.DjangoModelFactory):
    ...
    available_payment_methods = factory.LazyFunction(
        lambda: {"US": ["card"], "GB": ["card"], "DE": ["card"]}
    )
    enabled_payment_methods = factory.LazyFunction(
        lambda: {"US": ["card"], "GB": ["card"], "DE": ["card"]}
    )
        ...
        return encrypt_credentials(
            {
                ...
                "webhook_secret": os.environ.get(
                    "STRIPE_TEST_WEBHOOK_SECRET", "whsec_test_placeholder"
                ),
                "environment": "test",
            }
        )
...
class EmailAccountFactory(factory.django.DjangoModelFactory):
    ...
    site = factory.LazyFunction(
        lambda: __import__("django.contrib.sites.models", fromlist=["Site"]).Site.objects.get(pk=1)
    )
    ...
    credentials = factory.LazyFunction(
        lambda: __import__(
            "email_system.utils.encryption", fromlist=["encrypt_credentials"]
        ).encrypt_credentials(
...
class EmailEventFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        opened = factory.Trait(
            event_type="opened", user_agent="Mozilla/5.0", ip_address="127.0.0.1"
        )
        clicked = factory.Trait(
            event_type="clicked", user_agent="Mozilla/5.0", ip_address="127.0.0.1"
        )
        bounced = factory.Trait(
            event_type="bounced", bounce_type="hard", bounce_reason="Mailbox not found"
        )
