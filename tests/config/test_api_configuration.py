...
class TestDRFConfiguration:
    ...
    def test_required_settings_present(self):
        ...
        assert not missing, "Missing required REST_FRAMEWORK settings:\n  " + "\n  ".join(missing)
    ...
    def test_default_renderer_classes_importable(self):
        ...
        assert not errors, f"Failed to import {len(errors)} renderer classes:\n  " + "\n  ".join(
            errors
        )
    ...
    def test_default_parser_classes_importable(self):
        ...
        assert not errors, f"Failed to import {len(errors)} parser classes:\n  " + "\n  ".join(
            errors
        )
    ...
    def test_default_permission_classes_importable(self):
        ...
        assert not errors, f"Failed to import {len(errors)} permission classes:\n  " + "\n  ".join(
            errors
        )
    ...
    def test_page_size_configured(self):
        ...
        if rest_framework.get("DEFAULT_PAGINATION_CLASS"):
            assert page_size, "PAGE_SIZE must be configured when pagination is enabled"
...
class TestCORSConfiguration:
    ...
    def test_cors_middleware_present(self):
        ...
        assert cors_mw in middleware, "CorsMiddleware must be in MIDDLEWARE for CORS support"
...
class TestDRFSpectacularConfiguration:
    ...
    def test_spectacular_settings_configured(self):
        ...
        assert isinstance(spectacular_settings, dict), "SPECTACULAR_SETTINGS must be a dictionary"
...
class TestAPISecurityConfiguration:
    ...
    def test_csrf_trusted_origins_configured_in_production(self):
        ...
        if not settings.DEBUG:
            ...
            assert isinstance(csrf_trusted_origins, list), "CSRF_TRUSTED_ORIGINS must be a list"
    ...
    def test_csrf_cookie_httponly(self):
        ...
        assert csrf_httponly is True, "CSRF_COOKIE_HTTPONLY should be True to prevent XSS attacks"
    ...
    def test_rest_framework_uses_json_renderer(self):
        ...
        assert json_renderer in renderers, "JSONRenderer should be in DEFAULT_RENDERER_CLASSES"
