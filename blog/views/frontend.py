...
from blog.models import BlogCategory, BlogPost, BlogSettings, BlogSubscriber, BlogTag
...
def category_posts(request, slug):
    ...
    posts = BlogPost.published().filter(category=category).select_related("featured_image")
...
def tag_posts(request, slug):
    ...
    posts = BlogPost.published().filter(tags=tag).select_related("category", "featured_image")
...
def subscribe(request):
    ...
    if not settings.enable_subscriptions:
        return JsonResponse(
            {"success": False, "error": "Subscriptions are currently disabled."}, status=400
        )
    ...
    if existing:
        if existing.is_active and existing.verification_status == "verified":
            return JsonResponse(
                {"success": False, "error": "This email is already subscribed."}, status=400
            )
            ...
            if settings.require_double_opt_in:
                ...
                existing.verify()
                return JsonResponse({"success": True, "message": "You have been resubscribed!"})
    ...
    if settings.require_double_opt_in:
        ...
        subscriber.verify()
        return JsonResponse({"success": True, "message": "You have been subscribed!"})
...
def verify_subscription(request, token):
    ...
    subscriber = get_object_or_404(
        BlogSubscriber, verification_token=token, verification_status="pending"
    )
    ...
    return render(
        request,
        "blog/subscription_verified.html",
        {
...
def subscription_preferences(request, token):
    ...
    subscriber = get_object_or_404(BlogSubscriber, unsubscribe_token=token, is_active=True)
