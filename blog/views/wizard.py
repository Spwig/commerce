    ...
    def get(self, request):
        ...
        for provider in available_from_server:
            ...
            provider_data = {
                ...
                'documentation_url': provider.get('documentation_url') or manifest.get('api_docs_url', ''),
    ...
    def _exchange_code_for_token(self, provider_slug, oauth_config, code, redirect_uri):
        ...
        if provider_slug in ['facebook_page', 'instagram_business']:
            ...
            if token_data.get('access_token'):
                ...
                if long_lived:
                    token_data['access_token'] = long_lived.get('access_token', token_data['access_token'])
...
class WizardSelectAccountView(WizardSessionMixin, View):
    ...
    def _get_facebook_pages(self, access_token, account_selection):
        ...
        try:
            ...
            api_fields = account_selection.get('api_fields', 'id,name,category,picture{url},access_token')
...
class WizardStep3View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        if wizard_data.get('credentials'):
            ...
            suggested_name = creds.get('page_name') or creds.get('organization_name') or creds.get('username') or ''
        ...
        context = {
            ...
            'credential_schema': {} if has_oauth_credentials else manifest.get('credential_schema', {}),
        }
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            account = SocialConnectorAccount.objects.create(
                ...
                platform_account_id=wizard_data['credentials'].get('page_id') or
                                    wizard_data['credentials'].get('user_id') or
                                    wizard_data['credentials'].get('organization_id') or
                                    wizard_data['credentials'].get('instagram_account_id') or
                                    'pending',
                platform_account_name=wizard_data['credentials'].get('page_name') or
                                       wizard_data['credentials'].get('username') or
                                       wizard_data['credentials'].get('organization_name') or
                                       wizard_data['credentials'].get('instagram_username') or
                                       '',
        ...
        messages.success(
            request,
            _('Social account "{name}" connected successfully!').format(name=account.name)
        )
...
def install_provider_ajax(request, provider_slug):
    ...
    try:
        ...
        if result.get('success'):
            ...
            return JsonResponse({
                'success': False,
                'error': result.get('error', _('Installation failed'))
            }, status=400)
...
def update_provider_ajax(request, provider_slug):
    ...
    try:
        ...
        if result.get('success'):
            return JsonResponse({
                'success': True,
                'message': _('Provider updated successfully')
            })
        else:
            return JsonResponse({
                'success': False,
                'error': result.get('error', _('Update failed'))
            }, status=400)
...
def test_connection_ajax(request, account_id):
    ...
    try:
        ...
        if not account.component or not account.component.installed_path:
            return JsonResponse({
                'success': False,
                'error': _('Provider not properly installed')
            }, status=400)
        ...
        try:
            ...
            if not provider_path.exists():
                return JsonResponse({
                    'success': False,
                    'error': _('Provider module not found')
                }, status=400)
            ...
            if not class_name or not hasattr(module, class_name):
                return JsonResponse({
                    'success': False,
                    'error': _('Provider class not found')
                }, status=400)
