...
class BlogPostAutoShareInline(admin.TabularInline):
    ...
    def platform_post_url_link(self, obj):
        ...
        if obj.platform_post_url:
            return format_html(
                '<a href="{}" target="_blank" rel="noopener">View Post</a>', obj.platform_post_url
            )
        return "-"
...
class BlogCategoryAdmin(SEOGeneratorAdminMixin, TranslatableAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("name", "slug", "parent", "description")}),
        (_("Media"), {"fields": ("image_asset",)}),
        (
            _("Design & Theme"),
            {
        ...
        (_("Status"), {"fields": ("is_active", "sort_order")}),
    )
...
class BlogPostAdmin(SEOGeneratorAdminMixin, TranslatableAdminMixin, admin.ModelAdmin):
    ...
    list_filter = ["status", "is_featured", "is_pinned", "category", "created_at", "published_at"]
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("title", "slug", "status", "category", "tags")}),
        (
            _("Content"),
            {
                "fields": ("excerpt", "simple_content", "use_page_builder", "content_page"),
                "description": _(
        ...
        (_("SEO"), {"fields": ("meta_title", "meta_description", "seo_auto_generated")}),
        (_("Publishing"), {"fields": ("published_at", "scheduled_at", "is_featured", "is_pinned")}),
        (
    ...
    class Media:
        js = (
            TranslatableAdminMixin.Media.js
            + SEOGeneratorAdminMixin.Media.js
    ...
    def add_view(self, request, form_url="", extra_context=None):
        ...
        self.message_user(request, _("New draft post created. Start editing below."), level="INFO")

        return HttpResponseRedirect(reverse("admin:blog_blogpost_change", args=[new_post.pk]))
    ...
    def create_page_view(self, request, pk):
        ...
        if obj:
            ...
            self.message_user(
                request, _("Page Builder page created. You can now edit the page layout.")
            )
        return HttpResponseRedirect(reverse("admin:blog_blogpost_change", args=[pk]))
        ...
        if object_id:
            ...
            if post:
                ...
                extra_context["configured_social_accounts"] = SocialConnectorAccount.objects.filter(
                    status="connected", auto_share_enabled=True
...
class BlogSubscriberAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Subscriber Information"), {"fields": ("id", "email", "name", "user", "language_code")}),
        (_("Preferences"), {"fields": ("notification_frequency", "subscribed_categories")}),
        (
            _("Verification"),
            {
                "fields": (
        ...
        (
            ...
            {"fields": ("is_active", "unsubscribe_token", "unsubscribed_at", "unsubscribe_reason")},
        ),
        (
            _("Tracking"),
            {
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["active_count"] = BlogSubscriber.objects.filter(is_active=True).count()
        return super().changelist_view(request, extra_context=extra_context)
...
class SocialConnectorAccountAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Account Information"), {"fields": ("id", "name", "provider_key", "component", "site")}),
        (
            _("Platform Details"),
            {
                "fields": (
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["expired_count"] = all_accounts.filter(status="token_expired").count()
        extra_context["error_count"] = all_accounts.filter(status="error").count()

        # Get unique providers for filter dropdown
        extra_context["providers"] = (
            all_accounts.values_list("provider_key", flat=True).distinct().order_by("provider_key")
        )
    ...
    def refresh_tokens(self, request, queryset):
        ...
        if failed:
            self.message_user(
                request,
                _("Failed to refresh %d accounts (connector not available).") % failed,
                level="warning",
            )
...
class BlogPostAutoShareAdmin(admin.ModelAdmin):
    ...
    list_display = ["post", "social_account", "status", "scheduled_at", "posted_at", "retry_count"]
    ...
    fieldsets = (
        (_("Post & Account"), {"fields": ("id", "post", "social_account")}),
        (_("Status"), {"fields": ("status", "scheduled_at")}),
        (
            _("Result"),
            {"fields": ("platform_post_id", "platform_post_url", "posted_at", "posted_content")},
        ),
        (
            _("Retry"),
            {
...
class BlogSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("RSS Feed"), {"fields": ("rss_enabled", "rss_posts_count", "rss_include_full_content")}),
        (
            _("Subscriptions"),
            {"fields": ("enable_subscriptions", "require_double_opt_in", "default_frequency")},
        ),
        (
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        return HttpResponseRedirect(reverse("admin:blog_blogsettings_change", args=[obj.pk]))
