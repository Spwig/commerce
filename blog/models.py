...
class BlogCategory(DesignMixin):
    ...
    seo_auto_generated = models.BooleanField(
        default=False, help_text=_("Automatically regenerate SEO content when saved")
    )
    ...
    imported_meta = models.JSONField(
        default=dict, blank=True, help_text=_("Metadata from import (permalink, identifiers, etc.)")
    )
...
class BlogTag(models.Model):
    ...
    translations = models.JSONField(default=dict, blank=True, help_text=_("Multilingual tag names"))
...
class BlogPost(DesignMixin):
    ...
    use_page_builder = models.BooleanField(
        default=False, help_text=_("Use Page Builder for custom layout instead of simple content")
    )
    ...
    translations = models.JSONField(
        default=dict, blank=True, help_text=_("Multilingual content (title, excerpt, content, SEO)")
    )
    ...
    scheduled_at = models.DateTimeField(
        null=True, blank=True, help_text=_("Schedule post to be published at this time")
    )

    # Features
    is_featured = models.BooleanField(default=False, db_index=True)
    is_pinned = models.BooleanField(
        default=False, help_text=_("Pinned posts appear at the top of listings")
    )
    reading_time_minutes = models.PositiveIntegerField(
        default=0, help_text=_("Estimated reading time (auto-calculated)")
    )
    view_count = models.PositiveIntegerField(default=0, editable=False)

    # Subscriber Notification
    notify_subscribers = models.BooleanField(
        default=True, help_text=_("Send notification to subscribers when published")
    )
    ...
    social_share_message = models.TextField(
        blank=True, max_length=280, help_text=_("Custom message for social shares (optional)")
    )
    ...
    def get_related_posts(self, limit=3):
        ...
        if related.count() < limit and self.tags.exists():
            ...
            related = related | BlogPost.published().filter(tags__id__in=tag_ids).exclude(
                pk=self.pk
            )
...
class BlogSubscriber(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="blog_subscription"
    )
    ...
    verification_status = models.CharField(
        max_length=20, choices=VERIFICATION_STATUS, default="pending", db_index=True
    )
...
class SocialConnectorAccount(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    site = models.ForeignKey("sites.Site", on_delete=models.CASCADE, verbose_name=_("site"))
    ...
    credentials = models.BinaryField(
        verbose_name=_("encrypted credentials"), help_text=_("Encrypted OAuth tokens")
    )
...
class BlogPostAutoShare(models.Model):
    ...
    post = models.ForeignKey(BlogPost, on_delete=models.CASCADE, related_name="auto_shares")
    social_account = models.ForeignKey(
        SocialConnectorAccount, on_delete=models.CASCADE, related_name="auto_shares"
    )
    ...
    # Scheduling
    scheduled_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When to post (null = immediately on publish)")
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["post", "social_account"], name="unique_post_social_account"
            ),
        ]
    ...
    def can_retry(self):
        ...
        return (
            ...
            and self.social_account.status == "active"
        )
...
class BlogSettings(models.Model):
    ...
    rss_include_full_content = models.BooleanField(
        default=False, help_text=_("Include full content in RSS (False = excerpt only)")
    )
    ...
    default_frequency = models.CharField(
        max_length=20, choices=BlogSubscriber.FREQUENCY_CHOICES, default="weekly"
    )

    # Digest schedule (UTC)
    weekly_digest_day = models.PositiveIntegerField(
        default=1, help_text=_("Day of week for weekly digest (0=Sunday, 1=Monday, ...)")
    )
    weekly_digest_hour = models.PositiveIntegerField(
        default=9, help_text=_("Hour (UTC) to send weekly digest")
    )
    monthly_digest_day = models.PositiveIntegerField(
        default=1, help_text=_("Day of month for monthly digest")
    )
    monthly_digest_hour = models.PositiveIntegerField(
        default=9, help_text=_("Hour (UTC) to send monthly digest")
    )
