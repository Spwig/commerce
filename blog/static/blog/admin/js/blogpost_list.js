...
(function () {
    ...
    function init() {
        var dataEl = document.getElementById('blogpost-list-data');
        if (dataEl) {
        ...
        if (!bulkSelector || !applyBtn) { return; }

        function updateSelectedCount() {
            var checked = document.querySelectorAll('.post-select:checked').length;
            if (checked > 0) {
        ...
        applyBtn.addEventListener('click', async function () {
            var action = bulkSelector.value;
            var selected = Array.from(document.querySelectorAll('.post-select:checked')).map(function (cb) { return cb.value; });

            if (!action || selected.length === 0) { return; }

            if (action === 'delete') {
                if (!await AdminModal.confirm({ message: data.confirmDelete || 'Are you sure you want to delete the selected posts?', danger: true, confirmText: 'Delete' })) {
                    return;
                }
            }
