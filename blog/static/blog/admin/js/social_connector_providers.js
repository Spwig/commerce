...
(function () {
    ...
    function init() {
        var dataEl = document.getElementById('social-connector-providers-data');
        if (dataEl) {
    ...
    async function installProvider(slug, name) {
        ...
        .then(function (responseData) {
            ...
            if (responseData.success) {
                ...
                AdminModal.alert({message: (t.installFailed || 'Installation failed') + ': ' + responseData.error, type: 'error'});
            }
        })
        .catch(function (error) {
            if (loading) { loading.style.display = 'none'; }
            AdminModal.alert({message: (t.installFailed || 'Installation failed') + ': ' + error, type: 'error'});
        });
    }

    async function updateProvider(slug, version) {
        var t = data.translations || {};
        if (!await AdminModal.confirm((t.updateToVersion || 'Update to version') + ' ' + version + '?')) { return; }

        var loading = document.getElementById('provider-install-loading');
        var message = document.getElementById('loading-message');
        if (loading) { loading.style.display = 'flex'; }
        if (message) { message.textContent = t.updating || 'Updating provider...'; }
        ...
        .then(function (responseData) {
            ...
            if (responseData.success) {
                ...
                AdminModal.alert({message: (t.updateFailed || 'Update failed') + ': ' + responseData.error, type: 'error'});
            }
        })
        .catch(function (error) {
            if (loading) { loading.style.display = 'none'; }
            AdminModal.alert({message: (t.updateFailed || 'Update failed') + ': ' + error, type: 'error'});
        });
    }
