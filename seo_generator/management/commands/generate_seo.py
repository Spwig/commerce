...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help=_('SEO provider to use (default: primary provider, or deterministic if none configured)')
        )
        parser.add_argument(
            '--language',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            _("Using provider: %(name)s") % {'name': provider.provider_name}
        )
        ...
        self.stdout.write(
            _("Total Processed: %(count)d") % {'count': total_processed}
        )
        self.stdout.write(self.style.SUCCESS(
            _("Successful: %(count)d") % {'count': total_successful}
        ))
        if total_failed > 0:
            self.stdout.write(self.style.ERROR(
                _("Failed: %(count)d") % {'count': total_failed}
            ))
