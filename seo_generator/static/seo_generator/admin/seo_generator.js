...
(function () {
   ...
   */
  function getLabels() {
    const configEl = document.querySelector('#seo-generator-container, [data-seo-labels]');
    return {
      regenerate: (configEl && configEl.dataset.seoLabelRegenerate) || 'Regenerate SEO',
      regenerateTitle:
        (configEl && configEl.dataset.seoLabelRegenerateTitle) ||
      ...
      failed:
        (configEl && configEl.dataset.seoLabelFailed) || 'SEO generation failed. Please try again.',
  ...
  function createSEOButton(modelType, objectId, metaTitleField, metaDescriptionField) {
    ...
    loadingSpinner.innerHTML =
      '<i class="fas fa-spinner fa-spin"></i> ' + escapeHTML(labels.generating);
    ...
    if (customContainer) {
      ...
      const fieldsetContent = seoFieldset.querySelector('.module');
      if (fieldsetContent) {
    ...
    fetch('/api/seo/generate/' + modelType + '/' + objectId + '/', {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          // Update SEO preview
          const previewContainer = document.querySelector('#seo-preview-content');
          if (previewContainer) {
  ...
  function updateSEOPreview(metaTitleField, metaDescriptionField, previewContainer) {
    const labels = getLabels();
    const title = metaTitleField ? metaTitleField.value : '';
    const description = metaDescriptionField ? metaDescriptionField.value : '';
    ...
    const titleHTML =
      escapeHTML(title) ||
      '<span class="seo-preview-empty">' + escapeHTML(labels.noTitle) + '</span>';
    const descHTML =
      escapeHTML(description) ||
      '<span class="seo-preview-empty">' + escapeHTML(labels.noDescription) + '</span>';
    ...
    const previewHTML =
      ...
      '<div class="seo-preview-domain">' +
      escapeHTML(domain) +
      '</div>' +
      '<div class="seo-preview-breadcrumb">' +
      ...
      '</div>' +
      (productImage
        ? '<div class="seo-preview-image"><img src="' + escapeHTML(productImage) + '" alt=""></div>'
        : '') +
      '</div>' +
      '</div>';
  ...
  function getProductImage() {
    ...
    const imageCardPreview = document.querySelector('.image-card-preview img');
    if (imageCardPreview && imageCardPreview.src) {
      return imageCardPreview.src;
    }

    const mediaLibraryImage = document.querySelector('.media-item img, .media-gallery img');
