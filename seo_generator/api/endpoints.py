...
def extract_content_from_object(obj, model_type: str) -> Dict[str, str]:
    ...
    if model_type == 'product':
        ...
        if hasattr(obj, 'get_translated_description'):
            ...
            content['description'] = strip_tags(obj.full_description) if obj.full_description else ''
...
@extend_schema(
    ...
    description=_("Generate meta title, meta description, and keywords for a product, category, brand, page, blog post, or blog category. The generated content is automatically saved to the object."),
    parameters=[
        OpenApiParameter(name='model_type', location='path', type=str, description=_("Model type: product, category, brand, page, blogpost, or blogcategory")),
        OpenApiParameter(name='object_id', location='path', type=int, description=_("ID of the object to generate SEO for")),
    ],
    request=inline_serializer(
        name='SEOGenerateRequest',
        fields={
            'provider': serializers.CharField(required=False, help_text="Provider key (default: primary provider, or deterministic if none configured)"),
            'language': serializers.CharField(required=False, help_text="Language code (default: site primary language)"),
        }
    ),
    responses={
...
def generate_seo(request, model_type: str, object_id: int):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                'success': False,
                'error': _('SEO generation failed. Please try again.')
            }, status=500)
...
@extend_schema(
    ...
    description=_("Generate SEO content for multiple objects at once. Each item specifies a model type and object ID. Results include per-item success/failure and a summary."),
    request=inline_serializer(
        ...
        fields={
            'items': serializers.ListField(
                child=inline_serializer(
                    name='SEOBatchItem',
                    fields={
                        'model_type': serializers.CharField(help_text="Model type: product, category, brand, page, blogpost, or blogcategory"),
                        'object_id': serializers.IntegerField(help_text="ID of the object"),
                    }
                ),
                help_text="List of items to generate SEO for",
            ),
            'provider': serializers.CharField(required=False, help_text="Provider key (default: primary provider)"),
    ...
    try:
        ...
        for item in items:
            ...
            try:
                ...
                failed += 1
                logger.error("Batch SEO generation failed for %s %s: %s", item_model_type, item_object_id, e)
...
@extend_schema(
    ...
    description=_("Check whether an object has SEO content, whether it was auto-generated, and return the current meta title and description with their lengths."),
    parameters=[
        OpenApiParameter(name='model_type', location='path', type=str, description=_("Model type: product, category, brand, page, blogpost, or blogcategory")),
        OpenApiParameter(name='object_id', location='path', type=int, description=_("ID of the object")),
    ],
    responses={
        200: inline_serializer(
...
def seo_status(request, model_type: str, object_id: int):
    ...
    try:
        ...
        logger.error("SEO status check error for %s %s: %s", model_type, object_id, e, exc_info=True)
        return JsonResponse({
            'success': False,
            'error': _('An unexpected error occurred. Please try again.')
        }, status=500)
