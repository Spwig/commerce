...
def _auto_generate_seo(sender, instance, **kwargs):
    ...
    try:
        ...
        for key, (app_label, model_name) in MODEL_MAP.items():
            if (
                sender._meta.app_label == app_label
                and sender._meta.model_name.lower() == model_name.lower()
            ):
                model_type = key
                break
        ...
        logger.info(
            "Auto-generated SEO for %s %s: %s", model_type, instance.pk, result["meta_title"]
        )

    except Exception as e:
        logger.warning(
            "Auto SEO generation failed for %s %s: %s", sender._meta.label, instance.pk, e
        )
...
def connect_signals():
    ...
    for _model_type, (app_label, model_name) in MODEL_MAP.items():
        try:
            ...
            if hasattr(model_class, "seo_auto_generated"):
                ...
                logger.debug(
                    "Connected SEO auto-generation signal for %s.%s", app_label, model_name
                )
        except LookupError:
