...
class DeterministicSEOProvider(BaseSEOProvider):
    ...
    def _extract_words(self, text: str) -> list[str]:
        ...
        significant_words = [word for word in words if len(word) > 1 and word not in STOP_WORDS]
    ...
    def extract_keywords(self, content: dict[str, str], max_keywords: int = 10) -> list[str]:
        ...
        try:
            ...
            raise GenerationError(_("Keyword extraction failed: %(error)s") % {"error": e})
    ...
    def generate_meta_title(self, content: dict[str, str], language: str = "en") -> str:
        ...
        try:
            ...
            if content_type == "product":
                ...
                title = f"{name} - Shop {category}" if category else f"{name} - Shop Now"
            ...
            raise GenerationError(_("Meta title generation failed: %(error)s") % {"error": e})
    ...
    def generate_meta_description(self, content: dict[str, str], language: str = "en") -> str:
        ...
        try:
            ...
            raise GenerationError(_("Meta description generation failed: %(error)s") % {"error": e})
