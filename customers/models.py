...
class CustomerMetrics(models.Model):
    ...
    purchase_frequency = models.FloatField(
        default=0.0, 
        help_text="Average days between purchases"
    )
    ...
    predicted_purchases_12m = models.FloatField(
        default=0.0,
        help_text="Expected number of purchases in next 12 months"
    )
    predicted_purchases_24m = models.FloatField(
        default=0.0,
        help_text="Expected number of purchases in next 24 months"
    )
    ltv_confidence_score = models.FloatField(
        default=0.0,
        help_text="Confidence in LTV calculation (0.0 to 1.0)"
    )
    ltv_calculation_method = models.CharField(
        max_length=20,
        default='simple',
        help_text="Method used to calculate this customer's LTV"
    )
    ltv_last_calculated = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When LTV was last calculated for this customer"
    )
    ...
    cohort_month = models.DateField(
        null=True,
        blank=True,
        help_text="Customer's acquisition cohort (YYYY-MM-01)"
    )
    ...
    def calculate_for_user(cls, user):
        ...
        if completed_orders.exists():
            ...
            metrics.days_since_last_purchase = (
                timezone.now() - last_order.created_at
            ).days
            ...
            base_multiplier = Decimal('1.0') + (Decimal(str(rfm_score)) / Decimal('100')) * Decimal('1.5')
            ...
            if metrics.purchase_frequency and metrics.purchase_frequency < 45:  # Buying every 45 days or less
        ...
        if metrics.total_orders > 0:
            metrics.cart_abandonment_rate = abandoned_carts / (metrics.total_orders + abandoned_carts)
...
class CustomerSegment(models.Model):
    ...
    color = models.CharField(
        max_length=7, 
        default='#007bff',
        help_text="Hex color code for segment display"
    )
    priority = models.IntegerField(default=0, help_text="Higher priority segments are checked first")
    ...
    def determine_segment_for_user(cls, user):
        ...
        segments = cls.objects.filter(is_active=True).exclude(name='guest').order_by('-priority', 'name')
    ...
    def matches_user(self, metrics):
        ...
        if self.min_total_spent or self.max_total_spent:
            ...
            if self.min_total_spent:
                min_amount = self._get_comparable_amount(
                    self.min_total_spent, metrics_currency
                )
                if metrics.total_spent.amount < min_amount:
                    return False

            if self.max_total_spent:
                max_amount = self._get_comparable_amount(
                    self.max_total_spent, metrics_currency
                )
        ...
        if self.min_days_since_last_purchase and metrics.days_since_last_purchase < self.min_days_since_last_purchase:
            return False
        if self.max_days_since_last_purchase and metrics.days_since_last_purchase > self.max_days_since_last_purchase:
            return False
...
class LTVSettings(models.Model):
    ...
    last_calculation_run = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When LTV was last calculated for all customers"
    )
...
class ProductCategoryLTVMultiplier(models.Model):
    ...
    notes = models.TextField(
        blank=True,
        help_text="Internal notes about this category's purchase behavior"
    )
...
class CustomerCohort(models.Model):
    ...
    cohort_date = models.DateField(
        help_text="First day of the acquisition month (YYYY-MM-01)"
    )

    acquisition_channel = models.CharField(
        max_length=50,
        help_text="Acquisition channel from Order.source"
    )

    first_product_category = models.CharField(
        max_length=100,
        blank=True,
        help_text="Category of customer's first purchase"
    )

    customer_count = models.PositiveIntegerField(
        default=0,
        help_text="Total customers in this cohort"
    )
...
class CohortMetrics(models.Model):
    ...
    active_customers = models.PositiveIntegerField(
        default=0,
        help_text="Number of customers who made a purchase in this period"
    )
    ...
    cumulative_orders = models.PositiveIntegerField(
        default=0,
        help_text="Total orders placed by cohort up to this point"
    )
    ...
    retention_rate = models.FloatField(
        default=0.0,
        help_text="Percentage of customers still active (0.0 to 1.0)"
    )
...
class CustomerNote(models.Model):
    ...
    is_internal = models.BooleanField(
        default=True, 
        help_text="Internal notes are only visible to staff"
    )
