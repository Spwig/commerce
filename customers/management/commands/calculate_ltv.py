...
class Command(BaseCommand):
    ...
    def calculate_single_user(self, user_id, method, use_async):
        ...
        if use_async:
            ...
            if method == "simple":
                ...
                self.stdout.write(self.style.SUCCESS("✓ LTV calculated using simple RFM method"))
                ...
                if metrics and metrics.cohort_month:
                    ...
                    self.stdout.write(self.style.SUCCESS("✓ LTV calculated using cohort method"))
                else:
                    self.stdout.write(
                ...
                if not quality["can_use_probabilistic"]:
                    ...
                    self.stdout.write(
                        f"  Models fitted on {fit_result['customers_used']} customers"
                    )
    ...
    def calculate_all_customers(self, method, use_async):
        ...
        if use_async:
            ...
            if method == "simple":
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Updated {update_result['customers_updated']} customers")
                )
    ...
    def check_data_quality(self):
        ...
        if quality["can_use_probabilistic"]:
            ...
            self.stdout.write(
                self.style.WARNING(f"⚠ Data quality is insufficient: {quality['recommendation']}")
            )
            self.stdout.write(
                f"  Need at least {quality['min_threshold']} repeat customers and 180 days of history"
            )
    ...
    def rebuild_cohorts(self):
        ...
        duration = (timezone.now() - start_time).total_seconds()
        self.stdout.write(self.style.SUCCESS(f"✓ Cohorts rebuilt in {duration:.2f} seconds"))
