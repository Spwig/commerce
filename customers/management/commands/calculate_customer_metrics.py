...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for index, user in enumerate(users, 1):
            try:
                ...
                if not force and hasattr(user, "customer_metrics"):
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"[{index}/{total_users}] Skipped {user.username} (metrics exist)"
                        )
                    )
                    continue
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"[{index}/{total_users}] ✗ Error processing {user.username}: {str(e)}"
                    )
                )
        ...
        if show_segments and segment_distribution:
            ...
            for segment_name, count in sorted(
                segment_distribution.items(), key=lambda x: x[1], reverse=True
            ):
