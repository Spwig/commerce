...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (inviteBtn) {
    ...
    inviteBtn.addEventListener('click', async function () {
      const confirmMsg =
        inviteBtn.getAttribute('data-confirm-msg') ||
        'Send an account registration invitation to this guest customer?';
      ...
      fetch(actionUrl, {
        ...
        .then(data => {
          const successMsg =
            inviteBtn.getAttribute('data-success-msg') || 'Invitation sent successfully!';
          ...
          const defaultMsg =
            inviteBtn.getAttribute('data-default-msg') || 'Send Account Invitation';
        ...
        .catch(error => {
          ...
          const errorOccurredMsg =
            inviteBtn.getAttribute('data-error-occurred-msg') || 'An error occurred';
          const defaultMsg =
            inviteBtn.getAttribute('data-default-msg') || 'Send Account Invitation';
  ...
  if (convertBtn) {
    ...
    convertBtn.addEventListener('click', async function () {
      const confirmMsg =
        convertBtn.getAttribute('data-confirm-msg') ||
        'Are you sure you want to convert this customer to an affiliate?';
      if (!(await AdminModal.confirm(confirmMsg))) {
        return;
      }
      ...
      fetch(actionUrl, {
        ...
        .then(data => {
          if (data.success) {
            ...
            const errorMsg =
              convertBtn.getAttribute('data-error-msg') || 'Failed to convert to affiliate';
            const defaultMsg =
              convertBtn.getAttribute('data-default-msg') || 'Convert to Affiliate';
            AdminModal.alert({ message: data.message || errorMsg, type: 'error' });
            this.disabled = false;
        ...
        .catch(error => {
          console.error('Error:', error);
          const errorOccurredMsg =
            convertBtn.getAttribute('data-error-occurred-msg') || 'An error occurred';
    ...
    addNoteForm.addEventListener('submit', function (e) {
      ...
      fetch(addNoteUrl, {
        ...
        .catch(error => {
          console.error('Error:', error);
          const errorOccurredMsg =
            addNoteForm.getAttribute('data-error-occurred-msg') || 'An error occurred';
          AdminModal.alert({ message: errorOccurredMsg, type: 'error' });
        });
    });
  }
      ...
      fetch(`/api/customers/admin/addresses/${addressId}/set-default/`, {
        ...
        .then(data => {
          if (data.success) {
            ...
            const errorMsg =
              this.getAttribute('data-error-msg') || 'Failed to set address as default';
            AdminModal.alert({ message: data.message || errorMsg, type: 'error' });
            this.disabled = false;
        ...
        .catch(error => {
          console.error('Error:', error);
          const errorOccurredMsg =
            this.getAttribute('data-error-occurred-msg') ||
