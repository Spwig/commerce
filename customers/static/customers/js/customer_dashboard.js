...
(function () {
    ...
    var cohortComparisonChart;
    var retentionCurveChart;
    var ltvByChannelChart;
    var ltvByCategoryChart;
    var revenueCurveChart;
    var chartData = null;

    function init() {
        var dataEl = document.getElementById('customer-dashboard-data');
        if (dataEl) {
            try {
        ...
        fetch(cfg.analyticsApiUrl)
            ...
            .then(function (data) {
                ...
                if (data.churn_risk_insights && data.churn_risk_insights.length > 0) {
                    displayInsights(data.churn_risk_insights, 'churnRiskInsightsPanel', 'churnRiskInsightsList');
                }
                ...
                if (data.frequency_histogram_insights && data.frequency_histogram_insights.length > 0) {
                    displayInsights(data.frequency_histogram_insights, 'frequencyInsightsPanel', 'frequencyInsightsList');
                }
                ...
                if (data.prediction_insights && data.prediction_insights.length > 0) {
                    displayInsights(data.prediction_insights, 'predictionInsightsPanel', 'predictionInsightsList');
                }
            });

        var refreshBtn = document.getElementById('refreshMetrics');
        if (refreshBtn) {
    ...
    function renderSegmentsChart(data) {
        var segmentsCtx = document.getElementById('segmentsChart').getContext('2d');
        new Chart(segmentsCtx, {
            type: 'doughnut',
            data: {
                labels: data.segment_revenue.map(function (s) { return s.name; }),
                datasets: [{
                    data: data.segment_revenue.map(function (s) { return s.revenue; }),
                    backgroundColor: data.segment_revenue.map(function (s) { return s.color; })
                }]
            },
            options: {
                responsive: true,
    ...
    function renderRevenueBySegmentChart(segmentData) {
        var revenueChartCtx = document.getElementById('revenueBySegmentChart').getContext('2d');
        var segments = segmentData.segments;
        var labels = segments.map(function (s) { return s.name; });
        var revenues = segments.map(function (s) { return s.revenue; });
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                var segment = segments[context.dataIndex];
                                return [
                                    t('revenue') + ': ' + segmentData.currency + context.raw.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}),
    ...
    function displaySegmentInsights(insights) {
        var panel = document.getElementById('segmentInsightsPanel');
        var list = document.getElementById('segmentInsightsList');
        ...
        insights.forEach(function (insight) {
            var typeClass = 'insight-' + insight.type;
            html +=
    ...
    function renderChurnRiskChart(data) {
        var ctx = document.getElementById('churnRiskChart').getContext('2d');
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderColor: buckets.map(function (b) { return b.color.replace('0.8', '1'); }),
                    borderWidth: 1
                }]
            },
            options: {
    ...
    function renderFrequencyHistogram(data) {
        var ctx = document.getElementById('frequencyHistogramChart').getContext('2d');
        var histogram = data.histogram;

        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: histogram.map(function (h) { return h.label; }),
                datasets: [{
                    label: t('customers'),
                    data: histogram.map(function (h) { return h.count; }),
                    backgroundColor: histogram.map(function (h) { return h.color; }),
                    borderColor: histogram.map(function (h) { return h.color.replace('0.7', '1'); }),
                    borderWidth: 1
                }]
            },
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                var bucket = histogram[context.dataIndex];
                                return [
                                    t('customers') + ': ' + bucket.count,
                                    t('percentage') + ': ' + bucket.percentage.toFixed(1) + '%'
                                ];
    ...
    function renderPredictionAccuracyChart(data) {
        var ctx = document.getElementById('predictionAccuracyChart').getContext('2d');
        var scatterData = data.scatter_data || [];

        var accuracyLabel = t('accuracy') + ': ' +
            (data.accuracy_percentage ? data.accuracy_percentage.toFixed(1) : 'N/A') + '% | ' +
            t('sample') + ': ' + (data.sample_size || 0) + ' ' + t('customersLower');
        ...
        new Chart(ctx, {
            ...
            data: {
                datasets: [
                    {
                        label: t('customers'),
                        data: scatterData.map(function (d) { return { x: d.predicted, y: d.actual }; }),
                        ...
                        callbacks: {
                            label: function (context) {
                                if (context.dataset.label === t('customers')) {
                                    return t('predicted') + ': ' + context.parsed.x.toFixed(1) + ', ' + t('actual') + ': ' + context.parsed.y.toFixed(0);
                                }
        ...
        .then(function (data) {
            if (data.success) {
                ...
                if (data.category_insights && data.category_insights.length > 0) {
                    displayInsights(data.category_insights, 'categoryInsightsPanel', 'categoryInsightsList');
                }
                ...
                if (data.revenue_curve_insights && data.revenue_curve_insights.length > 0) {
                    displayInsights(data.revenue_curve_insights, 'revenueCurveInsightsPanel', 'revenueCurveInsightsList');
                }
            } else {
    ...
    function renderComparisonChart(chartType) {
        ...
        var ctx = document.getElementById('cohortComparisonChart').getContext('2d');
    ...
    function renderRetentionHeatmap(heatmapData) {
        var container = document.getElementById('retentionHeatmapContainer');

        if (!heatmapData || !heatmapData.cohorts || heatmapData.cohorts.length === 0) {
            container.innerHTML = '<div class="empty-state"><i class="fas fa-inbox"></i><p>' + t('noRetentionData') + '</p></div>';
            return;
        }

        var html = '<div class="table-responsive"><table class="table heatmap-table"><thead><tr>';
        html += '<th>' + t('cohortLabel') + '</th>';

        for (var i = 0; i < heatmapData.months.length; i++) {
        ...
        heatmapData.cohorts.forEach(function (cohort, rowIndex) {
            ...
            heatmapData.data[rowIndex].forEach(function (value) {
                if (value !== null) {
                    var colorClass = getHeatmapColor(value);
                    var textColor = value > 50 ? '#fff' : '#333';
                    html += '<td class="heatmap-cell ' + colorClass + '" style="color: ' + textColor + '">' + value.toFixed(0) + '%</td>';
                } else {
        ...
        html +=
            '<div class="heatmap-legend">' +
                '<span class="legend-label">' + t('low') + '</span>' +
                '<div class="legend-gradient"></div>' +
                '<span class="legend-label">' + t('high') + '</span>' +
            '</div>';
    ...
    function getHeatmapColor(value) {
        if (value >= 80) { return 'heatmap-excellent'; }
        if (value >= 60) { return 'heatmap-good'; }
        if (value >= 40) { return 'heatmap-moderate'; }
        if (value >= 20) { return 'heatmap-low'; }
        return 'heatmap-critical';
    }

    function displayRetentionInsights(insights) {
        var panel = document.getElementById('retentionInsightsPanel');
        var list = document.getElementById('retentionInsightsList');
    ...
    function renderLTVByChannelChart(data) {
        var ctx = document.getElementById('ltvByChannelChart').getContext('2d');
        ...
        if (!channels || channels.length === 0) {
            document.getElementById('ltvByChannelChart').parentElement.innerHTML =
                '<div class="empty-state"><i class="fas fa-inbox"></i><p>' + t('noChannelData') + '</p></div>';
            return;
        }
        ...
        ltvByChannelChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: channels.map(function (c) { return c.display_name; }),
                datasets: [{
                    label: t('averageLtv'),
                    data: channels.map(function (c) { return c.average_ltv; }),
                    backgroundColor: channels.map(function (c) { return c.color; }),
                    borderColor: channels.map(function (c) { return c.color.replace('0.8', '1'); }),
                    borderWidth: 1
                }]
            },
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                var channel = channels[context.dataIndex];
                                return [
                                    t('averageLtv') + ': $' + channel.average_ltv.toFixed(2),
                                    t('customers') + ': ' + channel.customer_count,
                                    t('totalRevenue') + ': $' + channel.total_revenue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})
                                ];
                            }
                        }
                    }
    ...
    function renderLTVByCategoryChart(data) {
        var ctx = document.getElementById('ltvByCategoryChart').getContext('2d');
        ...
        if (!categories || categories.length === 0) {
            document.getElementById('ltvByCategoryChart').parentElement.innerHTML =
                '<div class="empty-state"><i class="fas fa-inbox"></i><p>' + t('noCategoryData') + '</p></div>';
            return;
        }
        ...
        ltvByCategoryChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: categories.map(function (c) { return c.display_name; }),
                datasets: [{
                    label: t('averageLtv'),
                    data: categories.map(function (c) { return c.average_ltv; }),
                    backgroundColor: categories.map(function (c) { return c.color; }),
                    borderColor: categories.map(function (c) { return c.color.replace('0.7', '1'); }),
                    borderWidth: 1
                }]
            },
            options: {
                        ...
                        callbacks: {
                            label: function (context) {
                                ...
                                return [
                                    ...
                                    t('totalRevenue') + ': $' + category.total_revenue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})
                                ];
                            }
                        }
                    }
    ...
    function renderRevenueCurveChart(data) {
        var ctx = document.getElementById('revenueCurveChart').getContext('2d');
        ...
        if (!curves || curves.length === 0) {
            document.getElementById('revenueCurveChart').parentElement.innerHTML =
                '<div class="empty-state"><i class="fas fa-inbox"></i><p>' + t('noRevenueCurveData') + '</p></div>';
            return;
        }

        var datasets = curves.map(function (curve) {
            return {
                label: curve.cohort,
                data: curve.data.map(function (d) { return d.revenue; }),
    ...
    function setupCohortEventListeners() {
        ...
        document.querySelectorAll('.view-cohort-details').forEach(function (button) {
            button.addEventListener('click', function () {
                ...
                var cohortName = this.dataset.cohortName;
                showRetentionCurve(cohortId, cohortName);
            });
        });

        var rebuildButton = document.getElementById('rebuildCohortsButton');
        if (rebuildButton) {
    ...
    function showRetentionCurve(cohortId, cohortName) {
        document.getElementById('modalCohortName').textContent = cohortName + ' - ' + t('retentionCurve');
    ...
    function renderRetentionCurve(data) {
        var ctx = document.getElementById('retentionCurveChart').getContext('2d');
        ...
        var labels = data.retention_curve.map(function (point) {
            return t('month') + ' ' + point.month;
        });
        var retentionRates = data.retention_curve.map(function (point) {
        ...
        var statsHTML =
            '<div class="stats-grid">' +
                '<div class="stat-item">' +
                    '<span class="stat-label">' + t('cohortSizeLabel') + '</span>' +
                    '<span class="stat-value">' + data.cohort.customer_count + '</span>' +
                '</div>' +
                '<div class="stat-item">' +
                    '<span class="stat-label">' + t('totalRevenueLabel') + '</span>' +
                    '<span class="stat-value">$' + data.cohort.total_revenue.toFixed(2) + '</span>' +
                '</div>' +
                '<div class="stat-item">' +
                    '<span class="stat-label">' + t('avgLtvLabel') + '</span>' +
                    '<span class="stat-value">$' + data.cohort.average_ltv.toFixed(2) + '</span>' +
                '</div>' +
            '</div>';
    ...
    function rebuildCohorts() {
        var button = document.getElementById('rebuildCohortsButton');
