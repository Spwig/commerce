...
class AffiliateStatusFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() in ['pending', 'active', 'suspended', 'rejected']:
            affiliate_users = Affiliate.objects.filter(
                status=self.value()
...
class EnhancedCustomerProfileAdmin(CustomFieldsAdminMixin, admin.ModelAdmin):
    ...
    list_display = [
        'user', 'account_type_display', 'affiliate_status_display', 'customer_value',
        'customer_segment_display', 'total_orders_display', 'days_since_last_order',
        'is_vip_customer'
    ]
    list_filter = [
        AccountTypeFilter,
    ...
    readonly_fields = [
        'created_at', 'updated_at', 'customer_analytics_summary',
        'purchase_behavior_summary', 'engagement_summary'
    ]
    ...
    fieldsets = (
        ...
        (_('Customer Analytics'), {
            'fields': ('customer_analytics_summary', 'purchase_behavior_summary', 'engagement_summary'),
            'classes': ('wide',)
        }),
        (_('Timestamps'), {
    ...
    def send_activation_invitations(self, request, queryset):
        ...
        messages.success(
            request,
            _('Sent %(sent)d activation invitation(s). Skipped %(skipped)d (not guest or no email).') % {
    ...
    def customer_segment_display(self, obj):
        ...
        if segment:
            return format_html(
                ...
                segment.color, segment.display_name
            )
        return '-'
    ...
    def customer_analytics_summary(self, obj):
        ...
        try:
            ...
            cohort_display = metrics.cohort_month.strftime('%b %Y') if metrics.cohort_month else _('None')
        ...
        return format_html(
            ...
            '</div>',
            _('Customer Analytics'),
            _('Total Spent'), obj.total_spent,
            _('Lifetime Value'), obj.lifetime_value,
            _('Total Orders'), obj.total_orders,
            _('Completed Orders'), obj.completed_orders_count,
            _('Average Order Value'), obj.average_order_value,
            _('Purchase Frequency'), f'{obj.purchase_frequency:.1f}', _('month'),
            _('VIP Status'), _("Yes") if obj.is_vip_customer else _("No"),
            _('At Risk'), _("Yes") if obj.is_at_risk else _("No"),
            _('Lifetime Value Analysis'),
            _('LTV Method'), ltv_method,
            _('Confidence'), confidence_class, confidence_display,
            _('Cohort'), cohort_display,
            reverse('customers:ltv_settings'), _('View LTV Settings')
        )
    ...
    def purchase_behavior_summary(self, obj):
        ...
        return format_html(
            ...
            '</div>',
            _('Purchase Behavior'),
            _('Days Since Last Order'), obj.days_since_last_order or _("No orders yet"),
            _('Favorite Categories'), categories_str,
            _('Abandoned Carts'), obj.abandoned_carts_count,
            _('Customer Segment'), obj.customer_segment.display_name if obj.customer_segment else _("Not classified")
        )
    ...
    def engagement_summary(self, obj):
        ...
        return format_html(
            ...
            _('Wishlist Items'), obj.wishlist_items_count,
            _('Marketing Emails'), _("Yes") if marketing else _("No"),
            _('Order Notifications'), _("Yes") if transactional else _("No")
        )
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if object_id:
            ...
            if customer_profile:
                ...
                # Get customer notes
                extra_context['customer_notes'] = CustomerNote.objects.filter(
    ...
    actions = ['refresh_customer_metrics', 'export_customer_data', 'convert_to_affiliate', 'send_activation_invitations']
    ...
    def export_customer_data(self, request, queryset):
        ...
        writer.writerow([
            str(_('Username')), str(_('Email')), str(_('Full Name')),
            str(_('Phone')), str(_('Total Spent')), str(_('Lifetime Value')),
            str(_('Total Orders')), str(_('Completed Orders')),
            str(_('Last Purchase Date')), str(_('Customer Segment')),
            str(_('VIP Status')), str(_('At Risk')), str(_('Created Date')),
        ])
...
class CustomerMetricsAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'total_spent', 'lifetime_value_display', 'total_orders',
        'completed_orders', 'ltv_method_display', 'ltv_confidence_display',
        'cohort_month_display', 'last_calculated'
    ]
    list_filter = [
        LTVCalculationMethodFilter,
        LTVConfidenceFilter,
        'last_calculated', 'first_purchase_date', 'last_purchase_date', 'cohort_month'
    ]
    search_fields = ['user__username', 'user__email', 'user__first_name', 'user__last_name']
    readonly_fields = [
        'created_at', 'last_calculated', 'ltv_last_calculated'
    ]
    ...
    fieldsets = (
        ...
        (_('Financial Metrics'), {
            'fields': (
                'total_spent', 'lifetime_value', 'average_order_value'
            )
        }),
        (_('LTV Calculation'), {
            'fields': (
                'ltv_calculation_method', 'ltv_confidence_score', 'ltv_last_calculated',
                'cohort_month'
            ),
        ...
        (_('Probabilistic LTV Fields'), {
            'fields': (
                'probability_alive', 'predicted_purchases_12m', 'predicted_purchases_24m'
            ),
            'classes': ('collapse',),
        ...
        (_('Cart & Product Insights'), {
            'fields': (
                'abandoned_carts_count', 'cart_abandonment_rate',
                'favorite_category', 'most_purchased_product'
            ),
            'classes': ('collapse',)
        }),
        (_('Engagement & Risk'), {
            'fields': (
                'total_sessions', 'wishlist_items_count', 'reviews_count',
                'refund_rate', 'support_tickets_count'
            ),
            'classes': ('collapse',)
        }),
        (_('Timestamps'), {
    ...
    def lifetime_value_display(self, obj):
        ...
        return format_html(
            ...
            css_class, icon, currency, formatted_amount
        )
    ...
    def ltv_method_display(self, obj):
        ...
        return format_html(
            '<span class="customer-list-badge-sm {}">{}</span>',
            css_class, label
        )
    ...
    def ltv_confidence_display(self, obj):
        ...
        return format_html(
            '<span class="{}" title="{}">'
            '<i class="fas {}"></i> {}%</span>',
            css_classes[level], _('%s confidence') % level.capitalize(),
            icons[level], formatted_score
        )
    ...
    def recalculate_metrics(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f"Recalculated metrics for {updated} customer(s)."),
            level=messages.SUCCESS
        )
    ...
    def recalculate_ltv_all_methods(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f"Queued LTV recalculation for {len(task_ids)} customer(s) using {method} method. "
...
class CustomerSegmentAdmin(MoneyFieldCurrencyMixin, admin.ModelAdmin):
    ...
    list_display = [
        'display_name', 'name', 'color_preview', 'priority',
    ...
    def color_preview(self, obj):
        return format_html(
            '<div class="color-swatch" style="--swatch-color: {}"></div>',
            obj.color
        )
    ...
    def customer_count(self, obj):
        ...
        if obj.name == 'guest':
            return User.objects.filter(
                is_active=True, username__startswith='guest_'
            ).count()
...
class AbandonedCartAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'user', 'abandoned_at', 'total_value', 'total_items',
        'estimated_reason', 'recovery_status', 'days_since_abandonment'
    ]
    list_filter = [
        'estimated_reason', 'recovered', 'abandoned_at'
    ]
    search_fields = ['user__username', 'user__email', 'user__first_name', 'user__last_name']
    ...
    fieldsets = (
        ...
        (_('Recovery Tracking'), {
            'fields': (
                'recovery_emails_sent', 'recovered', 'recovered_at', 'recovery_order'
            ),
    ...
    class Media:
        css = {'all': ('customers/css/abandonedcart_change_form.css', 'customers/css/customer_admin_list.css')}
    ...
    def recovery_status(self, obj):
        if obj.recovered:
            ...
            return format_html(
                ...
                obj.recovery_emails_sent, _('emails sent')
            )
        else:
            return format_html(
    ...
    def send_recovery_email(self, request, queryset):
        ...
        eligible = queryset.filter(
            recovered=False, user__email__isnull=False
        ).exclude(user__email='')
        sent = 0
        errors = 0
        ...
        for cart in eligible.select_related('user'):
            try:
                ...
                subject = str(
                    _("%(name)s, you left items in your cart!") % {'name': name}
                )
                html_body = (
...
class CustomerNoteAdmin(admin.ModelAdmin):
    list_display = [
        'customer', 'note_type', 'title', 'requires_follow_up',
        'follow_up_date', 'completed', 'created_by', 'created_at'
    ]
    list_filter = [
        'note_type', 'requires_follow_up', 'completed', 'is_internal', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        (_('Follow-up'), {
            'fields': ('requires_follow_up', 'follow_up_date', 'completed')
        }),
        (_('Visibility'), {
            'fields': ('is_internal',)
        }),
