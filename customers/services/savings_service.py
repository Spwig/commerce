...
class SavingsService:
    ...
    def calculate_total_savings(user) -> dict[str, Any]:
        ...
        orders = Order.objects.filter(user=user, status__in=["processing", "shipped", "delivered"])

        # Calculate total savings from order-level discount_amount
        total_saved = _to_decimal(orders.aggregate(total=Sum("discount_amount"))["total"])

        # Count orders with savings
        orders_with_savings = orders.filter(discount_amount__gt=0).count()
        ...
        try:
            ...
            voucher_qs = AppliedVoucher.objects.filter(order__in=orders, order__isnull=False)
            if loyalty_voucher_ids:
                ...
                loyalty_savings = _to_decimal(
                    AppliedVoucher.objects.filter(
                        order__in=orders, voucher_id__in=loyalty_voucher_ids
        ...
        sale_savings = max(total_saved - voucher_savings - loyalty_savings, Decimal("0.00"))
    ...
    def get_savings_history(user, limit: int = 10) -> list[dict[str, Any]]:
        ...
        orders = (
            Order.objects.filter(
                user=user, discount_amount__gt=0, status__in=["processing", "shipped", "delivered"]
            )
            .prefetch_related("applied_vouchers", "loyalty_redemptions", "items")
        ...
        for order in orders:
            ...
            savings_pct = (
                (discount_amt / original_total * 100) if original_total > 0 else Decimal("0")
            )
            ...
            [
                {
                    ...
                    "savings_rate": float(data["orders_with_savings"] / data["orders_count"] * 100)
    ...
    def get_best_savings_month(user) -> dict[str, Any]:
        ...
        best_month = max(monthly_savings, key=lambda x: Decimal(x["total_savings"]))
    ...
    def calculate_potential_savings(user) -> dict[str, Any]:
        ...
        try:
            ...
            has_orders = Order.objects.filter(user=user, status="delivered").exists()
            if has_orders:
                active_vouchers = active_vouchers.exclude(first_time_customers_only=True)
        ...
        try:
            ...
            if wishlist:
                on_sale_items = (
                    ...
                    .filter(
                        Q(product__sale_start_date__lte=now)
                        | Q(product__sale_start_date__isnull=True),
                    )
                    .filter(
        ...
        try:
            ...
            member = (
                LoyaltyMember.objects.filter(customer=user, is_active=True)
    ...
    def get_savings_comparison(user) -> dict[str, Any]:
        ...
        platform_stats = Order.objects.filter(
            status__in=["processing", "shipped", "delivered"], discount_amount__gt=0
        ).aggregate(
        ...
        if user_total > 0:
            ...
            customers_below = (
                Order.objects.filter(status__in=completed_statuses, discount_amount__gt=0)
                .values("user")
                .annotate(total_discount=Sum("discount_amount"))
                .filter(total_discount__lt=user_total)
                .count()
            )
