...
class RecommendationService:
    ...
    def get_quick_recommendations(user, limit: int = 5) -> list[dict[str, Any]]:
        ...
        if len(recommendations) < limit:
            trending = (
                ...
                .filter(_in_stock_q())
                .distinct()
                .order_by("-views_count")[: limit - len(recommendations)]
            )
    ...
    def get_purchase_recommendations(user) -> dict[str, list[dict[str, Any]]]:
        ...
        if purchased_products:
            ...
            purchased_categories = (
                Product.objects.filter(id__in=purchased_products)
        ...
        if favorite_categories:
            ...
            trending_products = (
                Product.objects.filter(status="published")
                .filter(category__name__in=favorite_categories, created_at__gte=thirty_days_ago)
                .filter(_in_stock_q())
                .exclude(id__in=purchased_products)
                .distinct()
                ...
                back_in_stock.append(
                    {
                        ...
                        "discount_price": item.product.effective_price
        ...
        sale_products = (
            Product.objects.filter(sale_filter)
            .filter(_in_stock_q())
            .exclude(id__in=purchased_products)
            .distinct()
    ...
    def get_favorite_categories(user, limit: int = 5) -> list[dict[str, Any]]:
        ...
        category_purchases = (
            ...
            .values("product__category__id", "product__category__name", "product__category__slug")
            .annotate(
    ...
    def get_favorite_brands(user, limit: int = 5) -> list[dict[str, Any]]:
        ...
        brand_purchases = (
            ...
            .values("product__brand__id", "product__brand__name", "product__brand__slug")
            .annotate(
                order_count=Count("order", distinct=True),
                items_purchased=Count("id"),
                total_spent=Sum("total_price"),
