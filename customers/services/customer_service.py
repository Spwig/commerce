...
from ..models import AbandonedCart, CustomerMetrics, CustomerSegment, LTVSettings
...
class CustomerService:
    ...
    def get_dashboard_summary(user) -> dict[str, Any]:
        ...
        # Check for alerts
        abandoned_carts_count = AbandonedCart.objects.filter(user=user, recovered=False).count()

        # Get wishlist items
        wishlist = Wishlist.objects.filter(user=user).first()
        wishlist.items.count() if wishlist else 0

        # Calculate total saved from orders
        total_saved = Order.objects.filter(
            user=user, status__in=["processing", "shipped", "delivered"], discount_amount__gt=0
        ...
        return {
            ...
            "loyalty_points": int(
                (
                    ...
                    if hasattr(metrics.total_spent, "amount")
                    else metrics.total_spent
                )
                / 10
            ),  # Simple: 1 point per $10
    ...
    def get_order_statistics(user) -> dict[str, Any]:
        ...
        item_stats = OrderItem.objects.filter(
            order__user=user, order__status="delivered"
        ).aggregate(total_items=Sum("quantity"), unique_products=Count("product", distinct=True))
        ...
        try:
            ...
            total_returns = (
                ReturnRequest.objects.filter(user=user).exclude(status="cancelled").count()
            )
        except Exception:
            pass
    ...
    def _get_favorite_discount_type(user) -> str:
        ...
        gift_card_count = Order.objects.filter(user=user, gift_card_discount__gt=0).count()
        ...
        try:
            ...
            sale_count = (
                OrderItem.objects.filter(order__user=user, discount_value__gt=0)
                .exclude(discount_type="none")
                .values("order")
                .distinct()
                .count()
            )
        ...
        try:
            ...
            loyalty_count = LoyaltyRedemption.objects.filter(
                member__customer=user, order__isnull=False, status__in=["confirmed", "fulfilled"]
            ).count()
            if loyalty_count > 0:
    ...
    def get_spending_insights(user) -> dict[str, Any]:
        ...
        if top_brands and metrics.total_orders > 0:
            top_brand_orders = (
                OrderItem.objects.filter(order__user=user, product__brand__name=top_brands[0])
                .values("order")
                .distinct()
                .count()
            )
        ...
        # Discount usage
        orders_with_discounts = Order.objects.filter(user=user, discount_amount__gt=0).count()

        discount_rate = (
            (orders_with_discounts / metrics.total_orders * 100) if metrics.total_orders > 0 else 0
        )
    ...
    def get_lifetime_value(user) -> dict[str, Any]:
        ...
        calculated_ltv = (
            float(ltv_val.amount if hasattr(ltv_val, "amount") else ltv_val)
            if ltv_val
            else float(total_val.amount if hasattr(total_val, "amount") else total_val)
        )
        ...
        if metrics.purchase_frequency and metrics.average_order_value:
            ...
            simple_projected_ltv = metrics.total_spent + (
                projected_orders * metrics.average_order_value
            )
        else:
            simple_projected_ltv = metrics.total_spent
        ...
        result = {
            ...
            # Customer history
            "customer_since": (
                metrics.first_purchase_date.date()
                if metrics.first_purchase_date
            ...
            "cohort_month": metrics.cohort_month.strftime("%Y-%m")
            if metrics.cohort_month
            else None,
        }
        ...
        if calculation_method == "probabilistic":
            result.update(
                {
                    "probability_alive": float(metrics.probability_alive)
                    if metrics.probability_alive
                    else None,
                    "predicted_purchases_12m": float(metrics.predicted_purchases_12m)
                    if metrics.predicted_purchases_12m
                    else None,
                    "predicted_purchases_24m": float(metrics.predicted_purchases_24m)
                    if metrics.predicted_purchases_24m
                    else None,
                }
            )
    ...
    def get_loyalty_status(user) -> dict[str, Any]:
        ...
        benefits_map = {
            ...
            "Bronze": ["5% off first purchase", "Member-only deals"],
        }
    ...
    def get_revenue_by_segment() -> dict[str, Any]:
        ...
        users_with_metrics = (
            User.objects.filter(is_active=True, customer_metrics__isnull=False)
            .exclude(username__startswith="guest_")
            .select_related("customer_metrics")
        )
    ...
    def get_segment_insights() -> list[dict[str, Any]]:
        ...
        if new.get("customer_count", 0) > 0 and segment_data["total_customers"] > 10:
            ...
            conversion_rate = (
                (converted / (new_count + converted) * 100) if (new_count + converted) > 0 else 0
            )
        ...
        healthy_revenue = sum(segments.get(s, {}).get("revenue", 0) for s in healthy_segments)
        healthy_pct = (
            (healthy_revenue / segment_data["total_revenue"] * 100)
            if segment_data["total_revenue"] > 0
            else 0
        )
    ...
    def get_churn_risk_distribution() -> dict[str, Any]:
        ...
        for metric in metrics:
            ...
            for _key, bucket in risk_buckets.items():
    ...
    def get_predicted_vs_actual() -> dict[str, Any]:
        ...
        metrics = CustomerMetrics.objects.exclude(user__username__startswith="guest_").filter(
            ltv_last_calculated__lte=twelve_months_ago, predicted_purchases_12m__gt=0
        )
        ...
        if comparison_data:
            ...
            accuracy = min(100, total_actual / total_predicted * 100) if total_predicted > 0 else 0
            ...
            insights.append(
                {
                    ...
                    "description": "Your purchase predictions are highly accurate. "
                    "You can confidently use these for inventory and marketing planning.",
                    "priority": 2,
                }
            )
        elif accuracy >= 50:
            insights.append(
                {
                    ...
                    "title": f"{accuracy:.0f}% Prediction Accuracy",
                    "description": "Predictions are moderately accurate. Consider enriching customer "
                    "data or adjusting prediction models.",
                    "priority": 2,
                }
            )
        else:
