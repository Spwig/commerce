...
class ProbabilisticLTVService:
    ...
    def check_data_quality(cls):
        ...
        order_span = Order.objects.filter(status="delivered").aggregate(
        ...
        can_use = (
            repeat_customers >= min_threshold and data_span_days >= 180  # At least 6 months of data
        )
        ...
        return {
            ...
            "quality_score": min(
                1.0, repeat_customers / max(min_threshold, 1) * min(1.0, data_span_days / 365)
            ),
        }
    ...
    def prepare_transaction_data(cls):
        ...
        customers_with_orders = CustomerMetrics.objects.filter(
            completed_orders__gte=1, first_purchase_date__isnull=False
        ).select_related("user")
        ...
        for metrics in customers_with_orders:
            ...
            orders = Order.objects.filter(user=metrics.user, status="delivered").order_by(
                "created_at"
            )
    ...
    def fit_models(self):
        ...
        try:
            ...
            return {"success": False, "error": "lifetimes library not installed"}
        ...
        try:
            ...
            self.bgf_model.fit(df["frequency"], df["recency"], df["T"])
            ...
            if len(repeat_customers) >= 10:
                ...
                self.ggf_model.fit(
                    repeat_customers["frequency"], repeat_customers["monetary_value"]
                )
            else:
                self.ggf_model = None
    ...
    def predict_customer_ltv(self, user, time_horizon_days=365):
        ...
        if not self.bgf_model:
            return {"success": False, "error": "Models not fitted. Call fit_models() first."}

        try:
            metrics = CustomerMetrics.objects.get(user=user)
        except CustomerMetrics.DoesNotExist:
            return {"success": False, "error": "Customer metrics not found"}
        ...
        if T <= 0:
            return {"success": False, "error": "Invalid customer age"}
        ...
        predicted_purchases = float(
            self.bgf_model.conditional_expected_number_of_purchases_up_to_time(
                time_horizon_days, frequency, recency, T
            )
        )
    ...
    def update_all_customer_ltv(self):
        ...
        customers = CustomerMetrics.objects.filter(completed_orders__gte=1).select_related("user")
    ...
    def get_customer_predictions(cls, user):
        ...
        return {
            ...
            "confidence_level": (
                "High"
                if metrics.ltv_confidence_score >= 0.7
                else "Medium"
                if metrics.ltv_confidence_score >= 0.4
                else "Low"
            ),
            "ltv": float(metrics.lifetime_value.amount),
            "historical_value": float(metrics.total_spent.amount),
            "predicted_future_value": float(metrics.lifetime_value.amount)
            - float(metrics.total_spent.amount),
