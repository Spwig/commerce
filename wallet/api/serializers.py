...
class WalletBalanceSerializer(serializers.ModelSerializer):
    ...
    available_balance = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='available_balance.amount',
    )
    available_balance_currency = serializers.CharField(
        source='available_balance.currency', read_only=True,
    )
    pending_balance = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='pending_balance.amount',
    )
    pending_balance_currency = serializers.CharField(
        source='pending_balance.currency', read_only=True,
    )
    lifetime_credited = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='lifetime_credited.amount',
    )
    lifetime_used = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='lifetime_used.amount',
    )
...
class WalletTransactionListSerializer(serializers.ModelSerializer):
    ...
    amount = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='amount.amount',
    )
    amount_currency = serializers.CharField(
        source='amount.currency', read_only=True,
    )
    balance_after = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='balance_after.amount',
    )
    balance_after_currency = serializers.CharField(
        source='balance_after.currency', read_only=True,
    )
...
class CustomerWalletSerializer(serializers.ModelSerializer):
    ...
    lifetime_credited_currency = serializers.CharField(
        source='lifetime_credited.currency', read_only=True,
    )
    lifetime_used = serializers.DecimalField(
        max_digits=12, decimal_places=2, source='lifetime_used.amount',
    )
    lifetime_used_currency = serializers.CharField(
        source='lifetime_used.currency', read_only=True,
    )
