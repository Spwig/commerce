...
const VersionHistory = (function () {
  ...
  function loadI18n() {
    const el = document.getElementById('version-history-i18n');
    if (!el) return;
    try {
      msgs = JSON.parse(el.textContent);
    } catch (e) {
      /* ignore */
    }
  }
  ...
  function fetchVersions(slug) {
    ...
    const langPrefix =
      typeof AdminUtils !== 'undefined' && AdminUtils.getLanguagePrefix
        ? AdminUtils.getLanguagePrefix()
        : '/' + (document.documentElement.lang || 'en');
    const url =
      langPrefix +
      '/admin/component_updates/componentregistry/version-history/' +
  ...
  function renderTimeline(versions) {
    const container = makeEl('div', 'version-history-timeline');
    ...
    versions.forEach(function (v) {
      const entry = makeEl('div', 'version-history-entry');

      // Header row: version tag + badges + date
      const header = makeEl('div', 'version-history-header');

      header.appendChild(makeEl('span', 'version-history-tag', 'v' + v.version));

      if (v.security_update) {
        const secBadge = makeEl(
          'span',
          'version-history-badge security',
          i18n('security', 'Security')
        );
        header.appendChild(secBadge);
      }
      if (v.breaking_changes) {
        const breakBadge = makeEl(
          'span',
          'version-history-badge breaking',
          i18n('breaking', 'Breaking Changes')
        );
      ...
      if (v.published_at) {
        try {
          const dateStr = new Date(v.published_at).toLocaleDateString(undefined, {
            year: 'numeric',
            month: 'short',
            day: 'numeric',
          });
      ...
      // Changelog body
      const changelogText = v.changelog || v.release_notes || '';
      if (changelogText) {
        const body = makeEl('div', 'version-history-body');
