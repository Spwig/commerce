...
def filter_search_queries(request):
        ...
        queries_data.append(
            {
                ...
                "zero_rate": round(item["zero_count"] / item["search_count"] * 100, 1)
    ...
    html = render_to_string(
        "admin/search/partials/query_cards.html", {"queries": queries_data}, request=request
    )
...
def dashboard_data(request):
    ...
    stats = analytics.get_dashboard_stats(date_from=from_date, date_to=to_date, engine_slug=engine)
...
def filter_synonyms(request):
    ...
    search = request.GET.get("search", "").strip()
    if search:
        queryset = queryset.filter(Q(term__icontains=search) | Q(synonyms__icontains=search))
    ...
    html = render_to_string(
        "admin/search/partials/synonym_cards.html", {"synonyms": synonyms}, request=request
    )
...
def filter_redirects(request):
    ...
    search = request.GET.get("search", "").strip()
    if search:
        queryset = queryset.filter(Q(term__icontains=search) | Q(redirect_url__icontains=search))
    ...
    html = render_to_string(
        "admin/search/partials/redirect_cards.html", {"redirects": redirects}, request=request
    )
...
def export_analytics(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="search_analytics_{from_date}_{to_date}.csv"'
    )
    return response
