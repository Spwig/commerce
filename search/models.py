...
class SearchSettings(models.Model):
    ...
    autocomplete_product_price = models.BooleanField(
        _("Show Product Price"), default=True, help_text=_("Display price in autocomplete dropdown")
    )
    autocomplete_product_sku = models.BooleanField(
        _("Show Product SKU"), default=True, help_text=_("Display SKU in autocomplete dropdown")
    )
    ...
    search_products = models.BooleanField(
        _("Search Products"), default=True, help_text=_("Include products in search results")
    )
    search_categories = models.BooleanField(
        _("Search Categories"), default=True, help_text=_("Include categories in search results")
    )
    search_brands = models.BooleanField(
        _("Search Brands"), default=True, help_text=_("Include brands in search results")
    )
    search_blog_posts = models.BooleanField(
        _("Search Blog Posts"), default=True, help_text=_("Include blog posts in search results")
    )

    # Deep Indexing Settings
    index_skus = models.BooleanField(
        _("Index SKUs"), default=True, help_text=_("Include product SKUs in search")
    )
    ...
    index_reviews = models.BooleanField(
        _("Index Reviews"), default=True, help_text=_("Include product review content in search")
    )
    ...
    track_clicks = models.BooleanField(
        _("Track Clicks"), default=True, help_text=_("Track clicks on search results for analytics")
    )
    ...
    results_per_page = models.PositiveIntegerField(
        _("Results Per Page"), default=20, help_text=_("Number of results to display per page")
    )
...
class SearchEngine(models.Model):
    ...
    name = models.CharField(
        _("Name"), max_length=100, help_text=_('Engine name (e.g., "Shop Search", "Blog Search")')
    )
    slug = models.SlugField(
        _("Slug"), max_length=100, unique=True, help_text=_("URL-safe identifier")
    )
    is_active = models.BooleanField(
        _("Active"), default=True, help_text=_("Enable or disable this search engine")
    )
    content_types = models.JSONField(
        ...
        blank=True,
        help_text=_(
            'List of searchable content types for this engine (e.g., ["product", "category"])'
        ),
    )
...
class Synonym(models.Model):
    ...
    term = models.CharField(_("Term"), max_length=100, help_text=_("Original search term"))
...
class SearchRedirect(models.Model):
    ...
    term = models.CharField(
        _("Search Term"), max_length=200, help_text=_("Search term to trigger redirect")
    )
    ...
    hit_count = models.PositiveIntegerField(
        _("Hit Count"), default=0, help_text=_("Number of times this redirect was triggered")
    )
    engine = models.ForeignKey(
        SearchEngine,
        on_delete=models.CASCADE,
    ...
    def increment_hit_count(self):
        ...
        SearchRedirect.objects.filter(pk=self.pk).update(hit_count=models.F("hit_count") + 1)
...
class SearchQuery(models.Model):
    ...
    query = models.CharField(_("Query"), max_length=500, help_text=_("Original search query"))
    ...
    result_count = models.PositiveIntegerField(
        _("Result Count"), default=0, help_text=_("Number of results returned")
    )
    ...
    session_key = models.CharField(
        _("Session Key"), max_length=40, blank=True, help_text=_("For anonymous user tracking")
    )
    language = models.CharField(
        _("Language"), max_length=10, default="en", help_text=_("Language code of the search")
    )
    engine = models.ForeignKey(
        SearchEngine,
        on_delete=models.SET_NULL,
    ...
    is_zero_result = models.BooleanField(
        _("Zero Results"), default=False, help_text=_("Whether this query returned no results")
    )
    response_time_ms = models.PositiveIntegerField(
        _("Response Time (ms)"), default=0, help_text=_("Search response time in milliseconds")
    )
...
class SearchClick(models.Model):
    ...
    search_query = models.ForeignKey(
        SearchQuery, on_delete=models.CASCADE, related_name="clicks", verbose_name=_("Search Query")
    )
    ...
    position = models.PositiveIntegerField(
        _("Position"), default=0, help_text=_("Position of the clicked item in search results")
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
...
class SearchIndex(models.Model):
    ...
    extracted_text = models.TextField(
        _("Extracted Text"), blank=True, help_text=_("Text content extracted from the document")
    )
    file_type = models.CharField(
        _("File Type"), max_length=10, choices=FILE_TYPE_CHOICES, default="other"
    )
    checksum = models.CharField(
        _("Checksum"), max_length=64, blank=True, help_text=_("File checksum for change detection")
    )
