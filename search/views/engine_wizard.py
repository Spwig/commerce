...
WEIGHT_FIELDS = [
    ...
    (
        ...
        _("Attributes Weight"),
        0.7,
        _("Relevance weight for product attributes"),
    ),
...
class EngineWizardStep3View(WizardSessionMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if use_custom_weights:
            for field_name, _label, default, _help_text in WEIGHT_FIELDS:
...
class EngineWizardStep4View(WizardSessionMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.success(
                request, _('Search engine "%(name)s" created successfully!') % {"name": engine.name}
            )

            return redirect("admin:search_searchengine_changelist")

        except Exception as e:
            messages.error(request, _("Failed to create engine: %(error)s") % {"error": str(e)})
            return redirect("search_admin:engine_wizard_step4")
