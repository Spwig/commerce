...
class SearchAutocomplete {
  ...
  renderResults(data) {
    ...
    if (data.products && data.products.length > 0) {
      this.renderSection(
        this._str('js.search_products', 'Products'),
        data.products,
        'product',
        itemIndex
      );
    ...
    if (data.categories && data.categories.length > 0) {
      this.renderSection(
        this._str('js.search_categories', 'Categories'),
        data.categories,
        'category',
        itemIndex
      );
    ...
    if (data.total_count > 0) {
      ...
      const viewAllText = this._str(
        'js.search_view_all_results',
        'View all {count} results'
      ).replace('{count}', data.total_count);
  ...
  createResultItem(item, type, index) {
    ...
    if (type === 'product') {
      ...
      if (ds.product_stock_status) {
        const stockClass = item.in_stock
      ...
      if (ds.category_product_count && item.product_count !== undefined) {
        const countText = this._str('js.search_products_count', '{count} products').replace(
          '{count}',
          item.product_count
        );
      ...
      if (ds.blog_excerpt && item.excerpt) {
        ...
        const excerpt =
          item.excerpt.length > maxLen ? item.excerpt.substring(0, maxLen) + '...' : item.excerpt;
    ...
    if (item.is_translated) {
      html +=
        '<span class="autocomplete-translated" title="Available in your language"><i class="fas fa-language"></i></span>';
    }
