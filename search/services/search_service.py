...
class SearchService:
    ...
    def settings(self) -> SearchSettings:
        ...
        if self._settings is None or (
            self._settings_loaded_at and time.time() - self._settings_loaded_at > 60
        ):
            self._settings = SearchSettings.get_settings()
            self._settings_loaded_at = time.time()
        return self._settings

    def get_engine(self, engine_slug: str = "shop") -> SearchEngine | None:
    ...
    def _get_cache_key(
        self, prefix: str, query: str, language: str, engine: str = "shop", **kwargs
    ) -> str:
    ...
    def check_redirect(self, query: str, engine_slug: str = "shop") -> dict | None:
        """
        ...
        redirects = SearchRedirect.objects.filter(is_active=True).filter(
    ...
    def expand_synonyms(
        self, query: str, language: str = "en", engine_slug: str = "shop"
    ) -> list[str]:
        """
        ...
        synonyms = (
            Synonym.objects.filter(is_active=True)
            .filter(Q(engine__isnull=True) | Q(engine__slug=engine_slug))
            .filter(Q(language__isnull=True) | Q(language="") | Q(language=language))
        )
    ...
    def _search_products(
        self, query: str, language: str, limit: int = None, engine: SearchEngine = None, **filters
    ) -> list[dict]:
        ...
        for product in products:
            ...
            is_translated = (
                language != "en"
                and hasattr(product, "translations")
                and product.translations
                and language in product.translations
                and "name" in product.translations[language]
            )
    ...
    def _search_categories(
        self, query: str, language: str, limit: int = None, engine: SearchEngine = None
    ) -> list[dict]:
        ...
        for category in categories:
            ...
            is_translated = (
                language != "en"
                and hasattr(category, "translations")
                and category.translations
                and language in category.translations
            )
    ...
    def _search_blog_posts(
        ...
        for post in posts:
            ...
            is_translated = (
                language != "en"
                and hasattr(post, "translations")
                and post.translations
                and language in post.translations
            )
    ...
    def autocomplete(
        self, query: str, language: str = "en", engine_slug: str = "shop", limit: int = None
    ) -> dict[str, Any]:
        """
    ...
    def track_click(
        ...
        try:
            ...
            content_type = ContentType.objects.get(app_label=expected[0], model=expected[1])
