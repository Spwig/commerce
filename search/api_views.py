...
from .services import AnalyticsService, FuzzyService, SearchService
...
class AutocompleteAPIView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="lang", description=_("Language code (e.g., en, es, fr)"), type=str
            ),
            OpenApiParameter(
                name="engine", description=_("Search engine slug"), type=str, default="shop"
            ),
        ...
        description=_(
            "Fast predictive search suggestions. Returns products, categories, brands, and blog posts."
        ),
    )
...
class SearchResultsAPIView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="type",
                description=_("Filter by type (product/category/brand/blog_post)"),
                type=str,
            ),
            ...
            OpenApiParameter(
                name="sort",
                description=_("Sort order (relevance/price_asc/price_desc/newest)"),
                type=str,
            ),
...
class TrendingSearchesAPIView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="days", description=_("Number of days to analyze"), type=int, default=7
            ),
