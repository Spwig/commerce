...
class SearchSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Relevance Weights'), {
            ...
            'classes': ('tab-weights',),
            'description': _('Adjust the relevance weights for different content types. Higher values (up to 2.0) increase importance.')
        }),
        (_('Caching'), {
            'fields': (
...
class SearchEngineAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('search/admin/css/searchengine_form.css',)
        }
...
class SynonymAdmin(admin.ModelAdmin):
    ...
    list_display = ['term', 'synonyms_display', 'language_display', 'is_bidirectional', 'engine', 'is_active']
    ...
    fieldsets = (
        ...
        (_('Scope'), {
            ...
            'description': _('Optionally limit this synonym to a specific language or search engine.')
        }),
    )
...
class SearchRedirectAdmin(admin.ModelAdmin):
    ...
    list_display = ['term', 'match_type', 'redirect_url_display', 'redirect_type', 'hit_count', 'is_active']
...
class SearchQueryAdmin(admin.ModelAdmin):
    ...
    list_display = ['query', 'result_count', 'language', 'is_zero_result', 'response_time_ms', 'created_at']
    list_filter = ['is_zero_result', 'language', 'engine', 'created_at']
    search_fields = ['query', 'query_normalized']
    readonly_fields = ['query', 'query_normalized', 'result_count', 'user', 'session_key',
                       'language', 'engine', 'is_zero_result', 'response_time_ms', 'created_at']
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Zero result queries
        extra_context['zero_result_queries'] = SearchQuery.objects.filter(
            created_at__gte=week_start,
            is_zero_result=True
        ).values('query_normalized').annotate(
            count=Count('id')
...
class SearchClickAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['search_query', 'content_type', 'object_id', 'position',
...
class SearchIndexAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['content_type', 'object_id', 'extracted_text', 'file_type',
