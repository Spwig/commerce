...
Handles loading and validating payment provider component packages.
"""
import json
import logging
...
def validate_provider_package(manifest: Dict[str, Any], component_dir: Path) -> None:
    ...
    if manifest['component_type'] != 'payment_provider':
        raise ValueError(f"Invalid component type: {manifest['component_type']} (expected 'payment_provider')")
