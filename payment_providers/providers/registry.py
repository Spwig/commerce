"""
Provider registry for discovering and managing payment providers.
...
class ProviderRegistry:
    """
    Singleton registry for payment provider discovery and access.

    The registry:
    1. Discovers payment_provider components from ComponentRegistry
    ...
    Usage:
        ...
        provider_class = ProviderRegistry.get_provider('airwallex')
    ...
    def discover_providers(cls) -> None:
        """
        Discover and load all payment provider components.

        Scans ComponentRegistry for components of type 'payment_provider',
        ...
        logger.info("Discovering payment providers...")
        ...
        try:
            ...
            logger.info(f"Discovered {len(cls._providers)} payment providers")
    ...
    def _load_provider_from_component(cls, component) -> None:
        ...
        try:
            ...
            # Expected structure: components_data/integrations/payments/{slug}/current/provider.py
    ...
    def _import_provider_module(cls, component_dir: Path, module_path: str):
        ...
        component_name = component_dir.parent.name  # e.g., 'stripe'
        package_name = f"payment_provider_{component_name}"
    ...
    def get_provider(cls, provider_key: str) -> type[PaymentProviderBase] | None:
        ...
        Args:
            provider_key: Unique provider identifier (e.g., 'airwallex', 'stripe')
        ...
        if not cls._discovered:
            ...
            logger.info("Cache marker detected — reloading payment providers from disk")
            cls.reload_providers()

        # Try direct provider_key lookup first, then fall back to slug lookup
        result = cls._providers.get(provider_key)
        if result is None and provider_key in cls._slug_to_key:
            result = cls._providers.get(cls._slug_to_key[provider_key])
        return result
