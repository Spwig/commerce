...
(function() {
    ...
    var i18n = {};
    var configEl = document.getElementById('pp-list-config');
    if (configEl) {
        try {
    ...
    function initializeFilters() {
        ...
        var checkoutModeFilter = document.getElementById('checkout-mode-filter');
        var providerTypeFilter = document.getElementById('provider-type-filter');
    ...
    function buildAdminURL(accountId, action) {
        ...
        return langPrefix + '/admin/payment_providers/paymentprovideraccount/' + accountId + '/' + action + '/';
    }
    ...
    function testConnection(accountId, button) {
        ...
        .catch(function(error) {
            showMessage('error', (i18n.connectionTestFailed || 'Connection test failed') + ': ' + error);
            button.innerHTML = originalHTML;
            button.disabled = false;
        });
    }
        ...
        .then(function(data) {
            if (data.success) {
                ...
                if (card) {
                    ...
                    if (badge) {
                        badge.className = data.is_active ? 'badge badge-status badge-active' : 'badge badge-status badge-inactive';
    ...
    async function setDefaultProvider(accountId, button) {
        if (!await AdminModal.confirm(i18n.confirmSetDefault || 'Set this provider as the default payment provider?')) {
            return;
        }
    ...
    async function deleteProvider(accountId, button) {
        if (!await AdminModal.confirm({ message: i18n.confirmDelete || 'Are you sure you want to delete this payment provider? This action cannot be undone.', danger: true, confirmText: 'Delete' })) {
            return;
        }
    ...
    function showMessage(type, message) {
        ...
        if (container) {
            ...
            setTimeout(function() { messageContainer.remove(); }, 5000);
        }
    }
