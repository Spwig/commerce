...
(function() {
    ...
    var i18n = {};
    var configEl = document.getElementById('pp-methods-config');
    if (configEl) {
        try {
    ...
    function initializeSyncMethodsLink() {
        var syncLink = document.querySelector('.pp-sync-methods-link');
        if (!syncLink) return;
        ...
        syncLink.addEventListener('click', function(e) {
            ...
            var langPrefix = window.AdminUtils ? AdminUtils.getLanguagePrefix() : '';
            var url = langPrefix + '/admin/payment_providers/paymentprovideraccount/' + accountId + '/sync-methods/';
    ...
    function initializeSyncMethodsButton() {
        var btn = document.querySelector('.pp-sync-methods-btn');
        if (!btn) return;
        ...
        btn.addEventListener('click', function(e) {
            ...
            var csrfToken = getCsrf();
            var langPrefix = window.AdminUtils ? AdminUtils.getLanguagePrefix() : '';
            var url = langPrefix + '/admin/payment_providers/paymentprovideraccount/' + accountId + '/sync-payment-methods/';
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    button.innerHTML = '<i class="fas fa-exclamation-triangle"></i> ' + (data.error || 'Sync failed');
                    button.disabled = false;
                    setTimeout(function() { button.innerHTML = origHtml; }, 3000);
                }
            })
            .catch(function() {
    ...
    function initializeMethodToggles() {
        var checkboxes = document.querySelectorAll('.pp-method-checkbox');
        ...
        checkboxes.forEach(function(checkbox) {
            checkbox.addEventListener('change', function() {
                ...
                var methodSlug = this.dataset.methodSlug;
                var enabled = this.checked;
                var csrfToken = getCsrf();
                var langPrefix = window.AdminUtils ? AdminUtils.getLanguagePrefix() : '';

                fetch(langPrefix + '/admin/payment_providers/paymentprovideraccount/' + accountId + '/update-payment-method/', {
                    method: 'POST',
                    headers: {
                        'X-CSRFToken': csrfToken,
                        'Content-Type': 'application/json'
                ...
                .then(function(data) {
                    if (!data.success) {
                        AdminModal.alert({message: data.error || (i18n.error || 'Error saving change'), type: 'error'});
                    }
                })
                .catch(function() {
    ...
    function initializeToggleAllButtons() {
        var buttons = document.querySelectorAll('.pp-toggle-all-btn');
        if (!buttons.length) return;

        buttons.forEach(function(btn) {
            btn.addEventListener('click', function() {
                var action = this.dataset.action; // 'enable' or 'disable'
                var enable = action === 'enable';
                var card = this.closest('.dashboard-card');
                if (!card) return;

                var checkboxes = Array.from(card.querySelectorAll('.pp-method-checkbox'))
                    .filter(function(cb) { return cb.checked !== enable; });

                if (!checkboxes.length) return;

                // Disable buttons while processing
                var allBtns = card.querySelectorAll('.pp-toggle-all-btn');
                ...
                function processNext() {
                    ...
                    fetch(langPrefix + '/admin/payment_providers/paymentprovideraccount/' + cb.dataset.accountId + '/update-payment-method/', {
                        method: 'POST',
                        headers: {
                            'X-CSRFToken': csrfToken,
                            'Content-Type': 'application/json'
    ...
    function initializeRemoveCountryButtons() {
        var buttons = document.querySelectorAll('.pp-remove-country-btn');
        if (!buttons.length) return;
        ...
        buttons.forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                var card = this.closest('.pp-country-card');
                if (!card) return;

                // Find account ID from the first checkbox in the card (or global section)
                var firstCb = document.querySelector('.pp-method-checkbox');
                var accountId = firstCb ? firstCb.dataset.accountId : '';
                if (!accountId) return;

                var csrfToken = getCsrf();
                var langPrefix = window.AdminUtils ? AdminUtils.getLanguagePrefix() : '';
                var url = langPrefix + '/admin/payment_providers/paymentprovideraccount/' + accountId + '/remove-country-override/';
