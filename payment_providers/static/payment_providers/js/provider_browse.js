...
/**
 * Payment Provider Browse Page JavaScript
...
(function() {
    ...
    class ProviderInstaller {
        ...
        async install(providerSlug, providerName) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/payment-providers/providers/install/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    // Redirect to accounts changelist after short delay
                    setTimeout(() => {
        ...
        updateCardToInstalled(providerSlug) {
            ...
            if (installBtn) {
                ...
                configureBtn.href = `/${langCode}/admin/payment_providers/paymentprovideraccount/`;
    ...
    const InfoCardManager = {
        STORAGE_KEY: 'payment_provider_info_card_collapsed',
    ...
    class ProviderUpdater {
        ...
        async update(providerSlug, providerVersion) {
            try {
                ...
                const response = await fetch(AdminUtils.buildAdminUrl(`/admin/payment-providers/providers/update/${providerSlug}/`), {
                    method: 'POST',
                    headers: {
                        'X-CSRFToken': this.csrfToken,
                        'Content-Type': 'application/json',
                ...
                const contentType = response.headers.get('content-type') || '';
                if (!contentType.includes('application/json')) {
                    // Server likely timed out but the update may still complete
    ...
    function init() {
        ...
        // Auto-submit country filter
        var countryFilter = document.getElementById('countryFilter');
        if (countryFilter) {
            countryFilter.addEventListener('change', function() { this.form.submit(); });
        }
        ...
        detailsButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                try {
                    ...
                    providerData.provider_type = 'payment';
