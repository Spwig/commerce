    ...
    def get(self, request):
        ...
        try:
            ...
            messages.warning(
                request,
                _(
                    "Could not fetch providers from update server. Showing installed providers only."
                ),
            )
            available_providers = []
        ...
        for provider in available_providers:
            ...
            if (
                ...
                and selected_country not in supported_countries
            ):
                continue
        ...
        shipping_countries_data = [
            {"code": code, "name": countries.name(code)} for code in shipping_countries_list
        ]
...
def install_provider_ajax(request, provider_slug):
            ...
            return JsonResponse(
                {
                    ...
                    "redirect_url": reverse(
                        "admin:payment_providers_paymentprovideraccount_changelist"
                    ),
                }
            )
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": _("Installation failed: %(error)s") % {"error": str(e)}},
            status=500,
        )
...
def update_provider_ajax(request, provider_slug):
    ...
    try:
        ...
        if not latest_version:
            return JsonResponse(
                {"success": False, "error": _("Could not find latest version on update server")},
                status=404,
            )
        ...
        if success:
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": _("Provider updated successfully to v%(version)s")
                    % {"version": component.current_version},
                    "new_version": component.current_version,
                }
            )
        else:
            return JsonResponse({"success": False, "error": _("Update failed")}, status=400)
