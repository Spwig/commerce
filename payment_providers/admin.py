...
from .models import PaymentIntent, PaymentProviderAccount, PaymentTransaction, PaymentWebhook
...
class PaymentProviderAccountAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {"all": ("payment_providers/css/admin_provider_account.css",)}
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if obj:
            ...
            return (
                ...
                (_("Configuration"), {"fields": ("checkout_mode", "sort_order")}),
                ...
                (
                    _("Payment Method Synchronization"),
                    {
                        ...
                        "description": mark_safe(
                            ...
                            + '<a href="#" class="pp-sync-methods-link" data-account-id="{}">'.format(
                                obj.id if obj else ""
                            )
                        ...
                        if obj
                        else _("Save the account first to enable payment method synchronization."),
                    },
                ),
                (
                    _("Connection Health"),
                ...
                (
                    _("Create Provider"),
                    {
                        ...
                        "description": mark_safe(
                            ...
                            '<div class="warning">'
                            "<strong>"
                            + str(_("Please use the Payment Provider Connection Wizard"))
                            + "</strong><br/>"
                            + str(
                                _(
                                    'For the best experience, please use the <a href="%(url)s">Provider Connection Wizard</a> to set up new providers.'
                                )
                                % {"url": wizard_url}
                            )
                            + "</div>"
    ...
    def checkout_mode_badge(self, obj):
        ...
        css_map = {"hosted": "pp-list-badge-hosted", "integrated": "pp-list-badge-integrated"}
    ...
    def environment_badge(self, obj):
        ...
        css_class = "pp-list-badge-test" if obj.test_mode else "pp-list-badge-live"

        return format_html('<span class="pp-list-badge {}">{}</span>', css_class, str(env).title())
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        provider_types = (
            all_providers.values("component__name", "component__slug")
    ...
    def test_connection_view(self, request, account_id):
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": str(_("Provider account not found"))}, status=404
            )
    ...
    def set_default_view(self, request, account_id):
        ...
        try:
            ...
            account.save()
            return JsonResponse({"success": True, "message": _("Set as default provider")})
        except PaymentProviderAccount.DoesNotExist:
            return JsonResponse(
                {"success": False, "message": _("Provider account not found")}, status=404
            )
            ...
            return JsonResponse(
                {
                    ...
                    "message": str(_("Provider enabled"))
                    if account.is_active
                    else str(_("Provider disabled")),
                }
            )
        except PaymentProviderAccount.DoesNotExist:
            return JsonResponse(
    ...
    def sync_payment_methods_view(self, request, account_id):
        ...
        try:
                    ...
                    return JsonResponse(
                        {
                            "success": False,
                            "error": _(
                                "This provider does not support payment method synchronization"
                            ),
                        }
                    )
            except Exception as e:
            ...
            if result["success"]:
                ...
                return JsonResponse(
                    {
                        "success": True,
                        "message": _(
                            "Successfully synced %(methods)d payment methods across %(countries)d countries"
                        )
                        % {"methods": total_methods, "countries": total_countries},
                        "data": {
                            "methods": methods_data,
                            "total_countries": total_countries,
                            "total_methods": total_methods,
                            "last_sync": account.last_method_sync_at.isoformat()
                            if account.last_method_sync_at
                            else None,
                        },
                    }
                )
                ...
                return JsonResponse(
                    {"success": False, "error": result.get("message", _("Synchronization failed"))}
                )
            ...
            return JsonResponse(
                {"success": False, "error": _("Unexpected error: %(error)s") % {"error": str(e)}},
                status=500,
            )
    ...
    def configure_payment_methods_view(self, request, account_id):
        ...
        try:
            ...
            global_data = {
                ...
                "available_methods": [
                    {"slug": s, "name": s.replace("_", " ").title()} for s in global_available_slugs
                ],
            ...
            configured_countries = [k for k in account.enabled_payment_methods if k != GLOBAL_KEY]
            # Also include shipping countries as suggestions
            shipping_countries = ShippingCountry.objects.filter(site_id=1, is_active=True).order_by(
                "country_code"
            )
            shipping_country_map = {sc.country_code: str(sc) for sc in shipping_countries}

            # Merge: configured countries + shipping countries (no duplicates)
            all_country_codes = list(
                dict.fromkeys(configured_countries + list(shipping_country_map.keys()))
            )
            ...
            for country_code in all_country_codes:
                ...
                country_name = shipping_country_map.get(country_code, country_code)
            ...
            return render(
                request, "admin/payment_providers/configure_payment_methods.html", context
            )
    ...
    def update_payment_method_view(self, request, account_id):
        ...
        try:
            ...
            if str(country_code) != "_global" and not re.match(r"^[A-Za-z]{2}$", str(country_code)):
                return JsonResponse(
                    {"success": False, "error": _("Invalid country code")}, status=400
                )

            # Validate method_slug format (alphanumeric + underscores, max 50 chars)
            if not re.match(r"^[a-z0-9_]{1,50}$", str(method_slug)):
                return JsonResponse(
                    {"success": False, "error": _("Invalid payment method identifier")}, status=400
                )

            # Validate enabled is boolean
            if not isinstance(enabled, bool):
                return JsonResponse(
                    {"success": False, "error": _("Invalid enabled value")}, status=400
                )
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": _(
                        "Country override removed. Global settings will apply for %(country)s."
                    )
                    % {"country": country_code},
                }
            )
    ...
    def delete_provider_view(self, request, account_id):
        ...
        try:
                ...
                return JsonResponse(
                    {
                        ...
                        "message": str(
                            _(
                                "Cannot delete provider with existing transactions. Deactivate it instead."
                            )
                        ),
                    },
                    status=400,
            ...
            account.delete()
            return JsonResponse(
                {"success": True, "message": str(_("Provider deleted successfully"))}
            )
        except PaymentProviderAccount.DoesNotExist:
            return JsonResponse(
                {"success": False, "message": str(_("Provider account not found"))}, status=404
...
class PaymentTransactionAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "transaction_type", "provider_account", "created_at"]
    ...
    fieldsets = (
        ...
        (
            _("Provider Response"),
            {"fields": ("provider_response_display",), "classes": ("collapse",)},
        ),
        (
            _("Error Information"),
        ...
        (
            _("Timestamps"),
            {"fields": ("created_at", "updated_at", "expires_at"), "classes": ("collapse",)},
        ),
    )
    ...
    def provider_response_display(self, obj):
        ...
        if obj.provider_response:
            ...
            return format_html('<pre class="pp-json-display">{}</pre>', formatted_json)
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["provider_accounts"] = (
            PaymentProviderAccount.objects.select_related("component")
...
class PaymentWebhookAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Payload"), {"fields": ("payload_display",), "classes": ("collapse",)}),
        (_("Headers"), {"fields": ("headers_display",), "classes": ("collapse",)}),
        (
            _("Processing"),
            {
        ...
        (_("System Information"), {"fields": ("created_at",), "classes": ("collapse",)}),
    )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["provider_slugs"] = list(
            all_webhooks.exclude(provider_slug="")
...
class PaymentIntentAdmin(admin.ModelAdmin):
    ...
    list_display = ["id", "provider_account", "order", "status", "amount", "created_at"]
    ...
    fieldsets = (
        ...
        (
            _("Checkout Details"),
            {"fields": ("checkout_url", "client_secret"), "classes": ("collapse",)},
        ),
        (
        ...
        (
            _("Provider Response"),
            {"fields": ("provider_response", "metadata"), "classes": ("collapse",)},
        ),
