...
class PaymentProviderAccount(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            PaymentProviderAccount.objects.filter(user=self.user, is_default=True).exclude(
                pk=self.pk
    ...
    def sync_payment_methods(self) -> dict:
        ...
        try:
            ...
            return {"success": False, "message": f"Sync failed: {str(e)}", "methods": {}}
    ...
    def get_all_enabled_countries(self) -> list:
        ...
        """
        return [k for k in self.enabled_payment_methods if k != self.GLOBAL_KEY]
...
class PaymentTransaction(models.Model):
    ...
    def can_be_captured(self):
        ...
        return (
            self.transaction_type == "authorize"
            and self.status == "authorized"
    ...
    def can_be_refunded(self):
        ...
        return self.status in ["completed"] and self.transaction_type in ["charge", "capture"]
...
class PaymentWebhook(models.Model):
    ...
    # Webhook payload
    payload = models.JSONField(verbose_name=_("payload"), help_text=_("Complete webhook payload"))
    ...
    def mark_processed(self, result: dict = None, error: str = None):
        ...
        self.save(
            update_fields=["processed", "processed_at", "processing_result", "processing_error"]
        )
...
class PaymentIntent(models.Model):
    ...
    # Timestamps
    expires_at = models.DateTimeField(
        verbose_name=_("expires at"), help_text=_("When this payment intent expires")
    )
    ...
    def can_retry(self) -> bool:
        ...
        return self.status == "failed" and self.expires_at > timezone.now()
    ...
    def mark_failed(
        self, error_code: str = "", error_message: str = "", provider_data: dict = None
    ):
    ...
    def mark_requires_action(
        self, action_type: str, action_url: str = "", action_data: dict = None
    ):
