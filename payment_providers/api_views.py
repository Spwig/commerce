...
- Amounts are validated against checkout session totals
        ...
        responses={
            ...
            404: OpenApiResponse(description=_("Checkout session not found")),
        },
    )
    def post(self, request):
        ...
        success, intent, message = PaymentOrchestrationService.create_payment_intent(
            ...
            saved_method_id=str(data.get("saved_method_id"))
            if data.get("saved_method_id")
            else None,
    ...
    def _get_provider_account(self, checkout_session, provider_id=None):
        ...
        if provider_id:
            try:
                return PaymentProviderAccount.objects.get(id=provider_id, is_active=True)
        ...
        return PaymentProviderAccount.objects.filter(is_active=True, is_default=True).first()
        ...
        **Statuses**:
        ...
        responses={
            200: OpenApiResponse(
                response=PaymentIntentResponseSerializer,
                description=_(
                    "Payment intent details (full handler data for active intents, status-only for terminal states)"
                ),
            ),
...
class PaymentIntentConfirmView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=PaymentIntentStatusSerializer, description=_("Confirmation result")
            ),
    ...
    def post(self, request, intent_id):
        ...
        success, message = PaymentOrchestrationService.confirm_payment_intent(
            intent=intent, confirmation_data=serializer.validated_data.get("payment_method_data")
        )
...
class SavedMethodListCreateView(HeadlessAPIMixin, APIView):
    ...
    def get(self, request):
        ...
        tokens = (
            PaymentToken.objects.filter(user=request.user)
    ...
    def post(self, request):
        ...
        try:
            provider_account = PaymentProviderAccount.objects.get(
                id=data["provider_id"], is_active=True
            )
        except PaymentProviderAccount.DoesNotExist:
            return Response(
        ...
        try:
            ...
            if not hasattr(provider, "save_payment_method"):
                return Response(
                    {
                        "success": False,
                        "message": _("Provider does not support saved payment methods"),
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )

            result = provider.save_payment_method(
                token=data["payment_method_token"], customer_email=request.user.email
            )
            ...
            if not result.get("success"):
                return Response(
                    {
                        "success": False,
                        "message": result.get("message", "Failed to save payment method"),
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if data.get("set_as_default"):
                PaymentToken.objects.filter(user=request.user, is_default=True).update(
                    is_default=False
                )
            ...
            return Response(
                {"success": True, **response_serializer.data}, status=status.HTTP_201_CREATED
            )
...
class SavedMethodDetailView(HeadlessAPIMixin, APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=SavedPaymentMethodSerializer, description=_("Payment method details")
            ),
    ...
    def delete(self, request, method_id):
        ...
        token = get_object_or_404(PaymentToken, token_id=method_id, user=request.user)
