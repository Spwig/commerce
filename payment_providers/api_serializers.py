...
from core.utils import get_default_currency, get_default_country
...
class CreatePaymentIntentSerializer(serializers.Serializer):
    ...
    provider_id = serializers.UUIDField(
        required=False,
        help_text=_("Payment provider account ID. If not provided, uses session's selected provider.")
    )
    saved_method_id = serializers.UUIDField(
        required=False,
        help_text=_("Saved payment method ID for returning customers.")
    )
    ...
    cancel_url = serializers.URLField(
        required=True,
        help_text=_("URL to redirect on payment cancellation.")
    )
    metadata = serializers.JSONField(
        required=False,
        default=dict,
        help_text=_("Additional metadata to attach to the payment.")
    )
...
class PaymentActionSerializer(serializers.Serializer):
    ...
    url = serializers.URLField(
        required=False,
        allow_null=True,
        help_text=_("URL to redirect for action completion.")
    )
...
class PaymentIntentResponseSerializer(serializers.Serializer):
    ...
    intent_id = serializers.UUIDField(
        source='id',
        help_text=_("Payment intent ID.")
    )
    order_id = serializers.UUIDField(
        source='order.id',
        help_text=_("Order ID (created with 'unpaid' status).")
    )
    ...
    status = serializers.CharField(
        help_text=_("Current payment intent status.")
    )
    ...
    error = serializers.SerializerMethodField(
        help_text=_("Error details if payment failed.")
    )
    ...
    def get_handler_config(self, obj):
        ...
        if component.slug == 'airwallex':
            ...
            if obj.order:
                config['country_code'] = obj.order.shipping_country or obj.order.billing_country or get_default_country()
            else:
        ...
        # For Stripe
            ...
            if not config['publishable_key']:
                config['publishable_key'] = decrypted_creds.get('publishable_key', settings.get('publishable_key', ''))
...
class PaymentIntentStatusSerializer(serializers.Serializer):
    ...
    requires_action = serializers.BooleanField(
        help_text=_("Whether customer action is required.")
    )
    action = serializers.SerializerMethodField(
        help_text=_("Action details if requires_action is true.")
    )
...
class CreateSavedPaymentMethodSerializer(serializers.Serializer):
    ...
    """
    provider_id = serializers.UUIDField(
        help_text=_("Payment provider account ID.")
    )
    ...
    set_as_default = serializers.BooleanField(
        default=False,
        help_text=_("Whether to set this as the default payment method.")
    )
