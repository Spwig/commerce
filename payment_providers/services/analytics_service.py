...
class PaymentAnalyticsService:
    ...
    def get_action_cards() -> dict[str, int]:
        ...
        return {
            "failed_transactions": PaymentTransaction.objects.filter(
                status="failed", created_at__gte=timezone.now() - timedelta(days=7)
            ).count(),
            "pending_captures": PaymentTransaction.objects.filter(
                status="authorized", transaction_type="authorize"
            ).count(),
            "pending_refunds": Order.objects.filter(refunds__status="pending").distinct().count(),
            "connection_errors": PaymentProviderAccount.objects.filter(
                connection_status="error"
            ).count(),
            "unprocessed_webhooks": PaymentWebhook.objects.filter(processed=False).count(),
        }

    @staticmethod
    def get_payment_performance(
        start_date: datetime, end_date: datetime, compare: bool = False
    ) -> dict[str, Any]:
        """
        ...
        if compare:
            ...
            result["changes"] = {
                "revenue": PaymentAnalyticsService._calculate_change(
                    float(prev_stats["total_revenue"]), float(current_stats["total_revenue"])
                ),
                "transactions": PaymentAnalyticsService._calculate_change(
                    prev_stats["total_transactions"], current_stats["total_transactions"]
                ),
                "success_rate": PaymentAnalyticsService._calculate_change(
                    (prev_stats["successful_transactions"] / prev_stats["total_transactions"] * 100)
                    if prev_stats["total_transactions"] > 0
                    else 0,
                    success_rate,
                ),
            }
    ...
    @staticmethod
    def get_transaction_by_status(start_date: datetime, end_date: datetime) -> list[dict[str, Any]]:
            ...
            results.append(
                {
                    ...
                    "amount": txn.amount,
                    "status": txn.status,
                    "provider_name": txn.provider_account.display_name
                    or txn.provider_account.component.name,
    ...
    @staticmethod
    def get_webhook_stats(start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        # By provider
        by_provider = (
            webhooks.values("provider_slug")
            .annotate(count=Count("id"), processed_count=Count("id", filter=Q(processed=True)))
            .order_by("-count")
        )
    ...
    def get_refund_metrics(
        ...
        refund_rate = (
            (float(current_stats["total_refunded"]) / float(total_revenue) * 100)
            if total_revenue > 0
            else 0
        )
        ...
        if compare:
            ...
            result["changes"] = {
                "total_refunded": PaymentAnalyticsService._calculate_change(
                    float(prev_refunds["total_refunded"]), float(current_stats["total_refunded"])
                ),
                "refund_count": PaymentAnalyticsService._calculate_change(
                    prev_refunds["refund_count"], current_stats["refund_count"]
                ),
            }
