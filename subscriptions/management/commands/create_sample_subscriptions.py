...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if CustomerSubscription.objects.exists():
            self.stdout.write(
                self.style.WARNING("Subscriptions exist - skipping sample data creation")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"  - Basic Plan: {basic_plan.pricing_tiers.count()} tiers, {basic_plan.addons.count()} add-ons"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"  - Pro Plan: {pro_plan.pricing_tiers.count()} tiers, {pro_plan.addons.count()} add-ons"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"  - Enterprise Plan: {enterprise_plan.pricing_tiers.count()} tiers, {enterprise_plan.addons.count()} add-ons"
            )
        )
