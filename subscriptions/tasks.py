...
def process_due_subscriptions():
    ...
    due_subscriptions = CustomerSubscription.objects.filter(
            ...
            if billing_log.can_retry():
                ...
                if grace_days > 0 and not subscription.grace_period_end_date:
                    ...
                    subscription.grace_period_end_date = (
                        timezone.now() + timedelta(days=grace_days)
                    )
...
def process_trial_expirations():
        ...
        try:
            ...
            if subscription.provider_mode == 'fallback':
                ...
                if billing_log.status == 'successful':
                    converted_count += 1
                    _emit_fallback_event(
                        subscription, SubscriptionEventType.ACTIVATED,
                    )
                else:
                    failed_count += 1
    ...
    logger.info(
        f"Trial expiration complete: {converted_count} converted, {failed_count} failed"
    )
...
def process_auto_resume():
    ...
    logger.info(
        f"Auto-resume complete: {success_count} successful, {failed_count} failed"
    )
...
def send_payment_method_expiry_warnings():
    ...
    expiry_filter = Q(
        ...
        payment_token__card_exp_month__lte=(
            future_month if future_year == current_year else 12
        ),
    )
    ...
    expiring_subs = CustomerSubscription.objects.filter(
        status__in=['active', 'trial'],
        cancellation_type='none',
        ...
        'plan', 'pricing_tier', 'payment_token', 'user'
    ).distinct()
    ...
    for subscription in expiring_subs:
        ...
        if not subscription.payment_token.card_exp_month or not subscription.payment_token.card_exp_year:
            continue
...
def process_dunning_retries():
    ...
    grace_subscriptions = CustomerSubscription.objects.filter(
        ...
        'plan', 'payment_provider_account', 'payment_token', 'pricing_tier'
    )
    ...
    for subscription in grace_subscriptions:
        try:
            ...
            last_log = subscription.billing_logs.filter(
                status='failed'
            ...
            if last_log.billing_date:
                hours_since_last = (
                    (now - last_log.billing_date).total_seconds() / 3600
                )
            ...
            charge_result = manager.provider.charge_payment_token(
                ...
                description=(
                    f"Subscription billing (dunning retry) - "
            ...
            if charge_result['status'] == 'succeeded':
                ...
                last_log.save(update_fields=[
                    'status', 'provider_response', 'billing_date',
                ])
                ...
                subscription.current_period_end = (
                    manager._calculate_next_billing_date(
                        subscription.current_period_end,
                ...
                last_log.billing_date = now
                last_log.error_message = charge_result.get(
                    'error_message', ''
                )
                last_log.error_code = charge_result.get('error_code', '')
                last_log.provider_response = charge_result
                last_log.save(update_fields=[
                    'billing_date', 'error_message', 'error_code',
                    'provider_response',
                ])
            ...
            logger.exception(
                f"Error processing dunning retry for "
...
def send_dunning_final_notices():
    ...
    logger.info(
        f"Dunning final notices: {sent} sent, {skipped} skipped (already notified)"
    )
    return {'total': count, 'sent': sent, 'skipped': skipped}
...
def process_scheduled_plan_changes():
    ...
    """
    from .models import CustomerSubscription, SubscriptionPlan, PlanPricingTier
    from .manager import SubscriptionManager
    from .events import SubscriptionEventType
    ...
    pending_changes = CustomerSubscription.objects.filter(
        status__in=['active', 'trial'],
        current_period_end__lte=now,
        cancellation_type='none',
    ).exclude(
        ...
        'plan', 'pricing_tier', 'payment_provider_account',
        'payment_token', 'product', 'variant', 'user',
    )
    ...
    for subscription in pending_changes:
        try:
            ...
            new_plan = SubscriptionPlan.objects.get(
                plan_id=change_data['new_plan_id']
            )
            new_tier = PlanPricingTier.objects.get(
                tier_id=change_data['new_tier_id']
            )
            ...
            if change_type == 'downgrade':
                try:
                    ...
                    logger.warning(
                        f"Could not calculate proration for "
                        f"{subscription.subscription_id}: {e}"
                    )
            ...
            if proration_credit_data and change_type == 'downgrade':
                extra_data['credit_amount'] = str(
                    proration_credit_data['proration_amount'].amount
                )
            ...
            logger.error(
                f"Plan/tier not found for scheduled change on "
                f"{subscription.subscription_id}: {e}"
            )
            ...
            logger.exception(
                f"Error applying scheduled plan change for "
                f"{subscription.subscription_id}: {e}"
            )
            failed_count += 1
