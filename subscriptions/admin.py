...
from .models import (
    SubscriptionPlan, PlanPricingTier, PlanAddon,
    PaymentToken, CustomerSubscription, CustomerSubscriptionAddon,
    SubscriptionDiscount, BillingCycleLog
)
    ...
    class Meta:
        ...
        widgets = {
            ...
            'description': TranslatableFieldWidget(
                base_widget=forms.Textarea(attrs={'rows': 4, 'class': 'vLargeTextField', 'style': 'width: 100%;'})
            ),
        }
...
class PlanPricingTierForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {
            'discount_percentage': _('Discount applied to product price when customer subscribes (0 for full price, 10 for 10% off, etc.)'),
        }
    ...
    def clean_discount_percentage(self):
        ...
        if discount is None:
            raise forms.ValidationError(_("Discount percentage is required (use 0 for no discount)"))
...
class SubscriptionPlanAdmin(TranslatableAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Provider Integration'), {
            ...
            'classes': ('collapse',),
            'description': _('Map this plan to provider-specific plan IDs (e.g., {"stripe": "price_xxx", "paypal": "P-xxx"})')
        }),
        (_('Statistics'), {
            'fields': (
                'active_subscriptions_count',
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('wizard/', self.admin_site.admin_view(self.wizard_view), name='subscriptions_subscriptionplan_wizard'),
        ]
    ...
    def wizard_view(self, request):
        ...
        if request.method == 'POST':
            try:
                ...
                messages.success(
                    request,
                    _("Subscription plan '{}' created successfully! You can now add translations and configure additional settings.").format(name)
                )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        custom_filter_params = ['filter_search', 'filter_pricing_model', 'filter_status', 'filter_visibility', 'filter_cancellation_policy', 'filter_trial_period', 'ajax']
        ...
        is_ajax = original_get.get('ajax') == '1' or request.headers.get('X-Requested-With') == 'XMLHttpRequest'
    ...
    def total_revenue_display(self, obj):
        ...
        total = obj.subscriptions.filter(
            billing_logs__status='successful'
        ).aggregate(
...
class PaymentTokenAdmin(admin.ModelAdmin):
    ...
    def expiry_status(self, obj):
        ...
        if obj.is_expired():
            return format_html(
                '<span class="sub-status sub-status-error">{}</span>',
                _("Expired")
            )
...
class CustomerSubscriptionAdmin(admin.ModelAdmin):
    ...
    def status_display(self, obj):
        ...
        if obj.cancellation_type == 'end_of_period':
            html += format_html('<br><small class="sub-status-note sub-status-error">{}</small>', _("Cancels at period end"))
        elif obj.cancellation_type == 'immediate':
            html += format_html('<br><small class="sub-status-note sub-status-error">{}</small>', _("Canceled immediately"))
    ...
    def days_until_next_billing_display(self, obj):
        ...
        if days == 0:
            ...
            return format_html('<span class="sub-status sub-status-warning">{} {}</span>', days, _("days"))
        else:
    ...
    def pause_subscriptions(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully paused {paused_count} subscription(s)",
            level='success'
        )
    ...
    def resume_subscriptions(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully resumed {resumed_count} subscription(s)",
            level='success'
        )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Strip our custom filter parameters from request.GET
        custom_filter_params = ['filter_search', 'filter_status', 'filter_plan', 'filter_product', 'filter_provider_mode', 'filter_payment_provider', 'ajax']
        ...
        extra_context['payment_providers'] = PaymentProviderAccount.objects.filter(is_active=True).order_by('display_name')
