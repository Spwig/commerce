...
class ProviderWizardStep1View(WizardSessionMixin, View):
    ...
    def get(self, request):
        ...
        providers = ComponentRegistry.objects.filter(component_type="payout_provider").order_by(
            "name"
        )
                ...
                if provider_dir.exists():
                    ...
                    if manifest:
                        ...
                        if logo_filename:
                            ...
                            if logo_path.exists():
                                logo_url = static(
                                    f"payout_provider/{component.slug}/current/{logo_filename}"
                                )
    ...
    def post(self, request):
        ...
        try:
            component = ComponentRegistry.objects.get(
                id=component_id, component_type="payout_provider"
            )
        except ComponentRegistry.DoesNotExist:
    ...
    def get(self, request):
        ...
        try:
            ...
            if instructions_file.exists():
                ...
                webhook_url = (
                    request.build_absolute_uri(reverse(f"payout_webhooks:webhook_{component.slug}"))
                    if component.slug in ["paypal", "airwallex"]
                    else ""
                )
...
class ProviderWizardStep3View(WizardSessionMixin, View):
    ...
    def post(self, request):
        ...
        # Store in session
        self.update_wizard_data(
            credentials=credentials, settings=settings_data, provider_name=provider_name
        )
            ...
            if action == "test":
                ...
                try:
                    ...
                    is_first_provider = not PayoutProviderAccount.objects.filter(
                        provider_type=component_slug, is_active=True
                    ).exists()
                    ...
                    component_path = INTEGRATIONS_DIR / "payout_provider" / component.slug / version
                    manifest = (
                        load_provider_manifest(component_path) if component_path.exists() else {}
                    )
                    supported_methods = manifest.get("supported_methods", [])
