...
class PayoutProviderAccount(models.Model):
    ...
    is_active = models.BooleanField(
        _("Active"), default=True, help_text=_("Whether this provider account is active")
    )
    ...
    connection_status = models.CharField(
        _("Connection Status"), max_length=30, choices=CONNECTION_STATUS_CHOICES, default="untested"
    )

    last_tested_at = models.DateTimeField(_("Last Tested"), null=True, blank=True)

    last_error_message = models.TextField(
        _("Last Error"), blank=True, default="", help_text=_("Last connection error message")
    )
                ...
                raise ValidationError(
                    {
                        ...
                        % {
                            "provider": self.get_provider_type_display(),
...
class PayoutWebhookLog(models.Model):
    ...
    provider_type = models.CharField(
        _("Provider Type"), max_length=50, help_text=_("Provider that sent the webhook")
    )

    event_type = models.CharField(
        _("Event Type"), max_length=100, help_text=_("Type of webhook event")
    )
    ...
    payload = models.JSONField(_("Payload"), default=dict, help_text=_("Full webhook payload"))

    headers = models.JSONField(_("Headers"), default=dict, help_text=_("Request headers"))

    signature_valid = models.BooleanField(
        _("Signature Valid"), null=True, help_text=_("Whether signature verification passed")
    )

    processed = models.BooleanField(
        _("Processed"), default=False, help_text=_("Whether the webhook was successfully processed")
    )
    ...
    payout_reference = models.CharField(
        _("Payout Reference"), max_length=255, blank=True, default="", db_index=True
    )
