...
(function() {
    ...
    function loadI18n() {
        var el = document.getElementById('payout-providers-i18n');
        if (el) {
            try {
    ...
    function getSelectedProviders() {
        var checkboxes = document.querySelectorAll('.provider-select:checked');
    ...
    function updateSelectedCount() {
        var count = getSelectedProviders().length;
        var countDisplay = document.getElementById('selected-count');
        var numberDisplay = document.getElementById('selected-number');
        var applyButton = document.getElementById('apply-bulk-action');
    ...
    function toggleProviderActive(providerId, button) {
        var isActive = button.dataset.isActive === 'true';
        var card = button.closest('.provider-card');

        setLoading(true);

        fetch(AdminUtils.buildAdminUrl('/admin/payout_providers/payoutprovideraccount/' + providerId + '/toggle-active/'), {
            method: 'POST',
            headers: {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                button.title = data.is_active
                    ? (i18n.disable || 'Disable')
                    : (i18n.enable || 'Enable');
                setButtonContent(button,
                    data.is_active ? 'fas fa-toggle-on' : 'fas fa-toggle-off',
                    ''
                );
                ...
                // Update badge
                var badge = card.querySelector('.badge-active, .badge-inactive');
                if (badge) {
                    ...
                    badge.appendChild(document.createTextNode(' ' + (data.is_active ? (i18n.active || 'Active') : (i18n.inactive || 'Inactive'))));
                }
            } else {
                showNotification(data.message || (i18n.toggleFailed || 'Failed to toggle provider status'), 'error');
            }
        })
    ...
    function setProviderDefault(providerId, button) {
        ...
        fetch(AdminUtils.buildAdminUrl('/admin/payout_providers/payoutprovideraccount/' + providerId + '/set-default/'), {
            method: 'POST',
            headers: {
                'X-CSRFToken': AdminUtils.getCsrfToken(),
                'Content-Type': 'application/json',
        ...
        .then(function(data) {
            if (data.success) {
                ...
                showNotification(data.message || (i18n.defaultFailed || 'Failed to set default provider'), 'error');
            }
        })
        .catch(function(error) {
            console.error('Error:', error);
    ...
    function testProviderConnection(providerId, button) {
        ...
        var originalIconClass = originalIcon ? originalIcon.className : 'fas fa-plug';
        button.textContent = '';
        ...
        fetch(AdminUtils.buildAdminUrl('/admin/payout_providers/payoutprovideraccount/' + providerId + '/test-connection/'), {
            method: 'POST',
            headers: {
                'X-CSRFToken': AdminUtils.getCsrfToken(),
                'Content-Type': 'application/json',
        ...
        .then(function(data) {
            if (data.success) {
                showNotification(data.message || (i18n.testSuccess || 'Connection test successful!'), 'success');
                // Reload page to update connection status
                setTimeout(function() { window.location.reload(); }, 1000);
            } else {
                showNotification(data.message || data.error || (i18n.testFailed || 'Connection test failed'), 'error');
            }
        })
        .catch(function(error) {
            console.error('Error:', error);
    ...
    async function applyBulkAction() {
        ...
        var providerIds = getSelectedProviders();

        if (!action || providerIds.length === 0) {
            showNotification(i18n.selectActionWarning || 'Please select an action and at least one provider', 'warning');
            return;
        }

        // Confirm for delete action
        if (action === 'delete') {
            var deleteMsg = (i18n.confirmDelete || 'Are you sure you want to delete {count} provider(s)? This action cannot be undone.')
                .replace('{count}', providerIds.length);
            if (!await AdminModal.confirm({ message: deleteMsg, danger: true, confirmText: 'Delete' })) {
                return;
            }
        }
        ...
        if (action === 'set_default' && providerIds.length !== 1) {
            showNotification(i18n.selectOneDefault || 'Please select exactly one provider to set as default', 'warning');
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var selectAllCheckbox = document.getElementById('select-all-providers');
        if (selectAllCheckbox) {
            selectAllCheckbox.addEventListener('change', function() {
                var checkboxes = document.querySelectorAll('.provider-select');
        ...
        document.addEventListener('change', function(e) {
            if (e.target.classList.contains('provider-select')) {
                ...
                var selectAll = document.getElementById('select-all-providers');
                var checkboxes = document.querySelectorAll('.provider-select');
                var checkedBoxes = document.querySelectorAll('.provider-select:checked');
                if (selectAll) {
