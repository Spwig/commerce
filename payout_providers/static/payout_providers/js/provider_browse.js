...
(function () {
  ...
  function loadI18n() {
    const el = document.getElementById('payout-browse-i18n');
    if (!el) return {};
    try {
    ...
    async install(providerSlug, providerName) {
      try {
        ...
        const response = await fetch(
          AdminUtils.buildAdminUrl(`/admin/payout-providers/providers/install/${providerSlug}/`),
          {
            method: 'POST',
            headers: {
  ...
  class ProviderUpdater {
    async update(providerSlug, providerVersion) {
      try {
        ...
        const response = await fetch(
          AdminUtils.buildAdminUrl(`/admin/payout-providers/providers/update/${providerSlug}/`),
          {
            method: 'POST',
            headers: {
              'X-CSRFToken': AdminUtils.getCsrfToken(),
