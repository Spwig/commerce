...
class AirwallexPayoutProvider(BasePayoutProvider):
    ...
    def _create_beneficiary(self, recipient: PayoutRecipient, currency: str) -> str | None:
        """
    ...
    def create_payout(self, request: PayoutRequest) -> PayoutResult:
        ...
        if not all(
            [recipient.bank_account_holder, recipient.bank_account_number, recipient.bank_country]
        ):
            return PayoutResult(
                success=False,
        ...
        if not beneficiary_id:
            return PayoutResult(
                success=False, status=PayoutStatus.FAILED, message="Failed to create beneficiary"
            )
