...
class PayoutProviderAccountAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Provider Information"), {"fields": ("provider_type", "name", "component")}),
        ...
        (
            _("Configuration"),
            {"fields": ("supported_methods", "settings"), "classes": ("collapse",)},
        ),
    ...
    def test_connection_action(self, request, queryset):
        ...
        if fail_count:
            self.message_user(
                request,
                _("Failed to connect to %(count)d provider(s). Check error messages.")
                % {"count": fail_count},
                messages.ERROR,
            )
    ...
    def set_default_view(self, request, provider_id):
        ...
        try:
            ...
            PayoutProviderAccount.objects.filter(
                provider_type=provider.provider_type, is_default=True
            ...
            if action == "enable":
                ...
                for provider in providers:
                    ...
                    if result.get("success"):
                        ...
                        fail_count += 1
                message = _(
                    "Tested %(count)d provider(s): %(success)d succeeded, %(fail)d failed."
                ) % {"count": count, "success": success_count, "fail": fail_count}
...
class PayoutWebhookLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Status"), {"fields": ("signature_valid", "processed", "processing_error")}),
        (_("Payload"), {"fields": ("payload", "headers"), "classes": ("collapse",)}),
