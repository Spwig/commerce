...
def process_payout(self, payout_id: int, provider_account_id: int = None):
    ...
    from .providers.base import PayoutRequest, PayoutRecipient, PayoutMethod
    ...
    try:
        ...
        if result.success:
            ...
            if result.status.value == 'completed':
                ...
                payout.save()
                logger.info(f"Payout {payout_id} submitted, awaiting confirmation. Ref: {result.provider_reference}")
...
def process_batch_payouts(self, payout_ids: list, provider_account_id: int):
    ...
    logger.info(f"Processing batch of {len(payout_ids)} payouts with provider {provider_account_id}")
    ...
    if not provider_class:
        return {'success': False, 'error': f'Provider class not found: {provider_account.provider_type}'}
    ...
    if not hasattr(provider, 'create_batch_payout'):
        ...
        logger.info(f"Provider {provider_account.provider_type} doesn't support batch, processing individually")
        results = []
        ...
        return {
            'success': True,
            'message': 'Queued for individual processing',
            'tasks': results
        }
    ...
    for payout in payouts:
        ...
        payout_request = PayoutRequest(
            ...
            currency=payout.currency or get_default_currency(),
            reference=reference,
            method=PayoutMethod.PAYPAL if affiliate.payment_method == 'paypal' else PayoutMethod.BANK_TRANSFER,
            note=f"Commission payout for {affiliate.company_name or affiliate.user.email}",
...
def sync_payout_status(self, payout_id: int):
    ...
    try:
        ...
        if result.success:
            ...
            elif new_status in ('returned', 'cancelled') and payout.status not in ('completed', 'failed', 'cancelled'):
...
def sync_pending_payout_statuses():
    ...
    for payout_id in processing_payouts:
        try:
            result = sync_payout_status.delay(payout_id)
            synced += 1
        except Exception as e:
    ...
    return {
        'queued': synced,
        'failed': failed,
        'total_processing': len(processing_payouts)
    }
...
def handle_webhook_event(webhook_log_id: int):
    ...
    try:
        webhook_log = PayoutWebhookLog.objects.select_related('provider_account').get(id=webhook_log_id)
    ...
    provider_class = get_provider_class(
        webhook_log.provider_type,
        provider_account.component if provider_account else None
    )
    ...
    try:
        ...
        if not payout:
            ...
            return {'success': True, 'message': f'No payout found for reference: {payout_reference}'}
