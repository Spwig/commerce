...
class WebhookEndpoint(models.Model):
    ...
    name = models.CharField(
        _('name'),
        max_length=255,
        help_text=_('A friendly name to identify this webhook endpoint')
    )
    url = models.URLField(
        _('URL'),
        max_length=2048,
        help_text=_('The URL that will receive webhook POST requests')
    )
    ...
    timeout_seconds = models.PositiveIntegerField(
        _('timeout (seconds)'),
        default=30,
        help_text=_('Timeout in seconds for webhook requests')
    )
    ...
    description = models.TextField(
        _('description'),
        blank=True,
        help_text=_('Optional description of this webhook endpoint')
    )
    ...
    def __str__(self):
        return f"{self.name} ({self.url[:50]}...)" if len(self.url) > 50 else f"{self.name} ({self.url})"
...
class WebhookDelivery(models.Model):
    ...
    payload = models.JSONField(
        _('payload'),
        help_text=_('The JSON payload sent to the webhook endpoint')
    )
    ...
    def mark_failed(self, error_message, response_code=None, response_body=None,
                   response_time_ms=None, will_retry=False, next_retry_at=None):
