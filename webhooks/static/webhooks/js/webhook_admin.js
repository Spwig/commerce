...
(function() {
    ...
    function updateActiveFilters() {
        ...
        if (status) {
            hasFilters = true;
            const statusLabel = document.querySelector('#filter-status option:checked')?.textContent || status;
        ...
        if (health) {
            hasFilters = true;
            const healthLabel = document.querySelector('#filter-health option:checked')?.textContent || health;
    ...
    function testEndpoint(endpointId) {
        ...
        .then(data => {
            ...
            if (resultContainer) {
                ...
                if (timeEl) {
                    timeEl.textContent = data.response_time_ms ? `Response Time: ${data.response_time_ms}ms` : '';
                }
                if (errorEl) {
    ...
    async function resetEndpoint(endpointId) {
        if (!await AdminModal.confirm({ message: 'Reset failure count for this endpoint? This will re-enable it if it was disabled.', danger: true, confirmText: 'Reset' })) {
            return;
        }
