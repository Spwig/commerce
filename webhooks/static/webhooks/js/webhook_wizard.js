...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const el = document.getElementById('webhook-wizard-translations');
    if (el) {
      try {
  ...
  function initEventSelection() {
    ...
    const allEventsInput = document.getElementById('all-events-selected');
    ...
    categoryCheckboxes.forEach(function (categoryCheckbox) {
      categoryCheckbox.addEventListener('change', function () {
        ...
        document
          .querySelectorAll('.event-checkbox[data-category="' + category + '"]')
          .forEach(function (cb) {
  ...
  function updateCategoryCheckbox(category) {
    const categoryCheckbox = document.querySelector(
      '.select-category[data-category="' + category + '"]'
    );
    const categoryEvents = document.querySelectorAll(
      '.event-checkbox[data-category="' + category + '"]'
    );
    const allChecked = Array.from(categoryEvents).every(function (cb) {
      return cb.checked;
    });
    const someChecked = Array.from(categoryEvents).some(function (cb) {
      return cb.checked;
    });
  ...
  function updateSelectedCount() {
    const selectAllCheckbox = document.getElementById('select-all-events');
    const countEl = document.getElementById('selected-events-count');
    if (!countEl) return;
  ...
  function validateStep(step) {
    ...
    if (step === 2) {
      const selectAll = document.getElementById('select-all-events').checked;
  ...
  function populateReview() {
    ...
    // Events
    const selectAll = document.getElementById('select-all-events').checked;
    const eventsContainer = document.getElementById('review-events');

    if (selectAll) {
      eventsContainer.innerHTML =
        '<span class="review-badge all-events"><i class="fas fa-asterisk"></i> ' +
        (T.allEvents || 'All Events') +
        '</span>';
    } else {
      const selectedEvents = Array.from(document.querySelectorAll('.event-checkbox:checked')).map(
        function (cb) {
          return cb.value;
        }
      );
      ...
      if (selectedEvents.length === 0) {
        eventsContainer.innerHTML =
          '<span class="review-badge no-events">' + (T.noneSelected || 'None selected') + '</span>';
      } else {
        eventsContainer.innerHTML = selectedEvents
          .map(function (e) {
            return '<span class="review-badge event-badge">' + e + '</span>';
          })
          .join('');
      }
    }
