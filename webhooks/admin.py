...
class WebhookEndpointAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {"all": ("webhooks/admin/css/webhookendpoint_form.css",)}
    ...
    def is_active_display(self, obj):
        ...
        if obj.is_active:
            return format_html('<span style="color: #28a745;">&#x2714; {}</span>', _("Active"))
        return format_html('<span style="color: #6c757d;">&#x2716; {}</span>', _("Inactive"))
    ...
    def health_status(self, obj):
        ...
        return format_html('<span style="color: #28a745;">&#x2714; {}</span>', _("Healthy"))
    ...
    def last_triggered_display(self, obj):
        ...
        if delta < timedelta(hours=1):
            return format_html('<span style="color: #28a745;">{}</span>', _("Recently"))
    ...
    def secret_display(self, obj):
        ...
        if obj.secret:
            return format_html(
                '<code>{}...{}</code> <span style="color: #6c757d; font-size: 0.9em;">({})</span>',
                obj.secret[:8],
                obj.secret[-4:],
    ...
    def delivery_stats(self, obj):
        ...
        return format_html(
            ...
            _("Avg Response"),
            int(stats.get("avg_response_time_ms", 0)),
        )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        active = WebhookEndpoint.objects.filter(
            is_active=True, is_disabled_by_failures=False
        ).count()
        unhealthy = WebhookEndpoint.objects.filter(is_disabled_by_failures=True).count()
        ...
        deliveries_24h = WebhookDelivery.objects.filter(created_at__gte=last_24h).count()
...
class WebhookDeliveryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("id", "endpoint", "event_type", "status")}),
        (
            _("Payload"),
            {
    ...
    def status_display(self, obj):
        ...
        return format_html(
            '<span style="color: {};">{} {}</span>', color, icon, obj.get_status_display()
        )
    ...
    def payload_display(self, obj):
        ...
        try:
            ...
            return format_html(
                '<pre style="max-height: 400px; overflow: auto;">{}</pre>', formatted
            )
        except Exception:
