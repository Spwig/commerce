(function() {
    ...
    function updateFooter() {
        var isFirst = currentGroupIndex === 0;
        var isLast = currentGroupIndex === GROUPS.length - 1;
        var isOptional = currentGroupIndex === GROUPS.length - 1; // finetune
    ...
    function saveCurrentStep() {
        ...
        // Add CSRF token
        var csrfInput = body.querySelector('[name=csrfmiddlewaretoken]');
        if (csrfInput) {
        ...
        var checkboxes = body.querySelectorAll('input[type="checkbox"][name]');
    ...
    function updateProgressUI(progress) {
        ...
        // Update banner
        var banner = document.querySelector('.setup-progress-banner');
        if (banner) {
            var title = banner.querySelector('.setup-progress-title');
            var subtitle = banner.querySelector('.setup-progress-subtitle');
            var ringText = banner.querySelector('.ring-text');
            var ringFill = banner.querySelector('.ring-fill');
            ...
            if (progress.is_essential_complete) {
                ...
                var completed = progress.groups ? progress.groups.filter(function(g, i) {
                    return i < 4 && g.completed;
                }).length : 0;
                if (title) title.textContent = 'Store Setup: ' + completed + ' of 4 steps complete';
            }

            var pct = progress.essential_completion_percentage || 0;
    ...
    function displayErrors(errors) {
        ...
        body.querySelectorAll('.wizard-field-error').forEach(function(el) { el.remove(); });
        body.querySelectorAll('.wizard-form-error').forEach(function(el) { el.remove(); });
        ...
        Object.keys(errors).forEach(function(fieldName) {
            ...
            var input = body.querySelector('[name="' + fieldName + '"]');
            if (input) {
    ...
    function initTimezoneDisplay() {
        ...
        var el = document.getElementById('tz-user-time');
        if (!el) return;
