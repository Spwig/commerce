...
class SetupProgress(models.Model):
    ...
    admin_user_configured = models.BooleanField(
        default=False, help_text=_("Admin user account has been properly configured")
    )

    site_info_completed = models.BooleanField(
        default=False, help_text=_("Basic site information (name, URL, description) completed")
    )

    contact_info_completed = models.BooleanField(
        default=False, help_text=_("Contact information (emails, phone) completed")
    )

    business_address_completed = models.BooleanField(
        default=False, help_text=_("Business address information completed")
    )

    currency_locale_completed = models.BooleanField(
        default=False, help_text=_("Currency and locale settings configured")
    )

    payment_methods_configured = models.BooleanField(
        default=False, help_text=_("Payment provider setup has been acknowledged")
    )

    ecommerce_settings_completed = models.BooleanField(
        default=False, help_text=_("Core e-commerce settings have been configured")
    )

    # Optional setup completion flags
    favicon_uploaded = models.BooleanField(
        default=False, help_text=_("Site favicon has been uploaded")
    )

    seo_settings_completed = models.BooleanField(
        default=False, help_text=_("SEO meta tags have been configured")
    )

    social_media_completed = models.BooleanField(
        default=False, help_text=_("Social media links have been added")
    )

    email_settings_completed = models.BooleanField(
        default=False, help_text=_("Email notification settings configured")
    )

    # Setup metadata
    setup_started_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When the setup wizard was first started")
    )

    setup_completed_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When the essential setup was completed")
    )
    ...
    last_step_completed = models.CharField(
        max_length=50, blank=True, help_text=_("Last setup step that was completed")
    )

    wizard_skipped = models.BooleanField(
        default=False, help_text=_("User chose to skip the setup wizard")
    )
