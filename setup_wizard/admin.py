...
class SetupProgressAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            'all': ('setup_wizard/admin/css/admin_progress.css',)
        }
    ...
    fieldsets = (
        ...
        (_('Setup Timeline'), {
            'fields': (
                'setup_started_at', 'setup_started_by',
    ...
    def get_completion_status(self, obj):
        ...
        return format_html(
            '<span class="setup-progress-status {}">{} {}</span>',
            css_class, icon, text
        )
    ...
    def completion_summary(self, obj):
        ...
        if obj.is_setup_complete():
            ...
            status_desc = _('Your store is ready to start selling. Optional items can be completed anytime.')
        else:
        ...
        remaining = len(obj.get_incomplete_essential_items()) + len(obj.get_incomplete_optional_items())

        essential_class = 'progress-complete' if obj.is_essential_setup_complete() else 'progress-incomplete'
        action_text = _('Review Setup') if obj.is_essential_setup_complete() else _('Complete Setup')
    ...
    def response_change(self, request, obj):
        ...
        self.message_user(
            request,
            _("Setup progress reviewed. Current completion: {completion}%").format(completion=completion)
        )
        return response
