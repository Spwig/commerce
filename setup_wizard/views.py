...
WIZARD_GROUPS = [
    ...
    },
    {
        ...
        "form_class": None,
        "progress_fields": [
            "ecommerce_settings_completed",
            "email_settings_completed",
            "seo_settings_completed",
            "social_media_completed",
        ],
...
def get_wizard_groups(progress):
    ...
    for group in WIZARD_GROUPS:
        completed = all(getattr(progress, field, False) for field in group["progress_fields"])
        groups.append(
            {
                "key": group["key"],
...
def wizard_skip(request):
    ...
    if request.method == "POST":
        ...
        messages.warning(
            request, _("Setup wizard skipped. You can complete setup anytime from the dashboard.")
        )
...
def api_step_content(request, group_key):
    ...
    if group_key == "store":
        ...
        context["ecommerce_form"] = EcommerceSettingsForm(
            initial={
        ...
        context["email_form"] = EmailSettingsForm(
            initial={
        ...
        context["seo_form"] = SEOSettingsForm(
            initial={
...
def get_step_initial_data(step_key, site_settings):
    ...
    if step_key == "site_info":  # noqa: SIM116 — dict lookup would obscure the field mappings
        return {
            "site_name": site_settings.site_name,
            "site_url": site_settings.site_url,
            "site_description": site_settings.site_description,
            ...
            if admin_email:
                try:
                    ...
                    if builtin_account:
                        ...
                        site_name = SiteSettings.objects.values_list("site_name", flat=True).get(
                            pk=1
                        )
                        if site_name:
...
def create_default_inventory_structure(cleaned_data, site_settings):
    ...
    try:
        ...
        if not created and country not in region.countries:
